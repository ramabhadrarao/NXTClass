 ...
 * a NXTClass post. Mailbox connection information must be
...
if ( ! apply_filters( 'enable_post_by_email_configuration', true ) )
	nxt_die( __( 'This action has been disabled by the administrator.' ) );

/** Allow a plugin to do a complete takeover of Post by Email **/
do_action('nxt-mail.php');

/** Get the POP3 class with which to access the mailbox. */
require_once( ABSPATH . nxtINC . '/class-pop3.php' );

/** Only check at this interval for new messages. */
if ( !defined('nxt_MAIL_INTERVAL') )
	define('nxt_MAIL_INTERVAL', 300); // 5 minutes
...
for ( $i = 1; $i <= $count; $i++ ) {
		...
		if ( $bodysignal ) {
			...
			if (preg_match('/Subject: /i', $line)) {
				...
				if ( function_exists('iconv_mime_decode') ) {
					...
					$subject = nxt_iso_descrambler($subject);
				}
	...
	$content = apply_filters('nxt_mail_original_content', $content);
