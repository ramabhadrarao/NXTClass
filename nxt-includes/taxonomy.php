...
function create_initial_taxonomies() {
	...
	register_taxonomy( 'category', 'post', array(
		...
		'rewrite' => did_action( 'init' ) ? array(
					...
					'with_front' => ( get_option('category_base') && ! $nxt_rewrite->using_index_permalinks() ) ? false : true ) : false,
		'public' => true,
		'show_ui' => true,
		'_builtin' => true,
	) );
	...
	register_taxonomy( 'post_tag', 'post', array(
		...
		'rewrite' => did_action( 'init' ) ? array(
					...
					'with_front' => ( get_option('tag_base') && ! $nxt_rewrite->using_index_permalinks() ) ? false : true ) : false,
		'public' => true,
		'show_ui' => true,
		'_builtin' => true,
	) );
...
function get_taxonomies( $args = array(), $output = 'names', $operator = 'and' ) {
	global $nxt_taxonomies;

	$field = ('names' == $output) ? 'name' : false;

	return nxt_filter_object_list($nxt_taxonomies, $args, $operator, $field);
}
 ...
 * $nxt_taxonomies global variable.
 *
...
function get_object_taxonomies($object, $output = 'names') {
	...
	$taxonomies = array();
	foreach ( (array) $nxt_taxonomies as $tax_name => $tax_obj ) {
...
function get_taxonomy( $taxonomy ) {
	...
	return $nxt_taxonomies[$taxonomy];
}
...
function taxonomy_exists( $taxonomy ) {
	...
	return isset( $nxt_taxonomies[$taxonomy] );
}
 ...
 * defaults to true.
 *
 * show_ui - If the NXTClass UI admin tags UI should apply to this taxonomy;
 * defaults to public.
 *
 ...
 * @subpackage Taxonomy
 * @since 2.3.0
 * @uses $nxt_taxonomies Inserts new taxonomy object into the list
 * @uses $nxt_rewrite Adds rewrite tags and permastructs
 * @uses $nxt Adds query vars
 *
 * @param string $taxonomy Name of taxonomy object
...
function register_taxonomy( $taxonomy, $object_type, $args = array() ) {
	global $nxt_taxonomies, $nxt_rewrite, $nxt;

	if ( ! is_array($nxt_taxonomies) )
		$nxt_taxonomies = array();
	...
	if ( false !== $args['rewrite'] && '' != get_option('permalink_structure') ) {
		$args['rewrite'] = nxt_parse_args($args['rewrite'], array(
		...
		$nxt_rewrite->add_rewrite_tag("%$taxonomy%", $tag, $args['query_var'] ? "{$args['query_var']}=" : "taxonomy=$taxonomy&term=");
		$nxt_rewrite->add_permastruct($taxonomy, "{$args['rewrite']['slug']}/%$taxonomy%", $args['rewrite']['with_front']);
	}
	...
	$nxt_taxonomies[$taxonomy] = (object) $args;

	// register callback handling for metabox
 	add_filter('nxt_ajax_add-' . $taxonomy, '_nxt_ajax_add_hierarchical_term');
 ...
 * @subpackage Taxonomy
 * @since 3.0.0
 * @uses $nxt_taxonomies Modifies taxonomy object
 *
 * @param string $taxonomy Name of taxonomy object
...
function register_taxonomy_for_object_type( $taxonomy, $object_type) {
	...
	if ( !isset($nxt_taxonomies[$taxonomy]) )
		return false;

	if ( ! get_post_type_object($object_type) )
		return false;

	if ( ! in_array( $object_type, $nxt_taxonomies[$taxonomy]->object_type ) )
		$nxt_taxonomies[$taxonomy]->object_type[] = $object_type;
 ...
 * failure of finding a valid taxonomy, it will return an nxt_Error class, kind
 * of like Exceptions in PHP 5, except you can't catch them. Even so, you can
 * still test for the nxt_Error class and get the error message.
 *
 ...
 *
 * @uses $nxtdb
 * @uses nxt_parse_args() Creates an array from string $args.
 *
 * @param int|array $term_ids Term id or array of term ids of terms that will be used
 * @param string|array $taxonomies String of taxonomy name or Array of string values of taxonomy names
 * @param array|string $args Change the order of the object_ids, either ASC or DESC
 * @return nxt_Error|array If the taxonomy does not exist, then nxt_Error will be returned. On success
...
function get_objects_in_term( $term_ids, $taxonomies, $args = array() ) {
	...
	$object_ids = $nxtdb->get_col("SELECT tr.object_id FROM $nxtdb->term_relationships AS tr INNER JOIN $nxtdb->term_taxonomy AS tt ON tr.term_taxonomy_id = tt.term_taxonomy_id WHERE tt.taxonomy IN ($taxonomies) AND tt.term_id IN ($term_ids) ORDER BY tr.object_id $order");
...
function get_tax_sql( $tax_query, $primary_table, $primary_id_column ) {
	$tax_query_obj = new nxt_Tax_Query( $tax_query );
...
class nxt_Tax_Query {
	...
	public function get_sql( $primary_table, $primary_id_column ) {
		...
		foreach ( $this->queries as $query ) {
			...
			if ( 'IN' == $operator ) {
				...
				$alias = $i ? 'tt' . $i : $nxtdb->term_relationships;

				$join .= " INNER JOIN $nxtdb->term_relationships";
				$join .= $i ? " AS $alias" : '';
	...
	private function clean_query( &$query ) {
		if ( ! taxonomy_exists( $query['taxonomy'] ) ) {
			$query = new nxt_Error( 'Invalid taxonomy' );
			return;
		}
	...
	private function transform_query( &$query, $resulting_field ) {
		...
		switch ( $query['field'] ) {
			...
			case 'name':
				...
				$terms = $nxtdb->get_col( "
					SELECT $nxtdb->term_taxonomy.$resulting_field
					FROM $nxtdb->term_taxonomy
					INNER JOIN $nxtdb->terms USING (term_id)
					WHERE taxonomy = '{$query['taxonomy']}'
					AND $nxtdb->terms.{$query['field']} IN ($terms)
				" );
				break;
		...
		if ( 'AND' == $query['operator'] && count( $terms ) < count( $query['terms'] ) ) {
			$query = new nxt_Error( 'Inexistent terms' );
			return;
		}
 ...
 * be able to be captured by the hooks. Failure would be the same value as $nxtdb
 ...
 * @return mixed|null|nxt_Error Term Row from database. Will return null if $term is empty. If taxonomy does not
 * exist then nxt_Error will be returned.
 */
function &get_term($term, $taxonomy, $output = OBJECT, $filter = 'raw') {
	...
	if ( empty($term) ) {
		$error = new nxt_Error('invalid_term', __('Empty Term'));
		return $error;
	}
	...
	if ( is_object($term) && empty($term->filter) ) {
		...
		if ( !$term = (int) $term )
			return $null;
		if ( ! $_term = nxt_cache_get($term, $taxonomy) ) {
			$_term = $nxtdb->get_row( $nxtdb->prepare( "SELECT t.*, tt.* FROM $nxtdb->terms AS t INNER JOIN $nxtdb->term_taxonomy AS tt ON t.term_id = tt.term_id WHERE tt.taxonomy = %s AND t.term_id = %d LIMIT 1", $taxonomy, $term) );
			if ( ! $_term )
				return $null;
			nxt_cache_add($term, $_term, $taxonomy);
		}
	}
...
function get_term_by($field, $value, $taxonomy, $output = OBJECT, $filter = 'raw') {
	...
	$term = $nxtdb->get_row( $nxtdb->prepare( "SELECT t.*, tt.* FROM $nxtdb->terms AS t INNER JOIN $nxtdb->term_taxonomy AS tt ON t.term_id = tt.term_id WHERE tt.taxonomy = %s AND $field = %s LIMIT 1", $taxonomy, $value) );
	if ( !$term )
		return false;
 ...
 * @param string $taxonomy Taxonomy Name
 * @return array|nxt_Error List of Term Objects. nxt_Error returned if $taxonomy does not exist
 */
 ...
 * @param string $taxonomy Taxonomy Name
 * @return mixed|null|nxt_Error Will return empty string if $term is not an object.
 */
...
function &get_terms($taxonomies, $args = '') {
	...
	if ( empty($terms) ) {
		nxt_cache_add( $cache_key, array(), 'terms', 86400 ); // one day
	...
	nxt_cache_add( $cache_key, $terms, 'terms', 86400 ); // one day
...
function term_exists($term, $taxonomy = '', $parent = 0) {
	...
	$select = "SELECT term_id FROM $nxtdb->terms as t WHERE ";
	$tax_select = "SELECT tt.term_id, tt.term_taxonomy_id FROM $nxtdb->terms AS t INNER JOIN $nxtdb->term_taxonomy as tt ON tt.term_id = t.term_id WHERE ";
	...
	if ( is_int($term) ) {
		...
		$where = 't.term_id = %d';
		if ( !empty($taxonomy) )
			return $nxtdb->get_row( $nxtdb->prepare( $tax_select . $where . " AND tt.taxonomy = %s", $term, $taxonomy ), ARRAY_A );
		else
			return $nxtdb->get_var( $nxtdb->prepare( $select . $where, $term ) );
	}
	...
	if ( !empty($taxonomy) ) {
		...
		if ( $result = $nxtdb->get_row( $nxtdb->prepare("SELECT tt.term_id, tt.term_taxonomy_id FROM $nxtdb->terms AS t INNER JOIN $nxtdb->term_taxonomy as tt ON tt.term_id = t.term_id WHERE $where AND tt.taxonomy = %s", $where_fields), ARRAY_A) )
			return $result;

		return $nxtdb->get_row( $nxtdb->prepare("SELECT tt.term_id, tt.term_taxonomy_id FROM $nxtdb->terms AS t INNER JOIN $nxtdb->term_taxonomy as tt ON tt.term_id = t.term_id WHERE $else_where AND tt.taxonomy = %s", $else_where_fields), ARRAY_A);
	}

	if ( $result = $nxtdb->get_var( $nxtdb->prepare("SELECT term_id FROM $nxtdb->terms as t WHERE $where", $where_fields) ) )
		return $result;

	return $nxtdb->get_var( $nxtdb->prepare("SELECT term_id FROM $nxtdb->terms as t WHERE $else_where", $else_where_fields) );
}
 ...
 *
 * @uses get_terms()
 * @uses nxt_parse_args() Turns strings into arrays and merges defaults into an array.
 *
 * @param string $taxonomy Taxonomy name
 * @param array|string $args Overwrite defaults. See get_terms()
 * @return int How many terms are in $taxonomy
 */
function nxt_count_terms( $taxonomy, $args = array() ) {
 ...
 */
function nxt_delete_object_term_relationships( $object_id, $taxonomies ) {
	global $nxtdb;
	...
	foreach ( (array) $taxonomies as $taxonomy ) {
		$tt_ids = nxt_get_object_terms($object_id, $taxonomy, array('fields' => 'tt_ids'));
		$in_tt_ids = "'" . implode("', '", $tt_ids) . "'";
		do_action( 'delete_term_relationships', $object_id, $tt_ids );
		$nxtdb->query( $nxtdb->prepare("DELETE FROM $nxtdb->term_relationships WHERE object_id = %d AND term_taxonomy_id IN ($in_tt_ids)", $object_id) );
 ...
 * @return array|nxt_Error The requested term data or empty array if no terms found. nxt_Error if $taxonomy does not exist.
 */
function nxt_get_object_terms($object_ids, $taxonomies, $args = array()) {
	global $nxtdb;
	...
	if ( count($taxonomies) > 1 ) {
		foreach ( $taxonomies as $index => $taxonomy ) {
			...
			if ( isset($t->args) && is_array($t->args) && $args != array_merge($args, $t->args) ) {
				...
				$terms = array_merge($terms, nxt_get_object_terms($object_ids, $taxonomy, array_merge($args, $t->args)));
			}
		}
	} else {
	...
	$query = "SELECT $select_this FROM $nxtdb->terms AS t INNER JOIN $nxtdb->term_taxonomy AS tt ON tt.term_id = t.term_id INNER JOIN $nxtdb->term_relationships AS tr ON tr.term_taxonomy_id = tt.term_taxonomy_id WHERE tt.taxonomy IN ($taxonomies) AND tr.object_id IN ($object_ids) $orderby $order";

	if ( 'all' == $fields || 'all_with_object_id' == $fields ) {
		$terms = array_merge($terms, $nxtdb->get_results($query));
		update_term_cache($terms);
	} else if ( 'ids' == $fields || 'names' == $fields || 'slugs' == $fields ) {
		$terms = array_merge($terms, $nxtdb->get_col($query));
	} else if ( 'tt_ids' == $fields ) {
		$terms = $nxtdb->get_col("SELECT tr.term_taxonomy_id FROM $nxtdb->term_relationships AS tr INNER JOIN $nxtdb->term_taxonomy AS tt ON tr.term_taxonomy_id = tt.term_taxonomy_id WHERE tr.object_id IN ($object_ids) AND tt.taxonomy IN ($taxonomies) $orderby $order");
	}

	if ( ! $terms )
		$terms = array();

	return apply_filters('nxt_get_object_terms', $terms, $object_ids, $taxonomies, $args);
}
 ...
 * @return array|nxt_Error The Term ID and Term Taxonomy ID
 */
function nxt_insert_term( $term, $taxonomy, $args = array() ) {
	global $nxtdb;
	...
	if ( is_int($term) && 0 == $term )
		return new nxt_Error('invalid_term_id', __('Invalid term ID'));
	...
	$term_group = 0;
	if ( $alias_of ) {
		$alias = $nxtdb->get_row( $nxtdb->prepare( "SELECT term_id, term_group FROM $nxtdb->terms WHERE slug = %s", $alias_of) );
		if ( $alias->term_group ) {
			// The alias we want is already in a group, so let's use that one.
			$term_group = $alias->term_group;
		} else {
			// The alias isn't in a group, so let's create a new one and firstly add the alias term to it.
			$term_group = $nxtdb->get_var("SELECT MAX(term_group) FROM $nxtdb->terms") + 1;
			do_action( 'edit_terms', $alias->term_id );
			$nxtdb->update($nxtdb->terms, compact('term_group'), array('term_id' => $alias->term_id) );
			do_action( 'edited_terms', $alias->term_id );
		}
	}

	if ( $term_id = term_exists($slug) ) {
		$existing_term = $nxtdb->get_row( $nxtdb->prepare( "SELECT name FROM $nxtdb->terms WHERE term_id = %d", $term_id), ARRAY_A );
		...
		if ( is_taxonomy_hierarchical($taxonomy) && $existing_term['name'] == $name && $exists = term_exists( (int) $term_id, $taxonomy ) ) {
			...
			if ( in_array($name, $siblings) ) {
				return new nxt_Error('term_exists', __('A term with the name provided already exists with this parent.'), $exists['term_id']);
			} else {
				$slug = nxt_unique_term_slug($slug, (object) $args);
				if ( false === $nxtdb->insert( $nxtdb->terms, compact( 'name', 'slug', 'term_group' ) ) )
					return new nxt_Error('db_insert_error', __('Could not insert term into the database'), $nxtdb->last_error);
			...
			// Same name, same slug.
			return new nxt_Error('term_exists', __('A term with the name provided already exists.'), $exists['term_id']);
		}
	} else {
	...
	if ( empty($slug) ) {
		...
		$nxtdb->update( $nxtdb->terms, compact( 'slug' ), compact( 'term_id' ) );
		do_action( 'edited_terms', $term_id );
	}

	$tt_id = $nxtdb->get_var( $nxtdb->prepare( "SELECT tt.term_taxonomy_id FROM $nxtdb->term_taxonomy AS tt INNER JOIN $nxtdb->terms AS t ON tt.term_id = t.term_id WHERE tt.taxonomy = %s AND t.term_id = %d", $taxonomy, $term_id ) );

	if ( !empty($tt_id) )
		return array('term_id' => $term_id, 'term_taxonomy_id' => $tt_id);

	$nxtdb->insert( $nxtdb->term_taxonomy, compact( 'term_id', 'taxonomy', 'description', 'parent') + array( 'count' => 0 ) );
	$tt_id = (int) $nxtdb->insert_id;
 ...
 */
function nxt_unique_term_slug($slug, $term) {
	global $nxtdb;
	...
	if ( is_taxonomy_hierarchical($term->taxonomy) && !empty($term->parent) ) {
		...
		while ( ! empty($the_parent) ) {
			...
			if ( is_nxt_error($parent_term) || empty($parent_term) )
				break;
	...
	if ( !empty($args['term_id']) )
		$query = $nxtdb->prepare( "SELECT slug FROM $nxtdb->terms WHERE slug = %s AND term_id != %d", $slug, $args['term_id'] );
	else
		$query = $nxtdb->prepare( "SELECT slug FROM $nxtdb->terms WHERE slug = %s", $slug );

	if ( $nxtdb->get_var( $query ) ) {
		$num = 2;
		do {
			$alt_slug = $slug . "-$num";
			$num++;
 ...
 */
function nxt_update_term( $term_id, $taxonomy, $args = array() ) {
	global $nxtdb;
	...
	$parent = apply_filters( 'nxt_update_term_parent', $parent, $term_id, $taxonomy, compact( array_keys( $args ) ), $args );

	// Check for duplicate slug
	$id = $nxtdb->get_var( $nxtdb->prepare( "SELECT term_id FROM $nxtdb->terms WHERE slug = %s", $slug ) );
	if ( $id && ($id != $term_id) ) {
	...
	do_action( 'edit_terms', $term_id );
	$nxtdb->update($nxtdb->terms, compact( 'name', 'slug', 'term_group' ), compact( 'term_id' ) );
	if ( empty($slug) ) {
	...
	$nxtdb->update( $nxtdb->term_taxonomy, compact( 'term_id', 'taxonomy', 'description', 'parent' ), array( 'term_taxonomy_id' => $tt_id ) );
 ...
 */
function nxt_update_term_count_now( $terms, $taxonomy ) {
	global $nxtdb;
...
function clean_object_term_cache($object_ids, $object_type) {
	...
	foreach ( $object_ids as $id )
		foreach ( get_object_taxonomies($object_type) as $taxonomy )
			nxt_cache_delete($id, "{$taxonomy}_relationships");
 ...
 * @since 2.3.0
 *
 * @uses nxt_cache_get() Retrieves taxonomy relationship from cache
 *
...
function &get_object_term_cache($id, $taxonomy) {
	$cache = nxt_cache_get($id, "{$taxonomy}_relationships");
	return $cache;
}
 ...
 * @subpackage Taxonomy
 * @since 2.3.0
 * @uses nxt_get_object_terms() Used to get terms from the database to update
 *
...
function update_object_term_cache($object_ids, $object_type) {
	...
	$terms = nxt_get_object_terms($ids, $taxonomies, array('fields' => 'all_with_object_id'));
...
function _pad_term_counts(&$terms, $taxonomy) {
	...
	$results = $nxtdb->get_results("SELECT object_id, term_taxonomy_id FROM $nxtdb->term_relationships INNER JOIN $nxtdb->posts ON object_id = ID WHERE term_taxonomy_id IN (" . implode(',', array_keys($term_ids)) . ") AND post_type IN ('" . implode("', '", $object_types) . "') AND post_status = 'publish'");
...
function _update_post_term_count( $terms, $taxonomy ) {
	...
	foreach ( (array) $terms as $term ) {
		...
		if ( $check_attachments )
			$count += (int) $nxtdb->get_var( $nxtdb->prepare( "SELECT COUNT(*) FROM $nxtdb->term_relationships, $nxtdb->posts p1 WHERE p1.ID = $nxtdb->term_relationships.object_id AND ( post_status = 'publish' OR ( post_status = 'inherit' AND post_parent > 0 AND ( SELECT post_status FROM $nxtdb->posts WHERE ID = p1.post_parent ) = 'publish' ) ) AND post_type = 'attachment' AND term_taxonomy_id = %d", $term ) );

		if ( $object_types )
			$count += (int) $nxtdb->get_var( $nxtdb->prepare( "SELECT COUNT(*) FROM $nxtdb->term_relationships, $nxtdb->posts WHERE $nxtdb->posts.ID = $nxtdb->term_relationships.object_id AND post_status = 'publish' AND post_type IN ('" . implode("', '", $object_types ) . "') AND term_taxonomy_id = %d", $term ) );


		do_action( 'edit_term_taxonomy', $term, $taxonomy );
		$nxtdb->update( $nxtdb->term_taxonomy, compact( 'count' ), array( 'term_taxonomy_id' => $term ) );
...
function _update_generic_term_count( $terms, $taxonomy ) {
	...
	foreach ( (array) $terms as $term ) {
		$count = $nxtdb->get_var( $nxtdb->prepare( "SELECT COUNT(*) FROM $nxtdb->term_relationships WHERE term_taxonomy_id = %d", $term ) );
 ...
 * @return string|nxt_Error HTML link to taxonomy term archive on success, nxt_Error if term does not exist.
 */
function get_term_link( $term, $taxonomy = '') {
	...
	if ( !is_object($term) )
		$term = new nxt_Error('invalid_term', __('Empty Term'));
	...
	$termlink = $nxt_rewrite->get_extra_permastruct($taxonomy);
...
function get_the_taxonomies($post = 0, $args = array() ) {
	...
	$args = nxt_parse_args( $args, array(
		'template' => '%s: %l.',
	) );
	extract( $args, EXTR_SKIP );
...
function get_ancestors($object_id = 0, $object_type = '') {
	...
	if ( is_taxonomy_hierarchical( $object_type ) ) {
		...
		while ( ! is_nxt_error($term) && ! empty( $term->parent ) && ! in_array( $term->parent, $ancestors ) ) {
			$ancestors[] = (int) $term->parent;
			$term = get_term($term->parent, $object_type);
		}
	} elseif ( null !== get_post_type_object( $object_type ) ) {
		$object = get_post($object_id);
		if ( ! is_nxt_error( $object ) && isset( $object->ancestors ) && is_array( $object->ancestors ) )
			$ancestors = $object->ancestors;
		else {
			while ( ! is_nxt_error($object) && ! empty( $object->post_parent ) && ! in_array( $object->post_parent, $ancestors ) ) {
 ...
 */
function nxt_get_term_taxonomy_parent_id( $term_id, $taxonomy ) {
