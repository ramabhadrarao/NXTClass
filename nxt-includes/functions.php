 ...
 * and if the $nxt_locale object has the month and weekday set.
 *
 * @since 0.71
 *
...
function date_i18n( $dateformatstring, $unixtimestamp = false, $gmt = false ) {
	...
	if ( ( !empty( $nxt_locale->month ) ) && ( !empty( $nxt_locale->weekday ) ) ) {
		$datemonth = $nxt_locale->get_month( $datefunc( 'm', $i ) );
		$datemonth_abbrev = $nxt_locale->get_month_abbrev( $datemonth );
		$dateweekday = $nxt_locale->get_weekday( $datefunc( 'w', $i ) );
		$dateweekday_abbrev = $nxt_locale->get_weekday_abbrev( $dateweekday );
		$datemeridiem = $nxt_locale->get_meridiem( $datefunc( 'a', $i ) );
		$datemeridiem_capital = $nxt_locale->get_meridiem( $datefunc( 'A', $i ) );
...
function number_format_i18n( $number, $decimals = 0 ) {
	global $nxt_locale;
	$formatted = number_format( $number, absint( $decimals ), $nxt_locale->number_format['decimal_point'], $nxt_locale->number_format['thousands_sep'] );
...
function get_option( $option, $default = false ) {
	...
	if ( ! defined( 'nxt_INSTALLING' ) ) {
		...
		if ( isset( $alloptions[$option] ) ) {
			...
			$value = nxt_cache_get( $option, 'options' );

			if ( false === $value ) {
				$row = $nxtdb->get_row( $nxtdb->prepare( "SELECT option_value FROM $nxtdb->options WHERE option_name = %s LIMIT 1", $option ) );

				// Has to be get_row instead of get_var because of funkiness with 0, false, null values
				if ( is_object( $row ) ) {
					$value = $row->option_value;
					nxt_cache_add( $option, $value, 'options' );
				} else { // option does not exist, so we must cache its non-existence
					$notoptions[$option] = true;
					nxt_cache_set( 'notoptions', $notoptions, 'options' );
					return $default;
				}
			}
		}
...
/**
 * Protect NXTClass special option from being modified.
 *
 ...
 */
function nxt_protect_special_option( $option ) {
	$protected = array( 'alloptions', 'notoptions' );
	if ( in_array( $option, $protected ) )
		nxt_die( sprintf( __( '%s is a protected nxt option and may not be modified' ), esc_html( $option ) ) );
}
 ...
 */
function nxt_load_core_site_options( $site_id = null ) {
	global $nxtdb, $_nxt_using_ext_object_cache;

	if ( !is_multisite() || $_nxt_using_ext_object_cache || defined( 'nxt_INSTALLING' ) )
		return;

	if ( empty($site_id) )
		$site_id = $nxtdb->siteid;
	...
	$options = $nxtdb->get_results( $nxtdb->prepare("SELECT meta_key, meta_value FROM $nxtdb->sitemeta WHERE meta_key IN ($core_options_in) AND site_id = %d", $site_id) );
	...
	foreach ( $options as $option ) {
		...
		nxt_cache_set( $cache_key, $option->meta_value, 'site-options' );
	}
}
...
function update_option( $option, $newvalue ) {
	...
	nxt_protect_special_option( $option );
	...
	if ( ! defined( 'nxt_INSTALLING' ) ) {
		...
		if ( isset( $alloptions[$option] ) ) {
			...
			nxt_cache_set( 'alloptions', $alloptions, 'options' );
		} else {
			nxt_cache_set( $option, $_newvalue, 'options' );
		}
	}

	$result = $nxtdb->update( $nxtdb->options, array( 'option_value' => $newvalue ), array( 'option_name' => $option ) );
 ...
 * aren't adding a protected NXTClass option. Care should be taken to not name
 ...
 * @param bool $autoload Optional. Default is enabled. Whether to load the option when NXTClass starts up.
...
function add_option( $option, $value = '', $deprecated = '', $autoload = 'yes' ) {
	...
	if ( ! defined( 'nxt_INSTALLING' ) ) {
		if ( 'yes' == $autoload ) {
			...
			nxt_cache_set( $option, $value, 'options' );
		}
	}

	// This option exists now
	$notoptions = nxt_cache_get( 'notoptions', 'options' ); // yes, again... we need it to be fresh
	if ( is_array( $notoptions ) && isset( $notoptions[$option] ) ) {
		unset( $notoptions[$option] );
		nxt_cache_set( 'notoptions', $notoptions, 'options' );
	}

	$result = $nxtdb->query( $nxtdb->prepare( "INSERT INTO `$nxtdb->options` (`option_name`, `option_value`, `autoload`) VALUES (%s, %s, %s) ON DUPLICATE KEY UPDATE `option_name` = VALUES(`option_name`), `option_value` = VALUES(`option_value`), `autoload` = VALUES(`autoload`)", $option, $value, $autoload ) );
...
/**
 * Removes option by name. Prevents removal of protected NXTClass options.
 *
 * @package NXTClass
 * @subpackage Option
 * @since 1.2.0
...
function delete_option( $option ) {
	...
	$row = $nxtdb->get_row( $nxtdb->prepare( "SELECT autoload FROM $nxtdb->options WHERE option_name = %s", $option ) );
	if ( is_null( $row ) )
		return false;
	do_action( 'delete_option', $option );
	$result = $nxtdb->query( $nxtdb->prepare( "DELETE FROM $nxtdb->options WHERE option_name = %s", $option) );
	if ( ! defined( 'nxt_INSTALLING' ) ) {
		if ( 'yes' == $row->autoload ) {
			$alloptions = nxt_load_alloptions();
			...
			nxt_cache_delete( $option, 'options' );
		}
	}
	if ( $result ) {
...
function delete_transient( $transient ) {
	...
	if ( $_nxt_using_ext_object_cache ) {
		$result = nxt_cache_delete( $transient, 'transient' );
	} else {
...
function set_transient( $transient, $value, $expiration = 0 ) {
	...
	if ( $_nxt_using_ext_object_cache ) {
		$result = nxt_cache_set( $transient, $value, 'transient', $expiration );
	} else {
		$transient_timeout = '_transient_timeout_' . $transient;
...
function nxt_user_settings() {
	...
	if ( isset( $_COOKIE['nxt-settings-' . $user->ID] ) ) {
		$cookie = preg_replace( '/[^A-Za-z0-9=&_]/', '', $_COOKIE['nxt-settings-' . $user->ID] );
		...
		if ( ! empty( $cookie ) && strpos( $cookie, '=' ) ) {
			...
			$saved = isset( $_COOKIE['nxt-settings-time-' . $user->ID]) ? preg_replace( '/[^0-9]/', '', $_COOKIE['nxt-settings-time-' . $user->ID] ) : 0;
	...
	setcookie( 'nxt-settings-' . $user->ID, $settings, time() + 31536000, SITECOOKIEPATH );
	setcookie( 'nxt-settings-time-' . $user->ID, time(), time() + 31536000, SITECOOKIEPATH );
	$_COOKIE['nxt-settings-' . $user->ID] = $settings;
}
...
function set_user_setting( $name, $value ) {
	...
	return nxt_set_all_user_settings($all);
}
 ...
 * @return bool
 */
function nxt_set_all_user_settings($all) {
	global $_updated_user_settings;
...
function do_enclose( $content, $post_ID ) {
	...
	foreach ( $pung as $link_test ) {
		if ( !in_array( $link_test, $post_links_temp[0] ) ) { // link no longer in post
			$mid = $nxtdb->get_col( $nxtdb->prepare("SELECT meta_id FROM $nxtdb->postmeta WHERE post_id = %d AND meta_key = 'enclosure' AND meta_value LIKE (%s)", $post_ID, like_escape( $link_test ) . '%') );
			do_action( 'delete_postmeta', $mid );
			$nxtdb->query( $nxtdb->prepare("DELETE FROM $nxtdb->postmeta WHERE meta_id IN(%s)", implode( ',', $mid ) ) );
	...
	foreach ( (array) $post_links as $url ) {
		if ( $url != '' && !$nxtdb->get_var( $nxtdb->prepare( "SELECT post_id FROM $nxtdb->postmeta WHERE post_id = %d AND meta_key = 'enclosure' AND meta_value LIKE (%s)", $post_ID, like_escape( $url ) . '%' ) ) ) {

			if ( $headers = nxt_get_http_headers( $url) ) {
				...
				if ( in_array( substr( $type, 0, strpos( $type, "/" ) ), $allowed_types ) ) {
					$meta_value = "$url\n$len\n$type\n";
					$nxtdb->insert($nxtdb->postmeta, array('post_id' => $post_ID, 'meta_key' => 'enclosure', 'meta_value' => $meta_value) );
					do_action( 'added_postmeta', $nxtdb->insert_id, $post_ID, 'enclosure', $meta_value );
				}
			}
		}
	}
...
/**
 * Set up the NXTClass query.
 *
 * @since 2.0.0
 *
 * @param string $query_vars Default nxt_Query arguments.
 */
function nxt( $query_vars = '' ) {
	global $nxt, $nxt_query, $nxt_the_query;
	$nxt->main( $query_vars );

	if ( !isset($nxt_the_query) )
		$nxt_the_query = $nxt_query;
}
...
function get_status_header_desc( $code ) {
	global $nxt_header_to_desc;

	$code = absint( $code );

	if ( !isset( $nxt_header_to_desc ) ) {
		$nxt_header_to_desc = array(
			100 => 'Continue',
	...
	if ( isset( $nxt_header_to_desc[$code] ) )
		return $nxt_header_to_desc[$code];
	else
		return '';
}
 ...
 */
function nxt_get_nocache_headers() {
	$headers = array(
 ...
 *
 * @since 2.0.0
 * @uses nxt_get_nocache_headers()
 */
function nocache_headers() {
	$headers = nxt_get_nocache_headers();
...
/**
 * Retrieve the number of database queries during the NXTClass execution.
 *
 * @since 2.0.0
 *
 ...
 *
 * @since 2.1.0
 * @uses $nxt_query Used to tell if the use a comment feed.
...
function do_feed() {
	...
	if ( !has_action($hook) ) {
		...
		nxt_die( $message, '', array( 'response' => 404 ) );
	}

	do_action( $hook, $nxt_query->is_comment_feed );
}
...
function do_feed_rdf() {
	load_template( ABSPATH . nxtINC . '/feed-rdf.php' );
}
...
function do_feed_rss() {
	load_template( ABSPATH . nxtINC . '/feed-rss.php' );
}
...
function do_feed_rss2( $for_comments ) {
	if ( $for_comments )
		load_template( ABSPATH . nxtINC . '/feed-rss2-comments.php' );
	else
		load_template( ABSPATH . nxtINC . '/feed-rss2.php' );
}
...
function do_feed_atom( $for_comments ) {
	if ($for_comments)
		load_template( ABSPATH . nxtINC . '/feed-atom-comments.php');
	else
		load_template( ABSPATH . nxtINC . '/feed-atom.php' );
}
...
function do_robots() {
	...
	if ( '0' == $public ) {
		...
		$output .= "Disallow: $path/nxt-admin/\n";
		$output .= "Disallow: $path/nxt-includes/\n";
	}
 ...
 * cache values, then this will work. If you use the default NXTClass cache,
 * and the database goes away, then you might have problems.
 *
 * Checks for the option siteurl for whether NXTClass is installed.
 *
 * @since 2.1.0
 * @uses $nxtdb
 *
...
function is_blog_installed() {
	...
	if ( nxt_cache_get( 'is_blog_installed' ) )
		return true;
	...
	if ( !isset( $alloptions['siteurl'] ) )
		$installed = $nxtdb->get_var( "SELECT option_value FROM $nxtdb->options WHERE option_name = 'siteurl'" );
	else
		$installed = $alloptions['siteurl'];
	$nxtdb->suppress_errors( $suppress );

	$installed = !empty( $installed );
	nxt_cache_set( 'is_blog_installed', $installed );

	if ( $installed )
		return true;

	// If visiting repair.php, return true and let it take over.
	if ( defined( 'nxt_REPAIRING' ) )
		return true;

	$suppress = $nxtdb->suppress_errors();

	// Loop over the nxt tables.  If none exist, then scratch install is allowed.
	// If one or more exist, suggest table repair since we got here because the options
	// table could not be accessed.
	$nxt_tables = $nxtdb->tables();
	foreach ( $nxt_tables as $table ) {
		...
		if ( ! $nxtdb->get_results( "DESCRIBE $table;" ) )
			continue;

		// One or more tables exist. We are insane.

		// Die with a DB error.
		$nxtdb->error = sprintf( /*nxt_I18N_NO_TABLES*/'One or more database tables are unavailable.  The database may need to be <a href="%s">repaired</a>.'/*/nxt_I18N_NO_TABLES*/, 'maint/repair.php?referrer=is_blog_installed' );
		dead_db();
	}

	$nxtdb->suppress_errors( $suppress );

	nxt_cache_set( 'is_blog_installed', false );
 ...
 */
function nxt_nonce_url( $actionurl, $action = -1 ) {
	$actionurl = str_replace( '&amp;', '&', $actionurl );
	return esc_html( add_query_arg( '_nxtnonce', nxt_create_nonce( $action ), $actionurl ) );
}
 ...
 * input name is '_nxt_http_referer', in case you wanted to check manually.
 *
 * @package NXTClass
 * @subpackage Security
 * @since 2.0.4
 ...
 */
function nxt_referer_field( $echo = true ) {
	$ref = esc_attr( $_SERVER['REQUEST_URI'] );
	$referer_field = '<input type="hidden" name="_nxt_http_referer" value="'. $ref . '" />';
...
/**
 * Retrieve referer from '_nxt_http_referer' or HTTP referer. If it's the same
 ...
 */
function nxt_get_referer() {
	$ref = false;
	if ( ! empty( $_REQUEST['_nxt_http_referer'] ) )
		$ref = $_REQUEST['_nxt_http_referer'];
 ...
 */
function nxt_get_original_referer() {
	if ( !empty( $_REQUEST['_nxt_original_http_referer'] ) )
		return $_REQUEST['_nxt_original_http_referer'];
	return false;
}
...
function nxt_mkdir_p( $target ) {
	...
	if ( ( $target != '/' ) && ( nxt_mkdir_p( dirname( $target ) ) ) )
		return nxt_mkdir_p( $target );
 ...
 * Function's preference is to nxt_CONTENT_DIR followed by the return value of <code>sys_get_temp_dir()</code>, before finally defaulting to /tmp/
 *
 * In the event that this function does not find a writable location, It may be overridden by the <code>nxt_TEMP_DIR</code> constant in your <code>nxt-config.php</code> file.
 *
 * @since 2.5.0
 *
...
function get_temp_dir() {
	static $temp;
	if ( defined('nxt_TEMP_DIR') )
		return trailingslashit(nxt_TEMP_DIR);

	if ( $temp )
		return trailingslashit($temp);

	$temp = nxt_CONTENT_DIR . '/';
 ...
 * 'nxt_CONTENT_DIR/uploads'. If the 'UPLOADS' constant is defined, then it will
 * override the 'upload_path' option and 'nxt_CONTENT_DIR/uploads' path.
 *
 * The upload URL path is set either by the 'upload_url_path' option or by using
 * the 'nxt_CONTENT_URL' constant and appending '/uploads' to the path.
 *
 ...
 */
function nxt_upload_dir( $time = null ) {
	global $switched;
	...
	if ( empty($upload_path) ) {
		$dir = nxt_CONTENT_DIR . '/uploads';
	} else {
		$dir = $upload_path;
		if ( 'nxt-content/uploads' == $upload_path ) {
			$dir = nxt_CONTENT_DIR . '/uploads';
	...
	if ( !$url = get_option( 'upload_url_path' ) ) {
		if ( empty($upload_path) || ( 'nxt-content/uploads' == $upload_path ) || ( $upload_path == $dir ) )
			$url = nxt_CONTENT_URL . '/uploads';
		else
 ...
 */
function nxt_unique_filename( $dir, $filename, $unique_filename_callback = null ) {
 ...
 */
function nxt_ext2type( $ext ) {
	$ext2type = apply_filters( 'ext2type', array(
		'audio'       => array( 'aac', 'ac3',  'aif',  'aiff', 'm3a',  'm4a',   'm4b', 'mka', 'mp1', 'mp2',  'mp3', 'ogg', 'oga', 'ram', 'wav', 'wma' ),
		'video'       => array( 'asf', 'avi',  'divx', 'dv',   'flv',  'm4v',   'mkv', 'mov', 'mp4', 'mpeg', 'mpg', 'mpv', 'ogm', 'ogv', 'qt',  'rm', 'vob', 'wmv' ),
		'document'    => array( 'doc', 'docx', 'docm', 'dotm', 'odt',  'pages', 'pdf', 'rtf', 'nxt',  'nxtd' ),
 ...
 */
function nxt_check_filetype( $filename, $mimes = null ) {
	if ( empty($mimes) )
...
function get_allowed_mime_types() {
	...
	if ( !$mimes ) {
		...
		// wordperfect formats
		'nxt|nxtd' => 'application/wordperfect',
		) );
	}
 ...
 *
 * The filter will be called for any action, which is not defined by NXTClass.
 ...
 */
function nxt_explain_nonce( $action ) {
...
/**
 * Kill NXTClass execution and display HTML message with error message.
 *
 ...
 */
function nxt_die( $message, $title = '', $args = array() ) {
	if ( defined( 'DOING_AJAX' ) && DOING_AJAX )
		die('-1');

	if ( function_exists( 'apply_filters' ) ) {
		$function = apply_filters( 'nxt_die_handler', '_default_nxt_die_handler');
	} else {
		$function = '_default_nxt_die_handler';
	}
 ...
 * Kill NXTClass execution and display HTML message with error message.
 *
 * This is the default handler for nxt_die if you want a custom one for your
 * site then you can overload using the nxt_die_handler filter in nxt_die
 *
 * @since 3.0.0
 * @access private
 *
 ...
 * @param string|array $args Optional arguments to control behavior.
 */
function _default_nxt_die_handler( $message, $title = '', $args = array() ) {
	...
	if ( !function_exists( 'did_action' ) || !did_action( 'admin_head' ) ) :
		...
		if ( empty($title) )
			$title = $have_gettext ? __('NXTClass &rsaquo; Error') : 'NXTClass &rsaquo; Error';
...
/**
 * Kill NXTClass execution and display XML message with error message.
 *
 * This is the handler for nxt_die when processing XMLRPC requests.
 *
 * @since 3.2.0
 * @access private
 *
 ...
 * @param string|array $args Optional arguments to control behavior.
 */
function _xmlrpc_nxt_die_handler( $message, $title = '', $args = array() ) {
	global $nxt_xmlrpc_server;
	$defaults = array( 'response' => 500 );

	$r = nxt_parse_args($args, $defaults);

	if ( $nxt_xmlrpc_server ) {
		$error = new IXR_Error( $r['response'] , $message);
		$nxt_xmlrpc_server->output( $error->getXml() );
	}
	die();
}

/**
 * Filter to enable special nxt_die handler for xmlrpc requests.
 *
 * @since 3.2.0
 * @access private
 */
function _xmlrpc_nxt_die_filter() {
	return '_xmlrpc_nxt_die_handler';
}


/**
 * Retrieve the NXTClass home page URL.
 *
 * If the constant named 'nxt_HOME' exists, then it will be used and returned by
 ...
 */
function _config_nxt_home( $url = '' ) {
	if ( defined( 'nxt_HOME' ) )
		return untrailingslashit( nxt_HOME );
	return $url;
}

/**
 * Retrieve the NXTClass site URL.
 *
 * If the constant named 'nxt_SITEURL' is defined, then the value in that
 ...
 * @since 2.2.0
 *
 * @param string $url URL to set the NXTClass site location.
 * @return string The NXTClass Site URL
 */
function _config_nxt_siteurl( $url = '' ) {
	if ( defined( 'nxt_SITEURL' ) )
		return untrailingslashit( nxt_SITEURL );
	return $url;
}
 ...
 *
 * Will only set the direction to 'rtl', if the NXTClass locale has the text
 ...
 *
 * Plugins may override the default smiley list by setting the $nxtsmiliestrans
 * to an array, with the key the code the blogger types in and the value the
 * image file.
 *
 * The $nxt_smiliessearch global is for the regular expression and is set each
 ...
 * available.
 *
 * @global array $nxtsmiliestrans
 * @global array $nxt_smiliessearch
 * @since 2.2.0
 */
function smilies_init() {
	global $nxtsmiliestrans, $nxt_smiliessearch;

	// don't bother setting up smilies if they are disabled
	if ( !get_option( 'use_smilies' ) )
		return;

	if ( !isset( $nxtsmiliestrans ) ) {
		$nxtsmiliestrans = array(
	 ...
	 */
	krsort($nxtsmiliestrans);

	$nxt_smiliessearch = '/(?:\s|^)';

	$subchar = '';
	foreach ( (array) $nxtsmiliestrans as $smiley => $img ) {
		...
		if ($firstchar != $subchar) {
			...
			$subchar = $firstchar;
			$nxt_smiliessearch .= preg_quote($firstchar, '/') . '(?:';
		} else {
			$nxt_smiliessearch .= '|';
		}
		$nxt_smiliessearch .= preg_quote($rest, '/');
	}

	$nxt_smiliessearch .= ')(?:\s|$)/m';
}

/**
 * Merge user defined arguments into defaults array.
 *
 * This function is used throughout NXTClass to allow for both string or array
 ...
 */
function nxt_parse_args( $args, $defaults = '' ) {
	if ( is_object( $args ) )
		$r = get_object_vars( $args );
	elseif ( is_array( $args ) )
		$r =& $args;
	else
		nxt_parse_str( $args, $r );
 ...
 */
function nxt_parse_id_list( $list ) {
	if ( !is_array($list) )
 ...
 */
function nxt_array_slice_assoc( $array, $keys ) {
	$slice = array();
	foreach ( $keys as $key )
		if ( isset( $array[ $key ] ) )
			$slice[ $key ] = $array[ $key ];
 ...
 * @return array
 */
function nxt_list_filter( $list, $args = array(), $operator = 'AND' ) {
	if ( ! is_array( $list ) )
		return array();
 ...
 * @return array
 */
function nxt_list_pluck( $list, $field ) {
 ...
 *
 * @since 2.2.0
 * @uses add_action() Calls '_admin_menu' hook with 'nxt_widgets_add_menu' value.
 */
function nxt_maybe_load_widgets() {
	if ( ! apply_filters('load_default_widgets', true) )
		return;
	require_once( ABSPATH . nxtINC . '/default-widgets.php' );
	add_action( '_admin_menu', 'nxt_widgets_add_menu' );
}
...
/**
 * Load custom DB error or display NXTClass DB error.
 *
 * If a file exists in the nxt-content directory named db-error.php, then it will
 * be loaded instead of displaying the NXTClass DB error. If it is not found,
 * then the NXTClass DB error will be displayed instead.
 *
 * The NXTClass DB error sets the HTTP status header to 500 to try to prevent
 * search engines from caching the message. Custom DB messages should do the
 * same.
 *
 * This function was backported to the the NXTClass 2.3.2, but originally was
 * added in NXTClass 2.5.0.
 *
 * @since 2.3.2
 * @uses $nxtdb
 */
function dead_db() {
	...
	if ( file_exists( nxt_CONTENT_DIR . '/db-error.php' ) ) {
		require_once( nxt_CONTENT_DIR . '/db-error.php' );
		die();
	}

	// If installing or in the admin, provide the verbose message.
	if ( defined('nxt_INSTALLING') || defined('nxt_ADMIN') )
		nxt_die($nxtdb->error);
...
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
	<title><?php echo /*nxt_I18N_DB_ERROR*/'Database Error'/*/nxt_I18N_DB_ERROR*/; ?></title>

</head>
<body>
	<h1><?php echo /*nxt_I18N_DB_CONNECTION_ERROR*/'Error establishing a database connection'/*/nxt_I18N_DB_CONNECTION_ERROR*/; ?></h1>
</body>
</html>
<?php
	die();
 ...
 * function.
 *
 * The current behavior is to trigger a user error if nxt_DEBUG is true.
 *
 ...
 * @param string $version The version of NXTClass that deprecated the function
...
function _deprecated_function( $function, $version, $replacement = null ) {
	...
	if ( nxt_DEBUG && apply_filters( 'deprecated_function_trigger_error', true ) ) {
 ...
 * @param string $version The version of NXTClass that deprecated the file
...
function _deprecated_file( $file, $version, $replacement = null, $message = '' ) {
	...
	// Allow plugin to filter the output error trigger
	if ( nxt_DEBUG && apply_filters( 'deprecated_file_trigger_error', true ) ) {
 ...
 *
 * @param string $function The function that was called
 * @param string $version The version of NXTClass that deprecated the argument used
 * @param string $message Optional. A message regarding the change.
 */
function _deprecated_argument( $function, $version, $message = null ) {
	...
	// Allow plugin to filter the output error trigger
	if ( nxt_DEBUG && apply_filters( 'deprecated_argument_trigger_error', true ) ) {
		if ( ! is_null( $message ) )
 ...
 * @param string $version The version of NXTClass where the message was added.
 */
 ...
 *
 * Will remove nxt-admin links to retrieve only return URLs not in the nxt-admin
 * directory.
 *
 * @since 2.6.0
 *
 * @return string
 */
function nxt_guess_url() {
	if ( defined('nxt_SITEURL') && '' != nxt_SITEURL ) {
		$url = nxt_SITEURL;
	} else {
		$schema = is_ssl() ? 'https://' : 'http://';
		$url = preg_replace('|/nxt-admin/.*|i', '', $schema . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI']);
	}
	return rtrim($url, '/');
}
 ...
 */
function nxt_suspend_cache_addition( $suspend = null ) {
 ...
 * @return bool The current suspend setting
 */
function nxt_suspend_cache_invalidation($suspend = true) {
	global $_nxt_suspend_cache_invalidation;

	$current_suspend = $_nxt_suspend_cache_invalidation;
	$_nxt_suspend_cache_invalidation = $suspend;
...
function get_site_option( $option, $default = false, $use_cache = true ) {
	...
	if ( !is_multisite() ) {
		...
		$cache_key = "{$nxtdb->siteid}:$option";
		if ( $use_cache )
			$value = nxt_cache_get($cache_key, 'site-options');

		if ( !isset($value) || (false === $value) ) {
			$row = $nxtdb->get_row( $nxtdb->prepare("SELECT meta_value FROM $nxtdb->sitemeta WHERE meta_key = %s AND site_id = %d", $option, $nxtdb->siteid ) );

			// Has to be get_row instead of get_var because of funkiness with 0, false, null values
			if ( is_object( $row ) ) {
				$value = $row->meta_value;
				$value = maybe_unserialize( $value );
				nxt_cache_set( $cache_key, $value, 'site-options' );
			} else {
				$value = $default;
			}
		}
...
function add_site_option( $option, $value ) {
	...
	if ( !is_multisite() ) {
		...
		$_value = $value;
		$value = maybe_serialize( $value );
		$result = $nxtdb->insert( $nxtdb->sitemeta, array('site_id' => $nxtdb->siteid, 'meta_key' => $option, 'meta_value' => $value ) );
		$value = $_value;
	}
...
function delete_site_option( $option ) {
	...
	if ( !is_multisite() ) {
		...
		$row = $nxtdb->get_row( $nxtdb->prepare( "SELECT meta_id FROM {$nxtdb->sitemeta} WHERE meta_key = %s AND site_id = %d", $option, $nxtdb->siteid ) );
		if ( is_null( $row ) || !$row->meta_id )
			return false;
		$cache_key = "{$nxtdb->siteid}:$option";
		nxt_cache_delete( $cache_key, 'site-options' );

		$result = $nxtdb->query( $nxtdb->prepare( "DELETE FROM {$nxtdb->sitemeta} WHERE meta_key = %s AND site_id = %d", $option, $nxtdb->siteid ) );
	}
...
function update_site_option( $option, $value ) {
	...
	if ( !is_multisite() ) {
		...
		$_value = $value;
		$value = maybe_serialize( $value );
		$result = $nxtdb->update( $nxtdb->sitemeta, array( 'meta_value' => $value ), array( 'site_id' => $nxtdb->siteid, 'meta_key' => $option ) );
		$value = $_value;
	}
 ...
 * @return int
 */
function _nxt_timezone_choice_usort_callback( $a, $b ) {
 ...
 * @return string
 */
function nxt_timezone_choice( $selected_zone ) {
	...
	if ( !$mo_loaded ) {
		$locale = get_locale();
		$mofile = nxt_LANG_DIR . '/continents-cities-' . $locale . '.mo';
	...
	usort( $zonen, '_nxt_timezone_choice_usort_callback' );
 ...
 * @return string
 */
function _nxt_mysql_week( $column ) {
 ...
 */
function nxt_find_hierarchy_loop( $callback, $start, $start_parent, $callback_args = array() ) {
	$override = is_null( $start_parent ) ? array() : array( $start => $start_parent );

	if ( !$arbitrary_loop_member = nxt_find_hierarchy_loop_tortoise_hare( $callback, $start, $override, $callback_args ) )
		return array();

	return nxt_find_hierarchy_loop_tortoise_hare( $callback, $arbitrary_loop_member, $override, $callback_args, true );
}
 ...
 */
function nxt_find_hierarchy_loop_tortoise_hare( $callback, $start, $override = array(), $callback_args = array(), $_return_loop = false ) {
 ...
 */
function nxt_allowed_protocols() {
	static $protocols;
