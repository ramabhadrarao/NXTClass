<?php
/**
 * NXTClass API for media display.
 *
 * @package NXTClass
 */
...
function image_constrain_size_for_editor($width, $height, $size = 'medium') {
	global $content_width, $_nxt_additional_image_sizes;
	...
	} elseif ( isset( $_nxt_additional_image_sizes ) && count( $_nxt_additional_image_sizes ) && in_array( $size, array_keys( $_nxt_additional_image_sizes ) ) ) {
		$max_width = intval( $_nxt_additional_image_sizes[$size]['width'] );
		$max_height = intval( $_nxt_additional_image_sizes[$size]['height'] );
	...
	return nxt_constrain_dimensions( $width, $height, $max_width, $max_height );
}
...
function add_image_size( $name, $width = 0, $height = 0, $crop = false ) {
	global $_nxt_additional_image_sizes;
	$_nxt_additional_image_sizes[$name] = array( 'width' => absint( $width ), 'height' => absint( $height ), 'crop' => (bool) $crop );
}
 ...
 * parameters are: what NXTClass will use for the class, the Attachment ID,
...
function get_image_tag($id, $alt, $title, $align, $size='medium') {
	...
	$class = 'align' . esc_attr($align) .' size-' . esc_attr($size) . ' nxt-image-' . $id;
...
function nxt_load_image( $file ) {
	...
	@ini_set( 'memory_limit', apply_filters( 'image_memory_limit', nxt_MAX_MEMORY_LIMIT ) );
 ...
 */
function nxt_constrain_dimensions( $current_width, $current_height, $max_width=0, $max_height=0 ) {
...
function image_resize_dimensions($orig_w, $orig_h, $dest_w, $dest_h, $crop = false) {
	...
	if ( $crop ) {
		...
		list( $new_w, $new_h ) = nxt_constrain_dimensions( $orig_w, $orig_h, $dest_w, $dest_h );
	}
 ...
 * support. This is not the fault of NXTClass (where functionality is
 ...
 * @return mixed nxt_Error on failure. String with new destination path.
 */
function image_resize( $file, $max_w, $max_h, $crop = false, $suffix = null, $dest_path = null, $jpeg_quality = 90 ) {

	$image = nxt_load_image( $file );
	if ( !is_resource( $image ) )
		return new nxt_Error( 'error_loading_image', $image, $file );

	$size = @getimagesize( $file );
	if ( !$size )
		return new nxt_Error('invalid_image', __('Could not read image size'), $file);
	list($orig_w, $orig_h, $orig_type) = $size;

	$dims = image_resize_dimensions($orig_w, $orig_h, $max_w, $max_h, $crop);
	if ( !$dims )
		return new nxt_Error( 'error_getting_dimensions', __('Could not calculate resized image dimensions') );
	list($dst_x, $dst_y, $src_x, $src_y, $dst_w, $dst_h, $src_w, $src_h) = $dims;

	$newimage = nxt_imagecreatetruecolor( $dst_w, $dst_h );
	...
	if ( IMAGETYPE_GIF == $orig_type ) {
		if ( !imagegif( $newimage, $destfilename ) )
			return new nxt_Error('resize_path_invalid', __( 'Resize path invalid' ));
...
function image_make_intermediate_size($file, $width, $height, $crop=false) {
	if ( $width || $height ) {
		...
		if ( !is_nxt_error($resized_file) && $resized_file && $info = getimagesize($resized_file) ) {
			$resized_file = apply_filters('image_make_intermediate_size', $resized_file);
			return array(
				'file' => nxt_basename( $resized_file ),
				'width' => $info[0],
				'height' => $info[1],
			);
		}
...
function get_intermediate_image_sizes() {
	...
	if ( isset( $_nxt_additional_image_sizes ) && count( $_nxt_additional_image_sizes ) )
		$image_sizes = array_merge( $image_sizes, array_keys( $_nxt_additional_image_sizes ) );
...
function nxt_get_attachment_image($attachment_id, $size = 'thumbnail', $icon = false, $attr = '') {
	...
	if ( $image ) {
		...
		$default_attr = array(
			...
			'alt'	=> trim(strip_tags( get_post_meta($attachment_id, '_nxt_attachment_image_alt', true) )), // Use Alt field first
...
/**
 * Adds a 'nxt-post-image' class to post thumbnails
 ...
 * @return array
 */
function _nxt_post_thumbnail_class_filter( $attr ) {
	$attr['class'] .= ' nxt-post-image';
	return $attr;
}

/**
 * Adds _nxt_post_thumbnail_class_filter to the nxt_get_attachment_image_attributes filter
 *
 * @since 2.9.0
 */
function _nxt_post_thumbnail_class_filter_add( $attr ) {
	add_filter( 'nxt_get_attachment_image_attributes', '_nxt_post_thumbnail_class_filter' );
}

/**
 * Removes _nxt_post_thumbnail_class_filter from the nxt_get_attachment_image_attributes filter
 *
 * @since 2.9.0
 */
function _nxt_post_thumbnail_class_filter_remove( $attr ) {
	remove_filter( 'nxt_get_attachment_image_attributes', '_nxt_post_thumbnail_class_filter' );
}

add_shortcode('nxt_caption', 'img_caption_shortcode');
...
function img_caption_shortcode($attr, $content = null) {
	...
	return '<div ' . $id . 'class="nxt-caption ' . esc_attr($align) . '" style="width: ' . (10 + (int) $width) . 'px">'
	. do_shortcode( $content ) . '<p class="nxt-caption-text">' . $caption . '</p></div>';
}
 ...
 */
function nxt_imagecreatetruecolor($width, $height) {
...
class nxt_Embed {
	...
	function __construct() {
		// Hack to get the [embed] shortcode to run before nxtautop()
	 ...
	 * an AJAX request that will call nxt_Embed::cache_oembed().
	 */
...
<?php
	...
	/**
	 * Register an embed handler. Do not use this function directly, use {@link nxt_embed_register_handler()} instead.
	...
	/**
	 * Unregister a previously registered embed handler. Do not use this function directly, use {@link nxt_embed_unregister_handler()} instead.
	 *
	...
	function shortcode( $attr, $url = '' ) {
		...
		if ( !empty($this->post_ID) ) // Potentially set by nxt_Embed::cache_oembed()
	...
	/**
	 * Passes any unlinked URLs that are on their own line to {@link nxt_Embed::shortcode()} for potential embedding.
	 *
	 * @uses nxt_Embed::autoembed_callback()
	 *
	...
	/**
	 * Callback function for {@link nxt_Embed::autoembed()}.
	 *
	 * @uses nxt_Embed::shortcode()
	 *
...
$GLOBALS['nxt_embed'] = new nxt_Embed();

/**
 * Register an embed handler. This function should probably only be used for sites that do not support oEmbed.
 *
 * @since 2.9.0
 * @see nxt_Embed::register_handler()
 */
function nxt_embed_register_handler( $id, $regex, $callback, $priority = 10 ) {
	global $nxt_embed;
	$nxt_embed->register_handler( $id, $regex, $callback, $priority );
}
 ...
 *
 * @since 2.9.0
 * @see nxt_Embed::unregister_handler()
 */
function nxt_embed_unregister_handler( $id, $priority = 10 ) {
	global $nxt_embed;
	$nxt_embed->unregister_handler( $id, $priority );
}
 ...
 */
function nxt_oembed_get( $url, $args = '' ) {
 ...
 * @since 2.9.0
 */
function nxt_maybe_load_embeds() {
	if ( ! apply_filters( 'load_default_embeds', true ) )
		return;
	nxt_embed_register_handler( 'googlevideo', '#http://video\.google\.([A-Za-z.]{2,5})/videoplay\?docid=([\d-]+)(.*?)#i', 'nxt_embed_handler_googlevideo' );
}
 ...
 *
 * @see nxt_Embed::register_handler()
 * @see nxt_Embed::shortcode()
 *
 * @param array $matches The regex matches from the provided regex when calling {@link nxt_embed_register_handler()}.
 * @param array $attr Embed attributes.
 * @param string $url The original URL that was matched by the regex.
 * @param array $rawattr The original unmodified attributes.
 * @return string The embed HTML.
 */
function nxt_embed_handler_googlevideo( $matches, $attr, $url, $rawattr ) {
	// If the user supplied a fixed width AND height, use it
	if ( !empty($rawattr['width']) && !empty($rawattr['height']) ) {
		$width  = (int) $rawattr['width'];
		$height = (int) $rawattr['height'];
	} else {
		list( $width, $height ) = nxt_expand_dimensions( 425, 344, $attr['width'], $attr['height'] );
	}
