 ...
 * Metadata API
 *
 * Functions for retrieving and manipulating metadata of various NXTClass object types.  Metadata
...
function add_metadata($meta_type, $object_id, $meta_key, $meta_value, $unique = false) {
	...
	if ( $unique && $nxtdb->get_var( $nxtdb->prepare(
	...
	$result = $nxtdb->insert( $table, array(
		$column => $object_id,
		'meta_key' => $meta_key,
...
function update_metadata($meta_type, $object_id, $meta_key, $meta_value, $prev_value = '') {
	...
	if ( ! $meta_id = $nxtdb->get_var( $nxtdb->prepare( "SELECT $id_column FROM $table WHERE meta_key = %s AND $column = %d", $meta_key, $object_id ) ) )
...
function delete_metadata($meta_type, $object_id, $meta_key, $meta_value = '', $delete_all = false) {
	...
	$query = $nxtdb->prepare( "SELECT $id_column FROM $table WHERE meta_key = %s", $meta_key );

	if ( !$delete_all )
		$query .= $nxtdb->prepare(" AND $type_column = %d", $object_id );

	if ( $meta_value )
		$query .= $nxtdb->prepare(" AND meta_value = %s", $meta_value );

	$meta_ids = $nxtdb->get_col( $query );
	if ( !count( $meta_ids ) )
		return false;

	if ( $delete_all )
		$object_ids = $nxtdb->get_col( $nxtdb->prepare( "SELECT $type_column FROM $table WHERE meta_key = %s", $meta_key ) );
	...
	if ( $delete_all ) {
		foreach ( (array) $object_ids as $o_id ) {
			nxt_cache_delete($o_id, $meta_type . '_meta');
		}
	} else {
		nxt_cache_delete($object_id, $meta_type . '_meta');
	}
...
function get_metadata($meta_type, $object_id, $meta_key = '', $single = false) {
	...
	$meta_cache = nxt_cache_get($object_id, $meta_type . '_meta');
...
function get_metadata_by_mid( $meta_type, $meta_id ) {
	...
	$meta = $nxtdb->get_row( $nxtdb->prepare( "SELECT * FROM $table WHERE $id_column = %d", $meta_id ) );
...
function update_metadata_by_mid( $meta_type, $meta_id, $meta_value, $meta_key = false ) {
	...
	if ( $meta = get_metadata_by_mid( $meta_type, $meta_id ) ) {
		...
		$result = (bool) $nxtdb->update( $table, $data, $where, '%s', '%d' );
...
function update_meta_cache($meta_type, $object_ids) {
	...
	// Get meta info
	$id_list = join(',', $ids);
	$meta_list = $nxtdb->get_results( $nxtdb->prepare("SELECT $column, meta_key, meta_value FROM $table WHERE $column IN ($id_list)",
		$meta_type), ARRAY_A );
...
function get_meta_sql( $meta_query, $type, $primary_table, $primary_id_column, $context = null ) {
	$meta_query_obj = new nxt_Meta_Query( $meta_query );
...
class nxt_Meta_Query {
	...
	*/
	...
	function parse_query_vars( $qv ) {
		...
		// nxt_Query sets 'meta_value' = '' by default
	...
	function get_sql( $type, $primary_table, $primary_id_column, $context = null ) {
		...
		foreach ( $this->queries as $k => $q ) {
			...
			$where[$k] = '';
			if ( !empty( $meta_key ) )
				$where[$k] = $nxtdb->prepare( "$alias.meta_key = %s", $meta_key );
			...
			$where[$k] = ' (' . $where[$k] . $nxtdb->prepare( "CAST($alias.meta_value AS {$meta_type}) {$meta_compare} {$meta_compare_string})", $meta_value );
		}
...
function _get_meta_table($type) {
	...
	if ( empty($nxtdb->$table_name) )
		return false;

	return $nxtdb->$table_name;
}
