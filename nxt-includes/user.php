 ...
 * @return object Either nxt_Error on failure, or nxt_User on success.
 */
function nxt_signon( $credentials = '', $secure_cookie = '' ) {
	...
	// TODO do we deprecate the nxt_authentication action?
	do_action_ref_array('nxt_authenticate', array(&$credentials['user_login'], &$credentials['user_password']));
	...
	global $auth_secure_cookie; // XXX ugly hack to pass this to nxt_authenticate_cookie
	$auth_secure_cookie = $secure_cookie;

	add_filter('authenticate', 'nxt_authenticate_cookie', 30, 3);

	$user = nxt_authenticate($credentials['user_login'], $credentials['user_password']);
	...
	nxt_set_auth_cookie($user->ID, $credentials['remember'], $secure_cookie);
	do_action('nxt_login', $user->user_login, $user);
	return $user;
}
 ...
 */
add_filter('authenticate', 'nxt_authenticate_username_password', 20, 3);
function nxt_authenticate_username_password($user, $username, $password) {
	if ( is_a($user, 'nxt_User') ) { return $user; }
	...
	if ( !nxt_check_password($password, $userdata->user_pass, $userdata->ID) )
		return new nxt_Error( 'incorrect_password', sprintf( __( '<strong>ERROR</strong>: The password you entered for the username <strong>%1$s</strong> is incorrect. <a href="%2$s" title="Password Lost and Found">Lost your password</a>?' ),
		$username, nxt_lostpassword_url() ) );
...
function count_user_posts($userid) {
	...
	$count = $nxtdb->get_var( "SELECT COUNT(*) FROM $nxtdb->posts $where" );
...
function count_many_users_posts($users, $post_type = 'post' ) {
	...
	$result = $nxtdb->get_results( "SELECT post_author, COUNT(*) FROM $nxtdb->posts $where AND post_author IN ($userlist) GROUP BY post_author", ARRAY_N );
...
function get_user_option( $option, $user = 0, $deprecated = '' ) {
	...
	if ( empty( $user ) )
		...
		$user = new nxt_User( $user );

	if ( ! isset( $user->ID ) )
		return false;

	if ( $user->has_prop( $nxtdb->prefix . $option ) ) // Blog specific
		$result = $user->get( $nxtdb->prefix . $option );
 ...
 * it will prepend the NXTClass table prefix to the option name.
 *
...
/**
 * NXTClass User Query class.
 *
 * @since 3.1.0
 */
class nxt_User_Query {
	 ...
	 * @return nxt_User_Query
	 */
	function __construct( $query = null ) {
		if ( !empty( $query ) ) {
			$this->query_vars = nxt_parse_args( $query, array(
	...
	function query() {
		...
		if ( is_array( $this->query_vars['fields'] ) || 'all' == $this->query_vars['fields'] ) {
			$this->results = $nxtdb->get_results("SELECT $this->query_fields $this->query_from $this->query_where $this->query_orderby $this->query_limit");
		} else {
			$this->results = $nxtdb->get_col("SELECT $this->query_fields $this->query_from $this->query_where $this->query_orderby $this->query_limit");
		}

		if ( $this->query_vars['count_total'] )
			$this->total_users = $nxtdb->get_var( apply_filters( 'found_users_query', 'SELECT FOUND_ROWS()' ) );
		...
		if ( 'all_with_meta' == $this->query_vars['fields'] ) {
			...
			foreach ( $this->results as $userid )
				$r[ $userid ] = new nxt_User( $userid, '', $this->query_vars['blog_id'] );
...
function get_blogs_of_user( $user_id, $all = false ) {
	...
	if ( isset( $keys[ $nxtdb->base_prefix . 'capabilities' ] ) && defined( 'MULTISITE' ) ) {
		...
		unset( $keys[ $nxtdb->base_prefix . 'capabilities' ] );
	}
	...
	foreach ( $keys as $key ) {
		if ( 'capabilities' !== substr( $key, -12 ) )
			continue;
		if ( $nxtdb->base_prefix && 0 !== strpos( $key, $nxtdb->base_prefix ) )
			continue;
		$blog_id = str_replace( array( $nxtdb->base_prefix, '_capabilities' ), '', $key );
 ...
 *
 * Used by nxt_set_current_user() for back compat. Might be deprecated in the future.
 *
 * @since 2.0.4
...
function clean_user_cache($id) {
	$user = nxt_User::get_data_by( 'id', $id );

	nxt_cache_delete($id, 'users');
	nxt_cache_delete($user->user_login, 'userlogins');
	nxt_cache_delete($user->user_email, 'useremail');
	nxt_cache_delete($user->user_nicename, 'userslugs');
}
...
/**
 * Return a list of meta keys that nxt_insert_user() is supposed to set.
 *
 * @access private
 * @since 3.3.0
 *
 * @param object $user nxt_User instance
 * @return array
 */
function _get_additional_user_keys( $user ) {
	...
	return array_merge( $keys, array_keys( _nxt_get_user_contactmethods( $user ) ) );
}
 ...
 */
function _nxt_get_user_contactmethods( $user = null ) {
