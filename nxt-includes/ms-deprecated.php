<?php
/**
 * Deprecated functions from NXTClass MU and the multisite feature. You shouldn't
 ...
 * @since MU
 * @deprecated 3.0.0
 * @deprecated Use nxt_die()
 * @see nxt_die()
 */
function graceful_fail( $message ) {
	_deprecated_function( __FUNCTION__, '3.0', 'nxt_die()' );
...
function get_blog_list( $start = 0, $num = 10, $deprecated = '' ) {
	...
	global $nxtdb;
	$blogs = $nxtdb->get_results( $nxtdb->prepare("SELECT blog_id, domain, path FROM $nxtdb->blogs WHERE site_id = %d AND public = '1' AND archived = '0' AND mature = '0' AND spam = '0' AND deleted = '0' ORDER BY registered DESC", $nxtdb->siteid), ARRAY_A );

	foreach ( (array) $blogs as $details ) {
		$blog_list[ $details['blog_id'] ] = $details;
		$blog_list[ $details['blog_id'] ]['postcount'] = $nxtdb->get_var( "SELECT COUNT(ID) FROM " . $nxtdb->get_blog_prefix( $details['blog_id'] ). "posts WHERE post_status='publish' AND post_type='post'" );
	}
	unset( $blogs );
	$blogs = $blog_list;
 ...
 * @since MU
 * @deprecated 3.3.0
 * @deprecated Use nxt_redirect()
 * @uses nxtmu_admin_redirect_add_updated_param()
 *
 * @param string $url
 */
function nxtmu_admin_do_redirect( $url = '' ) {
	...
	if ( $ref ) {
		$ref = nxtmu_admin_redirect_add_updated_param( $ref );
		nxt_redirect( $ref );
		exit();
	}
	if ( empty( $_SERVER['HTTP_REFERER'] ) == false ) {
		nxt_redirect( $_SERVER['HTTP_REFERER'] );
		exit();
	}

	$url = nxtmu_admin_redirect_add_updated_param( $url );
	if ( isset( $_GET['redirect'] ) ) {
		if ( substr( $_GET['redirect'], 0, 2 ) == 's_' )
			$url .= '&action=blogs&s='. esc_html( substr( $_GET['redirect'], 2 ) );
	} elseif ( isset( $_POST['redirect'] ) ) {
		$url = nxtmu_admin_redirect_add_updated_param( $_POST['redirect'] );
	}
	nxt_redirect( $url );
	exit();
}
 ...
 * @return string
 */
function nxtmu_admin_redirect_add_updated_param( $url = '' ) {
	_deprecated_function( __FUNCTION__, '3.3' );
