...
function get_admin_users_for_domain( $sitedomain = '', $path = '' ) {
	...
	if ( ! $sitedomain )
		...
		$site_id = $nxtdb->get_var( $nxtdb->prepare( "SELECT id FROM $nxtdb->site WHERE domain = %s AND path = %s", $sitedomain, $path ) );

	if ( $site_id )
		return $nxtdb->get_results( $nxtdb->prepare( "SELECT u.ID, u.user_login, u.user_pass FROM $nxtdb->users AS u, $nxtdb->sitemeta AS sm WHERE sm.meta_key = 'admin_user_id' AND u.ID = sm.meta_value AND sm.site_id = %d", $site_id ), ARRAY_A );
...
function get_blog_post( $blog_id, $post_id ) {
	...
	$key = $blog_id . '-' . $post_id;
	$post = nxt_cache_get( $key, 'global-posts' );
	if ( $post == false ) {
		$post = $nxtdb->get_row( $nxtdb->prepare( 'SELECT * FROM ' . $nxtdb->get_blog_prefix( $blog_id ) . 'posts WHERE ID = %d', $post_id ) );
		nxt_cache_add( $key, $post, 'global-posts' );
	}
...
function create_empty_blog( $domain, $path, $weblog_title, $site_id = 1 ) {
	...
	// Need to back up nxtdb table names, and create a new nxt_blogs entry for new blog.
	// Need to get blog_id from nxt_blogs, and create new table names.
 ...
 * at nxt-admin/network/settings.php. The check is only run on
 * self-registrations; user creation at nxt-admin/network/users.php
 * bypasses this check.
 *
 * @since MU
 *
 ...
 *
 * The hook 'nxtmu_validate_user_signup' provides an easy way
 ...
 * messages created by the function. 'nxtmu_validate_user_signup' allows
 ...
 */
function nxtmu_validate_user_signup($user_name, $user_email) {
	global $nxtdb;
	...
	$signup = $nxtdb->get_row( $nxtdb->prepare("SELECT * FROM $nxtdb->signups WHERE user_login = %s", $user_name) );
	if ( $signup != null ) {
		...
		if ( $diff > 172800 )
			$nxtdb->query( $nxtdb->prepare("DELETE FROM $nxtdb->signups WHERE user_login = %s", $user_name) );
		else
	...
	$signup = $nxtdb->get_row( $nxtdb->prepare("SELECT * FROM $nxtdb->signups WHERE user_email = %s", $user_email) );
	if ( $signup != null ) {
		$diff = current_time( 'timestamp', true ) - mysql2date('U', $signup->registered);
		// If registered more than two days ago, cancel registration and let this signup go through.
		if ( $diff > 172800 )
			$nxtdb->query( $nxtdb->prepare("DELETE FROM $nxtdb->signups WHERE user_email = %s", $user_email) );
		else
	...
	return apply_filters('nxtmu_validate_user_signup', $result);
}
 ...
 *
 * Filter 'nxtmu_validate_blog_signup' if you want to modify
 * the way that NXTClass validates new site signups.
 *
 * @since MU
 * @uses domain_exists()
 * @uses username_exists()
 ...
 */
function nxtmu_validate_blog_signup($blogname, $blog_title, $user = '') {
	global $nxtdb, $domain, $base, $current_site;
	...
	if ( !is_subdomain_install() && $nxtdb->get_var( $nxtdb->prepare( "SELECT post_name FROM " . $nxtdb->get_blog_prefix( $current_site->blog_id ) . "posts WHERE post_type = 'page' AND post_name = %s", $blogname ) ) )
	...
	$signup = $nxtdb->get_row( $nxtdb->prepare("SELECT * FROM $nxtdb->signups WHERE domain = %s AND path = %s", $mydomain, $path) ); // TODO: Check email too?
	if ( ! empty($signup) ) {
		$diff = current_time( 'timestamp', true ) - mysql2date('U', $signup->registered);
		// If registered more than two days ago, cancel registration and let this signup go through.
		if ( $diff > 172800 )
			$nxtdb->query( $nxtdb->prepare("DELETE FROM $nxtdb->signups WHERE domain = %s AND path = %s", $mydomain, $path) );
		else
			$errors->add('blogname', __('That site is currently reserved but may be available in a couple days.'));
	}

	$result = array('domain' => $mydomain, 'path' => $path, 'blogname' => $blogname, 'blog_title' => $blog_title, 'errors' => $errors);
	return apply_filters('nxtmu_validate_blog_signup', $result);
}
 ...
 *
 * @since MU
 * @uses nxtmu_signup_blog_notification()
 *
 ...
 */
function nxtmu_signup_blog($domain, $path, $title, $user, $user_email, $meta = '') {
	global $nxtdb;

	$key = substr( md5( time() . rand() . $domain ), 0, 16 );
	$meta = serialize($meta);
	$domain = $nxtdb->escape($domain);
	$path = $nxtdb->escape($path);
	$title = $nxtdb->escape($title);

	$nxtdb->insert( $nxtdb->signups, array(
		'domain' => $domain,
		'path' => $path,
		'title' => $title,
		'user_login' => $user,
	...
	nxtmu_signup_blog_notification($domain, $path, $title, $user, $user_email, $key, $meta);
}
 ...
 *
 * @since MU
 * @uses nxtmu_signup_user_notification()
 *
 * @param string $user The user's requested login name.
 * @param string $user_email The user's email address.
 * @param array $meta By default, this is an empty array.
 */
function nxtmu_signup_user($user, $user_email, $meta = '') {
	global $nxtdb;
	...
	nxtmu_signup_user_notification($user, $user_email, $key, $meta);
}
 ...
 * is enabled.
 *
 * Filter 'nxtmu_signup_blog_notification' to bypass this function or
 * replace it with your own notification behavior.
 *
 * Filter 'nxtmu_signup_blog_notification_email' and
 * 'nxtmu_signup_blog_notification_subject' to change the content
 ...
 * @param string $user_email The user's email address.
 * @param array $meta By default, contains the requested privacy setting and lang_id.
 * @param string $key The activation key created in nxtmu_signup_blog()
 * @return bool
 */
function nxtmu_signup_blog_notification($domain, $path, $title, $user, $user_email, $key, $meta = '') {
	global $current_site;

	if ( !apply_filters('nxtmu_signup_blog_notification', $domain, $path, $title, $user, $user_email, $key, $meta) )
		return false;

	// Send email with activation link.
	if ( !is_subdomain_install() || $current_site->id != 1 )
		$activate_url = network_site_url("nxt-activate.php?key=$key");
	else
		$activate_url = "http://{$domain}{$path}nxt-activate.php?key=$key"; // @todo use *_url() API
	...
	$message = sprintf(
		apply_filters( 'nxtmu_signup_blog_notification_email',
	...
	$subject = sprintf(
		apply_filters( 'nxtmu_signup_blog_notification_subject',
			__( '[%1$s] Activate %2$s' ),
			$domain, $path, $title, $user, $user_email, $key, $meta
		),
		$from_name,
	...
	nxt_mail($user_email, $subject, $message, $message_headers);
	return true;
}
 ...
 * been requested.
 *
 * Filter 'nxtmu_signup_user_notification' to bypass this function or
 * replace it with your own notification behavior.
 *
 * Filter 'nxtmu_signup_user_notification_email' and
 * 'nxtmu_signup_user_notification_subject' to change the content
 * and subject line of the email sent to newly registered users.
 *
 * @since MU
 *
 ...
 * @param string $key The activation key created in nxtmu_signup_user()
 * @return bool
 */
function nxtmu_signup_user_notification($user, $user_email, $key, $meta = '') {
	if ( !apply_filters('nxtmu_signup_user_notification', $user, $user_email, $key, $meta) )
		return false;
	...
	$message_headers = "From: \"{$from_name}\" <{$admin_email}>\n" . "Content-Type: text/plain; charset=\"" . get_option('blog_charset') . "\"\n";
	$message = sprintf(
		apply_filters( 'nxtmu_signup_user_notification_email',
			__( "To activate your user, please click the following link:\n\n%s\n\nAfter you activate, you will receive *another email* with your login.\n\n" ),
			$user, $user_email, $key, $meta
		),
		site_url( "nxt-activate.php?key=$key" )
	);
	// TODO: Don't hard code activation link.
	$subject = sprintf(
		apply_filters( 'nxtmu_signup_user_notification_subject',
			__( '[%1$s] Activate %2$s' ),
			$user, $user_email, $key, $meta
		),
		$from_name,
 ...
 * as when a Super Admin creates a new site. Hook to 'nxtmu_new_blog'
 ...
 * @return mixed Returns nxt_Error object on failure, int $blog_id on success
 */
function nxtmu_create_blog($domain, $path, $title, $user_id, $meta = '', $site_id = 1) {
	...
	// Check if the domain has been used already. We should return an error message.
	if ( domain_exists($domain, $path, $site_id) )
		return new nxt_Error('blog_taken', __('Site already exists.'));

	if ( !defined('nxt_INSTALLING') )
		define( 'nxt_INSTALLING', true );

	if ( ! $blog_id = insert_blog($domain, $path, $site_id) )
		return new nxt_Error('insert_blog', __('Could not create site.'));
	...
	add_option( 'nxtLANG', get_site_option( 'nxtLANG' ) );
	...
	restore_current_blog();
	do_action( 'nxtmu_new_blog', $blog_id, $user_id, $domain, $path, $site_id, $meta );
...
Disable these notifications: %4s' ), $blogname, $siteurl, $_SERVER['REMOTE_ADDR'], $options_site_url);
	...
	nxt_mail( $email, sprintf( __( 'New Site Registration: %s' ), $siteurl ), $msg );
	return true;
}
...
Disable these notifications: %3s'), $user->user_login, $_SERVER['REMOTE_ADDR'], $options_site_url);
	...
	nxt_mail( $email, sprintf(__('New User Registration: %s'), $user->user_login), $msg );
	return true;
}
...
function domain_exists($domain, $path, $site_id = 1) {
	global $nxtdb;
	return $nxtdb->get_var( $nxtdb->prepare("SELECT blog_id FROM $nxtdb->blogs WHERE domain = %s AND path = %s AND site_id = %d", $domain, $path, $site_id) );
}

/**
 * Store basic site info in the blogs table.
 *
 * This function creates a row in the nxt_blogs table and returns
...
function insert_blog($domain, $path, $site_id) {
	...
	$result = $nxtdb->insert( $nxtdb->blogs, array('site_id' => $site_id, 'domain' => $domain, 'path' => $path, 'registered' => current_time('mysql')) );
	if ( ! $result )
		return false;

	refresh_blog_details($nxtdb->insert_id);
	return $nxtdb->insert_id;
}
 ...
 * directly, be sure to use switch_to_blog() first, so that $nxtdb
...
function install_blog($blog_id, $blog_title = '') {
	global $nxtdb, $table_prefix, $nxt_roles;
	...
	if ( $nxtdb->get_results("SELECT ID FROM $nxtdb->posts") )
		die(__('<h1>Already Installed</h1><p>You appear to have already installed NXTClass. To reinstall please clear your old database tables first.</p>') . '</body></html>');
	...
	$nxtdb->update( $nxtdb->options, array('option_value' => ''), array('option_name' => 'admin_email') );

	// remove all perms
	$nxtdb->query( $nxtdb->prepare("DELETE FROM $nxtdb->usermeta WHERE meta_key = %s", $table_prefix.'user_level') );
	$nxtdb->query( $nxtdb->prepare("DELETE FROM $nxtdb->usermeta WHERE meta_key = %s", $table_prefix.'capabilities') );
 ...
 * Set blog defaults.
 *
 * This function creates a row in the nxt_blogs table.
 *
 * @since MU
 * @deprecated MU
 * @deprecated Use nxt_install_defaults()
 * @uses nxt_install_defaults()
 *
 ...
 *
 * Filter 'nxtmu_welcome_notification' to disable or bypass.
 *
 ...
 * @return bool
 */
function nxtmu_welcome_notification($blog_id, $user_id, $password, $title, $meta = '') {
	global $current_site;

	if ( !apply_filters('nxtmu_welcome_notification', $blog_id, $user_id, $password, $title, $meta) )
		return false;
...
function get_most_recent_post_of_user( $user_id ) {
	...
	foreach ( (array) $user_blogs as $blog ) {
		$recent_post = $nxtdb->get_row( $nxtdb->prepare("SELECT ID, post_date_gmt FROM {$nxtdb->base_prefix}{$blog->userblog_id}_posts WHERE post_author = %d AND post_type = 'post' AND post_status = 'publish' ORDER BY post_date_gmt DESC LIMIT 1", $user_id ), ARRAY_A);
 ...
 *
 * NXTClass ships with a set of allowed upload filetypes,
 * which is defined in nxt-includes/functions.php in
 * get_allowed_mime_types(). This function is used to filter
 * that list against the filetype whitelist provided by Multisite
 * Super Admins at nxt-admin/network/settings.php.
 *
 * @since MU
 *
 * @param array $mimes
 ...
 *
 * NXTClass MS stores a blog's post count as an option so as
...
function update_posts_count( $deprecated = '' ) {
	global $nxtdb;
	update_option( 'post_count', (int) $nxtdb->get_var( "SELECT COUNT(ID) FROM {$nxtdb->posts} WHERE post_status = 'publish' and post_type = 'post'" ) );
}
 ...
 * @param int $user_id
 */
function nxtmu_log_new_registrations( $blog_id, $user_id ) {
	global $nxtdb;
	$user = new nxt_User( (int) $user_id );
	$nxtdb->insert( $nxtdb->registration_log, array('email' => $user->user_email, 'IP' => preg_replace( '/[^0-9., ]/', '',$_SERVER['REMOTE_ADDR'] ), 'blog_id' => $blog_id, 'date_registered' => current_time('mysql')) );
}
...
function global_terms( $term_id, $deprecated = '' ) {
	...
	$c = $nxtdb->get_row( $nxtdb->prepare( "SELECT * FROM $nxtdb->terms WHERE term_id = %d", $term_id ) );

	$global_id = $nxtdb->get_var( $nxtdb->prepare( "SELECT cat_ID FROM $nxtdb->sitecategories WHERE category_nicename = %s", $c->slug ) );
	if ( $global_id == null ) {
		$used_global_id = $nxtdb->get_var( $nxtdb->prepare( "SELECT cat_ID FROM $nxtdb->sitecategories WHERE cat_ID = %d", $c->term_id ) );
		if ( null == $used_global_id ) {
			$nxtdb->insert( $nxtdb->sitecategories, array( 'cat_ID' => $term_id, 'cat_name' => $c->name, 'category_nicename' => $c->slug ) );
			$global_id = $nxtdb->insert_id;
			if ( empty( $global_id ) )
				return $term_id;
		} else {
			$max_global_id = $nxtdb->get_var( "SELECT MAX(cat_ID) FROM $nxtdb->sitecategories" );
			$max_local_id = $nxtdb->get_var( "SELECT MAX(term_id) FROM $nxtdb->terms" );
			$new_global_id = max( $max_global_id, $max_local_id ) + mt_rand( 100, 400 );
			$nxtdb->insert( $nxtdb->sitecategories, array( 'cat_ID' => $new_global_id, 'cat_name' => $c->name, 'category_nicename' => $c->slug ) );
			$global_id = $nxtdb->insert_id;
		}
	} elseif ( $global_id != $term_id ) {
		$local_id = $nxtdb->get_row( $nxtdb->prepare( "SELECT term_id FROM $nxtdb->terms WHERE term_id = %d", $global_id ) );
		if ( null != $local_id )
	...
	if ( $global_id != $term_id ) {
		...
		$nxtdb->update( $nxtdb->terms, array('term_id' => $global_id), array('term_id' => $term_id) );
		$nxtdb->update( $nxtdb->term_taxonomy, array('term_id' => $global_id), array('term_id' => $term_id) );
		$nxtdb->update( $nxtdb->term_taxonomy, array('parent' => $global_id), array('parent' => $term_id) );
...
function upload_is_file_too_big( $upload ) {
	if ( is_array( $upload ) == false || defined( 'nxt_IMPORTING' ) )
		return $upload;
 ...
 *
 * @since MU
 * @uses nxt_nonce_field()
 */
function signup_nonce_fields() {
	$id = mt_rand();
	echo "<input type='hidden' name='signup_form_id' value='{$id}' />";
	nxt_nonce_field('signup_form_' . $id, '_signup_form', false);
}
 ...
 * added, as when a user is invited through the regular nxt Add User interface.
 *
 * @since MU
...
function maybe_add_existing_user_to_blog() {
	...
	if ( empty( $details ) || is_nxt_error( add_existing_user_to_blog( $details ) ) )
		nxt_die( sprintf(__('An error occurred adding you to this site. Back to the <a href="%s">homepage</a>.'), site_url() ) );

	nxt_die( sprintf(__('You have been added to this site. Please visit the <a href="%s">homepage</a> or <a href="%s">log in</a> using your username and password.'), site_url(), admin_url() ), __('Success') );
}
...
function update_blog_public( $old_value, $value ) {
	...
	update_blog_status( $nxtdb->blogid, 'public', (int) $value );
}
 ...
 *  @since 3.1.0
 */
function nxt_update_network_counts() {
	global $nxtdb;

	$count = $nxtdb->get_var( $nxtdb->prepare("SELECT COUNT(blog_id) as c FROM $nxtdb->blogs WHERE site_id = %d AND spam = '0' AND deleted = '0' and archived = '0'", $nxtdb->siteid) );
	update_site_option( 'blog_count', $count );

	$count = $nxtdb->get_var( $nxtdb->prepare("SELECT COUNT(ID) as c FROM $nxtdb->users WHERE spam = '0' AND deleted = '0'") );
