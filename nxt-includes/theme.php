 ...
 * NXTClass workflow, then change the former. If you just have the locale in a
...
function get_locale_stylesheet_uri() {
	...
	elseif ( !empty($nxt_locale->text_direction) && file_exists("$dir/{$nxt_locale->text_direction}.css") )
		$stylesheet_uri = "$stylesheet_dir_uri/{$nxt_locale->text_direction}.css";
	else
		$stylesheet_uri = '';
...
function get_themes() {
	...
	foreach ( (array) $theme_files as $theme_file ) {
		...
		if ( isset($nxt_themes[$name]) ) {
			$trump_cards = array(
				'classic'      => 'NXTClass Classic',
				'default'      => 'NXTClass Default',
		...
		$nxt_themes[$name] = array(
			...
			'Theme Root URI' => str_replace( nxt_CONTENT_DIR, content_url(), $theme_root ),
		);
	}
...
function get_theme_roots() {
	global $nxt_theme_directories;

	if ( count($nxt_theme_directories) <= 1 )
		return '/themes';
 ...
 * @since 2.9.0
 *
 * @param string $directory Either the full filesystem path to a theme folder or a folder within nxt_CONTENT_DIR
 * @return bool
 */
function register_theme_directory( $directory) {
	...
	if ( !file_exists( $directory ) )
		...
		$registered_directory = nxt_CONTENT_DIR . '/' . $directory;
	else
	...
	$nxt_theme_directories[] = $registered_directory;
...
function search_theme_directories() {
	global $nxt_theme_directories, $nxt_broken_themes;
	if ( empty( $nxt_theme_directories ) )
		return false;

	$theme_files = array();
	$nxt_broken_themes = array();

	/* Loop the registered theme directories and extract all themes */
	foreach ( (array) $nxt_theme_directories as $theme_root ) {
		$theme_loc = $theme_root;

		/* We don't want to replace all forward slashes, see Trac #4541 */
		if ( '/' != nxt_CONTENT_DIR )
			$theme_loc = str_replace(nxt_CONTENT_DIR, '', $theme_root);
			...
			if ( is_dir($theme_root . '/' . $theme_dir) && is_readable($theme_root . '/' . $theme_dir) ) {
				...
				if ( !$found_stylesheet ) { // look for themes in that dir
					...
					if ( !$found_subdir_themes )
						$nxt_broken_themes[$theme_dir] = array('Name' => $theme_dir, 'Title' => $theme_dir, 'Description' => __('Stylesheet is missing.'));
				}
			}
		}
		@closedir( $themes_dir );
...
function get_theme_root( $stylesheet_or_template = false ) {
	if ( $stylesheet_or_template ) {
		if ( $theme_root = get_raw_theme_root($stylesheet_or_template) )
			$theme_root = nxt_CONTENT_DIR . $theme_root;
		else
			$theme_root = nxt_CONTENT_DIR . '/themes';
	} else {
		$theme_root = nxt_CONTENT_DIR . '/themes';
	}
 ...
 *
 * Works by first retrieving the current tag name, for example 'tag-nxtclass.php' and then
...
function get_page_template() {
	...
	$template = get_post_meta($id, '_nxt_page_template', true);
...
function get_comments_popup_template() {
	...
	if ('' == $template)
		$template = ABSPATH . nxtINC . '/theme-compat/comments-popup.php';
...
/**
 * Require the template file with NXTClass environment.
 *
 * The globals are set up for the template file to ensure that the NXTClass
...
function load_template( $_template_file, $require_once = true ) {
	global $posts, $post, $nxt_did_header, $nxt_did_template_redirect, $nxt_query, $nxt_rewrite, $nxtdb, $nxt_version, $nxt, $id, $comment, $user_ID;

	if ( is_array( $nxt_query->query_vars ) )
		extract( $nxt_query->query_vars, EXTR_SKIP );
...
function preview_theme_ob_filter_callback( $matches ) {
	...
	if (
		( false !== strpos($matches[3], '/nxt-admin/') )
	||
...
function switch_theme($template, $stylesheet) {
	global $nxt_theme_directories, $sidebars_widgets;
 ...
 *
 * @since 1.5.0
 * @see nxt_DEFAULT_THEME
 *
 * @return bool
 */
function validate_current_theme() {
	// Don't validate during an install/upgrade.
	if ( defined('nxt_INSTALLING') || !apply_filters( 'validate_current_theme', true ) )
		return true;

	if ( get_template() != nxt_DEFAULT_THEME && !file_exists(get_template_directory() . '/index.php') ) {
		switch_theme( nxt_DEFAULT_THEME, nxt_DEFAULT_THEME );
		return false;
	}

	if ( get_stylesheet() != nxt_DEFAULT_THEME && !file_exists(get_template_directory() . '/style.css') ) {
		switch_theme( nxt_DEFAULT_THEME, nxt_DEFAULT_THEME );
		return false;
	}
...
function get_random_header_image() {
	...
	elseif ( ! empty( $_nxt_default_headers ) ) {
		if ( 'random-default-image' == $header_image_mod ) {
			$headers = $_nxt_default_headers;
		} else {
...
function get_uploaded_header_images() {
	...
	// @todo caching
	$headers = get_posts( array( 'post_type' => 'attachment', 'meta_key' => '_nxt_attachment_is_custom_header', 'meta_value' => get_option('stylesheet'), 'orderby' => 'none', 'nopaging' => true ) );
 ...
 * content for the 'nxt_head' action. The parameter $admin_header_callback
 ...
 *
 * @param callback $header_callback Call on 'nxt_head' action.
...
function add_custom_image_header( $header_callback, $admin_header_callback, $admin_image_div_callback = '' ) {
	...
	require_once( ABSPATH . 'nxt-admin/custom-header.php' );
...
function remove_custom_image_header() {
	...
	remove_action( 'nxt_head', $callback[0]['callback'] );
...
function register_default_headers( $headers ) {
	...
	$_nxt_default_headers = array_merge( (array) $_nxt_default_headers, (array) $headers );
}
...
function unregister_default_headers( $header ) {
	...
	} elseif ( isset( $_nxt_default_headers[ $header ] ) ) {
		unset( $_nxt_default_headers[ $header ] );
		return true;
	} else {
		return false;
	}
...
function add_custom_background( $header_callback = '', $admin_header_callback = '', $admin_image_div_callback = '' ) {
	...
	if ( ! is_admin() )
		return;
	require_once( ABSPATH . 'nxt-admin/custom-background.php' );
...
function add_theme_support( $feature ) {
	global $_nxt_theme_features;

	if ( func_num_args() == 1 )
		$_nxt_theme_features[$feature] = true;
	else
		$_nxt_theme_features[$feature] = array_slice( func_get_args(), 1 );

	if ( $feature == 'post-formats' && is_array( $_nxt_theme_features[$feature][0] ) )
		$_nxt_theme_features[$feature][0] = array_intersect( $_nxt_theme_features[$feature][0], array_keys( get_post_format_slugs() ) );
}
...
function get_theme_support( $feature ) {
	global $_nxt_theme_features;
	if ( !isset( $_nxt_theme_features[$feature] ) )
		return false;
	else
		return $_nxt_theme_features[$feature];
}
...
function _remove_theme_support( $feature ) {
	...
	if ( ! isset( $_nxt_theme_features[$feature] ) )
		return false;
	unset( $_nxt_theme_features[$feature] );
	return true;
}
...
function current_theme_supports( $feature ) {
	...
	switch ( $feature ) {
		case 'post-thumbnails':
			...
			if ( true === $_nxt_theme_features[$feature] )  // Registered for all types
				return true;
			$content_type = $args[0];
			return in_array( $content_type, $_nxt_theme_features[$feature][0] );
			break;
		...
		case 'post-formats':
			...
			// add_theme_support()
			$post_format = $args[0];
			return in_array( $post_format, $_nxt_theme_features[$feature][0] );
			break;
	}
...
/**
 * Checks if a theme has been changed and runs 'after_switch_theme' hook on the next nxt load
 *
 * @since 3.3
 */
