...
class nxt_oEmbed {
	...
	function __construct() {
		...
		// The nxt_Embed class disables discovery for non-unfiltered_html users, so only providers in this array will be used for them.
		// Add to this list using the nxt_oembed_add_provider() function (see its PHPDoc for details).
		$this->providers = apply_filters( 'oembed_providers', array(
			...
			'http://nxtclass.tv/*'                         => array( 'http://nxtclass.tv/oembed/',              false ),
		...
		// Fix any embeds that contain new lines in the middle of the HTML which breaks nxtautop().
	 ...
	 *
	 * @see nxt_oEmbed::discover()
	 * @see nxt_oEmbed::fetch()
	 * @see nxt_oEmbed::data2html()
	 *
	...
	function discover( $url ) {
		...
		// Fetch URL content
		if ( $html = nxt_remote_retrieve_body( nxt_remote_get( $url ) ) ) {
	...
	/**
	 * Converts a data object from {@link nxt_oEmbed::fetch()} and returns the HTML.
	 *
	 ...
	 * @param object $data Data object from nxt_oEmbed::data2html()
...
/**
 * Returns the initialized {@link nxt_oEmbed} object
 *
 * @since 2.9.0
 * @access private
 *
 ...
 * @uses nxt_oEmbed
 *
 * @return nxt_oEmbed object.
 */
function &_nxt_oembed_get_object() {
	static $nxt_oembed;

	if ( is_null($nxt_oembed) )
		$nxt_oembed = new nxt_oEmbed();
