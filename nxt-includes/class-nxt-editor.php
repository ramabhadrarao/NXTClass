<?php
/**
 * Facilitates adding of the NXTClass editor as used on the Write and Edit screens.
 *
 * @package NXTClass
 * @since 3.3
 *
 * Private, not included by default. See nxt_editor() in nxt-includes/general-template.php.
 */
...
final class _nxt_Editors {
	...
	public static function editor_settings($editor_id, $set) {
		...
		if ( self::$this_tinymce ) {
			...
			if ( empty(self::$first_init) ) {
				...
				*/
				...
				self::$first_init = array(
					...
					'nxteditimage_disable_captions' => $no_captions,
					'nxt_fullscreen_content_css' => self::$baseurl . '/plugins/nxtfullscreen/css/nxt-fullscreen.css',
			...
			$mceInit = array (
				'elements' => $editor_id,
				'nxtautop' => (bool) $set['nxtautop'],
				'remove_linebreaks' => (bool) $set['nxtautop'],
				'apply_source_formatting' => (bool) !$set['nxtautop'],
...
?>
	...
	<script type="text/javascript">
		(function(){
			var init, ed, qt, first_init, mce = <?php echo nxt_default_editor() == 'tinymce' ? 'true' : 'false'; ?>;
...
<?php
	...
	public static function nxt_fullscreen_html() {
	...
	?>
	<div id="nxt-fullscreen-body">
	<div id="fullscreen-topbar">
		<div id="nxt-fullscreen-toolbar">
			<div id="nxt-fullscreen-close"><a href="#" onclick="fullscreen.off();return false;"><?php _e('Exit fullscreen'); ?></a></div>
			<div id="nxt-fullscreen-central-toolbar" style="width:<?php echo $width; ?>px;">

			<div id="nxt-fullscreen-mode-bar"><div id="nxt-fullscreen-modes">
			...
			<div id="nxt-fullscreen-button-bar"><div id="nxt-fullscreen-buttons" class="nxt_themeSkin">
	<?php
		...
		$buttons = apply_filters( 'nxt_fullscreen_buttons', $buttons );
	...
	<?php		continue;
			...
			<div<?php if ( $args['both'] ) { ?> class="nxt-fullscreen-both"<?php } ?>>
			<a title="<?php echo $args['title']; ?>" onclick="<?php echo $args['onclick']; ?>return false;" class="mceButton mceButtonEnabled mce_<?php echo $button; ?>" href="#" id="nxt_fs_<?php echo $button; ?>" role="button" aria-pressed="false">
	...
	<div id="nxt-fullscreen-wrap" style="width:<?php echo $dfw_width; ?>px;">
		<?php if ( post_type_supports($post->post_type, 'title') ) { ?>
		<label id="nxt-fullscreen-title-prompt-text" for="nxt-fullscreen-title"><?php echo apply_filters( 'enter_title_here', __( 'Enter title here' ), $post ); ?></label>
		<input type="text" id="nxt-fullscreen-title" value="" autocomplete="off" />
		<?php } ?>

		<div id="nxt-fullscreen-container">
			<textarea id="nxt_mce_fullscreen"></textarea>
		</div>

		<div id="nxt-fullscreen-status">
			<div id="nxt-fullscreen-count"><?php printf( __( 'Word count: %s' ), '<span class="word-count">0</span>' ); ?></div>
			<div id="nxt-fullscreen-tagline"><?php _e('Just write.'); ?></div>
		</div>
	</div>
	</div>
	...
	<?php nxt_nonce_field( 'internal-linking', '_ajax_linking_nonce', false ); ?>
	<div id="link-selector">
		...
		<?php $show_internal = '1' == get_user_setting( 'nxtlink', '0' ); ?>
	...
	<div class="submitbox">
		...
		<div id="nxt-link-update">
			<input type="submit" tabindex="100" value="<?php esc_attr_e( 'Add Link' ); ?>" class="button-primary" id="nxt-link-submit" name="nxt-link-submit">
		</div>
	</div>
	</form>
	</div>
