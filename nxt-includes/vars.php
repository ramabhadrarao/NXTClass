<?php
/**
 * Creates common globals for the rest of NXTClass
 *
 * Sets $pagenow global which is the current page. Checks
 * for the browser to set which one is currently being used.
 *
 * Detects which user environment NXTClass is being used on.
...
if ( is_admin() ) {
	// nxt-admin pages are checked more carefully
	if ( is_network_admin() )
		preg_match('#/nxt-admin/network/?(.*?)$#i', $_SERVER['PHP_SELF'], $self_matches);
	elseif ( is_user_admin() )
		preg_match('#/nxt-admin/user/?(.*?)$#i', $_SERVER['PHP_SELF'], $self_matches);
	else
		preg_match('#/nxt-admin/?(.*?)$#i', $_SERVER['PHP_SELF'], $self_matches);
	...
	if ( '' === $pagenow || 'index' === $pagenow || 'index.php' === $pagenow ) {
		...
		if ( '.php' !== substr($pagenow, -4, 4) )
			$pagenow .= '.php'; // for Options +Multiviews: /nxt-admin/themes/index.php (themes.php is queried)
	}
} else {
