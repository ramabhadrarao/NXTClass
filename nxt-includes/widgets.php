 ...
 * themes. Includes both internal NXTClass routines and theme use routines.
 *
...
/**
 * This class must be extended for each widget and nxt_Widget::widget(), nxt_Widget::update()
 * and nxt_Widget::form() need to be over-ridden.
 *
 * @package NXTClass
 * @subpackage Widgets
 * @since 2.8
...
class nxt_Widget {
	...
	var $widget_options;	// Option array passed to nxt_register_sidebar_widget()
	var $control_options;	// Option array passed to nxt_register_widget_control()
	...
	function widget($args, $instance) {
		die('function nxt_Widget::widget() must be over-ridden in a sub-class.');
	}
	 ...
	 * @param array $widget_options Optional Passed to nxt_register_sidebar_widget()
	 *	 - description: shown on the configuration page
	 *	 - classname
	 * @param array $control_options Optional Passed to nxt_register_widget_control()
	 *	 - width: required if more than 250px
	 *	 - height: currently not used but may be needed in the future
	 */
	function __construct( $id_base = false, $name, $widget_options = array(), $control_options = array() ) {
		$this->id_base = empty($id_base) ? preg_replace( '/(nxt_)?widget_/', '', strtolower(get_class($this)) ) : strtolower($id_base);
		$this->name = $name;
		$this->option_name = 'widget_' . $this->id_base;
		$this->widget_options = nxt_parse_args( $widget_options, array('classname' => $this->option_name) );
		$this->control_options = nxt_parse_args( $control_options, array('id_base' => $this->id_base) );
	}
	...
	function display_callback( $args, $widget_args = 1 ) {
		...
		$widget_args = nxt_parse_args( $widget_args, array( 'number' => -1 ) );
	...
	function update_callback( $widget_args = 1 ) {
		...
		if ( isset($_POST['delete_widget']) && $_POST['delete_widget'] ) {
			...
			if ( isset($nxt_registered_widgets[$del_id]['params'][0]['number']) ) {
				$number = $nxt_registered_widgets[$del_id]['params'][0]['number'];
	...
	function get_settings() {
		...
		if ( !empty($settings) && !array_key_exists('_multiwidget', $settings) ) {
			...
			$settings = nxt_convert_widget_settings($this->id_base, $this->option_name, $settings);
		}
...
/**
 * Singleton that registers and instantiates nxt_Widget classes.
 *
 * @package NXTClass
 * @subpackage Widgets
 * @since 2.8
 */
class nxt_Widget_Factory {
	var $widgets = array();

	function nxt_Widget_Factory() {
	...
	function _register_widgets() {
		...
		$registered = array_keys($nxt_registered_widgets);
...
/** @ignore */
global $nxt_registered_sidebars, $nxt_registered_widgets, $nxt_registered_widget_controls, $nxt_registered_widget_updates;

/**
 * Stores the sidebars, since many themes can have more than one.
 *
 * @global array $nxt_registered_sidebars
 * @since 2.2.0
 */
$nxt_registered_sidebars = array();

/**
 * Stores the registered widgets.
 *
 * @global array $nxt_registered_widgets
 * @since 2.2.0
 */
$nxt_registered_widgets = array();

/**
 * Stores the registered widget control (options).
 *
 * @global array $nxt_registered_widget_controls
 * @since 2.2.0
 */
$nxt_registered_widget_controls = array();
$nxt_registered_widget_updates = array();

/**
 * Private
 */
$_nxt_sidebars_widgets = array();

/**
 * Private
 */
 $GLOBALS['_nxt_deprecated_widgets_callbacks'] = array(
 	'nxt_widget_pages',
	'nxt_widget_pages_control',
	'nxt_widget_calendar',
	'nxt_widget_calendar_control',
	'nxt_widget_archives',
	'nxt_widget_archives_control',
	'nxt_widget_links',
	'nxt_widget_meta',
	'nxt_widget_meta_control',
	'nxt_widget_search',
	'nxt_widget_recent_entries',
	'nxt_widget_recent_entries_control',
	'nxt_widget_tag_cloud',
	'nxt_widget_tag_cloud_control',
	'nxt_widget_categories',
	'nxt_widget_categories_control',
	'nxt_widget_text',
	'nxt_widget_text_control',
	'nxt_widget_rss',
	'nxt_widget_rss_control',
	'nxt_widget_recent_comments',
	'nxt_widget_recent_comments_control'
 );
 ...
 * Register a widget
 *
 * Registers a nxt_Widget widget
 *
 * @since 2.8.0
 *
 * @see nxt_Widget
 * @see nxt_Widget_Factory
 * @uses nxt_Widget_Factory
 *
 * @param string $widget_class The name of a class that extends nxt_Widget
 */
function register_widget($widget_class) {
	global $nxt_widget_factory;

	$nxt_widget_factory->register($widget_class);
}

/**
 * Unregister a widget
 *
 * Unregisters a nxt_Widget widget. Useful for unregistering default widgets.
...
function unregister_widget($widget_class) {
	...
	$nxt_widget_factory->unregister($widget_class);
}
...
function register_sidebars($number = 1, $args = array()) {
	...
	for ( $i = 1; $i <= $number; $i++ ) {
		...
		if ( isset($args['id']) ) {
			...
			while ( isset($nxt_registered_sidebars[$_args['id']]) )
				$_args['id'] = $args['id'] . '-' . $n++;
		} else {
			$n = count($nxt_registered_sidebars);
			do {
				$_args['id'] = 'sidebar-' . ++$n;
			} while ( isset($nxt_registered_sidebars[$_args['id']]) );
		}
 ...
 * for NXTClass to create the defaults for you.
 *
 ...
 *
 * @since 2.2.0
 * @uses $nxt_registered_sidebars Stores the new sidebar in this array by sidebar ID.
 *
 * @param string|array $args Builds Sidebar based off of 'name' and 'id' values
...
function register_sidebar($args = array()) {
	...
	$i = count($nxt_registered_sidebars) + 1;
	...
	$sidebar = nxt_parse_args( $args, $defaults );

	$nxt_registered_sidebars[$sidebar['id']] = $sidebar;
...
function unregister_sidebar( $name ) {
	...
	if ( isset( $nxt_registered_sidebars[$name] ) )
		unset( $nxt_registered_sidebars[$name] );
}
 ...
 * @since 2.2.0
 *
 * @uses $nxt_registered_widgets Uses stored registered widgets.
 * @uses $nxt_register_widget_defaults Retrieves widget defaults.
 *
 ...
 */
function nxt_register_sidebar_widget($id, $name, $output_callback, $options = array()) {
	global $nxt_registered_widgets, $nxt_registered_widget_controls, $nxt_registered_widget_updates, $_nxt_deprecated_widgets_callbacks;

	$id = strtolower($id);

	if ( empty($output_callback) ) {
		unset($nxt_registered_widgets[$id]);
		return;
	}

	$id_base = _get_widget_id_base($id);
	if ( in_array($output_callback, $_nxt_deprecated_widgets_callbacks, true) && !is_callable($output_callback) ) {
		if ( isset($nxt_registered_widget_controls[$id]) )
			unset($nxt_registered_widget_controls[$id]);

		if ( isset($nxt_registered_widget_updates[$id_base]) )
			unset($nxt_registered_widget_updates[$id_base]);
	...
	if ( is_callable($output_callback) && ( !isset($nxt_registered_widgets[$id]) || did_action( 'widgets_init' ) ) ) {
		do_action( 'nxt_register_sidebar_widget', $widget );
		$nxt_registered_widgets[$id] = $widget;
	}
}
 ...
 */
function nxt_widget_description( $id ) {
	if ( !is_scalar($id) )
		return;

	global $nxt_registered_widgets;

	if ( isset($nxt_registered_widgets[$id]['description']) )
		return esc_html( $nxt_registered_widgets[$id]['description'] );
}
 ...
 */
function nxt_sidebar_description( $id ) {
	if ( !is_scalar($id) )
		return;

	global $nxt_registered_sidebars;

	if ( isset($nxt_registered_sidebars[$id]['description']) )
		return esc_html( $nxt_registered_sidebars[$id]['description'] );
}
 ...
 * @param int|string $id Widget ID.
 */
function nxt_unregister_sidebar_widget($id) {
	do_action( 'nxt_unregister_sidebar_widget', $id );

	nxt_register_sidebar_widget($id, '', '');
	nxt_unregister_widget_control($id);
}
 ...
 */
function nxt_register_widget_control($id, $name, $control_callback, $options = array()) {
	global $nxt_registered_widget_controls, $nxt_registered_widget_updates, $nxt_registered_widgets, $_nxt_deprecated_widgets_callbacks;
	...
	if ( in_array($control_callback, $_nxt_deprecated_widgets_callbacks, true) && !is_callable($control_callback) ) {
	...
	if ( isset($nxt_registered_widget_controls[$id]) && !did_action( 'widgets_init' ) )
		return;
	...
	$nxt_registered_widget_controls[$id] = $widget;
	...
	$nxt_registered_widget_updates[$id_base] = $widget;
}

function _register_widget_update_callback($id_base, $update_callback, $options = array()) {
	global $nxt_registered_widget_updates;
...
function _register_widget_form_callback($id, $name, $form_callback, $options = array()) {
	global $nxt_registered_widget_controls;
 ...
 *
 * @since 2.2.0
 * @uses nxt_register_widget_control() Unregisters by using empty callback.
 *
 * @param int|string $id Widget ID.
 */
function nxt_unregister_widget_control($id) {
	return nxt_register_widget_control($id, '', '');
}
 ...
 * it as an oddity of NXTClass sidebar register and display.
 *
 * @since 2.2.0
 *
...
function dynamic_sidebar($index = 1) {
	global $nxt_registered_sidebars, $nxt_registered_widgets;

	if ( is_int($index) ) {
		$index = "sidebar-$index";
	} else {
		$index = sanitize_title($index);
		foreach ( (array) $nxt_registered_sidebars as $key => $value ) {
	...
	if ( empty($nxt_registered_sidebars[$index]) || !array_key_exists($index, $sidebars_widgets) || !is_array($sidebars_widgets[$index]) || empty($sidebars_widgets[$index]) )
		return false;

	$sidebar = $nxt_registered_sidebars[$index];

	$did_one = false;
	foreach ( (array) $sidebars_widgets[$index] as $id ) {

		if ( !isset($nxt_registered_widgets[$id]) ) continue;

		$params = array_merge(
			array( array_merge( $sidebar, array('widget_id' => $id, 'widget_name' => $nxt_registered_widgets[$id]['name']) ) ),
			(array) $nxt_registered_widgets[$id]['params']
		);

		// Substitute HTML id and class attributes into before_widget
		$classname_ = '';
		foreach ( (array) $nxt_registered_widgets[$id]['classname'] as $cn ) {
			if ( is_string($cn) )
				$classname_ .= '_' . $cn;
			elseif ( is_object($cn) )
		...
		$callback = $nxt_registered_widgets[$id]['callback'];

		do_action( 'dynamic_sidebar', $nxt_registered_widgets[$id] );
 ...
 * $id_base is the first argument when extending nxt_Widget class
 ...
 * @param string $id_base Optional, the base ID of a widget created by extending nxt_Widget.
 * @param bool $skip_inactive Optional, whether to check in 'nxt_inactive_widgets'.
...
function is_active_widget($callback = false, $widget_id = false, $id_base = false, $skip_inactive = true) {
	...
	if ( is_array($sidebars_widgets) ) {
		foreach ( $sidebars_widgets as $sidebar => $widgets ) {
			if ( $skip_inactive && 'nxt_inactive_widgets' == $sidebar )
				continue;

			if ( is_array($widgets) ) {
				foreach ( $widgets as $widget ) {
					if ( ( $callback && isset($nxt_registered_widgets[$widget]['callback']) && $nxt_registered_widgets[$widget]['callback'] == $callback ) || ( $id_base && _get_widget_id_base($widget) == $id_base ) ) {
						if ( !$widget_id || $widget_id == $nxt_registered_widgets[$widget]['id'] )
							return $sidebar;
					}
				}
			}
...
function is_dynamic_sidebar() {
	global $nxt_registered_widgets, $nxt_registered_sidebars;
	$sidebars_widgets = get_option('sidebars_widgets');
	foreach ( (array) $nxt_registered_sidebars as $index => $sidebar ) {
		if ( count($sidebars_widgets[$index]) ) {
			foreach ( (array) $sidebars_widgets[$index] as $widget )
				if ( array_key_exists($widget, $nxt_registered_widgets) )
					return true;
		}
	}
	return false;
 ...
 */
function nxt_get_sidebars_widgets($deprecated = true) {
	if ( $deprecated !== true )
		_deprecated_argument( __FUNCTION__, '2.8.1' );

	global $nxt_registered_widgets, $_nxt_sidebars_widgets, $sidebars_widgets;

	// If loading from front page, consult $_nxt_sidebars_widgets rather than options
	// to see if nxt_convert_widget_settings() has made manipulations in memory.
	if ( !is_admin() ) {
		if ( empty($_nxt_sidebars_widgets) )
			$_nxt_sidebars_widgets = get_option('sidebars_widgets', array());

		$sidebars_widgets = $_nxt_sidebars_widgets;
	} else {
 ...
 */
function nxt_set_sidebars_widgets( $sidebars_widgets ) {
 ...
 * @return array
 */
function nxt_get_widget_defaults() {
	global $nxt_registered_sidebars;
 ...
 * @return array
 */
function nxt_convert_widget_settings($base_name, $option_name, $settings) {
	...
	if ( $single ) {
		...
		if ( is_admin() ) {
			...
			if ( empty($GLOBALS['_nxt_sidebars_widgets']) )
				$GLOBALS['_nxt_sidebars_widgets'] = get_option('sidebars_widgets', array());
			$sidebars_widgets = &$GLOBALS['_nxt_sidebars_widgets'];
		}
...
function the_widget($widget, $instance = array(), $args = array()) {
	...
	$widget_obj = $nxt_widget_factory->widgets[$widget];
	if ( !is_a($widget_obj, 'nxt_Widget') )
		return;
	...
	$instance = nxt_parse_args($instance);
 ...
 * @since 3.3
 */
function _nxt_sidebars_changed() {
...
function retrieve_widgets($theme_changed = false) {
	...
	if ( is_array( $old_sidebars_widgets ) ) {
		...
		$_sidebars_widgets = array(
			'nxt_inactive_widgets' => !empty( $sidebars_widgets['nxt_inactive_widgets'] ) ? $sidebars_widgets['nxt_inactive_widgets'] : array()
		);
