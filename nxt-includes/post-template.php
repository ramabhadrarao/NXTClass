<?php
/**
 * NXTClass Post Template Functions.
 *
...
/**
 * Display the ID of the current item in the NXTClass Loop.
 *
 * @since 0.71
 */
function the_ID() {
...
/**
 * Retrieve the ID of the current item in the NXTClass Loop.
 *
 * @since 2.1.0
 * @uses $post
 *
...
function get_post_class( $class = '', $post_id = null ) {
	...
	if ( post_type_supports( $post->post_type, 'post-formats' ) ) {
		...
		if ( $post_format && !is_nxt_error($post_format) )
...
function get_body_class( $class = '' ) {
	...
	if ( is_single() ) {
		...
		if ( is_post_type_archive() ) {
			...
			$author = $nxt_query->get_queried_object();
			$classes[] = 'author';
			$classes[] = 'author-' . sanitize_html_class( $author->user_nicename , $author->ID );
			$classes[] = 'author-' . $author->ID;
		} elseif ( is_category() ) {
			$cat = $nxt_query->get_queried_object();
			$classes[] = 'category';
			$classes[] = 'category-' . sanitize_html_class( $cat->slug, $cat->term_id );
			$classes[] = 'category-' . $cat->term_id;
		} elseif ( is_tag() ) {
			$tags = $nxt_query->get_queried_object();
			$classes[] = 'tag';
		...
		$page_id = $nxt_query->get_queried_object_id();

		$post = get_page($page_id);

		$classes[] = 'page-id-' . $page_id;

		if ( $nxtdb->get_var( $nxtdb->prepare("SELECT ID FROM $nxtdb->posts WHERE post_parent = %d AND post_type = 'page' AND post_status = 'publish' LIMIT 1", $page_id) ) )
			$classes[] = 'page-parent';
		...
		if ( is_page_template() ) {
			...
			$classes[] = 'page-template-' . sanitize_html_class( str_replace( '.', '-', get_post_meta( $page_id, '_nxt_page_template', true ) ), '' );
		} else {
			$classes[] = 'page-template-default';
		}
	} elseif ( is_search() ) {
		if ( !empty( $nxt_query->posts ) )
	...
	$page = $nxt_query->get( 'page' );

	if ( !$page || $page < 2)
		$page = $nxt_query->get( 'paged' );
...
function post_password_required( $post = null ) {
	...
	if ( !isset($_COOKIE['nxt-postpass_' . COOKIEHASH]) )
		return true;

	if ( stripslashes( $_COOKIE['nxt-postpass_' . COOKIEHASH] ) != $post->post_password )
		return true;
...
function nxt_link_pages($args = '') {
	...
	$r = nxt_parse_args( $args, $defaults );
	$r = apply_filters( 'nxt_link_pages_args', $r );
	extract( $r, EXTR_SKIP );
...
/**
 * Helper function for nxt_link_pages().
 *
 * @since 3.1.0
 * @access private
 *
...
function _nxt_link_page( $i ) {
	...
	if ( 1 == $i ) {
		...
		elseif ( 'page' == get_option('show_on_front') && get_option('page_on_front') == $post->ID )
			$url = trailingslashit(get_permalink()) . user_trailingslashit("$nxt_rewrite->pagination_base/" . $i, 'single_paged');
		else
 ...
 */
function nxt_dropdown_pages($args = '') {
	$defaults = array(
		'depth' => 0, 'child_of' => 0,
		'selected' => 0, 'echo' => 1,
		'name' => 'page_id', 'id' => '',
	...
	$output = apply_filters('nxt_dropdown_pages', $output);
...
function nxt_list_pages($args = '') {
	...
	$r['exclude'] = implode( ',', apply_filters('nxt_list_pages_excludes', $exclude_array) );
	...
	if ( !empty($pages) ) {
		...
		global $nxt_query;
		if ( is_page() || is_attachment() || $nxt_query->is_posts_page )
			$current_page = $nxt_query->get_queried_object_id();
	...
	$output = apply_filters('nxt_list_pages', $output, $r);
...
function the_attachment_link( $id = 0, $fullsize = false, $deprecated = false, $permalink = false ) {
	...
	if ( $fullsize )
		echo nxt_get_attachment_link($id, 'full', $permalink);
	else
		echo nxt_get_attachment_link($id, 'thumbnail', $permalink);
}
...
function get_the_password_form() {
	...
	$output = '<form action="' . get_option('siteurl') . '/nxt-pass.php" method="post">
...
function is_page_template($template = '') {
	...
	$page = $nxt_query->get_queried_object();
	$custom_fields = get_post_custom_values('_nxt_page_template',$page->ID);
