<?php
/**
 * Deprecated functions from past NXTClass versions. You shouldn't use these
...
function start_nxt() {
	...
	_deprecated_function( __FUNCTION__, '1.5', __('new NXTClass Loop') );
 ...
 * @return bool|null
 */
function nxt_get_linksbyname($category, $args = '') {
 ...
 * $nxtdb->linkcategories and output it as a nested HTML unordered list.
 *
 * @since 1.0.1
 * @deprecated 2.1
 * @deprecated Use nxt_list_bookmarks()
...
function links_popup_script($text = 'Links', $width=400, $height=400, $file='links.all.php', $count = true) {
	...
	if ( $count )
		$counts = $nxtdb->get_var("SELECT COUNT(*) FROM $nxtdb->links");
...
function get_alloptions() {
	_deprecated_function( __FUNCTION__, '3.0', 'nxt_load_alloptions()' );
	return nxt_load_alloptions();
}
 ...
 * @since 2.0.0
 * @deprecated 2.5.0
 * @deprecated Use nxt_get_attachment_link()
 * @see nxt_get_attachment_link()
 *
...
function get_the_attachment_link($id = 0, $fullsize = false, $max_dims = false, $permalink = false) {
	_deprecated_function( __FUNCTION__, '2.5', 'nxt_get_attachment_link()' );
	$id = (int) $id;
	$_post = & get_post($id);

	if ( ('attachment' != $_post->post_type) || !$url = nxt_get_attachment_url($_post->ID) )
 ...
 * @since 2.0.0
 * @deprecated 2.5.0
 * @deprecated Use nxt_get_attachment_image()
 * @see nxt_get_attachment_image()
 *
 * @param int $id Optional. Post ID.
...
function get_attachment_icon( $id = 0, $fullsize = false, $max_dims = false ) {
	_deprecated_function( __FUNCTION__, '2.5', 'nxt_get_attachment_image()' );
	$id = (int) $id;
	if ( !$post = & get_post($id) )
		return false;
 ...
 *
 * Passes to {@link nxt_register_sidebar_widget()} after argument list and
 * backwards compatibility is complete.
 *
 * @since 2.2.0
 * @deprecated 2.8.0
 * @deprecated Use nxt_register_sidebar_widget()
 * @see nxt_register_sidebar_widget()
 *
...
function register_sidebar_widget($name, $output_callback, $classname = '') {
	_deprecated_function( __FUNCTION__, '2.8', 'nxt_register_sidebar_widget()' );
	// Compat
	if ( is_array($name) ) {
		if ( count($name) == 3 )
	...
	call_user_func_array('nxt_register_sidebar_widget', $args);
}

/**
 * Alias of {@link nxt_unregister_sidebar_widget()}.
 *
 * @since 2.2.0
 * @deprecated 2.8.0
 * @deprecated Use nxt_unregister_sidebar_widget()
 * @see nxt_unregister_sidebar_widget()
 *
 * @param int|string $id Widget ID.
 */
function unregister_sidebar_widget($id) {
	_deprecated_function( __FUNCTION__, '2.8', 'nxt_unregister_sidebar_widget()' );
	return nxt_unregister_sidebar_widget($id);
}
 ...
 * the array for the name.
 *
 * Passes to {@link nxt_register_widget_control()} after the argument list has
 * been compiled.
 *
 * @since 2.2.0
 * @deprecated 2.8.0
 * @deprecated Use nxt_register_widget_control()
 * @see nxt_register_widget_control()
 *
...
function register_widget_control($name, $control_callback, $width = '', $height = '') {
	_deprecated_function( __FUNCTION__, '2.8', 'nxt_register_widget_control()' );
	// Compat
	if ( is_array($name) ) {
		if ( count($name) == 3 )
			$name = sprintf($name[0], $name[2]);
	...
	call_user_func_array('nxt_register_widget_control', $args);
}

/**
 * Alias of {@link nxt_unregister_widget_control()}.
 *
 * @since 2.2.0
 * @deprecated 2.8.0
 * @deprecated Use nxt_unregister_widget_control()
 * @see nxt_unregister_widget_control()
 *
 * @param int|string $id Widget ID.
 */
function unregister_widget_control($id) {
	_deprecated_function( __FUNCTION__, '2.8', 'nxt_unregister_widget_control()' );
	return nxt_unregister_widget_control($id);
}
...
function delete_usermeta( $user_id, $meta_key, $meta_value = '' ) {
	...
	if ( ! empty($meta_value) )
		$nxtdb->query( $nxtdb->prepare("DELETE FROM $nxtdb->usermeta WHERE user_id = %d AND meta_key = %s AND meta_value = %s", $user_id, $meta_key, $meta_value) );
	else
		$nxtdb->query( $nxtdb->prepare("DELETE FROM $nxtdb->usermeta WHERE user_id = %d AND meta_key = %s", $user_id, $meta_key) );
...
function get_users_of_blog( $id = '' ) {
	...
	$users = $nxtdb->get_results( "SELECT user_id, user_id AS ID, user_login, display_name, user_email, meta_value FROM $nxtdb->users, $nxtdb->usermeta WHERE {$nxtdb->users}.ID = {$nxtdb->usermeta}.user_id AND meta_key = '{$blog_prefix}capabilities' ORDER BY {$nxtdb->usermeta}.user_id" );
	return $users;
}
...
function automatic_feed_links( $add = true ) {
	...
	if ( $add )
		...
		remove_action( 'nxt_head', 'feed_links_extra', 3 ); // Just do this yourself in 3.0+
}
 ...
 * @return bool
 */
function nxt_timezone_supported() {
 ...
 * @deprecated 3.3
 */
function nxt_admin_bar_dashboard_view_site_menu( $nxt_admin_bar ) {
	_deprecated_function( __FUNCTION__, '3.3' );
	...
	if ( 0 != $user_id ) {
		if ( is_admin() )
			$nxt_admin_bar->add_menu( array( 'id' => 'view-site', 'title' => __( 'Visit Site' ), 'href' => home_url() ) );
		elseif ( is_multisite() )
			$nxt_admin_bar->add_menu( array( 'id' => 'dashboard', 'title' => __( 'Dashboard' ), 'href' => get_dashboard_url( $user_id ) ) );
		else
			$nxt_admin_bar->add_menu( array( 'id' => 'dashboard', 'title' => __( 'Dashboard' ), 'href' => admin_url() ) );
	}
}
