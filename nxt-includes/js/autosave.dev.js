...
jQuery(document).ready( function($) {
	...
	window.onbeforeunload = function(){
		...
		if ( mce && !mce.isHidden() ) {
			...
			if ( fullscreen && fullscreen.settings.visible ) {
				title = $('#nxt-fullscreen-title').val();
	...
	doPreview = function() {
		$('input#nxt-preview').val('dopreview');
		$('form#post').attr('target', 'nxt-preview').submit().attr('target', '');
...
function autosave_parse_response(response) {
	var res = nxtAjax.parseAjaxResponse(response, 'autosave'), message = '', postID, sup;
...
function autosave_update_slug(post_id) {
	...
	if ( 'undefined' != makeSlugeditClickable && jQuery.isFunction(makeSlugeditClickable) && !jQuery('#edit-slug-box > *').size() ) {
		jQuery.post( ajaxurl, {
				...
				post_id: post_id,
				new_title: fullscreen && fullscreen.settings.visible ? jQuery('#nxt-fullscreen-title').val() : jQuery('#title').val(),
...
autosave = function() {
	...
	if ( rich && doAutoSave ) {
		...
		if ( ed.plugins.spellchecker && ed.plugins.spellchecker.active ) {
			...
			if ( 'mce_fullscreen' == ed.id || 'nxt_mce_fullscreen' == ed.id )
	...
	if ( fullscreen && fullscreen.settings.visible ) {
		post_data["post_title"] = jQuery('#nxt-fullscreen-title').val() || '';
		post_data["content"] = jQuery("#nxt_mce_fullscreen").val() || '';
	} else {
	...
	if ( doAutoSave ) {
		...
		jQuery(document).triggerHandler('nxtcountwords', [ post_data["content"] ]);
	} else {
