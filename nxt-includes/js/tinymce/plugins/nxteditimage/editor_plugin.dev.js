...
(function() {
	tinymce.create('tinymce.plugins.nxtEditImage', {
		...
		init : function(ed, url) {
			...
			ed.addCommand('nxt_EditImage', function() {
				var el = ed.selection.getNode(), vp = tinymce.DOM.getVienxtort(), H = vp.h, W = ( 720 < vp.w ) ? 720 : vp.w, cls = ed.dom.getAttrib(el, 'class');

				if ( cls.indexOf('mceItem') != -1 || cls.indexOf('nxtGallery') != -1 || el.nodeName != 'IMG' )
					return;
			...
			ed.onInit.add(function(ed) {
				tinymce.dom.Event.add(ed.getBody(), 'dragstart', function(e) {
					if ( !tinymce.isGecko && e.target.nodeName == 'IMG' && ed.dom.getParent(e.target, 'dl.nxt-caption') )
			...
			ed.onMouseDown.add(function(ed, e) {
				...
				if ( e.target && ( e.target.nodeName == 'IMG' || (e.target.firstChild && e.target.firstChild.nodeName == 'IMG') ) ) {
					...
					if ( ed.dom.getAttrib(e.target, 'class').indexOf('mceItem') == -1 )
						ed.plugins.nxtclass._showButtons(e.target, 'nxt_editbtns');
				}
			});
		...
		_do_shcode : function(co) {
			return co.replace(/(?:<p>)?\[(?:nxt_)?caption([^\]]+)\]([\s\S]+?)\[\/(?:nxt_)?caption\](?:<\/p>)?[\s\u00a0]*/g, function(a,b,c){
				...
				return '<div class="'+div_cls+'" draggable><dl id="'+id+'" class="nxt-caption '+cls+'" style="width: '+(10+parseInt(w))+
				'px"><dt class="nxt-caption-dt">'+c+'</dt><dd class="nxt-caption-dd">'+cap+'</dd></dl></div>';
			});
		},
		...
		_createButtons : function() {
			...
			editButton = DOM.add('nxt_editbtns', 'img', {
				src : t.url+'/img/image.png',
				id : 'nxt_editimgbtn',
				width : '24',
				height : '24',
				title : ed.getLang('nxteditimage.edit_img')
			});

			tinymce.dom.Event.add(editButton, 'mousedown', function(e) {
				var ed = tinyMCE.activeEditor;
				ed.windowManager.bookmark = ed.selection.getBookmark('simple');
				ed.execCommand("nxt_EditImage");
			});

			dellButton = DOM.add('nxt_editbtns', 'img', {
				src : t.url+'/img/delete.png',
				id : 'nxt_delimgbtn',
				width : '24',
				height : '24',
				title : ed.getLang('nxteditimage.del_img')
			});
	...
	tinymce.PluginManager.add('nxteditimage', tinymce.plugins.nxtEditImage);
})();
