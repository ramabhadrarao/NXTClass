...
var tinymce = null, tinyMCEPopup, tinyMCE, nxtImage;
...
nxtImage = {
	...
	img_seturl : function(u) {
		...
		if ( 'current' == u ) {
			...
			if ( rel ) {
				rel = rel.replace( /attachment|nxt-att-[0-9]+/gi, '' );
	...
	init : function() {
		...
		window.setTimeout( function(){nxtImage.setup();}, 500 );
	},
	...
	setup : function() {
		...
		if ( tinyMCEPopup.editor.getParam('nxteditimage_disable_captions', false) )
		...
		if ( DL = dom.getParent(el, 'dl') ) {
			...
			tinymce.each(DL.childNodes, function(e) {
				if ( e.nodeName == 'DD' && dom.hasClass(e, 'nxt-caption-dd') ) {
					caption = e.innerHTML;
					return;
				}
			});
	...
	update : function() {
		...
		if ( do_caption ) {
			...
			if ( DL ) {
				...
				if ( (DT = ed.dom.getParent(el, 'dt')) && (DD = DT.nextSibling) && ed.dom.hasClass(DD, 'nxt-caption-dd') )
					ed.dom.setHTML(DD, f.img_cap.value);

			} else {
				if ( (id = f.img_classes.value.match( /nxt-image-([0-9]{1,6})/ )) && id[1] )
				...
				html = '<dl id="'+cap_id+'" class="nxt-caption '+t.align+'" style="width: '+cap_width+
				'px"><dt class="nxt-caption-dt">'+html+'</dt><dd class="nxt-caption-dd">'+f.img_cap.value+'</dd></dl>';
	...
	updateImageData : function() {
		var f = document.forms[0], t = nxtImage, w = f.width.value, h = f.height.value;
	...
	getImageData : function() {
		var t = nxtImage, f = document.forms[0];
...
window.onload = function(){nxtImage.init();}
nxtImage.preInit();
