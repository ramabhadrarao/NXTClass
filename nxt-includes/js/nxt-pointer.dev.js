...
(function($){
	...
	$.widget("nxt.pointer", {
		options: {
			pointerClass: 'nxt-pointer',
			pointerWidth: 320,
			content: function( respond, event, t ) {
				return $(this).text();
			},
			buttons: function( event, t ) {
				var close  = ( nxtPointerL10n ) ? nxtPointerL10n.dismiss : 'Dismiss',
		...
		_create: function() {
			...
			this.content = $('<div class="nxt-pointer-content"></div>');
			this.arrow   = $('<div class="nxt-pointer-arrow"><div class="nxt-pointer-arrow-inner"></div></div>');
			...
			this.pointer = $('<div />')
				.append( this.content )
				.append( this.arrow )
				.attr('id', 'nxt-pointer-' + identifier++)
		...
		_update: function( event, content ) {
			...
			if ( buttons ) {
				buttons.wrap('<div class="nxt-pointer-buttons" />').parent().appendTo( this.content );
			}
		...
		repoint: function() {
			...
			this.pointer[0].className = this.pointer[0].className.replace( /nxt-pointer-[^\s'"]*/, '' );

			// Add arrow class.
			this.pointer.addClass( 'nxt-pointer-' + edge );
		},
