...
if ( typeof(jQuery) != 'undefined' ) {
	...
	jQuery(document).ready(function($){
		...
		$('#nxtadminbar').removeClass('nojq').removeClass('nojs').find('li.menupop').hoverIntent({
			over: function(e){
				$(this).addClass('hover');
			},
			out: function(e){
		...
		$('#nxt-admin-bar-get-shortlink').click(function(e){
			e.preventDefault();
			$(this).addClass('selected').children('.shortlink-input').blur(function(){
				$(this).parents('#nxt-admin-bar-get-shortlink').removeClass('selected');
			}).focus().select();
		});

		$('#nxtadminbar li.menupop > .ab-item').bind('keydown.adminbar', function(e){
			if ( e.which != 13 )
				return;
		...
		$('#nxtadminbar .ab-item').bind('keydown.adminbar', function(e){
			if ( e.which != 27 )
				return;
	...
	(function(d, w) {
		...
		clickShortlink = function(e) {
			...
			while ( true ) {
				...
				if ( t.id && t.id == 'nxt-admin-bar-get-shortlink' )
					break;
				t = t.parentNode;
			}
		...
		addEvent(w, 'load', function() {
			aB = d.getElementById('nxtadminbar');
