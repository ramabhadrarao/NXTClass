	...
	if ( max > hundredmb && file.size > hundredmb ) {
		setTimeout(function(){
			if ( file.status == 2 && file.loaded == 0 ) { // not uploading
				nxtFileError(file, pluploadL10n.big_upload_failed.replace('%1$s', '<a class="uploader-html" href="#">').replace('%2$s', '</a>'));
	...
	jQuery('a.delete', item).click(function(){
		...
		jQuery.ajax({
			...
			data: {
				...
				action : 'trash-post',
				_ajax_nonce : this.href.replace(/^.*nxtnonce=/,'')
			}
		});
		return false;
	});
...
function nxtQueueError(message) {
	jQuery('#media-upload-error').show().html( '<div class="error"><p>' + message + '</p></div>' );
}

// file-specific error messages
function nxtFileError(fileObj, message) {
...
function uploadError(fileObj, errorCode, message, uploader) {
	...
	switch (errorCode) {
		case plupload.FAILED:
			nxtFileError(fileObj, pluploadL10n.upload_failed);
			break;
		case plupload.FILE_EXTENSION_ERROR:
			nxtFileError(fileObj, pluploadL10n.invalid_filetype);
			break;
		case plupload.FILE_SIZE_ERROR:
			uploadSizeError(uploader, fileObj);
			break;
		case plupload.IMAGE_FORMAT_ERROR:
			nxtFileError(fileObj, pluploadL10n.not_an_image);
			break;
		case plupload.IMAGE_MEMORY_ERROR:
			nxtFileError(fileObj, pluploadL10n.image_memory_exceeded);
			break;
		case plupload.IMAGE_DIMENSIONS_ERROR:
			nxtFileError(fileObj, pluploadL10n.image_dimensions_exceeded);
			break;
		case plupload.GENERIC_ERROR:
			nxtQueueError(pluploadL10n.upload_failed);
			break;
		case plupload.IO_ERROR:
			max = parseInt(uploader.settings.max_file_size, 10);

			if ( max > hundredmb && fileObj.size > hundredmb )
				nxtFileError(fileObj, pluploadL10n.big_upload_failed.replace('%1$s', '<a class="uploader-html" href="#">').replace('%2$s', '</a>'));
			else
				nxtQueueError(pluploadL10n.io_error);
			break;
		case plupload.HTTP_ERROR:
			nxtQueueError(pluploadL10n.http_error);
			break;
		case plupload.INIT_ERROR:
			jQuery('.media-upload-form').addClass('html-uploader');
			break;
		case plupload.SECURITY_ERROR:
			nxtQueueError(pluploadL10n.security_error);
			break;
/*		case plupload.UPLOAD_ERROR.UPLOAD_STOPPED:
		case plupload.UPLOAD_ERROR.FILE_CANCELLED:
			...
			break;*/
		default:
			nxtFileError(fileObj, pluploadL10n.default_error);
	}
}
...
jQuery(document).ready(function($){
	...
	uploader_init = function() {
		uploader = new plupload.Uploader(nxtUploaderInit);
		...
		uploader.bind('Init', function(up) {
			...
			if ( up.features.dragdrop ) {
				...
				$('#drag-drop-area').bind('dragover.nxt-uploader', function(){ // dragenter doesn't fire right :(
					uploaddiv.addClass('drag-over');
				}).bind('dragleave.nxt-uploader, drop.nxt-uploader', function(){
					uploaddiv.removeClass('drag-over');
				});
			} else {
				uploaddiv.removeClass('drag-drop');
				$('#drag-drop-area').unbind('.nxt-uploader');
			}
		});
	...
	if ( typeof(nxtUploaderInit) == 'object' )
		uploader_init();
