 ...
 */
_deprecated_file( basename( __FILE__ ), '3.0', nxtINC . '/class-simplepie.php' );
...
define('RSS', 'RSS');
define('ATOM', 'Atom');
define('MAGPIE_USER_AGENT', 'NXTClass/' . $GLOBALS['nxt_version']);
...
/**
 * Retrieve URL headers and content using nxt HTTP Request API.
 *
 * @since 1.5.0
 * @package External
 * @subpackage MagpieRSS
...
function _fetch_remote_file($url, $headers = "" ) {
	$resp = nxt_remote_request($url, array('headers' => $headers, 'timeout' => MAGPIE_FETCH_TIME_OUT));
	if ( is_nxt_error($resp) ) {
	...
	// Also note, nxt_HTTP lowercases all keys, Snoopy did not.
	$return_headers = array();
	foreach ( nxt_remote_retrieve_headers( $resp ) as $key => $value ) {
		if ( !is_array($value) ) {
	...
	$response->status = nxt_remote_retrieve_response_code( $resp );
	$response->response_code = nxt_remote_retrieve_response_code( $resp );
	$response->headers = $return_headers;
	$response->results = nxt_remote_retrieve_body( $resp );
...
function init () {
	...
	if ( !defined('MAGPIE_USER_AGENT') ) {
		$ua = 'NXTClass/' . $GLOBALS['nxt_version'];
...
class RSSCache {
	...
	function RSSCache ($base='', $age='') {
		$this->BASE_CACHE = nxt_CONTENT_DIR . '/cache';
		if ( $base ) {
 ...
 */
function nxt_rss( $url, $num_items = -1 ) {
 ...
 * to display. You can't display all of them like you can with nxt_rss()
 * function.
 *
 * @since 1.5.0
 * @package External
