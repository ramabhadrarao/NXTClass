 ...
 *
 * These functions are meant to live inside of the NXTClass loop.
 *
 * @package NXTClass
 * @subpackage Template
 */
...
function get_comment_link( $comment = null, $args = array() ) {
	global $nxt_rewrite, $in_comment_loop;
...
function get_trackback_url() {
	if ( '' != get_option('permalink_structure') ) {
		...
		$tb_url = get_option('siteurl') . '/nxt-trackback.php?p=' . get_the_ID();
	}
 ...
 */
function nxt_comment_form_unfiltered_html_nonce() {
	global $post;
	...
	if ( current_user_can('unfiltered_html') )
		nxt_nonce_field('unfiltered-html-comment_' . $post_id, '_nxt_unfiltered_html_comment', false);
}
 ...
 *
 * @since 0.71
 * @global string $nxtcommentspopupfile The URL to use for the popup window
 * @global int $nxtcommentsjavascript Whether to use JavaScript. Set when function is called
 *
...
function comments_popup_script($width=400, $height=400, $file='') {
	global $nxtcommentspopupfile, $nxtcommentsjavascript;

	if (empty ($file)) {
		$nxtcommentspopupfile = '';  // Use the index.
	} else {
		$nxtcommentspopupfile = $file;
	}

	$nxtcommentsjavascript = 1;
	$javascript = "<script type='text/javascript'>\nfunction nxtopen (macagna) {\n    window.open(macagna, '_blank', 'width=$width,height=$height,scrollbars=yes,status=yes');\n}\n</script>\n";
	echo $javascript;
}
 ...
 *
 * @since 0.71
 * @uses $nxtcommentspopupfile
 * @uses $nxtcommentsjavascript
 * @uses $post
 *
...
function comments_popup_link( $zero = false, $one = false, $more = false, $css_class = '', $none = false ) {
	...
	echo '<a href="';
	if ( $nxtcommentsjavascript ) {
		if ( empty( $nxtcommentspopupfile ) )
			$home = home_url();
		else
			$home = get_option('siteurl');
		echo $home . '/' . $nxtcommentspopupfile . '?comments_popup=' . $id;
		echo '" onclick="nxtopen(this.href); return false"';
...
function get_comment_reply_link($args = array(), $comment = null, $post = null) {
	...
	if ( get_option('comment_registration') && !$user_ID )
		$link = '<a rel="nofollow" class="comment-reply-login" href="' . esc_url( nxt_login_url( get_permalink() ) ) . '">' . $login_text . '</a>';
	else
...
function get_post_reply_link($args = array(), $post = null) {
	...
	if ( get_option('comment_registration') && !$user_ID ) {
		$link = '<a rel="nofollow" href="' . nxt_login_url( get_permalink() ) . '">' . $login_text . '</a>';
	} else {
 ...
 */
function nxt_list_comments($args = array(), $comments = null ) {
	global $nxt_query, $comment_alt, $comment_depth, $comment_thread_alt, $overridden_cpage, $in_comment_loop;
	...
	if ( null !== $comments ) {
		...
		if ( empty($nxt_query->comments) )
			return;
		if ( 'all' != $r['type'] ) {
			if ( empty($nxt_query->comments_by_type) )
				$nxt_query->comments_by_type = &separate_comments($nxt_query->comments);
			if ( empty($nxt_query->comments_by_type[$r['type']]) )
				return;
			$_comments = $nxt_query->comments_by_type[$r['type']];
		} else {
			$_comments = $nxt_query->comments;
		}
	}
	...
	$nxt_query->max_num_comment_pages = $walker->max_pages;
...
function comment_form( $args = array(), $post_id = null ) {
	...
	$defaults = array(
		...
		'must_log_in'          => '<p class="must-log-in">' .  sprintf( __( 'You must be <a href="%s">logged in</a> to post a comment.' ), nxt_login_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>',
		'logged_in_as'         => '<p class="logged-in-as">' . sprintf( __( 'Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>' ), admin_url( 'profile.php' ), $user_identity, nxt_logout_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>',
	...
	?>
		<?php if ( comments_open() ) : ?>
			...
			<div id="respond">
				...
				<?php else : ?>
					<form action="<?php echo site_url( '/nxt-comments-post.php' ); ?>" method="post" id="<?php echo esc_attr( $args['id_form'] ); ?>">
