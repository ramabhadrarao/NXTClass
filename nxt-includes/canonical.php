<?php
/**
 * Canonical API to handle NXTClass Redirecting
 *
	...
	if ( is_404() ) {
		...
		if ( get_query_var('paged') || is_feed() || get_query_var('cpage') ) {
			while ( preg_match( "#/$nxt_rewrite->pagination_base/?[0-9]+?(/+)?$#", $redirect['path'] ) || preg_match( '#/(comments/?)?(feed|rss|rdf|atom|rss2)(/+)?$#', $redirect['path'] ) || preg_match( '#/comment-page-[0-9]+(/+)?$#', $redirect['path'] ) ) {
				...
				$redirect['path'] = preg_replace("#/$nxt_rewrite->pagination_base/?[0-9]+?(/+)?$#", '/', $redirect['path']); // strip off any existing paging
			...
			if ( is_feed() && in_array( get_query_var('feed'), $nxt_rewrite->feeds ) ) {
				...
				$old_feed_files = array(
					'nxt-atom.php'         => 'atom',
					'nxt-commentsrss2.php' => 'comments_rss2',
					'nxt-feed.php'         => get_default_feed(),
					'nxt-rdf.php'          => 'rdf',
					'nxt-rss.php'          => 'rss2',
					'nxt-rss2.php'         => 'rss2',
				);
...
function redirect_guess_404_permalink() {
	...
	$where = $nxtdb->prepare("post_name LIKE %s", like_escape( get_query_var('name') ) . '%');

	// if any of post_type, year, monthnum, or day are set, use them to refine the query
	if ( get_query_var('post_type') )
		$where .= $nxtdb->prepare(" AND post_type = %s", get_query_var('post_type'));
	if ( get_query_var('year') )
		$where .= $nxtdb->prepare(" AND YEAR(post_date) = %d", get_query_var('year'));
	if ( get_query_var('monthnum') )
		$where .= $nxtdb->prepare(" AND MONTH(post_date) = %d", get_query_var('monthnum'));
	if ( get_query_var('day') )
		$where .= $nxtdb->prepare(" AND DAYOFMONTH(post_date) = %d", get_query_var('day'));

	$post_id = $nxtdb->get_var("SELECT ID FROM $nxtdb->posts WHERE $where AND post_status = 'publish'");
	if ( !$post_id )
		return false;
