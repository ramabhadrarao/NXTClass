...
function get_header( $name = null ) {
	...
	if ('' == locate_template($templates, true))
		load_template( ABSPATH . nxtINC . '/theme-compat/header.php');
}
...
function get_footer( $name = null ) {
	...
	// Backward compat code will be removed in a future release
	if ('' == locate_template($templates, true))
		load_template( ABSPATH . nxtINC . '/theme-compat/footer.php');
}
...
function get_sidebar( $name = null ) {
	...
	// Backward compat code will be removed in a future release
	if ('' == locate_template($templates, true))
		load_template( ABSPATH . nxtINC . '/theme-compat/sidebar.php');
}
 ...
 * form into the sidebar and also by the search widget in NXTClass.
 *
 ...
 *
 * @since 2.7.0
 * @uses nxt_nonce_url() To protect against CSRF
 * @uses site_url() To generate the log in URL
 * @uses apply_filters() calls 'logout_url' hook on final logout url
 *
 * @param string $redirect Path to redirect to on logout.
 */
function nxt_logout_url($redirect = '') {
	$args = array( 'action' => 'logout' );
	if ( !empty($redirect) ) {
		$args['redirect_to'] = urlencode( $redirect );
	}

	$logout_url = add_query_arg($args, site_url('nxt-login.php', 'login'));
	$logout_url = nxt_nonce_url( $logout_url, 'log-out' );
 ...
 */
function nxt_login_url($redirect = '', $force_reauth = false) {
	$login_url = site_url('nxt-login.php', 'login');
...
/**
 * Provides a simple login form for use anywhere within NXTClass. By default, it echoes
 ...
 */
function nxt_login_form( $args = array() ) {
	$defaults = array( 'echo' => true,
						...
						'id_submit' => 'nxt-submit',
						'remember' => true,
						'value_username' => '',
						'value_remember' => false, // Set this to true to default the "Remember me" checkbox to checked
					);
	$args = nxt_parse_args( $args, apply_filters( 'login_form_defaults', $defaults ) );

	$form = '
		<form name="' . $args['form_id'] . '" id="' . $args['form_id'] . '" action="' . esc_url( site_url( 'nxt-login.php', 'login_post' ) ) . '" method="post">
			...
			<p class="login-submit">
				<input type="submit" name="nxt-submit" id="' . esc_attr( $args['id_submit'] ) . '" class="button-primary" value="' . esc_attr( $args['label_log_in'] ) . '" tabindex="100" />
 ...
 * @param string $redirect Path to redirect to on login.
 */
function nxt_lostpassword_url( $redirect = '' ) {
	$args = array( 'action' => 'lostpassword' );
	if ( !empty($redirect) ) {
		$args['redirect_to'] = $redirect;
	}

	$lostpassword_url = add_query_arg( $args, network_site_url('nxt-login.php', 'login') );
 ...
 * @param boolean $echo Default to echo and not return the link.
 */
function nxt_register( $before = '<li>', $after = '</li>', $echo = true ) {

	if ( ! is_user_logged_in() ) {
		if ( get_option('users_can_register') )
			$link = $before . '<a href="' . site_url('nxt-login.php?action=register', 'login') . '">' . __('Register') . '</a>' . $after;
		else
			$link = '';
	} else {
...
/**
 * Theme container function for the 'nxt_meta' action.
 *
 * The 'nxt_meta' action can have several purposes, depending on how you use it,
 ...
 * <li><strong>nxturl</strong> - Blog URI path to NXTClass.</li>
 ...
 */
function nxt_title($sep = '&raquo;', $display = true, $seplocation = '') {
	...
	$t_sep = '%nxt_TITILE_SEP%'; // Temporary separator, for accurate flipping, if necessary
	...
	if ( is_archive() && !empty($m) ) {
		...
		$my_month = $nxt_locale->get_month(substr($m, 4, 2));
	...
	if ( is_archive() && !empty($year) ) {
		$title = $year;
		if ( !empty($monthnum) )
			$title .= $t_sep . $nxt_locale->get_month($monthnum);
		if ( !empty($day) )
	...
	$title = apply_filters('nxt_title', $title, $sep, $seplocation);
 ...
 * for category page title and with less overhead than {@link nxt_title()}.
 *
 * It does not support placing the separator after the title, but by leaving the
 * prefix parameter empty, you can set the title separator manually. The prefix
 * does not automatically place a space between the prefix, so if there should
 ...
 * overhead than {@link nxt_title()}, because of its limited implementation.
 *
 * It does not support placing the separator after the title, but by leaving the
 * prefix parameter empty, you can set the title separator manually. The prefix
 * does not automatically place a space between the prefix, so if there should
 ...
 * It has less overhead than {@link nxt_title()}, because of its limited implementation.
 *
 * It does not support placing the separator after the title, but by leaving the
 * prefix parameter empty, you can set the title separator manually. The prefix
 * does not automatically place a space between the prefix, so if there should
 ...
 * is needed, should be better than {@link nxt_title()}.
 *
 * It does not support placing the separator after the title, but by leaving the
 * prefix parameter empty, you can set the title separator manually. The prefix
 * does not automatically place a space between the prefix, so if there should
...
function single_month_title($prefix = '', $display = true ) {
	...
	if ( !empty($monthnum) && !empty($year) ) {
		$my_year = $year;
		$my_month = $nxt_locale->get_month($monthnum);
	} elseif ( !empty($m) ) {
		$my_year = substr($m, 0, 4);
		$my_month = $nxt_locale->get_month(substr($m, 4, 2));
	}
 ...
 */
function nxt_get_archives($args = '') {
	global $nxtdb, $nxt_locale;
	...
	if ( 'monthly' == $type ) {
		$query = "SELECT YEAR(post_date) AS `year`, MONTH(post_date) AS `month`, count(ID) as posts FROM $nxtdb->posts $join $where GROUP BY YEAR(post_date), MONTH(post_date) ORDER BY post_date DESC $limit";
		$key = md5($query);
		$cache = nxt_cache_get( 'nxt_get_archives' , 'general');
		if ( !isset( $cache[ $key ] ) ) {
			$arcresults = $nxtdb->get_results($query);
			$cache[ $key ] = $arcresults;
			nxt_cache_set( 'nxt_get_archives', $cache, 'general' );
		} else {
		...
		if ( $arcresults ) {
			...
			foreach ( (array) $arcresults as $arcresult ) {
				...
				$text = sprintf(__('%1$s %2$d'), $nxt_locale->get_month($arcresult->month), $arcresult->year);
				if ( $show_post_count )
		...
		$query = "SELECT YEAR(post_date) AS `year`, count(ID) as posts FROM $nxtdb->posts $join $where GROUP BY YEAR(post_date) ORDER BY post_date DESC $limit";
		$key = md5($query);
		$cache = nxt_cache_get( 'nxt_get_archives' , 'general');
		if ( !isset( $cache[ $key ] ) ) {
			$arcresults = $nxtdb->get_results($query);
		...
		$query = "SELECT YEAR(post_date) AS `year`, MONTH(post_date) AS `month`, DAYOFMONTH(post_date) AS `dayofmonth`, count(ID) as posts FROM $nxtdb->posts $join $where GROUP BY YEAR(post_date), MONTH(post_date), DAYOFMONTH(post_date) ORDER BY post_date DESC $limit";
		$key = md5($query);
		$cache = nxt_cache_get( 'nxt_get_archives' , 'general');
		if ( !isset( $cache[ $key ] ) ) {
			$arcresults = $nxtdb->get_results($query);
		...
		$week = _nxt_mysql_week( '`post_date`' );
		$query = "SELECT DISTINCT $week AS `week`, YEAR( `post_date` ) AS `yr`, DATE_FORMAT( `post_date`, '%Y-%m-%d' ) AS `yyyymmdd`, count( `ID` ) AS `posts` FROM `$nxtdb->posts` $join $where GROUP BY $week, YEAR( `post_date` ) ORDER BY `post_date` DESC $limit";
		$key = md5($query);
		$cache = nxt_cache_get( 'nxt_get_archives' , 'general');
		if ( !isset( $cache[ $key ] ) ) {
			$arcresults = $nxtdb->get_results($query);
		...
		$query = "SELECT * FROM $nxtdb->posts $join $where ORDER BY $orderby $limit";
		$key = md5($query);
		$cache = nxt_cache_get( 'nxt_get_archives' , 'general');
		if ( !isset( $cache[ $key ] ) ) {
			$arcresults = $nxtdb->get_results($query);
...
function get_calendar($initial = true, $echo = true) {
	...
	$ak_post_titles = $nxtdb->get_results("SELECT ID, post_title, DAYOFMONTH(post_date) as dom "
		."FROM $nxtdb->posts "
...
function the_weekday() {
	global $nxt_locale, $post;
	$the_weekday = $nxt_locale->get_weekday(mysql2date('w', $post->post_date, false));
...
function the_weekday_date($before='',$after='') {
	global $nxt_locale, $post, $day, $previousweekday;
	$the_weekday_date = '';
	if ( $currentday != $previousweekday ) {
		$the_weekday_date .= $before;
		$the_weekday_date .= $nxt_locale->get_weekday(mysql2date('w', $post->post_date, false));
...
/**
 * Fire the nxt_head action
 *
 * @since 1.2.0
 * @uses do_action() Calls 'nxt_head' hook.
 */
function nxt_head() {
	do_action('nxt_head');
}
...
/**
 * Fire the nxt_footer action
 *
 * @since 1.5.1
 * @uses do_action() Calls 'nxt_footer' hook.
 */
function nxt_footer() {
	do_action('nxt_footer');
}
 ...
 */
function rsd_link() {
	echo '<link rel="EditURI" type="application/rsd+xml" title="RSD" href="' . get_bloginfo('nxturl') . "/xmlrpc.php?rsd\" />\n";
}
...
function wlwmanifest_link() {
	echo '<link rel="wlwmanifest" type="application/wlwmanifest+xml" href="'
		. get_bloginfo('nxturl') . '/nxt-includes/wlwmanifest.xml" /> ' . "\n";
}
 ...
 * output to tell web robots not to index the page content. Add this to the nxt_head action.
 * Typical usage is as a nxt_head callback. add_action( 'nxt_head', 'noindex' );
 *
 * @see nxt_no_robots
 *
 * @since 2.1.0
 ...
 * Typical usage is as a nxt_head callback. add_action( 'nxt_head', 'nxt_no_robots' );
 *
 * @since 3.3.0
 */
function nxt_no_robots() {
 ...
 *
 * Checks to see if the user has deleted the tinymce files to slim down there NXTClass install.
 *
 * @since 2.1.0
 *
...
function rich_edit_exists() {
	global $nxt_rich_edit_exists;
	if ( !isset($nxt_rich_edit_exists) )
		$nxt_rich_edit_exists = file_exists(ABSPATH . nxtINC . '/js/tinymce/tiny_mce.js');
	return $nxt_rich_edit_exists;
}
...
function user_can_richedit() {
	global $nxt_rich_edit, $is_gecko, $is_opera, $is_safari, $is_chrome, $is_iphone, $is_IE;

	if ( !isset($nxt_rich_edit) ) {
		$nxt_rich_edit = false;
	...
	return apply_filters('user_can_richedit', $nxt_rich_edit);
}
 ...
 */
function nxt_default_editor() {
	$r = user_can_richedit() ? 'tinymce' : 'html'; // defaults
	if ( $user = nxt_get_current_user() ) { // look for cookie
		$ed = get_user_setting('editor', 'tinymce');
		$r = ( in_array($ed, array('tinymce', 'html', 'test') ) ) ? $ed : $r;
	}
	return apply_filters( 'nxt_default_editor', $r ); // filter
}
...
/**
 * Retrieve the contents of the search NXTClass query variable.
 *
 ...
 * <code>
 * nxt_admin_css_color('classic', __('Classic'), admin_url("css/colors-classic.css"),
 ...
 */
function nxt_admin_css_color($key, $name, $url, $colors = array()) {
	global $_nxt_admin_css_colors;

	if ( !isset($_nxt_admin_css_colors) )
		$_nxt_admin_css_colors = array();

	$_nxt_admin_css_colors[$key] = (object) array('name' => $name, 'url' => $url, 'colors' => $colors);
}
...
function register_admin_color_schemes() {
	nxt_admin_css_color( 'classic', _x( 'Blue', 'admin color scheme' ), admin_url( 'css/colors-classic.css' ),
		array( '#5589aa', '#cfdfe9', '#d1e5ee', '#eff8ff' ) );
	nxt_admin_css_color( 'fresh', _x( 'Gray', 'admin color scheme' ), admin_url( 'css/colors-fresh.css' ),
		array( '#7c7976', '#c6c6c6', '#e0e0e0', '#f1f1f1' ) );
}

/**
 * Display the URL of a NXTClass admin CSS file.
 *
 * @see nxt_Styles::_css_href and its style_loader_src filter.
 *
 * @since 2.3.0
 *
 * @param string $file file relative to nxt-admin/ without its ".css" extension.
 */
function nxt_admin_css_uri( $file = 'nxt-admin' ) {
	...
	return apply_filters( 'nxt_admin_css_uri', $_file, $file );
}
...
function nxt_admin_css( $file = 'nxt-admin', $force_echo = false ) {
	...
	if ( $nxt_styles->query( $handle ) ) {
		if ( $force_echo || did_action( 'nxt_print_styles' ) ) // we already printed the style queue.  Print this one immediately
...
/**
 * Display the XHTML generator that is generated on the nxt_head hook.
 *
 * @since 2.5.0
 */
function nxt_generator() {
	the_generator( apply_filters( 'nxt_generator_type', 'xhtml' ) );
}
...
function get_the_generator( $type = '' ) {
	...
	switch ( $type ) {
		...
		case 'comment':
			...
			break;
		case 'export':
			$gen = '<!-- generator="NXTClass/' . get_bloginfo_rss('version') . '" created="'. date('Y-m-d H:i') . '" -->';
			break;
	}
