...
function check_comment($author, $email, $url, $comment, $user_ip, $user_agent, $comment_type) {
	...
	if ( 1 == get_option('comment_whitelist')) {
		if ( 'trackback' != $comment_type && 'pingback' != $comment_type && $author != '' && $email != '' ) {
			...
			$ok_to_comment = $nxtdb->get_var("SELECT comment_approved FROM $nxtdb->comments WHERE comment_author = '$author' AND comment_author_email = '$email' and comment_approved = '1' LIMIT 1");
			if ( ( 1 == $ok_to_comment ) &&
...
function get_approved_comments($post_id) {
	global $nxtdb;
	return $nxtdb->get_results($nxtdb->prepare("SELECT * FROM $nxtdb->comments WHERE comment_post_ID = %d AND comment_approved = '1' ORDER BY comment_date", $post_id));
}
...
function &get_comment(&$comment, $output = OBJECT) {
	...
	if ( empty($comment) ) {
		...
		nxt_cache_add($comment->comment_ID, $comment, 'comment');
		$_comment = $comment;
	} else {
		if ( isset($GLOBALS['comment']) && ($GLOBALS['comment']->comment_ID == $comment) ) {
			$_comment = & $GLOBALS['comment'];
		} elseif ( ! $_comment = nxt_cache_get($comment, 'comment') ) {
			$_comment = $nxtdb->get_row($nxtdb->prepare("SELECT * FROM $nxtdb->comments WHERE comment_ID = %d LIMIT 1", $comment));
			if ( ! $_comment )
				return $null;
			nxt_cache_add($_comment->comment_ID, $_comment, 'comment');
		}
	}
...
function get_comments( $args = '' ) {
	$query = new nxt_Comment_Query;
	return $query->query( $args );
}

/**
 * NXTClass Comment Query class.
 *
 * @since 3.1.0
 */
class nxt_Comment_Query {
...
/**
 * Retrieve all of the NXTClass supported comment statuses.
 *
...
function get_lastcommentmodified($timezone = 'server') {
	global $cache_lastcommentmodified, $nxtdb;
	...
	switch ( strtolower($timezone)) {
		case 'gmt':
			$lastcommentmodified = $nxtdb->get_var("SELECT comment_date_gmt FROM $nxtdb->comments WHERE comment_approved = '1' ORDER BY comment_date_gmt DESC LIMIT 1");
			break;
		case 'blog':
			$lastcommentmodified = $nxtdb->get_var("SELECT comment_date FROM $nxtdb->comments WHERE comment_approved = '1' ORDER BY comment_date_gmt DESC LIMIT 1");
			break;
		case 'server':
			$lastcommentmodified = $nxtdb->get_var($nxtdb->prepare("SELECT DATE_ADD(comment_date_gmt, INTERVAL %s SECOND) FROM $nxtdb->comments WHERE comment_approved = '1' ORDER BY comment_date_gmt DESC LIMIT 1", $add_seconds_server));
			break;
	}
 ...
 *
 * A lot like {@link nxt_count_comments()}, in that they both return comment
 * stats (albeit with different types). The {@link nxt_count_comments()} actual
...
function get_comment_count( $post_id = 0 ) {
	...
	$where = '';
	if ( $post_id > 0 ) {
		$where = $nxtdb->prepare("WHERE comment_post_ID = %d", $post_id);
	}

	$totals = (array) $nxtdb->get_results("
...
function check_comment_flood_db( $ip, $email, $date ) {
	...
	if ( $lasttime = $nxtdb->get_var( $nxtdb->prepare( "SELECT `comment_date_gmt` FROM `$nxtdb->comments` WHERE `comment_date_gmt` >= %s AND ( `comment_author_IP` = %s OR `comment_author_email` = %s ) ORDER BY `comment_date_gmt` DESC LIMIT 1", $hour_ago, $ip, $email ) ) ) {
		...
		if ( $flood_die ) {
			...
			nxt_die( __('You are posting comments too quickly.  Slow down.'), '', array('response' => 403) );
		}
	}
}
 ...
 * @uses Walker_Comment
 *
 * @param array $comments Optional array of comment objects.  Defaults to $nxt_query->comments
...
function get_comment_pages_count( $comments = null, $per_page = null, $threaded = null ) {
	...
	if ( null === $comments && null === $per_page && null === $threaded && !empty($nxt_query->max_num_comment_pages) )
		return $nxt_query->max_num_comment_pages;
...
function get_page_of_comment( $comment_ID, $args = array() ) {
	...
	$oldercoms = $nxtdb->get_var( $nxtdb->prepare( "SELECT COUNT(comment_ID) FROM $nxtdb->comments WHERE comment_post_ID = %d AND comment_parent = 0 AND comment_approved = '1' AND comment_date_gmt < '%s'" . $comtypewhere, $comment->comment_post_ID, $comment->comment_date_gmt ) );
 ...
 *
 * @since 1.5.0
 * @uses do_action() Calls 'nxt_blacklist_check' hook for all parameters.
 *
 ...
 */
function nxt_blacklist_check($author, $email, $url, $comment, $user_ip, $user_agent) {
	do_action('nxt_blacklist_check', $author, $email, $url, $comment, $user_ip, $user_agent);
 ...
 */
function nxt_count_comments( $post_id = 0 ) {
	global $nxtdb;

	$post_id = (int) $post_id;

	$stats = apply_filters('nxt_count_comments', array(), $post_id);
	if ( !empty($stats) )
		return $stats;

	$count = nxt_cache_get("comments-{$post_id}", 'counts');
	...
	$count = $nxtdb->get_results( "SELECT comment_approved, COUNT( * ) AS num_comments FROM {$nxtdb->comments} {$where} GROUP BY comment_approved", ARRAY_A );
	...
	$stats = (object) $stats;
	nxt_cache_set("comments-{$post_id}", $stats, 'counts');
 ...
 * @return mixed False on failure
 */
function nxt_untrash_comment($comment_id) {
	if ( ! (int)$comment_id )
		return false;
 ...
 * @return mixed False on failure
 */
function nxt_spam_comment($comment_id) {
	if ( !$comment = get_comment($comment_id) )
		return false;

	do_action('spam_comment', $comment_id);

	if ( nxt_set_comment_status($comment_id, 'spam') ) {
		add_comment_meta($comment_id, '_nxt_trash_meta_status', $comment->comment_approved);
 ...
 */
function nxt_get_comment_status($comment_id) {
 ...
 */
function nxt_transition_comment_status($new_status, $old_status, $comment) {
	// Translate raw statuses to human readable formats for the hooks
	// This is not a complete list of comment status, it's only the ones that need to be renamed
	$comment_statuses = array(
		0         => 'unapproved',
		'hold'    => 'unapproved', // nxt_set_comment_status() uses "hold"
		1         => 'approved',
		'approve' => 'approved', // nxt_set_comment_status() uses "approve"
	);
 ...
 */
function nxt_get_current_commenter() {
	...
	return apply_filters('nxt_get_current_commenter', compact('comment_author', 'comment_author_email', 'comment_author_url'));
}
 ...
 */
function nxt_insert_comment($commentdata) {
	global $nxtdb;
	...
	$nxtdb->insert($nxtdb->comments, $data);

	$id = (int) $nxtdb->insert_id;

	if ( $comment_approved == 1 )
		nxt_update_comment_count($comment_post_ID);

	$comment = get_comment($id);
	do_action('nxt_insert_comment', $id, $comment);
 ...
 */
function nxt_filter_comment($commentdata) {
 ...
 */
function nxt_throttle_comment_flood($block, $time_lastcomment, $time_newcomment) {
 ...
 *
 * The 'nxt_set_comment_status' action is called after the comment is handled and
 ...
 * @param bool $nxt_error Whether to return a nxt_Error object if there is a failure. Default is false.
 * @return bool False on failure or deletion and true on success.
 */
function nxt_set_comment_status($comment_id, $comment_status, $nxt_error = false) {
	global $nxtdb;
	...
	switch ( $comment_status ) {
		...
		case '1':
			...
			if ( get_option('comments_notify') ) {
				$comment = get_comment($comment_id);
				nxt_notify_postauthor($comment_id, $comment->comment_type);
			}
			break;
		case 'spam':
			$status = 'spam';
	...
	if ( !$nxtdb->update( $nxtdb->comments, array('comment_approved' => $status), array('comment_ID' => $comment_id) ) ) {
		if ( $nxt_error )
			return new nxt_Error('db_update_error', __('Could not update comment status'), $nxtdb->last_error);
		else
			return false;
	}
	...
	do_action('nxt_set_comment_status', $comment_id, $comment_status);
	nxt_transition_comment_status($comment_status, $comment_old->comment_approved, $comment);

	nxt_update_comment_count($comment->comment_post_ID);
 ...
 */
function nxt_update_comment($commentarr) {
	global $nxtdb;
	...
	$commentarr = nxt_filter_comment( $commentarr );
	...
	$rval = $nxtdb->update( $nxtdb->comments, $data, compact( 'comment_ID' ) );

	clean_comment_cache($comment_ID);
	nxt_update_comment_count($comment_post_ID);
	do_action('edit_comment', $comment_ID);
	$comment = get_comment($comment_ID);
	nxt_transition_comment_status($comment->comment_approved, $old_status, $comment);
	return $rval;
}
 ...
 * updated without having to call nxt_update_comment_count() after.
 *
 * @since 2.5.0
 ...
 * @return unknown
 */
function nxt_defer_comment_counting($defer=null) {
	static $_defer = false;
	...
	if ( is_bool($defer) ) {
		...
		if ( !$defer )
			nxt_update_comment_count( null, true );
	}
...
function do_all_pings() {
	...
	// Do pingbacks
	while ($ping = $nxtdb->get_row("SELECT ID, post_content, meta_id FROM {$nxtdb->posts}, {$nxtdb->postmeta} WHERE {$nxtdb->posts}.ID = {$nxtdb->postmeta}.post_id AND {$nxtdb->postmeta}.meta_key = '_pingme' LIMIT 1")) {
		delete_metadata_by_mid( 'post', $ping->meta_id );
		pingback( $ping->post_content, $ping->ID );
	}

	// Do Enclosures
	while ($enclosure = $nxtdb->get_row("SELECT ID, post_content, meta_id FROM {$nxtdb->posts}, {$nxtdb->postmeta} WHERE {$nxtdb->posts}.ID = {$nxtdb->postmeta}.post_id AND {$nxtdb->postmeta}.meta_key = '_encloseme' LIMIT 1")) {
		delete_metadata_by_mid( 'post', $enclosure->meta_id );
		do_enclose( $enclosure->post_content, $enclosure->ID );
	}

	// Do Trackbacks
	$trackbacks = $nxtdb->get_col("SELECT ID FROM $nxtdb->posts WHERE to_ping <> '' AND post_status = 'publish'");
...
function pingback($content, $post_ID) {
	...
	include_once(ABSPATH . nxtINC . '/class-nxt-http-ixr-client.php');
	...
	foreach ( (array) $post_links as $pagelinkedto ) {
		...
		if ( $pingback_server_url ) {
			...
			$client = new nxt_HTTP_IXR_Client($pingback_server_url);
			$client->timeout = 3;
			$client->useragent = apply_filters( 'pingback_useragent', $client->useragent . ' -- NXTClass/' . $nxt_version, $client->useragent, $pingback_server_url, $pagelinkedto, $pagelinkedfrom);
...
function weblog_ping($server = '', $path = '') {
	...
	// using a timeout of 3 seconds should be enough to cover slow servers
	$client = new nxt_HTTP_IXR_Client($server, ((!strlen(trim($path)) || ('/' == $path)) ? false : $path));
	$client->timeout = 3;
	$client->useragent .= ' -- NXTClass/'.$nxt_version;
...
function clean_comment_cache($ids) {
	foreach ( (array) $ids as $id )
		nxt_cache_delete($id, 'comment');

	nxt_cache_set('last_changed', time(), 'comment');
}
