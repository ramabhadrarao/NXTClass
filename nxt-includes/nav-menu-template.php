...
function nxt_nav_menu( $args = array() ) {
	...
	'echo' => true, 'fallback_cb' => 'nxt_page_menu', 'before' => '', 'after' => '', 'link_before' => '', 'link_after' => '', 'items_wrap' => '<ul id="%1$s" class="%2$s">%3$s</ul>',
	...
	if ( ( !$menu || is_nxt_error($menu) || ( isset($menu_items) && empty($menu_items) && !$args->theme_location ) )
...
function _nxt_menu_item_classes_by_context( &$menu_items ) {
	...
	foreach ( (array) $menu_items as $key => $menu_item ) {
		...
		// if the menu item corresponds to the currently-queried post or taxonomy object
			...
			(
				( ! empty( $home_page_id ) && 'post_type' == $menu_item->type && $nxt_query->is_home && $home_page_id == $menu_item->object_id ) ||
				( 'post_type' == $menu_item->type && $nxt_query->is_singular ) ||
				( 'taxonomy' == $menu_item->type && ( $nxt_query->is_category || $nxt_query->is_tag || $nxt_query->is_tax ) )
			)
		) {
