 ...
 *
 * If the locale is not set already, then the nxtLANG constant is used if it is
...
function get_locale() {
	...
	// nxtLANG is defined in nxt-config.
	if ( defined( 'nxtLANG' ) )
		$locale = nxtLANG;
	...
	if ( is_multisite() ) {
		...
		if ( defined( 'nxt_INSTALLING' ) || ( false === $ms_locale = get_option( 'nxtLANG' ) ) )
			$ms_locale = get_site_option('nxtLANG');
 ...
 *
 * Loads the .mo file in nxt_LANG_DIR constant path from NXTClass root. The
...
function load_default_textdomain() {
	...
	load_textdomain( 'default', nxt_LANG_DIR . "/$locale.mo" );

	if ( is_multisite() || ( defined( 'nxt_INSTALLING_NETWORK' ) && nxt_INSTALLING_NETWORK ) ) {
		load_textdomain( 'default', nxt_LANG_DIR . "/ms-$locale.mo" );
	}
}
 ...
 * @param string $plugin_rel_path Optional. Relative path to nxt_PLUGIN_DIR. This is the preferred argument to use. It takes precedence over $abs_rel_path
 */
function load_plugin_textdomain( $domain, $abs_rel_path = false, $plugin_rel_path = false ) {
	$locale = apply_filters( 'plugin_locale', get_locale(), $domain );

	if ( false !== $plugin_rel_path	) {
		$path = nxt_PLUGIN_DIR . '/' . trim( $plugin_rel_path, '/' );
 ...
 *
 * @param string $domain Unique identifier for retrieving translated strings
 * @param string $mu_plugin_rel_path Relative to nxtMU_PLUGIN_DIR directory in which
 * the MO file resides. Defaults to empty string.
 */
function load_muplugin_textdomain( $domain, $mu_plugin_rel_path = '' ) {
	$locale = apply_filters( 'plugin_locale', get_locale(), $domain );
	$path = nxtMU_PLUGIN_DIR . '/' . ltrim( $mu_plugin_rel_path, '/' );
...
/**
 * Get all available languages based on the presence of *.mo files in a given directory. The default directory is nxt_LANG_DIR.
 *
 * @since 3.0.0
 *
 * @param string $dir A directory in which to search for language files. The default directory is nxt_LANG_DIR.
 * @return array Array of language codes or an empty array if no languages are present. Language codes are formed by stripping the .mo extension from the language file names.
 */
function get_available_languages( $dir = null ) {
	$languages = array();

	foreach( (array)glob( ( is_null( $dir) ? nxt_LANG_DIR : $dir ) . '/*.mo' ) as $lang_file ) {
