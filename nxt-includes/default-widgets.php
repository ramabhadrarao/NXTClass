 ...
 * @since 2.8.0
 */
class nxt_Widget_Pages extends nxt_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_pages', 'description' => __( 'Your site&#8217;s NXTClass Pages') );
	...
	function form( $instance ) {
		//Defaults
		$instance = nxt_parse_args( (array) $instance, array( 'sortby' => 'post_title', 'title' => '', 'exclude' => '') );
 ...
 * @since 2.8.0
 */
class nxt_Widget_Links extends nxt_Widget {
	...
	function widget( $args, $instance ) {
		...
		nxt_list_bookmarks(apply_filters('widget_links_args', array(
	...
	function form( $instance ) {
		...
		//Defaults
		$instance = nxt_parse_args( (array) $instance, array( 'images' => true, 'name' => true, 'description' => false, 'rating' => false, 'category' => false ) );
 ...
 * @since 2.8.0
 */
class nxt_Widget_Search extends nxt_Widget {
	...
	function form( $instance ) {
		$instance = nxt_parse_args( (array) $instance, array( 'title' => '') );
...
<?php
	...
	function update( $new_instance, $old_instance ) {
		$instance = $old_instance;
		$new_instance = nxt_parse_args((array) $new_instance, array( 'title' => ''));
		$instance['title'] = strip_tags($new_instance['title']);
		return $instance;
	}
 ...
 * @since 2.8.0
 */
class nxt_Widget_Archives extends nxt_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ( $d ) {
?>
		<select name="archive-dropdown" onchange='document.location.href=this.options[this.selectedIndex].value;'> <option value=""><?php echo esc_attr(__('Select Month')); ?></option> <?php nxt_get_archives(apply_filters('widget_archives_dropdown_args', array('type' => 'monthly', 'format' => 'option', 'show_post_count' => $c))); ?> </select>
<?php
		} else {
?>
		<ul>
		<?php nxt_get_archives(apply_filters('widget_archives_args', array('type' => 'monthly', 'show_post_count' => $c))); ?>
		</ul>
<?php
		}
	...
	function update( $new_instance, $old_instance ) {
		$instance = $old_instance;
		$new_instance = nxt_parse_args( (array) $new_instance, array( 'title' => '', 'count' => 0, 'dropdown' => '') );
		$instance['title'] = strip_tags($new_instance['title']);
	...
	function form( $instance ) {
		$instance = nxt_parse_args( (array) $instance, array( 'title' => '', 'count' => 0, 'dropdown' => '') );
 ...
 * @since 2.8.0
 */
class nxt_Widget_Meta extends nxt_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_meta', 'description' => __( "Log in/out, admin, feed and NXTClass links") );
...
?>
			...
			<li><a href="http://opensource.nxtclass.tk/" title="<?php echo esc_attr(__('Powered by NXTClass, state-of-the-art semantic personal publishing platform.')); ?>">opensource.nxtclass.tk</a></li>
			<?php nxt_meta(); ?>
			</ul>
<?php
		echo $after_widget;
 ...
 * @since 2.8.0
 */
class nxt_Widget_Calendar extends nxt_Widget {
 ...
 * @since 2.8.0
 */
class nxt_Widget_Text extends nxt_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ( !empty( $title ) ) { echo $before_title . $title . $after_title; } ?>
			<div class="textwidget"><?php echo !empty( $instance['filter'] ) ? nxtautop( $text ) : $text; ?></div>
		<?php
		echo $after_widget;
	}
	...
	function update( $new_instance, $old_instance ) {
		...
		if ( current_user_can('unfiltered_html') )
			...
			$instance['text'] = stripslashes( nxt_filter_post_kses( addslashes($new_instance['text']) ) ); // nxt_filter_post_kses() expects slashed
		$instance['filter'] = isset($new_instance['filter']);
		return $instance;
	}

	function form( $instance ) {
		$instance = nxt_parse_args( (array) $instance, array( 'title' => '', 'text' => '' ) );
		$title = strip_tags($instance['title']);
 ...
 * @since 2.8.0
 */
class nxt_Widget_Categories extends nxt_Widget {
 ...
 * @since 2.8.0
 */
class nxt_Widget_Recent_Posts extends nxt_Widget {
...
<?php
	...
	function flush_widget_cache() {
		nxt_cache_delete('widget_recent_posts', 'widget');
	}
 ...
 * @since 2.8.0
 */
class nxt_Widget_Recent_Comments extends nxt_Widget {
	...
	function __construct() {
		...
		if ( is_active_widget(false, false, $this->id_base) )
			add_action( 'nxt_head', array(&$this, 'recent_comments_style') );
...
<?php
	...
	function widget( $args, $instance ) {
		...
		$cache = nxt_cache_get('widget_recent_comments', 'widget');
		...
		nxt_cache_set('widget_recent_comments', $cache, 'widget');
	}
 ...
 * @since 2.8.0
 */
class nxt_Widget_RSS extends nxt_Widget {
	...
	function widget($args, $instance) {
		...
		if ( $title )
			echo $before_title . $title . $after_title;
		nxt_widget_rss_output( $rss, $instance );
		echo $after_widget;
 ...
 */
function nxt_widget_rss_form( $args, $inputs = null ) {

	$default_inputs = array( 'url' => true, 'title' => true, 'items' => true, 'show_summary' => true, 'show_author' => true, 'show_date' => true );
	$inputs = nxt_parse_args( $inputs, $default_inputs );
	extract( $args );
 ...
 * @return array
 */
function nxt_widget_rss_process( $widget_rss, $check_feed = true ) {
 ...
 * @since 2.8.0
 */
class nxt_Widget_Tag_Cloud extends nxt_Widget {
 ...
 * @since 3.0.0
 */
 class nxt_Nav_Menu_Widget extends nxt_Widget {
...
/**
 * Register all of the default NXTClass widgets on startup.
 *
 * Calls 'widgets_init' action after all of the NXTClass widgets have been
 * registered.
 *
 * @since 2.2.0
 */
function nxt_widgets_init() {
	...
	register_widget('nxt_Widget_Pages');

	register_widget('nxt_Widget_Calendar');

	register_widget('nxt_Widget_Archives');

	register_widget('nxt_Widget_Links');

	register_widget('nxt_Widget_Meta');

	register_widget('nxt_Widget_Search');

	register_widget('nxt_Widget_Text');

	register_widget('nxt_Widget_Categories');

	register_widget('nxt_Widget_Recent_Posts');

	register_widget('nxt_Widget_Recent_Comments');

	register_widget('nxt_Widget_RSS');

	register_widget('nxt_Widget_Tag_Cloud');

	register_widget('nxt_Nav_Menu_Widget');

	do_action('widgets_init');
}

add_action('init', 'nxt_widgets_init', 1);
