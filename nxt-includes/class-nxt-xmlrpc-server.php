...
/**
 * NXTClass XMLRPC server implementation.
 *
 * Implements compatibility for Blogger API, MetaWeblog API, MovableType, and
 * pingback. Additional NXTClass API for managing comments, pages, posts,
 * options, etc.
 *
 * Since NXTClass 2.6.0, NXTClass XMLRPC server can be disabled in the
 ...
 * @since 1.5.0
 */
class nxt_xmlrpc_server extends IXR_Server {
	 ...
	 * @since 1.5.0
	 *
	 * @return nxt_xmlrpc_server
	 */
	function __construct() {
		$this->methods = array(
			// NXTClass API
			'nxt.getUsersBlogs'		=> 'this:nxt_getUsersBlogs',
			'nxt.getPage'			=> 'this:nxt_getPage',
			'nxt.getPages'			=> 'this:nxt_getPages',
			'nxt.nenxtage'			=> 'this:nxt_nenxtage',
			'nxt.deletePage'			=> 'this:nxt_deletePage',
			'nxt.editPage'			=> 'this:nxt_editPage',
			'nxt.getPageList'		=> 'this:nxt_getPageList',
			'nxt.getAuthors'			=> 'this:nxt_getAuthors',
			'nxt.getCategories'		=> 'this:mw_getCategories',		// Alias
			'nxt.getTags'			=> 'this:nxt_getTags',
			'nxt.newCategory'		=> 'this:nxt_newCategory',
			'nxt.deleteCategory'		=> 'this:nxt_deleteCategory',
			'nxt.suggestCategories'	=> 'this:nxt_suggestCategories',
			'nxt.uploadFile'			=> 'this:mw_newMediaObject',	// Alias
			'nxt.getCommentCount'	=> 'this:nxt_getCommentCount',
			'nxt.getPostStatusList'	=> 'this:nxt_getPostStatusList',
			'nxt.getPageStatusList'	=> 'this:nxt_getPageStatusList',
			'nxt.getPageTemplates'	=> 'this:nxt_getPageTemplates',
			'nxt.getOptions'			=> 'this:nxt_getOptions',
			'nxt.setOptions'			=> 'this:nxt_setOptions',
			'nxt.getComment'			=> 'this:nxt_getComment',
			'nxt.getComments'		=> 'this:nxt_getComments',
			'nxt.deleteComment'		=> 'this:nxt_deleteComment',
			'nxt.editComment'		=> 'this:nxt_editComment',
			'nxt.newComment'			=> 'this:nxt_newComment',
			'nxt.getCommentStatusList' => 'this:nxt_getCommentStatusList',
			'nxt.getMediaItem'		=> 'this:nxt_getMediaItem',
			'nxt.getMediaLibrary'	=> 'this:nxt_getMediaLibrary',
			'nxt.getPostFormats'     => 'this:nxt_getPostFormats',
			...
			'blogger.nenxtost' => 'this:blogger_nenxtost',
			'blogger.editPost' => 'this:blogger_editPost',
			'blogger.deletePost' => 'this:blogger_deletePost',

			// MetaWeblog API (with MT extensions to structs)
			'metaWeblog.nenxtost' => 'this:mw_nenxtost',
	 ...
	 * @deprecated use nxt_xmlrpc_server::login
	 * @see nxt_xmlrpc_server::login
	 */
	 ...
	 */
	function nxt_getUsersBlogs( $args ) {
		global $current_site;
		// If this isn't on nxtMU then just use blogger_getUsersBlogs
		if ( !is_multisite() ) {
		...
		do_action( 'xmlrpc_call', 'nxt.getUsersBlogs' );
	...
	function nxt_getPage($args) {
		...
		do_action('xmlrpc_call', 'nxt.getPage');
		...
		if ( $page->ID && ($page->post_type == 'page') ) {
			...
			$categories = array();
			foreach ( nxt_get_post_categories($page->ID) as $cat_id ) {
			...
			$page_template = get_post_meta( $page->ID, '_nxt_page_template', true );
			...
			$page_struct = array(
				...
				'nxt_slug'				=> $page->post_name,
				'nxt_password'			=> $page->post_password,
				'nxt_author'				=> $author->display_name,
				'nxt_page_parent_id'		=> $page->post_parent,
				'nxt_page_parent_title'	=> $parent_title,
				'nxt_page_order'			=> $page->menu_order,
				'nxt_author_id'			=> (string) $author->ID,
				'nxt_author_display_name'	=> $author->display_name,
				'date_created_gmt'		=> new IXR_Date($page_date_gmt),
				'custom_fields'			=> $this->get_custom_fields($page_id),
				'nxt_page_template'		=> $page_template
			);
	...
	function nxt_getPages($args) {
		...
		do_action('xmlrpc_call', 'nxt.getPages');
		...
		if ( $num_pages >= 1 ) {
			...
			for ( $i = 0; $i < $num_pages; $i++ ) {
				$page = nxt_xmlrpc_server::nxt_getPage(array(
	 ...
	 * @since 2.2.0
	 *
	 * @param array $args Method parameters. See {@link nxt_xmlrpc_server::mw_nenxtost()}
	 * @return unknown
	 */
	function nxt_nenxtage($args) {
		// Items not escaped here will be escaped in nenxtost.
		...
		do_action('xmlrpc_call', 'nxt.nenxtage');
		...
		// Let mw_nenxtost do all of the heavy lifting.
		return($this->mw_nenxtost($args));
	}
	...
	function nxt_deletePage($args) {
		...
		do_action('xmlrpc_call', 'nxt.deletePage');

		// Get the current page based on the page_id and
		// make sure it is a page and not a post.
		$actual_page = nxt_get_single_post($page_id, ARRAY_A);
		...
		$result = nxt_delete_post($page_id);
		if ( !$result )
	...
	function nxt_editPage($args) {
		...
		do_action('xmlrpc_call', 'nxt.editPage');
	...
	function nxt_getPageList($args) {
		...
		do_action('xmlrpc_call', 'nxt.getPageList');

		// Get list of pages ids and titles
		$page_list = $nxtdb->get_results("
			SELECT ID page_id,
	...
	function nxt_getAuthors($args) {
		...
		do_action('xmlrpc_call', 'nxt.getAuthors');
	...
	function nxt_getTags( $args ) {
		...
		do_action( 'xmlrpc_call', 'nxt.getKeywords' );
	 ...
	 * @return array
	 */
	function nxt_suggestCategories($args) {
		$this->escape($args);
		...
		do_action('xmlrpc_call', 'nxt.suggestCategories');
	...
	function nxt_getComment($args) {
		...
		do_action('xmlrpc_call', 'nxt.getComment');
	 ...
	 *
	 * @param array $args Method parameters.
	 * @return array. Contains a collection of comments. See {@link nxt_xmlrpc_server::nxt_getComment()} for a description of each item contents
	 */
	function nxt_getComments($args) {
		$raw_args = $args;
		$this->escape($args);
		...
		do_action('xmlrpc_call', 'nxt.getComments');
    ...
    // FIXME: we already have the comments, why query them again?
		for ( $i = 0; $i < $num_comments; $i++ ) {
			$comment = nxt_xmlrpc_server::nxt_getComment(array(
	 ...
	 *
	 * @param array $args Method parameters.
	 * @return mixed {@link nxt_new_comment()}
	 */
	function nxt_newComment($args) {
		global $nxtdb;
		...
		if ( $logged_in ) {
			$comment['comment_author'] = $nxtdb->escape( $user->display_name );
			$comment['comment_author_email'] = $nxtdb->escape( $user->user_email );
			$comment['comment_author_url'] = $nxtdb->escape( $user->user_url );
		...
		do_action('xmlrpc_call', 'nxt.newComment');

		return nxt_new_comment($comment);
	}
	 ...
	 * @return array
	 */
	function nxt_getCommentStatusList($args) {
		$this->escape( $args );
		...
		do_action('xmlrpc_call', 'nxt.getCommentStatusList');
	 ...
	 * @return array
	 */
	function nxt_getPostStatusList( $args ) {
		$this->escape( $args );
		...
		do_action('xmlrpc_call', 'nxt.getPostStatusList');
	 ...
	 * @return array
	 */
	function nxt_getPageStatusList( $args ) {
		$this->escape( $args );
		...
		do_action('xmlrpc_call', 'nxt.getPageStatusList');
	 ...
	 * @return array
	 */
	function nxt_getPageTemplates( $args ) {
		$this->escape( $args );
	...
	function nxt_getMediaItem($args) {
		...
		$attachment_struct = array(
			...
			'metadata'				=> nxt_get_attachment_metadata($attachment->ID),
		);
	 ...
	 *  - password
	 *  - filter
	 * @return array. Contains a collection of media items. See {@link nxt_xmlrpc_server::nxt_getMediaItem()} for a description of each item contents
	 */
	function nxt_getMediaLibrary($args) {
		$raw_args = $args;
		$this->escape($args);
		...
		do_action('xmlrpc_call', 'nxt.getMediaLibrary');
		...
		foreach ($attachments as $attachment )
			$attachments_struct[] = $this->nxt_getMediaItem( array( $raw_args[0], $raw_args[1], $raw_args[2], $attachment->ID ) );
	...
	/**
	  ...
	  * @return array
	  */
	function nxt_getPostFormats( $args ) {
		$this->escape( $args );
		...
		do_action( 'xmlrpc_call', 'nxt.getPostFormats' );
	...
	function _multisite_getUsersBlogs($args) {
		...
		$rpc->query('nxt.getUsersBlogs', $args[1], $args[2]);
	...
	function blogger_getPost($args) {
		...
		$post_data = nxt_get_single_post($post_ID, ARRAY_A);

		$categories = implode(',', nxt_get_post_categories($post_ID));
	...
	function blogger_getRecentPosts($args) {
		...
		foreach ($posts_list as $entry) {
			...
			$categories = implode(',', nxt_get_post_categories($entry['ID']));
	 ...
	 * @return int
	 */
	function blogger_nenxtost($args) {
		...
		do_action('xmlrpc_call', 'blogger.nenxtost');
		...
		$post_ID = nxt_insert_post($post_data);
	...
	function mw_nenxtost($args) {
		...
		do_action('xmlrpc_call', 'metaWeblog.nenxtost');
		...
		// Let NXTClass generate the post_name (slug) unless
		...
		$post_ID = nxt_insert_post( $postdata, true );
		if ( is_nxt_error( $post_ID ) )
			return new IXR_Error(500, $post_ID->get_error_message());
	...
	function attach_uploads( $post_ID, $post_content ) {
		...
		$attachments = $nxtdb->get_results( "SELECT ID, guid FROM {$nxtdb->posts} WHERE post_parent = '0' AND post_type = 'attachment'" );
		if ( is_array( $attachments ) ) {
			foreach ( $attachments as $file ) {
				if ( strpos( $post_content, $file->guid ) !== false )
					$nxtdb->update($nxtdb->posts, array('post_parent' => $post_ID), array('ID' => $file->ID) );
			}
		}
	}
	...
	function mw_getPost($args) {
		...
		if ($postdata['post_date'] != '') {
			...
			$categories = array();
			$catids = nxt_get_post_categories($post_ID);
			foreach($catids as $catid)
				$categories[] = get_cat_name($catid);

			$tagnames = array();
			$tags = nxt_get_post_tags( $post_ID );
			if ( !empty( $tags ) ) {
				foreach ( $tags as $tag )
					$tagnames[] = $tag->name;
			...
			$resp = array(
				...
				'nxt_slug' => $postdata['post_name'],
				'nxt_password' => $postdata['post_password'],
				'nxt_author_id' => (string) $author->ID,
				'nxt_author_display_name'	=> $author->display_name,
				'date_created_gmt' => new IXR_Date($post_date_gmt),
				'post_status' => $postdata['post_status'],
				'custom_fields' => $this->get_custom_fields($post_ID),
				'nxt_post_format' => $post_format,
				'sticky' => $sticky
			);
	...
	function mw_getRecentPosts($args) {
		...
		foreach ($posts_list as $entry) {
			...
			$struct[] = array(
				...
				'mt_allow_pings' => $allow_pings,
				'mt_keywords' => $tagnames,
				'nxt_slug' => $entry['post_name'],
				'nxt_password' => $entry['post_password'],
				'nxt_author_id' => (string) $author->ID,
				'nxt_author_display_name' => $author->display_name,
				'date_created_gmt' => new IXR_Date($post_date_gmt),
	 ...
	 *
	 * @link http://mycvs.org/archives/2004/06/30/file-upload-to-nxtclass-in-ecto/
	 *
	 * @since 1.5.0
	 *
	 * @param array $args Method parameters.
	...
	function mw_newMediaObject($args) {
		...
		$upload = nxt_upload_bits($name, NULL, $bits);
		...
		return apply_filters( 'nxt_handle_upload', array( 'file' => $name, 'url' => $upload[ 'url' ], 'type' => $type ), 'upload' );
	}
	...
	function mt_getPostCategories($args) {
		...
		$categories = array();
		$catids = nxt_get_post_categories(intval($post_ID));
	...
	function pingback_ping($args) {
		...
		foreach ( $p as $para ) {
			if ( strpos($para, $pagelinkedto) !== false ) { // it exists, but is it a link?
				...
				$marker = '<nxtcontext>'.$context[1].'</nxtcontext>';    // set up our marker
				$excerpt= str_replace($context[0], $marker, $excerpt); // swap out the link for our marker
				$excerpt = strip_tags($excerpt, '<nxtcontext>');        // strip all tags but our context marker
