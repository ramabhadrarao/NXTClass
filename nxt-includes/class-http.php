...
class nxt_Http {
	...
	function request( $url, $args = array() ) {
		...
		$defaults = array(
			...
			'user-agent' => apply_filters( 'http_headers_useragent', 'NXTClass/' . $nxt_version . '; ' . get_bloginfo( 'url' )  ),
			'blocking' => true,
			'headers' => array(),
			'cookies' => array(),
			'body' => null,
		...
		if ( empty( $url ) || empty( $arrURL['scheme'] ) )
			return new nxt_Error('http_request_failed', __('A valid URL was not provided.'));

		if ( $this->block_request( $url ) )
			return new nxt_Error( 'http_request_failed', __( 'User has blocked requests through HTTP.' ) );
		...
		// If we are streaming to a file but no filename was given drop it in the nxt temp dir
		...
		if ( $r['stream'] ) {
			...
			if ( ! is_writable( dirname( $r['filename'] ) ) )
				return new nxt_Error( 'http_request_failed', __( 'Destination directory for file streaming does not exist or is not writable.' ) );
		}
	...
	private function _dispatch_request( $url, $args ) {
		...
		if ( !$class )
			return new nxt_Error( 'http_failure', __( 'There are no HTTP transports available which can complete the requested request.' ) );
	...
	function block_request($uri) {
		...
		if ( ! defined( 'nxt_HTTP_BLOCK_EXTERNAL' ) || ! nxt_HTTP_BLOCK_EXTERNAL )
			return false;
		...
		if ( null == $accessible_hosts ) {
			...
			if ( false !== strpos(nxt_ACCESSIBLE_HOSTS, '*') ) {
	...
	function request($url, $args = array()) {
		...
		if ( $r['stream'] ) {
			if ( ! nxt_DEBUG )
				...
				$stream_handle = fopen( $r['filename'], 'w+' );
			if ( ! $stream_handle )
				return new nxt_Error( 'http_request_failed', sprintf( __( 'Could not open handle for fopen() to %s' ), $r['filename'] ) );
...
class nxt_Http_Curl {
	...
	function request($url, $args = array()) {
		...
		$theBody = '';
		$theHeaders = nxt_Http::processHeaders( $this->headers );
...
class nxt_HTTP_Proxy {
	...
	function send_through_proxy( $uri ) {
		...
		if ( null == $bypass_hosts ) {
			...
			if ( false !== strpos(nxt_PROXY_BYPASS_HOSTS, '*') ) {
				$wildcard_regex = array();
