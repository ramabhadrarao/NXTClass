 ...
 *
 * @since 2.1.0
 * @uses $nxt_rewrite nxt_Rewrite
 * @return string The URL to the author's page.
 */
function get_author_posts_url($author_id, $author_nicename = '') {
	global $nxt_rewrite;
	$auth_ID = (int) $author_id;
	$link = $nxt_rewrite->get_author_permastruct();
...
function is_multi_author() {
	...
	if ( false === ( $is_multi_author = nxt_cache_get('is_multi_author', 'posts') ) ) {
		$rows = (array) $nxtdb->get_col("SELECT DISTINCT post_author FROM $nxtdb->posts WHERE post_type = 'post' AND post_status = 'publish' LIMIT 2");
		$is_multi_author = 1 < count( $rows ) ? 1 : 0;
		nxt_cache_set('is_multi_author', $is_multi_author, 'posts');
	}
...
function __clear_multi_author_cache() {
	nxt_cache_delete('is_multi_author', 'posts');
}
