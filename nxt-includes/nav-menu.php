 ...
 */
function nxt_get_nav_menu_object( $menu ) {
	if ( ! $menu )
		return false;
...
function is_nav_menu( $menu ) {
	...
	$menu_obj = nxt_get_nav_menu_object( $menu );

	if (
		$menu_obj &&
		! is_nxt_error( $menu_obj ) &&
...
function register_nav_menus( $locations = array() ) {
	global $_nxt_registered_nav_menus;

	add_theme_support( 'menus' );

	$_nxt_registered_nav_menus = array_merge( (array) $_nxt_registered_nav_menus, $locations );
}
...
function unregister_nav_menu( $location ) {
	...
	if ( is_array( $_nxt_registered_nav_menus ) && isset( $_nxt_registered_nav_menus[$location] ) ) {
		unset( $_nxt_registered_nav_menus[$location] );
		return true;
	}
	return false;
}
...
function get_registered_nav_menus() {
	global $_nxt_registered_nav_menus;
	if ( isset( $_nxt_registered_nav_menus ) )
		return $_nxt_registered_nav_menus;
	return array();
}
...
function is_nav_menu_item( $menu_item_id = 0 ) {
	return ( ! is_nxt_error( $menu_item_id ) && ( 'nav_menu_item' == get_post_type( $menu_item_id ) ) );
}
...
function nxt_update_nav_menu_object( $menu_id = 0, $menu_data = array() ) {
	...
	if (
		$_possible_existing &&
		! is_nxt_error( $_possible_existing ) &&
 ...
 */
function nxt_get_nav_menus( $args = array() ) {
	$defaults = array( 'hide_empty' => false, 'orderby' => 'none' );
	$args = nxt_parse_args( $args, $defaults );
	return apply_filters( 'nxt_get_nav_menus', get_terms( 'nav_menu',  $args), $args );
}
 ...
 */
function nxt_get_nav_menu_items( $menu, $args = array() ) {
	...
	if ( is_nxt_error( $items ) || ! is_array( $items ) )
		return false;

	// Get all posts and terms at once to prime the caches
	if ( empty( $fetched[$menu->term_id] ) || $_nxt_using_ext_object_cache ) {
	...
	$items = array_map( 'nxt_setup_nav_menu_item', $items );
	...
	return apply_filters( 'nxt_get_nav_menu_items',  $items, $menu, $args );
}
 ...
 *
 */
function _nxt_delete_post_menu_item( $object_id = 0 ) {
	$object_id = (int) $object_id;

	$menu_item_ids = nxt_get_associated_nav_menu_items( $object_id, 'post_type' );
