 ...
 *
 * @since 0.71
 * @uses $nxt_cockneyreplace Array of formatted entities for certain common phrases
 *
 * @param string $text The text to be formatted
 * @return string The string replaced with html entities
 */
function nxttexturize($text) {
	global $nxt_cockneyreplace;
	...
	if ( empty( $opening_quote ) ) {
		...
		if ( isset($nxt_cockneyreplace) ) {
			$cockney = array_keys($nxt_cockneyreplace);
			$cockneyreplace = array_values($nxt_cockneyreplace);
		} else {
	...
	// Transform into regexp sub-expression used in _nxttexturize_pushpop_element
	...
	foreach ( $textarr as &$curl ) {
		...
		// Only call _nxttexturize_pushpop_element if first char is correct tag opening
		$first = $curl[0];
		if ( '<' === $first ) {
			_nxttexturize_pushpop_element($curl, $no_texturize_tags_stack, $no_texturize_tags, '<', '>');
		} elseif ( '[' === $first ) {
			_nxttexturize_pushpop_element($curl, $no_texturize_shortcodes_stack, $no_texturize_shortcodes, '[', ']');
 ...
 * @return object
 */
function _nxttexturize_pushpop_element($text, &$stack, $disabled_elements, $opening = '<', $closing = '>') {
...
/**
 * Accepts matches array from preg_replace_callback in nxtautop() or a string.
 *
...
function nxtautop($pee, $br = 1) {
	...
	if ($br) {
		...
		$pee = str_replace('<nxtPreserveNewline />', "\n", $pee);
	}
...
/**
 * Newline preservation help function for nxtautop
 *
 * @since 3.1.0
 * @access private
 ...
 * @param mixed $quote_style Optional. Converts double quotes if set to ENT_COMPAT, both single and double if set to ENT_QUOTES or none if set to ENT_NOQUOTES. Also compatible with old _nxt_specialchars() values; converting single quotes if set to 'single', double if set to 'double' or both if otherwise set. Default is ENT_NOQUOTES.
 * @return string The decoded text without HTML entities.
 */
function nxt_specialchars_decode( $string, $quote_style = ENT_NOQUOTES ) {
	$string = (string) $string;
	...
	// Match the previous behaviour of _nxt_specialchars() when the $quote_style is not an accepted value
	if ( empty( $quote_style ) ) {
		$quote_style = ENT_NOQUOTES;
	} elseif ( !in_array( $quote_style, array( 0, 2, 3, 'single', 'double' ), true ) ) {
		$quote_style = ENT_QUOTES;
 ...
 */
function nxt_check_invalid_utf8( $string, $strip = false ) {
	$string = (string) $string;
...
function sanitize_user( $username, $strict = false ) {
	...
	$username = nxt_strip_all_tags( $username );
...
function convert_chars($content, $deprecated = '') {
	...
	// Fix Word pasting
	$content = strtr($content, $nxt_htmltranswinuni);
...
function force_balance_tags( $text ) {
	...
	// nxt bug fix for comments - in case you REALLY meant to type '< !--'
	$text = str_replace('< !--', '<    !--', $text);
	// nxt bug fix for LOVE <3 (and other situations with '<' before a number)
	...
	// nxt fix for the bug with HTML comments
 ...
 */
function nxt_rel_nofollow( $text ) {
	// This is a pre save filter, so text is already escaped.
	$text = stripslashes($text);
	$text = preg_replace_callback('|<a (.+?)>|i', 'nxt_rel_nofollow_callback', $text);
	$text = esc_sql($text);
	return $text;
}
 ...
 */
function nxt_rel_nofollow_callback( $matches ) {
	$text = $matches[1];
 ...
 *
 * Looks up one smiley code in the $nxtsmiliestrans global array and returns an
...
function translate_smiley($smiley) {
	...
	$img = $nxtsmiliestrans[$smiley];
	$smiley_masked = esc_attr($smiley);

	$srcurl = apply_filters('smilies_src', includes_url("images/smilies/$img"), $img, site_url());

	return " <img src='$srcurl' alt='$smiley_masked' class='nxt-smiley' /> ";
}
 ...
 *
 * @since 0.71
 * @uses $nxt_smiliessearch
 *
 * @param string $text Content to convert smilies from text.
 * @return string Converted content with text smilies replaced with images.
 */
function convert_smilies($text) {
	global $nxt_smiliessearch;
	$output = '';
	if ( get_option('use_smilies') && !empty($nxt_smiliessearch) ) {
		...
		for ($i = 0; $i < $stop; $i++) {
			...
			if ((strlen($content) > 0) && ('<' != $content[0])) { // If it's not a tag
				$content = preg_replace_callback($nxt_smiliessearch, 'translate_smiley', $content);
			}
			$output .= $content;
		}
	} else {
 ...
 *
 * @since 1.2.0
 * @usedby nxt_mail() handles charsets in email subjects
 *
 * @param string $string Subject line
 * @return string Converted string to ASCII
 */
function nxt_iso_descrambler($string) {
	/* this may only work with iso-8859-1, I'm afraid */
	if (!preg_match('#\=\?(.+)\?Q\?(.+)\?\=#i', $string, $matches)) {
		return $string;
	} else {
		$subject = str_replace('_', ' ', $matches[2]);
		$subject = preg_replace_callback('#\=([0-9a-f]{2})#i', '_nxt_iso_convert', $subject);
		return $subject;
	}
}
 ...
 */
function _nxt_iso_convert( $match ) {
 ...
 */
function nxt_trim_words( $text, $num_words = 55, $more = null ) {
	if ( null === $more )
		$more = __( '&hellip;' );
	$original_text = $text;
	$text = nxt_strip_all_tags( $text );
	...
	return apply_filters( 'nxt_trim_words', $text, $num_words, $more, $original_text );
}
 ...
 */
function nxt_htmledit_pre($output) {
	if ( !empty($output) )
 ...
 *
 * This is just a handy shortcut for $nxtdb->escape(), for completeness' sake
 *
 * @since 2.8.0
...
function esc_js( $text ) {
	$safe_text = nxt_check_invalid_utf8( $text );
	$safe_text = _nxt_specialchars( $safe_text, ENT_COMPAT );
...
function esc_html( $text ) {
	$safe_text = nxt_check_invalid_utf8( $text );
	$safe_text = _nxt_specialchars( $safe_text, ENT_QUOTES );
 ...
 */
function nxt_make_link_relative( $link ) {
...
function sanitize_option($option, $value) {
	...
	switch ( $option ) {
		...
		case 'blogname':
			...
			$value = nxt_filter_post_kses( $value ); // calls stripslashes then addslashes
			$value = stripslashes($value);
		...
		case 'upload_path':
			...
			$value = addslashes($value);
			$value = nxt_filter_kses($value); // calls stripslashes then addslashes
			$value = stripslashes($value);
			break;
 ...
 *
 * @since 2.2.1
 * @uses apply_filters() for the 'nxt_parse_str' filter.
 *
 * @param string $string The string to be parsed.
 * @param array $array Variables will be stored in this array.
 */
function nxt_parse_str( $string, &$array ) {
	parse_str( $string, $array );
	if ( get_magic_quotes_gpc() )
		$array = stripslashes_deep( $array );
	$array = apply_filters( 'nxt_parse_str', $array );
}
 ...
 *
 * @uses nxt_pre_kses_less_than_callback in the callback function.
 * @since 2.3.0
 *
 * @param string $text Text to be converted.
 * @return string Converted text.
 */
function nxt_pre_kses_less_than( $text ) {
	return preg_replace_callback('%<[^>]*?((?=<)|>|$)%', 'nxt_pre_kses_less_than_callback', $text);
}
 ...
 */
function nxt_pre_kses_less_than_callback( $matches ) {
...
/**
 * NXTClass implementation of PHP sprintf() with filters.
 *
 * @since 2.5.0
...
function nxt_sprintf( $pattern ) {
	...
	while ( $len > $start ) {
		...
		if ( $pattern[$start] == '%' ) {
			...
			$_fragment = apply_filters( 'nxt_sprintf', $fragment, $arg );
 ...
 */
function nxt_sprintf_l($pattern, $args) {
	// Not a match
	...
	$l = apply_filters('nxt_sprintf_l', array(
 ...
 * @return string The excerpt.
 */
function nxt_html_excerpt( $str, $count ) {
	$str = nxt_strip_all_tags( $str, true );
 ...
 * @return string The processed string.
 */
function nxt_strip_all_tags($string, $remove_breaks = false) {
 ...
 * @return string
 */
function nxt_basename( $path, $suffix = '' ) {
	return urldecode( basename( str_replace( '%2F', '/', urlencode( $path ) ), $suffix ) );
}

/**
 * Forever eliminate "nxtclass" from the planet (or at least the little bit we can influence).
 *
...
function capital_P_dangit( $text ) {
	...
	if ( 'the_title' === current_filter() )
		return str_replace( 'nxtclass', 'NXTClass', $text );
	// Still here? Use the more judicious replacement
	static $dblq = false;
	if ( false === $dblq )
		$dblq = _x('&#8220;', 'opening curly quote');
	return str_replace(
		array( ' nxtclass', '&#8216;nxtclass', $dblq . 'nxtclass', '>nxtclass', '(nxtclass' ),
		array( ' NXTClass', '&#8216;NXTClass', $dblq . 'NXTClass', '>NXTClass', '(NXTClass' ),
	$text );
