...
// Users
add_filter( 'nxtmu_validate_user_signup', 'signup_nonce_check' );
add_action( 'init', 'maybe_add_existing_user_to_blog' );
add_action( 'nxtmu_new_user', 'newuser_notify_siteadmin' );
add_action( 'nxtmu_activate_user', 'add_new_user_to_blog', 10, 3 );
add_action( 'sanitize_user', 'strtolower' );

// Blogs
add_filter( 'nxtmu_validate_blog_signup', 'signup_nonce_check' );
add_action( 'nxtmu_new_blog', 'nxtmu_log_new_registrations', 10, 2 );
add_action( 'nxtmu_new_blog', 'newblog_notify_siteadmin', 10, 2 );
...
add_action( 'delete_post', 'nxtmu_update_blogs_date' );
add_action( 'transition_post_status', '_update_blog_date_on_post_publish', 10, 3 );
add_action( 'admin_init', 'nxt_schedule_update_network_counts');
add_action( 'update_network_counts', 'nxt_update_network_counts');

// Files
add_filter( 'nxt_upload_bits', 'upload_is_file_too_big' );
...
// nxt_HOME and nxt_SITEURL should not have any effect in MS
remove_filter( 'option_siteurl', '_config_nxt_siteurl' );
remove_filter( 'option_home',    '_config_nxt_home'    );
