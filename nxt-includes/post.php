 ...
 * '_nxt_attached_file' and returning it. This is a convenience function to
...
function get_attached_file( $attachment_id, $unfiltered = false ) {
	$file = get_post_meta( $attachment_id, '_nxt_attached_file', true );
	// If the file is relative, prepend upload dir
	if ( 0 !== strpos($file, '/') && !preg_match('|^.:\\\|', $file) && ( ($uploads = nxt_upload_dir()) && false === $uploads['error'] ) )
 ...
 * '_nxt_attached_file' to store the path of the attachment.
 *
 * @since 2.1.0
...
function update_attached_file( $attachment_id, $file ) {
	...
	$file = _nxt_relative_upload_path($file);

	return update_post_meta( $attachment_id, '_nxt_attached_file', $file );
}
 ...
 *
 * @since 2.9.0
 * @uses apply_filters() Calls '_nxt_relative_upload_path' on file path.
 *
 * @param string $path Full path to the file
 * @return string relative path on success, unchanged path on failure.
 */
function _nxt_relative_upload_path( $path ) {
	$new_path = $path;
	...
	return apply_filters( '_nxt_relative_upload_path', $new_path, $path );
}
 ...
 * context of the inner workings of NXTClass, pages, posts, and attachments
 ...
 * criteria of posts to retrieve. The 'post_type' can be anything, but NXTClass
...
function &get_post(&$post, $output = OBJECT, $filter = 'raw') {
	...
	if ( empty($post) ) {
		...
		nxt_cache_add($post->ID, $_post, 'posts');
		...
		$post_id = (int) $post_id;
		if ( ! $_post = nxt_cache_get($post_id, 'posts') ) {
			$_post = $nxtdb->get_row($nxtdb->prepare("SELECT * FROM $nxtdb->posts WHERE ID = %d LIMIT 1", $post_id));
			if ( ! $_post )
				return $null;
			_get_post_ancestors($_post);
			$_post = sanitize_post($_post, 'raw');
			nxt_cache_add($_post->ID, $_post, 'posts');
		}
	}
 ...
 * @return nxt_Error|string Value in post field or nxt_Error on failure
 */
function get_post_field( $field, $post, $context = 'display' ) {
	$post = (int) $post;
	$post = get_post( $post );

	if ( is_nxt_error($post) )
		return $post;
 ...
 *
 * @return mixed The format if successful. False if no format is set.  nxt_Error if errors.
 */
 ...
 * @return mixed nxt_Error on error. Array of affected term IDs on success.
 */
function set_post_format( $post, $format ) {
	$post = get_post($post);

	if ( empty($post) )
		return new nxt_Error('invalid_post', __('Invalid post'));
	...
	return nxt_set_post_terms($post->ID, $format, 'post_format');
}
...
/**
 * Retrieve all of the NXTClass supported post statuses.
 *
...
/**
 * Retrieve all of the NXTClass support page statuses.
 *
 ...
 * @subpackage Post
 * @since 3.0.0
 * @uses $nxt_post_statuses Inserts new post status object into the list
 *
 * @param string $post_status Name of the post status.
 * @param array|string $args See above description.
 */
function register_post_status($post_status, $args = array()) {
	global $nxt_post_statuses;

	if (!is_array($nxt_post_statuses))
		$nxt_post_statuses = array();
	...
	$nxt_post_statuses[$post_status] = $args;
...
function get_post_status_object( $post_status ) {
	...
	if ( empty($nxt_post_statuses[$post_status]) )
		return null;

	return $nxt_post_statuses[$post_status];
}
...
function get_post_stati( $args = array(), $output = 'names', $operator = 'and' ) {
	...
	return nxt_filter_object_list($nxt_post_statuses, $args, $operator, $field);
}
...
function get_post_type_object( $post_type ) {
	global $nxt_post_types;

	if ( empty($nxt_post_types[$post_type]) )
		return null;

	return $nxt_post_types[$post_type];
}
...
function get_post_types( $args = array(), $output = 'names', $operator = 'and' ) {
	...
	return nxt_filter_object_list($nxt_post_types, $args, $operator, $field);
}
 ...
 *
 * @since 2.9.0
 * @uses $nxt_post_types Inserts new post type object into the list
 *
 * @param string $post_type Name of the post type.
 * @param array|string $args See above description.
 * @return object|nxt_Error the registered post type object, or an error object
 */
function register_post_type($post_type, $args = array()) {
	global $nxt_post_types, $nxt_rewrite, $nxt;

	if ( !is_array($nxt_post_types) )
		$nxt_post_types = array();
	...
	if ( strlen( $post_type ) > 20 )
			return new nxt_Error( 'post_type_too_long', __( 'Post types cannot exceed 20 characters in length' ) );
	...
	if ( false !== $args->query_var && !empty($nxt) ) {
		if ( true === $args->query_var )
			$args->query_var = $post_type;
		$args->query_var = sanitize_title_with_dashes($args->query_var);
		$nxt->add_query_var($args->query_var);
	}
	...
	if ( false !== $args->rewrite && ( is_admin() || '' != get_option('permalink_structure') ) ) {
		...
		if ( $args->hierarchical )
			$nxt_rewrite->add_rewrite_tag("%$post_type%", '(.+?)', $args->query_var ? "{$args->query_var}=" : "post_type=$post_type&name=");
		else
			$nxt_rewrite->add_rewrite_tag("%$post_type%", '([^/]+)', $args->query_var ? "{$args->query_var}=" : "post_type=$post_type&name=");
		...
		if ( $args->has_archive ) {
			...
			if ( $args->rewrite['with_front'] )
				$archive_slug = substr( $nxt_rewrite->front, 1 ) . $archive_slug;
			else
				$archive_slug = $nxt_rewrite->root . $archive_slug;

			$nxt_rewrite->add_rule( "{$archive_slug}/?$", "index.php?post_type=$post_type", 'top' );
			if ( $args->rewrite['feeds'] && $nxt_rewrite->feeds ) {
				$feeds = '(' . trim( implode( '|', $nxt_rewrite->feeds ) ) . ')';
				$nxt_rewrite->add_rule( "{$archive_slug}/feed/$feeds/?$", "index.php?post_type=$post_type" . '&feed=$matches[1]', 'top' );
				$nxt_rewrite->add_rule( "{$archive_slug}/$feeds/?$", "index.php?post_type=$post_type" . '&feed=$matches[1]', 'top' );
			}
			if ( $args->rewrite['pages'] )
				$nxt_rewrite->add_rule( "{$archive_slug}/{$nxt_rewrite->pagination_base}/([0-9]{1,})/?$", "index.php?post_type=$post_type" . '&paged=$matches[1]', 'top' );
		}

		$nxt_rewrite->add_permastruct($post_type, "{$args->rewrite['slug']}/%$post_type%", $args->rewrite['with_front'], $args->permalink_epmask);
	}
...
function add_post_type_support( $post_type, $feature ) {
	global $_nxt_post_type_features;

	$features = (array) $feature;
	foreach ($features as $feature) {
		if ( func_num_args() == 2 )
			$_nxt_post_type_features[$post_type][$feature] = true;
		else
			$_nxt_post_type_features[$post_type][$feature] = array_slice( func_get_args(), 2 );
	}
}
...
function remove_post_type_support( $post_type, $feature ) {
	...
	if ( !isset($_nxt_post_type_features[$post_type]) )
		return;

	if ( isset($_nxt_post_type_features[$post_type][$feature]) )
		unset($_nxt_post_type_features[$post_type][$feature]);
}
...
function set_post_type( $post_id = 0, $post_type = 'post' ) {
	...
	$return = $nxtdb->update($nxtdb->posts, array('post_type' => $post_type), array('ID' => $post_id) );
 ...
 *     'offset' - Default is 0. See {@link nxt_Query::query()} for more.
 *     'category' - What category to pull the posts from.
 *     'orderby' - Default is 'post_date'. How to order the posts.
 *     'order' - Default is 'DESC'. The order to retrieve the posts.
 *     'include' - See {@link nxt_Query::query()} for more.
 *     'exclude' - See {@link nxt_Query::query()} for more.
 *     'meta_key' - See {@link nxt_Query::query()} for more.
 *     'meta_value' - See {@link nxt_Query::query()} for more.
...
function get_posts($args = null) {
	...
	if ( ! empty($r['category']) )
		$r['cat'] = $r['category'];
	if ( ! empty($r['include']) ) {
		$incposts = nxt_parse_id_list( $r['include'] );
		$r['posts_per_page'] = count($incposts);  // only the number of posts included
		$r['post__in'] = $incposts;
	} elseif ( ! empty($r['exclude']) )
		$r['post__not_in'] = nxt_parse_id_list( $r['exclude'] );
...
function get_post_custom( $post_id = 0 ) {
	...
	if ( ! nxt_cache_get( $post_id, 'post_meta' ) )
		update_postmeta_cache( $post_id );

	return nxt_cache_get( $post_id, 'post_meta' );
}
 ...
 */
function nxt_count_attachments( $mime_type = '' ) {
	global $nxtdb;

	$and = nxt_post_mime_type_where( $mime_type );
	$count = $nxtdb->get_results( "SELECT post_mime_type, COUNT( * ) AS num_posts FROM $nxtdb->posts WHERE post_type = 'attachment' AND post_status != 'trash' $and GROUP BY post_mime_type", ARRAY_A );

	$stats = array( );
	foreach( (array) $count as $row ) {
		$stats[$row['post_mime_type']] = $row['num_posts'];
	}
	$stats['trash'] = $nxtdb->get_var( "SELECT COUNT( * ) FROM $nxtdb->posts WHERE post_type = 'attachment' AND post_status = 'trash' $and");
 ...
 */
function nxt_match_mime_types($wildcard_mime_types, $real_mime_types) {
	$matches = array();
 ...
 */
function nxt_post_mime_type_where($post_mime_types, $table_alias = '') {
	$where = '';
	$wildcards = array('', '%', '%/%');
 ...
 * @uses nxt_delete_post() if trash is disabled
 *
 * @param int $post_id Post ID.
 * @return mixed False on failure
 */
function nxt_trash_post($post_id = 0) {
	if ( !EMPTY_TRASH_DAYS )
		return nxt_delete_post($post_id, true);

	if ( !$post = nxt_get_single_post($post_id, ARRAY_A) )
		return $post;

	if ( $post['post_status'] == 'trash' )
		return false;

	do_action('nxt_trash_post', $post_id);

	add_post_meta($post_id,'_nxt_trash_meta_status', $post['post_status']);
	add_post_meta($post_id,'_nxt_trash_meta_time', time());

	$post['post_status'] = 'trash';
	nxt_insert_post($post);

	nxt_trash_post_comments($post_id);
 ...
 * @return mixed False on failure
 */
function nxt_untrash_post($post_id = 0) {
	if ( !$post = nxt_get_single_post($post_id, ARRAY_A) )
		return $post;
	...
	$post_status = get_post_meta($post_id, '_nxt_trash_meta_status', true);
	...
	nxt_untrash_post_comments($post_id);
 ...
 * @return mixed False on failure
 */
function nxt_trash_post_comments($post = null) {
	global $nxtdb;
	...
	$comments = $nxtdb->get_results( $nxtdb->prepare("SELECT comment_ID, comment_approved FROM $nxtdb->comments WHERE comment_post_ID = %d", $post_id) );
	if ( empty($comments) )
		return;
	...
	add_post_meta($post_id, '_nxt_trash_meta_comments_status', $statuses);

	// Set status for all comments to post-trashed
	$result = $nxtdb->update($nxtdb->comments, array('comment_approved' => 'post-trashed'), array('comment_post_ID' => $post_id));
 ...
 * @return mixed False on failure
 */
function nxt_untrash_post_comments($post = null) {
	global $nxtdb;
	...
	$statuses = get_post_meta($post_id, '_nxt_trash_meta_comments_status', true);
	...
	foreach ( $group_by_status as $status => $comments ) {
		...
		$nxtdb->query( "UPDATE $nxtdb->comments SET comment_approved = '$status' WHERE comment_ID IN ('" . $comments_in . "')" );
	}

	clean_comment_cache( array_keys($statuses) );

	delete_post_meta($post_id, '_nxt_trash_meta_comments_status');
 ...
 * @since 2.1.0
 *
 * @uses nxt_get_object_terms() Retrieves the categories. Args details can be found here.
 *
 * @param int $post_id Optional. The Post ID.
 * @param array $args Optional. Overwrite the defaults.
 * @return array
 */
function nxt_get_post_categories( $post_id = 0, $args = array() ) {
	$post_id = (int) $post_id;

	$defaults = array('fields' => 'ids');
	$args = nxt_parse_args( $args, $defaults );

	$cats = nxt_get_object_terms($post_id, 'category', $args);
	return $cats;
}
 ...
 * {@link nxt_get_object_terms()}.
 *
 * @package NXTClass
 * @subpackage Post
 * @since 2.3.0
 *
 * @uses nxt_get_object_terms() Gets the tags for returning. Args can be found here
 *
 * @param int $post_id Optional. The Post ID
 * @param array $args Optional. Overwrite the defaults
 * @return array List of post tags.
 */
function nxt_get_post_tags( $post_id = 0, $args = array() ) {
	return nxt_get_post_terms( $post_id, 'post_tag', $args);
}
 ...
 * @return array List of post tags.
 */
function nxt_get_post_terms( $post_id = 0, $taxonomy = 'post_tag', $args = array() ) {
	$post_id = (int) $post_id;

	$defaults = array('fields' => 'all');
	$args = nxt_parse_args( $args, $defaults );

	$tags = nxt_get_object_terms($post_id, $taxonomy, $args);
 ...
 * @return unknown.
 */
function nxt_get_recent_posts( $args = array(), $output = ARRAY_A ) {
 ...
 * @uses do_action() Calls 'save_post' and 'nxt_insert_post' on post id and post data just before returning.
 * @uses apply_filters() Calls 'nxt_insert_post_data' passing $data, $postarr prior to database update or insert.
 * @uses nxt_transition_post_status()
 *
 * @param array $postarr Elements that make up post to insert.
 * @param bool $nxt_error Optional. Allow return of nxt_Error on failure.
 * @return int|nxt_Error The value 0 or nxt_Error on failure. The post ID on success.
 */
function nxt_insert_post($postarr, $nxt_error = false) {
	global $nxtdb, $nxt_rewrite, $user_ID;
	...
	$postarr = nxt_parse_args($postarr, $defaults);
	...
	if ( apply_filters( 'nxt_insert_post_empty_content', $maybe_empty, $postarr ) ) {
		if ( $nxt_error )
			return new nxt_Error( 'empty_content', __( 'Content, title, and excerpt are empty.' ) );
		else
			return 0;
	}
	...
	$post_parent = apply_filters( 'nxt_insert_post_parent', $post_parent, $post_ID, compact( array_keys( $postarr ) ), $postarr );
	...
	$data = apply_filters('nxt_insert_post_data', $data, $postarr);
	$data = stripslashes_deep( $data );
	$where = array( 'ID' => $post_ID );

	if ( $update ) {
		do_action( 'pre_post_update', $post_ID );
		if ( false === $nxtdb->update( $nxtdb->posts, $data, $where ) ) {
			if ( $nxt_error )
				return new nxt_Error('db_update_error', __('Could not update post in the database'), $nxtdb->last_error);
			else
				return 0;
		}
	} else {
		...
		if ( false === $nxtdb->insert( $nxtdb->posts, $data ) ) {
			if ( $nxt_error )
				return new nxt_Error('db_insert_error', __('Could not insert post into the database'), $nxtdb->last_error);
			else
				return 0;
		}
	...
	if ( empty($data['post_name']) && !in_array( $data['post_status'], array( 'draft', 'pending', 'auto-draft' ) ) ) {
		...
		$nxtdb->update( $nxtdb->posts, array( 'post_name' => $data['post_name'] ), $where );
	}
	...
	if ( isset( $tags_input ) && is_object_in_taxonomy($post_type, 'post_tag') )
		nxt_set_post_tags( $post_ID, $tags_input );
	...
	if ( !empty($tax_input) ) {
		foreach ( $tax_input as $taxonomy => $tags ) {
			...
			if ( current_user_can($taxonomy_obj->cap->assign_terms) )
				nxt_set_post_terms( $post_ID, $tags, $taxonomy );
		}
	}
	...
	if ( !$update && '' == $current_guid )
		$nxtdb->update( $nxtdb->posts, array( 'guid' => get_permalink( $post_ID ) ), $where );
	...
	if ( !empty($page_template) && 'page' == $data['post_type'] ) {
		...
		if ( 'default' != $page_template && !in_array($page_template, $page_templates) ) {
			if ( $nxt_error )
				return new nxt_Error('invalid_page_template', __('The page template is invalid.'));
			else
				return 0;
		}
		update_post_meta($post_ID, '_nxt_page_template',  $page_template);
	}

	nxt_transition_post_status($data['post_status'], $previous_status, $post);
	...
	do_action('nxt_insert_post', $post_ID, $post);
 ...
 */
function nxt_update_post($postarr = array()) {
	...
	$post = nxt_get_single_post($postarr['ID'], ARRAY_A);
	...
	if ($postarr['post_type'] == 'attachment')
		return nxt_insert_attachment($postarr);

	return nxt_insert_post($postarr);
}
 ...
 * @since 2.1.0
 * @uses $nxtdb
 * @uses do_action() Calls 'edit_post', 'save_post', and 'nxt_insert_post' on post_id and post data.
 *
 * @param int $post_id Post ID.
 * @return null
 */
function nxt_publish_post($post_id) {
	global $nxtdb;
	...
	$nxtdb->update( $nxtdb->posts, array( 'post_status' => 'publish' ), array( 'ID' => $post_id ) );

	$old_status = $post->post_status;
	$post->post_status = 'publish';
	nxt_transition_post_status('publish', $old_status, $post);
...
function check_and_publish_future_post($post_id) {
	...
	if ( $time > time() ) { // Uh oh, someone jumped the gun!
		nxt_clear_scheduled_hook( 'publish_future_post', array( $post_id ) ); // clear anything else in the system
		nxt_schedule_single_event( $time, 'publish_future_post', array( $post_id ) );
		return;
	}

	return nxt_publish_post($post_id);
}
 ...
 *
 * @global nxtdb $nxtdb
 * @global nxt_Rewrite $nxt_rewrite
 ...
 */
function nxt_unique_post_slug( $slug, $post_ID, $post_status, $post_type, $post_parent ) {
	if ( in_array( $post_status, array( 'draft', 'pending', 'auto-draft' ) ) )
		return $slug;

	global $nxtdb, $nxt_rewrite;

	$feeds = $nxt_rewrite->feeds;
	if ( ! is_array( $feeds ) )
		$feeds = array();
	...
	if ( 'attachment' == $post_type ) {
		...
		$check_sql = "SELECT post_name FROM $nxtdb->posts WHERE post_name = %s AND ID != %d LIMIT 1";
		$post_name_check = $nxtdb->get_var( $nxtdb->prepare( $check_sql, $slug, $post_ID ) );

		if ( $post_name_check || in_array( $slug, $feeds ) || apply_filters( 'nxt_unique_post_slug_is_bad_attachment_slug', false, $slug ) ) {
			$suffix = 2;
			do {
				$alt_post_name = substr ($slug, 0, 200 - ( strlen( $suffix ) + 1 ) ) . "-$suffix";
				$post_name_check = $nxtdb->get_var( $nxtdb->prepare($check_sql, $alt_post_name, $post_ID ) );
				$suffix++;
		...
		$check_sql = "SELECT post_name FROM $nxtdb->posts WHERE post_name = %s AND post_type IN ( '" . implode( "', '", esc_sql( $hierarchical_post_types ) ) . "' ) AND ID != %d AND post_parent = %d LIMIT 1";
		$post_name_check = $nxtdb->get_var( $nxtdb->prepare( $check_sql, $slug, $post_ID, $post_parent ) );

		if ( $post_name_check || in_array( $slug, $feeds ) || preg_match( "@^($nxt_rewrite->pagination_base)?\d+$@", $slug )  || apply_filters( 'nxt_unique_post_slug_is_bad_hierarchical_slug', false, $slug, $post_type, $post_parent ) ) {
			$suffix = 2;
			do {
				$alt_post_name = substr( $slug, 0, 200 - ( strlen( $suffix ) + 1 ) ) . "-$suffix";
				$post_name_check = $nxtdb->get_var( $nxtdb->prepare( $check_sql, $alt_post_name, $post_ID, $post_parent ) );
				$suffix++;
			} while ( $post_name_check );
			$slug = $alt_post_name;
		}
		...
		$check_sql = "SELECT post_name FROM $nxtdb->posts WHERE post_name = %s AND post_type = %s AND ID != %d LIMIT 1";
		$post_name_check = $nxtdb->get_var( $nxtdb->prepare( $check_sql, $slug, $post_type, $post_ID ) );

		if ( $post_name_check || in_array( $slug, $feeds ) || apply_filters( 'nxt_unique_post_slug_is_bad_flat_slug', false, $slug, $post_type ) ) {
			$suffix = 2;
			do {
				$alt_post_name = substr( $slug, 0, 200 - ( strlen( $suffix ) + 1 ) ) . "-$suffix";
				$post_name_check = $nxtdb->get_var( $nxtdb->prepare( $check_sql, $alt_post_name, $post_type, $post_ID ) );
				$suffix++;
			} while ( $post_name_check );
			$slug = $alt_post_name;
		}
	...
	return apply_filters( 'nxt_unique_post_slug', $slug, $post_ID, $post_status, $post_type, $post_parent );
}

/**
 * Adds tags to a post.
 *
 * @uses nxt_set_post_tags() Same first two parameters, but the last parameter is always set to true.
 *
 * @package NXTClass
 * @subpackage Post
 * @since 2.3.0
 ...
 */
function nxt_add_post_tags($post_id = 0, $tags = '') {
	return nxt_set_post_tags($post_id, $tags, true);
}
 ...
 * @return mixed Array of affected term IDs. nxt_Error or false on failure.
 */
function nxt_set_post_terms( $post_id = 0, $tags = '', $taxonomy = 'post_tag', $append = false ) {
	$post_id = (int) $post_id;
	...
	return nxt_set_object_terms($post_id, $tags, $taxonomy, $append);
}
 ...
 */
function nxt_transition_post_status($new_status, $old_status, $post) {
...
function add_ping($post_id, $uri) {
	global $nxtdb;
	$pung = $nxtdb->get_var( $nxtdb->prepare( "SELECT pinged FROM $nxtdb->posts WHERE ID = %d", $post_id ));
	$pung = trim($pung);
	...
	return $nxtdb->update( $nxtdb->posts, array( 'pinged' => $new ), array( 'ID' => $post_id ) );
}
...
function get_to_ping($post_id) {
	global $nxtdb;
	$to_ping = $nxtdb->get_var( $nxtdb->prepare( "SELECT to_ping FROM $nxtdb->posts WHERE ID = %d", $post_id ));
...
function get_all_page_ids() {
	...
	if ( ! $page_ids = nxt_cache_get('all_page_ids', 'posts') ) {
		$page_ids = $nxtdb->get_col("SELECT ID FROM $nxtdb->posts WHERE post_type = 'page'");
		nxt_cache_add('all_page_ids', $page_ids, 'posts');
	}
...
function get_page_by_path($page_path, $output = OBJECT, $post_type = 'page') {
	...
	$nxtdb->escape_by_ref( $post_type_sql );
	$pages = $nxtdb->get_results( "SELECT ID, post_name, post_parent FROM $nxtdb->posts WHERE post_name IN ($in_string) AND (post_type = '$post_type_sql' OR post_type = 'attachment')", OBJECT_K );
...
function get_page_by_title($page_title, $output = OBJECT, $post_type = 'page' ) {
	global $nxtdb;
	$page = $nxtdb->get_var( $nxtdb->prepare( "SELECT ID FROM $nxtdb->posts WHERE post_title = %s AND post_type= %s", $page_title, $post_type ) );
	if ( $page )
 ...
 */
function nxt_get_attachment_metadata( $post_id = 0, $unfiltered = false ) {
	$post_id = (int) $post_id;
	if ( !$post =& get_post( $post_id ) )
		return false;

	$data = get_post_meta( $post->ID, '_nxt_attachment_metadata', true );

	if ( $unfiltered )
		return $data;

	return apply_filters( 'nxt_get_attachment_metadata', $data, $post->ID );
}
 ...
 * @return int
 */
function nxt_update_attachment_metadata( $post_id, $data ) {
	$post_id = (int) $post_id;
	if ( !$post =& get_post( $post_id ) )
		return false;

	$data = apply_filters( 'nxt_update_attachment_metadata', $data, $post->ID );

	return update_post_meta( $post->ID, '_nxt_attachment_metadata', $data);
}
...
function nxt_get_attachment_url( $post_id = 0 ) {
	...
	$url = '';
	if ( $file = get_post_meta( $post->ID, '_nxt_attached_file', true) ) { //Get attached file
		if ( ($uploads = nxt_upload_dir()) && false === $uploads['error'] ) { //Get upload directory
 ...
 * @return bool
 */
function nxt_attachment_is_image( $post_id = 0 ) {
	$post_id = (int) $post_id;
	if ( !$post =& get_post( $post_id ) )
		return false;
...
function nxt_mime_type_icon( $mime = 0 ) {
	...
	return apply_filters( 'nxt_mime_type_icon', $icon, $mime, $post_id ); // Last arg is 0 if function pass mime type.
}
 ...
 * added to the post meta field ('_nxt_old_slug') for storing old slugs for that
 * post.
 *
 ...
 */
function nxt_check_for_changed_slugs($post_id, $post, $post_before) {
	...
	$old_slugs = (array) get_post_meta($post_id, '_nxt_old_slug');

	// if we haven't added this old slug before, add it now
	if ( !empty( $post_before->post_name ) && !in_array($post_before->post_name, $old_slugs) )
		add_post_meta($post_id, '_nxt_old_slug', $post_before->post_name);

	// if the new slug was used previously, delete it from the list
	if ( in_array($post->post_name, $old_slugs) )
		delete_post_meta($post_id, '_nxt_old_slug', $post->post_name);
}
...
function get_posts_by_author_sql( $post_type, $full = true, $post_author = null ) {
	...
	if ( $full ) {
		if ( null === $post_author ) {
			$sql = $nxtdb->prepare( 'WHERE post_type = %s AND ', $post_type );
		} else {
			$sql = $nxtdb->prepare( 'WHERE post_author = %d AND post_type = %s AND ', $post_author, $post_type );
		}
	} else {
		$sql = '';
	}
 ...
 *
 * This function not run if $_nxt_suspend_cache_invalidation is not empty. See
 * nxt_suspend_cache_invalidation().
 *
 * @package NXTClass
 * @subpackage Cache
 * @since 2.0.0
...
function clean_post_cache($id) {
	global $_nxt_suspend_cache_invalidation, $nxtdb;

	if ( !empty($_nxt_suspend_cache_invalidation) )
		return;
	...
	nxt_cache_delete($id, 'posts');
	nxt_cache_delete($id, 'post_meta');

	clean_object_term_cache($id, 'post');

	nxt_cache_delete( 'nxt_get_archives', 'general' );

	do_action('clean_post_cache', $id);

	if ( $children = $nxtdb->get_col( $nxtdb->prepare("SELECT ID FROM $nxtdb->posts WHERE post_parent = %d", $id) ) ) {
	...
	if ( is_multisite() )
		nxt_cache_delete( $nxtdb->blogid . '-' . $id, 'global-posts' );
}
...
function clean_page_cache($id) {
	...
	nxt_cache_delete( 'all_page_ids', 'posts' );
	nxt_cache_delete( 'get_pages', 'posts' );
 ...
 *
 * This function will not run if $_nxt_suspend_cache_invalidation is not empty. See
 * nxt_suspend_cache_invalidation().
 *
 * @package NXTClass
 * @subpackage Cache
...
function clean_attachment_cache($id, $clean_terms = false) {
	global $_nxt_suspend_cache_invalidation;
 ...
 *   nxt_transition_post_status() and the default filter for _future_post_hook().
 * @param object $post Object type containing the post information
 */
function _future_post_hook( $deprecated = '', $post ) {
	nxt_clear_scheduled_hook( 'publish_future_post', array( $post->ID ) );
	nxt_schedule_single_event( strtotime( get_gmt_from_date( $post->post_date ) . ' GMT') , 'publish_future_post', array( $post->ID ) );
}
 ...
 * recently. For pages, the rewrite rules of NXTClass are flushed to allow for
 * any changes.
 *
 ...
 * @since 2.3.0
 * @access private
 * @uses $nxt_rewrite Flushes Rewrite Rules.
 *
...
function _get_post_ancestors(&$_post) {
	...
	while ( $ancestor = $nxtdb->get_var( $nxtdb->prepare("SELECT `post_parent` FROM $nxtdb->posts WHERE ID = %d LIMIT 1", $id) ) ) {
 ...
 * @since 2.6.0
 * @access private
 * @uses apply_filters() Calls '_nxt_post_revision_fields' on 'title', 'content' and 'excerpt' fields.
 *
 * @param array $post Optional a post array to be processed for insertion as a post revision.
 * @param bool $autosave optional Is the revision an autosave?
 * @return array Post array ready to be inserted as a post revision or array of fields that can be versioned.
 */
function _nxt_post_revision_fields( $post = null, $autosave = false ) {
	static $fields = false;
	...
	if ( !$fields ) {
		...
		// Runs only once
		$fields = apply_filters( '_nxt_post_revision_fields', $fields );

		// nxt uses these internally either in versioning or elsewhere - they cannot be versioned
...
/**
 * Internally used to hack nxt_Query into submission.
 *
 * @package NXTClass
 * @subpackage Post_Revisions
 * @since 2.6.0
 *
 * @param object $query nxt_Query object
 */
function _nxt_get_post_autosave_hack( $query ) {
 ...
 */
function nxt_is_post_revision( $post ) {
	if ( !$post = nxt_get_post_revision( $post ) )
		return false;
 ...
 */
function nxt_is_post_autosave( $post ) {
	if ( !$post = nxt_get_post_revision( $post ) )
		return false;
 ...
 * @since 2.6.0
 *
 * @uses nxt_insert_post()
 *
 * @param int|object|array $post Post ID, post object OR post array.
 * @param bool $autosave Optional. Is the revision an autosave?
 * @return mixed Null or 0 if error, new revision ID if success.
 */
function _nxt_put_post_revision( $post = null, $autosave = false ) {
	if ( is_object($post) )
	...
	if ( isset($post['post_type']) && 'revision' == $post['post_type'] )
		return new nxt_Error( 'post_type', __( 'Cannot create a revision of a revision' ) );

	$post = _nxt_post_revision_fields( $post, $autosave );
	$post = add_magic_quotes($post); //since data is from db

	$revision_id = nxt_insert_post( $post );
	if ( is_nxt_error($revision_id) )
		return $revision_id;

	if ( $revision_id )
		do_action( '_nxt_put_post_revision', $revision_id );
	return $revision_id;
}
 ...
 */
function &nxt_get_post_revision(&$post, $output = OBJECT, $filter = 'raw') {
	$null = null;
 ...
 */
function nxt_get_post_revisions( $post_id = 0, $args = null ) {
	if ( ! nxt_POST_REVISIONS )
		return array();
 ...
 */
function nxt_get_post_parent_id( $post_ID ) {
	$post = get_post( $post_ID );
	if ( !$post || is_nxt_error( $post ) )
		return false;
	return (int) $post->post_parent;
}
...
function _post_format_link( $link, $term, $taxonomy ) {
	...
	if ( 'post_format' != $taxonomy )
		return $link;
	if ( $nxt_rewrite->get_extra_permastruct( $taxonomy ) ) {
...
/**
 * Remove the post format prefix from the name property of the term objects created by nxt_get_object_terms().
 *
 * @access private
 * @since 3.1.0
 */
function _post_format_nxt_get_object_terms( $terms ) {
	foreach ( (array) $terms as $order => $term ) {
		if ( isset( $term->taxonomy ) && 'post_format' == $term->taxonomy ) {
			$terms[$order]->name = get_post_format_string( str_replace( 'post-format-', '', $term->slug ) );
		}
...
add_filter( 'nxt_get_object_terms', '_post_format_nxt_get_object_terms' );
