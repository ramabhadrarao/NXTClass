...
if ( !function_exists('nxt_set_current_user') ) :
/**
 * Changes the current user by ID or name.
 *
 * Set $id to null and specify a name if you do not know a user's ID.
 *
 * Some NXTClass functionality is based on the current user and not based on
 ...
 * @return nxt_User Current user User object
 */
function nxt_set_current_user($id, $name = '') {
	global $current_user;

	if ( isset($current_user) && ($id == $current_user->ID) )
		return $current_user;

	$current_user = new nxt_User($id, $name);
...
if ( !function_exists('nxt_get_current_user') ) :
/**
 * Retrieve the current user object.
 *
 * @since 2.0.3
 *
 * @return nxt_User Current user nxt_User object
 */
function nxt_get_current_user() {
	global $current_user;
 ...
 *
 * Using the two 'nxt_mail_from' and 'nxt_mail_from_name' hooks allow from
 * creating a from address like 'Name <email@address.com>' when both are set. If
 * just 'nxt_mail_from' is set, then just the email address will be used with no
 * name.
 *
 * The default content type is 'text/plain' which does not allow using HTML.
 * However, you can set the content type of the email by using the
 * 'nxt_mail_content_type' filter.
 *
 * The default charset is based on the charset used on the blog. The charset can
 * be set using the 'nxt_mail_charset' filter.
 *
 * @since 1.2.1
 * @uses apply_filters() Calls 'nxt_mail' hook on an array of all of the parameters.
 * @uses apply_filters() Calls 'nxt_mail_from' hook to get the from email address.
 * @uses apply_filters() Calls 'nxt_mail_from_name' hook to get the from address name.
 * @uses apply_filters() Calls 'nxt_mail_content_type' hook to get the email content type.
 * @uses apply_filters() Calls 'nxt_mail_charset' hook to get the email charset
 ...
 */
function nxt_mail( $to, $subject, $message, $headers = '', $attachments = array() ) {
	// Compact the input, apply the filters, and extract them back out
	extract( apply_filters( 'nxt_mail', compact( 'to', 'subject', 'message', 'headers', 'attachments' ) ) );
	...
	if ( !is_object( $phpmailer ) || !is_a( $phpmailer, 'PHPMailer' ) ) {
		require_once ABSPATH . nxtINC . '/class-phpmailer.php';
		require_once ABSPATH . nxtINC . '/class-smtp.php';
	...
	/* If we don't have an email from the input headers default to nxtclass@$sitename
	...
	if ( !isset( $from_email ) ) {
		...
		$from_email = 'nxtclass@' . $sitename;
	}

	// Plugin authors can override the potentially troublesome default
	$phpmailer->From     = apply_filters( 'nxt_mail_from'     , $from_email );
	$phpmailer->FromName = apply_filters( 'nxt_mail_from_name', $from_name  );
	...
	$content_type = apply_filters( 'nxt_mail_content_type', $content_type );
	...
	$phpmailer->CharSet = apply_filters( 'nxt_mail_charset', $charset );
...
if ( !function_exists('nxt_authenticate') ) :
/**
 ...
 * @return nxt_Error|nxt_User nxt_User object if login successful, otherwise nxt_Error object.
 */
function nxt_authenticate($username, $password) {
	...
	if ( $user == null ) {
		...
		$user = new nxt_Error('authentication_failed', __('<strong>ERROR</strong>: Invalid username or incorrect password.'));
	}

	$ignore_codes = array('empty_username', 'empty_password');

	if (is_nxt_error($user) && !in_array($user->get_error_code(), $ignore_codes) ) {
		do_action('nxt_login_failed', $username);
	}
...
if ( !function_exists('nxt_validate_auth_cookie') ) :
/**
 ...
 */
function nxt_validate_auth_cookie($cookie = '', $scheme = '') {
	if ( ! $cookie_elements = nxt_parse_auth_cookie($cookie, $scheme) ) {
	...
	$key = nxt_hash($username . $pass_frag . '|' . $expiration, $scheme);
...
if ( !function_exists('nxt_generate_auth_cookie') ) :
/**
 ...
 */
function nxt_generate_auth_cookie($user_id, $expiration, $scheme = 'auth') {
	$user = get_userdata($user_id);

	$pass_frag = substr($user->user_pass, 8, 4);

	$key = nxt_hash($user->user_login . $pass_frag . '|' . $expiration, $scheme);
...
if ( !function_exists('nxt_parse_auth_cookie') ) :
/**
 ...
 */
function nxt_parse_auth_cookie($cookie = '', $scheme = '') {
	if ( empty($cookie) ) {
		switch ($scheme){
			case 'auth':
...
if ( !function_exists('nxt_set_auth_cookie') ) :
/**
...
function nxt_set_auth_cookie($user_id, $remember = false, $secure = '') {
	...
	$auth_cookie = nxt_generate_auth_cookie($user_id, $expiration, $scheme);
	$logged_in_cookie = nxt_generate_auth_cookie($user_id, $expiration, 'logged_in');
...
if ( !function_exists('nxt_clear_auth_cookie') ) :
/**
 * Removes all of the cookies associated with authentication.
 *
 * @since 2.5
 */
function nxt_clear_auth_cookie() {
...
if ( !function_exists('nxt_redirect') ) :
/**
 * Redirects to another page.
 *
 * @since 1.5.1
 * @uses apply_filters() Calls 'nxt_redirect' hook on $location and $status.
 *
 * @param string $location The path to redirect to
 * @param int $status Status code to use
 * @return bool False if $location is not set
 */
function nxt_redirect($location, $status = 302) {
	global $is_IIS;

	$location = apply_filters('nxt_redirect', $location, $status);
	$status = apply_filters('nxt_redirect_status', $status, $location);

	if ( !$location ) // allows the nxt_redirect filter to cancel a redirect
		return false;
...
if ( !function_exists('nxt_sanitize_redirect') ) :
/**
...
if ( !function_exists('nxt_safe_redirect') ) :
/**
...
if ( !function_exists('nxt_validate_redirect') ) :
/**
 ...
 *		NXTClass host string and $location host string.
 *
 * @param string $location The redirect to validate
 * @param string $default The value to return is $location is not allowed
 * @return string redirect-sanitized URL
 **/
function nxt_validate_redirect($location, $default = '') {
	...
	$nxtp = parse_url(home_url());

	$allowed_hosts = (array) apply_filters('allowed_redirect_hosts', array($nxtp['host']), isset($lp['host']) ? $lp['host'] : '');

	if ( isset($lp['host']) && ( !in_array($lp['host'], $allowed_hosts) && $lp['host'] != strtolower($nxtp['host'])) )
		$location = $default;
...
if ( ! function_exists('nxt_notify_postauthor') ) :
/**
 ...
 */
function nxt_notify_postauthor( $comment_id, $comment_type = '' ) {
	...
	$nxt_email = 'nxtclass@' . preg_replace('#^www\.#', '', strtolower($_SERVER['SERVER_NAME']));

	if ( '' == $comment->comment_author ) {
		$from = "From: \"$blogname\" <$nxt_email>";
		if ( '' != $comment->comment_author_email )
			$reply_to = "Reply-To: $comment->comment_author_email";
	} else {
		$from = "From: \"$comment->comment_author\" <$nxt_email>";
		if ( '' != $comment->comment_author_email )
	...
	@nxt_mail( $author->user_email, $subject, $notify_message, $message_headers );

	return true;
}
endif;

if ( !function_exists('nxt_notify_moderator') ) :
/**
 ...
 */
function nxt_notify_moderator($comment_id) {
	global $nxtdb;
	...
	$comment_author_domain = @gethostbyaddr($comment->comment_author_IP);
	$comments_waiting = $nxtdb->get_var("SELECT count(comment_ID) FROM $nxtdb->comments WHERE comment_approved = '0'");
	...
	foreach ( $email_to as $email )
		@nxt_mail($email, $subject, $notify_message, $message_headers);

	return true;
}
endif;

if ( !function_exists('nxt_password_change_notification') ) :
/**
 ...
 */
function nxt_password_change_notification(&$user) {
	...
	if ( $user->user_email != get_option('admin_email') ) {
		...
		// we want to reverse this for the plain text arena of emails.
		$blogname = nxt_specialchars_decode(get_option('blogname'), ENT_QUOTES);
		nxt_mail(get_option('admin_email'), sprintf(__('[%s] Password Lost/Changed'), $blogname), $message);
	}
}
endif;

if ( !function_exists('nxt_new_user_notification') ) :
/**
...
if ( !function_exists('nxt_nonce_tick') ) :
/**
...
if ( !function_exists('nxt_verify_nonce') ) :
/**
 ...
 */
function nxt_verify_nonce($nonce, $action = -1) {
	$user = nxt_get_current_user();
	$uid = (int) $user->ID;
	...
	if ( substr(nxt_hash($i . $action . $uid, 'nonce'), -12, 10) == $nonce )
		return 1;
	// Nonce generated 12-24 hours ago
	if ( substr(nxt_hash(($i - 1) . $action . $uid, 'nonce'), -12, 10) == $nonce )
		return 2;
	// Invalid nonce
	return false;
}
...
if ( !function_exists('nxt_hash') ) :
/**
 * Get hash of given string.
 *
 * @since 2.0.3
 * @uses nxt_salt() Get NXTClass salt
 *
 * @param string $data Plain text to hash
 * @return string Hash of $data
 */
function nxt_hash($data, $scheme = 'auth') {
	$salt = nxt_salt($scheme);

	return hash_hmac('md5', $data, $salt);
}
endif;

if ( !function_exists('nxt_hash_password') ) :
/**
 ...
 *
 * @since 2.5
 * @global object $nxt_hasher PHPass object
 * @uses PasswordHash::HashPassword
 *
 * @param string $password Plain text user password to hash
 * @return string The hash string of the password
 */
function nxt_hash_password($password) {
	global $nxt_hasher;

	if ( empty($nxt_hasher) ) {
		require_once( ABSPATH . 'nxt-includes/class-phpass.php');
		// By default, use the portable hash from phpass
		$nxt_hasher = new PasswordHash(8, TRUE);
	}

	return $nxt_hasher->HashPassword($password);
}
endif;

if ( !function_exists('nxt_check_password') ) :
/**
...
function nxt_check_password($password, $hash, $user_id = '') {
	...
	if ( strlen($hash) <= 32 ) {
		...
		if ( $check && $user_id ) {
			...
			nxt_set_password($password, $user_id);
...
if ( !function_exists('nxt_generate_password') ) :
/**
 ...
 **/
function nxt_generate_password( $length = 12, $special_chars = true, $extra_special_chars = false ) {
	...
	$password = '';
	for ( $i = 0; $i < $length; $i++ ) {
		$password .= substr($chars, nxt_rand(0, strlen($chars) - 1), 1);
	}
...
function nxt_rand( $min = 0, $max = 0 ) {
	...
	// Strip the first eight, leaving the remainder for the next call to nxt_rand().
...
if ( !function_exists('nxt_set_password') ) :
/**
...
if ( !function_exists( 'nxt_text_diff' ) ) :
/**
 ...
 *
 * @since 2.6
 * @see nxt_parse_args() Used to change defaults to user defined settings.
 * @uses Text_Diff
 * @uses nxt_Text_Diff_Renderer_Table
 *
 ...
 */
function nxt_text_diff( $left_string, $right_string, $args = null ) {
	$defaults = array( 'title' => '', 'title_left' => '', 'title_right' => '' );
	$args = nxt_parse_args( $args, $defaults );

	if ( !class_exists( 'nxt_Text_Diff_Renderer_Table' ) )
		require( ABSPATH . nxtINC . '/nxt-diff.php' );
	...
	$renderer  = new nxt_Text_Diff_Renderer_Table();
