 ...
 *
 * Schedules a hook which will be executed once by the NXTClass actions core at
 ...
 */
function nxt_schedule_single_event( $timestamp, $hook, $args = array()) {
	// don't schedule a duplicate if there's already an identical event due in the next 10 minutes
	$next = nxt_next_scheduled($hook, $args);
 ...
 *
 * Schedules a hook which will be executed by the NXTClass actions core on a
 * specific interval, specified by you. The action will trigger when someone
 * visits your NXTClass site, if the scheduled time has passed.
 *
 * Valid values for the recurrence are hourly, daily and twicedaily.  These can
 * be extended using the cron_schedules filter in nxt_get_schedules().
 *
 * Use nxt_next_scheduled() to prevent duplicates
 *
 * @since 2.1.0
 *
 * @param int $timestamp Timestamp for when to run the event.
 ...
 */
function nxt_schedule_event( $timestamp, $recurrence, $hook, $args = array()) {
	$crons = _get_cron_array();
 ...
 */
function nxt_unschedule_event( $timestamp, $hook, $args = array() ) {
	$crons = _get_cron_array();
	$key = md5(serialize($args));
 ...
 */
function nxt_next_scheduled( $hook, $args = array() ) {
	$crons = _get_cron_array();
	$key = md5(serialize($args));
	if ( empty($crons) )
		return false;
 ...
 */
function nxt_get_schedule($hook, $args = array()) {
	$crons = _get_cron_array();
	$key = md5(serialize($args));
	if ( empty($crons) )
		return false;
