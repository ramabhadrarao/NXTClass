 ...
 *
 * @see nxt_Rewrite::add_rule() for long description.
 * @since 2.1.0
 *
...
function add_rewrite_rule($regex, $redirect, $after = 'bottom') {
	global $nxt_rewrite;
	$nxt_rewrite->add_rule($regex, $redirect, $after);
}
...
function add_rewrite_tag($tagname, $regex) {
	...
	global $nxt_rewrite, $nxt;
	$nxt->add_query_var($qv);
	$nxt_rewrite->add_rewrite_tag($tagname, $regex, $qv . '=');
}

/**
 * Add permalink structure.
 *
 * @see nxt_Rewrite::add_permastruct()
 * @since 3.0.0
 *
...
function add_permastruct( $name, $struct, $with_front = true, $ep_mask = EP_NONE ) {
	global $nxt_rewrite;
	return $nxt_rewrite->add_permastruct( $name, $struct, $with_front, $ep_mask );
}
...
function add_feed($feedname, $function) {
	global $nxt_rewrite;
	if ( ! in_array($feedname, $nxt_rewrite->feeds) ) //override the file if it is
		$nxt_rewrite->feeds[] = $feedname;
 ...
 *
 * Be sure to flush the rewrite rules (nxt_rewrite->flush_rules()) when your plugin gets
 * activated (register_activation_hook()) and deactivated (register_deactivation_hook())
 *
 * @since 2.1.0
 * @see nxt_Rewrite::add_endpoint() Parameters and more description.
 * @uses $nxt_rewrite
 *
...
function add_rewrite_endpoint($name, $places) {
	global $nxt_rewrite;
	$nxt_rewrite->add_endpoint($name, $places);
}
 ...
 * @return string
 */
function _nxt_filter_taxonomy_base( $base ) {
	if ( !empty( $base ) ) {
...
/**
 * NXTClass Rewrite Component.
 *
 * The NXTClass Rewrite class writes the rewrite module rules to the .htaccess
 * file. It also handles parsing the request to get the correct setup for the
 * NXTClass Query class.
 *
 * The Rewrite along with nxt class function as a front controller for NXTClass.
...
class nxt_Rewrite {
	/**
	 * Default permalink structure for NXTClass.
	 *
	 * @since 1.5.0
	 * @access private
	 * @var string
	 ...
	 * Front URL path.
	 *
	 * The difference between the root property is that NXTClass might be
	 * located at example/NXTClass/index.php, if permalinks are turned off. The
	 * NXTClass/index.php will be the front portion. If permalinks are turned
	...
	/**
	 * Root URL path to NXTClass (without domain).
	 *
	 * The difference between front property is that NXTClass might be located
	 * at example.com/NXTClass/. The root is the 'NXTClass/' portion.
	 *
	 * @since 1.5.0
	 * @access private
	 * @var string
	...
	/**
	 * Rules that don't redirect to nxt's index.php.
	 *
	...
	/**
	 * Whether to write every mod_rewrite rule for NXTClass.
	 *
	...
	/**
	 * Whether to write every mod_rewrite rule for NXTClass pages.
	 *
	 * @since 2.5.0
	 * @access public
	 * @var bool
	...
	/**
	 * Preg_replace values for the search, see {@link nxt_Rewrite::$rewritecode}.
	 *
	 * @since 1.5.0
	 * @access private
	 * @var array
	...
	function page_uri_index() {
		...
		$posts = get_page_hierarchy( $nxtdb->get_results("SELECT ID, post_name, post_parent FROM $nxtdb->posts WHERE post_type = 'page' AND post_status != 'auto-draft'") );
		...
		foreach ( $posts as $id => $post ) {
			// URL => page name
			$uri = get_page_uri($id);
			$attachments = $nxtdb->get_results( $nxtdb->prepare( "SELECT ID, post_name, post_parent FROM $nxtdb->posts WHERE post_type = 'attachment' AND post_parent = %d", $id ));
	 ...
	 *
	 * The main nxt_Rewrite function for building the rewrite rule list. The
	 ...
	 *
	 * Shorten version of {@link nxt_Rewrite::generate_rewrite_rules()} that
	 * allows for shorter list of parameters. See the method for longer
	 * description of what generating rewrite rules does.
	 *
	 * @uses nxt_Rewrite::generate_rewrite_rules() See for long description and rest of parameters.
	 * @since 1.5.0
	 * @access public
	 *
	 ...
	 * reference to the current nxt_Rewrite instance to further manipulate the
	...
	function rewrite_rules() {
		...
		// Old feed files
		$old_feed_files = array( '.*nxt-(atom|rdf|rss|rss2|feed|commentsrss2)\.php$' => $this->index . '?feed=old' );

		// Registration rules
		$registration_pages = array();
		if ( is_multisite() && is_main_site() ) {
			$registration_pages['.*nxt-signup.php$'] = $this->index . '?signup=true';
			$registration_pages['.*nxt-activate.php$'] = $this->index . '?activate=true';
		}
	 ...
	 * nxt_Rewrite::rewrite_rules()} is that this method stores the rewrite rules
	 ...
	 *
	 * Will add  the non_nxt_rules property rules to the .htaccess file before
	 * the NXTClass rewrite rules one.
	 *
	 * @since 1.5.0
	 * @access public
	 *
	...
	function mod_rewrite_rules() {
		...
		//add in the rules that don't redirect to nxt's index.php (and thus shouldn't be handled by nxt at all)
		foreach ( (array) $this->non_nxt_rules as $match => $query) {
	...
	<system.webServer>
		...
		if ( !is_multisite() ) {
			$rules .= '
				<rule name="nxtclass" patternSyntax="Wildcard">
					<match url="*" />
						<conditions>
	...
	function add_rule($regex, $redirect, $after = 'bottom') {
		...
		if ( $front != $this->index ) { //it doesn't redirect to nxt's index.php
	...
	function add_external_rule($regex, $redirect) {
		$this->non_nxt_rules[$regex] = $redirect;
	}
	 ...
	 * Remove rewrite rules and then recreate rewrite rules.
	 *
	 * Calls {@link nxt_Rewrite::nxt_rewrite_rules()} after removing the
	 ...
	 */
	function init() {
		$this->extra_rules = $this->non_nxt_rules = $this->endpoints = array();
	 ...
	 * {@link nxt_Rewrite::init()} after the option is updated.
	 *
