...
function get_bookmarks($args = '') {
	...
	$cache = array();
	$key = md5( serialize( $r ) );
	if ( $cache = nxt_cache_get( 'get_bookmarks', 'bookmark' ) ) {
	...
	if ( !empty($category_name) ) {
		if ( $category = get_term_by('name', $category_name, 'link_category') ) {
			...
			$cache[ $key ] = array();
			nxt_cache_set( 'get_bookmarks', $cache, 'bookmark' );
	...
	if (!empty($category_query)) {
		...
		$join = " INNER JOIN $nxtdb->term_relationships AS tr ON ($nxtdb->links.link_id = tr.object_id) INNER JOIN $nxtdb->term_taxonomy as tt ON tt.term_taxonomy_id = tr.term_taxonomy_id";
	}
	...
	$query = "SELECT * $length $recently_updated_test $get_updated FROM $nxtdb->links $join WHERE 1=1 $visible $category_query";
 ...
 *
 * @since 2.7.0
 * @uses nxt_cache_delete() Deletes the contents of 'get_bookmarks'
 */
function clean_bookmark_cache($bookmark_id) {
	nxt_cache_delete( $bookmark_id, 'bookmark' );
	nxt_cache_delete( 'get_bookmarks', 'bookmark' );
}
