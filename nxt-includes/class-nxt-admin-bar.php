...
class nxt_Admin_Bar {
	...
	public function __get( $name ) {
		switch ( $name ) {
			case 'proto' :
				...
				break;
			case 'menu' :
				_deprecated_argument( 'nxt_Admin_Bar', '3.3', 'Modify admin bar nodes with nxt_Admin_Bar::get_node(), nxt_Admin_Bar::add_node(), and nxt_Admin_Bar::remove_node(), not the <code>menu</code> property.' );
	...
	public function add_node( $args ) {
		...
		if ( ! empty( $defaults['meta'] ) && empty( $args['meta'] ) )
			$args['meta'] = nxt_parse_args( $args['meta'], $defaults['meta'] );
	...
	final protected function _render( $root ) {
		...
		?>
		<div id="nxtadminbar" class="<?php echo $class; ?>" role="navigation">
	...
	final protected function _render_container( $node ) {
		...
		?><div id="<?php echo esc_attr( 'nxt-admin-bar-' . $node->id ); ?>" class="ab-group-container"><?php
	...
	final protected function _render_group( $node ) {
		...
		?><ul id="<?php echo esc_attr( 'nxt-admin-bar-' . $node->id ); ?>" class="<?php echo esc_attr( $class ); ?>"><?php
	...
	final protected function _render_item( $node ) {
		...
		<li id="<?php echo esc_attr( 'nxt-admin-bar-' . $node->id ); ?>" class="<?php echo esc_attr( $menuclass ); ?>"><?php
			if ( $has_link ):
	...
	public function recursive_render( $id, $node ) {
		_deprecated_function( __METHOD__, '3.3', 'nxt_Admin_bar::render(), nxt_Admin_Bar::_render_item()' );
		$this->_render_item( $node );
	}

	public function add_menus() {
		// User related, aligned right.
		add_action( 'admin_bar_menu', 'nxt_admin_bar_my_account_menu', 0 );
		add_action( 'admin_bar_menu', 'nxt_admin_bar_search_menu', 4 );
		add_action( 'admin_bar_menu', 'nxt_admin_bar_my_account_item', 7 );

		// Site related.
		add_action( 'admin_bar_menu', 'nxt_admin_bar_nxt_menu', 10 );
		add_action( 'admin_bar_menu', 'nxt_admin_bar_my_sites_menu', 20 );
		add_action( 'admin_bar_menu', 'nxt_admin_bar_site_menu', 30 );
		add_action( 'admin_bar_menu', 'nxt_admin_bar_updates_menu', 40 );

		// Content related.
		if ( ! is_network_admin() && ! is_user_admin() ) {
			add_action( 'admin_bar_menu', 'nxt_admin_bar_comments_menu', 60 );
			add_action( 'admin_bar_menu', 'nxt_admin_bar_new_content_menu', 70 );
		}
		add_action( 'admin_bar_menu', 'nxt_admin_bar_edit_menu', 80 );

		add_action( 'admin_bar_menu', 'nxt_admin_bar_add_secondary_groups', 200 );
