 ...
 * @since MU
 */
function nxtmu_update_blogs_date() {
	global $nxtdb;

	update_blog_details( $nxtdb->blogid, array('last_updated' => current_time('mysql', true)) );

	do_action( 'nxtmu_blog_updated', $nxtdb->blogid );
}
...
function get_id_from_blogname( $name ) {
	...
	$blog_id = nxt_cache_get( 'get_id_from_blogname_' . $name, 'blog-details' );
	if ( $blog_id )
		return $blog_id;
	...
	$blog_id = $nxtdb->get_var( $nxtdb->prepare("SELECT blog_id FROM {$nxtdb->blogs} WHERE domain = %s AND path = %s", $domain, $path) );
	nxt_cache_set( 'get_id_from_blogname_' . $name, $blog_id, 'blog-details' );
	return $blog_id;
}
...
function get_blog_details( $fields, $get_all = true ) {
	...
	if ( is_array($fields ) ) {
		if ( isset($fields['blog_id']) ) {
			...
			$blog = nxt_cache_get($key, 'blog-lookup');
			if ( false !== $blog )
				return $blog;
			if ( substr( $fields['domain'], 0, 4 ) == 'www.' ) {
				$nowww = substr( $fields['domain'], 4 );
				$blog = $nxtdb->get_row( $nxtdb->prepare( "SELECT * FROM $nxtdb->blogs WHERE domain IN (%s,%s) AND path = %s ORDER BY CHAR_LENGTH(domain) DESC", $nowww, $fields['domain'], $fields['path'] ) );
			} else {
				$blog = $nxtdb->get_row( $nxtdb->prepare( "SELECT * FROM $nxtdb->blogs WHERE domain = %s AND path = %s", $fields['domain'], $fields['path'] ) );
			}
			if ( $blog ) {
				nxt_cache_set($blog->blog_id . 'short', $blog, 'blog-details');
			...
			if ( substr( $fields['domain'], 0, 4 ) == 'www.' ) {
				$nowww = substr( $fields['domain'], 4 );
				$blog = $nxtdb->get_row( $nxtdb->prepare( "SELECT * FROM $nxtdb->blogs WHERE domain IN (%s,%s) ORDER BY CHAR_LENGTH(domain) DESC", $nowww, $fields['domain'] ) );
			} else {
				$blog = $nxtdb->get_row( $nxtdb->prepare( "SELECT * FROM $nxtdb->blogs WHERE domain = %s", $fields['domain'] ) );
			}
			if ( $blog ) {
				nxt_cache_set($blog->blog_id . 'short', $blog, 'blog-details');
				$blog_id = $blog->blog_id;
	...
	$details = nxt_cache_get( $blog_id . $all, 'blog-details' );
	...
	if ( $details ) {
		if ( ! is_object( $details ) ) {
			if ( $details == -1 ) {
				...
				nxt_cache_delete( $blog_id . $all, 'blog-details' );
				unset($details);
			}
		} else {
			return $details;
	...
	// Try the other cache.
	if ( $get_all ) {
		$details = nxt_cache_get( $blog_id . 'short', 'blog-details' );
	} else {
		$details = nxt_cache_get( $blog_id, 'blog-details' );
	...
	if ( empty($details) ) {
		$details = $nxtdb->get_row( $nxtdb->prepare( "SELECT * FROM $nxtdb->blogs WHERE blog_id = %d /* get_blog_details */", $blog_id ) );
		if ( ! $details ) {
			// Set the full cache.
			nxt_cache_set( $blog_id, -1, 'blog-details' );
			return false;
		}
	}

	if ( ! $get_all ) {
		nxt_cache_set( $blog_id . $all, $details, 'blog-details' );
		return $details;
	}
	...
	nxt_cache_set( $key, $details, 'blog-lookup' );
...
function update_blog_details( $blog_id, $details = array() ) {
	...
	$nxtdb->update( $nxtdb->blogs, $update_details, array('blog_id' => $blog_id) );
...
function delete_blog_option( $id, $key ) {
	...
	restore_current_blog();
	if ( $return )
		nxt_cache_set( $id . '-' . $key . '-blog_option', '', 'site-options' );
	return $return;
}
...
function switch_to_blog( $new_blog, $validate = false ) {
	global $nxtdb, $table_prefix, $blog_id, $switched, $switched_stack, $nxt_roles, $nxt_object_cache;
...
function update_blog_status( $blog_id, $pref, $value, $deprecated = null ) {
	...
	$nxtdb->update( $nxtdb->blogs, array($pref => $value, 'last_updated' => current_time('mysql', true)), array('blog_id' => $blog_id) );
...
function get_blog_status( $id, $pref ) {
	...
	return $nxtdb->get_var( $nxtdb->prepare("SELECT %s FROM {$nxtdb->blogs} WHERE blog_id = %d", $pref, $id) );
}
...
function get_last_updated( $deprecated = '', $start = 0, $quantity = 40 ) {
	...
	return $nxtdb->get_results( $nxtdb->prepare("SELECT blog_id, domain, path FROM $nxtdb->blogs WHERE site_id = %d AND public = '1' AND archived = '0' AND mature = '0' AND spam = '0' AND deleted = '0' AND last_updated != '0000-00-00 00:00:00' ORDER BY last_updated DESC limit %d, %d", $nxtdb->siteid, $start, $quantity ) , ARRAY_A );
}
