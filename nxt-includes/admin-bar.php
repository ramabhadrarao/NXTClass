...
function _nxt_admin_bar_init() {
	...
	require( ABSPATH . nxtINC . '/class-nxt-admin-bar.php' );

	/* Instantiate the admin bar */
	$admin_bar_class = apply_filters( 'nxt_admin_bar_class', 'nxt_Admin_Bar' );
	if ( class_exists( $admin_bar_class ) )
		$nxt_admin_bar = new $admin_bar_class;
	else
		return false;

	$nxt_admin_bar->initialize();
	$nxt_admin_bar->add_menus();

	return true;
}
add_action( 'init', '_nxt_admin_bar_init' ); // Don't remove. Wrong way to disable.

/**
 * Render the admin bar to the page based on the $nxt_admin_bar->menu member var.
 ...
 * @since 3.1.0
 */
function nxt_admin_bar_render() {
	global $nxt_admin_bar;

	if ( ! is_admin_bar_showing() || ! is_object( $nxt_admin_bar ) )
		return false;

	do_action_ref_array( 'admin_bar_menu', array( &$nxt_admin_bar ) );

	do_action( 'nxt_before_admin_bar_render' );

	$nxt_admin_bar->render();

	do_action( 'nxt_after_admin_bar_render' );
}
add_action( 'nxt_footer', 'nxt_admin_bar_render', 1000 );
add_action( 'admin_footer', 'nxt_admin_bar_render', 1000 );

/**
 * Add the NXTClass logo menu.
 *
 * @since 3.3.0
 */
function nxt_admin_bar_nxt_menu( $nxt_admin_bar ) {
	$nxt_admin_bar->add_menu( array(
		'id'    => 'nxt-logo',
		'title' => '<span class="ab-icon"></span>',
		'href'  => admin_url( 'about.php' ),
		'meta'  => array(
			'title' => __('About NXTClass'),
		),
	) );

	if ( is_user_logged_in() ) {
		// Add "About NXTClass" link
		$nxt_admin_bar->add_menu( array(
			'parent' => 'nxt-logo',
			'id'     => 'about',
			'title'  => __('About NXTClass'),
 ...
 * @since 3.3.0
 */
function nxt_admin_bar_my_account_item( $nxt_admin_bar ) {
 ...
 * @since 3.1.0
 */
function nxt_admin_bar_edit_menu( $nxt_admin_bar ) {
	global $post, $tag, $nxt_the_query;
	...
	if ( is_admin() ) {
		...
		$current_object = $nxt_the_query->get_queried_object();
 ...
 * @since 3.1.0
 */
function nxt_admin_bar_new_content_menu( $nxt_admin_bar ) {
	$actions = array();
 ...
 * @since 3.1.0
 */
function nxt_admin_bar_appearance_menu( $nxt_admin_bar ) {
	$nxt_admin_bar->add_group( array( 'parent' => 'site-name', 'id' => 'appearance' ) );
 ...
 * @since 3.3.0
 */
function nxt_admin_bar_search_menu( $nxt_admin_bar ) {
	if ( is_admin() )
		return;
 ...
 * @since 3.3.0
 */
function nxt_admin_bar_add_secondary_groups( $nxt_admin_bar ) {
	$nxt_admin_bar->add_group( array(
		'id'     => 'top-secondary',
		'meta'   => array(
 ...
 *
 */
function nxt_admin_bar_header() { ?>
<style type="text/css" media="print">#nxtadminbar { display:none; }</style>
<?php
}
...
function is_admin_bar_showing() {
	...
	if ( ! isset( $show_admin_bar ) ) {
		if ( ! is_user_logged_in() || 'nxt-login.php' == $pagenow ) {
