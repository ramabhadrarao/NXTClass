<?php
/**
 * NXTClass Link Template Functions
 *
 * @package NXTClass
 * @subpackage Template
 */
...
function user_trailingslashit($string, $type_of_url = '') {
	global $nxt_rewrite;
	if ( $nxt_rewrite->use_trailing_slashes )
	...
	if ( '' != $permalink && !in_array($post->post_status, array('draft', 'pending', 'auto-draft')) ) {
		...
		if ( strpos($permalink, '%category%') !== false ) {
			...
			if ( empty($category) ) {
				...
				$category = is_nxt_error( $default_category ) ? '' : $default_category->slug;
			}
		}
...
function get_post_permalink( $id = 0, $leavename = false, $sample = false ) {
	...
	$post_link = $nxt_rewrite->get_extra_permastruct($post->post_type);
...
function _get_page_link( $id = false, $leavename = false, $sample = false ) {
	...
	$link = $nxt_rewrite->get_page_permastruct();
 ...
 *
 * This can be used in the NXTClass Loop or outside of it.
 *
 * @since 2.0.0
 *
 * @param int $id Optional. Post ID.
...
function get_attachment_link($id = false) {
	...
	$object = get_post($id);
	if ( $nxt_rewrite->using_permalinks() && ($object->post_parent > 0) && ($object->post_parent != $id) ) {
...
function get_year_link($year) {
	...
	$yearlink = $nxt_rewrite->get_year_permastruct();
	if ( !empty($yearlink) ) {
...
function get_month_link($year, $month) {
	...
	$monthlink = $nxt_rewrite->get_month_permastruct();
	if ( !empty($monthlink) ) {
...
function get_day_link($year, $month, $day) {
	...
	$daylink = $nxt_rewrite->get_day_permastruct();
	if ( !empty($daylink) ) {
...
function get_feed_link($feed = '') {
	...
	$permalink = $nxt_rewrite->get_feed_permastruct();
	if ( '' != $permalink ) {
		if ( false !== strpos($feed, 'comments_') ) {
			$feed = str_replace('comments_', '', $feed);
			$permalink = $nxt_rewrite->get_comment_feed_permastruct();
		}
...
function get_term_feed_link( $term_id, $taxonomy = 'category', $feed = '' ) {
	...
	if ( empty( $term ) || is_nxt_error( $term ) )
		return false;
...
function get_search_link( $query = '' ) {
	...
	$permastruct = $nxt_rewrite->get_search_permastruct();
...
function get_post_type_archive_link( $post_type ) {
	...
	if ( get_option( 'permalink_structure' ) && is_array( $post_type_obj->rewrite ) ) {
		...
		if ( $post_type_obj->rewrite['with_front'] )
			$struct = $nxt_rewrite->front . $struct;
		else
			$struct = $nxt_rewrite->root . $struct;
 ...
 *
 * Can be used within the NXTClass loop or outside of it. Can be used with
 ...
 *
 * Can be used within the NXTClass loop or outside of it, with any post type.
 *
 * @since 2.9.0
 *
 * @param int $id Optional. Post ID.
...
function get_delete_post_link( $id = 0, $deprecated = '', $force_delete = false ) {
	...
	return apply_filters( 'get_delete_post_link', nxt_nonce_url( $delete_link, "$action-{$post->post_type}_{$post->ID}" ), $post->ID, $force_delete );
}
 ...
 *
 * This is meant to be attached to actions like 'nxt_head'. Do not call this directly in plugins or theme templates.
 * @since 3.0.0
 *
 */
function adjacent_posts_rel_link_nxt_head() {
...
function get_pagenum_link($pagenum = 1) {
	...
	if ( !$nxt_rewrite->using_permalinks() || is_admin() ) {
		...
		$request = preg_replace( "|$nxt_rewrite->pagination_base/\d+/?$|", '', $request);
		$request = preg_replace( '|^index\.php|', '', $request);
		$request = ltrim($request, '/');

		$base = trailingslashit( get_bloginfo( 'url' ) );

		if ( $nxt_rewrite->using_index_permalinks() && ( $pagenum > 1 || '' != $request ) )
			$base .= 'index.php/';

		if ( $pagenum > 1 ) {
			$request = ( ( !empty( $request ) ) ? trailingslashit( $request ) : $request ) . user_trailingslashit( $nxt_rewrite->pagination_base . "/" . $pagenum, 'paged' );
		}
...
function get_next_posts_link( $label = null, $max_page = 0 ) {
	global $paged, $nxt_query;

	if ( !$max_page )
		$max_page = $nxt_query->max_num_pages;
...
function get_posts_nav_link( $args = array() ) {
	...
	if ( !is_singular() ) {
		...
		$max_num_pages = $nxt_query->max_num_pages;
...
function get_next_comments_link( $label = '', $max_page = 0 ) {
	...
	if ( empty($max_page) )
		$max_page = $nxt_query->max_num_comment_pages;
...
function get_admin_url( $blog_id = null, $path = '', $scheme = 'admin' ) {
	$url = get_site_url($blog_id, 'nxt-admin/', $scheme);
...
function plugins_url($path = '', $plugin = '') {
	...
	$mu_plugin_dir = nxtMU_PLUGIN_DIR;
...
function network_admin_url( $path = '', $scheme = 'admin' ) {
	...
	$url = network_site_url('nxt-admin/network/', $scheme);
...
function user_admin_url( $path = '', $scheme = 'admin' ) {
	$url = network_site_url('nxt-admin/user/', $scheme);
...
function rel_canonical() {
	...
	if ( !$id = $nxt_the_query->get_queried_object_id() )
		return;
 ...
 */
function nxt_get_shortlink($id = 0, $context = 'post', $allow_slugs = true) {
	...
	if ( 'query' == $context && is_single() ) {
		$post_id = $nxt_query->get_queried_object_id();
	} elseif ( 'post' == $context ) {
		$post = get_post($id);
		$post_id = $post->ID;
	}
 ...
 *
 *  Attached to the nxt_head action.
 *
 * @since 3.0.0
 *
 * @uses nxt_get_shortlink()
 */
function nxt_shortlink_nxt_head() {
	$shortlink = nxt_get_shortlink( 0, 'query' );
 ...
 *
 * Attached to the nxt action.
 *
 * @since 3.0.0
 *
 * @uses nxt_get_shortlink()
 */
function nxt_shortlink_header() {
	if ( headers_sent() )
		return;
