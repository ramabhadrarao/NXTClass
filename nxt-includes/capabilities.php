<?php
/**
 * NXTClass Roles and Capabilities.
 *
 * @package NXTClass
 * @subpackage User
 */
...
class nxt_Roles {
	 ...
	 *
	 * The role key is set to the current prefix for the $nxtdb object with
	 * 'user_roles' appended. If the $nxt_user_roles global is set, then it will
	 * be used and the role option will not be updated or used.
	 *
	 * @since 2.1.0
	 * @access protected
	 * @uses $nxtdb Used to get the database prefix.
	 * @global array $nxt_user_roles Used to set the 'roles' property value.
	 */
	function _init () {
		global $nxtdb, $nxt_user_roles;
		$this->role_key = $nxtdb->prefix . 'user_roles';
		if ( ! empty( $nxt_user_roles ) ) {
			$this->roles = $nxt_user_roles;
			$this->use_db = false;
		} else {
		...
		foreach ( (array) $this->roles as $role => $data ) {
			$this->role_objects[$role] = new nxt_Role( $role, $this->roles[$role]['capabilities'] );
	 ...
	 * @return null|nxt_Role nxt_Role object if role is added, null if already exists.
	 */
	function add_role( $role, $display_name, $capabilities = array() ) {
		...
		$this->role_objects[$role] = new nxt_Role( $role, $capabilities );
	 ...
	 *
	 * @param string $role Role name.
	 * @return object|null Null, if role does not exist. nxt_Role object, if found.
	 */
...
class nxt_Role {
	 ...
	 *
	 * @see nxt_Roles::add_cap() Method uses implementation for role.
	 * @since 2.0.0
	 * @access public
	 *
	 * @param string $cap Capability name.
	...
	function add_cap( $cap, $grant = true ) {
		...
		$nxt_roles->add_cap( $this->name, $cap, $grant );
	}

	/**
	 * Remove capability from role.
	 *
	 * This is a container for {@link nxt_Roles::remove_cap()} to remove the
	 * capability from the role. That is to say, that {@link
	 * nxt_Roles::remove_cap()} implements the functionality, but it also makes
	...
	function remove_cap( $cap ) {
		...
		$nxt_roles->remove_cap( $this->name, $cap );
	}
...
class nxt_User {
	...
	static function get_data_by( $field, $value ) {
		...
		switch ( $field ) {
			case 'id':
				...
				break;
			case 'slug':
				$user_id = nxt_cache_get($value, 'userslugs');
				$db_field = 'user_nicename';
				break;
			case 'email':
				$user_id = nxt_cache_get($value, 'useremail');
				$db_field = 'user_email';
				break;
			case 'login':
				$value = sanitize_user( $value );
				$user_id = nxt_cache_get($value, 'userlogins');
				$db_field = 'user_login';
				break;
			default:
				return false;
		...
		if ( false !== $user_id ) {
			if ( $user = nxt_cache_get( $user_id, 'users' ) )
				return $user;
		}

		if ( !$user = $nxtdb->get_row( $nxtdb->prepare(
			"SELECT * FROM $nxtdb->users WHERE $db_field = %s", $value
		) ) )
			return false;
	...
	function __isset( $key ) {
		if ( 'id' == $key ) {
			_deprecated_argument( 'nxt_User->id', '2.1', __( 'Use <code>nxt_User->ID</code> instead.' ) );
			$key = 'ID';
		}
	...
	function _init_caps( $cap_key = '' ) {
		...
		if ( empty($cap_key) )
			$this->cap_key = $nxtdb->prefix . 'capabilities';
		else
	...
	function update_user_level_from_caps() {
		...
		update_user_meta( $this->ID, $nxtdb->prefix . 'user_level', $this->user_level );
	}
	...
	function remove_all_caps() {
		...
		delete_user_meta( $this->ID, $nxtdb->prefix . 'user_level' );
		$this->get_role_caps();
	}
	...
	function for_blog( $blog_id = '' ) {
		global $nxtdb;
		if ( ! empty( $blog_id ) )
			$cap_key = $nxtdb->get_blog_prefix( $blog_id ) . 'capabilities';
		else
			$cap_key = '';
	...
	case 'delete_page':
		...
		if ( is_object( $post_author_data ) && $user_id == $post_author_data->ID ) {
			...
			if ( 'publish' == $post->post_status ) {
				...
				if ('publish' == get_post_meta($post->ID, '_nxt_trash_meta_status', true) )
					$caps[] = $post_type->cap->delete_published_posts;
			} else {
...
function author_can( $post, $capability ) {
	...
	$author = new nxt_User( $post->post_author );
 ...
 *
 * @see nxt_Roles::get_role() Uses method to retrieve role object.
 * @since 2.0.0
 *
 * @param string $role Role name.
 * @return object
...
function get_role( $role ) {
	...
	return $nxt_roles->get_role( $role );
}

/**
 * Add role, if it does not exist.
 *
 * @see nxt_Roles::add_role() Uses method to add role.
 * @since 2.0.0
 *
 * @param string $role Role name.
...
function add_role( $role, $display_name, $capabilities = array() ) {
	...
	return $nxt_roles->add_role( $role, $display_name, $capabilities );
}

/**
 * Remove role, if it exists.
 *
 * @see nxt_Roles::remove_role() Uses method to remove role.
 * @since 2.0.0
 *
 * @param string $role Role name.
 * @return null
...
function remove_role( $role ) {
	...
	return $nxt_roles->remove_role( $role );
}
