...
function get_the_category_list( $separator = '', $parents='', $post_id = false ) {
	...
	$rel = ( is_object( $nxt_rewrite ) && $nxt_rewrite->using_permalinks() ) ? 'rel="category tag"' : 'rel="category"';
 ...
 *
 * Prior to v2.5 of NXTClass, category names were not supported.
 * Prior to v2.7, category slugs were not supported.
 * Prior to v2.7, only one category could be compared: in_category( $single_category ).
 * Prior to v2.7, this function could only be used in the NXTClass Loop.
 ...
 */
function nxt_dropdown_categories( $args = '' ) {
	$defaults = array(
	...
	$output = apply_filters( 'nxt_dropdown_cats', $output );
 ...
 * @return string HTML content only if 'echo' argument is 0.
 */
function nxt_list_categories( $args = '' ) {
	$defaults = array(
	...
	$output = apply_filters( 'nxt_list_categories', $output, $args );
...
function nxt_tag_cloud( $args = '' ) {
	...
	$return = nxt_generate_tag_cloud( $tags, $args ); // Here's where those top tags get sorted according to $args
 ...
 * @return string
 */
function nxt_generate_tag_cloud( $tags, $args = '' ) {
	global $nxt_rewrite;
	$defaults = array(
	...
	if ( $tags_sorted != $tags  ) { // the tags have been sorted by a plugin
		...
		if ( 'RAND' == $order ) {
			...
			if ( 'name' == $orderby )
				uasort( $tags, '_nxt_object_name_sort_cb' );
			else
				uasort( $tags, '_nxt_object_count_sort_cb' );
	...
	if ( $filter )
		return apply_filters( 'nxt_generate_tag_cloud', $return, $tags, $args );
	else
		return $return;
}
 ...
 * @access private
 */
function _nxt_object_name_sort_cb( $a, $b ) {
 ...
 * @access private
 */
function _nxt_object_count_sort_cb( $a, $b ) {
...
function term_description( $term = 0, $taxonomy = 'post_tag' ) {
	...
	return is_nxt_error( $description ) ? '' : $description;
}
 ...
 * @param string $sep Optional. Separate items using this.
 * @param string $after Optional. After list.
 * @return null|bool False on NXTClass error. Returns null when displaying.
 */
function the_terms( $id = 0, $taxonomy, $before = '', $sep = ', ', $after = '' ) {
	$term_list = get_the_term_list( $id, $taxonomy, $before, $sep, $after );

	if ( is_nxt_error( $term_list ) )
		return false;
 ...
 *
 * Prior to v2.7 of NXTClass, tags given as integers would also be checked against the post's tags' names and slugs (in addition to term_ids)
 * Prior to v2.7, this function could only be used in the NXTClass Loop.
 * As of 2.7, the function can be used anywhere if it is provided a post ID or post object.
 *
 * @since 2.6.0
