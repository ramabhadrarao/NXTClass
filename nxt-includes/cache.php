 ...
 * @return unknown
 */
function nxt_cache_add($key, $data, $group = '', $expire = 0) {
	global $nxt_object_cache;

	return $nxt_object_cache->add($key, $data, $group, $expire);
}
 ...
 */
function nxt_cache_decr( $key, $offset = 1, $group = '' ) {
	global $nxt_object_cache;

	return $nxt_object_cache->decr( $key, $offset, $group );
}
 ...
 */
function nxt_cache_delete($key, $group = '') {
	global $nxt_object_cache;

	return $nxt_object_cache->delete($key, $group);
}
 ...
 * @return bool Always returns true
 */
function nxt_cache_flush() {
	global $nxt_object_cache;

	return $nxt_object_cache->flush();
}
 ...
 *		contents on success
 */
function nxt_cache_get( $key, $group = '', $force = false ) {
	global $nxt_object_cache;

	return $nxt_object_cache->get( $key, $group, $force );
}
 ...
 * @return false|int False on failure, the item's new value on success.
 */
function nxt_cache_incr( $key, $offset = 1, $group = '' ) {
	global $nxt_object_cache;

	return $nxt_object_cache->incr( $key, $offset, $group );
}
 ...
 */
function nxt_cache_replace($key, $data, $group = '', $expire = 0) {
	global $nxt_object_cache;

	return $nxt_object_cache->replace($key, $data, $group, $expire);
}
 ...
 * @return bool False if cache key and group already exist, true on success
 */
function nxt_cache_set($key, $data, $group = '', $expire = 0) {
	global $nxt_object_cache;

	return $nxt_object_cache->set($key, $data, $group, $expire);
}
 ...
 * @since 2.6.0
 */
function nxt_cache_reset() {
	global $nxt_object_cache;

	return $nxt_object_cache->reset();
}

/**
 * NXTClass Object Cache
 *
 * The NXTClass Object Cache is used to save on trips to the database. The
 * Object Cache stores all of the cache data to memory and makes the cache
 * contents available by using a key, which is used to name and later retrieve
 * the cache contents.
 *
 * The Object Cache can be replaced by other caching mechanisms by placing files
 * in the nxt-content folder which is looked at in nxt-settings. If that file
...
class nxt_Object_Cache {
	 ...
	 *
	 * @uses nxt_Object_Cache::get Checks to see if the cache already has data.
	 * @uses nxt_Object_Cache::set Sets the data after the checking the cache
	 *		contents existence.
	 *
	 * @since 2.0.0
	 *
	 ...
	 *
	 * @since 2.0.8
	 * @return null|nxt_Object_Cache If cache is disabled, returns null.
	 */
