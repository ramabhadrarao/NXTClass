<?php
/**
 * NXTClass Cron Implementation for hosts, which do not offer CRON or for which
...
/**
 * Tell NXTClass we are doing the CRON task.
 *
 * @var bool
 */
...
// Use global $doing_nxt_cron lock otherwise use the GET lock. If no lock, trying grabbing a new lock.
if ( empty( $doing_nxt_cron ) ) {
	if ( empty( $_GET[ 'doing_nxt_cron' ] ) ) {
		// Called from external script/job. Try setting a lock.
		if ( $doing_cron_transient && ( $doing_cron_transient + nxt_CRON_LOCK_TIMEOUT > $local_time ) )
			return;
		$doing_cron_transient = $doing_nxt_cron = time();
