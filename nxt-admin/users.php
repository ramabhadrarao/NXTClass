...
get_current_screen()->set_help_sidebar(
    ...
    '<p>' . __('<a href="http://codex.opensource.nxtclass.tk/Users_Screen" target="_blank">Documentation on Managing Users</a>') . '</p>' .
    '<p>' . __('<a href="http://codex.opensource.nxtclass.tk/Roles_and_Capabilities" target="_blank">Descriptions of Roles and Capabilities</a>') . '</p>' .
    '<p>' . __('<a href="http://opensource.nxtclass.tk/support/" target="_blank">Support Forums</a>') . '</p>'
);

if ( empty($_REQUEST) ) {
	$referer = '<input type="hidden" name="nxt_http_referer" value="'. esc_attr(stripslashes($_SERVER['REQUEST_URI'])) . '" />';
} elseif ( isset($_REQUEST['nxt_http_referer']) ) {
	$redirect = remove_query_arg(array('nxt_http_referer', 'updated', 'delete_count'), stripslashes($_REQUEST['nxt_http_referer']));
	$referer = '<input type="hidden" name="nxt_http_referer" value="' . esc_attr($redirect) . '" />';
} else {
	$redirect = 'users.php';
	$referer = '';
}
...
case 'promote':
	...
	if ( ! current_user_can( 'promote_users' ) )
		nxt_die( __( 'You can&#8217;t edit that user.' ) );
	...
	foreach ( $userids as $id ) {
		...
		if ( $id == $current_user->ID && ! $nxt_roles->role_objects[ $_REQUEST['new_role'] ]->has_cap('promote_users')
	...
	nxt_redirect(add_query_arg('update', $update, $redirect));
	exit();
...
case 'dodelete':
	...
	if ( ! current_user_can( 'delete_users' ) )
		nxt_die(__('You can&#8217;t delete users.'));
...
case 'delete':
	...
	if ( ! current_user_can( 'delete_users' ) )
		$errors = new nxt_Error( 'edit_users', __( 'You can&#8217;t delete users.' ) );
...
<?php nxt_nonce_field('delete-users') ?>
<?php echo $referer; ?>
...
<?php if ( $go_delete ) : ?>
	...
	<ul style="list-style:none;">
		...
		nxt_dropdown_users( array( 'name' => 'reassign_user', 'exclude' => array_diff( $userids, array($current_user->ID) ) ) ); ?></li>
	</ul></fieldset>
...
case 'doremove':
	...
	if ( ! is_multisite() )
		nxt_die( __( 'You can&#8217;t remove users.' ) );
...
case 'remove':
	...
	if ( !current_user_can('remove_users') )
		$error = new nxt_Error('edit_users', __('You can&#8217;t remove users.'));
...
?>
<form action="" method="post" name="updateusers" id="updateusers">
<?php nxt_nonce_field('remove-users') ?>
<?php echo $referer; ?>
...
default:
	...
	if ( !empty($_GET['_nxt_http_referer']) ) {
...
<?php $nxt_list_table->search_box( __( 'Search Users' ), 'user' ); ?>
