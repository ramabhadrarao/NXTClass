...
nxt_enqueue_script('list-revisions');

nxt_reset_vars(array('revision', 'left', 'right', 'action'));
...
case 'restore' :
	...
	nxt_restore_post_revision( $revision->ID );
...
case 'diff' :
	...
	if ( ! nxt_POST_REVISIONS || !post_type_supports($post->post_type, 'revisions') ) { // Revisions disabled
		if (
			// we're not looking at an autosave
			( !nxt_is_post_autosave( $left_revision ) && !nxt_is_post_autosave( $right_revision ) )
		||
	...
	||
		...
		( !nxt_get_post_revision( $left_revision->ID ) && !nxt_get_post_revision( $right_revision->ID ) )
	)
		break;
...
<tr id="revision">
	...
	<th scope="col" class="th-full">
		<span class="alignleft"><?php printf( __('Older: %s'), nxt_post_revision_title( $left_revision ) ); ?></span>
		<span class="alignright"><?php printf( __('Newer: %s'), nxt_post_revision_title( $right_revision ) ); ?></span>
	</th>
</tr>
<?php endif;
