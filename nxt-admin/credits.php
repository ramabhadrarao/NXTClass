...
function nxt_credits() {
	...
	if ( ! is_array( $results ) ) {
		...
		set_site_transient( 'nxtclass_credits_' . $locale, $results, 86400 ); // One day
	}

	return $results;
}

function _nxt_credits_add_profile_link( &$display_name, $username, $profiles ) {
	$display_name = '<a href="' . esc_url( sprintf( $profiles, $username ) ) . '">' . esc_html( $display_name ) . '</a>';
}

function _nxt_credits_build_object_link( &$data ) {
...
echo '<p class="about-description">' . __( 'NXTClass is created by a worldwide team of passionate individuals.' ) . "</p>\n";
...
__( 'Core Developers' );
__( 'Recent Rockstars' );
__( 'Core Contributors to NXTClass %s' );
