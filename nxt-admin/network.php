...
function network_domain_check() {
	global $nxtdb;
	if ( $nxtdb->get_var( "SHOW TABLES LIKE '$nxtdb->site'" ) )
		return $nxtdb->get_var( "SELECT domain FROM $nxtdb->site ORDER BY id ASC LIMIT 1" );
	return false;
}
...
function allow_subdirectory_install() {
	...
	$post = $nxtdb->get_row( "SELECT ID FROM $nxtdb->posts WHERE post_date < DATE_SUB(NOW(), INTERVAL 1 MONTH) AND post_status = 'publish'" );
	if ( empty( $post ) )
		return true;
...
$network_help = '<p>' . __('This screen allows you to configure a network as having subdomains (<code>site1.example.com</code>) or subdirectories (<code>example.com/site1</code>). Subdomains require wildcard subdomains to be enabled in Apache and DNS records, if your host allows it.') . '</p>' .
	...
	'<p>' . __('The next screen for Network Setup will give you individually-generated lines of code to add to your nxt-config.php and .htaccess files. Make sure the settings of your FTP client make files starting with a dot visible, so that you can find .htaccess; you may have to create this file if it really is not there. Make backup copies of those two files.') . '</p>' .
	'<p>' . __('Add a <code>blogs.dir</code> directory under <code>/nxt-content</code> and add the designated lines of code to nxt-config.php (just before <code>/*...stop editing...*/</code>) and <code>.htaccess</code> (replacing the existing NXTClass rules).') . '</p>' .
...
function network_step1( $errors = false ) {
	...
	<p><?php _e( 'Fill in the information below and you&#8217;ll be on your way to creating a network of NXTClass sites. We will create configuration files in the next step.' ); ?></p>
	<?php
	...
	if ( allow_subdomain_install() && allow_subdirectory_install() ) : ?>
		...
		<p><?php _e( 'Please choose whether you would like sites in your NXTClass network to use sub-domains or sub-directories. <strong>You cannot change this later.</strong>' ); ?></p>
...
<?php
		...
		<?php elseif ( !allow_subdirectory_install() ) : ?>
			<tr>
				...
				<td><?php _e( 'Because your install is not new, the sites in your NXTClass network must use sub-domains.' );
...
?>
	<div class="error"><p><strong><?php _e('Warning:'); ?></strong> <?php _e( 'An existing NXTClass network was detected.' ); ?></p></div>
...
?>
		<ol>
			...
			?></p></li>
			<li><p><?php printf( __( 'Add the following to your <code>nxt-config.php</code> file in <code>%s</code> <strong>above</strong> the line reading <code>/* That&#8217;s all, stop editing! Happy blogging. */</code>:' ), ABSPATH ); ?></p>
...
?>
	<p><?php
		echo _n( 'This unique authentication key is also missing from your <code>nxt-config.php</code> file.', 'These unique authentication keys are also missing from your <code>nxt-config.php</code> file.', $num_keys_salts ); ?> <?php _e( 'To make your installation more secure, you should also add:' ) ?></p>
...
<configuration>
	...
	?>
		<li><p><?php printf( __( 'Add the following to your <code>web.config</code> file in <code>%s</code>, replacing other NXTClass rules:' ), ABSPATH ); ?></p>
...
RewriteRule ^ - [L]';
		...
		?>
		<li><p><?php printf( __( 'Add the following to your <code>.htaccess</code> file in <code>%s</code>, replacing other NXTClass rules:' ), ABSPATH ); ?></p>
...
<?php echo esc_textarea( $htaccess_file ); ?></textarea></li>
	...
	if ( !is_multisite() ) { ?>
		<p><?php printf( __( 'Once you complete these steps, your network is enabled and configured. You will have to log in again.') ); ?> <a href="<?php echo esc_url( site_url( 'nxt-login.php' ) ); ?>"><?php _e( 'Log In' ); ?></a></p>
<?php
	}
}
...
<?php include( ABSPATH . 'nxt-admin/admin-footer.php' ); ?>
