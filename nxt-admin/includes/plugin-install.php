<?php
/**
 * NXTClass Plugin Install Administration API
 *
 * @package NXTClass
...
/**
 * Retrieve popular NXTClass plugin tags.
 *
 * @since 2.7.0
 *
 * @param array $args
...
?>
	...
	<form method="post" enctype="multipart/form-data" action="<?php echo self_admin_url('update.php?action=upload-plugin') ?>">
		<?php nxt_nonce_field( 'plugin-upload') ?>
...
function install_plugin_install_status($api, $loop = false) {
		...
		foreach ( (array)$update_plugins->response as $file => $plugin ) {
			if ( $plugin->slug === $api->slug ) {
				...
				if ( current_user_can('update_plugins') )
					$url = nxt_nonce_url(self_admin_url('update.php?action=upgrade-plugin&plugin=' . $update_file), 'upgrade-plugin_' . $update_file);
				break;
			}
		}
	}
	...
	if ( 'install' == $status ) {
		if ( is_dir( nxt_PLUGIN_DIR  . '/' . $api->slug ) ) {
			$installed_plugin = get_plugins('/' . $api->slug);
			if ( empty($installed_plugin) ) {
				if ( current_user_can('install_plugins') )
					$url = nxt_nonce_url(self_admin_url('update.php?action=install-plugin&plugin=' . $api->slug), 'install-plugin_' . $api->slug);
			} else {
...
<?php endif; if ( ! empty($api->requires) ) : ?>
			<li><strong><?php _e('Requires NXTClass Version:') ?></strong> <?php printf(__('%s or higher'), $api->requires) ?></li>
...
<?php endif; ?>
	...
	<?php
		...
		else if ( !empty($api->requires) && version_compare( substr($GLOBALS['nxt_version'], 0, strlen($api->requires)), $api->requires, '<') )
