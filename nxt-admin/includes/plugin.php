 ...
 * Version: Must be set in the plugin for NXTClass 2.3+
...
function get_plugin_files($plugin) {
	$plugin_file = nxt_PLUGIN_DIR . '/' . $plugin;
	$dir = dirname($plugin_file);
	$plugin_files = array($plugin);
	if ( is_dir($dir) && $dir != nxt_PLUGIN_DIR ) {
 ...
 *
 * NXTClass only supports plugin files in the base plugins directory
 * (nxt-content/plugins) and in one directory above the plugins directory
 * (nxt-content/plugins/my-plugin). The file it looks for has the plugin data and
...
function get_plugins($plugin_folder = '') {
	...
	if ( ! $cache_plugins = nxt_cache_get('plugins', 'plugins') )
		$cache_plugins = array();
	...
	$nxt_plugins = array ();
	$plugin_root = nxt_PLUGIN_DIR;
	if ( !empty($plugin_folder) )
		$plugin_root .= $plugin_folder;

	// Files in nxt-content/plugins directory
	...
	foreach ( $plugin_files as $plugin_file ) {
		...
		$nxt_plugins[plugin_basename( $plugin_file )] = $plugin_data;
	}

	uasort( $nxt_plugins, '_sort_uname_callback' );

	$cache_plugins[ $plugin_folder ] = $nxt_plugins;
	nxt_cache_set('plugins', $cache_plugins, 'plugins');
 ...
 *
 * NXTClass only includes mu-plugin files in the base mu-plugins directory (nxt-content/mu-plugins).
 *
 * @since 3.0.0
...
function get_mu_plugins() {
	$nxt_plugins = array();
	// Files in nxt-content/mu-plugins directory
	$plugin_files = array();

	if ( ! is_dir( nxtMU_PLUGIN_DIR ) )
		return $nxt_plugins;
	if ( $plugins_dir = @ opendir( nxtMU_PLUGIN_DIR ) ) {
		while ( ( $file = readdir( $plugins_dir ) ) !== false ) {
			if ( substr( $file, -4 ) == '.php' )
				$plugin_files[] = $file;
		}
	...
	foreach ( $plugin_files as $plugin_file ) {
		if ( !is_readable( nxtMU_PLUGIN_DIR . "/$plugin_file" ) )
			continue;

		$plugin_data = get_plugin_data( nxtMU_PLUGIN_DIR . "/$plugin_file", false, false ); //Do not apply markup/translate as it'll be cached.

		if ( empty ( $plugin_data['Name'] ) )
			$plugin_data['Name'] = $plugin_file;

		$nxt_plugins[ $plugin_file ] = $plugin_data;
	}

	if ( isset( $nxt_plugins['index.php'] ) && filesize( nxtMU_PLUGIN_DIR . '/index.php') <= 30 ) // silence is golden
		unset( $nxt_plugins['index.php'] );
...
/**
 * Check the nxt-content directory and retrieve all drop-ins with any plugin data.
 *
 * @since 3.0.0
...
function get_dropins() {
	...
	// These exist in the nxt-content directory
	if ( $plugins_dir = @ opendir( nxt_CONTENT_DIR ) ) {
		while ( ( $file = readdir( $plugins_dir ) ) !== false ) {
	...
	foreach ( $plugin_files as $plugin_file ) {
		if ( !is_readable( nxt_CONTENT_DIR . "/$plugin_file" ) )
			continue;
		$plugin_data = get_plugin_data( nxt_CONTENT_DIR . "/$plugin_file", false, false ); //Do not apply markup/translate as it'll be cached.
		if ( empty( $plugin_data['Name'] ) )
			$plugin_data['Name'] = $plugin_file;
...
/**
 * Returns drop-ins that NXTClass uses.
 *
...
function _get_dropins() {
	$dropins = array(
		'advanced-cache.php' => array( __( 'Advanced caching plugin.'       ), 'nxt_CACHE' ), // nxt_CACHE
...
function is_network_only_plugin( $plugin ) {
	$plugin_data = get_plugin_data( nxt_PLUGIN_DIR . '/' . $plugin );
	if ( $plugin_data )
...
function activate_plugin( $plugin, $redirect = '', $network_wide = false, $silent = false ) {
	...
	if ( !in_array($plugin, $current) ) {
		if ( !empty($redirect) )
			nxt_redirect(add_query_arg('_error_nonce', nxt_create_nonce('plugin-activation-error_' . $plugin), $redirect)); // we'll override this later if the plugin can be included without fatal error
		ob_start();
		include_once(nxt_PLUGIN_DIR . '/' . $plugin);
		...
		if ( ob_get_length() > 0 ) {
			$output = ob_get_clean();
			return new nxt_Error('unexpected_output', __('The plugin generated unexpected output.'), $output);
		}
		ob_end_clean();
	}
 ...
 *
 * When nxt_Error is returned, it does not mean that one of the plugins had
 ...
 * @return bool|nxt_Error True when finished or nxt_Error if there were errors during a plugin activation.
 */
function activate_plugins( $plugins, $redirect = '', $network_wide = false, $silent = false ) {
	...
	if ( !empty($errors) )
		return new nxt_Error('plugins_invalid', __('One of the plugins is invalid.'), $errors);
 ...
 * @return nxt_Error|int 0 on success, nxt_Error on failure.
 */
function validate_plugin($plugin) {
	if ( validate_file($plugin) )
		return new nxt_Error('plugin_invalid', __('Invalid plugin path.'));
	if ( ! file_exists(nxt_PLUGIN_DIR . '/' . $plugin) )
		return new nxt_Error('plugin_not_found', __('Plugin file does not exist.'));

	$installed_plugins = get_plugins();
	if ( ! isset($installed_plugins[$plugin]) )
		return new nxt_Error('no_plugin_header', __('The plugin does not have a valid header.'));
	return 0;
}
...
function is_uninstallable_plugin($plugin) {
	...
	if ( isset( $uninstallable_plugins[$file] ) || file_exists( nxt_PLUGIN_DIR . '/' . dirname($file) . '/uninstall.php' ) )
		return true;
...
function uninstall_plugin($plugin) {
	...
	$uninstallable_plugins = (array) get_option('uninstall_plugins');
	if ( file_exists( nxt_PLUGIN_DIR . '/' . dirname($file) . '/uninstall.php' ) ) {
		...
		define('nxt_UNINSTALL_PLUGIN', $file);
		include nxt_PLUGIN_DIR . '/' . dirname($file) . '/uninstall.php';
...
function add_object_page( $page_title, $menu_title, $capability, $menu_slug, $function = '', $icon_url = '') {
	global $_nxt_last_object_menu;

	$_nxt_last_object_menu++;

	return add_menu_page($page_title, $menu_title, $capability, $menu_slug, $function, $icon_url, $_nxt_last_object_menu);
}
...
function add_utility_page( $page_title, $menu_title, $capability, $menu_slug, $function = '', $icon_url = '') {
	global $_nxt_last_utility_menu;

	$_nxt_last_utility_menu++;

	return add_menu_page($page_title, $menu_title, $capability, $menu_slug, $function, $icon_url, $_nxt_last_utility_menu);
}
 ...
 * that the user has the required capability as well.
 *
 * @param string $parent_slug The slug name for the parent menu (or the file name of a standard NXTClass admin page)
 * @param string $page_title The text to be displayed in the title tags of the page when the menu is selected
 * @param string $menu_title The text to be used for the menu
 * @param string $capability The capability required for this menu to be displayed to the user.
 * @param string $menu_slug The slug name to refer to this menu by (should be unique for this menu)
...
function add_submenu_page( $parent_slug, $page_title, $menu_title, $capability, $menu_slug, $function = '' ) {
	global $submenu;
	global $menu;
	global $_nxt_real_parent_file;
	global $_nxt_submenu_nopriv;
	...
	if ( isset( $_nxt_real_parent_file[$parent_slug] ) )
		$parent_slug = $_nxt_real_parent_file[$parent_slug];

	if ( !current_user_can( $capability ) ) {
		$_nxt_submenu_nopriv[$parent_slug][$menu_slug] = true;
		return false;
	}
	...
	$_registered_pages[$hookname] = true;
	// backwards-compatibility for plugins using add_management page.  See nxt-admin/admin.php for redirect from edit.php to tools.php
...
function get_admin_page_parent( $parent = '' ) {
	...
	global $plugin_page;
	global $_nxt_real_parent_file;
	global $_nxt_menu_nopriv;
	global $_nxt_submenu_nopriv;

	if ( !empty ( $parent ) && 'admin.php' != $parent ) {
		if ( isset( $_nxt_real_parent_file[$parent] ) )
			$parent = $_nxt_real_parent_file[$parent];
		return $parent;
	}

	/*
	if ( !empty ( $parent_file ) ) {
		if ( isset( $_nxt_real_parent_file[$parent_file] ) )
			$parent_file = $_nxt_real_parent_file[$parent_file];
	...
	*/
	...
	if ( $pagenow == 'admin.php' && isset( $plugin_page ) ) {
		...
		if ( isset( $_nxt_menu_nopriv[$plugin_page] ) ) {
			$parent_file = $plugin_page;
			if ( isset( $_nxt_real_parent_file[$parent_file] ) )
					$parent_file = $_nxt_real_parent_file[$parent_file];
			return $parent_file;
	...
	if ( isset( $plugin_page ) && isset( $_nxt_submenu_nopriv[$pagenow][$plugin_page] ) ) {
		$parent_file = $pagenow;
		if ( isset( $_nxt_real_parent_file[$parent_file] ) )
			$parent_file = $_nxt_real_parent_file[$parent_file];
		return $parent_file;
...
function user_can_access_admin_page() {
	...
	if ( !isset( $plugin_page ) && isset( $_nxt_submenu_nopriv[$parent][$pagenow] ) )
		return false;

	if ( isset( $plugin_page ) ) {
		if ( isset( $_nxt_submenu_nopriv[$parent][$plugin_page] ) )
			return false;
	...
	if ( empty( $parent) ) {
		if ( isset( $_nxt_menu_nopriv[$pagenow] ) )
			return false;
		if ( isset( $_nxt_submenu_nopriv[$pagenow][$pagenow] ) )
			return false;
		if ( isset( $plugin_page ) && isset( $_nxt_submenu_nopriv[$pagenow][$plugin_page] ) )
			return false;
		if ( isset( $plugin_page ) && isset( $_nxt_menu_nopriv[$plugin_page] ) )
			return false;
		foreach (array_keys( $_nxt_submenu_nopriv ) as $key ) {
			if ( isset( $_nxt_submenu_nopriv[$key][$pagenow] ) )
				return false;
			if ( isset( $plugin_page ) && isset( $_nxt_submenu_nopriv[$key][$plugin_page] ) )
			return false;
		}
		return true;
	}

	if ( isset( $plugin_page ) && ( $plugin_page == $parent ) && isset( $_nxt_menu_nopriv[$plugin_page] ) )
		return false;
...
function settings_fields($option_group) {
	...
	nxt_nonce_field("$option_group-options");
}
