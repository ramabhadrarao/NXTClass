...
$nxt_file_descriptions = array(
	...
	// Deprecated files
	'nxt-layout.css' => __( 'Stylesheet' ),
	'nxt-comments.php' => __( 'Comments Template' ),
	'nxt-comments-popup.php' => __( 'Popup Comments Template' ),
);

/**
 * Get the description for standard NXTClass theme files and other various standard
 * NXTClass files
 *
 * @since 1.5.0
 *
 * @uses _cleanup_header_comment
 * @uses $nxt_file_descriptions
 * @param string $file Filesystem path or filename
 * @return string Description of file from $nxt_file_descriptions or basename of $file if description doesn't exist
 */
function get_file_description( $file ) {
	global $nxt_file_descriptions;

	if ( isset( $nxt_file_descriptions[basename( $file )] ) ) {
		return $nxt_file_descriptions[basename( $file )];
	}
...
/**
 * Get the absolute filesystem path to the root of the NXTClass installation
 *
 * @since 1.5.0
 *
 * @uses get_option
 * @return string Full filesystem path to the root of the NXTClass installation
 */
function get_home_path() {
	$home = get_option( 'home' );
	$siteurl = get_option( 'siteurl' );
	if ( $home != '' && $home != $siteurl ) {
		$nxt_path_rel_to_home = str_replace($home, '', $siteurl); /* $siteurl - $home */
		$pos = strpos($_SERVER["SCRIPT_FILENAME"], $nxt_path_rel_to_home);
 ...
 * to the install root, otherwise it is assumed the file is relative to the nxt-content
 * directory
 *
 * @since 1.5.0
 *
 * @uses get_home_path
 * @uses nxt_CONTENT_DIR full filesystem path to the nxt-content directory
 * @param string $file filesystem path relative to the NXTClass install directory or to the nxt-content directory
...
function get_real_file_to_edit( $file ) {
	if ('index.php' == $file || '.htaccess' == $file ) {
		...
		$real_file = nxt_CONTENT_DIR . $file;
	}
 ...
 */
function nxt_tempnam($filename = '', $dir = '') {
	if ( empty($dir) )
		$dir = get_temp_dir();
	...
	$filename = $dir . nxt_unique_filename($dir, $filename);
	touch($filename);
	return $filename;
}
...
function validate_file_to_edit( $file, $allowed_files = '' ) {
	...
	switch ( $code ) {
		case 1 :
			nxt_die( __('Sorry, can&#8217;t edit files with &#8220;..&#8221; in the name. If you are trying to edit a file in your NXTClass home directory, you can just type the name of the file in.' ));

		//case 2 :
		//	nxt_die( __('Sorry, can&#8217;t call files with their real path.' ));

		case 3 :
			nxt_die( __('Sorry, that file cannot be edited.' ));
	}
}

/**
 * Handle PHP uploads in NXTClass, sanitizing file names, checking extensions for mime type,
 ...
 */
function nxt_handle_upload( &$file, $overrides = false, $time = null ) {
	...
	$file = apply_filters( 'nxt_handle_upload_prefilter', $file );

	// You may define your own function and pass the name in $overrides['upload_error_handler']
	$upload_error_handler = 'nxt_handle_upload_error';

	// You may have had one or more 'nxt_handle_upload_prefilter' functions error out the file.  Handle that gracefully.
	...
	return apply_filters( 'nxt_handle_upload', array( 'file' => $new_file, 'url' => $url, 'type' => $type ), 'upload' );
}
...
/**
 * Unzips a specified ZIP file to a location on the Filesystem via the NXTClass Filesystem Abstraction.
 * Assumes that nxt_Filesystem() has already been called and set up. Does not extract a root-level __MACOSX directory, if present.
 *
...
function unzip_file($file, $to) {
	...
	if ( ! $nxt_filesystem || !is_object($nxt_filesystem) )
	...
	if ( ! $nxt_filesystem->is_dir($to) ) { //Only do parents if no children exist
 ...
 * Assumes that nxt_Filesystem() has already been called and set up.
 *
 * @since 3.0.0
 * @see unzip_file
 * @access private
...
function _unzip_file_ziparchive($file, $to, $needed_dirs = array() ) {
	...
	if ( true !== $zopen )
		return new nxt_Error('incompatible_archive', __('Incompatible Archive.'));

	for ( $i = 0; $i < $z->numFiles; $i++ ) {
		if ( ! $info = $z->statIndex($i) )
			return new nxt_Error('stat_failed', __('Could not retrieve file from archive.'));
	...
	foreach ( $needed_dirs as $_dir ) {
		if ( ! $nxt_filesystem->mkdir($_dir, FS_CHMOD_DIR) && ! $nxt_filesystem->is_dir($_dir) ) // Only check to see if the Dir exists upon creation failure. Less I/O this way.
			return new nxt_Error('mkdir_failed', __('Could not create directory.'), $_dir);
	}
	unset($needed_dirs);
	...
	for ( $i = 0; $i < $z->numFiles; $i++ ) {
		...
		if ( false === $contents )
			return new nxt_Error('extract_failed', __('Could not extract file from archive.'), $info['name']);

		if ( ! $nxt_filesystem->put_contents( $to . $info['name'], $contents, FS_CHMOD_FILE) )
			return new nxt_Error('copy_failed', __('Could not copy file.'), $to . $info['filename']);
	}
...
function _unzip_file_pclzip($file, $to, $needed_dirs = array()) {
	...
	require_once(ABSPATH . 'nxt-admin/includes/class-pclzip.php');
	...
	if ( !is_array($archive_files) )
		return new nxt_Error('incompatible_archive', __('Incompatible Archive.'), $archive->errorInfo(true));

	if ( 0 == count($archive_files) )
		return new nxt_Error('empty_archive', __('Empty archive.'));
	...
	foreach ( $archive_files as $file ) {
		...
		if ( ! $nxt_filesystem->put_contents( $to . $file['filename'], $file['content'], FS_CHMOD_FILE) )
			return new nxt_Error('copy_failed', __('Could not copy file.'), $to . $file['filename']);
	}
	return true;
}
...
/**
 * Initialises and connects the NXTClass Filesystem Abstraction classes.
 * This function will include the chosen transport and attempt connecting.
 *
 * Plugins may add extra transports, And force NXTClass to use them by returning the filename via the 'filesystem_method_file' filter.
 *
 * @since 2.5.0
 *
 * @param array $args (optional) Connection args, These are passed directly to the nxt_Filesystem_*() classes.
 * @param string $context (optional) Context for get_filesystem_method(), See function declaration for more information.
 * @return boolean false on failure, true on success
 */
function nxt_Filesystem( $args = false, $context = false ) {
	global $nxt_filesystem;

	require_once(ABSPATH . 'nxt-admin/includes/class-nxt-filesystem-base.php');
	...
	if ( ! class_exists("nxt_Filesystem_$method") ) {
		$abstraction_file = apply_filters('filesystem_method_file', ABSPATH . 'nxt-admin/includes/class-nxt-filesystem-' . $method . '.php', $method);
		if ( ! file_exists($abstraction_file) )
			return;

		require_once($abstraction_file);
	}
	$method = "nxt_Filesystem_$method";

	$nxt_filesystem = new $method($args);
 ...
 *  - By defining FS_METHOD in your <code>nxt-config.php</code> file
 *  - By using the filesystem_method filter
 * Valid values for these are: 'direct', 'ssh', 'ftpext' or 'ftpsockets'
 * Plugins may also define a custom transport handler, See the nxt_Filesystem function for more information.
 *
 * @since 2.5.0
 *
...
<p><?php
	...
	_e('To perform the requested action, NXTClass needs to access your web server.');
	echo ' ';
