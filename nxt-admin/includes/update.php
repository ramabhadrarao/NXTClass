<?php
/**
 * NXTClass Administration Update API
 *
 * @package NXTClass
...
function core_update_footer( $msg = '' ) {
	if ( !current_user_can('update_core') )
		return sprintf( __( 'Version %s' ), $GLOBALS['nxt_version'] );
	...
	switch ( $cur->response ) {
	case 'development' :
		return sprintf( __( 'You are using a development version (%1$s). Cool! Please <a href="%2$s">stay updated</a>.' ), $GLOBALS['nxt_version'], network_admin_url( 'update-core.php' ) );
	break;
...
function update_nag() {
	...
	if ( current_user_can('update_core') ) {
		$msg = sprintf( __('<a href="http://codex.opensource.nxtclass.tk/Version_%1$s">NXTClass %1$s</a> is available! <a href="%2$s">Please update now</a>.'), $cur->current, network_admin_url( 'update-core.php' ) );
	} else {
		$msg = sprintf( __('<a href="http://codex.opensource.nxtclass.tk/Version_%1$s">NXTClass %1$s</a> is available! Please notify the site administrator.'), $cur->current );
	}
...
function update_right_now_message() {
	$msg = sprintf( __('You are using <span class="b">NXTClass %s</span>.'), $GLOBALS['nxt_version'] );
	...
	echo "<span id='nxt-version-message'>$msg</span>";
}
...
function nxt_plugin_update_rows() {
	...
	if ( isset($plugins->response) && is_array($plugins->response) ) {
		...
		foreach( $plugins as $plugin_file ) {
			add_action( "after_plugin_row_$plugin_file", 'nxt_plugin_update_row', 10, 2 );
		}
	}
}
add_action( 'admin_init', 'nxt_plugin_update_rows' );

function nxt_plugin_update_row( $file, $plugin_data ) {
	$current = get_site_transient( 'update_plugins' );
	...
	$plugin_name = nxt_kses( $plugin_data['Name'], $plugins_allowedtags );
	...
	if ( is_network_admin() || !is_multisite() ) {
		...
		if ( ! current_user_can('update_plugins') )
			...
			printf( __('There is a new version of %1$s available. <a href="%2$s" class="thickbox" title="%3$s">View version %4$s details</a> or <a href="%5$s">update automatically</a>.'), $plugin_name, esc_url($details_url), esc_attr($plugin_name), $r->new_version, nxt_nonce_url( self_admin_url('update.php?action=upgrade-plugin&plugin=') . $file, 'upgrade-plugin_' . $file) );
...
function nxt_update_plugin($plugin, $feedback = '') {
	if ( !empty($feedback) )
		add_filter('update_feedback', $feedback);

	include ABSPATH . 'nxt-admin/includes/class-nxt-upgrader.php';
...
function nxt_update_theme($theme, $feedback = '') {
	if ( !empty($feedback) )
		add_filter('update_feedback', $feedback);
...
function nxt_theme_update_rows() {
	...
	if ( isset($themes->response) && is_array($themes->response) ) {
		...
		foreach( $themes as $theme ) {
			add_action( "after_theme_row_$theme", 'nxt_theme_update_row', 10, 2 );
		}
	}
}
add_action( 'admin_init', 'nxt_theme_update_rows' );
...
function nxt_update_core($current, $feedback = '') {
	if ( !empty($feedback) )
		add_filter('update_feedback', $feedback);
...
function maintenance_nag() {
	...
	if ( current_user_can('update_core') )
		$msg = sprintf( __('An automated NXTClass update has failed to complete - <a href="%s">please attempt the update again now</a>.'), 'update-core.php' );
	else
		$msg = __('An automated NXTClass update has failed to complete! Please notify the site administrator.');
