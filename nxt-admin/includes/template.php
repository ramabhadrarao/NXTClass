<?php
/**
 * Template NXTClass Administration API.
 *
 ...
 * @return unknown
 */
function nxt_popular_terms_checklist( $taxonomy, $default = 0, $number = 10, $echo = true ) {
	global $post_ID;

	if ( $post_ID )
		$checked_terms = nxt_get_object_terms($post_ID, $taxonomy, array('fields'=>'ids'));
	else
		$checked_terms = array();
...
<?php if ( $table_row ) : ?>
<table style="display:none;"><tbody id="com-reply"><tr id="replyrow" style="display:none;"><td colspan="<?php echo $nxt_list_table->get_column_count(); ?>" class="colspanchange">
<?php else : ?>
...
<?php endif; ?>
	...
	<img class="waiting" style="display:none;" src="<?php echo esc_url( admin_url( 'images/nxtspin_light.gif' ) ); ?>" alt="" />
 ...
 * @since 2.9.0
 */
function nxt_comment_trashnotice() {
?>
...
function _list_meta_row( $entry, &$count ) {
	...
	if ( !$update_nonce )
		$update_nonce = nxt_create_nonce( 'add-meta' );
	...
	$delete_nonce = nxt_create_nonce( 'delete-meta_' . $entry['meta_id'] );
	...
	$r .= "</div>";
	$r .= nxt_nonce_field( 'change-meta', '_ajax_nonce', false, false );
	$r .= "</td>";
...
<?php nxt_nonce_field( 'add-meta', '_ajax_nonce-add-meta', false ); ?>
</td></tr>
</tbody>
</table>
<?php
...
function touch_time( $edit = 1, $for_post = 1, $tab_index = 0, $multi = 0 ) {
	global $nxt_locale, $post, $comment;
	...
	for ( $i = 1; $i < 13; $i = $i +1 ) {
		...
		$month .= '>' . $monthnum . '-' . $nxt_locale->get_month_abbrev( $nxt_locale->get_month( $i ) ) . "</option>\n";
	}
	$month .= '</select>';
...
function parent_dropdown( $default = 0, $parent = 0, $level = 0 ) {
	global $nxtdb, $post_ID;
	$items = $nxtdb->get_results( $nxtdb->prepare("SELECT ID, post_parent, post_title FROM $nxtdb->posts WHERE post_parent = %d AND post_type = 'page' ORDER BY menu_order", $parent) );
...
<table>
	...
	<tr>
		...
		<td><textarea rows="1" cols="40" type="text" class="attachmentlinks" readonly="readonly"><?php echo esc_textarea( nxt_get_attachment_url() ); ?></textarea></td>
	</tr>
<?php if ( $icon ) : ?>
	<tr>
		<th scope="row"><?php $thumb ? _e( 'Thumbnail linked to file' ) : _e( 'Image linked to file' ); ?></th>
		<td><textarea rows="1" cols="40" type="text" class="attachmentlinks" readonly="readonly"><a href="<?php echo nxt_get_attachment_url(); ?>"><?php echo $icon ?></a></textarea></td>
	</tr>
	<tr>
		<th scope="row"><?php $thumb ? _e( 'Thumbnail linked to page' ) : _e( 'Image linked to page' ); ?></th>
		<td><textarea rows="1" cols="40" type="text" class="attachmentlinks" readonly="readonly"><a href="<?php echo get_attachment_link( $post->ID ) ?>" rel="attachment nxt-att-<?php echo $post->ID; ?>"><?php echo $icon ?></a></textarea></td>
	</tr>
<?php else : ?>
	<tr>
		<th scope="row"><?php _e( 'Link to file' ) ?></th>
		<td><textarea rows="1" cols="40" type="text" class="attachmentlinks" readonly="readonly"><a href="<?php echo nxt_get_attachment_url(); ?>" class="attachmentlink"><?php echo basename( nxt_get_attachment_url() ); ?></a></textarea></td>
	</tr>
	<tr>
		<th scope="row"><?php _e( 'Link to page' ) ?></th>
		<td><textarea rows="1" cols="40" type="text" class="attachmentlinks" readonly="readonly"><a href="<?php echo get_attachment_link( $post->ID ) ?>" rel="attachment nxt-att-<?php echo $post->ID ?>"><?php the_title(); ?></a></textarea></td>
	</tr>
<?php endif; ?>
</table>
</form>
 ...
 */
function nxt_dropdown_roles( $selected = false ) {
	$p = '';
	$r = '';
 ...
 * @return unknown
 */
function nxt_convert_hr_to_bytes( $size ) {
 ...
 * @return unknown
 */
function nxt_convert_bytes_to_hr( $bytes ) {
 ...
 * @return unknown
 */
function nxt_max_upload_size() {
	$u_bytes = nxt_convert_hr_to_bytes( ini_get( 'upload_max_filesize' ) );
	$p_bytes = nxt_convert_hr_to_bytes( ini_get( 'post_max_size' ) );
...
?>
<form enctype="multipart/form-data" id="import-upload-form" method="post" action="<?php echo esc_attr(nxt_nonce_url($action, 'import-upload')); ?>">
<p>
...
function add_meta_box( $id, $title, $callback, $screen = null, $context = 'advanced', $priority = 'default', $callback_args = null ) {
	...
	if ( !isset($nxt_meta_boxes) )
		$nxt_meta_boxes = array();
	if ( !isset($nxt_meta_boxes[$page]) )
		$nxt_meta_boxes[$page] = array();
	if ( !isset($nxt_meta_boxes[$page][$context]) )
		$nxt_meta_boxes[$page][$context] = array();

	foreach ( array_keys($nxt_meta_boxes[$page]) as $a_context ) {
		foreach ( array('high', 'core', 'default', 'low') as $a_priority ) {
			if ( !isset($nxt_meta_boxes[$page][$a_context][$a_priority][$id]) )
				continue;

			// If a core box was previously added or removed by a plugin, don't add.
			if ( 'core' == $priority ) {
				// If core box previously deleted, don't add
				if ( false === $nxt_meta_boxes[$page][$a_context][$a_priority][$id] )
					return;
				// If box was added with default priority, give it core priority to maintain sort order
				if ( 'default' == $a_priority ) {
					$nxt_meta_boxes[$page][$a_context]['core'][$id] = $nxt_meta_boxes[$page][$a_context]['default'][$id];
					unset($nxt_meta_boxes[$page][$a_context]['default'][$id]);
				}
				return;
			}
			...
			// else if we're adding to the sorted priority, we don't know the title or callback. Grab them from the previously added context/priority.
				...
				$title = $nxt_meta_boxes[$page][$a_context][$a_priority][$id]['title'];
				$callback = $nxt_meta_boxes[$page][$a_context][$a_priority][$id]['callback'];
				$callback_args = $nxt_meta_boxes[$page][$a_context][$a_priority][$id]['args'];
			}
			// An id can be in only one priority and one context
			if ( $priority != $a_priority || $context != $a_context )
				unset($nxt_meta_boxes[$page][$a_context][$a_priority][$id]);
		}
	}
	...
	if ( !isset($nxt_meta_boxes[$page][$context][$priority]) )
		$nxt_meta_boxes[$page][$context][$priority] = array();

	$nxt_meta_boxes[$page][$context][$priority][$id] = array('id' => $id, 'title' => $title, 'callback' => $callback, 'args' => $callback_args);
}
...
function do_meta_boxes( $screen, $context, $object ) {
	...
	do {
		...
		if ( !isset($nxt_meta_boxes) || !isset($nxt_meta_boxes[$page]) || !isset($nxt_meta_boxes[$page][$context]) )
			break;

		foreach ( array('high', 'sorted', 'core', 'default', 'low') as $priority ) {
			if ( isset($nxt_meta_boxes[$page][$context][$priority]) ) {
				foreach ( (array) $nxt_meta_boxes[$page][$context][$priority] as $box ) {
...
function remove_meta_box($id, $screen, $context) {
	...
	foreach ( array('high', 'core', 'default', 'low') as $priority )
		$nxt_meta_boxes[$page][$context][$priority][$id] = false;
}
 ...
 * @since 2.7.0
 *
 * @global $nxt_settings_sections Storage array of all settings sections added to admin pages
 *
...
function add_settings_section($id, $title, $callback, $page) {
	...
	if ( !isset($nxt_settings_sections) )
		$nxt_settings_sections = array();
	if ( !isset($nxt_settings_sections[$page]) )
		$nxt_settings_sections[$page] = array();
	if ( !isset($nxt_settings_sections[$page][$id]) )
		$nxt_settings_sections[$page][$id] = array();

	$nxt_settings_sections[$page][$id] = array('id' => $id, 'title' => $title, 'callback' => $callback);
}
 ...
 * @since 2.7.0
 *
 * @global $nxt_settings_fields Storage array of settings fields and info about their pages/sections
 *
...
function add_settings_field($id, $title, $callback, $page, $section = 'default', $args = array()) {
	...
	if ( !isset($nxt_settings_fields) )
		$nxt_settings_fields = array();
	if ( !isset($nxt_settings_fields[$page]) )
		$nxt_settings_fields[$page] = array();
	if ( !isset($nxt_settings_fields[$page][$section]) )
		$nxt_settings_fields[$page][$section] = array();

	$nxt_settings_fields[$page][$section][$id] = array('id' => $id, 'title' => $title, 'callback' => $callback, 'args' => $args);
}
...
function do_settings_sections($page) {
	global $nxt_settings_sections, $nxt_settings_fields;

	if ( !isset($nxt_settings_sections) || !isset($nxt_settings_sections[$page]) )
		return;

	foreach ( (array) $nxt_settings_sections[$page] as $section ) {
		if ( $section['title'] )
			echo "<h3>{$section['title']}</h3>\n";
		call_user_func($section['callback'], $section);
		if ( !isset($nxt_settings_fields) || !isset($nxt_settings_fields[$page]) || !isset($nxt_settings_fields[$page][$section['id']]) )
			continue;
 ...
 *
 * @global $nxt_settings_fields Storage array of settings fields and their pages/sections
 *
 * @since 2.7.0
 *
...
function do_settings_fields($page, $section) {
	...
	if ( !isset($nxt_settings_fields) || !isset($nxt_settings_fields[$page]) || !isset($nxt_settings_fields[$page][$section]) )
		return;

	foreach ( (array) $nxt_settings_fields[$page][$section] as $field ) {
		echo '<tr valign="top">';
 ...
 * @since 3.0.0
 *
 * @global array $nxt_settings_errors Storage array of errors registered during this pageload
 *
...
function add_settings_error( $setting, $code, $message, $type = 'error' ) {
	global $nxt_settings_errors;

	if ( !isset($nxt_settings_errors) )
		$nxt_settings_errors = array();
	...
	$nxt_settings_errors[] = $new_error;
}

/**
 * Fetch settings errors registered by add_settings_error()
 *
 * Checks the $nxt_settings_errors array for any errors declared during the current
...
function get_settings_errors( $setting = '', $sanitize = FALSE ) {
	...
	} elseif ( count( $nxt_settings_errors ) ) {
		$settings_errors = $nxt_settings_errors;
	} else {
		return;
	}
	...
	<div id="find-posts" class="find-box" style="display:none;">
		...
		<div class="find-box-inside">
			<div class="find-box-search">
				...
				<?php nxt_nonce_field( 'find-posts', '_ajax_nonce', false ); ?>
...
?>
<title><?php bloginfo('name') ?> &rsaquo; <?php echo $title ?> &#8212; <?php _e('NXTClass'); ?></title>
<?php
...
<body<?php if ( isset($GLOBALS['body_id']) ) echo ' id="' . $GLOBALS['body_id'] . '"'; ?>  class="nxt-admin no-js iframe <?php echo apply_filters( 'admin_body_class', '' ) . ' ' . $admin_body_class; ?>">
<script type="text/javascript">
//<![CDATA[
(function(){
...
function _media_states( $post ) {
	...
	if ( current_theme_supports( 'custom-header') ) {
		$meta_header = get_post_meta($post->ID, '_nxt_attachment_is_custom_header', true );
		if ( ! empty( $meta_header ) && $meta_header == $stylesheet )
			$media_states[] = __( 'Header Image' );
	}

	if ( current_theme_supports( 'custom-background') ) {
		$meta_background = get_post_meta($post->ID, '_nxt_attachment_is_custom_background', true );
	...
	var testCompression = {
		get : function(test) {
			...
			if (x) {
				...
				x.open('GET', ajaxurl + '?action=nxt-compression-test&test='+test+'&'+(new Date()).getTime(), true);
				x.send('');
			}
		},
...
function _nxt_admin_html_begin() {
?>
<!DOCTYPE html>
<!--[if IE 8]>
...
final class nxt_Internal_Pointers {
	/**
	 ...
	 *     remove_action( 'admin_enqueue_scripts', array( 'nxt_Internal_Pointers', 'enqueue_scripts' ) );
	 *
	 * Individual pointers (e.g. nxt330_toolbar) can be disabled using the following:
	 *     remove_action( 'admin_print_footer_scripts', array( 'nxt_Internal_Pointers', 'pointer_nxt330_toolbar' ) );
	 */
	public static function enqueue_scripts( $hook_suffix ) {
		...
		$registered_pointers = array(
			'index.php'    => 'nxt330_toolbar',
			'post-new.php' => 'nxt330_media_uploader',
			'post.php'     => 'nxt330_media_uploader',
			'themes.php'   => 'nxt330_saving_widgets',
		);
		...
		$caps_required = array(
			'nxt330_media_uploader' => array( 'upload_files' ),
			'nxt330_saving_widgets' => array( 'edit_theme_options', 'switch_themes' ),
		);
		...
		$dismissed = explode( ',', (string) get_user_meta( get_current_user_id(), 'dismissed_nxt_pointers', true ) );
		...
		add_action( 'admin_print_footer_scripts', array( 'nxt_Internal_Pointers', 'pointer_' . $pointer ) );

		// Add pointers script and style to queue
		nxt_enqueue_style( 'nxt-pointer' );
		nxt_enqueue_script( 'nxt-pointer' );
	}
	 ...
	 * @param array  $args Arguments to be passed to the pointer JS (see nxt-pointer.dev.js).
	 */
	...
	public static function pointer_nxt330_toolbar() {
		...
		nxt_Internal_Pointers::print_js( 'nxt330_toolbar', '#nxtadminbar', array(
			'content'  => $content,
	 ...
	 * @since 3.3.0
	 */
	public static function pointer_nxt330_media_uploader() {
		...
		nxt_Internal_Pointers::print_js( 'nxt330_media_uploader', '#content-add_media', array(
			'content'  => $content,
	 ...
	 * @since 3.3.0
	 */
	public static function pointer_nxt330_saving_widgets() {
		...
		nxt_Internal_Pointers::print_js( 'nxt330_saving_widgets', '#message2', array(
			'content'  => $content,
	...
	public static function dismiss_pointers_for_new_users( $user_id ) {
		add_user_meta( $user_id, 'dismissed_nxt_pointers', 'nxt330_toolbar,nxt330_media_uploader,nxt330_saving_widgets' );
	}
}

add_action( 'admin_enqueue_scripts', array( 'nxt_Internal_Pointers', 'enqueue_scripts'                ) );
add_action( 'user_register',         array( 'nxt_Internal_Pointers', 'dismiss_pointers_for_new_users' ) );
