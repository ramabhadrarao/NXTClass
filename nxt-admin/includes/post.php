 ...
 * @return object|bool nxt_Error on failure, true on success.
 */
function _nxt_translate_postdata( $update = false, $post_data = null ) {
...
function edit_post( $post_data = null ) {
	...
	$post_data = _nxt_translate_postdata( true, $post_data );
	if ( is_nxt_error($post_data) )
		nxt_die( $post_data->get_error_message() );
...
function get_default_post_to_edit( $post_type = 'post', $create_in_db = false ) {
	...
	if ( $create_in_db ) {
		...
		foreach ( (array) $old_posts as $delete )
			nxt_delete_post( $delete, true ); // Force delete
...
function get_post_to_edit( $id ) {
	...
	if ( $post->post_type == 'page' )
		$post->page_template = get_post_meta( $id, '_nxt_page_template', true );
...
function post_exists($title, $content = '', $date = '') {
	...
	$query = "SELECT ID FROM $nxtdb->posts WHERE 1=1";
	$args = array();
	...
	if ( !empty ( $args ) )
		return $nxtdb->get_var( $nxtdb->prepare($query, $args) );
...
/**
 * Calls nxt_write_post() and handles the errors.
 *
 * @since 2.0.0
 ...
 */
function write_post() {
	$result = nxt_write_post();
	if ( is_nxt_error( $result ) )
		nxt_die( $result->get_error_message() );
	else
		return $result;
}
...
function has_meta( $postid ) {
	...
	return $nxtdb->get_results( $nxtdb->prepare("SELECT meta_key, meta_value, meta_id, post_id
			FROM $nxtdb->postmeta WHERE post_id = %d
...
function _relocate_children( $old_ID, $new_ID ) {
	...
	$children = $nxtdb->get_col( $nxtdb->prepare("
		SELECT post_id
		FROM $nxtdb->postmeta
		WHERE meta_key = '_nxt_attachment_temp_parent'
		AND meta_value = %d", $old_ID) );

	foreach ( $children as $child_id ) {
		$nxtdb->update($nxtdb->posts, array('post_parent' => $new_ID), array('ID' => $child_id) );
		delete_post_meta($child_id, '_nxt_attachment_temp_parent');
	}
}
...
function get_available_post_statuses($type = 'post') {
	$stati = nxt_count_posts($type);

	return array_keys(get_object_vars($stati));
}

/**
 * Run the nxt query to fetch the posts for listing on the edit posts page
 *
 * @since 2.5.0
 *
 ...
 * @return array
 */
function nxt_edit_posts_query( $q = false ) {
	if ( false === $q )
		$q = $_GET;
...
function get_available_post_mime_types($type = 'attachment') {
	...
	$types = $nxtdb->get_col($nxtdb->prepare("SELECT DISTINCT post_mime_type FROM $nxtdb->posts WHERE post_type = %s", $type));
	return $types;
}
 ...
 */
function nxt_check_post_lock( $post_id ) {
	...
	$time_window = apply_filters( 'nxt_check_post_lock_window', AUTOSAVE_INTERVAL * 2 );
 ...
 */
function nxt_set_post_lock( $post_id ) {
	if ( !$post = get_post( $post_id ) )
		return false;
