 ...
 * @return unknown
 */
function nxt_create_category( $cat_name, $parent = 0 ) {
	if ( $id = category_exists($cat_name, $parent) )
		return $id;

	return nxt_insert_category( array('cat_name' => $cat_name, 'category_parent' => $parent) );
}
 ...
 * @return unknown
 */
function nxt_create_categories($categories, $post_id = '') {
	$cat_ids = array ();
	foreach ($categories as $category) {
		if ($id = category_exists($category))
			$cat_ids[] = $id;
		else
			if ($id = nxt_create_category($category))
				$cat_ids[] = $id;
	}

	if ( $post_id )
		nxt_set_post_categories($post_id, $cat_ids);
 ...
 * @return unknown
 */
function nxt_create_tag($tag_name) {
	return nxt_create_term( $tag_name, 'post_tag');
}
...
function get_terms_to_edit( $post_id, $taxonomy = 'post_tag' ) {
	...
	$tags = nxt_get_post_terms($post_id, $taxonomy, array());
