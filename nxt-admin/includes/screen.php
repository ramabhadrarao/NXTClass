 ...
 * @since 2.7.0
 *
 * @param string|nxt_Screen $screen The screen you want the headers for
 ...
 * @since 2.7.0
 *
 * @param string|nxt_Screen $screen The screen you want the hidden columns for
 * @return array
 */
 ...
 * @since 2.7.0
 *
 * @param string|nxt_Screen $screen
 */
function meta_box_prefs( $screen ) {
	...
	if ( empty($nxt_meta_boxes[$screen->id]) )
		return;

	$hidden = get_hidden_meta_boxes($screen);

	foreach ( array_keys($nxt_meta_boxes[$screen->id]) as $context ) {
		foreach ( array_keys($nxt_meta_boxes[$screen->id][$context]) as $priority ) {
			foreach ( $nxt_meta_boxes[$screen->id][$context][$priority] as $box ) {
 ...
 * @since 2.7.0
 *
 * @param string|nxt_Screen $screen Screen identifier
 ...
 * @since 2.7.0
 *
 * @param string|nxt_Screen $screen Optional. Accepts a screen object (and defaults to the current screen object)
...
function set_current_screen( $hook_name =  '' ) {
	nxt_Screen::get( $hook_name )->set_current_screen();
}
...
final class nxt_Screen {
	 ...
	 * @since 3.3.0
	 *
	 * @param nxt_Screen $screen A screen object.
	...
	public function show_screen_options() {
		...
		$show_screen = ! empty( $nxt_meta_boxes[ $this->id ] ) || $columns || $this->get_option( 'per_page' );
	...
	public function render_screen_options() {
		global $nxt_meta_boxes, $nxt_list_table;
		...
		<?php
		if ( isset( $nxt_meta_boxes[ $this->id ] ) ) : ?>
				...
				<?php
					...
					if ( 'dashboard' === $this->id && current_user_can( 'edit_theme_options' ) ) {
						if ( isset( $_GET['welcome'] ) ) {
							...
							if ( 2 == $welcome_checked && nxt_get_current_user()->user_email != get_option( 'admin_email' ) )
								$welcome_checked = false;
						}
						echo '<label for="nxt_welcome_panel-hide">';
						echo '<input type="checkbox" id="nxt_welcome_panel-hide"' . checked( (bool) $welcome_checked, true, false )  . ' />';
		...
		?>
		<div><?php nxt_nonce_field( 'screen-options-nonce', 'screenoptionnonce', false ); ?></div>
		</form>
		</div>
		<?php
	}
	...
	function render_per_page_options() {
		...
		<div class='screen-options'>
			<?php if ( !empty($per_page_label) ): ?>
				<input type='text' class='screen-per-page' name='nxt_screen_options[value]'
			...
			<input type='hidden' name='nxt_screen_options[option]' value='<?php echo esc_attr($option); ?>' />
		</div>
		<?php
	}
}
