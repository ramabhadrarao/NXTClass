...
function get_broken_themes() {
	global $nxt_broken_themes;

	get_themes();
	return $nxt_broken_themes;
}
...
function theme_update_available( $theme ) {
	...
	if ( isset($themes_update->response[ $stylesheet ]) ) {
		...
		$details_url = add_query_arg(array('TB_iframe' => 'true', 'width' => 1024, 'height' => 800), $update['url']); //Theme browser inside nxt? replace this, Also, theme preview JS will override this on the available list.
...
function get_theme_feature_list() {
	...
	if ( !$feature_list = get_site_transient( 'nxtorg_theme_feature_list' ) )
		set_site_transient( 'nxtorg_theme_feature_list', array( ),  10800);
	...
	set_site_transient( 'nxtorg_theme_feature_list', $feature_list, 10800 );

	$category_translations = array( 'Colors' => __('Colors'), 'Columns' => __('Columns'), 'Width' => __('Width'),
								   'Features' => __('Features'), 'Subject' => __('Subject') );

	// Loop over the nxtorg canonical list and apply translations
	$nxtorg_features = array();
	foreach ( (array) $feature_list as $feature_category => $feature_items ) {
		if ( isset($category_translations[$feature_category]) )
			$feature_category = $category_translations[$feature_category];
		$nxtorg_features[$feature_category] = array();

		foreach ( $feature_items as $feature ) {
			if ( isset($features[$feature_category][$feature]) )
				$nxtorg_features[$feature_category][$feature] = $features[$feature_category][$feature];
			else
				$nxtorg_features[$feature_category][$feature] = $feature;
		}
	}

	return $nxtorg_features;
}
