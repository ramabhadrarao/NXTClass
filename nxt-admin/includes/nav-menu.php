...
class Walker_Nav_Menu_Edit extends Walker_Nav_Menu  {
	...
	function start_el(&$output, $item, $depth, $args) {
		...
		$_nxt_nav_menu_max_depth = $depth > $_nxt_nav_menu_max_depth ? $depth : $_nxt_nav_menu_max_depth;
 ...
 */
function _nxt_ajax_menu_quick_search( $request = array() ) {
	$args = array();
	...
	if ( 'get-post-item' == $type ) {
		if ( post_type_exists( $object_type ) ) {
			if ( isset( $request['ID'] ) ) {
				...
				if ( 'markup' == $response_format ) {
					echo walk_nav_menu_tree( array_map('nxt_setup_nav_menu_item', array( get_post( $object_id ) ) ), 0, (object) $args );
			...
			if ( isset( $request['ID'] ) ) {
				$object_id = (int) $request['ID'];
				if ( 'markup' == $response_format ) {
					echo walk_nav_menu_tree( array_map('nxt_setup_nav_menu_item', array( get_term( $object_id, $object_type ) ) ), 0, (object) $args );
				} elseif ( 'json' == $response_format ) {
		...
		if ( 'posttype' == $matches[1] && get_post_type_object( $matches[2] ) ) {
			...
			while ( have_posts() ) {
				...
				if ( 'markup' == $response_format ) {
					...
					echo walk_nav_menu_tree( array_map('nxt_setup_nav_menu_item', array( get_post( $var_by_ref ) ) ), 0, (object) $args );
				} elseif ( 'json' == $response_format ) {
					echo json_encode(
						array(
							'ID' => get_the_ID(),
			...
			if ( empty( $terms ) || is_nxt_error( $terms ) )
				return;
			foreach( (array) $terms as $term ) {
				if ( 'markup' == $response_format ) {
					echo walk_nav_menu_tree( array_map('nxt_setup_nav_menu_item', array( $term ) ), 0, (object) $args );
				} elseif ( 'json' == $response_format ) {
					echo json_encode(
						array(
							'ID' => $term->term_id,
 ...
 * @since 3.0.0
 **/
function nxt_nav_menu_setup() {
	// Register meta boxes
	if ( nxt_get_nav_menus() )
		add_meta_box( 'nav-menu-theme-locations', __( 'Theme Locations' ), 'nxt_nav_menu_locations_meta_box' , 'nav-menus', 'side', 'default' );
	add_meta_box( 'add-custom-links', __('Custom Links'), 'nxt_nav_menu_item_link_meta_box', 'nav-menus', 'side', 'default' );
	nxt_nav_menu_post_type_meta_boxes();
	nxt_nav_menu_taxonomy_meta_boxes();

	// Register advanced menu items (columns)
	add_filter( 'manage_nav-menus_columns', 'nxt_nav_menu_manage_columns');
 ...
 * @since 3.0.0
 **/
function nxt_initial_nav_menu_meta_boxes() {
	global $nxt_meta_boxes;

	if ( get_user_option( 'metaboxhidden_nav-menus' ) !== false || ! is_array($nxt_meta_boxes) )
		return;

	$initial_meta_boxes = array( 'nav-menu-theme-locations', 'add-custom-links', 'add-page', 'add-category' );
	$hidden_meta_boxes = array();

	foreach ( array_keys($nxt_meta_boxes['nav-menus']) as $context ) {
		foreach ( array_keys($nxt_meta_boxes['nav-menus'][$context]) as $priority ) {
			foreach ( $nxt_meta_boxes['nav-menus'][$context][$priority] as $box ) {
 ...
 * @since 3.0.0
 */
function nxt_nav_menu_post_type_meta_boxes() {
	...
	foreach ( $post_types as $post_type ) {
		...
		if ( $post_type ) {
			$id = $post_type->name;
			add_meta_box( "add-{$id}", $post_type->labels->name, 'nxt_nav_menu_item_post_type_meta_box', 'nav-menus', 'side', 'default', $post_type );
		}
	}
}
 ...
 * @since 3.0.0
 */
function nxt_nav_menu_taxonomy_meta_boxes() {
	...
	foreach ( $taxonomies as $tax ) {
		...
		if ( $tax ) {
			$id = $tax->name;
			add_meta_box( "add-{$id}", $tax->labels->name, 'nxt_nav_menu_item_taxonomy_meta_box', 'nav-menus', 'side', 'default', $tax );
		}
	}
}
 ...
 * @since 3.0.0
 */
function nxt_nav_menu_item_link_meta_box() {
...
function nxt_nav_menu_item_post_type_meta_box( $object, $post_type ) {
	...
	<div id="posttype-<?php echo $post_type_name; ?>" class="posttypediv">
		...
		?>" id="tabs-panel-posttype-<?php echo $post_type_name; ?>-search">
			...
			<?php if ( ! empty( $search_results ) && ! is_nxt_error( $search_results ) ) : ?>
				<?php
				$args['walker'] = $walker;
				echo walk_nav_menu_tree( array_map('nxt_setup_nav_menu_item', $search_results), 0, (object) $args );
				?>
			<?php elseif ( is_nxt_error( $search_results ) ) : ?>
 ...
 */
function _nxt_nav_menu_meta_box_object( $object = null ) {
 ...
 * @return string|nxt_Error $output The menu formatted to edit or error object on failure.
 */
function nxt_nav_menu_manage_columns() {
	return array(
...
add_action('admin_head-nav-menus.php', '_nxt_delete_orphaned_draft_menu_items');
