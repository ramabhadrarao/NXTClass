...
class nxt_Importer {
	...
	/**
	 * Returns array with imported permalinks from NXTClass database
	 *
	 * @param string $bid
	 * @return array
	 */
	function get_imported_posts( $importer_name, $bid ) {
		...
		do {
			...
			$sql = $nxtdb->prepare( "SELECT post_id, meta_value FROM $nxtdb->postmeta WHERE meta_key = '%s' LIMIT %d,%d", $meta_key, $offset, $limit );
	...
	/**
	 * Return count of imported permalinks from NXTClass database
	 *
	 * @param string $bid
	 * @return int
	 */
	function count_imported_posts( $importer_name, $bid ) {
		...
		$meta_key = $importer_name . '_' . $bid . '_permalink';
		$sql = $nxtdb->prepare( "SELECT COUNT( post_id ) AS cnt FROM $nxtdb->postmeta WHERE meta_key = '%s'", $meta_key );

		$result = $nxtdb->get_results( $sql );
	...
	/**
	 * Set array with imported comments from NXTClass database
	 *
	 * @param string $bid
	 * @return array
	 */
	function get_imported_comments( $bid ) {
		...
		do {
			$sql = $nxtdb->prepare( "SELECT comment_ID, comment_agent FROM $nxtdb->comments LIMIT %d,%d", $offset, $limit );
			$results = $nxtdb->get_results( $sql );
	...
	function stop_the_insanity() {
		global $nxtdb, $nxt_actions;
		// Or define( 'nxt_IMPORTING', true );
		$nxtdb->queries = array();
		// Reset $nxt_actions to keep it from growing out of control
		$nxt_actions = array();
	}
}
