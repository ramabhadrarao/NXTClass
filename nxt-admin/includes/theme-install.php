<?php
/**
 * NXTClass Theme Install Administration API
 *
 * @package NXTClass
...
/**
 * Retrieve list of NXTClass theme features (aka theme tags)
 *
 * @since 2.8.0
 *
...
function install_themes_feature_list( ) {
	if ( !$cache = get_transient( 'nxtorg_theme_feature_list' ) )
		set_transient( 'nxtorg_theme_feature_list', array( ),  10800);

	if ( $cache  )
		return $cache;

	$feature_list = themes_api( 'feature_list', array( ) );
	if ( is_nxt_error( $feature_list ) )
		return $features;

	set_transient( 'nxtorg_theme_feature_list', $feature_list, 10800 );
...
<form method="post" enctype="multipart/form-data" action="<?php echo self_admin_url('update.php?action=upload-theme') ?>">
	<?php nxt_nonce_field( 'theme-upload') ?>
...
function display_theme($theme, $actions = null, $show_details = true) {
	...
	$name = nxt_kses($theme->name, $themes_allowedtags);
	$desc = nxt_kses($theme->description, $themes_allowedtags);
	//if ( strlen($desc) > 30 )
...
<p><strong><?php _e('Version:') ?></strong> <?php echo nxt_kses($theme->version, $themes_allowedtags) ?></p>
<p><strong><?php _e('Author:') ?></strong> <?php echo nxt_kses($theme->author, $themes_allowedtags) ?></p>
...
<p><strong><?php _e('Requires NXTClass Version:') ?></strong> <?php printf(__('%s or higher'), $theme->requires) ?></p>
...
<?php }
	/*
	 ...
	 public 'preview_url' => string 'http://nxt-themes.com/?magazine-basic' (length=36)
...
function display_themes() {
	global $nxt_list_table;

	$nxt_list_table->display();
}
...
function install_theme_information() {
	...
	else if ( !empty($api->requires) && version_compare($GLOBALS['nxt_version'], $api->requires, '<') )
