<?php
/**
 * NXTClass Administration Scheme API
 *
...
global $nxtdb, $nxt_queries, $charset_collate;
 ...
 */
function nxt_get_db_schema( $scope = 'all', $blog_id = null ) {
	global $nxtdb;
	...
	if ( $blog_id && $blog_id != $nxtdb->blogid )
		$old_blog_id = $nxtdb->set_blog_id( $blog_id );

	// Engage multisite if in the middle of turning it on from network.php.
	$is_multisite = is_multisite() || ( defined( 'nxt_INSTALLING_NETWORK' ) && nxt_INSTALLING_NETWORK );

	// Blog specific tables.
	$blog_tables = "CREATE TABLE $nxtdb->terms (
...
CREATE TABLE $nxtdb->term_taxonomy (
...
CREATE TABLE $nxtdb->term_relationships (
...
CREATE TABLE $nxtdb->commentmeta (
...
CREATE TABLE $nxtdb->posts (
  ...
  KEY post_author (post_author)
	...
	$users_single_table = "CREATE TABLE $nxtdb->users (
  ID bigint(20) unsigned NOT NULL auto_increment,
  ...
  KEY user_nicename (user_nicename)
	...
	$users_multi_table = "CREATE TABLE $nxtdb->users (
  ID bigint(20) unsigned NOT NULL auto_increment,
  user_login varchar(60) NOT NULL default '',
  user_pass varchar(64) NOT NULL default '',
  user_nicename varchar(50) NOT NULL default '',
  ...
  KEY user_nicename (user_nicename)
	...
	// usermeta
	$usermeta_table = "CREATE TABLE $nxtdb->usermeta (
  ...
  KEY meta_key (meta_key)
	...
	$ms_global_tables = "CREATE TABLE $nxtdb->blogs (
...
CREATE TABLE $nxtdb->blog_versions (
...
CREATE TABLE $nxtdb->registration_log (
...
CREATE TABLE $nxtdb->signups (
  ...
  KEY domain (domain)
	...
	if ( isset( $old_blog_id ) )
		$nxtdb->set_blog_id( $old_blog_id );

	return $queries;
}

// Populate for back compat.
$nxt_queries = nxt_get_db_schema( 'all' );
...
function populate_options() {
	...
	// 3.3
	if ( ! is_multisite() ) {
		$options['initial_db_version'] = ! empty( $nxt_current_db_version ) && $nxt_current_db_version < $nxt_db_version
			? $nxt_current_db_version : $nxt_db_version;
	}
...
/**
 * Execute NXTClass role creation for the various NXTClass versions.
 *
 * @since 2.0.0
 */
...
/**
 * Create the roles for NXTClass 2.0
 *
 * @since 2.0.0
 */
...
/**
 * Create and modify NXTClass roles for NXTClass 2.1.
 *
 * @since 2.1.0
 */
...
/**
 * Create and modify NXTClass roles for NXTClass 2.3.
 *
 * @since 2.3.0
 */
...
/**
 * Create and modify NXTClass roles for NXTClass 2.5.
 *
 * @since 2.5.0
 */
...
/**
 * Create and modify NXTClass roles for NXTClass 2.6.
 *
 * @since 2.6.0
 */
...
/**
 * Create and modify NXTClass roles for NXTClass 2.7.
 *
 * @since 2.7.0
 */
...
/**
 * Create and modify NXTClass roles for NXTClass 2.8.
 *
 * @since 2.8.0
 */
...
/**
 * Create and modify NXTClass roles for NXTClass 3.0.
 *
 * @since 3.0.0
 */
...
function install_network() {
	if ( ! defined( 'nxt_INSTALLING_NETWORK' ) )
		define( 'nxt_INSTALLING_NETWORK', true );

	dbDelta( nxt_get_db_schema( 'global' ) );
}
endif;
...
--The SITE_NAME Team' );
	...
	$sitemeta = array(
		...
		'nxtmu_upgrade_site' => $nxt_db_version,
