<?php
/**
 * A File upgrader class for NXTClass.
 *
...
/**
 * NXTClass Upgrader class for Upgrading/Installing a local set of files via the Filesystem Abstraction classes from a Zip file.
 *
...
class nxt_Upgrader {
	...
	function __construct($skin = null) {
		if ( null == $skin )
			$this->skin = new nxt_Upgrader_Skin();
		else
			$this->skin = $skin;
	}
	...
	function generic_strings() {
		...
		$this->strings['fs_no_root_dir'] = __('Unable to locate NXTClass Root directory.');
		$this->strings['fs_no_content_dir'] = __('Unable to locate NXTClass Content directory (nxt-content).');
		$this->strings['fs_no_plugins_dir'] = __('Unable to locate NXTClass Plugin directory.');
		$this->strings['fs_no_themes_dir'] = __('Unable to locate NXTClass Theme directory.');
	...
	function download_package($package) {
		...
		if ( empty($package) )
			return new nxt_Error('no_package', $this->strings['no_package']);

		$this->skin->feedback('downloading_package', $package);

		$download_file = download_url($package);

		if ( is_nxt_error($download_file) )
			return new nxt_Error('download_failed', $this->strings['download_failed'], $download_file->get_error_message());
	...
	function unpack_package($package, $delete_package = true) {
		...
		$upgrade_folder = $nxt_filesystem->nxt_content_dir() . 'upgrade/';

		//Clean up contents of upgrade directory beforehand.
		$upgrade_files = $nxt_filesystem->dirlist($upgrade_folder);
		if ( !empty($upgrade_files) ) {
			foreach ( $upgrade_files as $file )
				$nxt_filesystem->delete($upgrade_folder . $file['name'], true);
		}
		...
		if ( $nxt_filesystem->is_dir($working_dir) )
			$nxt_filesystem->delete($working_dir, true);
		...
		if ( is_nxt_error($result) ) {
			...
			if ( 'incompatible_archive' == $result->get_error_code() ) {
				return new nxt_Error( 'incompatible_archive', $this->strings['incompatible_archive'], $result->get_error_data() );
			}
			return $result;
		}
	...
	function install_package($args = array()) {
		...
		if ( empty($source) || empty($destination) )
			return new nxt_Error('bad_request', $this->strings['bad_request']);
		...
		$source_files = array_keys( $nxt_filesystem->dirlist($remote_source) );
		$remote_destination = $nxt_filesystem->find_folder($local_destination);

		//Locate which directory to copy to the new folder, This is based on the actual folder holding the files.
		if ( 1 == count($source_files) && $nxt_filesystem->is_dir( trailingslashit($source) . $source_files[0] . '/') ) //Only one folder? Then we want its contents.
			$source = trailingslashit($source) . trailingslashit($source_files[0]);
		elseif ( count($source_files) == 0 )
			return new nxt_Error( 'incompatible_archive', $this->strings['incompatible_archive'], __( 'The plugin contains no files.' ) ); //There are no files?
		else //Its only a single file, The upgrader will use the foldername of this file as the destination folder. foldername is based on zip filename.
			$source = trailingslashit($source);

		//Hook ability to change the source file location..
		$source = apply_filters('upgrader_source_selection', $source, $remote_source, $this);
		if ( is_nxt_error($source) )
			return $source;

		//Has the source location changed? If so, we need a new source_files list.
		if ( $source !== $remote_source )
			$source_files = array_keys( $nxt_filesystem->dirlist($source) );

		//Protection against deleting files in any important base directories.
		if ( in_array( $destination, array(ABSPATH, nxt_CONTENT_DIR, nxt_PLUGIN_DIR, nxt_CONTENT_DIR . '/themes') ) ) {
		...
		if ( $clear_destination ) {
			...
			$removed = true;
			if ( $nxt_filesystem->exists($remote_destination) )
				$removed = $nxt_filesystem->delete($remote_destination, true);
			$removed = apply_filters('upgrader_clear_destination', $removed, $local_destination, $remote_destination, $hook_extra);

			if ( is_nxt_error($removed) )
				return $removed;
			else if ( ! $removed )
				return new nxt_Error('remove_old_failed', $this->strings['remove_old_failed']);
		} elseif ( $nxt_filesystem->exists($remote_destination) ) {
			//If we're not clearing the destination folder and something exists there already, Bail.
			//But first check to see if there are actually any files in the folder.
			$_files = $nxt_filesystem->dirlist($remote_destination);
			if ( ! empty($_files) ) {
				$nxt_filesystem->delete($remote_source, true); //Clear out the source files.
				return new nxt_Error('folder_exists', $this->strings['folder_exists'], $remote_destination );
			}
		}

		//Create destination if needed
		if ( !$nxt_filesystem->exists($remote_destination) )
			if ( !$nxt_filesystem->mkdir($remote_destination, FS_CHMOD_DIR) )
				return new nxt_Error('mkdir_failed', $this->strings['mkdir_failed'], $remote_destination);

		// Copy new version of item into place.
		$result = copy_dir($source, $remote_destination);
		if ( is_nxt_error($result) ) {
			if ( $clear_working )
				$nxt_filesystem->delete($remote_source, true);
			return $result;
		}
	...
	function run($options) {
		...
		$options = nxt_parse_args($options, $defaults);
		extract($options);

		//Connect to the Filesystem first.
		$res = $this->fs_connect( array(nxt_CONTENT_DIR, $destination) );
	...
	function maintenance_mode($enable = false) {
		global $nxt_filesystem;
		$file = $nxt_filesystem->abspath() . '.maintenance';
		if ( $enable ) {
			$this->skin->feedback('maintenance_start');
			// Create maintenance file to signal that we are upgrading
			$maintenance_string = '<?php $upgrading = ' . time() . '; ?>';
			$nxt_filesystem->delete($file);
			$nxt_filesystem->put_contents($file, $maintenance_string, FS_CHMOD_FILE);
		} else if ( !$enable && $nxt_filesystem->exists($file) ) {
...
/**
 * Plugin Upgrader class for NXTClass Plugins, It is designed to upgrade/install plugins from a local zip, remote zip URL, or uploaded zip file.
 *
 * @TODO More Detailed docs, for methods as well.
 *
 * @package NXTClass
 ...
 * @since 2.8.0
 */
class Plugin_Upgrader extends nxt_Upgrader {
	...
	function install($package) {
		...
		if ( ! $this->result || is_nxt_error($this->result) )
			return $this->result;
	...
	function bulk_upgrade($plugins) {
		...
		// Connect to the Filesystem first.
		$res = $this->fs_connect( array(nxt_CONTENT_DIR, nxt_PLUGIN_DIR) );
		if ( ! $res ) {
			$this->skin->footer();
			return false;
		}
		...
		foreach ( $plugins as $plugin ) {
			$this->update_current++;
			$this->skin->plugin_info = get_plugin_data( nxt_PLUGIN_DIR . '/' . $plugin, false, true);
	...
	function check_package($source) {
		...
		$working_directory = str_replace( $nxt_filesystem->nxt_content_dir(), trailingslashit(nxt_CONTENT_DIR), $source);
		...
		if ( ! $plugins_found )
			return new nxt_Error( 'incompatible_archive', $this->strings['incompatible_archive'], __('No valid plugins were found.') );
	...
	function delete_old_plugin($removed, $local_destination, $remote_destination, $plugin) {
		...
		if ( ! $nxt_filesystem->exists($this_plugin_dir) ) //If its already vanished.
			return $removed;
		...
		if ( strpos($plugin, '/') && $this_plugin_dir != $plugins_dir ) //base check on if plugin includes directory separator AND that its not the root plugin folder
			...
			$deleted = $nxt_filesystem->delete($plugins_dir . $plugin);
...
/**
 * Theme Upgrader class for NXTClass Themes, It is designed to upgrade/install themes from a local zip, remote zip URL, or uploaded zip file.
 *
 * @TODO More Detailed docs, for methods as well.
 *
 * @package NXTClass
 ...
 * @since 2.8.0
 */
class Theme_Upgrader extends nxt_Upgrader {
	...
	function install($package) {
		...
		$options = array(
						'package' => $package,
						'destination' => nxt_CONTENT_DIR . '/themes',
						'clear_destination' => false, //Do not overwrite files.
						'clear_working' => true
						);
	...
	function bulk_upgrade($themes) {
		...
		// Connect to the Filesystem first.
		$res = $this->fs_connect( array(nxt_CONTENT_DIR) );
		if ( ! $res ) {
			$this->skin->footer();
			return false;
		}
	...
	function check_package($source) {
		...
		if ( ! file_exists( $working_directory . 'style.css' ) ) // A proper archive should have a style.css file in the single subdirectory
			return new nxt_Error( 'incompatible_archive', $this->strings['incompatible_archive'], __('The theme is missing the <code>style.css</code> stylesheet.') );

		$info = get_theme_data( $working_directory . 'style.css' );
		if ( empty($info['Name']) )
			return new nxt_Error( 'incompatible_archive', $this->strings['incompatible_archive'], __("The <code>style.css</code> stylesheet doesn't contain a valid theme header.") );
		...
		if ( empty($info['Template']) && ! file_exists( $working_directory . 'index.php' ) ) // If no template is set, it must have at least an index.php to be legit.
			return new nxt_Error( 'incompatible_archive', $this->strings['incompatible_archive'], __('The theme is missing the <code>index.php</code> file.') );
	...
	function delete_old_theme($removed, $local_destination, $remote_destination, $theme) {
		...
		if ( is_nxt_error($removed) || empty($theme) )
			return $removed; //Pass errors through.

		$themes_dir = $nxt_filesystem->nxt_themes_dir();
		if ( $nxt_filesystem->exists( trailingslashit($themes_dir) . $theme ) )
			if ( ! $nxt_filesystem->delete( trailingslashit($themes_dir) . $theme, true ) )
				return false;
		return true;
	}
	...
	function theme_info($theme = null) {
		...
		return get_theme_data(nxt_CONTENT_DIR . '/themes/' . $theme . '/style.css');
	}

}

/**
 * Core Upgrader class for NXTClass. It allows for NXTClass to upgrade itself in combination with the nxt-admin/includes/update-core.php file
 *
 * @TODO More Detailed docs, for methods as well.
 *
 * @package NXTClass
 ...
 * @since 2.8.0
 */
class Core_Upgrader extends nxt_Upgrader {

	function upgrade_strings() {
		$this->strings['up_to_date'] = __('NXTClass is at the latest version.');
		$this->strings['no_package'] = __('Update package not available.');
		$this->strings['downloading_package'] = __('Downloading update from <span class="code">%s</span>&#8230;');
		$this->strings['unpack_package'] = __('Unpacking the update&#8230;');
	...
	function upgrade($current) {
		global $nxt_filesystem, $nxt_version;
		...
		if ( !isset( $current->response ) || $current->response == 'latest' )
			return new nxt_Error('up_to_date', $this->strings['up_to_date']);

		$res = $this->fs_connect( array(ABSPATH, nxt_CONTENT_DIR) );
		if ( is_nxt_error($res) )
			return $res;

		$nxt_dir = trailingslashit($nxt_filesystem->abspath());
		...
		if ( $current->packages->partial && 'reinstall' != $current->response && $nxt_version == $current->partial_version )
			$to_download = 'partial';
		elseif ( $current->packages->new_bundled && version_compare( $nxt_version, $current->new_bundled, '<' )
		...
		if ( !$nxt_filesystem->copy($working_dir . '/nxtclass/nxt-admin/includes/update-core.php', $nxt_dir . 'nxt-admin/includes/update-core.php', true) ) {
			$nxt_filesystem->delete($working_dir, true);
			return new nxt_Error('copy_failed', $this->strings['copy_failed']);
		}
		$nxt_filesystem->chmod($nxt_dir . 'nxt-admin/includes/update-core.php', FS_CHMOD_FILE);

		require(ABSPATH . 'nxt-admin/includes/update-core.php');

		return update_core($working_dir, $nxt_dir);
	}

}

/**
 * Generic Skin for the NXTClass Upgrader classes. This skin is designed to be extended for specific purposes.
 *
 * @TODO More Detailed docs, for methods as well.
 *
 * @package NXTClass
...
class nxt_Upgrader_Skin {
	...
	function __construct($args = array()) {
		...
		$this->options = nxt_parse_args($args, $defaults);
	}
	...
	function request_filesystem_credentials($error = false) {
		...
		if ( !empty($this->options['nonce']) )
			$url = nxt_nonce_url($url, $this->options['nonce']);
	...
	function error($errors) {
		...
		} elseif ( is_nxt_error($errors) && $errors->get_error_code() ) {
...
/**
 * Plugin Upgrader Skin for NXTClass Plugin Upgrades.
 *
 * @TODO More Detailed docs, for methods as well.
 *
 * @package NXTClass
 ...
 * @since 2.8.0
 */
class Plugin_Upgrader_Skin extends nxt_Upgrader_Skin {
	var $plugin = '';
	...
	function after() {
		...
		if ( !empty($this->plugin) && !is_nxt_error($this->result) && $this->plugin_active ){
			show_message(__('Reactivating the plugin&#8230;'));
			echo '<iframe style="border:0;overflow:hidden" width="100%" height="170px" src="' . nxt_nonce_url('update.php?action=activate-plugin&networkwide=' . $this->plugin_network_active . '&plugin=' . $this->plugin, 'activate-plugin_' . $this->plugin) .'"></iframe>';
		}

		$update_actions =  array(
			'activate_plugin' => '<a href="' . nxt_nonce_url('plugins.php?action=activate&amp;plugin=' . $this->plugin, 'activate-plugin_' . $this->plugin) . '" title="' . esc_attr__('Activate this plugin') . '" target="_parent">' . __('Activate Plugin') . '</a>',
 ...
 * @since 3.0.0
 */
class Bulk_Upgrader_Skin extends nxt_Upgrader_Skin {
	var $in_loop = false;
	var $error = false;
	...
	function before($title = '') {
		$this->in_loop = true;
		printf( '<h4>' . $this->upgrader->strings['skin_before_update_header'] . ' <img alt="" src="' . admin_url( 'images/nxtspin_light.gif' ) . '" class="hidden waiting-' . $this->upgrader->update_current . '" style="vertical-align:middle;" /></h4>',  $title, $this->upgrader->update_current, $this->upgrader->update_count);
	...
	function after($title = '') {
		...
		if ( !empty($this->result) && !is_nxt_error($this->result) ) {
...
class Bulk_Plugin_Upgrader_Skin extends Bulk_Upgrader_Skin {
	...
	function bulk_footer() {
		...
		$update_actions =  array(
			'plugins_page' => '<a href="' . self_admin_url('plugins.php') . '" title="' . esc_attr__('Go to plugins page') . '" target="_parent">' . __('Return to Plugins page') . '</a>',
			'updates_page' => '<a href="' . self_admin_url('update-core.php') . '" title="' . esc_attr__('Go to NXTClass Updates page') . '" target="_parent">' . __('Return to NXTClass Updates') . '</a>'
		);
...
/**
 * Plugin Installer Skin for NXTClass Plugin Installer.
 *
 * @TODO More Detailed docs, for methods as well.
 *
 * @package NXTClass
 ...
 * @since 2.8.0
 */
class Plugin_Installer_Skin extends nxt_Upgrader_Skin {
	var $api;
	var $type;
	...
	function after() {
		...
		if ( 'import' == $from )
			$install_actions['activate_plugin'] = '<a href="' . nxt_nonce_url('plugins.php?action=activate&amp;from=import&amp;plugin=' . $plugin_file, 'activate-plugin_' . $plugin_file) . '" title="' . esc_attr__('Activate this plugin') . '" target="_parent">' . __('Activate Plugin &amp; Run Importer') . '</a>';
		else
			$install_actions['activate_plugin'] = '<a href="' . nxt_nonce_url('plugins.php?action=activate&amp;plugin=' . $plugin_file, 'activate-plugin_' . $plugin_file) . '" title="' . esc_attr__('Activate this plugin') . '" target="_parent">' . __('Activate Plugin') . '</a>';

		if ( is_multisite() && current_user_can( 'manage_network_plugins' ) ) {
			$install_actions['network_activate'] = '<a href="' . nxt_nonce_url('plugins.php?action=activate&amp;networkwide=1&amp;plugin=' . $plugin_file, 'activate-plugin_' . $plugin_file) . '" title="' . esc_attr__('Activate this plugin for all sites in this network') . '" target="_parent">' . __('Network Activate') . '</a>';
...
/**
 * Theme Installer Skin for the NXTClass Theme Installer.
 *
 * @TODO More Detailed docs, for methods as well.
 *
 * @package NXTClass
 ...
 * @since 2.8.0
 */
class Theme_Installer_Skin extends nxt_Upgrader_Skin {
	var $api;
	var $type;
	...
	function after() {
		...
		if ( is_network_admin() && current_user_can( 'manage_network_themes' ) )
			$install_actions['network_enable'] = '<a href="' . esc_url( nxt_nonce_url( 'themes.php?action=enable&amp;theme=' . $template, 'enable-theme_' . $template ) ) . '" title="' . esc_attr__( 'Enable this theme for all sites in this network' ) . '" target="_parent">' . __( 'Network Enable' ) . '</a>';
		...
		if ( ! $this->result || is_nxt_error($this->result) || is_network_admin() )
...
/**
 * Theme Upgrader Skin for NXTClass Theme Upgrades.
 *
 * @TODO More Detailed docs, for methods as well.
 *
 * @package NXTClass
 ...
 * @since 2.8.0
 */
class Theme_Upgrader_Skin extends nxt_Upgrader_Skin {
	var $theme = '';
