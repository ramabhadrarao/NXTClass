...
function check_upload_size( $file ) {
	...
	if ( $file['error'] != '0' && !isset($_POST['html-upload']) )
		nxt_die( $file['error'] . ' <a href="javascript:history.go(-1)">' . __( 'Back' ) . '</a>' );

	return $file;
}
add_filter( 'nxt_handle_upload_prefilter', 'check_upload_size' );
 ...
 * @return void
 */
function nxtmu_delete_blog( $blog_id, $drop = false ) {
	...
	if ( $drop ) {
		...
		$drop_tables = apply_filters( 'nxtmu_drop_tables', $nxtdb->tables( 'blog' ) );

		foreach ( (array) $drop_tables as $table ) {
			$nxtdb->query( "DROP TABLE IF EXISTS `$table`" );
		}

		$nxtdb->query( $nxtdb->prepare( "DELETE FROM $nxtdb->blogs WHERE blog_id = %d", $blog_id ) );
		$dir = apply_filters( 'nxtmu_delete_blog_upload_dir', nxt_CONTENT_DIR . "/blogs.dir/{$blog_id}/files/", $blog_id );
...
// @todo Merge with nxt_delete_user() ?
...
function nxtmu_get_blog_allowedthemes( $blog_id = 0 ) {
	$themes = get_themes();
...
###SITEURL### "), $new_admin_email );
	...
	nxt_mail( $value, sprintf( __( '[%s] New Admin Email Address' ), get_option( 'blogname' ) ), $content );
}
...
add_action( 'nxtmueditblogaction', 'upload_space_setting' );
...
function update_user_status( $id, $pref, $value, $deprecated = null ) {
	...
	$nxtdb->update( $nxtdb->users, array( $pref => $value ), array( 'ID' => $id ) );
...
function mu_dropdown_languages( $lang_files = array(), $current = '' ) {
	...
	if ( $flag === false ) // NXTClass english
...
/* Warn the admin if SECRET SALT information is missing from nxt-config.php */
...
function site_admin_notice() {
	...
	if ( !is_super_admin() )
		return false;
	if ( get_site_option( 'nxtmu_upgrade_site' ) != $nxt_db_version )
...
add_filter( 'nxt_insert_post_data', 'avoid_blog_page_permalink_collision', 10, 2 );
	...
	<?php if ( in_array( get_site_option( 'registration' ), array( 'all', 'blog' ) ) ) : ?>
		<tr>
			<th scope="row" colspan="2" class="th-full">
				<a href="<?php echo apply_filters( 'nxt_signup_location', network_home_url( 'nxt-signup.php' ) ); ?>"><?php _e( 'Create a New Site' ); ?></a>
			</th>
		</tr>
	<?php endif; ?>
	</table>
...
function ms_deprecated_blogs_file() {
	if ( ! is_super_admin() )
		return;
	if ( ! file_exists( nxt_CONTENT_DIR . '/blogs.php' ) )
		return;
	echo '<div class="update-nag">' . sprintf( __( 'The <code>%1$s</code> file is deprecated. Please remove it and update your server rewrite rules to use <code>%2$s</code> instead.' ), 'nxt-content/blogs.php', 'nxt-includes/ms-files.php' ) . '</div>';
}
...
//<![CDATA[
var tb_pathToImage = "../../nxt-includes/js/thickbox/loadingAnimation.gif";
var tb_closeImage = "../../nxt-includes/js/thickbox/tb-close.png";
//]]>
</script>
<?php
}
 ...
 * Plugins can alter this criteria  using the 'nxt_is_large_network' filter.
 *
 * @since 3.3.0
 * @param string $using 'sites or 'users'.  Default is 'sites'.
 * @return bool True if the network meets the criteria for large. False otherwise.
 */
function nxt_is_large_network( $using = 'sites' ) {
	if ( 'users' == $using ) {
		$count = get_user_count();
		return apply_filters( 'nxt_is_large_network', $count > 10000, 'users', $count );
	}

	$count = get_blog_count();
	return apply_filters( 'nxt_is_large_network', $count > 10000, 'sites', $count );
}
?>
