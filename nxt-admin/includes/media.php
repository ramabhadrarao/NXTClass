<?php
/**
 * NXTClass Administration Media API.
 *
 * @package NXTClass
...
function update_gallery_tab($tabs) {
	...
	if ( $post_id )
		$attachments = intval( $nxtdb->get_var( $nxtdb->prepare( "SELECT count(*) FROM $nxtdb->posts WHERE post_type = 'attachment' AND post_status != 'trash' AND post_parent = %d", $post_id ) ) );
...
function get_image_send_to_editor($id, $caption, $title, $align, $url='', $rel = false, $size='medium', $alt = '') {
	...
	$rel = $rel ? ' rel="attachment nxt-att-' . esc_attr($id).'"' : '';
 ...
 * @param array $overrides allows you to override the {@link nxt_handle_upload()} behavior
...
function media_handle_upload($file_id, $post_id, $post_data = array(), $overrides = array( 'test_form' => false )) {
	...
	$file = nxt_handle_upload($_FILES[$file_id], $overrides, $time);
...
?>
<title><?php bloginfo('name') ?> &rsaquo; <?php _e('Uploads'); ?> &#8212; <?php _e('NXTClass'); ?></title>
<?php
...
var userSettings = {'url':'<?php echo SITECOOKIEPATH; ?>','uid':'<?php if ( ! isset($current_user) ) $current_user = nxt_get_current_user(); echo $current_user->ID; ?>','time':'<?php echo time(); ?>'};
...
function nxt_media_upload_handler() {
	...
	if ( !empty($_POST['insertonlybutton']) ) {
		...
		if ( isset( $_POST['media_type'] ) && 'image' != $_POST['media_type'] ) {
			...
			$type = 'file';
			if ( ( $ext = preg_replace( '/^.+?\.([^.]+)$/', '$1', $src ) ) && ( $ext_type = nxt_ext2type( $ext ) )
 ...
 * @return string|nxt_Error Populated HTML img tag on success
 */
...
function media_upload_gallery() {
	...
	nxt_enqueue_script('admin-gallery');
	return nxt_iframe( 'media_upload_gallery_form', $errors );
}
...
function media_upload_library() {
	...
	return nxt_iframe( 'media_upload_library_form', $errors );
}
...
function image_link_input_fields($post, $url_type = '') {
	...
	$file = nxt_get_attachment_url($post->ID);
...
function image_attachment_fields_to_edit($form_fields, $post) {
	if ( substr($post->post_mime_type, 0, 5) == 'image' ) {
		$alt = get_post_meta($post->ID, '_nxt_attachment_image_alt', true);
		if ( empty($alt) )
			$alt = '';
...
function get_attachment_fields_to_edit($post, $errors = null) {
	...
	$form_fields = array(
		...
		'image_url'	=> array(
			...
			'value'      => nxt_get_attachment_url($post->ID),
...
function get_media_items( $post_id, $errors ) {
	...
	if ( $post_id ) {
		...
		if ( is_array($GLOBALS['nxt_the_query']->posts) )
			foreach ( $GLOBALS['nxt_the_query']->posts as $attachment )
...
$post_params = array(
		"post_id" => $post_id,
		"_nxtnonce" => nxt_create_nonce('media-form'),
		"type" => $_type,
		"tab" => $_tab,
		"short" => "1",
);
...
nxtUploaderInit = <?php echo json_encode($plupload_init); ?>;
</script>
...
<?php nxt_nonce_field('media-form'); ?>
...
<?php } ?>
	...
	getImageData : function() {
		...
		document.getElementById('status_img').innerHTML = '<img src="<?php echo esc_url( admin_url( 'images/nxtspin_light.gif' ) ); ?>" alt="" />';
...
<?php echo apply_filters( 'type_url_form_media', nxt_media_insert_url_form( $type ) ); ?>
</div>
</div>
</form>
<?php
...
<p class="ml-submit">
<input type="button" class="button" style="display:none;" onMouseDown="nxtgallery.update();" name="insert-gallery" id="insert-gallery" value="<?php esc_attr_e( 'Insert gallery' ); ?>" />
<input type="button" class="button" style="display:none;" onMouseDown="nxtgallery.update();" name="update-gallery" id="update-gallery" value="<?php esc_attr_e( 'Update gallery settings' ); ?>" />
</p>
</div>
</form>
<?php
...
$page_links = paginate_links( array(
	...
	'prev_text' => __('&laquo;'),
	'next_text' => __('&raquo;'),
	'total' => ceil($nxt_query->found_posts / 10),
	'current' => $_GET['paged']
));
 ...
 * @return unknown
 */
function nxt_media_insert_url_form( $default_view = 'image' ) {
...
function media_upload_html_bypass() {
	...
	<?php _e('You are using the browser&#8217;s built-in file uploader. The new NXTClass uploader includes multiple file selection and drag and drop capability. <a href="#">Switch to the new uploader</a>.'); ?>
	</p>
	<?php
}
...
add_action( 'media_upload_image', 'nxt_media_upload_handler' );
add_action( 'media_upload_audio', 'nxt_media_upload_handler' );
add_action( 'media_upload_video', 'nxt_media_upload_handler' );
add_action( 'media_upload_file',  'nxt_media_upload_handler' );
