 ...
 * @access private
 */
class nxt_Media_List_Table extends nxt_List_Table {
	...
	function prepare_items() {
		global $lost, $nxtdb, $nxt_query, $post_mime_types, $avail_post_mime_types;
		...
		list( $post_mime_types, $avail_post_mime_types ) = nxt_edit_attachments_query( $q );

 		$this->is_trash = isset( $_REQUEST['status'] ) && 'trash' == $_REQUEST['status'];

		$this->set_pagination_args( array(
			'total_items' => $nxt_query->found_posts,
			'total_pages' => $nxt_query->max_num_pages,
			'per_page' => $nxt_query->query_vars['posts_per_page'],
		) );
	}
