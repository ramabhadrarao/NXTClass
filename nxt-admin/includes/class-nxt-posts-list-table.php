 ...
 * @access private
 */
class nxt_Posts_List_Table extends nxt_List_Table {
	...
	function prepare_items() {
		global $post_type_object, $avail_post_stati, $nxt_query, $per_page, $mode;

		$avail_post_stati = nxt_edit_posts_query();

		$this->hierarchical_display = ( $post_type_object->hierarchical && 'menu_order title' == $nxt_query->query['orderby'] );

		$total_items = $this->hierarchical_display ? $nxt_query->post_count : $nxt_query->found_posts;
		...
		if ( $this->hierarchical_display )
			...
			$total_pages = $nxt_query->max_num_pages;
	...
	function get_views() {
		...
		$status_links = array();
		$num_posts = nxt_count_posts( $post_type, 'readable' );
		$class = '';
		$allposts = '';
...
<?php
	...
	function display_rows( $posts = array() ) {
		global $nxt_query, $post_type_object, $per_page;

		if ( empty( $posts ) )
			$posts = $nxt_query->posts;
		...
		if ( empty( $_REQUEST['s'] ) ) {
			...
			foreach ( $pages as $page ) {
				...
				if ( $page->post_parent == $page->ID ) {
					$page->post_parent = 0;
					$nxtdb->update( $nxtdb->posts, array( 'post_parent' => 0 ), array( 'ID' => $page->ID ) );
...
<?php
				...
				if ( current_user_can( $post_type_object->cap->delete_post, $post->ID ) ) {
					if ( 'trash' == $post->post_status )
						$actions['untrash'] = "<a title='" . esc_attr( __( 'Restore this item from the Trash' ) ) . "' href='" . nxt_nonce_url( admin_url( sprintf( $post_type_object->_edit_link . '&amp;action=untrash', $post->ID ) ), 'untrash-' . $post->post_type . '_' . $post->ID ) . "'>" . __( 'Restore' ) . "</a>";
	...
	<?php foreach ( $hierarchical_taxonomies as $taxonomy ) : ?>
			...
			<ul class="cat-checklist <?php echo esc_attr( $taxonomy->name )?>-checklist">
				<?php nxt_terms_checklist( null, array( 'taxonomy' => $taxonomy->name ) ) ?>
			</ul>
