...
if ( !current_user_can('upload_files') )
	nxt_die( __( 'You do not have permission to upload files.' ) );

$nxt_list_table = _get_list_table('nxt_Media_List_Table');
...
if ( $doaction ) {
	...
	if ( 'delete_all' == $doaction ) {
		$post_ids = $nxtdb->get_col( "SELECT ID FROM $nxtdb->posts WHERE post_type='attachment' AND post_status = 'trash'" );
		$doaction = 'delete';
	...
	switch ( $doaction ) {
		case 'find_detached':
			if ( !current_user_can('edit_posts') )
				nxt_die( __('You are not allowed to scan for lost attachments.') );

			$lost = $nxtdb->get_col( "
				SELECT ID FROM $nxtdb->posts
		...
		case 'attach':
			...
			if ( ! empty( $attach ) ) {
				...
				$attached = $nxtdb->query( $nxtdb->prepare( "UPDATE $nxtdb->posts SET post_parent = %d WHERE post_type = 'attachment' AND ID IN ( $attach )", $parent_id ) );
			}
		...
		case 'trash':
			...
			foreach ( (array) $post_ids as $post_id ) {
				if ( !current_user_can( 'delete_post', $post_id ) )
					nxt_die( __( 'You are not allowed to move this post to the trash.' ) );
		...
		case 'untrash':
			...
			foreach ( (array) $post_ids as $post_id ) {
				if ( !current_user_can( 'delete_post', $post_id ) )
					nxt_die( __( 'You are not allowed to move this post out of the trash.' ) );
		...
		case 'delete':
			...
			foreach ( (array) $post_ids as $post_id_del ) {
				if ( !current_user_can( 'delete_post', $post_id_del ) )
					nxt_die( __( 'You are not allowed to delete this post.' ) );

				if ( !nxt_delete_attachment( $post_id_del ) )
...
<?php $nxt_list_table->search_box( __( 'Search Media' ), 'media' ); ?>
