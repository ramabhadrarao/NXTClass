...
/** NXTClass Administration Widgets API */
require_once(ABSPATH . 'nxt-admin/includes/widgets.php');
...
function nxt_widgets_access_body_class($classes) {
	return "$classes widgets_access ";
}

if ( 'on' == $widgets_access )
	add_filter( 'admin_body_class', 'nxt_widgets_access_body_class' );
else
	nxt_enqueue_script('admin-widgets');
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __('<a href="http://codex.opensource.nxtclass.tk/Appearance_Widgets_Screen" target="_blank">Documentation on Widgets</a>') . '</p>' .
	'<p>' . __('<a href="http://opensource.nxtclass.tk/support/" target="_blank">Support Forums</a>') . '</p>'
);

if ( empty($nxt_registered_sidebars) ) {
...
?>
	...
	<h2><?php echo esc_html( $title ); ?></h2>
		...
		<p><?php _e( 'The theme you are currently using isn&#8217;t widget-aware, meaning that it has no sidebars that you are able to change. For information on making your theme widget-aware, please <a href="http://codex.opensource.nxtclass.tk/Widgetizing_Themes">follow these instructions</a>.' ); ?></p>
	</div>
...
if ( empty( $sidebars_widgets ) )
	$sidebars_widgets = nxt_get_widget_defaults();

foreach ( $sidebars_widgets as $sidebar_id => $widgets ) {
	if ( 'nxt_inactive_widgets' == $sidebar_id )
		continue;

	if ( !isset( $nxt_registered_sidebars[ $sidebar_id ] ) ) {
...
if ( isset($_POST['savewidget']) || isset($_POST['removewidget']) ) {
	...
	if ( isset($_POST['removewidget']) && $_POST['removewidget'] ) {
		...
		if ( !in_array($widget_id, $sidebar, true) ) {
			nxt_redirect( admin_url('widgets.php?error=0') );
			exit;
		}
...
if ( isset($_GET['editwidget']) && $_GET['editwidget'] ) {
		...
		if ( isset($_GET['base']) && isset($_GET['num']) ) { // multi-widget
			...
			foreach ( $nxt_registered_widget_controls as $control ) {
				if ( $_GET['base'] === $control['id_base'] ) {
					...
					$nxt_registered_widget_controls[$control['id']] = $control;
					break;
				}
			}
		}
	...
	if ( isset($nxt_registered_widget_controls[$widget_id]) && !isset($control) ) {
		$control = $nxt_registered_widget_controls[$widget_id];
		$control_callback = $control['callback'];
	} elseif ( !isset($nxt_registered_widget_controls[$widget_id]) && isset($nxt_registered_widgets[$widget_id]) ) {
		$name = esc_html( strip_tags($nxt_registered_widgets[$widget_id]['name']) );
	}
	...
	if ( !isset($sidebar) )
		$sidebar = isset($_GET['sidebar']) ? $_GET['sidebar'] : 'nxt_inactive_widgets';
...
<?php
	foreach ( $nxt_registered_sidebars as $sbname => $sbvalue ) {
		echo "\t\t<tr><td><label><input type='radio' name='sidebar' value='" . esc_attr($sbname) . "'" . checked( $sbname, $sidebar, false ) . " /> $sbvalue[name]</label></td><td>";
		if ( 'nxt_inactive_widgets' == $sbname || 'orphaned_widgets' == substr( $sbname, 0, 16 ) ) {
			echo '&nbsp;';
		} else {
...
<?php	nxt_nonce_field("save-delete-widget-$widget_id"); ?>
	<br class="clear" />
	</div>
	</form>
	</div>
...
<?php
foreach ( $nxt_registered_sidebars as $sidebar => $registered_sidebar ) {
		...
		<div class="<?php echo esc_attr( $wrap_class ); ?>">
			<div class="sidebar-name">
				...
				<h3><?php echo esc_html( $registered_sidebar['name'] ); ?>
					<span><img src="<?php echo esc_url( admin_url( 'images/nxtspin_light.gif' ) ); ?>" class="ajax-feedback" title="" alt="" /></span>
				</h3>
			</div>
			<div class="widget-holder inactive">
				<?php nxt_list_widget_controls( $registered_sidebar['id'] ); ?>
				<br class="clear" />
			</div>
		</div>
<?php
...
foreach ( $nxt_registered_sidebars as $sidebar => $registered_sidebar ) {
	...
	<div class="sidebar-name-arrow"><br /></div>
	<h3><?php echo esc_html( $registered_sidebar['name'] ); ?>
	<span><img src="<?php echo esc_url( admin_url( 'images/nxtspin_dark.gif' ) ); ?>" class="ajax-feedback" title="" alt="" /></span></h3></div>
	<?php nxt_list_widget_controls( $sidebar ); // Show the control forms for each of the widgets in this sidebar ?>
	</div>
<?php
	$i++;
} ?>
...
<?php nxt_nonce_field( 'save-sidebar-widgets', '_nxtnonce_widgets', false ); ?>
</form>
<br class="clear" />
</div>
