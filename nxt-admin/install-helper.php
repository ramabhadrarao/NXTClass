 ...
 * plugin installation. This file is not included by NXTClass and it is
 ...
 *
 * <code>
 * check_column('nxt_links', 'link_description', 'mediumtext');
 * if (check_column($nxtdb->comments, 'comment_author', 'tinytext'))
 *     echo "ok\n";
 *
 * $error_count = 0;
 * $tablename = $nxtdb->links;
 * // check the column
 * if (!check_column($nxtdb->links, 'link_description', 'varchar(255)')) {
 *     $ddl = "ALTER TABLE $nxtdb->links MODIFY COLUMN link_description varchar(255) NOT NULL DEFAULT '' ";
 *     $q = $nxtdb->query($ddl);
 * }
 *
 * if (check_column($nxtdb->links, 'link_description', 'varchar(255)')) {
...
function maybe_create_table($table_name, $create_ddl) {
	global $nxtdb;
	foreach ($nxtdb->get_col("SHOW TABLES",0) as $table ) {
		if ($table == $table_name) {
			return true;
		}
	}
	//didn't find it try to create it.
	$nxtdb->query($create_ddl);
...
function check_column($table_name, $col_name, $col_type, $is_null = null, $key = null, $default = null, $extra = null) {
	global $nxtdb, $debug;
	$diffs = 0;
	$results = $nxtdb->get_results("DESC $table_name");
