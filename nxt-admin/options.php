...
nxt_reset_vars(array('action', 'option_page'));
...
if ( is_multisite() ) {
	if ( ! empty($_GET[ 'adminhash' ] ) ) {
		...
		nxt_redirect( admin_url( $redirect ) );
		exit;
	} elseif ( ! empty( $_GET['dismiss'] ) && 'new_admin_email' == $_GET['dismiss'] ) {
		delete_option( 'adminhash' );
		delete_option( 'new_admin_email' );
		nxt_redirect( admin_url( 'options-general.php?updated=true' ) );
		exit;
	}
}
...
if ( !is_multisite() ) {
	...
	$whitelist_options['general'][] = 'nxtLANG';
...
if ( 'update' == $action ) {
	...
	if ( !isset( $whitelist_options[ $option_page ] ) )
		nxt_die( __( '<strong>ERROR</strong>: options page not found.' ) );

	if ( 'options' == $option_page ) {
		if ( is_multisite() && ! is_super_admin() )
			nxt_die( __( 'You do not have sufficient permissions to modify unregistered settings for this site.' ) );
	 ...
	 */
	$goback = add_query_arg( 'settings-updated', 'true',  nxt_get_referer() );
	nxt_redirect( $goback );
	exit;
}
...
<?php screen_icon(); ?>
  ...
  <?php nxt_nonce_field('options-options') ?>
  <input type="hidden" name="action" value="update" />
  <input type='hidden' name='option_page' value='options' />
  <table class="form-table">
<?php
$options = $nxtdb->get_results( "SELECT * FROM $nxtdb->options ORDER BY option_name" );
