...
class Custom_Image_Header {
	...
	function help() {
		...
		get_current_screen()->set_help_sidebar(
			...
			'<p>' . __( '<a href="http://codex.opensource.nxtclass.tk/Appearance_Header_Screen" target="_blank">Documentation on Custom Header</a>' ) . '</p>' .
	...
	function js_includes() {
		...
		if ( ( 1 == $step || 3 == $step ) && $this->header_text() )
			nxt_enqueue_script('farbtastic');
		elseif ( 2 == $step )
			nxt_enqueue_script('imgareaselect');
	}
	...
	function take_action() {
		...
		if ( isset( $_POST['resetheader'] ) ) {
			check_admin_referer( 'custom-header-options', '_nxtnonce-custom-header-options' );
	...
	function process_default_headers() {
		...
		if ( !isset($_nxt_default_headers) )
			return;

		$this->default_headers = $_nxt_default_headers;
...
<td>
	...
	<p>
		...
		<?php nxt_nonce_field( 'custom-header-upload', '_nxtnonce-custom-header-upload' ) ?>
...
nxt_nonce_field( 'custom-header-options', '_nxtnonce-custom-header-options' ); ?>
...
<?php }
	 ...
	 */
	function step_2() {
		check_admin_referer('custom-header-upload', '_nxtnonce-custom-header-upload');
		...
		if ( isset($file['error']) )
			nxt_die( $file['error'],  __( 'Image Upload Error' ) );
		...
		if ( $width == HEADER_IMAGE_WIDTH && $height == HEADER_IMAGE_HEIGHT ) {
			...
			update_post_meta( $id, '_nxt_attachment_is_custom_header', get_option('stylesheet' ) );

			set_theme_mod('header_image', esc_url($url));
			do_action('nxt_create_file_in_uploads', $file, $id); // For replication
			return $this->finished();
		} elseif ( $width > HEADER_IMAGE_WIDTH ) {
			$oitar = $width / HEADER_IMAGE_WIDTH;
			$image = nxt_crop_image($file, 0, 0, $width, $height, HEADER_IMAGE_WIDTH, $height / $oitar, false, str_replace(basename($file), 'midsize-'.basename($file), $file));
			if ( is_nxt_error( $image ) )
				nxt_die( __( 'Image could not be processed.  Please go back and try again.' ), __( 'Image Processing Error' ) );

			$image = apply_filters('nxt_create_file_in_uploads', $image, $id); // For replication
...
<form method="post" action="<?php echo esc_attr(add_query_arg('step', 3)); ?>">
	...
	<?php nxt_nonce_field( 'custom-header-crop-image' ) ?>
	...
	function step_3() {
		...
		$cropped = apply_filters('nxt_create_file_in_uploads', $cropped, $attachment_id); // For replication
	...
	function admin_page() {
		if ( ! current_user_can('edit_theme_options') )
			nxt_die(__('You do not have permission to customize headers.'));
		$step = $this->step();
		if ( 1 == $step )
			$this->step_1();
		elseif ( 2 == $step )
