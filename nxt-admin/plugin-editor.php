...
if ( is_multisite() && ! is_network_admin() ) {
	nxt_redirect( network_admin_url( 'plugin-editor.php' ) );
	exit();
}

if ( !current_user_can('edit_plugins') )
	nxt_die( __('You do not have sufficient permissions to edit plugins for this site.') );

$title = __("Edit Plugins");
$parent_file = 'plugins.php';

nxt_reset_vars(array('action', 'redirect', 'profile', 'error', 'warning', 'a', 'file', 'plugin'));

$plugins = get_plugins();

if ( empty($plugins) )
	nxt_die( __('There are no plugins installed on this site.') );
...
default:
	...
	if ( ! is_file($real_file) ) {
		nxt_die(sprintf('<p>%s</p>', __('No such file exists! Double check the name and try again.')));
	} else {
		...
		if ( preg_match('/\.([^.]+)$/', $real_file, $matches) ) {
			...
			if ( !in_array( $ext, $editable_extensions) )
				nxt_die(sprintf('<p>%s</p>', __('Files of this type are not editable.')));
		}
	}
	...
	update_recently_edited(nxt_PLUGIN_DIR . '/' . $file);

	$content = file_get_contents( $real_file );

	if ( '.php' == substr( $real_file, strrpos( $real_file, '.' ) ) ) {
		$functions = nxt_doc_link_parse( $content );
...
<?php elseif (isset($_GET['phperror'])) : ?>
 <div id="message" class="updated"><p><?php _e('This plugin has been deactivated because your changes resulted in a <strong>fatal error</strong>.') ?></p>
	...
	<iframe style="border:0" width="100%" height="70px" src="<?php bloginfo('nxturl'); ?>/nxt-admin/plugins.php?action=error_scrape&amp;plugin=<?php echo esc_attr($file); ?>&amp;_nxtnonce=<?php echo esc_attr($_GET['_error_nonce']); ?>"></iframe>
	<?php } ?>
</div>
<?php endif; ?>
<div class="wrap">
...
<form name="template" id="template" action="plugin-editor.php" method="post">
	<?php nxt_nonce_field('edit-plugin_' . $file) ?>
