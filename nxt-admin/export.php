...
get_current_screen()->add_help_tab( array(
	...
	'content' => '<p>' . __('You can export a file of your site&#8217;s content in order to import it into another installation or platform. The export file will be an XML file format called WXR. Posts, pages, comments, custom fields, categories, and tags can be included. You can choose for the WXR file to include only certain posts or pages by setting the dropdown filters to  limit the export by category, author, date range by month, or publishing status.') . '</p>' .
		'<p>' . __('Once generated, your WXR file can be imported by another NXTClass site or by another blogging platform able to access this format.') . '</p>',
) );

get_current_screen()->set_help_sidebar(
	'<p><strong>' . __('For more information:') . '</strong></p>' .
	'<p>' . __('<a href="http://codex.opensource.nxtclass.tk/Tools_Export_Screen" target="_blank">Documentation on Export</a>') . '</p>' .
...
function export_date_options() {
	...
	foreach ( $months as $date ) {
		...
		echo '<option value="' . $date->year . '-' . $month . '">' . $nxt_locale->get_month( $month ) . ' ' . $date->year . '</option>';
	}
}
?>
...
<p><?php _e('When you click the button below NXTClass will create an XML file for you to save to your computer.'); ?></p>
<p><?php _e('This format, which we call NXTClass eXtended RSS or WXR, will contain your posts, pages, comments, custom fields, categories, and tags.'); ?></p>
<p><?php _e('Once you&#8217;ve saved the download file, you can use the Import function in another NXTClass installation to import the content from this site.'); ?></p>
...
<ul id="post-filters" class="export-filters">
	<li>
		...
		<?php nxt_dropdown_categories( array( 'show_option_all' => __('All') ) ); ?>
	</li>
	<li>
