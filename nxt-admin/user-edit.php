...
nxt_reset_vars(array('action', 'redirect', 'profile', 'user_id', 'nxt_http_referer'));
...
$profile_help = '<p>' . __('Your profile contains information about you (your &#8220;account&#8221;) as well as some personal options related to using NXTClass.') . '</p>' .
	'<p>' . __('You can change your password, turn on keyboard shortcuts, change the color scheme of your NXTClass administration screens, and turn off the WYSIWYG (Visual) editor, among other things. You can hide the Toolbar (formerly called the Admin Bar) from the front end of your site, however it cannot be disabled on the admin screens.') . '</p>' .
...
get_current_screen()->set_help_sidebar(
    ...
    '<p>' . __('<a href="http://codex.opensource.nxtclass.tk/Users_Your_Profile_Screen" target="_blank">Documentation on User Profiles</a>') . '</p>' .
    '<p>' . __('<a href="http://opensource.nxtclass.tk/support/" target="_blank">Support Forums</a>') . '</p>'
);

$nxt_http_referer = remove_query_arg(array('update', 'delete_count'), stripslashes($nxt_http_referer));
...
if ( !is_multisite() ) {
	...
	if ( $user->user_login && isset( $_POST[ 'email' ] ) && is_email( $_POST[ 'email' ] ) && $nxtdb->get_var( $nxtdb->prepare( "SELECT user_login FROM {$nxtdb->signups} WHERE user_login = %s", $user->user_login ) ) )
		$nxtdb->query( $nxtdb->prepare( "UPDATE {$nxtdb->signups} SET user_email = %s WHERE user_login = %s", $_POST[ 'email' ], $user_login ) );

	// nxtMU must delete the user from the current blog if nxt added him after editing.
	$delete_role = false;
	$blog_prefix = $nxtdb->get_blog_prefix();
	if ( $user_id != $current_user->ID ) {
		$cap = $nxtdb->get_var( "SELECT meta_value FROM {$nxtdb->usermeta} WHERE user_id = '{$user_id}' AND meta_key = '{$blog_prefix}capabilities' AND meta_value = 'a:0:{}'" );
...
if ( !is_nxt_error( $errors ) ) {
	...
	if ( $nxt_http_referer )
		$redirect = add_query_arg('nxt_http_referer', urlencode($nxt_http_referer), $redirect);
...
<div id="message" class="updated">
	...
	<?php endif; ?>
	<?php if ( $nxt_http_referer && !IS_PROFILE_PAGE ) : ?>
	<p><a href="<?php echo esc_url( $nxt_http_referer ); ?>"><?php _e('&larr; Back to Users'); ?></a></p>
	<?php endif; ?>
</div>
<?php endif; ?>
<?php if ( isset( $errors ) && is_nxt_error( $errors ) ) : ?>
...
<?php nxt_nonce_field('update-user_' . $user_id) ?>
<?php if ( $nxt_http_referer ) : ?>
	<input type="hidden" name="nxt_http_referer" value="<?php echo esc_url($nxt_http_referer); ?>" />
<?php endif; ?>
<p>
...
<?php endif; ?>
<?php if ( count($_nxt_admin_css_colors) > 1 && has_action('admin_color_scheme_picker') ) : ?>
<tr>
<th scope="row"><?php _e('Admin Color Scheme')?></th>
<td><?php do_action( 'admin_color_scheme_picker' ); ?></td>
</tr>
<?php
endif; // $_nxt_admin_css_colors
if ( !( IS_PROFILE_PAGE && !$user_can_edit ) ) : ?>
<tr>
<th scope="row"><?php _e( 'Keyboard Shortcuts' ); ?></th>
<td><label for="comment_shortcuts"><input type="checkbox" name="comment_shortcuts" id="comment_shortcuts" value="true" <?php if ( !empty($profileuser->comment_shortcuts) ) checked('true', $profileuser->comment_shortcuts); ?> /> <?php _e('Enable keyboard shortcuts for comment moderation.'); ?></label> <?php _e('<a href="http://codex.opensource.nxtclass.tk/Keyboard_Shortcuts" target="_blank">More information</a>'); ?></td>
</tr>
<?php endif; ?>
...
<?php
	foreach (_nxt_get_user_contactmethods( $profileuser ) as $name => $desc) {
?>
<tr>
