...
$nxt_list_table = _get_list_table('nxt_Themes_List_Table');

if ( current_user_can( 'switch_themes' ) && isset($_GET['action'] ) ) {
	if ( 'activate' == $_GET['action'] ) {
		check_admin_referer('switch-theme_' . $_GET['template']);
		switch_theme($_GET['template'], $_GET['stylesheet']);
		nxt_redirect( admin_url('themes.php?activated=true') );
		exit;
		...
		nxt_redirect( admin_url('themes.php?deleted=true') );
		exit;
	}
}
...
$help_manage = '<p>' . __('Aside from the default theme included with your NXTClass installation, themes are designed and developed by third parties.') . '</p>' .
...
<?php elseif ( isset($_GET['activated']) ) :
		if ( isset($nxt_registered_sidebars) && count( (array) $nxt_registered_sidebars ) && current_user_can('edit_theme_options') ) { ?>
...
<?php if ( !empty( $_REQUEST['s'] ) || !empty( $_REQUEST['filter'] ) || $nxt_list_table->has_items() ) : ?>
		...
		<div class="feature-container">
			...
			<ol class="feature-group">
				<?php foreach ( $features as $key => $feature ) :
						...
						?>
				<li>
					<input type="checkbox" name="features[]" id="feature-id-<?php echo $key; ?>" value="<?php echo $key; ?>" <?php checked( in_array( $key, $nxt_list_table->features ) ); ?>/>
