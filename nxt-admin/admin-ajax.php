<?php
/**
 * NXTClass AJAX Process Execution.
 *
 * @package NXTClass
...
if ( ! is_user_logged_in() ) {
	...
	if ( isset( $_POST['action'] ) && $_POST['action'] == 'autosave' ) {
		...
		$message = sprintf( __('<strong>ALERT: You are logged out!</strong> Could not save draft. <a href="%s" target="_blank">Please log in again.</a>'), nxt_login_url() );
		$x = new nxt_Ajax_Response( array(
			'what' => 'autosave',
			'id' => $id,
			'data' => $message
		) );
	...
	if ( !empty( $_REQUEST['action'] ) )
		do_action( 'nxt_ajax_nopriv_' . $_REQUEST['action'] );
...
case 'fetch-list' :
	...
	define( 'nxt_NETWORK_ADMIN', $current_screen->is_network );
	define( 'nxt_USER_ADMIN', $current_screen->is_user );

	$nxt_list_table = _get_list_table( $list_class );
	if ( ! $nxt_list_table )
		die( '0' );

	if ( ! $nxt_list_table->ajax_user_can() )
		die( '-1' );

	$nxt_list_table->ajax_response();
...
case 'ajax-tag-search' :
	...
	$results = $nxtdb->get_col( $nxtdb->prepare( "SELECT t.name FROM $nxtdb->term_taxonomy AS tt INNER JOIN $nxtdb->terms AS t ON tt.term_id = t.term_id WHERE tt.taxonomy = %s AND t.name LIKE (%s)", $taxonomy, '%' . like_escape( $s ) . '%' ) );

	echo join( $results, "\n" );
	die;
	break;
case 'nxt-compression-test' :
...
case 'menu-quick-search':
	...
	break;
case 'oembed-cache' :
	$return = ( $nxt_embed->cache_oembed( $_GET['post'] ) ) ? '1' : '0';
	die( $return );
	break;
default :
	do_action( 'nxt_ajax_' . $_GET['action'] );
	die('0');
	break;
endswitch;
endif;
...
case 'delete-comment' : // On success, die with time() instead of 1
	...
	if ( $r ) // Decide if we need to send back '1' or a more complicated response including page links and comment counts
		_nxt_ajax_delete_comment_response( $comment->comment_ID, $delta );
	die( '0' );
	break;
case 'delete-tag' :
...
case 'dim-comment' : // On success, die with time() instead of 1
	...
	if ( !$comment = get_comment( $id ) ) {
		$x = new nxt_Ajax_Response( array(
			'what' => 'comment',
			'id' => new nxt_Error('invalid_comment', sprintf(__('Comment %d does not exist'), $id))
		) );
		$x->send();
	}
...
case 'add-link-category' : // On the Fly
	...
	foreach ( $names as $cat_name ) {
		...
		if ( !$cat_id = term_exists( $cat_name, 'link_category' ) ) {
			$cat_id = nxt_insert_term( $cat_name, 'link_category' );
		}
	...
	break;
case 'add-tag' :
	check_ajax_referer( 'add-tag', '_nxtnonce_add-tag' );
	...
	$tag = nxt_insert_term($_POST['tag-name'], $taxonomy, $_POST );

	if ( !$tag || is_nxt_error($tag) || (!$tag = get_term( $tag['term_id'], $taxonomy )) ) {
		...
		$x->add( array(
			'what' => 'taxonomy',
			'data' => new nxt_Error('error', $message )
		) );
		$x->send();
	}
	...
	if ( is_taxonomy_hierarchical($taxonomy) ) {
		...
		ob_start();
		$nxt_list_table->single_row( $tag, $level );
		$noparents = ob_get_clean();
	}

	ob_start();
	$nxt_list_table->single_row( $tag );
...
case 'get-comments' :
	...
	$x = new nxt_Ajax_Response();
	ob_start();
	foreach ( $nxt_list_table->items as $comment ) {
		if ( ! current_user_can( 'edit_comment', $comment->comment_ID ) )
			continue;
		get_comment( $comment );
...
case 'replyto-comment' :
	...
	$status = $nxtdb->get_var( $nxtdb->prepare("SELECT post_status FROM $nxtdb->posts WHERE ID = %d", $comment_post_ID) );
	...
	if ( $user->ID ) {
		$comment_author       = $nxtdb->escape($user->display_name);
		$comment_author_email = $nxtdb->escape($user->user_email);
		$comment_author_url   = $nxtdb->escape($user->user_url);
		$comment_content      = trim($_POST['content']);
		if ( current_user_can( 'unfiltered_html' ) ) {
			if ( nxt_create_nonce( 'unfiltered-html-comment' ) != $_POST['_nxt_unfiltered_html_comment'] ) {
...
case 'edit-comment' :
	...
	$nxt_list_table = _get_list_table( $checkbox ? 'nxt_Comments_List_Table' : 'nxt_Post_Comments_List_Table' );

	ob_start();
		$nxt_list_table->single_row( get_comment( $comment_id ) );
		$comment_list_item = ob_get_contents();
	ob_end_clean();
...
case 'add-user' :
	...
	$user_object = new nxt_User( $user_id );
	...
	$x = new nxt_Ajax_Response( array(
		'what' => 'user',
		'id' => $user_id,
		'data' => $nxt_list_table->single_row( $user_object, '', $user_object->roles[0] ),
		'supplemental' => array(
...
case 'menu-get-metabox' :
	...
	if ( isset( $_POST['item-type'] ) && 'post_type' == $_POST['item-type'] ) {
		$type = 'posttype';
		$callback = 'nxt_nav_menu_item_post_type_meta_box';
		$items = (array) get_post_types( array( 'show_in_nav_menus' => true ), 'object' );
	} elseif ( isset( $_POST['item-type'] ) && 'taxonomy' == $_POST['item-type'] ) {
		$type = 'taxonomy';
		$callback = 'nxt_nav_menu_item_taxonomy_meta_box';
...
case 'inline-save':
	...
	if ( $last = nxt_check_post_lock( $post_ID ) ) {
		$last_user = get_userdata( $last );
		$last_user_name = $last_user ? $last_user->display_name : __( 'Someone' );
	...
	$nxt_list_table = _get_list_table('nxt_Posts_List_Table');

	$mode = $_POST['post_view'];
	$nxt_list_table->display_rows( array( get_post( $_POST['post_ID'] ) ) );
...
case 'find_posts':
	...
	foreach ( (array) $search_terms as $term ) {
		...
		$search .= "{$searchand}(($nxtdb->posts.post_title LIKE '%{$term}%') OR ($nxtdb->posts.post_content LIKE '%{$term}%'))";
		$searchand = ' AND ';
	}
	$term = esc_sql( like_escape( $s ) );
	if ( count($search_terms) > 1 && $search_terms[0] != $s )
		$search .= " OR ($nxtdb->posts.post_title LIKE '%{$term}%') OR ($nxtdb->posts.post_content LIKE '%{$term}%')";

	$posts = $nxtdb->get_results( "SELECT ID, post_title, post_status, post_date FROM $nxtdb->posts WHERE post_type = '$what' AND post_status IN ('draft', 'publish') AND ($search) ORDER BY post_date_gmt DESC LIMIT 50" );
...
case 'widgets-order' :
	...
	if ( is_array($_POST['sidebars']) ) {
		...
		nxt_set_sidebars_widgets($sidebars);
		die('1');
	}
...
case 'save-widget' :
	...
	$sidebars = nxt_get_sidebars_widgets();
	$sidebar = isset($sidebars[$sidebar_id]) ? $sidebars[$sidebar_id] : array();

	// delete
	if ( isset($_POST['delete_widget']) && $_POST['delete_widget'] ) {

		if ( !isset($nxt_registered_widgets[$widget_id]) )
			die($error);
	...
	foreach ( (array) $nxt_registered_widget_updates as $name => $control ) {
	...
	if ( $form = $nxt_registered_widget_controls[$widget_id] )
...
case 'nxt-fullscreen-save-post' :
	...
	check_ajax_referer('update-' . $post_type . '_' . $post_id, '_nxtnonce');
...
case 'nxt-remove-post-lock' :
	...
	$new_lock = ( time() - apply_filters( 'nxt_check_post_lock_window', AUTOSAVE_INTERVAL * 2 ) + 5 ) . ':' . $active_lock[1];
	update_post_meta( $post_id, '_edit_lock', $new_lock, implode( ':', $active_lock ) );
	die( '1' );
case 'dismiss-nxt-pointer' :
...
//	check_ajax_referer( 'dismiss-pointer_' . $pointer );
	...
	$dismissed = array_filter( explode( ',', (string) get_user_meta( get_current_user_id(), 'dismissed_nxt_pointers', true ) ) );
	...
	update_user_meta( get_current_user_id(), 'dismissed_nxt_pointers', $dismissed );
	die( '1' );
	break;
default :
	do_action( 'nxt_ajax_' . $_POST['action'] );
	die('0');
	break;
endswitch;
?>
