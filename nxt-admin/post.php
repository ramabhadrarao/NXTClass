...
nxt_reset_vars(array('action', 'safe_mode', 'withcomments', 'posts', 'content', 'edited_post_title', 'comment_error', 'profile', 'trackback_url', 'excerpt', 'showcomments', 'commentstart', 'commentend', 'commentorder'));
...
if ( isset( $_POST['deletepost'] ) )
	$action = 'delete';
elseif ( isset($_POST['nxt-preview']) && 'dopreview' == $_POST['nxt-preview'] )
	$action = 'preview';

$sendback = nxt_get_referer();
if ( ! $sendback ||
...
case 'edit':
	...
	if ( 'trash' == $post->post_status )
		nxt_die( __('You can&#8217;t edit this item because it is in the Trash. Please restore it and try again.') );

	if ( null == $post_type_object )
		nxt_die( __('Unknown post type.') );
...
case 'trash':
	...
	nxt_redirect( add_query_arg( array('trashed' => 1, 'ids' => $post_id), $sendback ) );
	exit();
	break;
...
case 'untrash':
	...
	if ( !current_user_can($post_type_object->cap->delete_post, $post_id) )
		nxt_die( __('You are not allowed to move this item out of the Trash.') );
