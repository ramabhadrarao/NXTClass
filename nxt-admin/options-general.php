...
if ( ! is_multisite() ) {
	$options_help .= '<p>' . __('The NXTClass URL and the Site URL can be the same (example.com) or different; for example, having the NXTClass core files (example.com/nxtclass) in a subdirectory instead of the root directory.') . '</p>' .
...
<?php if ( !is_multisite() ) { ?>
<tr valign="top">
<th scope="row"><label for="siteurl"><?php _e('NXTClass Address (URL)') ?></label></th>
<td><input name="siteurl" type="text" id="siteurl" value="<?php form_option('siteurl'); ?>"<?php disabled( defined( 'nxt_SITEURL' ) ); ?> class="regular-text code<?php if ( defined( 'nxt_SITEURL' ) ) echo ' disabled' ?>" /></td>
</tr>
<tr valign="top">
...
<td>
<select name="default_role" id="default_role"><?php nxt_dropdown_roles( get_option('default_role') ); ?></select>
</td>
</tr>
<?php } else { ?>
<tr valign="top">
...
<?php echo nxt_timezone_choice($tzstring); ?>
</select>
...
<?php
	...
	checked( $custom );
	echo '/> ' . __('Custom:') . ' </label><input type="text" name="time_format_custom" value="' . esc_attr( get_option('time_format') ) . '" class="small-text" /> <span class="example"> ' . date_i18n( get_option('time_format') ) . "</span> <img class='ajax-loading' src='" . esc_url( admin_url( 'images/nxtspin_light.gif' ) ) . "' />\n";
	;
?>
	</fieldset>
</td>
...
for ($day_index = 0; $day_index <= 6; $day_index++) :
	...
	echo "\n\t<option value='" . esc_attr($day_index) . "' $selected>" . $nxt_locale->get_weekday($day_index) . '</option>';
endfor;
?>
</select></td>
</tr>
...
?>
	<tr valign="top">
		...
		<td>
			<select name="nxtLANG" id="nxtLANG">
				<?php mu_dropdown_languages( $languages, get_option('nxtLANG') ); ?>
			</select>
		</td>
	</tr>
<?php
