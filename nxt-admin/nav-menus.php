<?php
/**
 * NXTClass Administration for Navigation Menus
 * Interface functions
 *
 * @version 2.0.0
 *
...
if ( ! current_theme_supports( 'menus' ) && ! current_theme_supports( 'widgets' ) )
	nxt_die( __( 'Your theme does not support navigation menus or widgets.' ) );
...
switch ( $action ) {
		...
		if ( is_nav_menu_item( $menu_item_id ) ) {
			...
			if ( ! is_nxt_error( $menus ) && ! empty( $menus[0] ) ) {
				...
				if (
					...
					$next_item_data = (array) nxt_setup_nav_menu_item( get_post( $next_item_id ) );
	...
	case 'delete':
		...
		if ( is_nav_menu( $nav_menu_selected_id ) ) {
			$deleted_nav_menu = nxt_get_nav_menu_object( $nav_menu_selected_id );
			$delete_nav_menu = nxt_delete_nav_menu( $nav_menu_selected_id );

			if ( is_nxt_error($delete_nav_menu) ) {
				...
				$_nav_menus = nxt_get_nav_menus( array('orderby' => 'name') );
	...
	case 'update':
		...
		if ( 0 == $nav_menu_selected_id ) {
			...
			if ( $new_menu_title ) {
				$_nav_menu_selected_id = nxt_update_nav_menu_object( 0, array('menu-name' => $new_menu_title) );
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __('<a href="http://codex.opensource.nxtclass.tk/Appearance_Menus_Screen" target="_blank">Documentation on Menus</a>') . '</p>' .
...
<div class="wrap">
	...
	<div id="menu-settings-column" class="metabox-holder<?php if ( !$nav_menu_selected_id ) { echo ' metabox-holder-disabled'; } ?>">
		...
		<form id="nav-menu-meta" action="<?php echo admin_url( 'nav-menus.php' ); ?>" class="nav-menu-meta" method="post" enctype="multipart/form-data">
			...
			<?php nxt_nonce_field( 'add-menu_item', 'menu-settings-column-nonce' ); ?>
						...
						<div id="submitpost" class="submitbox">
							<div class="major-publishing-actions">
								...
								<div class="delete-action">
									<a class="submitdelete deletion menu-delete" href="<?php echo esc_url( nxt_nonce_url( admin_url('nav-menus.php?action=delete&amp;menu=' . $nav_menu_selected_id), 'delete-nav_menu-' . $nav_menu_selected_id ) ); ?>"><?php _e('Delete Menu'); ?></a>
