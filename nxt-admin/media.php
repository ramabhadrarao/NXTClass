...
case 'edit' :
	...
	if ( empty( $_GET['attachment_id'] ) ) {
		nxt_redirect( admin_url('upload.php') );
		exit();
	}
	...
	if ( empty($att->ID) ) nxt_die( __('You attempted to edit an attachment that doesn&#8217;t exist. Perhaps it was deleted?') );
	if ( $att->post_status == 'trash' ) nxt_die( __('You can&#8217;t edit this attachment because it is in the Trash. Please move it out of the Trash and try again.') );
	...
	'<p>' . __('<a href="http://codex.opensource.nxtclass.tk/Media_Add_New_Screen#Edit_Media" target="_blank">Documentation on Edit Media</a>') . '</p>' .
...
<?php nxt_original_referer_field(true, 'previous'); ?>
