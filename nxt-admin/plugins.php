...
if ( !current_user_can('activate_plugins') )
	nxt_die( __( 'You do not have sufficient permissions to manage plugins for this site.' ) );

$nxt_list_table = _get_list_table('nxt_Plugins_List_Table');
	...
	switch ( $action ) {
		case 'activate':
			...
			if ( isset($_GET['from']) && 'import' == $_GET['from'] ) {
				nxt_redirect( self_admin_url("import.php?import=" . str_replace('-importer', '', dirname($plugin))) ); // overrides the ?error=true one above and redirects to the Imports page, stripping the -importer suffix
			} else {
				nxt_redirect( self_admin_url("plugins.php?activate=true&plugin_status=$status&paged=$page&s=$s") ); // overrides the ?error=true one above
			}
			exit;
			break;
		...
		case 'network-activate-selected':
			...
			nxt_redirect( self_admin_url("plugins.php?activate-multi=true&plugin_status=$status&paged=$page&s=$s") );
			exit;
			break;
		...
		case 'deactivate':
			if ( ! current_user_can('activate_plugins') )
				nxt_die(__('You do not have sufficient permissions to deactivate plugins for this site.'));
			...
			if ( headers_sent() )
				...
				nxt_redirect( self_admin_url("plugins.php?deactivate=true&plugin_status=$status&paged=$page&s=$s") );
			exit;
			break;
		...
		case 'delete-selected':
			...
			<div class="wrap">
				<?php
					...
					foreach ( (array) $plugins as $plugin ) {
						if ( '.' == dirname($plugin) ) {
							$files_to_delete[] = nxt_PLUGIN_DIR . '/' . $plugin;
							if( $data = get_plugin_data(nxt_PLUGIN_DIR . '/' . $plugin) ) {
							...
							$files = list_files( nxt_PLUGIN_DIR . '/' . dirname($plugin) );
							if ( $files ) {
				...
				<div id="files-list" style="display:none;">
					...
					<?php
						foreach ( (array)$files_to_delete as $file )
							echo '<li>' . esc_html(str_replace(nxt_PLUGIN_DIR, '', $file)) . '</li>';
					?>
					</ul>
				</div>
			</div>
			...
			nxt_redirect( self_admin_url("plugins.php?deleted=true&plugin_status=$status&paged=$page&s=$s") );
			exit;
			break;
...
'title'		=> __('Overview'),
'content'	=>
	'<p>' . __('Plugins extend and expand the functionality of NXTClass. Once a plugin is installed, you may activate it or deactivate it here.') . '</p>' .
	'<p>' . sprintf(__('You can find additional plugins for your site by using the <a href="%1$s">Plugin Browser/Installer</a> functionality or by browsing the <a href="%2$s" target="_blank">NXTClass Plugin Directory</a> directly and installing new plugins manually. To manually install a plugin you generally just need to upload the plugin file into your <code>/nxt-content/plugins</code> directory. Once a plugin has been installed, you can activate it here.'), 'plugin-install.php', 'http://opensource.nxtclass.tk/extend/plugins/') . '</p>'
) );
get_current_screen()->add_help_tab( array(
'id'		=> 'compatibility-problems',
'title'		=> __('Troubleshooting'),
'content'	=>
	'<p>' . __('Most of the time, plugins play nicely with the core of NXTClass and with other plugins. Sometimes, though, a plugin&#8217;s code will get in the way of another plugin, causing compatibility issues. If your site starts doing strange things, this may be the problem. Try deactivating all your plugins and re-activating them in various combinations until you isolate which one(s) caused the issue.') . '</p>' .
	'<p>' . sprintf( __('If something goes wrong with a plugin and you can&#8217;t use NXTClass, delete or rename that file in the <code>%s</code> directory and it will be automatically deactivated.'), nxt_PLUGIN_DIR) . '</p>'
) );

get_current_screen()->set_help_sidebar(
	'<p><strong>' . __('For more information:') . '</strong></p>' .
	'<p>' . __('<a href="http://codex.opensource.nxtclass.tk/Managing_Plugins#Plugin_Management" target="_blank">Documentation on Managing Plugins</a>') . '</p>' .
...
<?php if ( isset($_GET['error']) ) :
	...
	<iframe style="border:0" width="100%" height="70px" src="<?php echo 'plugins.php?action=error_scrape&amp;plugin=' . esc_attr($plugin) . '&amp;_nxtnonce=' . esc_attr($_GET['_error_nonce']); ?>"></iframe>
	<?php
		}
	?>
	</div>
...
<?php $nxt_list_table->search_box( __( 'Search Installed Plugins' ), 'plugin' ); ?>
</form>
