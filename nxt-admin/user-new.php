...
%%s' ), get_bloginfo('name'), site_url(), esc_html( $_REQUEST[ 'role' ] ) );
	...
	add_filter( 'nxtmu_signup_user_notification_email', 'admin_created_user_email' );
...
if ( isset($_REQUEST['action']) && 'adduser' == $_REQUEST['action'] ) {
	...
	if ( false !== strpos($_REQUEST[ 'email' ], '@') ) {
		...
		if ( is_super_admin() ) {
			...
			nxt_redirect( add_query_arg( array('update' => 'enter_email'), 'user-new.php' ) );
			die();
		}
	}

	if ( !$user_details ) {
		nxt_redirect( add_query_arg( array('update' => 'does_not_exist'), 'user-new.php' ) );
		die();
	}
...
%4$s' );
	...
	if ( !is_multisite() ) {
		...
		if ( is_nxt_error( $user_details[ 'errors' ] ) && !empty( $user_details[ 'errors' ]->errors ) ) {
			...
			$new_user_login = apply_filters('pre_user_login', sanitize_user(stripslashes($_REQUEST['user_login']), true));
			if ( isset( $_POST[ 'noconfirmation' ] ) && is_super_admin() ) {
				add_filter( 'nxtmu_signup_user_notification', '__return_false' ); // Disable confirmation email
			}
...
get_current_screen()->set_help_sidebar(
    ...
    '<p>' . __('<a href="http://codex.opensource.nxtclass.tk/Users_Add_New_Screen" target="_blank">Documentation on Adding New Users</a>') . '</p>' .
...
<?php if ( isset($add_user_errors) && is_nxt_error( $add_user_errors ) ) : ?>
	<div class="error">
		<?php
...
<table class="form-table">
	...
	<tr class="form-field">
		...
		<td><select name="role" id="adduser-role">
			<?php nxt_dropdown_roles( get_option('default_role') ); ?>
			</select>
		</td>
	</tr>
...
<?php nxt_nonce_field( 'create-user', '_nxtnonce_create-user' ) ?>
<?php
