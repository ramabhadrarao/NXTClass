...
foreach ( array( 'p', 'attachment_id', 'page_id' ) as $_redirect ) {
	if ( ! empty( $_REQUEST[ $_redirect ] ) ) {
		nxt_redirect( admin_url( 'edit-comments.php?p=' . absint( $_REQUEST[ $_redirect ] ) ) );
		exit;
	}
}
unset( $_redirect );
...
if ( 'post' == $post_type ) {
	...
	'<p>' . __('<a href="http://codex.opensource.nxtclass.tk/Posts_Screen" target="_blank">Documentation on Managing Posts</a>') . '</p>' .
	...
	get_current_screen()->set_help_sidebar(
	'<p><strong>' . __('For more information:') . '</strong></p>' .
	'<p>' . __('<a href="http://codex.opensource.nxtclass.tk/Pages_Screen" target="_blank">Documentation on Managing Pages</a>') . '</p>' .
	'<p>' . __('<a href="http://opensource.nxtclass.tk/support/" target="_blank">Support Forums</a>') . '</p>'
	);
}
...
<?php $nxt_list_table->search_box( $post_type_object->labels->search_items, 'post' ); ?>
