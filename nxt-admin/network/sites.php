...
$nxt_list_table = _get_list_table('nxt_MS_Sites_List_Table');
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __('<a href="http://codex.opensource.nxtclass.tk/Network_Admin_Sites_Screens" target="_blank">Documentation on Site Management</a>') . '</p>' .
	...
	switch ( $_GET['action'] ) {
		...
		case 'deleteblog':
			...
			if ( $id != '0' && $id != $current_site->blog_id && current_user_can( 'delete_site', $id ) ) {
				nxtmu_delete_blog( $id, true );
				nxt_safe_redirect( add_query_arg( array( 'updated' => 'true', 'action' => 'delete' ), nxt_get_referer() ) );
			} else {
				nxt_safe_redirect( add_query_arg( array( 'updated' => 'true', 'action' => 'not_deleted' ), nxt_get_referer() ) );
			}
		...
		case 'unarchiveblog':
			...
			nxt_safe_redirect( add_query_arg( array( 'updated' => 'true', 'action' => 'unarchive' ), nxt_get_referer() ) );
			exit();
		break;
		...
		case 'activateblog':
			...
			nxt_safe_redirect( add_query_arg( array( 'updated' => 'true', 'action' => 'activate' ), nxt_get_referer() ) );
			exit();
		break;
		...
		case 'deactivateblog':
			...
			nxt_safe_redirect( add_query_arg( array( 'updated' => 'true', 'action' => 'deactivate' ), nxt_get_referer() ) );
			exit();
		break;
		...
		case 'unspamblog':
			...
			nxt_safe_redirect( add_query_arg( array( 'updated' => 'true', 'action' => 'unspam' ), nxt_get_referer() ) );
			exit();
		break;
		...
		case 'spamblog':
			...
			nxt_safe_redirect( add_query_arg( array( 'updated' => 'true', 'action' => 'spam' ), nxt_get_referer() ) );
			exit();
		break;
		...
		case 'confirm':
			...
			<html xmlns="http://www.w3.org/1999/xhtml" <?php if ( function_exists( 'language_attributes' ) ) language_attributes(); ?>>
				<head>
					<title><?php _e( 'NXTClass &rsaquo; Confirm your action' ); ?></title>
				...
				<body>
					<h1 id="logo"><img alt="NXTClass" src="<?php echo esc_attr( admin_url( 'images/nxtclass-logo.png' ) ); ?>" /></h1>
					<form action="sites.php?action=<?php echo esc_attr( $_GET['action2'] ) ?>" method="post">
						<input type="hidden" name="action" value="<?php echo esc_attr( $_GET['action2'] ) ?>" />
						<input type="hidden" name="id" value="<?php echo esc_attr( $id ); ?>" />
						<input type="hidden" name="_nxt_http_referer" value="<?php echo esc_attr( nxt_get_referer() ); ?>" />
						<?php nxt_nonce_field( $_GET['action2'], '_nxtnonce', false ); ?>
...
<?php $nxt_list_table->search_box( __( 'Search Sites' ), 'site' ); ?>
