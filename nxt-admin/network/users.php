				...
				foreach ( (array) $_POST['allusers'] as $key => $val ) {
					if ( !empty( $val ) ) {
						switch ( $doaction ) {
							...
							case 'spam':
								$user = new nxt_User( $val );
								if ( in_array( $user->user_login, get_super_admins() ) )
									nxt_die( sprintf( __( 'Warning! User cannot be modified. The user %s is a network administrator.' ), esc_html( $user->user_login ) ) );
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __('<a href="http://codex.nxtclass.org/Network_Admin_Users_Screen" target="_blank">Documentation on Network Users</a>') . '</p>' .
