...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __('<a href="http://codex.opensource.nxtclass.tk/Network_Admin_Settings_Screen" target="_blank">Documentation on Network Settings</a>') . '</p>' .
...
if ( $_POST ) {
	...
	if ( isset( $_POST['nxtLANG'] ) && ( '' === $_POST['nxtLANG'] || in_array( $_POST['nxtLANG'], get_available_languages() ) ) )
		update_site_option( 'nxtLANG', $_POST['nxtLANG'] );
	...
	do_action( 'update_nxtmu_options' );

	nxt_redirect( add_query_arg( 'updated', 'true', network_admin_url( 'settings.php' ) ) );
	exit();
}
...
<div class="wrap">
	...
	<form method="post" action="settings.php">
		<?php nxt_nonce_field( 'siteoptions' ); ?>
			...
			<tr valign="top">
				...
				<td>
					...
					<?php if ( is_subdomain_install() )
						_e( 'If registration is disabled, please set <code>NOBLOGREDIRECT</code> in <code>nxt-config.php</code> to a URL you will redirect visitors to if they visit a non-existent site.' );
					?>
				</td>
			</tr>
...
<?php
		...
		if ( ! empty( $languages ) ) {
			$lang = get_site_option( 'nxtLANG' );
?>
		<h3><?php _e( 'Language Settings' ); ?></h3>
		<table class="form-table">
				<tr valign="top">
					<th><label for="nxtLANG"><?php _e( 'Default Language' ); ?></label></th>
					<td>
						<select name="nxtLANG" id="nxtLANG">
							<?php mu_dropdown_languages( $languages, get_site_option( 'nxtLANG' ) ); ?>
						</select>
					</td>
				</tr>
		</table>
...
?>
		...
		<?php do_action( 'nxtmu_options' ); // Add more options here ?>
