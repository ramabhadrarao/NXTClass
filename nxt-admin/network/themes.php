...
if ( !current_user_can('manage_network_themes') )
	nxt_die( __( 'You do not have sufficient permissions to manage network themes.' ) );
...
$referer = remove_query_arg( $temp_args, nxt_get_referer() );
...
if ( $action ) {
	...
	switch ( $action ) {
		case 'enable':
			...
			nxt_redirect( network_admin_url( 'themes.php?enabled=1' ) );
			exit;
			break;
		case 'disable':
			...
			update_site_option( 'allowedthemes', $allowed_themes );
			nxt_safe_redirect( add_query_arg( 'disabled', '1', $referer ) );
			exit;
			break;
		case 'enable-selected':
			...
			update_site_option( 'allowedthemes', $allowed_themes );
			nxt_safe_redirect( add_query_arg( 'enabled', count( $themes ), $referer ) );
			exit;
			break;
		case 'disable-selected':
			...
			update_site_option( 'allowedthemes', $allowed_themes );
			nxt_safe_redirect( add_query_arg( 'disabled', count( $themes ), $referer ) );
			exit;
			break;
		...
		case 'delete-selected':
			...
			<div class="wrap">
				...
				<form method="post" action="<?php echo esc_url($_SERVER['REQUEST_URI']); ?>" style="display:inline;">
					...
					?>
					<?php nxt_nonce_field('bulk-themes') ?>
				...
				<div id="files-list" style="display:none;">
					...
					<?php
						foreach ( (array) $files_to_delete as $file )
							echo '<li>' . esc_html( str_replace( nxt_CONTENT_DIR . "/themes", '', $file) ) . '</li>';
					?>
					</ul>
				</div>
			</div>
			...
			nxt_redirect( network_admin_url( "themes.php?deleted=".count( $themes )."&paged=$paged&s=$s" ) );
			exit;
			break;
	}
}
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __('<a href="http://codex.opensource.nxtclass.tk/Network_Admin_Themes_Screen" target="_blank">Documentation on Network Themes</a>') . '</p>' .
...
<?php $nxt_list_table->search_box( __( 'Search Installed Themes' ), 'theme' ); ?>
</form>
