...
nxtWidgets = {
	...
	init : function() {
		...
		$('input.widget-control-save').live('click', function(){
			nxtWidgets.save( $(this).closest('div.widget'), 0, 1, 0 );
			return false;
		});

		$('a.widget-control-remove').live('click', function(){
			nxtWidgets.save( $(this).closest('div.widget'), 1, 1, 0 );
			return false;
		});

		$('a.widget-control-close').live('click', function(){
			nxtWidgets.close( $(this).closest('div.widget') );
			return false;
		});

		sidebars.children('.widget').each(function() {
			nxtWidgets.appendTitle(this);
		...
		sidebars.sortable({
			...
			stop: function(e,ui) {
				...
				if ( ui.item.hasClass('deleting') ) {
					nxtWidgets.save( ui.item, 1, 0, 1 ); // delete widget
					ui.item.remove();
					return;
				}
				...
				if ( add ) {
					...
					nxtWidgets.save( ui.item, 0, 0, 1 );
					ui.item.find('input.add_new').val('');
					ui.item.find('a.widget-action').click();
					return;
				}
				nxtWidgets.saveOrder(sb);
			},
	...
	saveOrder : function(sb) {
		...
		var a = {
			...
			savewidgets: $('#_nxtnonce_widgets').val(),
			sidebars: []
		};
...
$(document).ready(function($){ nxtWidgets.init(); });
