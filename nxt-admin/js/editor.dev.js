...
var switchEditors = {
	...
	go: function(id, mode) { // mode can be 'html', 'tmce', or 'toggle'
		...
		if ( 'tmce' == mode || 'tinymce' == mode ) {
			...
			if ( tinyMCEPreInit.mceInit[id] && tinyMCEPreInit.mceInit[id].nxtautop )
				txtarea_el.value = t.nxtautop( txtarea_el.value );
	...
	_nxt_Nop : function(content) {
		...
		content = content.replace(/<nxt_temp>/g, '\n');

		return content;
	},

	_nxt_Autop : function(pee) {
		...
		// put back the line breaks in pre|script
		pee = pee.replace(/<nxt_temp_br>/g, '\n');

		return pee;
	},

	pre_nxtautop : function(content) {
		var t = this, o = { o: t, data: content, unfiltered: content },
			q = typeof(jQuery) != 'undefined';

		if ( q )
			jQuery('body').trigger('beforePrenxtautop', [o]);
		o.data = t._nxt_Nop(o.data);
		if ( q )
			jQuery('body').trigger('afterPrenxtautop', [o]);

		return o.data;
	},

	nxtautop : function(pee) {
		var t = this, o = { o: t, data: pee, unfiltered: pee },
			q = typeof(jQuery) != 'undefined';

		if ( q )
			jQuery('body').trigger('beforenxtautop', [o]);
		o.data = t._nxt_Autop(o.data);
		if ( q )
			jQuery('body').trigger('afternxtautop', [o]);
