...
screenMeta = {
	...
	page:    null, // #nxtcontent

	init: function() {
		this.element = $('#screen-meta');
		this.toggles = $('.screen-meta-toggle a');
		this.page    = $('#nxtcontent');
...
$(document).ready( function() {
	...
	$('li.nxt-has-submenu', menu).hoverIntent({
		over: function(e){
			var b, h, o, f, m = $(this).find('.nxt-submenu'), menutop, wintop, maxtop;

			if ( !$(document.body).hasClass('folded') && $(this).hasClass('nxt-menu-open') )
				return;
			...
			h = $('#nxtwrap').height(); // Height of the entire page
			o = 60 + b - h;
			...
			menu.find('.nxt-submenu').removeClass('sub-open');
			m.addClass('sub-open');
		},
		out: function(){
			$(this).find('.nxt-submenu').removeClass('sub-open');
		},
		timeout: 200,
		sensitivity: 7,
		interval: 90
	...
	$('li.nxt-has-submenu > a.nxt-not-current-submenu', menu).bind('keydown.adminmenu', function(e){
		if ( e.which != 13 )
			return;
		...
		menu.find('.nxt-submenu').removeClass('sub-open');
		target.siblings('.nxt-submenu').toggleClass('sub-open').find('a[role="menuitem"]').each(refresh);
	}).each(refresh);
	...
	$('a[role="menuitem"]', menu).bind('keydown.adminmenu', function(e){
		...
		target.add( target.siblings() ).closest('.sub-open').removeClass('sub-open').siblings('a.nxt-not-current-submenu').focus();
	});
	...
	// tab in textareas
	$('#newcontent').bind('keydown.nxtevent_InsertTab', function(e) {
		if ( e.keyCode != 9 )
			return true;
	...
	$('#newcontent').bind('blur.nxtevent_InsertTab', function(e) {
...
// internal use
$(document).bind( 'nxt_CloseOnEscape', function( e, data ) {
