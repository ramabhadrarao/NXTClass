...
setCommentsList = function() {
	...
	delBefore = function( settings, list ) {
		...
		if ( action ) {
			...
			a = $('.undo a', '#undo-' + id);
			a.attr('href', 'comment.php?action=un' + action + 'comment&c=' + id + '&_nxtnonce=' + settings.data._ajax_nonce);
	...
	delAfter = function( r, settings ) {
		...
		theList.get(0).nxtList.add( theExtraList.children(':eq(0)').remove().clone() );
	...
	refillTheExtraList = function(ev) {
		...
		if (ev) {
			theExtraList.empty();
			args.number = Math.min(8, per_page); // see nxt_Comments_List_Table::prepare_items() @ class-nxt-comments-list-table.php
		} else {
			args.number = 1;
		...
		$.ajax({
			...
			success: function(response) {
				theExtraList.get(0).nxtList.add( response.rows );
			}
		});
	};

	theExtraList = $('#the-extra-comment-list').nxtList( { alt: '', delColor: 'none', addColor: 'none' } );
	theList = $('#the-comment-list').nxtList( { alt: '', delBefore: delBefore, dimAfter: dimAfter, delAfter: delAfter, addColor: 'none' } )
		.bind('nxtListDelEnd', function(e, s){
...
commentReply = {
	...
	show : function(xml) {
		...
		r = nxtAjax.parseAjaxResponse(xml);
		if ( r.errors ) {
			t.error({'responseText': nxtAjax.broken});
			return false;
		}
