 ...
 * Private use only!
 */
var PubSub, fullscreen, nxttitlehint;
...
(function($){
	...
	api.on = function() {
		...
		// Settings can be added or changed by defining "nxt_fullscreen_settings" JS object.
		// This can be done by defining it as PHP associative array, json encoding it and passing it to JS with:
		// nxt_add_script_before( 'nxt-fullscreen', 'nxt_fullscreen_settings = ' . $json_encoded_array . ';' );
		if ( typeof(nxt_fullscreen_settings) == 'object' )
			$.extend( s, nxt_fullscreen_settings );

		s.editor_id = nxtActiveEditor || 'content';
		...
		if ( !s.title_id ) {
			if ( $('input#title').length && s.editor_id == 'content' )
				...
				$('#nxt-fullscreen-title, #nxt-fullscreen-title-prompt-text').hide();
		}
	...
	api.save = function() {
		var hidden = $('#hiddenaction'), old = hidden.val(), spinner = $('#nxt-fullscreen-save img'),
			message = $('#nxt-fullscreen-save span');

		spinner.show();
		api.savecontent();

		hidden.val('nxt-fullscreen-save-post');
		...
		$.post( ajaxurl, $('form#post').serialize(), function(r){
			...
			if ( r.last_edited )
				$('#nxt-fullscreen-save input').attr( 'title',  r.last_edited );
	...
	api.savecontent = function() {
		...
		if ( s.title_id )
			$('#' + s.title_id).val( $('#nxt-fullscreen-title').val() );

		if ( s.mode === 'tinymce' && (ed = tinyMCE.get('nxt_mce_fullscreen')) ) {
			content = ed.save();
		} else {
			content = $('#nxt_mce_fullscreen').val();
		}

		$('#' + s.editor_id).val( content );
		$(document).triggerHandler('nxtcountwords', [ content ]);
	}
	...
	api.dfw_width = function(n) {
		var el = $('#nxt-fullscreen-wrap'), w = el.width();

		if ( !n ) { // reset to theme width
			el.width( $('#nxt-fullscreen-central-toolbar').width() );
	...
	ps.subscribe( 'showToolbar', function() {
		...
		$('#nxt-fullscreen-body').addClass('nxt-fullscreen-focus');
	...
	ps.subscribe( 'hideToolbar', function() {
		...
		$('#nxt-fullscreen-body').removeClass('nxt-fullscreen-focus');
	});
	...
	ps.subscribe( 'show', function() { // This event occurs before the overlay blocks the UI.
		...
		if ( s.title_id ) {
			title = $('#nxt-fullscreen-title').val( $('#' + s.title_id).val() );
			set_title_hint( title );
		}

		$('#nxt-fullscreen-save input').attr( 'title',  $('#last-edit').text() );

		s.textarea_obj.value = s.qt_canvas.value;

		if ( s.has_tinymce && s.mode === 'tinymce' )
			tinyMCE.execCommand('nxtFullScreenInit');
	...
	ps.subscribe( 'shown', function() { // This event occurs after the DFW overlay is shown
		...
		if ( s.has_tinymce && ! s.is_mce_on ) {
			...
			interim_init = function(mce, ed) {
				...
				if ( settings && settings.nxtautop && typeof(switchEditors) != 'undefined' )
					el.value = switchEditors.nxtautop( el.value );
		...
		nxtActiveEditor = 'nxt_mce_fullscreen';
	});
	...
	ps.subscribe( 'hide', function() { // This event occurs before the overlay blocks DFW.
		...
		if ( s.has_tinymce && s.mode === 'tinymce' )
			tinyMCE.execCommand('nxtFullScreenSave');
	...
	ps.subscribe( 'hidden', function() { // This event occurs after DFW is removed.
		s.visible = false;
		$('#nxt_mce_fullscreen, #nxt-fullscreen-title').removeAttr('style');

		if ( s.has_tinymce && s.is_mce_on )
			tinyMCE.execCommand('nxtFullScreenClose');

		s.textarea_obj.value = '';
		api.oldheight = 0;
		nxtActiveEditor = s.editor_id;
	});
	...
	ps.subscribe( 'switchMode', function( from, to ) {
		...
		ed = tinyMCE.get('nxt_mce_fullscreen');

		if ( from === 'html' && to === 'tinymce' ) {

			if ( tinyMCE.get(s.editor_id).getParam('nxtautop') && typeof(switchEditors) != 'undefined' )
				s.textarea_obj.value = switchEditors.nxtautop( s.textarea_obj.value );
	...
	api.medialib = function() {
		if ( s.has_tinymce && 'tinymce' === s.mode ) {
			tinyMCE.execCommand('nxt_Medialib');
		} else {
			var href = $('#nxt-' + s.editor_id + '-media-buttons a.thickbox').attr('href') || '';
	...
	api.refresh_buttons = function( fade ) {
		...
		if ( s.mode === 'html' ) {
			$('#nxt-fullscreen-mode-bar').removeClass('nxt-tmce-mode').addClass('nxt-html-mode');

			if ( fade )
				$('#nxt-fullscreen-button-bar').fadeOut( 150, function(){
					$(this).addClass('nxt-html-mode').fadeIn( 150 );
				});
			else
				$('#nxt-fullscreen-button-bar').addClass('nxt-html-mode');

		} else if ( s.mode === 'tinymce' ) {
			$('#nxt-fullscreen-mode-bar').removeClass('nxt-html-mode').addClass('nxt-tmce-mode');

			if ( fade )
				$('#nxt-fullscreen-button-bar').fadeOut( 150, function(){
					$(this).removeClass('nxt-html-mode').fadeIn( 150 );
				});
			else
				$('#nxt-fullscreen-button-bar').removeClass('nxt-html-mode');
		}
	}
	...
	api.ui = {
		init: function() {
			var topbar = $('#fullscreen-topbar'), txtarea = $('#nxt_mce_fullscreen'), last = 0;

			s.toolbars = topbar.add( $('#nxt-fullscreen-status') );
			s.element = $('#fullscreen-fader');
			s.textarea_obj = txtarea[0];
			s.has_tinymce = typeof(tinymce) != 'undefined';

			if ( !s.has_tinymce )
				$('#nxt-fullscreen-mode-bar').hide();

			if ( nxttitlehint && $('#nxt-fullscreen-title').length )
				nxttitlehint('nxt-fullscreen-title');
			...
			$(document).keyup(function(e){
				...
				if ( 27 == c ) { // Esc
					data = {
						...
						cb: fullscreen.off,
						condition: function(){
							if ( $('#TB_window').is(':visible') || $('.nxt-dialog').is(':visible') )
								return false;
							return true;
						}
					};

					if ( ! jQuery(document).triggerHandler( 'nxt_CloseOnEscape', [data] ) )
						fullscreen.off();
				}
			...
			if ( typeof(nxtWordCount) != 'undefined' ) {
				...
				txtarea.keyup( function(e) {
					...
					if ( 13 == k || 8 == last || 46 == last )
						$(document).triggerHandler('nxtcountwords', [ txtarea.val() ]);
