var tagBox, commentsBox, editPermalink, makeSlugeditClickable, nxtSetThumbnailHTML, nxtSetThumbnailID, nxtRemoveThumbnail, nxttitlehint;
	...
	get : function(total, num) {
		...
		$.post(ajaxurl, data,
			function(r) {
				...
				$('#the-comment-list').append('<tr><td colspan="2">'+nxtAjax.broken+'</td></tr>');
			}
		);
...
nxtSetThumbnailHTML = function(html){
	$('.inside', '#postimagediv').html(html);
};

nxtSetThumbnailID = function(id){
...
nxtRemoveThumbnail = function(nonce){
	$.post(ajaxurl, {
...
jQuery(document).ready( function($) {
	...
	$('.categorydiv').each( function(){
		...
		// TODO: move to jQuery 1.3+, support for multiple hierarchical taxonomies, see nxt-lists.dev.js
		...
		$('#' + taxonomy + '-add-toggle').click( function() {
			$('#' + taxonomy + '-adder').toggleClass( 'nxt-hidden-children' );
	...
	if ( $('#postcustom').length ) {
		$('#the-list').nxtList( { addAfter: function( xml, s ) {
	...
	if ( typeof(nxtWordCount) != 'undefined' ) {
		$(document).triggerHandler('nxtcountwords', [ co.val() ]);
