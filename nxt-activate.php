...
if ( is_object( $nxt_object_cache ) )
	$nxt_object_cache->cache_enabled = false;

do_action( 'activate_header' );

function do_activate_header() {
	do_action( 'activate_nxt_head' );
}
add_action( 'nxt_head', 'do_activate_header' );

function nxtmu_activate_stylesheet() {
	?>
	<style type="text/css">
		form { margin-top: 2em; }
...
add_action( 'nxt_head', 'nxtmu_activate_stylesheet' );
...
<div id="content" class="widecolumn">
	<?php if ( empty($_GET['key']) && empty($_POST['key']) ) { ?>
		...
		<form name="activateform" id="activateform" method="post" action="<?php echo network_site_url('nxt-activate.php'); ?>">
			<p>
	...
	<?php } else {
		...
		if ( is_nxt_error($result) ) {
			...
			<?php if ( $url != network_home_url('', 'http') ) : ?>
				<p class="view"><?php printf( __('Your account is now activated. <a href="%1$s">View your site</a> or <a href="%2$s">Log in</a>'), $url, $url . 'nxt-login.php' ); ?></p>
			<?php else: ?>
				<p class="view"><?php printf( __('Your account is now activated. <a href="%1$s">Log in</a> or go back to the <a href="%2$s">homepage</a>.' ), network_site_url('nxt-login.php', 'login'), network_home_url() ); ?></p>
			<?php endif;
		}
	}
	?>
