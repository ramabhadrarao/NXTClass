...
/** Sets up the NXTClass Environment. */
...
add_action( 'nxt_head', 'do_signup_header' );
...
if ( !is_main_site() ) {
	nxt_redirect( network_home_url( 'nxt-signup.php' ) );
	die();
}
...
function nxtmu_signup_stylesheet() {
	?>
	<style type="text/css">
...
add_action( 'nxt_head', 'nxtmu_signup_stylesheet' );
get_header();
...
function validate_user_form() {
	return nxtmu_validate_user_signup($_POST['user_name'], $_POST['user_email']);
}
...
function signup_another_blog($blogname = '', $blog_title = '', $errors = '') {
	...
	<form id="setupform" method="post" action="nxt-signup.php">
...
function confirm_another_blog_signup($domain, $path, $blog_title, $user_name, $user_email = '', $meta = '') {
	...
	<p>
		<?php printf( __( '<a href="http://%1$s">http://%2$s</a> is your new site.  <a href="%3$s">Log in</a> as &#8220;%4$s&#8221; using your existing password.' ), $domain.$path, $domain.$path, "http://" . $domain.$path . "nxt-login.php", $user_name ) ?>
	</p>
	<?php
...
$active_signup = apply_filters( 'nxtmu_active_signup', $active_signup ); // return "all", "none", "blog" or "user"
...
if ( $active_signup == 'none' ) {
	...
	if ( is_ssl() )
		...
		$proto = 'http://';
	$login_url = site_url( 'nxt-login.php?redirect_to=' . urlencode($proto . $_SERVER['HTTP_HOST'] . '/nxt-signup.php' ));
