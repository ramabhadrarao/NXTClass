<?php
/**
 * Atom Publishing Protocol support for NXTClass
 *
 * @version 1.0.5-dc
 */

/**
 * NXTClass is handling an Atom Publishing Protocol request.
 *
 * @var bool
 */
define('APP_REQUEST', true);

/** Set up NXTClass environment */
require_once('./nxt-load.php');

/** Atom Publishing Protocol Class */
require_once(ABSPATH . nxtINC . '/atomlib.php');

/** Admin Image API for metadata updating */
require_once(ABSPATH . '/nxt-admin/includes/image.php');

$_SERVER['PATH_INFO'] = preg_replace( '/.*\/nxt-app\.php/', '', $_SERVER['REQUEST_URI'] );
...
/**
 * NXTClass AtomPub API implementation.
 *
 * @package NXTClass
...
EOD;
	...
	function create_post() {
		...
		$nxt_cats = get_categories(array('hide_empty' => false));

		$post_category = array();

		foreach ( $nxt_cats as $cat ) {
		...
		$postID = nxt_insert_post($post_data);
		if ( is_nxt_error( $postID ) )
		...
		// could we clean up nxt_set_post_categories or cache to not print
		// this could affect our ability to send back the right headers
		@nxt_set_post_categories($postID, $post_category);
	...
	function create_attachment() {
		...
		$file = nxt_upload_bits( $slug, NULL, $bits);

		log_app('nxt_upload_bits returns:',print_r($file,true));

		$url = $file['url'];
		$file = $file['file'];

		do_action('nxt_create_file_in_uploads', $file); // replicate
		...
		// Save the data
		$postID = nxt_insert_attachment($attachment, $file);
	...
	function delete_attachment($postID) {
		...
		$location = get_post_meta($entry['ID'], '_nxt_attached_file', true);
		$filetype = nxt_check_filetype($location);
	...
	function put_file($postID) {
		...
		$result = nxt_update_post($post_data);

		if ( !$result )
			$this->internal_error(__('Sorry, your entry could not be posted. Something wrong happened.'));

		nxt_update_attachment_metadata( $postID, nxt_generate_attachment_metadata( $postID, $location ) );
	...
	function set_current_entry($postID) {
		...
		$entry = nxt_get_single_post($postID,ARRAY_A);
	...
	function get_feed($page = 1, $post_type = 'post') {
		global $post, $nxt, $nxt_query, $posts, $nxtdb, $blog_id;
		...
		nxt('posts_per_page=' . $count . '&offset=' . ($count * ($page-1) . '&orderby=modified'));

		$post = $GLOBALS['post'];
		$posts = $GLOBALS['posts'];
		$nxt = $GLOBALS['nxt'];
		$nxt_query = $GLOBALS['nxt_query'];
		$nxtdb = $GLOBALS['nxtdb'];
		$blog_id = (int) $GLOBALS['blog_id'];
		log_app('function',"query_posts(# " . print_r($nxt_query, true) . "#)");

		log_app('function',"total_count(# $nxt_query->max_num_pages #)");
		$last_page = $nxt_query->max_num_pages;
...
EOD;
	...
	function auth_required($msg) {
		...
		nocache_headers();
		header('WWW-Authenticate: Basic realm="NXTClass Atom Protocol"');
...
EOD;
	...
	function authenticate() {
		...
		if (isset($_SERVER['PHP_AUTH_USER']) && isset($_SERVER['PHP_AUTH_PW'])) {
			...
			$user = nxt_authenticate($_SERVER['PHP_AUTH_USER'], $_SERVER['PHP_AUTH_PW']);
	...
	function process_conditionals() {
		...
		switch($this->params[0]) {
			case $this->ENTRY_PATH:
				global $post;
				$post = nxt_get_single_post($this->params[1]);
				$nxt_last_modified = get_post_modified_time('D, d M Y H:i:s', true);
				$post = NULL;
				break;
		...
		@header("Last-Modified: $nxt_last_modified");
		@header("ETag: $nxt_etag");
