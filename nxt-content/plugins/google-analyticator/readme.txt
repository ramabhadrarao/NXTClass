...
Google Analyticator adds the necessary JavaScript code to enable Google Analytics logging on any NXTClass blog. This eliminates the need to edit your template code to begin logging. Google Analyticator also includes several widgets for displaying Analytics data in the admin and on your blog.
...
- Supports outbound link tracking of all links on the page, including links not managed by NXTClass
...
= 6.1.3 =
* Fixes a Javascript error on the NXTClass login page.
...
= 6.1.2 =
* Fixes deprecated warnings when nxt_debug is enabled.
...
* Cleans up the display of user role names for NXTClass versions below NXTClass 3.0.
...
* Adds two hooks, google_analyticator_extra_js_before and google_analyticator_extra_js_after, enabling other NXTClass plugins to insert additional tracking code.

= 6.1 =
* Prepares Google Analyticator for NXTClass 3.0 compatibility.
...
= 5.3.2 =
* Prepares Google Analyticator for NXTClass 2.9 compatibility.

= 5.3.1 =
* Corrects a fatal error on the settings page under NXTClass 2.7.
...
* Removes the Google authentication and widgets from NXTClass 2.7 due to compatibility issues. Users wishing to authenticate and use the widgets should upgrade to NXTClass 2.8.
...
* Updates the Google Analytics API class to use the NXTClass HTTP API, thus removing cURL as a core requirement for the widget.
...
= 4.3.2 =
* Adds support for NXTClass' new changelog readme.txt standard. Version information is now available from within the plugin updater.
...
= 4.2.1 =
* Fixes an issue where stable versions of NXTClass 2.8 were not using the new widget API.
* Changes SimplePie include to use NXTClass' version if possible, since SimplePie is included in NXTClass 2.8.
* Adds version number to the Google Analyticator comment.

= 4.2 =
* Adds support for the NXTClass 2.8 widget API.
...
* Corrects the widget image location for users with NXTClass installed in a sub-directory.
* Prevents Google API calls when widget information is not configured.
* Supports NXTClass 2.8.
...
* Adds a function to enable use of the widget for users not using NXTClass widgets.
...
= 2.22 =
* Improves comment author regex causing some issues in NXTClass 2.7. Thanks to jdub.

= 2.21 =
* Adds NXTClass 2.7 support.
...
= 2.11 =
* Adds an option to change what Google Analyticator considers a NXTClass administrator.

= 2.1 =
* Fixes a bug preventing options from being saved under NXTClass 2.5.
* Updates option page to comply with NXTClass 2.5 user interface changes.
* Note: Users of NXTClass 2.3 may wish to stay on 2.02 as the UI will look ‘weird’ under 2.3.
...
= 1.54 =
* Corrects problem where certain installation of NXTClass do not have the user level value.
...
* Note: Google Analyticator is now in a folder. If upgrading from a version less than 1.5, delete google-analyticator.php from your /nxt-content/plugins/ folder before proceeding.
...
= 1.3 =
* NXTClass 2.0 beta is now supported.
...
Recommended update. Highlights include NXTClass 3.0 support, updated async tracking code, dashboard stats by Analytics profile, more control over who gets tracked, and more control over who can see the dashboard widget. Settings have changed, so revisit the settings to verify.