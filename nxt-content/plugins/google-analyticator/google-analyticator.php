...
# Check if we have a version of NXTClass greater than 2.8
...
function ga_admin_init() {
	...
	load_plugin_textdomain('google-analyticator', 'nxt-content/plugins/' . $plugin_dir . '/localizations', $plugin_dir . '/localizations');
}

# Add the core Google Analytics script, with a high priority to ensure last script for async tracking
add_action('nxt_head', 'add_google_analytics', 999999);
...
function ga_options_page() {
	...
	?>
		...
		<form method="post" action="options-general.php?page=google-analyticator.php">
			<?php
			# Add a nonce
			nxt_nonce_field('google-analyticator-update_settings');
			?>
			...
			<table class="form-table" cellspacing="2" cellpadding="5" width="100%">
				...
				<?php
				# Check if we have a version of NXTClass greater than 2.8, and check if we have the memory to use the api
				if ( function_exists('register_widget') ) {
				?>
				<tr>
					<th width="30%" valign="top" style="padding-top: 10px;">
			...
			<h3><?php _e('Advanced Settings', 'google-analyticator'); ?></h3>
				...
				<tr>
					<th width="30%" valign="top" style="padding-top: 10px;">
						<label for="<?php echo key_ga_admin ?>"><?php _e('Track all logged in NXTClass users', 'google-analyticator'); ?>:</label>
					</th>
					<td>
						<?php
						...
						?>
						<p style="margin: 5px 10px;" class="setting-description"><?php _e('Selecting "no" to this option will prevent logged in NXTClass users from showing up on your Google Analytics reports. This setting will prevent yourself or other users from showing up in your Analytics reports. Use the next setting to determine what user groups to exclude.', 'google-analyticator'); ?></p>
					</td>
				</tr>
				<tr>
					<th width="30%" valign="top" style="padding-top: 10px;">
					...
					<td>
						...
						?>
						<p style="margin: 5px 10px;" class="setting-description"><?php _e('Specifies the user roles to not include in your NXTClass Analytics report. If a user is logged into NXTClass with one of these roles, they will not show up in your Analytics report.', 'google-analyticator'); ?></p>
					</td>
				</tr>
				<tr>
					<th width="30%" valign="top" style="padding-top: 10px;">
			...
			jQuery(document).ready(function(){
				...
				jQuery.ajax({
					...
					data: {
						...
						_ajax_nonce: '<?php echo nxt_create_nonce("google-analyticator-accounts_get"); ?>'<?php if ( isset($_GET['token']) ) { ?>,
...
add_action('nxt_ajax_ga_ajax_accounts', 'ga_ajax_accounts');
...
/**
 * Checks if the NXTClass API is a valid method for selecting an account
 *
	...
	if (  (get_option(key_ga_status) != ga_disabled ) && ( $uid != "XX-XXXXX-X" ) )
	{
		...
		if ( get_option(key_ga_outbound) == ga_enabled )
		{
			...
			if ( !is_admin() )
			{
				...
				if ( ( get_option(key_ga_admin) == ga_enabled || !ga_current_user_is(get_option(key_ga_admin_role)) ) && get_option(key_ga_admin_disable) == 'remove' || get_option(key_ga_admin_disable) != 'remove' )
				{
					add_action('nxt_print_scripts', 'ga_external_tracking_js');
				}
			}
		}
	}
...
{
	nxt_enqueue_script('ga-external-tracking', plugins_url('/google-analyticator/external-tracking.min.js'), array('jquery'), GOOGLE_ANALYTICATOR_VERSION);
}
...
function ga_current_user_is($roles)
{
	...
	$user = new nxt_User($user_id); // $user->roles
