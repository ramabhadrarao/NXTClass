<?php
require_once(nxtWIKI_FILE_PATH.'/lib/class_wikiparser.php');

class nxtW_WikiParser extends WikiParser {
	// 2011-01-02 arjen adjusted to deal with namespaces
    function wiki_link($topic,$namespace='') {
	    global $nxtdb;
	    $wiki = $nxtdb->get_var('SELECT `p`.`id` FROM `' . $nxtdb->posts . '` `p` WHERE `p`.`post_type` = "wiki" AND `p`.`post_name` = "' . strtolower(preg_replace('/[ -]+/', '-', $topic)) .'"');
    ...
    function handle_internallink($matches) {
        ...
        $title = preg_replace('/^.*?\:/','',$title);
        $wiki = $nxtdb->get_var('SELECT `p`.`id` FROM `' . $nxtdb->posts . '` `p` WHERE `p`.`post_type` = "wiki" AND `p`.`post_name` = "' . strtolower(preg_replace('/[ -]+/', '-', $href)) .'"');
