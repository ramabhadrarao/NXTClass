...
class WikiPageController {
	...
	function WikiPageController() {
		add_filter('nxt_insert_post_data',array($this,'save_code'), '99');
	...
	function table_of_contents($content) {
		...
		if ( !$toc || is_front_page() && !$nxtw_options['show_toc_onfrontpage'] ) {
			return $content;
		}
	...
	function scripts() {
	   ...
	   nxt_enqueue_script('nxtclass-wiki', PLUGIN_URL ."/".nxtWIKI_DIR_NAME."/static/nxtclass-wiki.js");
	...
	function set_query() {
		if(isset($_GET['revision'])) {
			...
			$warning = '<div id="nxtw_read_revision_warning">'.__('Currently working with revision').' @ '.$revision_data->post_modified.' '.__('by').' '.$revision_author->display_name.'. <a href="'.get_permalink($post->ID).'">'.__('Current version').'</a></div>';
	...
	function invoke_editor() {
		...
		if ( $this->WikiHelper->is_wiki('front_end_check') ) {
			...
			if ( !$this->WikiHelper->is_restricted() ) {
				...
				add_action('nxt_footer',array($this,'inline_editor'));
			} else {
				add_filter('the_content',array($this,'nxtw_nope') );
			}
		}
	}
	...
	function get_edit($content, $class = null ){
		global $post;
		return '<div id="nxtw_edit_div" '.$class.'>
					<form action="" method="post">
						<textarea name="nxtw_editor_content" style="width:100%;height:200px;" id="area1">'.$content.'</textarea>
						'.nxt_nonce_field('nxtw_edit_form').'
						<input type="submit" value="save" id="nxtw_save" />
						<input type="hidden" value="'.$post->ID.'" name="nxtw_id" />
					</form>
				</div>';
	}
	
	function get_history($content, $class = null){
		return '<div id="nxtw_view_history_div" '.$class.'>'.$this->post_revisions().'</div>';
	}
	...
	function front_end_interface($content) {
		...
		get_option('nxtw_options');
		
		$wiki_interface = array('content','edit','history');
		$return = "";
		$interface = "content";
		
		if ( in_array( @$_GET['nxtw_action'], $wiki_interface ) )
			$interface = $_GET['nxtw_action'];
		
		(isset($post->revision_warning)) ? $warning = $post->revision_warning : $warning = false;
		(isset($post->nxtw_post_saved)) ? $update = "Post updated!" : $update = false;
		
		foreach( $wiki_interface as $wiki ):
			
			if ( $interface != $wiki ):
				$class = 'class="nxtw-hide-it"';
			else:
				$class = null;
			endif;
		...
		return 
			...
			<div id="nxtw_tabs">
			<ul id="nxtw_tab_nav">
				<li><a id="nxtw_read" href="?nxtw_action=content">Read</a></li>
				<li><a id="nxtw_edit" href="?nxtw_action=edit">Edit</a></li>
				<li><a id="nxtw_view_history" href="?nxtw_action=history">View History</a></li>
			</ul>
			'.$warning
			 .$return.'
			</div>
	...
	function inline_editor() {
		...
		$nxtw_ajax_url = admin_url('admin-ajax.php');
		$get_content_to_save = '$("#area1").val()';
		$nicedit = false;
		
		$ajax_args = '	action : "ajax_save",
						nxtw_editor_content : '.$get_content_to_save.',
						nxtw_id : "'.$post->ID.'",
						_nxtnonce : $("#_nxtnonce").val()';
		
		if ( isset($nxtw_options['revision_pending']) && $nxtw_options['revision_pending'] == "true" )
			$ajax_args .= ', nxtw_revision_stack: "1"';
		...
		<script type="text/javascript">
			jQuery(document).ready(function($) {
				...
				$("#nxtw_save").click(function( e ) {
					e.preventDefault();
					data = {
						'.$ajax_args.'
					};
					$.post("'.$nxtw_ajax_url.'", data, function(results) {
						$("#nxtw_view_history_div").load(location.href+" #nxtw_view_history_div>*", function() {
							$("#nxtw_read_div").load(location.href+" #nxtw_read_div>*", function() {
								alert(results);
							});
						});
					});
				...
				$("#nxtw_tab_nav li a").each(function() {
					$(this).attr( "href", "#"+$(this).attr( "id" )+"_div" );
				});
				
				$(window).load(function() {
					$(".nxtw-hide-it").removeClass("nxtw-hide-it");
					$("#nxtw_tabs").tabs();
				});	
			});
		</script>
	...
	function no_js_save() {
		if ( isset( $_POST['nxtw_editor_content'] ) && nxt_verify_nonce( $_POST['_nxtnonce'], 'nxtw_edit_form' ) ):
			if ( $this->save_post() ):
				$post->nxtw_post_saved = true;
			endif;
		endif;
	}
	
	//Code shamelessly stolen from here: http://www.blogseye.com/2010/05/creating-fake-nxtclass-posts-on-the-fly/comment-page-1/#comment-253
	function fake_page() {
