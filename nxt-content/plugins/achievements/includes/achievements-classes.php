...
class DPA_Achievement {
	 ...
	 * @param DPA_Achievement $old_achievement A copy of the Achievement which is about to be saved, for comparision purposes
	 * @param nxt_Error $errors Holds any errors (by ref)
	 * @since 2.0
	 * @static
	 * @uses nxt_Error
	 */
	...
	function delete( $slug ) {
		...
		if ( !$id = $nxtdb->get_var( $nxtdb->prepare( "SELECT id FROM {$bp->achievements->table_achievements} WHERE slug = %s LIMIT 1", $slug ) ) )
			return false;

		$nxtdb->query( $nxtdb->prepare( "DELETE FROM {$bp->achievements->table_unlocked} WHERE achievement_id = %d", $id ) );
		$nxtdb->query( $nxtdb->prepare( "DELETE FROM {$bp->achievements->table_achievements} WHERE id = %d", $id ) );
	...
	function achievement_slug_exists( $slug ) {
		...
		if ( dpa_is_achievement_edit_page() || dpa_is_achievement_change_picture_page() )
			$result = $nxtdb->get_var( $nxtdb->prepare( "SELECT name FROM {$bp->achievements->table_achievements} WHERE slug = %s AND id != %d LIMIT 1", $slug, $bp->achievements->current_achievement->id ) );
		else
			$result = $nxtdb->get_var( $nxtdb->prepare( "SELECT name FROM {$bp->achievements->table_achievements} WHERE slug = %s LIMIT 1", $slug ) );
...
function dpa_points_get_high_scorers( $limit=5 ) {
	...
	if ( !$high_scorers = nxt_cache_get( 'dpa_high_scorers_' . $limit, 'dpa' ) ) {
		$high_scorers = $nxtdb->get_results( $nxtdb->prepare( "SELECT CAST(meta_value AS UNSIGNED INTEGER) as points, user_id as id FROM $nxtdb->usermeta WHERE meta_key = %s ORDER BY points DESC LIMIT %d", 'achievements_points', $limit ) );
		nxt_cache_set( 'dpa_high_scorers_' . $limit, $high_scorers, 'dpa' );
	}
...
function dpa_unlock_achievement( $user_id=0, $achievement=null ) {
	...
	$nxtdb->insert( $bp->achievements->table_unlocked, array( 'achievement_id' => $achievement->id, 'user_id' => $user_id, 'achieved_at' => bp_core_current_time() ), array( '%d', '%d', '%s' ) );
}
...
function dpa_get_total_achievements_count() {
	...
	if ( !$count = nxt_cache_get( 'dpa_get_total_achievements_count', 'dpa' ) ) {
		$admin_sql = '';

		if ( !dpa_permission_can_user_edit() )
			$admin_sql = $nxtdb->prepare( "WHERE is_active=1" );

		$count = $nxtdb->get_var( $nxtdb->prepare( "SELECT COUNT(id) FROM {$bp->achievements->table_achievements} {$admin_sql}" ) );
		nxt_cache_set( 'dpa_get_total_achievements_count', $count, 'dpa' );
	}
...
function dpa_get_total_achievement_unlocked_count( $achievement_id ) {
	...
	return $nxtdb->get_var( $nxtdb->prepare( "SELECT COUNT(u.id) FROM {$bp->achievements->table_achievements} as a, {$bp->achievements->table_unlocked} as u WHERE a.id = %d AND a.id = u.achievement_id", $achievement_id ) );
}
...
function dpa_delete_achievement_unlocks_for_user( $user_id ) {
	...
	$nxtdb->query( $nxtdb->prepare( "DELETE FROM {$bp->achievements->table_unlocked} WHERE user_id = %d", $user_id ) );
}
?>