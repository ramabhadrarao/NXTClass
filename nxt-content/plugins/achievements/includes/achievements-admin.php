<?php
/**
 * The nxt Admin area pages for Achievements.
 *
...
/**
 * Adds a notification message to the nxt-admin screens if the installed version of Achievements doesn't match
...
/**
 * Add link to settings screen on the nxt Admin 'plugins' page
 *
...
function dpa_admin_screen_validate( $input ) {
	...
	if ( is_string( $input ) )  // nxtmu-edit.php
...
/**
 * Tells nxt that we support two columns
 *
 * @param array $columns Column settings for NXTClass admin pages
 * @param string NXTClass' admin page "name"
 * @since 2.0
 */
...
function dpa_admin_screen() {
	...
	if ( !empty( $_POST['contact_body'] ) && !empty( $_POST['contact_type'] ) && !empty( $_POST['contact_email'] ) ) {
		...
		if ( $body && $type && $email && is_email( $email ) )
			$email_sent = nxt_mail( array( 'paul@byotos.com', $email ), "Achievements support request: " . $type, $body );
	}
?>
	<div id="bp-admin">
		<div id="dpa-admin-metaboxes-general" class="wrap">
			...
			<div id="bp-admin-nav">
				<ol>
					<li <?php if ( !$is_support_tab ) echo 'class="current"' ?>><a href="<?php echo site_url( 'nxt-admin/admin.php?page=' . $bp->achievements->id, 'admin') ?>"><?php _e( 'Configure', 'dpa' ) ?></a></li>
					<li <?php if ( $is_support_tab ) echo 'class="current"' ?>><a href="<?php echo site_url( 'nxt-admin/admin.php?page=' . $bp->achievements->id . '&amp;tab=' . DPA_SLUG_ADMIN_SUPPORT, 'admin')  ?>"><?php _e( 'Support', 'dpa' ) ?></a></li>
				</ol>
			</div>
			...
			<div class="dpa-spacer">
				<?php if ( !$is_support_tab ) : ?>
					<p><?php _e( "Achievements gives your BuddyPress community fresh impetus by promoting and rewarding social interaction with challenges, badges and points. For information, support, premium enhancements and developer documentation, visit <a href='http://achievementsapp.nxtclass.com/'>our website</a>.", 'dpa' ) ?></p>
...
?>
	...
	<ul>
		...
		<li><p><a href="http://nxtclass.org/extend/plugins/achievements/"><?php _e( 'Give it a good rating on NXTClass.org', 'dpa' ) ?></a>.</p></li>
	...
	<ul class="menu">
		<li><a href="http://twitter.com/?status=Check%20out%20Achievements%20for%20%23BuddyPress%20http://nxtclass.org/extend/plugins/achievements/"><img src="<?php echo plugins_url( '/images/twitter_32.png', __FILE__ ) ?>" alt="<?php _e( 'Twitter', 'dpa' ) ?>" /></a></li>
		<li><a href="http://www.facebook.com/sharer.php?u=http://nxtclass.org/extend/plugins/achievements/"><img src="<?php echo plugins_url( '/images/facebook_32.png', __FILE__ ) ?>" alt="<?php _e( 'Facebook', 'dpa' ) ?>" /></a></li>
		<li><a href="http://del.icio.us/post?url=http://nxtclass.org/extend/plugins/achievements&title=Achievements%20for%20BuddyPress"><img src="<?php echo plugins_url( '/images/delicious_32.png', __FILE__ ) ?>" alt="<?php _e( 'Delicious - social bookmarking', 'dpa' ) ?>" /></a></li>
		<li><a href="http://www.stumbleupon.com/submit?url=http://nxtclass.org/extend/plugins/achievements&amp;title=Achievements%20for%20BuddyPress"><img src="<?php echo plugins_url( '/images/stumbleupon_32.png', __FILE__ ) ?>" alt="<?php _e( 'Stumble Upon', 'dpa' ) ?>" /></a></li>
	</ul>
<?php
}
...
/**
 * Adds nxt Help panel link (the "Help" dropdown in the top-right of the page)
 *
...
?>
	<p><?php _e( "If you're submitting a support request, some information about your NXTClass site really helps us to diagnose the problem.", 'dpa' ) ?></p>
	...
	<ul>
		...
		<li><?php printf( __( 'MySQL: %s', 'dpa' ), $nxtdb->db_version() ) ?></li>
		<li><?php printf( __( 'Permalinks: %s', 'dpa' ), $custom_permalinks ) ?></li>
		<li><?php printf( __( 'PHP: %s', 'dpa' ), phpversion() ) ?></li>
		<li><?php printf( __( 'NXTClass: %s', 'dpa' ), $nxt_version ) ?></li>
		<li><?php printf( __( 'NXTClass multisite: %s', 'dpa' ), $is_multisite ) ?></li>
	</ul>
...
?>
	<div class="component">
		...
		<div class="setting-group setting-medialibrary">
			<div class="setting wide">
				<div class="settingname">
					<p><?php printf( __( "If your <a href='%s'>NXTClass Media Library</a> contains a lot of content, it may be hard to find the images which you want to use for your Achievements' pictures.", 'dpa' ), admin_url( 'upload.php' ) ) ?></p>
