...
<?php if ( dpa_change_picture_has_pictures() && current_user_can( 'upload_files' ) ) : ?>
	...
	<?php if ( dpa_change_picture_has_manylots() ) : ?>
		<div id="message" class="info">
			<p><?php printf( __( "We've noticed that you've got a lot of pictures to choose from. <a href='%s'>Set a filter</a> to restrict which images are retrieved from your <a href='%s'>NXTClass Media Library</a>.", 'dpa' ), admin_url( 'admin.php?page=achievements' ), admin_url( 'upload.php' ) ) ?></p>
		</div>
	<?php endif;  ?>
	...
	<?php nxt_nonce_field( 'achievement-change-picture-' . dpa_get_achievement_slug() ) ?>
	</form>
...
<?php elseif ( current_user_can( 'upload_files' ) ) : ?>
	...
	<p><?php printf( __( "Before you can choose a picture, you need to upload an image to the <a href='%s'>NXTClass Media Library</a>.", 'dpa' ), admin_url( 'upload.php' ) ) ?></p>

<?php else : ?>

	<p><?php _e( "Unfortunately, there aren't any pictures to choose from. You'll need to ask an Administrator to upload some to the NXTClass Media Library for you.", 'dpa' ) ?></p>
