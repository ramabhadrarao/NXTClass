...
add_filter( 'dpa_get_addedit_action_description', 'nxttexturize'   );
...
add_filter( 'dpa_get_achievements_search_query', 'nxt_kses_data', 1 );  // From an external source
add_filter( 'dpa_get_achievements_search_query', 'nxttexturize'       );
add_filter( 'dpa_get_achievements_search_query', 'convert_chars'     );

add_filter( 'dpa_admin_get_rss_feed', 'nxt_kses_data', 1 );  // From an external source
add_filter( 'dpa_admin_get_rss_feed', 'nxttexturize'       );
add_filter( 'dpa_admin_get_rss_feed', 'convert_chars'     );

add_filter( 'dpa_widget_title', 'nxttexturize'   );
add_filter( 'dpa_widget_title', 'convert_chars' );

add_filter( 'dpa_admin_settings_mediakeywords', 'nxttexturize'   );
...
// CRUD filters
add_filter( 'dpa_achievement_name_before_save', 'nxt_filter_kses', 1   );
add_filter( 'dpa_achievement_name_before_save', 'sanitize_text_field' );

add_filter( 'dpa_achievement_slug_before_save', 'nxt_filter_kses', 1 );
...
if ( bp_is_active( 'activity' ) )
	...
	add_filter( 'dpa_achievement_description_before_save', 'nxt_filter_kses', 1          );

// More CRUD filters (widgets + admin settings)
add_filter( 'dpa_admin_settings_mediakeywords_before_save', 'nxt_kses_data', 1     );
add_filter( 'dpa_admin_settings_mediakeywords_before_save', 'sanitize_text_field' );

add_filter( 'dpa_widget_title_before_save', 'nxt_filter_kses', 1   );
