...
/**
 * Add nxt Admin Bar support
 *
 * @global object $bp BuddyPress global settings
 * @global object $nxt_admin_bar nxt Admin Bar object
 * @since 2.1
 */
function dpa_setup_admin_bar() {
	...
	if ( is_user_logged_in() ) {
		...
		foreach( $nxt_admin_nav as $admin_menu )
...
/**
 * This function adds a nxt-admin menu item under "BuddyPress."
 *
 * @global object $bp BuddyPress global settings
 * @since 2.0
 */
function dpa_add_admin_menu() {
	...
	// This file holds everything that goes into /nxt-admin/
	require ( dirname( __FILE__ ) . '/achievements-admin.php' );

	nxt_enqueue_style( 'achievements-nxtadmin', plugins_url( '/css/nxtadmin.css', __FILE__ ), array(), ACHIEVEMENTS_VERSION );
...
function dpa_get_active_actions() {
	...
	if ( !$actions = nxt_cache_get( 'dpa_active_actions', 'dpa' ) ) {
		$actions = $nxtdb->get_results( $nxtdb->prepare( "SELECT DISTINCT action.name FROM {$bp->achievements->table_achievements} as achievement, {$bp->achievements->table_actions} as action WHERE achievement.action_id = action.id AND achievement.action_id != -1 AND (achievement.is_active = 1 OR achievement.is_active = 2)" ) );
		nxt_cache_set( 'dpa_active_actions', $actions, 'dpa' );
	}
...
function dpa_achievement_modification_update_meta( $new_achievement, $old_achievement ) {
	...
	if ( $new_achievement->picture_id != $old_achievement->picture_id )
		nxt_cache_delete( 'dpa_achievement_picture_urls', 'dpa' );

	// Achievement type has changed, clear user count meta.
	if ( $new_achievement->action_id != $old_achievement->action_id ) {
		$users = $nxtdb->query( "SELECT user_id FROM {$nxtdb->usermeta} WHERE meta_key = 'achievements_counters'" );
...
add_action( 'nxtmu_delete_user', 'dpa_remove_user_data', 1, 1 );
...
function dpa_delete_highscore_cache() {
	...
	foreach ( (array) $sitewide_widget_options as $widget ) {
		...
		if ( !empty( $widget->limit ) )
			nxt_cache_delete( 'dpa_high_scorers_' . apply_filters( 'dpa_widget_limit', $widget->limit ), 'dpa' );
	}
}
...
/**
 * Hooks into the relevant NXTClass actions which Achievements need to make its magic work.
 *
 * @global object $bp BuddyPress global settings
 * @since 2.0
 */
...
/**
 * Implements the comment_post action for NXTClass
 *
 * @since 2.0
 */
...
/**
 * Implements the draft_to_publish (posts, pages) action for NXTClass
 *
 * @since 2.0
 */
...
/**
 * Implements the pending_to_publish (posts, pages) action for NXTClass. Bit of a kludge that works in conjunction with dpa_handle_action_draft_to_publish().
 *
...
/**
 * Implements the trashed_post action for NXTClass
 *
 * @since 2.0
 */
...
/**
 * Implements the signup_finished action for NXTClass
 *
 * @since 2.0
 */
...
/**
 * Implements the nxt_login action for BuddyPress
 *
 * @since 2.0.5
 */
function dpa_handle_action_nxt_login() { $func_get_args = func_get_args(); dpa_handle_action( 'nxt_login', $func_get_args ); }
...
add_filter( 'dpa_handle_action_user_id', 'dpa_filter_nxt_login_user_id', 10, 3 );
...
add_action( 'nxt', 'dpa_setup_nav_directory', 2 );
...
function dpa_screen_achievement_change_picture() {
  ...
  if ( !nxt_verify_nonce( $_POST['_nxtnonce'], 'achievement-change-picture-' . $bp->current_item ) ) {
		nxt_nonce_ays( '' );
		die();
  }
...
function dpa_screen_achievement_grant() {
  ...
  if ( !nxt_verify_nonce( $_POST['_nxtnonce_achievements_grant'], 'achievement_grant_' . $bp->current_item ) ) {
		nxt_nonce_ays( '' );
		die();
  }
...
add_action( 'nxt', 'dpa_screen_achievement_create', 3 );
