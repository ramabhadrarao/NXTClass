 ...
 * @since 2.0
 * @global DPA_Achievement_Template $achievements_template Achievements template tag object
 * @return string Achievement picture id (nxt Media Library)
 */
...
function dpa_achievement_picture( $size='thumb' ) {
	...
	function dpa_get_achievement_picture( $size='' ) {
		...
		if ( ( $picture_id = dpa_get_achievement_picture_id() ) < 1 ) {
			...
			if ( $cached_urls = nxt_cache_get( 'dpa_achievement_picture_urls', 'dpa' ) && isset( $cached_urls ) &&
...
function dpa_addedit_warning( $form_field ) {
	...
	if ( !is_nxt_error( $achievements_errors ) || empty( $achievements_errors->errors[$form_field] ) )
		return;
...
function dpa_addedit_action_groups() {
	...
	function dpa_get_addedit_action_groups() {
		...
		$groups = $nxtdb->get_results( $nxtdb->prepare( "SELECT id, name FROM {$bp->groups->table_name} ORDER BY name ASC" ) );
...
function dpa_grant_achievement_userlist() {
	...
	function dpa_get_grant_achievement_userlist() {
		...
		$members = $nxtdb->get_results( $nxtdb->prepare( "SELECT ID, display_name FROM {$nxtdb->users} WHERE {$column} = 0 ORDER BY display_name ASC" ) );
...
function dpa_addedit_action_multisites() {
	...
	function dpa_get_addedit_action_multisites() {
		global $blogs_template, $bp, $nxtdb;
		...
		$sites = $nxtdb->get_col( $nxtdb->prepare( "SELECT blog_id as id FROM {$nxtdb->blogs} WHERE site_id = %d", $nxtdb->siteid ) );
...
function dpa_change_picture_pagination_count() {
	...
	$start_num = intval( ( dpa_change_picture_get_page() - 1 ) * $nxt_query->query_vars['posts_per_page'] ) + 1;
	$from_num = bp_core_number_format( $start_num );
	$to_num = bp_core_number_format( ( $start_num + ( $nxt_query->query_vars['posts_per_page'] - 1 ) > $nxt_query->found_posts ) ? $nxt_query->found_posts : $start_num + ( $nxt_query->query_vars['posts_per_page'] - 1 ) );
	$total = bp_core_number_format( $nxt_query->found_posts );
...
function dpa_change_picture_pagination() {
	...
	$pag_links = paginate_links( array(
		...
		'format' => '',
		'total' => ceil( $nxt_query->found_posts / $nxt_query->query_vars['posts_per_page'] ),
		'current' => $pag_page,
		'prev_text' => '&larr;',
		'next_text' => '&rarr;'
	) );
...
/**
 * Are there any pictures in the nxt Media Library for the change picture page?
 *
 * @global nxt_Query $nxt_query NXTClass query object
 * @since 2.0
 * @return bool
...
function dpa_change_picture_has_pictures() {
	...
	return ( $nxt_query->found_posts );
}

/**
 * Are there lots of pictures in the media library, and has no search string been set in the nxt-admin?
 *
 * @global nxt_Query $nxt_query NXTClass query object
 * @return bool
 * @since 2.0
...
function dpa_change_picture_has_manylots() {
	...
	return ( ceil( $nxt_query->found_posts / $nxt_query->query_vars['posts_per_page'] ) > 3 && ( !isset( $settings['mediakeywords'] ) || !$settings['mediakeywords'] ) );
}
