...
function bp_tpack_admin_notices() {
	...
	// if nxt version is less than 3.2, show notice when on TPack options page
	if ( isset( $_GET['page'] ) && 'bp-tpack-options' == $_GET['page'] ) {
		if ( version_compare( $nxt_version, '3.2', '<' ) ) {
		?>
			<div class="error">
				<p><?php _e( "Hey you! You're using an older version of NXTClass.  Please upgrade to <strong>NXTClass 3.2</strong>, otherwise the javascript bundled with BuddyPress will cease to work with your NXTClass theme.", 'bp-tpack' ); ?></p>
			</div>
		<?php
		}
...
function bp_tpack_theme_menu() {
	$theme_dir = nxt_CONTENT_DIR . '/themes/' . get_option('stylesheet') . '/';
...
?>
	<div class="wrap">
		...
		<p><?php _e( 'Adding support for BuddyPress to your existing NXTClass theme is a straightforward process. Follow the setup instructions on this page.', 'bp-tpack' ); ?></p>
		...
		<?php case 2: ?>
				...
				<?php if ( $error ) : ?>
					...
					<p><?php _e( 'You will need to connect to your NXTClass files using FTP. When you are connected browse to the following directory:', 'bp-tpack' ); ?><p>
...
[FOOTER]
			...
			<p><?php _e( "You're now all done with the conversion process. Your NXTClass theme will now happily provide BuddyPress compatibility support. Once you hit the finish button you will be presented with a new permanent theme options page, which will allow you to tweak some settings.", 'bp-tpack' ); ?></p>
...
/**
 * Function to copy over bp-default's main templates to the current nxt theme
 *
 * @uses bp_tpack_recurse_copy()
 */
function bp_tpack_move_templates() {
	$destination_dir = nxt_CONTENT_DIR . '/themes/' . get_option('stylesheet') . '/';
