 ...
 *
 * Sets up the current nxt theme for BuddyPress compatibility.
...
add_action( 'nxt_enqueue_scripts', 'bp_tpack_enqueue_scripts' );
...
add_action( 'nxt_print_styles', 'bp_tpack_enqueue_styles' );

if ( !function_exists( 'bp_tpack_use_nxtlogin' ) ) :
/**
 * BP Template Pack doesn't use bp-default's built-in sidebar login block,
 * so during no access requests, we need to redirect them to nxt-login for
 * authentication.
 *
 * @since 1.2
 */
function bp_tpack_use_nxtlogin() {
	// returning 2 will automatically use nxt-login
	return 2;
}
add_filter( 'bp_no_access_mode', 'bp_tpack_use_nxtlogin' );
endif;
...
if ( version_compare( BP_VERSION, '1.3', '<' ) ) :
	...
	function bp_tpack_nxt_pages_filter( $page_html ) {
	...
	add_filter( 'nxt_dropdown_pages', 'bp_tpack_nxt_pages_filter' );
	...
	function bp_tpack_fix_get_posts_on_activity_front() {
		...
		if ( !empty($nxt_query->query_vars['page_id']) && 'activity' == $nxt_query->query_vars['page_id'] )
			$nxt_query->query_vars['page_id'] = '"activity"';
	}
