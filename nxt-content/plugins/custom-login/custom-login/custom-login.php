...
function custom_login_setup() {
	...
	//add_action( 'login_head', 'nxt_print_scripts' );
...
if ( !function_exists( 'is_version' ) ) {
	function is_version( $version = '3.0' ) {
		...
		if ( version_compare( $nxt_version, $version, '<' ) ) {
			return false;
		}
		return true;
	}
...
function custom_login() {
	...
	if ( ( custom_login_get_setting( 'custom_html' ) != '' || custom_login_get_setting( 'gravatar' ) != false ) && get_option( 'users_can_register' ) ) {
		nxt_print_scripts( array( 'jquery', 'gravatar' ) );
	}
	if ( custom_login_get_setting( 'custom_html' ) != '' ) {
		nxt_print_scripts( array( 'jquery' ) );
	}
	nxt_register_style( 'custom-login-defualt', CUSTOM_LOGIN_CSS . 'custom-login.css', false, 0.8, 'screen' );
	nxt_register_style( 'custom-login', CUSTOM_LOGIN_CSS . 'custom-login.css.php', false, 0.8, 'screen' );
	
	if ( custom_login_get_setting( 'custom' ) != false ) {		
		nxt_print_styles( 'custom-login' );		
	} else {		
		nxt_print_styles( 'custom-login-defualt' );		
	}
}
...
jQuery(document).ready(
	function($) {
		...
		$('html body.login').append('<?php echo nxt_specialchars_decode( stripslashes( custom_login_get_setting( 'custom_html' ) ), 1, 0, 1 ); ?>');
		<?php } ?>
