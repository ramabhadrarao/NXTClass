...
add_action( 'nxt_dashboard_setup', 'thefrosty_dashboard_widgets' );

if ( !function_exists( 'thefrosty_dashboard_widgets' ) ) {
	function thefrosty_dashboard_widgets() {
		global $nxt_meta_boxes;
	
		nxt_add_dashboard_widget( 'thefrosty_dashboard', __( 'The Frosty Network <em>feeds</em>' ), 'thefrosty_dashboard_widget_rss' );
	}
}
...
if ( !function_exists( 'thefrosty_dashboard_widget_rss' ) ) {
	function thefrosty_dashboard_widget_rss( $sidebar_args ) {
		...
		extract( array($sidebar_args, EXTR_SKIP));
			...
			$style .= 'span.nxtworkshop { background-position: -221px 0 !important }' . "\n";
