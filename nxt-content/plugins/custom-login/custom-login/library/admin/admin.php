...
function custom_login_scripts() {
	...
	nxt_register_script( 'autoresize', CUSTOM_LOGIN_JS . 'autoresize.min.js', array( 'jquery' ), '1.04', false );
	
	nxt_register_script( 'custom-login', CUSTOM_LOGIN_JS . 'custom-login.js', array( 'jquery' ), $plugin_data['Version'], false );
	
	nxt_register_script( 'jscolor', CUSTOM_LOGIN_JS . 'jscolor.js', false, '1.3.1', false );
	
	nxt_register_script( 'gravatar', CUSTOM_LOGIN_JS . 'gravatar.js', array( 'jquery' ), '1.2', false );
}
...
function custom_login_styles() {	
	...
	nxt_register_style( 'custom-login-defualt', CUSTOM_LOGIN_CSS . 'custom-login.css', false, $plugin_data['Version'], 'screen' );
	
	nxt_register_style( 'custom-login', CUSTOM_LOGIN_CSS . 'custom-login.css.php', false, $plugin_data['Version'], 'screen' );
	
	nxt_register_style( 'custom-login-tabs', CUSTOM_LOGIN_CSS . 'tabs.css', false, $plugin_data['Version'], 'screen' );
	
	nxt_register_style( 'custom-login-admin', CUSTOM_LOGIN_CSS . 'admin.css', false, $plugin_data['Version'], 'screen' );
}
...
function custom_login_settings() {
	...
	$settings = array(
		...
		'custom_html' => '',	
		/* html */
		'html_border_top_color' => '', //nxt < 3.x
		'html_border_top_background' => '', //nxt > 3.x
...
function custom_login_load_settings_page() {
	...
	//if ( !empty( $settings['use_custom'] ) ) {
		...
		//nxt_redirect( admin_url( 'options-general.php?page=custom-login' ) );
		//exit;
	//}
	///////////////////////////////////////////////////////////////////////////////////////////////
	...
	elseif ( isset( $_POST['custom-login-settings-submit'] ) ) {
		...
		nxt_redirect( admin_url( 'options-general.php?page=custom-login&updated=true' ) );
		exit;
	}
}
...
function custom_login_load_upgrade_page() {
	...
	if ( isset( $_POST['custom-login-authorization-form'] ) ) {
		...
		/* Redirect the page so that the new settings are reflected on the settings page. */
		nxt_redirect( admin_url( 'options-general.php?page=' . $custom_login->upgrade_page_link[2] ) );
		exit;
	}
}
...
function custom_login_upgrade_meta_box() {
	...
	if ( isset( $_POST['username'] ) && isset( $_POST['password'] ) && isset( $_POST['custom-login-authorization-form'] ) ) {
		...
		require_once( ABSPATH . nxtINC . '/class-IXR.php' );
	...
	jQuery(document).ready(function($) {
		...
		$('#auth-button').click(function(e) {
			$('#auth-button').prop("disabled","disabled").after('&nbsp;&nbsp;<img src="<?php echo network_site_url( '/nxt-admin/images/nxtspin_light.gif' ); ?>" alt="loading" style="vertical-align: middle;" /> Authorizing, please wait.');
 ...
 * http://codex.nxtclass.org/Function_Reference/remove_submenu_page
 *
 * @since 1.0.1
 */
...
function custom_login_support_meta_box() { ?>
	...
	<table class="form-table">
        ...
        <tr>
            ...
            <td><?php _e( '<a href="http://nxtclass.org/extend/plugins/custom-login/">This plugin on NXTClass.org</a>.', 'custom-login' ); ?></td>
        </tr>
        <tr>
		...
		<tr>
			...
			<td><?php _e( '<a href="http://nxtclass.org/tags/custom-login">NXTClass support forums</a>.', 'custom-login' ); ?></td>
		</tr>
		<tr class="alt">
...
function custom_login_preview_meta_box() { ?>
    ...
    <p style="font-weight: bold; text-align: center;"><a class="thickbox thickbox-preview" href="<?php echo nxt_login_url(); ?>?TB_iframe=true" title=""><?php _e( 'Click here to see a live preview!', 'custom-login' ); ?></a></p>
	<p style="text-align: center;"><small><?php _e( '(May not work as of NXTClass 3.1.1)', 'custom-login' ); ?></small></p><?php
}
	...
	<table class="form-table">
        ...
        <tr style="border-top: 1px solid #eee;">
            ...
            <td>
                ...
                <a class="question" title="Help &amp; Examples">[?]</a><br />
                <span class="hide"><?php _e( 'Upload an image and put the full path here.<br />
                Suggested size: <code>310px X 70px</code>, which will replace NXTClass logo. Be sure to leave blank if not in use. NOTE: Will go <strong>above</strong> the form and it&prime;s border.', 'custom-login' ); ?>
                </span>
            </td>
   		</tr>        
        <!-- Break -->
...
function custom_login_advanced_meta_box() { ?>
	...
	<table class="form-table">
		<tr>
            ...
            <td>             
                <textarea id="custom_css" name="custom_css" cols="50" rows="3" class="large-text code"><?php echo nxt_specialchars_decode( stripslashes( custom_login_get_setting( 'custom_css' ) ), 1, 0, 1 ); ?></textarea>
                <a class="question" title="Help &amp; Examples">[?]</a><br />
		...
		<tr>
            ...
            <td>             
                <textarea id="custom_html" name="custom_html" cols="50" rows="3" class="large-text code"><?php echo nxt_specialchars_decode( stripslashes( custom_login_get_setting( 'custom_html' ) ), 1, 0, 1 ); ?></textarea>
                <a class="question" title="Help &amp; Examples">[?]</a><br />
...
function custom_login_tabs_meta_box() { ?>
	<table class="form-table">
        ...
        <ul class="tabs">        
            ...
            <li class="t3 t"><a class="t3 tab">&fnof;xThemes (nxt themes)</a></li> 
            <li class="t4 t"><a class="t4 tab">TheFrosty (nxt plugins)</a></li>  
	...
	<div class="wrap">
		...
		<div id="poststuff">
			...
			<form method="post" action="<?php echo esc_url( admin_url( 'options-general.php?page=custom-login' ) ); ?>">
				...
				<?php nxt_nonce_field( 'custom-login-settings-page' ); ?>
				<?php nxt_nonce_field( 'closedpostboxes', 'closedpostboxesnonce', false ); ?>

				<?php nxt_nonce_field( 'meta-box-order', 'meta-box-order-nonce', false ); ?>
	...
	<div class="wrap">
		...
		<div id="poststuff">
			...
			<form method="post" action="<?php echo esc_url( admin_url( 'options-general.php?page=' . $custom_login->upgrade_page_link[2] ) ); ?>" id="custom-login-authorization-form">
				...
				<?php nxt_nonce_field( 'custom-login-upgrade-page' ); ?>
				<?php nxt_nonce_field( 'closedpostboxes', 'closedpostboxesnonce', false ); ?>
				<?php nxt_nonce_field( 'meta-box-order', 'meta-box-order-nonce', false ); ?>
...
function custom_login_settings_page_enqueue_script() {	
	...
	nxt_enqueue_script( 'nxt-lists' );
	nxt_enqueue_script( 'postbox' );
	nxt_enqueue_script( 'thickbox' );
	nxt_enqueue_script( 'theme-preview' );
	nxt_enqueue_script( 'autoresize' );
	nxt_enqueue_script( 'custom-login' );
	nxt_enqueue_script( 'jscolor' );
}
...
function custom_login_settings_page_enqueue_style() {
	...
	nxt_enqueue_style( 'custom-login-tabs' );
	nxt_enqueue_style( 'custom-login-admin' );
}
...
if ( !function_exists( 'thefrosty_network_feed' ) ) {
	function thefrosty_network_feed( $attr, $count ) {		
		...
		include_once( ABSPATH . nxtINC . '/class-simplepie.php' );
		$feed = new SimplePie();
