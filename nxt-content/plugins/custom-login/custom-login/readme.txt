...
Join in on the [conversation](http://austinpassy.com/nxtclass-plugins/custom-login) on my personal blog. I've also just released a [PRO](http://thefrosty.com/custom-login-pro) version on [http://thefrosty.com](http://thefrosty.com). New features include faster login loading (no database access), Custom Post Types (for multiple designs) and four default CSS designs.

Activate this plugin and customize your NXTClass login screen. Use the built-in and easy to use settings page to do the work for you. Theres no need to understand CSS at all!
...
3. Read more about the [Custom Login Plugin](http://austinpassy.com/nxtclass-plugins/custom-login/) & [most recent blog post](http://austinpassy.com/2010/09/custom-login-version-0-8/).
...
1. Upload the `custom-login` directory to the /nxt-content/plugins/ directory.
...
I created this plugin to allow for custom login of any NXTClass login screen. See working example at: [TheFrosty](http://thefrosty.com/nxt-login.php?action=login).
...
* Updated minor CSS changes to NXTClass 3.3.
...
* Updated `nxt_enqueue_style` to `nxt_register_style` for the custom CSS that causes issues in NXTClass 3.3

= Version 0.9.2 (7/6/11) =

* Added `!important` to the `html` CSS attribute for background because of new CSS rule in NXTClass 3.2.
...
* fixed bug where multiple installs in a single server of different nxt installs exist.
...
* Removed [nxtads.net](http://nxtads.net).
* Cleaned up options.
...
* Fixed bug where plugin isn't allowed to be *deleted*(removed) when using NXTClass 3.0+.
...
* Added `nxt_wb_version` check for NXTClass 3+
* Added new option for NXTClass 3+, *body background*.
* Added check for version in animated error.
** nxt3 now includes the jQuery error
* Now 100% compatible with NXTClass 3.x
...
* More options added
* Cleaned up code
* Testing new inline ad feature from [nxtAds](http://bit.ly/nxtadsnet) *Please leave [feedback/comments](http://austinpassy.com/2010/02/custom-login-version-0-5/) on this feature*
...
= 0.9.6 =
NXTClass 3.3 style registration compatible. Some changes may effect your design, please update accordingly.

= 0.9.3 =
NXTClass 3.3 style registration compatible.
...
= 0.7.2 =
Uninstall script no longer valid for NXTClass 3.0+
