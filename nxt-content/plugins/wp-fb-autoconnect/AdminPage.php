...
/*
 * Tell nxt about the Admin page
 */
add_action('admin_menu', 'jfb_add_admin_page', 99);
function jfb_add_admin_page()
{ 
    global $jfb_name;
    add_options_page("$jfb_name Options", 'nxt-FB AutoConn', 'administrator', "nxt-fb-autoconnect", 'jfb_admin_page');
}
...
function jfb_add_plugin_links($links, $file)
{
    if( dirname(plugin_basename( __FILE__ )) == dirname($file) )
        $links[] = '<a href="options-general.php?page=' . "nxt-fb-autoconnect" .'">' . __('Settings','sitemap') . '</a>';
    return $links;
}
...
function jfb_admin_page()
{
    ...
    global $opt_jfb_bp_avatars, $opt_jfb_nxt_avatars, $opt_jfb_valid, $opt_jfb_fulllogerr, $opt_jfb_disablenonce, $opt_jfb_show_credit;
    ...
    {
        ?><div class="error"><p><strong>Warning:</strong> Another plugin has included the Facebook API throughout all of nxtclass.  I suggest you contact that plugin's author and ask them to include it only in pages where it's actually needed.<br /><br />Things may work fine as-is, but *if* the API version included by the other plugin is older than the one required by nxt-FB AutoConnect, it's possible that the login process could fail.</p></div><?php
    }
    if(version_compare('5', PHP_VERSION, ">"))
    {
        ?><div class="error"><p>Sorry, but as of v1.3.0, nxt-FB AutoConnect requires PHP5.</p></div><?php
        die();
    }
    if( function_exists('is_multisite') && is_multisite() && !jfb_premium() )
    {
        ?><div class="error"><p><strong>Warning:</strong> nxtclass MultiSite is only fully supported by the premium version of this plugin; please see <a href="<?php echo $jfb_homepage ?>#premium"><b>here</b></a> for details.</p></div><?php
    }
	
    do_action('nxtfb_admin_messages');
    ...
    if( isset($_POST['main_opts_updated']) )
    {
        ...
        update_option( $opt_jfb_nxt_avatars, $_POST[$opt_jfb_nxt_avatars] );
    ...
    if( isset($_POST['remove_all_settings']) )
    {
        ...
        delete_option($opt_jfb_nxt_avatars);
    ...
    <div class="jfb-admin_wrapper">
        ...
        <div class="<?php echo $allTabsClass ?>" id="<?php echo $tab2Id?>" style="display:<?php echo ($shownTab==2?"block":"none")?>">
            ...
            <form name="formMainOptions" method="post" action="">
                ...
                <input type="radio" name="<?php echo $opt_jfb_username_style; ?>" value="2" <?php echo (get_option($opt_jfb_username_style)==2?"checked='checked'":"")?> >Legacy Format (i.e. John.Smith) <i><b>(Not Recommended, <dfn title="Although the original 'BuddyPress-friendly' username format included a period, I later learned that this creates issues with author links in nxtclass.  I've left the option here for legacy support, but advise against using it (unless you have only one author on your blog, in which case Facebook-connected users won't have author links and so it doesn't matter).  If you do have multiple authors and are experiencing broken author links, changing this option will fix it for all NEW users, but you may want to consider fixing your existing users by replacing all of the '.'s with '_'s in the 'user_nicename' field of the 'nxt_users' database table.">mouseover for why</dfn>)</b></i><br /><br />
        		...
        		<?php add_option($opt_jfb_stream_content, "has connected to " . get_option('blogname') . " with nxt-FB AutoConnect."); ?>
        		...
        		<br /><b>Avatars:</b><br />
                <input type="checkbox" name="<?php echo $opt_jfb_nxt_avatars?>" value="1" <?php echo get_option($opt_jfb_nxt_avatars)?'checked="checked"':''?> /> Use Facebook profile pictures as avatars<br />
        ...
        <div class="<?php echo $allTabsClass ?>" id="<?php echo $tab4Id?>" style="display:<?php echo ($shownTab==4?"block":"none")?>">
            ...
            The following button will <i>permanently</i> delete all of this plugin's options from your nxtclass database, as if it had never been installed.  Use with care!
...
function jfb_auth($name, $version, $event, $message=0)
{
    ...
    $data = serialize(array(
          ...
          'nxt_version'  => $GLOBALS['nxt_version'],
...
function jfb_output_premium_panel_tease()
{
    ...
    <form name="formPremOptions" method="post" action="">
		...
		Translate Facebook prompts to the same locale as your nxtclass blog (Detected locale: <i><?php echo ( (defined('nxtLANG')&&nxtLANG!="") ? nxtLANG : "en_US" ); ?></i>)
        ...
        <input <?php disableatt() ?> type="checkbox" name="<?php echo $opt_jfbp_wordbooker_integrate?>" value="1" <?php echo get_option($opt_jfbp_wordbooker_integrate)?'checked="checked"':''?> /> Use Facebook avatars for <a href="http://nxtclass.org/extend/plugins/wordbooker/">Wordbooker</a>-imported comments
        ...
        Please use the <a href="<?php echo admin_url('widgets.php') ?>" target="widgets">nxt-FB AutoConnect <b><i>Premium</i></b> Widget</a> if you'd like to:<br />
        ...
        Text: <input <?php disableatt() ?> type="text" size="30" name="<?php echo $opt_jfbp_buttontext; ?>" value="<?php echo get_option($opt_jfbp_buttontext); ?>" /> <dfn title="This setting applies to ALL of your Facebook buttons (in the widget, nxt-login.php, comment forms, etc).">(Mouseover for more info)</dfn><br />
        Style: 
        ...
        <input <?php disableatt() ?> type="checkbox" name="<?php echo $opt_jfbp_nxtloginfrmlogin?>" value="1" <?php echo get_option($opt_jfbp_nxtloginfrmlogin)?'checked="checked"':''?> /> Add a Facebook Login button to the standard Login page (nxt-login.php)<br />
        <input <?php disableatt() ?> type="checkbox" name="<?php echo $opt_jfbp_registrationfrmlogin?>" value="1" <?php echo get_option($opt_jfbp_registrationfrmlogin)?'checked="checked"':''?> /> Add a Facebook Login button to the Registration page (nxt-login.php)<br />
        <input <?php disableatt() ?> type="checkbox" name="<?php echo $opt_jfbp_signupfrmlogin?>" value="1" <?php echo get_option($opt_jfbp_signupfrmlogin)?'checked="checked"':''?> /> Add a Facebook Login button to the Signup page (nxt-signup.php) (nxtMU Only)<br /><br />
