...
This file is for support NXTClass default/core gallery settings and image edit
...
(function()
{
	CKEDITOR.plugins.add( 'nxtgallery',
	{
		...
		init: function( editor )
		{
			...
			editor.addCommand( 'nxtgallery_edit',
			{
				exec : function(editor)
				{
					...
					if(CKEDITOR && element && element.data && element.data('gallery')) CKEDITOR.plugins.nxtgallery.createGallery( editor, element, element.data('gallery'), 0 );
					...
					tb_show('',CKEDITOR.getUrl( window.CKEDITOR_BASEPATH + '../../../../nxt-admin/media-upload.php?post_id='+post_id+'&tab=gallery&TB_iframe=1'));
			...
			editor.addCommand( 'nxtgallery_delete',
			{
				exec : function( editor )
				{
					var sel = editor.getSelection(),
			...
			editor.addCommand( 'captionimage_edit',
			{
				exec : function(editor)
				{
					...
					{
						tb_show('',CKEDITOR.getUrl( window.CKEDITOR_BASEPATH + '../plugins/nxteditimage/editimage.html?ver=321&TB_iframe=true'));
					}
				}
			});
			...
			editor.on( 'doubleclick', function( evt )
			{
				...
				if ( element.is( 'img' ) && element.hasClass( 'cke_nxtgallery' ) )
				{
					editor.execCommand('nxtgallery_edit');
					evt.cancel();
				}

				//for image inserted via nxt open edit by nxt, remove CKEditors image dialog
				var pattern = /nxt-image-[0-9]+/;
				...
				editor.contextMenu.addListener( function( element, selection )
					{
						...
						if ( element && element.getName() == 'img' && element.hasClass( 'cke_nxtgallery' ) )
						{
							return { nxtgallery_edit : CKEDITOR.TRISTATE_OFF, nxtgallery_delete : CKEDITOR.TRISTATE_OFF };
						}//if element is image and is inserted via nxt remove CKEditor's image button
			...
			if ( dataFilter )
			{
				dataFilter.addRules(
				{
					text : function( text )
					{
						...
						{
							return CKEDITOR.plugins.nxtgallery.createGallery( editor, null, match, 1 );
						});
						...
						'img' : function(element)
						{
							...
							if(element.attributes && pattern.test(element.attributes['class']))
							{
								...
								if (match && match[1] && caption !== '' && pattern.test(caption) !== false)
								{
									...
									pattern = new RegExp('nxt-caption',"i");
									if (!pattern.exec(element.attributes['class']))
									{
										element.attributes['class'] = element.attributes['class'] + ' nxt-caption';
									}
				...
				htmlFilter.addRules(
				{
					elements :
					{
						'img' : function( element )
						{
							...
							if (element.attributes && element.attributes['data-cke-nxtgallery'])
							{
	...
	CKEDITOR.plugins.nxtgallery =
	{
		createGallery : function( editor, oldElement, text, isGet )
		{
			...
			{
				'data-cke-nxtgallery'	: 1,
				'contentEditable' : false,
				'class' : 'nxtGallery, cke_nxtgallery',
				'title' : title,
				'alt' : 'gallery',
