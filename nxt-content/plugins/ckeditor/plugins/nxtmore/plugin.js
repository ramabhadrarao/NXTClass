...
CKEDITOR.plugins.add( 'nxtmore',
{
	...
	init : function( editor )
	{
		...
		editor.addCommand( 'nxtmore',
		{
			exec : function()
			{
				...
				for ( var i = 0, len = images.count() ; i < len ; i++ )
				{
					...
					if ( img.hasClass( 'cke_nxtclass_more' ) )
					{
		...
		function insertComment( text )
		{
			...
			var fakeElement = editor.createFakeElement( new CKEDITOR.dom.comment( text ), 'cke_nxtclass_' + text, 'hr' );
		...
		editor.dataProcessor.dataFilter.addRules(
			{
				comment : function( value )
				{
					...
					if ( value == 'more' )
						return editor.createFakeParserElement( new CKEDITOR.htmlParser.comment( value ), 'cke_nxtclass_' + value, 'hr' );
