...
This file is for support NXTClass default/core image settings edit (style, size , css class, title, caption , link).
...
nxtImage = {
	...
	setup : function() {
		...
		if (el.getName() != 'img')
			return;
		if(el.getAttribute('src').indexOf('../nxt-content') != -1 )
		{
		...
		pa = el.getParent() ;
		if ( pa.getName()== 'a'  ) {
			if(pa.getAttribute('href').indexOf('../nxt-content') != -1 )
			{
				tmp = window.parent.document.location.pathname.split('/');
	...
	update : function() {
		...
		{
			//img_class = img_class.replace( /nxt-caption/gi, '' );
			el.removeClass('nxt-caption');
