...
class ckeditor_nxtclass {
	...
	public function __construct() {
		...
		if (DEFINED('nxt_PLUGIN_URL')) {
			$this->plugin_path = nxt_PLUGIN_URL . '/' . basename(dirname(__FILE__)) . '/';
		} else if (DEFINED('nxt_PLUGIN_DIR')) {
			$this->plugin_path = $siteurl . '/' . nxt_PLUGIN_DIR . '/' . basename(dirname(__FILE__)) . '/';
		} else {
			$this->plugin_path = $siteurl . 'nxt-content/plugins/' . basename(dirname(__FILE__)) . '/';
		}
		if(is_ssl()) {
		...
		$this->default_options = array(
			'appearance' => array(
				...
				'default_state' => 't',
				'excerpt_state' => 'f',
				'post_toolbar' => 'NXTClassFull',
				'post_editor_height' => 300,
				/* basic comment settings */
				'comment_editor' => 't',
				'comment_toolbar' => 'NXTClassBasic',
				'comment_editor_height' => 120,
			),
			'upload' => array(
				'browser' => 'disabled',
				'type' => 'native',
				'user_file_path' => 'nxt-content/uploads/',
		...
		$options = get_option('ckeditor_nxtclass');
		if (!$options) {
			add_option('ckeditor_nxtclass', $this->default_options);
		...
		if ($dir == ABSPATH) { //the option was empty
			$dir = ABSPATH . 'nxt-content/uploads';
		}
		$dir = rtrim($dir, "/\\") . "/";
		...
		private function get_sorted_roles() {
			...
			if (class_exists('RoleManager')) {
				foreach ($roles as $role_key => $role_name) {
					...
					if (empty($role))
						continue;
					$role_user_level = array_reduce(array_keys($role->capabilities), array('nxt_User', 'level_reduction'), 0);
	...
	private function checkbox($section, $var, $text, $onClick = '') {
		return '<label id="lbl_' . $var . '"><input type="checkbox" id="' . $var . '" name="options[' . $section . '][' . $var . ']"' .
			...
			($this->options[$section][$var] == 't' ? "checked" : '') . '>&nbsp;' . __($text, 'ckeditor_nxtclass') . "</label>\n";
	}
	...
	public function add_admin_head() {
		...
		<style type="text/css">
			...
			#icon-nxt-ckeditor {background: transparent url(<?php echo $this->plugin_path; ?>images/ckeditor_ico32.png) no-repeat;}
		</style>
		<?php
	...
	public function print_admin_styles() {
		nxt_enqueue_style('ckeditor_admin', $this->plugin_path . 'includes/overview.css', false, $this->version, 'screen');
	}

	public function print_admin_upload_styles() {
		nxt_enqueue_style('ckeditor_admin', $this->plugin_path . 'includes/upload.css', false, $this->version, 'screen');
	}
	...
	private function ckeditor_get_version() {
		...
		return __('N/A', 'ckeditor_nxtclass');
	}
	...
	private function ckfinder_status() {
		...
		if (!file_exists($ckfinder_php)) {
			if (file_exists($nested_ckfinder_php)) {
					return '<span class="ckeditor_error">' . __('CKFinder installed in wrong (nested) directory. Move files from ckeditor/ckfinder/ckfinder to ckeditor/ckfinder directory.', 'ckeditor_nxtclass') . '</span>';
			} else {
					return __('Not installed', 'ckeditor_nxtclass');
			}
		} else {
			$contents = @file_get_contents($config_path);
			if (!$contents) {
				return '<span class="ckeditor_error">' . __('CKFinder is installed, configuration file not readable or empty.', 'ckeditor_nxtclass') . '</span>';
			}
			if (strpos($contents, 'nxt-config.php') === false) {
				return '<span class="ckeditor_error">' . __('CKFinder is installed, but invalid config.php was detected. Rename ckfinder_config.php to config.php (overwriting the default config.php file distributed with CKFinder).', 'ckeditor_nxtclass') . '</span>';
			}
			if ($this->options['upload']['browser'] != 'ckfinder') {
				return '<span class="ckeditor_ok">' . __('CKFinder is installed, not enabled.', 'ckeditor_nxtclass') . '</span>';
			}
			return '<span class="ckeditor_ok">' . __('CKFinder is installed', 'ckeditor_nxtclass') . '</span>';
		}
	}
	...
	public function plugin_overview() {
		...
		if (isset($_POST['reset']) && $_POST['reset'] == 1) {
			if (!nxt_verify_nonce($_POST['csrf_ckeditor-for-nxtclass'], 'ckeditor_create_nonce_overview') || empty($_POST['_nxt_http_referer']) || ( isset($_SERVER['HTTP_REFERER']) && !strstr($_SERVER['HTTP_REFERER'], $_POST['_nxt_http_referer']) ))
				nxt_die("You do not have sufficient permissions to access this page.");
			update_option('ckeditor_nxtclass', $this->default_options);
	...
	public function option_page() {
		if (!empty($_POST['submit_update'])) {
			$message = array();
			/* validation */
			if (!nxt_verify_nonce($_POST['csrf_ckeditor-for-nxtclass'], 'ckeditor_create_nonce_basic') || empty($_POST['_nxt_http_referer']) || ( isset($_SERVER['HTTP_REFERER']) && !strstr($_SERVER['HTTP_REFERER'], $_POST['_nxt_http_referer']) ))
				nxt_die("You do not have sufficient permissions to access this page.");
	...
	public function upload_options() {
		if (!empty($_POST['submit_update'])) {
				if (!nxt_verify_nonce($_POST['csrf_ckeditor-for-nxtclass'], 'ckeditor_create_nonce_upload') || empty($_POST['_nxt_http_referer']) || ( isset($_SERVER['HTTP_REFERER']) && !strstr($_SERVER['HTTP_REFERER'], $_POST['_nxt_http_referer']) ))
					nxt_die("You do not have sufficient permissions to access this page.");
				$new_options = $_POST['options'];
				...
				if ($new_options['upload']['browser'] != 'disabled') {
					...
					foreach ($inputs as $input) {
						if (empty($new_options['upload'][$input])) {
							$message['upload_' . $input] = __('This field shouldn\'t be empty', 'ckeditor_nxtclass');
						} elseif (!preg_match('#^([a-z0-9]+){1}(,[a-z0-9]+)*,?$#Uis', trim($new_options['upload'][$input]))) {
							$message['upload_' . $input] = __('Files extensions has wrong chars.', 'ckeditor_nxtclass');
						}
					}
					...
					if ($new_options['upload']['browser'] == 'ckfinder') {
						...
						if (empty($new_options['ckfinder']['file_max_size'])) {
							$message['ckfinder_file_max_size'] = __('This field is required.', 'ckeditor_nxtclass');
						} elseif (!preg_match('/^\d+[MKG]?$/i', trim($new_options['ckfinder']['file_max_size']))) {
								$message['ckfinder_file_max_size'] = __('Enter valid value. Example: 400 or 10M', 'ckeditor_nxtclass');
						}
						$inputs = array('images_width', 'images_height', 'thumbnails_width', 'thumbnails_height');
						foreach ($inputs as $input) {
							if (empty($new_options['ckfinder'][$input])) {
								$message['ckfinder_' . $input] = __('This field is required.', 'ckeditor_nxtclass');
							} elseif (!preg_match('#^\d+$#', trim($new_options['ckfinder'][$input]))) {
								$message['ckfinder_' . $input] = __('Enter valid value. Example: 400', 'ckeditor_nxtclass');
							}
						}
						if (empty($new_options['ckfinder']['thumbnails_quality'])) {
							$message['ckfinder_thumbnails_quality'] = __('This field is required.', 'ckeditor_nxtclass');
						} elseif (!(preg_match('#^[0-9]{1,2}$#', trim($new_options['ckfinder']['thumbnails_quality'])) || trim($new_options['ckfinder']['thumbnails_quality']) == '100')) {
							$message['ckfinder_thumbnails_quality'] = __('Value should be betwen 1 and 100.', 'ckeditor_nxtclass');
						}
						if (empty($new_options['ckfinder']['images_quality'])) {
							$message['ckfinder_images_quality'] = __('This field is required.', 'ckeditor_nxtclass');
						} elseif (!(preg_match('#^[0-9]{1,2}$#', trim($new_options['ckfinder']['images_quality'])) || trim($new_options['ckfinder']['images_quality']) == '100')) {
							$message['ckfinder_images_quality'] = __('Value should be betwen 1 and 100.', 'ckeditor_nxtclass');
						}
				}
			}
	...
	public function advanced_options() {
		if (!empty($_POST['submit_update'])) {
				if (!nxt_verify_nonce($_POST['csrf_ckeditor-for-nxtclass'], 'ckeditor_create_nonce_advanced') || empty($_POST['_nxt_http_referer']) || ( isset($_SERVER['HTTP_REFERER']) && !strstr($_SERVER['HTTP_REFERER'], $_POST['_nxt_http_referer']) ))
					nxt_die("You do not have sufficient permissions to access this page.");
				$new_options = $_POST['options'];
				...
				$new_options['plugins']['nxtgallery'] = 't';
				...
				if (!empty($new_options['css']['path'])) {
					if ($new_options['css']['mode'] != 'self') {
						$message['css_path'] = __('CSS path is not empty. Please set the "Editor CSS" option to "define css" mode.', 'ckeditor_nxtclass');
					} elseif (FALSE !== strpos($new_options['css']['path'], '"')) {
						$massage['css_path'] = __('Double quotes are not allowed in CSS path.', 'ckeditor_nxtclass');
					} elseif (substr($new_options['css']['path'], 0, 1) == "'" && substr($new_options['css']['path'], -1) == "'") {
						$massage['css_path'] = __('Enter valid path, do not surround it with quotes.', 'ckeditor_nxtclass');
					}
				}
				/** styles * */
				if (!empty($new_options['css']['style_path'])) {
					if ($new_options['css']['styles'] != 'self') {
						$message['css_style_path'] = __('Path to predefined styles is not empty. Please set the "Predefined styles" option to "define path to ckeditor.styles.js" mode.', 'ckeditor_nxtclass');
					} elseif (FALSE !== strpos($new_options['css']['styles_path'], '"')) {
						$message['css_style_path'] = __('Double quotes are not allowed in path.', 'ckeditor_nxtclass');
					} elseif (substr($new_options['css']['style_path'], 0, 1) == "'" && substr($new_options['css']['style_path'], -1) == "'") {
						$message['css_style_path'] = __('Enter valid path, do not surround it with quotes.', 'ckeditor_nxtclass');
					}
				}
				/** load timeout * */
				if (!preg_match('#^\d+$#', trim($new_options['advanced']['load_timeout']))) {
					$massage['advanced_load_timeout'] = __('Enter valid load timeout in seconds.', 'ckeditor_nxtclass');
				}

				/* language settings */
				if (!preg_match('#^\d\d$#', trim($new_options['advanced']['language'])) && !preg_match('#^\d\d-\d\d$#', trim($new_options['advanced']['language']))) {
					$massage['advanced_language'] = __('Enter a valid language.', 'ckeditor_nxtclass');
				}

				if (trim($new_options['advanced']['detect_language_auto']) != 't' && trim($new_options['advanced']['detect_language_auto'] != 'f')) {
					$massage['advanced_detect_language_auto'] = __('Enter a valid auto detect language value.', 'ckeditor_nxtclass');
				}
				...
				if (trim($new_options['advanced']['language_direction']) != 'default' && trim($new_options['advanced']['language_direction']) != 'ltr' && trim($new_options['advanced']['language_direction']) != 'rtl') {
					$massage['advanced_language_direction'] = __('Enter a valid language direction value.', 'ckeditor_nxtclass');
				}
	...
	public function basic_settings_js() {
		nxt_enqueue_script('ckeditor', $this->ckeditor_path . 'ckeditor.js');
		nxt_enqueue_script('ckeditor_admin', $this->plugin_path . 'includes/basic.js');
	}

	public function file_editor_js() {
		/* codepress */
		//add_action('admin_print_footer_scripts', 'codepress_footer_js');
		//nxt_enqueue_script('codepress');
	}
	...
	public function remove_tinymce() {
		if (has_action('admin_print_footer_scripts', 'nxt_tiny_mce')) {
			remove_action('admin_print_footer_scripts', 'nxt_tiny_mce', 25);
		}
	}

	public function add_post_js() {
		if (has_filter('admin_print_footer_scripts', 'nxt_tiny_mce') || has_filter('before_nxt_tiny_mce', 'nxt_print_editor_js') || has_filter('after_nxt_tiny_mce', 'nxt_preload_dialogs')) {
			remove_filter('admin_print_footer_scripts', 'nxt_tiny_mce', 25);
			remove_filter('before_nxt_tiny_mce', 'nxt_print_editor_js');
			remove_filter('after_nxt_tiny_mce', 'nxt_preload_dialogs');
			$this->editor_js();
		}
	}
	...
	public function add_nxtcompat_styles() {
		?>
		<style type="text/css">
	...
	private function update_options($new_options, $error) {
		...
		if (!$error) {
			update_option("ckeditor_nxtclass", $options);
			echo '<div class="updated"><p>' . __('Configuration updated!') . '</p></div>';
		}
		return $options;
	}
	...
	private function generate_js_options($is_comment) {
		...
		switch ($options['css']['mode']) {
			case 'theme':
				...
				//fix for default NXTClass theme
		...
		switch ($options['css']['styles']) {
			case 'theme':
				$settings['stylesCombo_stylesSet'] = 'nxtclass:' . get_template_directory_uri() . '/ckeditor.styles.js';
				break;
			case 'self':
				$options['css']['style_path'] = trim(str_replace("%h%t", "%t", $options['css']['style_path']));
				$settings['stylesCombo_stylesSet'] = 'nxtclass:' . str_replace(array('%h', '%t'), array($_SERVER['HTTP_HOST'], get_template_directory_uri()), $options['css']['style_path']);
				break;
			default:
				$settings['stylesCombo_stylesSet'] = 'nxtclass:' . $this->plugin_path . 'ckeditor.styles.js';
				break;
		}
	...
	public function ckeditor_nxtmore_plugin($plugins) {
		$plugins['nxtmore'] = $this->plugin_path . 'plugins/nxtmore/';
		return $plugins;
	}

	public function ckeditor_nxtmore_button($buttons) {
		$buttons[] = array('nxtMore');
		return $buttons;
	}

	public function ckeditor_nxtgallery_plugin($plugins) {
		$plugins['nxtgallery'] = $this->plugin_path . 'plugins/nxtgallery/';
		return $plugins;
	}

	//filter to change data for nxteditimage plugin before insert/update in database
	...
	public function ckeditor_vvqbuttons($buttons) {
		...
		public function nxtpoll_external($plugins) {
				if (function_exists('poll_menu')) {
						$plugins['nxtpolls'] = $this->plugin_path . 'plugins/nxtpolls/';
				}
				return $plugins;
		}

	public function nxtpoll_buttons($buttons) {
		if (function_exists('poll_menu')) {
			$buttons[] = array('nxtPolls');
		}
		return $buttons;
	}
	...
	public function ckeditor_load_lang_options() {
		...
		if (DEFINED('nxt_PLUGIN_DIR')) {
			$lang_file = nxt_PLUGIN_DIR . '/' . basename(dirname(__FILE__)) . '/ckeditor/lang/_languages.js';
		} else {
			$lang_file = '../nxt-content/plugins/ckeditor-for-nxtclass/ckeditor/lang/_languages.js';
		}
...
final class _nxt_Editors {
	...
	public static function parse_settings($editor_id, $settings) {
		$set = nxt_parse_args($settings, array(
			'nxtautop' => true, // use nxtautop?
...
$ckeditor_nxtclass = ckeditor_nxtclass::getInstance();
?>
