	...
	if ( ckeditorSettings.qtransEnabled ){
		...
		if(typeof(window.switchEditors) != 'undefined') {
			window.switchEditors.go = function(id, lang) {
				...
				if(lang!='tmce' && lang!='html' && document.getElementById('qtrans_select_'+lang).className=='nxt-switch-editor switch-tmce switch-html') {
					return;
				}

				if(lang!='tmce' && lang!='html') {
					document.getElementById('qtrans_select_'+qtrans_get_active_language()).className='nxt-switch-editor';
					document.getElementById('qtrans_select_'+lang).className='nxt-switch-editor switch-tmce switch-html';
				}
				if(lang=='html') {
					if ( inst && inst.isHidden() )
						return false;
					...
					if(inst && ! inst.isHidden())
						return false;
					if ( tinyMCEPreInit.mceInit[id] && tinyMCEPreInit.mceInit[id].nxtautop )
						vta.value = this.nxtautop(qtrans_use(qtrans_get_active_language(),ta.value));
					if (inst) {
						inst.show();
					} else {
				...
				activeEditor: {
						...
						getNode : function(){
							...
							jQuery.each(obj,function(i, val){
								...
								jQuery.each(images, function(key, value){
									if (jQuery(value).hasClass('nxtGallery, cke_nxtgallery'))
									{
										index = i;
										return;
									}
					...
					dom :{
						select : function(selector) {
							...
							for (var i in images.$)
							{
								if ( typeof images.$[i] != 'undefined' && ((CKEDITOR.env.ie && images.$[i].className == 'nxtGallery, cke_nxtgallery') || images.$[i].classList == 'nxtGallery, cke_nxtgallery'))
								{
...
function afterCommandEvent(ev)
{
	if (ev.data.name != 'maximize') { return; }
		//if maximize button was clicked hide/show nxt admin bar - prevention of hiding buttton under nxt admin bar
		if (ev.data.command.state == CKEDITOR.TRISTATE_ON) { jQuery("#nxtadminbar").hide() } else { jQuery("#nxtadminbar").show() }
}