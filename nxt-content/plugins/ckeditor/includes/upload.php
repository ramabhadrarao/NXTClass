<div class="wrap">
	...
	<h2><?php _e('CKEditor - Upload Settings', 'ckeditor_nxtclass') ?></h2>
	<form method="post">
		<?php nxt_nonce_field('ckeditor_create_nonce_upload','csrf_ckeditor-for-nxtclass'); ?>
		<h3><?php _e('Upload Options', 'ckeditor_nxtclass') ?></h3>
		<table class="form-table">
			<tr valign="top">
				<th scope="row"><?php _e('File browser', 'ckeditor_nxtclass')?></th>
				<td>
					<select name="options[upload][browser]">
						<option value="disabled"<?php echo ($this->options['upload']['browser'] == 'disabled'?' selected="selected"':'') ?>><?php _e('Disabled', 'ckeditor_nxtclass'); ?></option>
						<option value="builtin"<?php echo ($this->options['upload']['browser'] == 'builtin'?' selected="selected"':'') ?>><?php _e('Built-in (old)', 'ckeditor_nxtclass'); ?></option>
			...
			<tr valign="top">
				<th scope="row"><?php _e('Store uploads in this folder', 'ckeditor_nxtclass')?></th>
				<td>
					...
					<div class="description"><?php _e('Default value:')?> nxt-content/uploads</div>
				</td>
			</tr>
			<tr valign="top">
				<th scope="row"><?php _e('Allowed file extensions', 'ckeditor_nxtclass') ?></th>
				...
				<div class="description"><?php _e('Allowed extensions separated by comma.', 'ckeditor_nxtclass');?></div>
				</td>
			</tr>
			<tr valign="top">
				<th scope="row"><?php _e('Allowed image extensions', 'ckeditor_nxtclass') ?></th>
			...
			<tr valign="top">
				<th scope="row"><?php _e('Allowed flash extensions', 'ckeditor_nxtclass') ?></th>
		...
		<h3><?php _e('CKFinder Options', 'ckeditor_nxtclass') ?></h3>
		<?php
		$checkCKFinder = $this->ckfinder_status();
		if (!strpos($checkCKFinder, "ckeditor_ok")){?>
				<div class="instruction_box">
						<b><?php _e('CKFinder status: ', 'ckeditor_nxtclass'); ?></b><?php echo $checkCKFinder; ?><br/><br/>
						<b><?php _e('Installation instruction:' , 'ckeditor_nxtclass');?></b><br/>
						<?php _e('1. Download CKFinder for PHP: ', 'ckeditor_nxtclass');?>
						<a href="http://ckfinder.com/download" target="_blank">http://ckfinder.com/download</a><br/>
						<?php _e('2. Unpack contents of the "ckfinder" folder to ', 'ckeditor_nxtclass');?>
						"nxt-content/plugins/ckeditor-for-nxtclass/ckfinder/" directory<br/>
						<?php _e('3. Rename ckfinder_config.php to config.php (overwrite default config.php distributed with CKFinder.)', 'ckeditor_nxtclass');?>
				</div>
		<?php } ?>
		<table class="form-table">
			<tr valign="top">
				<th scope="row"><?php _e('CKFinder access', 'ckeditor_nxtclass') ?></th>
				<td>
				<select name="options[ckfinder][permissions][access]"><?php nxt_dropdown_roles( $this->get_role('ckeditor_ckfinder_access') ); ?></select>
				<div class="description"><?php _e('The lowest role which should be able to access CKFinder', 'ckeditor_nxtclass');?></div>
				</td>
			</tr>
			<tr valign="top">
				<th scope="row"><?php _e('Maximum size of uploaded file', 'ckeditor_nxtclass') ?></th>
				<td>
				...
				<div class="description"><?php _e('Maximum size is defined in bytes, but shorthand notation may be also used. Available options are: G, M, K (case insensitive). 1M equals 1048576 bytes (one Megabyte), 1K equals 1024 bytes (one Kilobyte), 1G equals one Gigabyte.', 'ckeditor_nxtclass') ?></div>
				</td>
			</tr>
		</table>
		...
		<thead>
		<tr>
			<th colspan="4"><?php _e('Permissions', 'ckeditor_nxtclass') ?></th>
		</tr>
		</thead>
		<tbody>
		<tr>
		<tr valign="top">
			<th scope="row"><?php _e('Folder View');?></th><td><select name="options[ckfinder][permissions][folderView]"><?php nxt_dropdown_roles( $this->get_role('ckeditor_ckfinder_folderView') ); ?></select></td>
			<th scope="row"><?php _e('File View');?></th><td><select name="options[ckfinder][permissions][fileView]"><?php nxt_dropdown_roles( $this->get_role('ckeditor_ckfinder_fileView') ); ?></select></td>
		</tr>
		<tr valign="top">
			<th scope="row"><?php _e('Folder Create');?></th><td><select name="options[ckfinder][permissions][folderCreate]"><?php nxt_dropdown_roles( $this->get_role('ckeditor_ckfinder_folderCreate') ); ?></select></td>
			<th scope="row"><?php _e('File Upload');?></th><td><select name="options[ckfinder][permissions][fileUpload]"><?php nxt_dropdown_roles( $this->get_role('ckeditor_ckfinder_fileUpload') ); ?></select></td>
		</tr>
		<tr valign="top">
			<th scope="row"><?php _e('Folder Rename');?></th><td><select name="options[ckfinder][permissions][folderRename]"><?php nxt_dropdown_roles( $this->get_role('ckeditor_ckfinder_folderRename') ); ?></select></td>
			<th scope="row"><?php _e('File Rename');?></th><td><select name="options[ckfinder][permissions][fileRename]"><?php nxt_dropdown_roles( $this->get_role('ckeditor_ckfinder_fileRename') ); ?></select></td>
		</tr>
		<tr valign="top">
			<th scope="row"><?php _e('Folder Delete');?></th><td><select name="options[ckfinder][permissions][folderDelete]"><?php nxt_dropdown_roles( $this->get_role('ckeditor_ckfinder_folderDelete') ); ?></select></td>
			<th scope="row"><?php _e('File Delete');?></th><td><select name="options[ckfinder][permissions][fileDelete]"><?php nxt_dropdown_roles( $this->get_role('ckeditor_ckfinder_fileDelete') ); ?></select></td>
		</tr>
		</tbody>
		</table>
		...
		<thead>
		<tr>
			<th colspan="4"><?php _e('CKFinder License', 'ckeditor_nxtclass') ?></th>
		</tr>
		</thead>
		<tr>
		<tr valign="top">
		...
		<tr valign="top">
			<td colspan="4"><div class="description"><?php _e('Paste your CKFinder license key here. See <a href="http://ckfinder.com/license">CKFinder license</a>.', 'ckeditor_nxtclass'); ?></div></td>
		</tr>
		</table>
