...
/*
 ...
 Google XML Sitemaps Generator for NXTClass
 ==============================================================================
 
 This generator will create a sitemaps.org compliant sitemap of your NXTClass blog.
 ...
 Plugin URI: http://nxtclass.org/extend/plugins/google-xml-sitemaps-v3-for-qtranslate/
...
class GoogleSitemapGeneratorLoader {
	...
	/**
	 * Invokes the ShonxtingResult method of the generator
	 */
	function CallShonxtingResult() {
		if(GoogleSitemapGeneratorLoader::LoadPlugin()) {
			$gs = &GoogleSitemapGenerator::GetInstance();
			$gs->ShonxtingResult();
		}
	...
	/**
	 * Loads the actual generator class and tries to raise the memory and time limits if not already done by nxt
	 *
	 ...
	 *
	 * Uses the nxt API to get the meta data from the top of this file (comment)
	 *
	...
	function GetVersion() {
		if(!isset($GLOBALS["sm_version"])) {
			if(!function_exists('get_plugin_data')) {
				if(file_exists(ABSPATH . 'nxt-admin/includes/plugin.php')) require_once(ABSPATH . 'nxt-admin/includes/plugin.php'); //2.3+
				else if(file_exists(ABSPATH . 'nxt-admin/admin-functions.php')) require_once(ABSPATH . 'nxt-admin/admin-functions.php'); //2.1
				else return "0.ERROR";
			}
...
//Enable the plugin for the init hook, but only if nxt is loaded. Calling this php file directly will do nothing.
if(defined('ABSPATH') && defined('nxtINC')) {
