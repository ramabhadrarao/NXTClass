...
class GoogleSitemapGeneratorUI {
	...
	function HtmlShowOptionsPage() {
		...
		if(!empty($_REQUEST["sm_rebuild"])) { //Pressed Button: Rebuild Sitemap
			...
			if(function_exists('nxt_clear_scheduled_hook')) nxt_clear_scheduled_hook('sm_build_cron');
		...
		a.sm_pluginSupport {
			background-image:url(<?php echo $this->sg->GetPluginUrl(); ?>img/icon-nxtclass.gif);
		}
		...
		<?php
			...
			<?php elseif(version_compare($nxt_version,"2.5",">=")): ?>
				<style type="text/css">
					div#moremeta {
						float:right;
						width:200px;
		...
		<div class="wrap" id="sm_div">
			<form method="post" action="<?php echo $this->sg->GetBackLink() ?>">
				<h2><?php _e('XML Sitemap Generator for NXTClass', 'sitemap'); echo " " . $this->sg->GetVersion() ?> </h2>
				<?php
				...
				<?php if(version_compare($nxt_version,"2.5","<")): ?>
				...
				<?php endif; ?>
					...
					<?php $this->HtmlPrintBoxHeader('sm_basic_options',__('Basic Options', 'sitemap')); ?>
						...
						<ul>
							...
							<li>
								...
								<?php echo str_replace("%1",trailingslashit(get_bloginfo('url')) . "?sm_command=build&amp;sm_key=" . $this->sg->GetOption("b_manual_key"),__('This will allow you to refresh your sitemap if an external tool wrote into the NXTClass database without using the NXTClass API. Use the following URL to start the process: <a href="%1">%1</a> Please check the result box above to see if sitemap was successfully built.', 'sitemap')); ?>
								</span>
							</li>
						</ul>
						...
						<ul>
							...
							<li>
								...
								<br />
								<small><?php _e('The virtual robots.txt generated by NXTClass is used. A real robots.txt file must NOT exist in the blog directory!','sitemap'); ?></small>
							</li>
						</ul>
						...
						<ul>
							...
							<li>
								<label for="sm_b_safemode">
									<?php $forceSafeMode = (floatval($nxt_version)<2.2); ?>
									...
									<?php if($forceSafeMode):?> <br /><small><?php _e("Upgrade NXTClass at least to 2.2 to enable the faster MySQL access",'sitemap'); ?></small><?php endif; ?>
								</label>
							</li>
							<li>
								<label for="sm_b_auto_delay">
								<?php $forceDirect = (floatval($nxt_version) < 2.1);?>
									...
									<?php if($forceDirect):?> <br /><small><?php _e("Upgrade NXTClass at least to 2.1 to enable background building",'sitemap'); ?></small><?php endif; ?>
								</label>
							</li>
						</ul>
							...
							<ul>
								<li>
									<label for="sm_b_filename_manual">
										...
										echo "<br />";
										_e('Example','sitemap');
										echo ": /var/www/htdocs/nxtclass/sitemap.xml"; ?><br />
					...
					<?php $this->HtmlPrintBoxHeader('sm_includes',__('Sitemap Content', 'sitemap')); ?>
						<b><?php _e('NXTClass standard content', 'sitemap') ?>:</b>
						<ul>
							<li>
					...
					<?php $this->HtmlPrintBoxHeader('sm_excludes',__('Excluded items', 'sitemap')); ?>
						...
						<?php if(version_compare($nxt_version,"2.5.1",">=")): ?>
							...
							<ul>
								<?php nxt_category_checklist(0,0,$this->sg->GetOption("b_exclude_cats"),false); ?>
							</ul>
							</div>
						<?php else: ?>
							<ul><li><?php  echo sprintf(__("This feature requires at least NXTClass 2.5.1, you are using %s","sitemap"),$nxt_version); ?></li></ul>
						<?php endif; ?>
			...
			<form action="https://www.paypal.com/cgi-bin/webscr" method="post" id="sm_donate_form">
				<?php
					...
					if(!empty($nxtl)) {
						if(array_key_exists($nxtl,$lc)) $myLc = $lc[$nxtl];
						else {
