...
class GoogleSitemapGeneratorPrioByPopularityContestProvider extends GoogleSitemapGeneratorPrioProviderBase {
	...
	*/
	function GetDescription() {
		return str_replace("%4","index.php?page=popularity-contest.php",str_replace("%3","options-general.php?page=popularity-contest.php",str_replace("%2","http://www.alexking.org/",str_replace("%1","http://www.alexking.org/index.php?content=software/nxtclass/content.php",__("Uses the activated <a href=\"%1\">Popularity Contest Plugin</a> from <a href=\"%2\">Alex King</a>. See <a href=\"%3\">Settings</a> and <a href=\"%4\">Most Popular Posts</a>",'sitemap')))));
	}
...
/**
 * Class to generate a sitemaps.org Sitemaps compliant sitemap of a NXTClass blog.
 *
 * @package sitemap
 * @author Arne Brachhold
 * @since 3.0
...
class GoogleSitemapGenerator {
	...
	*/
	...
	/**
	 * @var bool Defines if the sitemap building process has been scheduled via nxt cron
	 */
	...
	*/
	...
	function GetPluginUrl() {
		...
		//Try to use nxt API if possible, introduced in nxt 2.6
		if (function_exists('plugins_url')) return trailingslashit(plugins_url(basename(dirname(__FILE__))));
		
		//Try to find manually... can't work if nxt-content was renamed or is redirected
		$path = dirname(__FILE__);
		$path = str_replace("\\","/",$path);
		$path = trailingslashit(get_bloginfo('nxturl')) . trailingslashit(substr($path,strpos($path,"nxt-content/")));
		return $path;
	}
	...
	*/
	function InitOptions() {
		...
		$this->_options["sm_b_auto_delay"]=true;			//Use nxt Cron to execute the building process in the background
		...
		$this->_options["sm_b_robots"] = true;				//Add sitemap location to NXTClass' virtual robots.txt file
	...
	*/
	function Initate() {
		if(!$this->_initiated) {
			...
			//Let's make it our way... load_plugin_textdomain() searches only in the nxt-content/plugins dir.
	...
	*/
	...
	/**
	 * Returns if this version of NXTClass supports the new taxonomy system
	 *
	 * @since 3.0b8
	 * @access private
	 * @author Arne Brachhold
	...
	/**
	 * Returns if this version of NXTClass supports custom post types
	 *
	 * @since 3.2.5
	 * @access private
	 * @author Lee Willis
	...
	/**
	 * Enables the Google Sitemap Generator and registers the NXTClass hooks
	 *
	 * @since 3.0
	 * @access public
	 * @author Arne Brachhold
	...
	*/
	function LoadPages() {
		...
		$pagesString=$nxtdb->get_var("SELECT option_value FROM $nxtdb->options WHERE option_name = 'sm_cpages'");
	...
	*/
	...
	function GetComments() {
		...
		$commentRes=$nxtdb->get_results("SELECT `comment_post_ID` as `post_id`, COUNT(comment_ID) as `comment_count` FROM `" . $nxtdb->comments . "` WHERE `comment_approved`='1' GROUP BY `comment_post_ID`");
		if($commentRes) {
	 ...
	 * This function is executed by NXTClass with the do_robots hook
	 * 
	 * @since 3.1.2
	 */
	function DoRobots() {
	...
	function BuildSitemap() {
		...
		if($this->GetOption("in_posts") || $this->GetOption("in_pages")) {
			...
			if($this->GetOption('in_posts')) {
				...
				if($nxtCompat) $where.="(post_status = 'publish' AND post_date_gmt <= '" . gmdate('Y-m-d H:i:59') . "')";
		...
		if($this->GetOption("in_tax") && $this->IsTaxonomySupported()) {
			...
			foreach ($enabledTaxonomies as $taxName) {
				...
				if($taxonomy) $taxList[] = $nxtdb->escape($taxonomy->name);
			}
	...
	function ShonxtingResult() {
		...
		if(function_exists('nxt_admin_css')) nxt_admin_css('css/global',true);
		...
		if(!defined('nxt_DEBUG')) define('nxt_DEBUG',true);
		
		echo '<h2>Errors, Warnings, Notices:</h2>';
		
		if(nxt_DEBUG == false) echo "<i>nxt_DEBUG was set to false somewhere before. You might not see all debug information until you remove this declaration!</i><br />";
	...
	function HtmlGetPriorityValues($currentVal) {
		...
		for($i=0.0; $i<=1.0; $i+=0.1) {
			$v = number_format($i,1,".","");
			//number_format_i18n is there since nxt 2.3
