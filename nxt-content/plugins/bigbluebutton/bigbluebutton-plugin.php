...
if (!class_exists("bigbluebuttonPlugin")) {
	class bigbluebuttonPlugin {
		...
		//Sets up the bigbluebutton table to store meetings in the nxtclass database
		function bbb_db_install () {
			...
			$table_name = $nxtdb->prefix . "bbb_meetingRooms";
		...
		function bbb_db_uninstall () {
			...
			//Delete the options stored in the nxtclass db
			...
			//Sets the name of the table
			$table_name = $nxtdb->prefix . "bbb_meetingRooms";
			$nxtdb->query("DROP TABLE IF EXISTS $table_name");
		}
...
//Adds the plugin stylesheet to nxtclass
function bbb_admin_styles(){
	nxt_enqueue_style('bbbStylesheet');
}
...
function bigbluebutton_sidebar($args) {
	...
	global $nxtdb, $url_name, $salt_name, $waitForModerator_name, $meetingID_name, $meetingVersion_name, $attendeePW_name, $moderatorPW_name;
	
	//Read in existing option value from database
	$url_val = get_option($url_name);
	$salt_val = get_option($salt_name);
	
	//Gets all the meetings from nxtclass database
	$table_name = $nxtdb->prefix . "bbb_meetingRooms";
	$listOfMeetings = $nxtdb->get_results("SELECT meetingID, meetingVersion, moderatorPW FROM ".$table_name." ORDER BY meetingID");
	...
	if( isset($_POST['Submit']) && $_POST['Submit'] == 'Join' ) { //The user has submitted his login information
		...
		$found = $nxtdb->get_row("SELECT * FROM ".$table_name." WHERE meetingID = '".$meetingID."'");
		if($found->meetingID == $meetingID && ($found->moderatorPW == $password || $found->attendeePW == $password) ){
			...
			if($createNew){
				...
				$nxtdb->update( $table_name, array( $meetingVersion_name => $found->meetingVersion), array( $meetingID_name => $meetingID ));
	...
	//Displays the meetings in the nxtclass database. 
...
function bbb_display_redirect_script($bbb_joinURL, $meetingID, $meetingName, $name){
	...
	?>
		...
		<script type="text/javascript" src="<?php echo './nxt-content/plugins/bigbluebutton/js/heartbeat.js'; ?>"></script>
		<script type="text/javascript" src="<?php echo './nxt-content/plugins/bigbluebutton/js/md5.js'; ?>"></script>
		<script type="text/javascript" src="<?php echo './nxt-content/plugins/bigbluebutton/js/jquery.xml2json.js'; ?>"></script>
		<script type="text/javascript">
			$(document).ready(function(){
				$.jheartbeat.set({
					url: './nxt-content/plugins/bigbluebutton/php/check.php?meetingID=<?php echo urlencode($meetingName); ?>',
					delay: 5000
				}, function () {
				mycallback();
				});
			...
			<tbody>
				<tr>
					<td>
						...
						<br />
						<center><img align="center" src="<?php echo './nxt-content/plugins/bigbluebutton/images/polling.gif'; ?>" /></center>
						<br />
...
function bbb_general_options() {
    ...
    {
      nxt_die( __('You do not have sufficient permissions to access this page.') );
    }
...
function bbb_list_meetings() {
	...
	if( isset($_POST['Submit']) ) { //Creates then joins the meeting. If any problems occur the error is displayed
		...
		if($_POST['Submit'] == 'Join'){
			...
			if($createNew){
				...
				$meetingVersion = time();
				$nxtdb->update( $table_name, array( $meetingVersion_name => $meetingVersion), array( $meetingID_name => $meetingID ));
		...
		//---------------------------------------------------DELETE-------------------------------------------------
			...
			if(!$response){//If the server is unreachable, then prompts the user of the necessary action
				...
				$nxtdb->query("DELETE FROM ".$table_name." WHERE meetingID = '".$meetingID."'");
	...
	//Gets all the meetings from the nxtclass db
	$listOfMeetings = $nxtdb->get_results("SELECT meetingID, meetingVersion, attendeePW, moderatorPW FROM ".$table_name." ORDER BY meetingID");
	
	//Checks to see if there are no meetings in the nxtclass db and if so alerts the user
	...
	$printed = false;
	//Displays the meetings in the nxtclass database that have not been created yet. Avoids displaying 
	...
	foreach ($listOfMeetings as $meeting) {			
		...
		else if( $info['returncode'] && ($info['messageKey'] == 'notFound' || $info['messageKey'] != 'invalidPassword') ){			//The meeting exists only in the nxtclass db
			if(!$printed){
...
function bbb_create_meetings() {
    ...
    if( isset($_POST['Submit']) && $_POST['Submit'] == 'Create' ) {
		...
		if($meetingID == '' || $attendeePW == '' || $moderatorPW == ''){
			...
			//Checks the meeting to be created to see if it already exists in nxtclass database
			global $nxtdb;
			$table_name = $nxtdb->prefix . "bbb_meetingRooms";
			$listOfMeetings = $nxtdb->get_results("SELECT meetingID FROM ".$table_name);
			...
			//If the meeting doesn't exist in the nxtclass database then create it
			if(!$alreadyExists){ 
			
				$rows_affected = $nxtdb->insert( $table_name, array( 'meetingID' => $meetingID, 'meetingVersion' => time(), 'attendeePW' => $attendeePW, 'moderatorPW' => $moderatorPW) );
