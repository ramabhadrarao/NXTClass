...
/*
 Plugin Name: OpenID
 Plugin URI: http://nxtclass.org/extend/plugins/openid
 Description: Allows the use of OpenID for account registration, authentication, and commenting.  Also includes an OpenID provider which can turn NXTClass author URLs into OpenIDs.
...
function get_user_openids($id_or_name = null) {
	...
	if ( $user ) {
		global $nxtdb;
		return $nxtdb->get_col( $nxtdb->prepare('SELECT url FROM '.openid_identity_table().' WHERE user_id = %s', $user->ID) );
	} else {
		return array();
	}
}
...
function get_user_by_openid($url) {
	global $nxtdb;
	return $nxtdb->get_var( $nxtdb->prepare('SELECT user_id FROM '.openid_identity_table().' WHERE url = %s', $url) );
}
 ...
 * actual plugin files do not reside within the NXTClass directory on the filesystem (such as 
