...
// -- nxt Hooks
// Add hooks to handle actions in NXTClass
add_action( 'init', 'openid_textdomain' ); // load textdomain

// include internal stylesheet
add_action( 'nxt', 'openid_style');
...
if (isset($nxtmu_version)) {
	// nxtmu doesn't support non-autoload options
 ...
 *
 * @return NXTClassOpenID_Store internal SQL store
 */
function openid_getStore() {
	static $store;

	if (!$store) {
		$store = new NXTClass_OpenID_OptionStore();
	}
...
function openid_activate_plugin() {
	...
	// flush rewrite rules
	if ( !isset($nxt_rewrite) ) { $nxt_rewrite = new nxt_Rewrite(); }
...
function openid_customer_error_handler($errno, $errmsg, $filename, $linenum, $vars) {
	...
	if (!defined('nxt_DEBUG') || !(nxt_DEBUG)) {
		// XML errors
...
/**
 * Generate a unique NXTClass username for the given OpenID URL.
 *
 ...
 *  - substitute certain characters which are not allowed by NXTClass
 *
...
function openid_create_new_user($identity_url, &$user_data) {
	...
	@include_once( ABSPATH . 'nxt-admin/upgrade-functions.php');	// 2.1
	@include_once( ABSPATH . nxtINC . '/registration-functions.php'); // 2.0.4
	...
	if( $user_id ) { // created ok
		...
		if ( !$user->has_cap('edit_posts') ) $redirect_to = '/nxt-admin/profile.php';
 ...
 *
 * Apache's rewrite module is often used to produce "pretty URLs" in NXTClass.  
...
function openid_clean_request() {
	...
	if (array_key_exists('q', $_GET)) {
		...
		// handle lighttpd hack which uses a custom error-handler, passing 404 errors to NXTClass.  
...
function openid_service_url($service, $scheme = null) {
	global $nxt_rewrite;
	if (!$nxt_rewrite) $nxt_rewrite = new nxt_Rewrite();
...
/**
 * Add rewrite rules to nxt_Rewrite for the OpenID services.
 */
function openid_rewrite_rules($nxt_rewrite) {
	$openid_rules = array(
		'openid/(.+)' => 'index.php?openid=$matches[1]',
	);

	$nxt_rewrite->rules = $openid_rules + $nxt_rewrite->rules;
}


/**
 * Add valid query vars to NXTClass for OpenID.
 */
...
function openid_js_setup() {
	if (have_comments() || comments_open() || is_admin()) {
		nxt_enqueue_script('openid', plugins_url('openid/f/openid.js'), array('jquery'), OPENID_PLUGIN_REVISION);
	}
}
...
/**
 * When a NXTClass user is deleted, make sure all associated OpenIDs are deleted as well.
 */
...
function openid_add_identity($user_id, $url) {
	global $nxtdb;
	$sql = $nxtdb->prepare('INSERT INTO ' . openid_identity_table() . ' (user_id,url,hash) VALUES ( %s, %s, MD5(%s) )', $user_id, $url, $url);
	return $nxtdb->query( $sql );
}
...
function openid_drop_identity($user_id, $identity_url) {
	global $nxtdb;
	return $nxtdb->query( $nxtdb->prepare('DELETE FROM '.openid_identity_table().' WHERE user_id = %s AND url = %s', $user_id, $identity_url) );
}
...
function openid_drop_all_identities($user_id) {
	global $nxtdb;
	return $nxtdb->query( $nxtdb->prepare('DELETE FROM '.openid_identity_table().' WHERE user_id = %s', $user_id ) );
}
