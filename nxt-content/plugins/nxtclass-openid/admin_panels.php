...
function openid_admin_panels() {
	...
	add_action("load-$hookname", create_function('', 'nxt_enqueue_script("admin-forms");'));
	...
	if ( function_exists('is_site_admin') ) {
		// add OpenID options to nxtMU Site Admin page
		add_action('nxtmu_options', 'openid_nxtmu_options');
		add_action('update_nxtmu_options', 'openid_update_nxtmu_options');
	} else {
...
function openid_options_page() {
	...
	<div class="wrap">
		<form method="post" action="options.php">
			...
			. 'local NXTClass username and password, or by delegating to another OpenID Provider.', 'openid'); ?></p>
			...
			<?php
				...
				$users = array_filter($users, create_function('$u', '$u = new nxt_User($u->user_id); return $u->has_cap("use_openid_provider");'));
				...
				if (defined('OPENID_DISALLOW_OWNER') && OPENID_DISALLOW_OWNER) {
					echo '
						<p class="error">' . __('A Blog Owner cannot be set for this blog.  To set a Blog Owner, '
							. 'first remove the following line from your <code>nxt-config.php</code>:', 'openid') 
			...
			<table class="form-table optiontable editform">
				<tr valign="top">
					...
					<td>
						...
						'<a href="' . nxt_nonce_url(add_query_arg('action', 'rebuild_tables'), 'rebuild_tables') . '">', '</a>'); ?></p>
					</td>
				</tr>
			</table>
...
function openid_profile_panel() {
	...
	<div class="wrap">
		<form action="<?php printf('%s?page=%s', $_SERVER['PHP_SELF'], $_REQUEST['page']); ?>" method="post">
			...
			. 'allow you to login to NXTClass with your OpenID instead of a username and password.  '
		...
		<div class="tablenav">
			<div class="alignleft actions">
				...
				<?php nxt_nonce_field('openid-delete_openids'); ?>
			</div>
			<div class="clear"></div>
		</div>
		...
		<?php nxt_nonce_field('openid-add_openid'); ?>
		<p class="submit">
...
function openid_manage_trusted_sites() {
	...
	<div class="wrap">
		...
		<form method="post">
			<div class="tablenav">
				<div class="alignleft actions">
					...
					<input type="submit" value="<?php _e('Apply'); ?>" name="doaction" id="doaction" class="button-secondary action" />
					<?php nxt_nonce_field('openid-delete_trusted_sites'); ?>
				</div>
				<div class="clear"></div>
			</div>
		...
		<form method="post">
			...
			<?php nxt_nonce_field('openid-add_trusted_sites'); ?>
...
function openid_printSystemStatus() {
	...
	$status[] = array( 'NXTClass version', 'info', $nxt_version );
	$status[] = array( 'PHP OpenID Library Version', 'info', Auth_OpenID_VERSION );
	$status[] = array( 'MySQL version', 'info', function_exists('mysql_get_client_info') ? mysql_get_client_info() : 'Mysql client information not available. Very strange, as NXTClass requires MySQL.' );

	$status[] = array('NXTClass\' table prefix', 'info', isset($nxtdb->base_prefix) ? $nxtdb->base_prefix : $nxtdb->prefix );
	...
	if (function_exists('xrds_meta')) {
		...
		$status[] = array( 'XRDS-Simple', false, '<a href="http://nxtclass.org/extend/plugins/xrds-simple/">XRDS-Simple</a> plugin is not installed.  Some features may not work properly (including providing OpenIDs).');
	}
...
function openid_profile_delete_openids($delete) {
	...
	if (sizeof($urls) == sizeof($delete) && !@$_REQUEST['confirm']) {
		...
		$html .= nxt_nonce_field('openid-delete_openids', '_nxtnonce', true, false) . '
	...
	if ($count) {
		...
		@include_once(ABSPATH . nxtINC . '/registration.php');	// nxt < 2.3
		@include_once(ABSPATH . 'nxt-admin/includes/admin.php');	// nxt >= 2.3

		if (!openid_ensure_url_match($user)) {
			$identities = get_user_openids($user->ID);
			nxt_update_user( array('ID' => $user->ID, 'user_url' => $identities[0]) );
...
function openid_personal_options_update() {
	...
	if (!openid_ensure_url_match($user, $_POST['url'])) {
		nxt_die(sprintf(__('For security reasons, your profile URL must be one of your claimed OpenIDs: %s', 'openid'),
...
/**
 * Add OpenID options to the NXTClass user profile page.
 */
...
/**
 * Update OpenID options set from the NXTClass user profile page.
 */
...
/**
 * Add OpenID options to the NXTClass MU site options page.
 */
function openid_nxtmu_options() {
...
/**
 * Update the OpenID options set from the NXTClass MU site options page.
 */
function openid_update_nxtmu_options() {
...
/**
 * Add OpenID options to the NXTClass general settings page.
 */
...
/**
 * Add OpenID options to the NXTClass discussion settings page.
 */
