...
add_action( 'nxt_head', 'openid_provider_link_tags');
...
function openid_provider_xrds_simple($xrds) {
	...
	if (!$nxt_roles) $nxt_roles = new nxt_Roles();
...
function openid_server_requested_user() {
	...
	if (array_key_exists('author', $_REQUEST) && $_REQUEST['author']) {
		...
		$regex = preg_replace('/%author%/', '(.+)', $nxt_rewrite->get_author_permastruct());
		...
		if ($request->mode != 'checkid_immediate') {
			if ($_REQUEST['action'] == 'cancel') {
				...
				echo '
					...
					. nxt_nonce_field('openid-server_cancel', '_nxtnonce', true, false)
					.'</form>';

				$html = ob_get_contents();
				ob_end_clean();
				nxt_die($html, 'OpenID Login Error');
			}
		}
	}
