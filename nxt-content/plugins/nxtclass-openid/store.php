 ...
 * store.php
 *
 * Database Connector for NXTClass OpenID
...
if (!class_exists('NXTClass_OpenID_OptionStore')):
/**
 * OpenID store that uses the NXTClass options table for storage.  Originally 
 * written by Simon Willison for use in the mu-open-id plugin.  Modified a fair 
 * amount for use in NXTClass OpenID.
 */
class NXTClass_OpenID_OptionStore extends Auth_OpenID_OpenIDStore {
	...
	function storeAssociation($server_url, $association) {
		...
		nxt_cache_delete('openid_associations', 'options');
...
function openid_check_tables($retry=true) {
	...
	foreach( $tables as $t ) {
		if( $nxtdb->get_var("SHOW TABLES LIKE '$t'") != $t ) {
			$ok = false;
...
/**
 * Create OpenID related tables in the NXTClass database.
 */
function openid_create_tables()
{
	...
	// Create the SQL and call the nxt schema upgrade function
	$statements = array(
...
function openid_delete_tables() {
	global $nxtdb;
	$nxtdb->query('DROP TABLE IF EXISTS ' . openid_identity_table());
	$nxtdb->query( $nxtdb->prepare('DELETE FROM ' . $nxtdb->postmeta . ' WHERE meta_key=%s', 'openid_comments') );
	
	// old database changes... just to make sure
	$nxtdb->query('DROP TABLE IF EXISTS ' . openid_table_prefix(true) . 'openid_nonces');
	$nxtdb->query('DROP TABLE IF EXISTS ' . openid_table_prefix(true) . 'openid_associations');

	// clear old way of storing OpenID comments
	$openid_column = $nxtdb->get_row('SHOW COLUMNS FROM ' . openid_table_prefix(true) . 'comments LIKE "openid"');
	if ($openid_column) {
		$nxtdb->query('ALTER table ' . $comments_table . ' DROP COLUMN openid');
		$nxtdb->query( $nxtdb->prepare('UPDATE ' . $comments_table . ' SET comment_type=%s WHERE comment_type=%s', '', 'openid') );
	}
}
...
function openid_migrate_old_data() {
	...
	if ($openid_column) {
		...
		$comment_data = $nxtdb->get_results( $nxtdb->prepare('SELECT comment_ID, comment_post_ID from ' . $comments_table . ' WHERE openid=%s OR comment_type=%s', 1, 'openid') );
	...
	$nxtdb->query( $nxtdb->prepare('DELETE FROM ' . $usermeta_table . ' WHERE meta_key=%s OR meta_key=%s', 'has_openid', 'registered_with_openid') );
}

function openid_table_prefix($blog_specific = false) {
	global $nxtdb;
	if (isset($nxtdb->base_prefix)) {
		return $nxtdb->base_prefix . ($blog_specific ? $nxtdb->blogid . '_' : '');
	} else {
		return $nxtdb->prefix;
	}
}
