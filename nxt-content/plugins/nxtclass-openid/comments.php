...
if( get_option('openid_enable_commentform') ) {
	add_action( 'nxt', 'openid_js_setup', 9);
	add_action( 'nxt_footer', 'openid_comment_profilelink', 10);
...
function openid_option_require_name_email( $value ) {
	...
	$comment_page = (defined('OPENID_COMMENTS_POST_PAGE') ? OPENID_COMMENTS_POST_PAGE : 'nxt-comments-post.php');
 ...
 * may be an i-name, but NXTClass is expecting the comment URL cookie to be a valid URL.
 *
 * @nxtclass-action sanitize_comment_cookies
 */
...
?>
		...
		<script type="text/javascript">jQuery(function(){ add_openid_to_comment_form('<?php echo site_url('index.php') ?>', '<?php echo nxt_create_nonce('openid_ajax') ?>') })</script>
<?php
	}
}
...
function openid_recent_comments() {
	...
	if ( $nxt_widget_factory && array_key_exists('nxt_Widget_Recent_Comments', $nxt_widget_factory->widgets) ) {
		// this is an ugly hack because remove_action doesn't actually work the way it should with objects
		foreach ( array_keys($GLOBALS['nxt_filter']['nxt_head'][10]) as $key ) {
			if ( strpos($key, 'nxt_Widget_Recent_Commentsrecent_comments_style') === 0 ) {
				remove_action('nxt_head', $key);
				return;
			}
		}
	}
