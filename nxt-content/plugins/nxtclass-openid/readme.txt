...
Allows NXTClass to provide and consumer OpenIDs for authentication of users and comments.
...
their local NXTClass account using an OpenID, as well as enabling commenters
...
own personal NXTClass account. [XRDS-Simple][] is required for the OpenID
...
[XRDS-Simple]: http://nxtclass.org/extend/plugins/xrds-simple/
[dev-doc]: http://wiki.diso-project.org/nxtclass-openid-api

== Installation ==

This plugin follows the [standard NXTClass installation method][]:

1. Upload the `openid` folder to the `/nxt-content/plugins/` directory
...
[standard NXTClass installation method]: http://codex.nxtclass.org/Managing_Plugins#Installing_Plugins
...
[ioni2]: http://ioni2.com/2009/nxtclass-openid-login-failed-invalid-openid-mode-no-mode-set-solved-for-both-nxtclass-and-drupal/


= How do I use SSL for OpenID transactions? =

First, be aware that this only works in NXTClass 2.6 and up.  Make sure you've
turned on SSL in NXTClass by [defining either of the following][nxt-ssl]
globals as "true" in your `nxt-config.php` file:

 - FORCE\_SSL\_LOGIN
 - FORCE\_SSL\_ADMIN

Then, also define the following global as "true" in your `nxt-config.php` file:
...
4. Users can add additional OpenIDs which they can use to login to NXTClass
...
= version 3.3.3 (Aug 24, 2010) =
 ...
 - fix a few PHP and NXTClass warnings and notices
...
= version 3.3 (Sep 28, 2009) =
 - minimum required version has been bumped to NXTClass 2.8
 - fix support for NXTClass MU
 - new, less obtrusive UI for comment form.  Should also work with all themes in some form (with or without js).
 - many administrative options have been moved to their respective locations on core NXTClass Settings pages
...
= version 3.2.2 (Mar 19, 2009) =
 ...
 - UI tweaks to better match nxt 2.7
...
= version 3.2 (Jan 20, 2009) =
 - add uninstall hook for NXTClass 2.7 -- this will remove all traces of the plugin from the database
 - UI fixes for NXTClass 2.7
 - add settings link to plugins page
 - silence XML parsing errors with PHP4
 - ensure nxt\_scripts is set
...
= version 2.2.0 (Jul 23, 2008) =
 ...
 - fixed openid spoofing vulnerability (http://plugins.trac.nxtclass.org/ticket/702)
...
= version 2.1.8 (Apr 02, 2008) =
 - fix UI issue with nxt-login.php page in nxt2.5
 - fix bug printing supported curl protocols (http://nxtclass.org/support/topic/159062)
 - fix jquery bug while adding category in  nxt2.5  (http://nxtclass.org/support/topic/164305)
...
= version 2.1 =
 ...
 - better tracking of which users have OpenIDs linked to their local nxt account
...
= version 2.0 =
 - simplified admin interface by using reasonable defaults.  Default behaviors include:
  - "unobtrusive mode"
  - always add openid to nxt-login.php
  - always use nxt option 'home' for the trust root
 - new features
...
= version 1.0.1 =
 - added nxtclass.org style readme.txt
 
= version 1.0 (also known as r13) =

Full SVN logs are available at <http://dev.nxt-plugins.org/log/openid/>.

[729]: http://dev.nxt-plugins.org/ticket/729
[openid.pl]: http://openid.pl/

The original OpenID plugin for NXTClass was a collaborative effort between Alan Castonguay and Hans Granqvist.
