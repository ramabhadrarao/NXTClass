<?php
/**
 * All the code required for handling logins via nxt-login.php.  These functions should not be considered public, 
 * and may change without notice.
 */


add_action( 'login_head', 'openid_nxt_login_head');
add_action( 'login_form', 'openid_nxt_login_form');
add_action( 'register_form', 'openid_nxt_register_form', 9);
...
function openid_finish_login($identity_url, $action) {
	...
	$url = add_query_arg( array( 
		...
		'_nxtnonce' => nxt_create_nonce('openid_login_' . md5($identity_url)), 
	), $url);
 ...
 */
function openid_nxt_login_head() {
	openid_style();
}


/**
 * Add OpenID input field to nxt-login.php
 *
 * @action: login_form
 **/
function openid_nxt_login_form() {
...
/**
 * Add information about registration to nxt-login.php?action=register 
 *
 * @action: register_form
 **/
function openid_nxt_register_form() {
	echo '
...
/**
 * Handle NXTClass registration errors.
 */
