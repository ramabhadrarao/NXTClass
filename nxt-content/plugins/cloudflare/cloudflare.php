...
Plugin URI: http://www.cloudflare.com/wiki/CloudFlareNXTClassPlugin
...
Plugin adapted from the Akismet nxt plugin.
...
function cloudflare_init() {
    ...
    // Let people know that the CF nxt plugin is turned on.
    if (!headers_sent()) {
        header("X-CF-Powered-By: nxt " . CLOUDFLARE_VERSION);
    }
...
function cloudflare_conf() {
	...
	if ( isset($_POST['submit']) 
        ...
        if(current_user_can('administrator')) {
            ...
            $tables = $nxtdb->get_col("SHOW TABLES");
            foreach($tables as $table_name) {
                $optimize = $nxtdb->query("OPTIMIZE TABLE `$table_name`");
                $analyze = $nxtdb->query("ANALYZE TABLE `$table_name`");
                if (!$optimize || !$analyze) {
    ...
    <h4><?php _e('CLOUDFLARE nxtclass PLUGIN:'); ?></h4>
        <?php //    <div class="narrow"> ?>

CloudFlare has developed a plugin for NXTClass. By using the CloudFlare NXTClass Plugin, you receive: 
<ol>
<li>Correct IP Address information for comments posted to your site</li>
<li>Better protection as spammers from your NXTClass blog get reported to CloudFlare</li>
<li>Optimization of your server database (optional)</li>
</ol>

<h4>VERSION COMPATIBILITY:</h4>

The plugin is compatible with NXTClass version 2.8.6 and later. The plugin will not install unless you have a compatible platform.
...
<li>We recommend that any user on CloudFlare with NXTClass use this plugin. </li>
...
// if ($is_cf) {
    ...
    <?php nxt_nonce_field('cloudflare-db-api','cloudflare-db-api-nonce'); ?>
    ...
    <?php nxt_nonce_field('cloudflare-db-opt','cloudflare-db-opt-nonce'); ?>
    ...
    <input type="submit" name="submit" value="<?php _e('Run the optimizer'); ?>" /> (<?php _e('<a href="http://www.cloudflare.com/wiki/NXTClassDBOptimizer">What is this?</a>'); ?>)</h4>
...
function cloudflare_set_comment_status($id, $status) {
    ...
    // ajax/external-event.html?email=ian@cloudflare.com&t=94606855d7e42adf3b9e2fd004c7660b941b8e55aa42d&evnt_v={%22dd%22:%22d%22}&evnt_t=nxt_SPAM
...
add_action('nxt_set_comment_status', 'cloudflare_set_comment_status', 1, 2);
