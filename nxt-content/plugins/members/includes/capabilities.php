 ...
 *
 * @since 0.1.0
 * @uses members_get_default_capabilities() Gets an array of nxt's default capabilities.
...
function members_get_capabilities() {
	...
	/* Merge all the capability arrays (current role caps, plugin caps, and default nxt caps) together. */
 ...
 * folks accidentally deleting the default NXTClass capabilities, the members_default_capabilities() will 
 * return all the defaults.
 *
 * @since 0.1.0
 * @return array $capabilities All the capabilities of all the user roles.
 * @global array $nxt_roles Holds all the roles for the installation.
 */
function members_get_role_capabilities() {
	...
	foreach ( $nxt_roles->role_objects as $key => $role ) {
 ...
 * useful NXTClass capability from all roles.  When this happens, the capability is no longer stored in any of 
 * the roles, so it basically doesn't exist.  This function will house all of the default NXTClass capabilities in 
 ...
 * @link http://codex.nxtclass.org/Roles_and_Capabilities#Capabilities
 *
 * @since 0.1
 * @return array $defaults All the default NXTClass capabilities.
 */
function members_get_default_capabilities() {

	/* Create an array of all the default NXTClass capabilities so the user doesn't accidentally get rid of them. */
	$defaults = array(
		'activate_plugins',
		'add_users',
		'create_users',
...
/**
 * Old NXTClass levels system.  This is mostly useful for filtering out the levels when shown in admin 
