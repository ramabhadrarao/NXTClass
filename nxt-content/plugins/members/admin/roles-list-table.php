...
/* Get the global $nxt_roles variable. */
global $nxt_roles;
...
/* If viewing the regular role list table. */
	...
	/* Set up the roles array. */
	$list_roles = $nxt_roles->role_names;
...
<div class="wrap">
	...
	<div id="poststuff">
		...
		<form id="roles" action="<?php echo $current_page; ?>" method="post">
			...
			<?php nxt_nonce_field( members_get_nonce( 'edit-roles' ) ); ?>
				...
				<?php foreach ( $list_roles as $role => $name ) { ?>
					...
					<tr valign="top" class="<?php echo ( isset( $active_roles[$role] ) ? 'active' : 'inactive' ); ?>">
						...
						<td class="plugin-title">
							...
							<?php $edit_link = admin_url( nxt_nonce_url( "users.php?page=roles&amp;action=edit&amp;role={$role}", members_get_nonce( 'edit-roles' ) ) ); ?> 
							...
							<div class="row-actions">
								...
								<?php if ( ( is_multisite() && is_super_admin() && $role !== get_option( 'default_role' ) ) || ( current_user_can( 'delete_roles' ) && $role !== get_option( 'default_role' ) && !current_user_can( $role ) ) ) { ?>
									| <a href="<?php echo admin_url( nxt_nonce_url( "users.php?page=roles&amp;action=delete&amp;role={$role}", members_get_nonce( 'edit-roles' ) ) ); ?>" title="<?php printf( esc_attr__( 'Delete the %s role', 'members' ), $name ); ?>"><?php _e( 'Delete', 'members' ); ?></a>
								<?php } ?>
