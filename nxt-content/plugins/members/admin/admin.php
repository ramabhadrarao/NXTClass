...
function members_admin_contextual_help( $text, $screen ) {
	...
	if ( 'settings_page_members-settings' == $screen ) {
		...
		$text .= '<p>' . __( '<strong>Role Manager:</strong> This feature allows you to manage roles on your site by giving you the ability to create, edit, and delete any role. Note that changes to roles do not change settings for the Members plugin. You are literally changing data in your NXTClass database. This plugin feature merely provides an interface for you to make these changes.', 'members' ) . '</p>';
	...
	elseif ( 'users_page_roles' == $screen ) {
		...
		$text .= '<ul>';
		$text .= '<li><a href="http://justintadlock.com/archives/2009/08/30/users-roles-and-capabilities-in-nxtclass">' . __( 'Users, Roles, and Capabilities', 'members' ) . '</a></li>';
		$text .= '<li><a href="' . MEMBERS_URI . 'docs/readme.html">' . __( 'Documentation', 'members' ) . '</a></li>';
		$text .= '<li><a href="http://themehybrid.com/support">' . __( 'Support Forums', 'members' ) . '</a></li>';
		$text .= '</ul>';
	}
...
/**
 * Returns an array of all the user meta keys in the $nxtdb->usermeta table.
 *
 * @since 0.2.0
...
function members_get_user_meta_keys() {
	...
	$keys = $nxtdb->get_col( "SELECT meta_key FROM $nxtdb->usermeta GROUP BY meta_key ORDER BY meta_key" );
