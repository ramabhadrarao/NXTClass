...
/*
  ...
  Plugin URI: http://jasonpenney.net/nxtclass-plugins/use-google-libraries/
  Description:Allows your site to use common javascript libraries from Google's AJAX Libraries CDN, rather than from nxtclass's own copies.
  Version: 1.2.1
  Author: Jason Penney
...
if ( !class_exists( 'JCP_UseGoogleLibraries' ) ) {
	...
	if ( ! defined( 'nxt_PLUGIN_URL' ) )
		define( 'nxt_PLUGIN_URL', nxt_CONTENT_URL. '/plugins' );
	if ( ! defined( 'nxt_PLUGIN_DIR' ) )
		define( 'nxt_PLUGIN_DIR', nxt_CONTENT_DIR . '/plugins' );
	...
	class JCP_UseGoogleLibraries {
		...
		function __construct() {
			$this->google_scripts =
				array(
				// any extra scripts listed here not provided by NXTClass
			...
			$this->noconflict_url = nxt_PLUGIN_URL . '/use-google-libraries/js/jQnc.js';
		...
		static function configure_plugin() {
			add_action( 'nxt_default_scripts',
			...
			// There's a chance some plugin has called nxt_enqueue_script outside
			// of any hooks, which means that this plugin's 'nxt_default_scripts'
			...
			if ( is_a( $nxt_scripts, 'nxt_Scripts' ) ) {
				...
				$ugl->replace_default_scripts( $nxt_scripts );
			}
		}
		...
		/**
		 * Replace as many of the nxtclass default script registrations as possible
		...
		function replace_default_scripts( &$scripts ) {
			...
			foreach ( $this->google_scripts as $name => $values ) {
				if ( $script = $scripts->query( $name ) ) {
					...
					if ( strpos( $ver, '-' ) !== false ) {
						if ( nxt_DEBUG !== false ) {
							error_log("NXTClass appears to be using a non-standard version of $name (version $ver). Use Google Libraries not enabled for $name.");
						}
						continue;
					}
			...
			foreach ( $newscripts as $script ) {
				$olddata = $this->nxt_Dependency_get_data( $scripts, $script->handle );
		...
		function nxt_Dependency_get_data( $dep_obj, $handle, $data_name = false ) {
