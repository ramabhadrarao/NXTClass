...
Libraries CDN, rather than from NXTClass's own copies.

== Description ==

A number of the javascript libraries distributed with nxtclass are also 
hosted on Google's [AJAX Libraries API](http://code.google.com/apis/ajaxlibs/).
This plugin allows your nxtclass site to use the content distribution 
network side of Google's AJAX Library API, rather than serving these files from your NXTClass install directly.
...
# Upload the `use-google-libraries` folder to the `/nxt-content/plugins/` folder.
# Activate **Use Google Libraries** through the 'Plugins' menu in NXTClass.
# Er... That's it really.
...
In general, anything that calls nxt_register_script and/or
nxt_eneque_script before 'init' causes trouble for **Use Google
Libraries**.  I've made an effort to force it to try and run anyhow,
so please report any issues with this.  If you have 'nxt_DEBUG'
...
'nxt_enqueue_script'.  In fact you should develop your theme or plugin without
...
+ Added check for NXTClass including non-standard versions of scripts (fixes
NXTClass 3.2.2/swfobject).
...
+ Updated jQuery UI to work with NXTClass 3.1rc1
...
+ No longer disable script concatenation when using NXTClass 3.0 or
greater
+ Attempt to detect when another plugin or theme has called
'nxt_register_script' and/or 'nxt_enque_script' before 'init' and work
around it.
+ Limited debugging output when nxt_DEBUG is enabled.
...
+ Disables script concatenation in NXTClass 2.8, since it seems to have
issues when some of the dependencies are outside of the concatenation.
+ Persists flag to load scripts in the footer in NXTClass 2.8

= 1.0.5 =

Implemented a pair of
[suggestions](http://jasonpenney.net/nxtclass-plugins/use-google-libraries/comment-page-1/#comment-32427)
...
+ It no longer drops the micro version number from the url.  The reasons for this are twofold:
  ...
  servers.  If the version requested by your NXTClass install
...
= 1.2.1 =
Required for NXTClass 3.3.2

== A Request ==

If you're going to flag the plugin as "broken" in the NXTClass Plugin
Directory, please try and [let me know *what* is
broken](http://nxtclass.org/tags/use-google-libraries?forum_id=10).
...
with the standard NXTClass version.
...
**[Google AJAX Libraries](http://lud.icro.us/nxtclass-plugin-google-ajax-libraries/)**, 
