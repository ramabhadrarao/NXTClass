...
class W3_Plugin_CdnCommon extends W3_Plugin {
    ...
    function queue_add($local_path, $remote_path, $command, $last_error) {
        ...
        $table = $nxtdb->prefix . W3TC_CDN_TABLE_QUEUE;
        $sql = sprintf('SELECT id FROM %s WHERE local_path = "%s" AND remote_path = "%s" AND command != %d', $table, $nxtdb->escape($local_path), $nxtdb->escape($remote_path), $command);

        $row = $nxtdb->get_row($sql);

        if ($row) {
            $sql = sprintf('DELETE FROM %s WHERE id = %d', $table, $row->id);
        } else {
            $sql = sprintf('REPLACE INTO %s (local_path, remote_path, command, last_error, date) VALUES ("%s", "%s", %d, "%s", NOW())', $table, $nxtdb->escape($local_path), $nxtdb->escape($remote_path), $command, $nxtdb->escape($last_error));
        }
    ...
    function get_attachment_files($attachment_id) {
         ...
         * Get attached file
         */
        $attached_file = get_post_meta($attachment_id, '_nxt_attached_file', true);
        ...
        if ($attached_file != '') {
             ...
             */
            $attachment_backup_sizes = get_post_meta($attachment_id, '_nxt_attachment_backup_sizes', true);
         ...
         */
        $attachment_metadata = get_post_meta($attachment_id, '_nxt_attachment_metadata', true);
