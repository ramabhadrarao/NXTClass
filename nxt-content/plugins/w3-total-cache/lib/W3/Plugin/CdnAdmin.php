...
class W3_Plugin_CdnAdmin extends W3_Plugin {
    ...
    function activate() {
        ...
        if ($this->_config->get_boolean('cdn.enabled') && !w3_is_cdn_mirror($this->_config->get_string('cdn.engine')) && !$this->table_create(true)) {
            $error = sprintf('Unable to create table <strong>%s%s</strong>: %s', $nxtdb->prefix, W3TC_CDN_TABLE_QUEUE, $nxtdb->last_error);
    ...
    function schedule() {
        if ($this->_config->get_boolean('cdn.enabled') && !w3_is_cdn_mirror($this->_config->get_string('cdn.engine'))) {
            if (!nxt_next_scheduled('w3_cdn_cron_queue_process')) {
                nxt_schedule_event(time(), 'w3_cdn_cron_queue_process', 'w3_cdn_cron_queue_process');
            }
        } else {
            $this->unschedule();
        }
    ...
    function unschedule() {
        if (nxt_next_scheduled('w3_cdn_cron_queue_process')) {
            nxt_clear_scheduled_hook('w3_cdn_cron_queue_process');
        }
    }
    ...
    function unschedule_upload() {
        if (nxt_next_scheduled('w3_cdn_cron_upload')) {
            nxt_clear_scheduled_hook('w3_cdn_cron_upload');
        }
    }
    ...
    function table_create($drop = false) {
        ...
        if ($drop) {
            $sql = sprintf('DROP TABLE IF EXISTS `%s%s`', $nxtdb->prefix, W3TC_CDN_TABLE_QUEUE);
        ...
        $sql = sprintf("CREATE TABLE IF NOT EXISTS `%s%s` (
            ...
            KEY `date` (`date`)
        ) /*!40100 CHARACTER SET latin1 */", $nxtdb->prefix, W3TC_CDN_TABLE_QUEUE);
     ...
     * Purge attachment
     *
     * Upload _nxt_attached_file, _nxt_attachment_metadata, _nxt_attachment_backup_sizes
     *
    ...
    function queue_update($queue_id, $last_error) {
        ...
        $sql = sprintf('UPDATE %s SET last_error = "%s", date = NOW() WHERE id = %d', $nxtdb->prefix . W3TC_CDN_TABLE_QUEUE, $nxtdb->escape($last_error), $queue_id);
    ...
    function queue_delete($queue_id) {
        ...
        $sql = sprintf('DELETE FROM %s WHERE id = %d', $nxtdb->prefix . W3TC_CDN_TABLE_QUEUE, $queue_id);
    ...
    function queue_empty($command) {
        ...
        $sql = sprintf('DELETE FROM %s WHERE command = %d', $nxtdb->prefix . W3TC_CDN_TABLE_QUEUE, $command);
    ...
    function queue_get($limit = null) {
        ...
        $sql = sprintf('SELECT * FROM %s%s ORDER BY date', $nxtdb->prefix, W3TC_CDN_TABLE_QUEUE);
    ...
    function export_library($limit, $offset, &$count, &$total, &$results) {
        ...
        if ($upload_info) {
            ...
            FROM
                %sposts AS p
            LEFT JOIN
                %spostmeta AS pm ON p.ID = pm.post_ID AND pm.meta_key = "_nxt_attached_file"
            LEFT JOIN
            	%spostmeta AS pm2 ON p.ID = pm2.post_ID AND pm2.meta_key = "_nxt_attachment_metadata"
            WHERE
                p.post_type = "attachment"
            GROUP BY
            	p.ID', $nxtdb->prefix, $nxtdb->prefix, $nxtdb->prefix);
                                ...
                                if (preg_match($regexp, $check_src)) {
                                    ...
                                    if (isset($attachments[$src])) {
                                        ...
                                        if ($download_result) {
                                             ...
                                             * Insert attachment
                                             */
                                            $id = nxt_insert_attachment(array(
    ...
    function get_attachments_count() {
        ...
        $sql = sprintf('SELECT
            ...
            FROM
                %sposts AS p
            JOIN
                %spostmeta AS pm ON p.ID = pm.post_ID AND (pm.meta_key = "_nxt_attached_file" OR pm.meta_key = "_nxt_attachment_metadata")
            WHERE
                p.post_type = "attachment"', $nxtdb->prefix, $nxtdb->prefix);

        return $nxtdb->get_var($sql);
    }
