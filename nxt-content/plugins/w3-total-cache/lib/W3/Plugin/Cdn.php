...
class W3_Plugin_Cdn extends W3_Plugin {
    ...
    function run() {
        ...
        if (!w3_is_cdn_mirror($cdn_engine)) {
            ...
            add_filter('nxt_update_attachment_metadata', array(
                &$this,
     ...
     *
     * Delete _nxt_attached_file, _nxt_attachment_metadata, _nxt_attachment_backup_sizes
     *
     ...
     *
     * Upload _nxt_attachment_metadata
     *
        ...
        if ($buffer != '' && w3_is_xml($buffer)) {
            if ($this->can_cdn2($buffer)) {
                ...
                if ($this->_config->get_boolean('cdn.includes.enable')) {
                    ...
                    if ($mask != '') {
                        $regexps[] = '~(["\'])((' . $domain_url_regexp . ')?(' . w3_preg_quote($site_path . nxtINC) . '/(' . $this->get_regexp_by_mask($mask) . ')))~';
                    }
                }
    ...
    function get_files_includes() {
        $includes_root = w3_path(ABSPATH . nxtINC);
    ...
    function link_replace_callback($matches) {
        ...
        if ($queue === null) {
            if (!w3_is_cdn_mirror($this->_config->get_string('cdn.engine'))) {
                $sql = sprintf('SELECT remote_path FROM %s', $nxtdb->prefix . W3TC_CDN_TABLE_QUEUE);
                $queue = $nxtdb->get_col($sql);
            } else {
                $queue = false;
            }
        }
    ...
    function can_cdn() {
        ...
        if (defined('nxt_ADMIN')) {
            $this->cdn_reject_reason = 'nxt-admin';
