...
class W3_Plugin_TotalCacheAdmin extends W3_Plugin {
    ...
    function run() {
        ...
        if ($this->_config->get_boolean('widget.latest.enabled') || $this->_config->get_boolean('widget.pagespeed.enabled')) {
            add_action('nxt_dashboard_setup', array(
                &$this,
                'nxt_dashboard_setup'
            ));
        }
    ...
    function admin_print_styles() {
        nxt_enqueue_style('w3tc-options');
        nxt_enqueue_style('w3tc-lightbox');
    }
    ...
    function admin_print_scripts() {
        ...
        nxt_enqueue_script('w3tc-options');
        nxt_enqueue_script('w3tc-lightbox');
    ...
    function favorite_actions($actions) {
        $actions[nxt_nonce_url(admin_url('admin.php?page=w3tc_general&amp;w3tc_flush_all'), 'w3tc')] = array(
            'Empty Caches',
            'manage_options'
        );
    ...
    function media_row_actions($actions, $post) {
        $actions = array_merge($actions, array(
            'cdn_purge' => sprintf('<a href="%s">Purge from CDN</a>', nxt_nonce_url(sprintf('admin.php?page=w3tc_general&w3tc_cdn_purge_attachment&attachment_id=%d', $post->ID), 'w3tc'))
        ));
    ...
    function post_row_actions($actions, $post) {
        $actions = array_merge($actions, array(
            'pgcache_purge' => sprintf('<a href="%s">Purge from Page Cache</a>', nxt_nonce_url(sprintf('admin.php?page=w3tc_general&w3tc_pgcache_purge_post&post_id=%d', $post->ID), 'w3tc'))
        ));
    ...
    function page_row_actions($actions, $post) {
        $actions = array_merge($actions, array(
            'pgcache_purge' => sprintf('<a href="%s">Purge from Page Cache</a>', nxt_nonce_url(sprintf('admin.php?page=w3tc_general&w3tc_pgcache_purge_page&post_id=%d', $post->ID), 'w3tc'))
        ));
    ...
    function nxt_dashboard_setup() {
        ...
        if ($this->_config->get_boolean('widget.latest.enabled')) {
            nxt_add_dashboard_widget('w3tc_latest', 'The Latest from W3 EDGE', array(
                &$this,
                'widget_latest'
            ), array(
                &$this,
        ...
        if ($this->_config->get_boolean('widget.pagespeed.enabled')) {
            nxt_add_dashboard_widget('w3tc_pagespeed', 'W3 Total Cache: Google Page Speed Report', array(
                &$this,
                'widget_pagespeed'
            ), array(
                &$this,
    ...
    function admin_notices() {
        ...
        $browsercache_rules_no404nxt_path = w3_get_browsercache_rules_no404nxt_path();
        ...
        $error_messages = array(
            ...
            'fancy_permalinks_disabled_browsercache' => sprintf('Fancy permalinks are disabled. Please %s it first, then re-attempt to enabling the \'Do not process 404 errors for static objects with NXTClass\'.', $this->button_link('enable', 'options-permalink.php')),
            ...
            'pgcache_remove_rules_nxtsc' => sprintf('The nxt Super Cache rules could not be removed. Please run <strong>chmod 777 %s</strong> to resolve this issue.', (file_exists($pgcache_rules_cache_path) ? $pgcache_rules_cache_path : dirname($pgcache_rules_cache_path))),
            ...
            'browsercache_write_rules_no404nxt' => sprintf('The browser cache rules could not be modified. Please %srun <strong>chmod 777 %s</strong> to resolve this issue.', (file_exists($browsercache_rules_no404nxt_path) ? '' : sprintf('create an empty file in <strong>%s</strong> and ', $browsercache_rules_no404nxt_path)), $browsercache_rules_no404nxt_path),
            ...
            'support_request_nxt_login' => 'Please enter an administrator login. Create a temporary one just for this support case if needed.',
            'support_request_nxt_password' => 'Please enter nxt Admin password, be sure it\'s spelled correctly.',
            ...
            'enable_cookie_domain' => sprintf('<strong>%snxt-config.php</strong> could not be written, please edit config and add:<br /><strong style="color:#f00;">define(\'COOKIE_DOMAIN\', \'%s\');</strong> before <strong style="color:#f00;">require_once(ABSPATH . \'nxt-settings.php\');</strong>.', ABSPATH, addslashes($cookie_domain)),
            'disable_cookie_domain' => sprintf('<strong>%snxt-config.php</strong> could not be written, please edit config and add:<br /><strong style="color:#f00;">define(\'COOKIE_DOMAIN\', false);</strong> before <strong style="color:#f00;">require_once(ABSPATH . \'nxt-settings.php\');</strong>.', ABSPATH),
        ...
        $note_messages = array(
            ...
            'pgcache_remove_rules_nxtsc' => 'nxt Super Cache configuration settings have been successfully removed.',
            'browsercache_write_rules_cache' => 'Browser cache directives have been successfully written.',
            'browsercache_write_rules_no404nxt' => 'Browser cache directives have been successfully written.',
        ...
        if (!w3_is_preview_config() && !file_exists(W3TC_CONFIG_PATH)) {
            $errors[] = sprintf('<strong>W3 Total Cache Error:</strong> Default settings are in use. The configuration file could not be read or doesn\'t exist. Please %s to create the file.', $this->button_link('save the settings', nxt_nonce_url(sprintf('admin.php?page=%s&w3tc_save_config', $this->_page), 'w3tc')));
        }
        ...
        if ($this->_config->get_boolean('cdn.enabled') && !w3_is_cdn_mirror($this->_config->get_string('cdn.engine'))) {
            ...
            /**
             * Show notification after nxt upgrade
             */
            if ($this->_config->get_boolean('notes.nxt_upgraded')) {
                $notes[] = sprintf('Upgraded NXTClass? Please %s files now to ensure proper operation. %s', $this->button_popup('upload nxt-includes', 'cdn_export', 'cdn_export_type=includes'), $this->button_hide_note('Hide this message', 'nxt_upgraded'));
            }
            ...
            if ($this->_config->get_boolean('notes.cdn_upload') || $this->_config->get_boolean('notes.cdn_reupload')) {
                ...
                if ($this->_config->get_boolean('cdn.includes.enable')) {
                    $cdn_upload_buttons[] = $this->button_popup('nxt-includes', 'cdn_export', 'cdn_export_type=includes');
                }
        ...
        if ($this->_config->get_boolean('notes.plugins_updated')) {
            ...
            if ($this->_config->get_boolean('pgcache.enabled')) {
                $texts[] = $this->button_link('empty the page cache', nxt_nonce_url(sprintf('admin.php?page=%s&w3tc_flush_pgcache', $this->_page), 'w3tc'));
            }
        ...
        if ($this->_config->get_boolean('pgcache.enabled') && $this->_config->get('notes.need_empty_pgcache') && !w3_is_preview_config()) {
            $notes[] = sprintf('The setting change(s) made either invalidate the cached data or modify the behavior of the site. %s now to provide a consistent user experience.', $this->button_link('Empty the page cache', nxt_nonce_url(sprintf('admin.php?page=%s&w3tc_flush_pgcache', $this->_page), 'w3tc')));
        }
        ...
        if ($this->_config->get_boolean('objectcache.enabled') && $this->_config->get('notes.need_empty_objectcache') && !w3_is_preview_config()) {
            $notes[] = sprintf('The setting change(s) made either invalidate the cached data or modify the behavior of the site. %s now to provide a consistent user experience.', $this->button_link('Empty the object cache', nxt_nonce_url(sprintf('admin.php?page=%s&w3tc_flush_objectcache', $this->_page), 'w3tc')));
        }
        ...
        if ($this->_config->get_boolean('minify.enabled')) {
            ...
            if ($this->_config->get('notes.need_empty_minify') && !w3_is_preview_config()) {
                $notes[] = sprintf('The setting change(s) made either invalidate the cached data or modify the behavior of the site. %s now to provide a consistent user experience.', $this->button_link('Empty the minify cache', nxt_nonce_url(sprintf('admin.php?page=%s&w3tc_flush_minify', $this->_page), 'w3tc')));
            }
        }
    ...
    function options() {
        ...
        if ($this->_config->get_boolean('pgcache.enabled')) {
            if (!$this->advanced_cache_installed()) {
                $this->_errors[] = sprintf('Page caching is not available: %s is not installed. Either the <strong>%s</strong> directory is not write-able or another caching plugin installed. This error message will automatically disappear once the change is successfully made.', W3TC_ADDIN_FILE_ADVANCED_CACHE, nxt_CONTENT_DIR);
            ...
            } elseif (!defined('nxt_CACHE') || !nxt_CACHE) {
                $this->_errors[] = sprintf('Page caching is not available: please add: <strong>define(\'nxt_CACHE\', true);</strong> to <strong>%snxt-config.php</strong>. This error message will automatically disappear once the change is successfully made.', ABSPATH);
                ...
                if ($w3_plugin_pgcache->check_rules_core()) {
                    ...
                    $this->_errors[] = sprintf('Disk enhanced page caching is not active. To enable it, add the following rules into the server configuration file (<strong>%s</strong>) of the site above the NXTClass directives %s <textarea class="w3tc-rules" cols="120" rows="10" readonly="readonly">%s</textarea>. Or if permission allow this can be done automatically, by clicking here: %s. %s', w3_get_pgcache_rules_core_path(), $this->button('view code', '', 'w3tc-show-rules'), htmlspecialchars($w3_plugin_pgcache->generate_rules_core()), $this->button_link('auto-install', nxt_nonce_url(sprintf('admin.php?page=%s&w3tc_pgcache_write_rules_core', $this->_page), 'w3tc')), $this->button_hide_note('Hide this message', 'pgcache_rules_core'));
                }

                if ($this->_config->get_boolean('notes.pgcache_rules_legacy') && $w3_plugin_pgcache->check_rules_legacy()) {
                    $this->_errors[] = sprintf('Legacy Page Cache rewrite rules have been found. To remove them manually, edit the configuration file (<strong>%s</strong>) and remove all lines between and including <strong>%s</strong> and <strong>%s</strong> markers inclusive. Or if permission allow this can be done automatically, by clicking here: %s. %s', w3_get_pgcache_rules_core_path(), W3TC_MARKER_BEGIN_PGCACHE_LEGACY, W3TC_MARKER_END_PGCACHE_LEGACY, $this->button_link('auto-remove', nxt_nonce_url(sprintf('admin.php?page=%s&w3tc_pgcache_remove_rules_legacy', $this->_page), 'w3tc')), $this->button_hide_note('Hide this message', 'pgcache_rules_legacy'));
                }

                if ($this->_config->get_boolean('notes.pgcache_rules_nxtsc') && $w3_plugin_pgcache->check_rules_nxtsc()) {
                    $this->_errors[] = sprintf('nxt Super Cache rewrite rules have been found. To remove them manually, edit the configuration file (<strong>%s</strong>) and remove all lines between and including <strong>%s</strong> and <strong>%s</strong> markers inclusive. Or if permission allow this can be done automatically, by clicking here: %s. %s', w3_get_pgcache_rules_core_path(), W3TC_MARKER_BEGIN_PGCACHE_nxtSC, W3TC_MARKER_END_PGCACHE_nxtSC, $this->button_link('auto-remove', nxt_nonce_url(sprintf('admin.php?page=%s&w3tc_pgcache_remove_rules_nxtsc', $this->_page), 'w3tc')), $this->button_hide_note('Hide this message', 'pgcache_rules_nxtsc'));
                }

                if ($this->_config->get_boolean('notes.pgcache_rules_cache') && !$w3_plugin_pgcache->check_rules_cache()) {
                    $this->_errors[] = sprintf('Disk enhanced page caching is not active. To enable it, add the following rules into the server configuration file (<strong>%s</strong>) of the site %s <textarea class="w3tc-rules" cols="120" rows="10" readonly="readonly">%s</textarea>. This can be done automatically, by clicking here: %s. %s', w3_get_pgcache_rules_cache_path(), $this->button('view code', '', 'w3tc-show-rules'), htmlspecialchars($w3_plugin_pgcache->generate_rules_cache()), $this->button_link('auto-install', nxt_nonce_url(sprintf('admin.php?page=%s&w3tc_pgcache_write_rules_cache', $this->_page), 'w3tc')), $this->button_hide_note('Hide this message', 'pgcache_rules_cache'));
                }
            }
        }
        ...
        if ($this->_config->get_boolean('browsercache.enabled') && $this->_config->get_boolean('config.check') && w3_can_check_rules()) {
            ...
            if ($this->_config->get_boolean('notes.browsercache_rules_cache') && !$w3_plugin_browsercache->check_rules_cache()) {
                $this->_errors[] = sprintf('Browser caching is not active. To enable it, add the following rules into the server configuration file (<strong>%s</strong>) of the site %s <textarea class="w3tc-rules" cols="120" rows="10" readonly="readonly">%s</textarea>. Or if permission allow this can be done automatically, by clicking here: %s. %s', w3_get_browsercache_rules_cache_path(), $this->button('view code', '', 'w3tc-show-rules'), htmlspecialchars($w3_plugin_browsercache->generate_rules_cache()), $this->button_link('auto-install', nxt_nonce_url(sprintf('admin.php?page=%s&w3tc_browsercache_write_rules_cache', $this->_page), 'w3tc')), $this->button_hide_note('Hide this message', 'browsercache_rules_cache'));
            }

            if ($this->_config->get_boolean('notes.browsercache_rules_no404nxt') && $this->_config->get_boolean('browsercache.no404nxt') && !$w3_plugin_browsercache->check_rules_no404nxt()) {
                $this->_errors[] = sprintf('"Do not process 404 errors for static objects with NXTClass" feature is not active. To enable it, add the following rules into the server configuration file (<strong>%s</strong>) of the site %s <textarea class="w3tc-rules" cols="120" rows="10" readonly="readonly">%s</textarea>. Or if permission allow this can be done automatically, by clicking here: %s. %s', w3_get_browsercache_rules_no404nxt_path(), $this->button('view code', '', 'w3tc-show-rules'), htmlspecialchars($w3_plugin_browsercache->generate_rules_no404nxt()), $this->button_link('auto-install', nxt_nonce_url(sprintf('admin.php?page=%s&w3tc_browsercache_write_rules_no404nxt', $this->_page), 'w3tc')), $this->button_hide_note('Hide this message', 'browsercache_rules_no404nxt'));
            }
        }
        ...
        if ($this->_config->get_boolean('minify.enabled')) {
            if ($this->_config->get_boolean('minify.rewrite') && $this->_config->get_boolean('config.check') && w3_can_check_rules()) {
                ...
                if ($this->_config->get_string('minify.engine') == 'file' && $this->_config->get_boolean('notes.minify_rules_cache') && !$w3_plugin_minify->check_rules_cache()) {
                    $this->_errors[] = sprintf('Minify is not active. To enable it, add the following rules into the server configuration file (<strong>%s</strong>) of the site %s <textarea class="w3tc-rules" cols="120" rows="10" readonly="readonly">%s</textarea>. This can be done automatically, by clicking here: %s. %s', w3_get_minify_rules_cache_path(), $this->button('view code', '', 'w3tc-show-rules'), htmlspecialchars($w3_plugin_minify->generate_rules_cache()), $this->button_link('auto-install', nxt_nonce_url(sprintf('admin.php?page=%s&w3tc_minify_write_rules_cache', $this->_page), 'w3tc')), $this->button_hide_note('Hide this message', 'minify_rules_cache'));
                }
            }
        ...
        if ($this->_config->get_boolean('dbcache.enabled')) {
            if (!$this->db_installed()) {
                $this->_errors[] = sprintf('Database caching is not available: %s is not installed. Either the <strong>%s</strong> directory is not write-able or another caching plugin is installed. This error message will automatically disappear once the change is successfully made.', W3TC_ADDIN_FILE_DB, nxt_CONTENT_DIR);
        ...
        if ($this->_config->get_boolean('objectcache.enabled')) {
            if (!$this->objectcache_installed()) {
                $this->_errors[] = sprintf('Object caching is not available: %s is not installed. Either the <strong>%s</strong> directory is not write-able or another caching plugin is installed. This error message will automatically disappear once the change is successfully made.', W3TC_ADDIN_FILE_OBJECT_CACHE, nxt_CONTENT_DIR);
        ...
        if ($this->_config->get_boolean('notes.zlib_output_compression') && w3_zlib_output_compression()) {
            $this->_notes[] = sprintf('Either the PHP configuration, web server configuration or a script in the NXTClass installation has <strong>zlib.output_compression</strong> enabled.<br />Please locate and disable this setting to ensure proper HTTP compression behavior. %s', $this->button_hide_note('Hide this message', 'zlib_output_compression'));
        }

        /**
         * Check nxt-content permissions
         */
        if (!W3TC_WIN && $this->_config->get_boolean('notes.nxt_content_perms')) {
            $nxt_content_stat = stat(nxt_CONTENT_DIR);
            $nxt_content_mode = ($nxt_content_stat['mode'] & 0777);

            if ($nxt_content_mode != 0755) {
                $this->_notes[] = sprintf('<strong>%s</strong> is write-able. When finished installing the plugin, change the permissions back to the default: <strong>chmod 755 %s</strong>. %s', nxt_CONTENT_DIR, nxt_CONTENT_DIR, $this->button_hide_note('Hide this message', 'nxt_content_perms'));
            }
        }
         ...
         * Check permalinks
         */
        if ($this->_config->get_boolean('notes.no_permalink_rules') && (($this->_config->get_boolean('pgcache.enabled') && $this->_config->get_string('pgcache.engine') == 'file_generic') || ($this->_config->get_boolean('browsercache.enabled') && $this->_config->get_boolean('browsercache.no404nxt'))) && !w3_is_permalink_rules()) {
            $this->_errors[] = sprintf('The required directives for fancy permalinks could not be detected, please confirm they are available: <a href="http://codex.nxtclass.org/Using_Permalinks#Creating_and_editing_.28.htaccess.29">Creating and editing</a> %s', $this->button_hide_note('Hide this message', 'no_permalink_rules'));
        }
        ...
        if ($this->_config->get_boolean('cdn.enabled')) {
            ...
            if (!$upload_info) {
                ...
                if (empty($upload_path)) {
                    $upload_path = nxt_CONTENT_DIR . '/uploads';

                    $this->_errors[] = sprintf('The uploads directory is not available. Default NXTClass directories will be created: <strong>%s</strong>.', $upload_path);
                }

                if (!w3_is_multisite()) {
                    $this->_errors[] = sprintf('The uploads path found in the database (%s) is inconsistent with the actual path. Please manually adjust the upload path either in miscellaneous settings or if not using a custom path %s automatically to resolve the issue.', $upload_path, $this->button_link('update the path', nxt_nonce_url(sprintf('admin.php?page=%s&w3tc_update_upload_path', $this->_page), 'w3tc')));
                }
            }
    ...
    function options_install() {
        ...
        if (w3_can_check_rules()) {
            ...
            if ($this->_config->get_boolean('browsercache.enabled') && $this->_config->get_boolean('browsercache.no404nxt')) {
                $w3_plugin_browsercache = & w3_instance('W3_Plugin_BrowserCacheAdmin');

                $browsercache_rules_no404nxt_path = w3_get_browsercache_rules_no404nxt_path();

                if (!isset($rewrite_rules[$browsercache_rules_no404nxt_path])) {
                    $rewrite_rules[$browsercache_rules_no404nxt_path] = '';
                }

                $rewrite_rules[$browsercache_rules_no404nxt_path] .= $w3_plugin_browsercache->generate_rules_no404nxt();
            }
    ...
    function action_widget_latest_ajax() {
        ...
        if ($nxt_version >= 2.8) {
            ...
            include_once (ABSPATH . nxtINC . '/rss.php');
    ...
    function action_support_request() {
        ...
        $required = array(
            ...
            'plugin_config' => 'url,name,email,subject,description,nxt_login,nxt_password',
            'theme_config' => 'url,name,email,subject,description,nxt_login,nxt_password,ftp_host,ftp_login,ftp_password',
            'linux_config' => 'url,name,email,subject,description,nxt_login,nxt_password,ftp_host,ftp_login,ftp_password'
        );
        ...
        if (strstr($required[$request_type], 'nxt_login') !== false && $nxt_login == '') {
            $this->redirect(array_merge($params, array(
                'w3tc_error' => 'support_request_nxt_login'
            )));
        }

        if (strstr($required[$request_type], 'nxt_password') !== false && $nxt_password == '') {
            $this->redirect(array_merge($params, array(
                'w3tc_error' => 'support_request_nxt_password'
            )));
        }
        ...
        $attach_files = array(
            /**
             * Attach nxt config file
             */
            w3_get_nxt_config_path(),
            ...
            w3_get_browsercache_rules_no404nxt_path(),
    ...
    function action_save_options() {
        ...
        if ($this->_page == 'w3tc_mobile') {
            ...
            /**
             * Allow plugins modify nxtSC mobile groups
             */
    ...
    /**
     * Remove page cache nxtSC rules action
     *
     * @return void
     */
    function action_pgcache_remove_rules_nxtsc() {
        $w3_plugin_pgcache = & w3_instance('W3_Plugin_PgCacheAdmin');

        if ($w3_plugin_pgcache->remove_rules_nxtsc()) {
            $this->redirect(array(
                'w3tc_note' => 'pgcache_remove_rules_nxtsc'
            ));
        } else {
            $this->redirect(array(
                'w3tc_error' => 'pgcache_remove_rules_nxtsc'
            ));
        }
    }
    ...
    /**
     * Write browser cache no404nxt rules action
     *
     * @return void
     */
    function action_browsercache_write_rules_no404nxt() {
        $w3_plugin_browsercache = & w3_instance('W3_Plugin_BrowserCacheAdmin');

        if ($w3_plugin_browsercache->write_rules_no404nxt()) {
            $this->redirect(array(
                'w3tc_note' => 'browsercache_write_rules_no404nxt'
            ));
        } else {
            $this->redirect(array(
                'w3tc_error' => 'browsercache_write_rules_no404nxt'
            ));
        }
    }
    ...
    function action_pagespeed_results() {
        ...
        if ($force) {
            $this->redirect(array(
                ...
                '_nxtnonce' => nxt_create_nonce('w3tc')
            ));
        }
    ...
    function get_theme_urls($theme_name) {
        ...
        if ($theme && isset($theme['Template Files'])) {
            ...
            if (get_option('show_on_front') == 'page') {
                ...
                if ($front_page_id) {
                    $front_page_template_file = get_post_meta($front_page_id, '_nxt_page_template', true);
            ...
            foreach ($template_files as $template_file) {
                ...
                switch (true) {
                    ...
                    case ($template == 'taxonomy'):
                        $taxonomy = '';
                        if (isset($GLOBALS['nxt_taxonomies']) && is_array($GLOBALS['nxt_taxonomies'])) {
                            foreach ($GLOBALS['nxt_taxonomies'] as $nxt_taxonomy) {
                                if (!in_array($nxt_taxonomy->name, array(
                                    'category',
                                    'post_tag',
                                    'link_category'
                                ))) {
                                    $taxonomy = $nxt_taxonomy->name;
                                    break;
                                }
                            }
                        }
    ...
    function get_themes() {
        $themes = array();
        $nxt_themes = get_themes();

        foreach ($nxt_themes as $nxt_theme) {
            $theme_key = w3_get_theme_key($nxt_theme['Theme Root'], $nxt_theme['Template'], $nxt_theme['Stylesheet']);
            $themes[$theme_key] = $nxt_theme['Name'];
        }
    ...
    function get_server_info() {
        global $nxt_version, $nxt_db_version, $nxtdb;

        $nxtclass_plugins = get_plugins();
        $nxtclass_plugins_active = array();

        foreach ($nxtclass_plugins as $nxtclass_plugin_file => $nxtclass_plugin) {
            if (is_plugin_active($nxtclass_plugin_file)) {
                $nxtclass_plugins_active[$nxtclass_plugin_file] = $nxtclass_plugin;
            }
        }

        $mysql_version = $nxtdb->get_var('SELECT VERSION()');
        $mysql_variables_result = (array) $nxtdb->get_results('SHOW VARIABLES', ARRAY_N);
        ...
        $server_info = array(
            ...
            'nxt' => array(
                ...
                'theme' => get_theme(get_current_theme()),
                'nxt_cache' => ((defined('nxt_CACHE') && nxt_CACHE) ? 'true' : 'false'),
                'plugins' => $nxtclass_plugins_active
            ),
            'mysql' => array(
    ...
    function is_queue_empty() {
        ...
        $sql = sprintf('SELECT COUNT(*) FROM %s', $nxtdb->prefix . W3TC_CDN_TABLE_QUEUE);
        $result = $nxtdb->get_var($sql);
