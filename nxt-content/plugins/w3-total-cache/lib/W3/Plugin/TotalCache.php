...
class W3_Plugin_TotalCache extends W3_Plugin {
    ...
    function run() {
        ...
        if ($this->_config->get_boolean('cloudflare.enabled')) {
            add_action('nxt_set_comment_status', array(
                &$this,
        ...
        if (current_user_can('manage_options')) {
            $menu_items = array(
                ...
                array(
                    ...
                    'href' => nxt_nonce_url(admin_url('admin.php?page=w3tc_general&amp;w3tc_flush_all'), 'w3tc')
                ),
                array(
                    'id' => 'w3tc-faq',
                    'parent' => 'w3tc',
            ...
            foreach ($menu_items as $menu_item) {
                $nxt_admin_bar->add_menu($menu_item);
            }
        }
    }
     ...
     */
    function footer() {
        echo '<div style="text-align: center;">Performance Optimization <a href="http://www.w3-edge.com/nxtclass-plugins/" rel="external">NXTClass Plugins</a> by W3 EDGE</div>';
    }
        ...
        if ($buffer != '' && w3_is_xml($buffer)) {
            if (w3_is_database_error($buffer)) {
                ...
                if ($this->_config->get_string('common.support') != '' || $this->_config->get_boolean('common.tweeted')) {
                    ...
                    if ($this->_config->get_boolean('dbcache.enabled') && !$this->_config->get_boolean('dbcache.debug') && is_a($nxtdb, 'W3_Db')) {
                        $append = (is_user_logged_in() ? ' (user is logged in)' : '');

                        if ($nxtdb->query_hits) {
                            $strings[] = sprintf("Database Caching %d/%d queries in %.3f seconds using %s%s", $nxtdb->query_hits, $nxtdb->query_total, $nxtdb->time_total, w3_get_engine_name($this->_config->get_string('dbcache.engine')), $append);
                        } else {
                    ...
                    $buffer .= "\r\n<!-- Performance optimized by W3 Total Cache. Learn more: http://www.w3-edge.com/nxtclass-plugins/\r\n";
    ...
    function can_ob() {
        ...
        /**
         * Check for nxtMU's and nxt's 3.0 short init
         */
    ...
    function cloudflare_set_comment_status($id, $status) {
        if ($status == 'spam') {
            ...
            if ($email && $key) {
                ...
                $w3_cloudflare->external_event('nxt_SPAM', json_encode($value));
            }
        }
    }
}
