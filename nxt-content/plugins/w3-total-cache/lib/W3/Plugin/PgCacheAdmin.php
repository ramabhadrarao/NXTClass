...
class W3_Plugin_PgCacheAdmin extends W3_Plugin {
    ...
    function prime($start = 0) {
        ...
        if (count($urls) > ($start + $limit)) {
            nxt_schedule_single_event(time() + $interval, 'w3_pgcache_prime', array(
                $start + $limit
            ));
        }
    ...
    function schedule() {
        if ($this->_config->get_boolean('pgcache.enabled') && ($this->_config->get_string('pgcache.engine') == 'file' || $this->_config->get_string('pgcache.engine') == 'file_generic')) {
            if (!nxt_next_scheduled('w3_pgcache_cleanup')) {
                nxt_schedule_event(time(), 'w3_pgcache_cleanup', 'w3_pgcache_cleanup');
            }
        } else {
            $this->unschedule();
        }
    ...
    function unschedule() {
        if (nxt_next_scheduled('w3_pgcache_cleanup')) {
            nxt_clear_scheduled_hook('w3_pgcache_cleanup');
        }
    }
    ...
    function unschedule_prime() {
        if (nxt_next_scheduled('w3_pgcache_prime')) {
            nxt_clear_scheduled_hook('w3_pgcache_prime');
        }
    }

    /**
     * Erases nxt_CACHE define
     *
     * @param string $content
     * @return mixed
     */
    function erase_nxt_cache($content) {
        $content = preg_replace("~\r\n\\/\\*\\* Enable W3 Total Cache \\*\\*?\\/.*?\\/\\/ Added by W3 Total Cache\r\n~s", '', $content);
        $content = preg_replace("~(\\/\\/\\s*)?define\\s*\\(\\s*['\"]?nxt_CACHE['\"]?\\s*,.*?\\)\\s*;+\\r?\\n?~is", '', $content);
    ...
    function enable_nxt_cache() {
        ...
        $new_config_data = preg_replace('~<\?(php)?~', "\\0\r\n/** Enable W3 Total Cache */\r\ndefine('nxt_CACHE', true); // Added by W3 Total Cache\r\n", $new_config_data, 1);
    ...
    function generate_rules_core_apache() {
        ...
        $reject_uris = array(
            '\/nxt-admin\/',
            '\/xmlrpc.php',
            '\/nxt-(app|cron|login|register|mail)\.php'
        );
    ...
    /**
     * Writes directives to nxt .htaccess
     *
     * @return boolean
     */
    function write_rules_core() {
        ...
        if (file_exists($path)) {
            ...
            if ($data !== false) {
                ...
                $data = $this->erase_rules_nxtsc($data);
            } else {
                return false;
            }
        } else {
    ...
    /**
     * Erases nxt Super Cache rules directives
     *
     * @param string $data
     * @return string
     */
    function erase_rules_nxtsc($data) {
        $data = w3_erase_rules($data, W3TC_MARKER_BEGIN_PGCACHE_nxtSC, W3TC_MARKER_END_PGCACHE_nxtSC);
    ...
    /**
     * Removes nxtSC directives
     *
     * @return boolean
     */
    function remove_rules_nxtsc() {
        $path = w3_get_pgcache_rules_core_path();
    ...
    /**
     * Check if nxtSC rules exists
     *
     * @return boolean
     */
    function check_rules_nxtsc() {
        $path = w3_get_pgcache_rules_core_path();

        return (($data = @file_get_contents($path)) && w3_has_rules(w3_clean_rules($data), W3TC_MARKER_BEGIN_PGCACHE_nxtSC, W3TC_MARKER_END_PGCACHE_nxtSC));
    }
}