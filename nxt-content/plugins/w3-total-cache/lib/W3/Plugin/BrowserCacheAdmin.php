...
class W3_Plugin_BrowserCacheAdmin extends W3_Plugin {
    ...
    function activate() {
        if ($this->_config->get_boolean('browsercache.enabled')) {
            ...
            if ($this->_config->get_boolean('browsercache.no404nxt') && w3_can_modify_rules(w3_get_browsercache_rules_no404nxt_path())) {
                $this->write_rules_no404nxt();
            }
        }
    }
    ...
    /**
     * Generate rules related to prevent for media 404 error by nxt
     *
     * @return string
     */
    function generate_rules_no404nxt() {
        switch (true) {
            case w3_is_apache():
            case w3_is_litespeed():
                return $this->generate_rules_no404nxt_apache();

            case w3_is_nginx():
                return $this->generate_rules_no404nxt_nginx();
        }
     ...
     * @return string
     */
    function generate_rules_no404nxt_apache() {
        $cssjs_types = $this->_get_cssjs_types();
        $html_types = $this->_get_html_types();
        $other_types = $this->_get_other_types();
        ...
        $exceptions = $this->_config->get_array('browsercache.no404nxt.exceptions');

        $rules = '';
        $rules .= W3TC_MARKER_BEGIN_BROWSERCACHE_NO404nxt . "\n";
        ...
        $rules .= "    RewriteRule .* - [L]\n";
        $rules .= "</IfModule>\n";
        $rules .= W3TC_MARKER_END_BROWSERCACHE_NO404nxt . "\n";
     ...
     * @return string
     */
    function generate_rules_no404nxt_nginx() {
        $cssjs_types = $this->_get_cssjs_types();
        $html_types = $this->_get_html_types();
        $other_types = $this->_get_other_types();
    ...
    function write_rules_cache() {
        ...
        if ($replace_start !== false && $replace_end !== false && $replace_start < $replace_end) {
            ...
            $search = array(
                ...
                W3TC_MARKER_BEGIN_BROWSERCACHE_NO404nxt => 0,
                W3TC_MARKER_BEGIN_nxtclass => 0,
    ...
    /**
     * Writes no 404 by nxt rules
     *
     * @return boolean
     */
    function write_rules_no404nxt() {
        $path = w3_get_browsercache_rules_no404nxt_path();
        ...
        $replace_start = strpos($data, W3TC_MARKER_BEGIN_BROWSERCACHE_NO404nxt);
        $replace_end = strpos($data, W3TC_MARKER_END_BROWSERCACHE_NO404nxt);

        if ($replace_start !== false && $replace_end !== false && $replace_start < $replace_end) {
            $replace_length = $replace_end - $replace_start + strlen(W3TC_MARKER_END_BROWSERCACHE_NO404nxt) + 1;
        } else {
            $replace_start = false;
            $replace_length = 0;
        ...
        $rules = $this->generate_rules_no404nxt();
    ...
    /**
     * Erases no404nxt rules
     *
     * @param string $data
     * @return string
     */
    function erase_rules_no404nxt($data) {
        $data = w3_erase_rules($data, W3TC_MARKER_BEGIN_BROWSERCACHE_NO404nxt, W3TC_MARKER_END_BROWSERCACHE_NO404nxt);
    ...
    /**
     * Removes no404nxt rules
     *
     * @return boolean
     */
    function remove_rules_no404nxt() {
        $path = w3_get_browsercache_rules_no404nxt_path();

        if (file_exists($path)) {
            if (($data = @file_get_contents($path)) !== false) {
                $data = $this->erase_rules_no404nxt($data);
    ...
    /**
     * Check no404nxt rules
     *
     * @return boolean
     */
    function check_rules_no404nxt() {
        $path = w3_get_browsercache_rules_no404nxt_path();
        $search = $this->generate_rules_no404nxt();
