...
class W3_PgCache {
    ...
    /**
     * Returns instance. 
     * For backward compatibility with 0.9.2.3 version of /nxt-content files
     *
     * @return W3_PgCache
     */
    function &instance() {
    ...
    function process() {
        ...
        switch (true) {
            ...
            case defined('nxt_ADMIN'):
            case (defined('SHORTINIT') && SHORTINIT):
                return;
        }

        /*
         * Legacy mode with 0.9.2.3 version of /nxt-content add-ins
         */
    ...
    /**
     * Checks NXTClass cookies
     *
     * @return boolean
     */
    function _check_cookies() {
        foreach (array_keys($_COOKIE) as $cookie_name) {
            if ($cookie_name == 'nxtclass_test_cookie') {
                continue;
            }
            if (preg_match('/^(nxt-postpass|comment_author)/', $cookie_name)) {
                return false;
            }
        }
    ...
    function _check_logged_in() {
        foreach (array_keys($_COOKIE) as $cookie_name) {
            ...
            if (strpos($cookie_name, 'nxtclass') === 0) {
                return false;
            }
        }
    ...
    function _bad_behavior() {
        if (file_exists(nxt_CONTENT_DIR . '/plugins/bad-behavior/bad-behavior-generic.php')) {
            $bb_file = nxt_CONTENT_DIR . '/plugins/bad-behavior/bad-behavior-generic.php';
        } elseif (file_exists(nxt_CONTENT_DIR . '/plugins/Bad-Behavior/bad-behavior-generic.php')) {
            $bb_file = nxt_CONTENT_DIR . '/plugins/Bad-Behavior/bad-behavior-generic.php';
        } else {
            $bb_file = false;
        }
