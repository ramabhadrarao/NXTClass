...
jQuery(function() {
    ...
    jQuery('.button-tweet').live('click', function() {
        window.open('http://twitter.com/?status=' + encodeURIComponent('YES! I optimized my #nxtclass site\'s #performance using the W3 Total Cache #plugin by @w3edge. Check it out! http://j.mp/A69xX'), '_blank');
    });

    jQuery('.button-rating').live('click', function() {
        window.open('http://nxtclass.org/extend/plugins/w3-total-cache/', '_blank');
    });
    ...
    jQuery('#cdn_export_library').click(function() {
        w3tc_popup('admin.php?page=w3tc_cdn&w3tc_cdn_export_library&_nxtnonce=' + jQuery(this).metadata().nonce, 'cdn_export_library');
    });

    jQuery('#cdn_import_library').click(function() {
        w3tc_popup('admin.php?page=w3tc_cdn&w3tc_cdn_import_library&_nxtnonce=' + jQuery(this).metadata().nonce, 'cdn_import_library');
    });

    jQuery('#cdn_queue').click(function() {
        w3tc_popup('admin.php?page=w3tc_cdn&w3tc_cdn_queue&_nxtnonce=' + jQuery(this).metadata().nonce, 'cdn_queue');
    });

    jQuery('#cdn_rename_domain').click(function() {
        w3tc_popup('admin.php?page=w3tc_cdn&w3tc_cdn_rename_domain&_nxtnonce=' + jQuery(this).metadata().nonce, 'cdn_rename_domain');
    });

    jQuery('#cdn_purge').click(function() {
        w3tc_popup('admin.php?page=w3tc_cdn&w3tc_cdn_purge&_nxtnonce=' + jQuery(this).metadata().nonce, 'cdn_purge');
    });

    jQuery('.cdn_export').click(function() {
        var metadata = jQuery(this).metadata();
        w3tc_popup('admin.php?page=w3tc_cdn&w3tc_cdn_export&cdn_export_type=' + metadata.type + '&_nxtnonce=' + metadata.nonce, 'cdn_export_' + metadata.type);
    });
    ...
    jQuery('#memcached_test').click(function() {
        ...
        jQuery.post('admin.php?page=w3tc_general', {
            ...
            _nxtnonce: jQuery(this).metadata().nonce
        }, function(data) {
            status.addClass(data.result ? 'w3tc-success' : 'w3tc-error');
            status.html(data.error);
        }, 'json');
    ...
    jQuery('#support_form').live('submit', function() {
        ...
        var nxt_login = jQuery('.required #support_nxt_login');
        var nxt_password = jQuery('.required #support_nxt_password');
        ...
        if (nxt_login.size() && nxt_login.val() == '') {
            alert('Please enter an administrator login. Remember you can create a temporary one just for this support case.');
            nxt_login.focus();
            return false;
        }

        if (nxt_password.size() && nxt_password.val() == '') {
            alert('Please enter nxt Admin password, be sure it\'s spelled correctly.');
            nxt_password.focus();
            return false;
        }
    ...
    jQuery('#support_request_type').live('change', function() {
        ...
        if (action) {
            jQuery('#support_container').html('<div id="support_loading">Loading...</div>').load('admin.php?page=w3tc_support&w3tc_' + action + '&request_type=' + type + '&_nxtnonce=' + request_type.metadata().nonce);
    ...
    jQuery('#support_cancel').live('click', function() {
        jQuery('#support_container').html('<div id="support_loading">Loading...</div>').load('admin.php?page=w3tc_support&w3tc_support_select&_nxtnonce=' + jQuery(this).metadata().nonce);
    });
