...
define('W3TC_LINK_URL', 'http://www.w3-edge.com/nxtclass-plugins/');
define('W3TC_LINK_NAME', 'NXTClass Plugins');
define('W3TC_FEED_URL', 'http://feeds.feedburner.com/W3TOTALCACHE');
define('W3TC_README_URL', 'http://plugins.trac.nxtclass.org/browser/w3-total-cache/trunk/readme.txt?format=txt');
...
define('W3TC_INSTALL_DIR', W3TC_DIR . '/nxt-content');
define('W3TC_INSTALL_MINIFY_DIR', W3TC_INSTALL_DIR . '/w3tc/min');

define('W3TC_BLOGNAMES_PATH', nxt_CONTENT_DIR . '/w3-total-cache-blognames.php');
define('W3TC_BLOGNAME', w3_get_blogname());
define('W3TC_SUFFIX', (W3TC_BLOGNAME != '' ? '-' . W3TC_BLOGNAME : ''));

defined('nxt_CONTENT_DIR') || define('nxt_CONTENT_DIR', realpath(W3TC_DIR . '/../..'));
define('nxt_CONTENT_DIR_PATH', dirname(nxt_CONTENT_DIR));
define('nxt_CONTENT_DIR_NAME', basename(nxt_CONTENT_DIR));
define('W3TC_CONTENT_DIR_NAME', nxt_CONTENT_DIR_NAME . '/w3tc' . W3TC_SUFFIX);
define('W3TC_CONTENT_DIR', nxt_CONTENT_DIR_PATH . '/' . W3TC_CONTENT_DIR_NAME);
define('W3TC_CONTENT_MINIFY_DIR_NAME', W3TC_CONTENT_DIR_NAME . '/min');
define('W3TC_CONTENT_MINIFY_DIR', nxt_CONTENT_DIR_PATH . '/' . W3TC_CONTENT_DIR_NAME . '/min');
...
define('W3TC_CONFIG_PATH', nxt_CONTENT_DIR . '/w3-total-cache-config' . W3TC_SUFFIX . '.php');
define('W3TC_CONFIG_PREVIEW_PATH', nxt_CONTENT_DIR . '/w3-total-cache-config' . W3TC_SUFFIX . '-preview.php');
define('W3TC_CONFIG_MASTER_PATH', nxt_CONTENT_DIR . '/w3-total-cache-config.php');
...
define('W3TC_MARKER_BEGIN_nxtclass', '# BEGIN NXTClass');
...
define('W3TC_MARKER_BEGIN_PGCACHE_nxtSC', '# BEGIN nxtSuperCache');
define('W3TC_MARKER_BEGIN_BROWSERCACHE_CACHE', '# BEGIN W3TC Browser Cache');
define('W3TC_MARKER_BEGIN_BROWSERCACHE_NO404nxt', '# BEGIN W3TC Skip 404 error handling by NXTClass for static files');
...
define('W3TC_MARKER_END_nxtclass', '# END NXTClass');
...
define('W3TC_MARKER_END_PGCACHE_nxtSC', '# END nxtSuperCache');
define('W3TC_MARKER_END_BROWSERCACHE_CACHE', '# END W3TC Browser Cache');
define('W3TC_MARKER_END_BROWSERCACHE_NO404nxt', '# END W3TC Skip 404 error handling by NXTClass for static files');
...
define('W3TC_ADDIN_FILE_ADVANCED_CACHE', nxt_CONTENT_DIR . '/advanced-cache.php');
define('W3TC_ADDIN_FILE_DB', nxt_CONTENT_DIR . '/db.php');
define('W3TC_ADDIN_FILE_OBJECT_CACHE', nxt_CONTENT_DIR . '/object-cache.php');
...
/**
 * Returns true if it's nxtMU
 *
 * @return boolean
 */
function w3_is_nxtmu() {
    static $nxtmu = null;

    if ($nxtmu === null) {
        $nxtmu = file_exists(ABSPATH . 'nxtmu-settings.php');
    }
...
/**
 * Returns true if nxtMU uses vhosts
 *
 * @return boolean
 */
...
/**
 * Returns true if it's nxt with enabled Network mode
 *
 * @return boolean
 */
...
function w3_is_network() {
    return (w3_is_nxtmu() || w3_is_multisite());
}
...
function w3_is_preview_mode() {
    return (w3_is_preview_config() && (defined('nxt_ADMIN') || isset($_REQUEST['w3tc_preview']) || (isset($_SERVER['HTTP_REFERER']) && strstr($_SERVER['HTTP_REFERER'], 'w3tc_preview') !== false)));
}
...
function w3_get_blognames() {
    ...
    $sql = sprintf('SELECT domain, path FROM %s', $nxtdb->blogs);
    $blogs = $nxtdb->get_results($sql);
...
/**
 * Returns path to nxt directory relative to document root
 *
 * Example:
 *
 * DOCUMENT_ROOT=/var/www/vhosts/domain.com/
...
/**
 * Returns nxt config file path
 *
 * @return string
 */
function w3_get_nxt_config_path() {
    $search = array(
        ABSPATH . 'nxt-config.php',
        dirname(ABSPATH) . '/nxt-config.php'
    );
...
/**
 * Remove nxt query string from URL
 *
 * @param string $url
 * @return string
 */
