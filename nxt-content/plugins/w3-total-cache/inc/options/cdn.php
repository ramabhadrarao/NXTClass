...
<?php if ($cdn_mirror): ?>
<p>
	Maximize <acronym title="Content Delivery Network">CDN</acronym> usage by <input id="cdn_rename_domain" class="button {nonce: '<?php echo nxt_create_nonce('w3tc'); ?>'}" type="button" value="modify attachment URLs" /> or
	<input id="cdn_import_library" class="button {nonce: '<?php echo nxt_create_nonce('w3tc'); ?>'}" type="button" value="importing attachments into the Media Library" />.
    <?php if (w3_can_cdn_purge($cdn_engine)): ?>
    <input id="cdn_purge" class="button {nonce: '<?php echo nxt_create_nonce('w3tc'); ?>'}" type="button" value="Purge" /> objects from the <acronym title="Content Delivery Network">CDN</acronym> using this tool.
    <?php endif; ?>
</p>
<?php else: ?>
<p>
	...
	<input id="cdn_import_library" class="button {nonce: '<?php echo nxt_create_nonce('w3tc'); ?>'}" type="button" value="importing attachments into the Media Library" />.
	Check <input id="cdn_queue" class="button {nonce: '<?php echo nxt_create_nonce('w3tc'); ?>'}" type="button" value="unsuccessful file transfers" /> if some objects appear to be missing.
    <?php if (w3_can_cdn_purge($cdn_engine)): ?>
    <input id="cdn_purge" class="button {nonce: '<?php echo nxt_create_nonce('w3tc'); ?>'}" type="button" value="Purge" /> objects from the <acronym title="Content Delivery Network">CDN</acronym> if needed.
    <?php endif; ?>
	<input id="cdn_rename_domain" class="button {nonce: '<?php echo nxt_create_nonce('w3tc'); ?>'}" type="button" value="Modify attachment URLs" /> if the domain name of your site has ever changed.
</p>
<?php endif; ?>
    ...
    <div class="metabox-holder">
        ...
        <table class="form-table">
            <tr>
                ...
                <?php if (! $cdn_mirror): ?>
                <td>
	                <input id="cdn_export_library" class="button {nonce: '<?php echo nxt_create_nonce('w3tc'); ?>'}" type="button" value="Upload attachments" />
                </td>
                <?php endif; ?>
            </tr>
        	<tr>
        		<th<?php if ($cdn_mirror): ?> colspan="2"<?php endif; ?>>
        			...
        			<label><input type="checkbox" name="cdn.includes.enable" value="1"<?php checked($this->_config->get_boolean('cdn.includes.enable'), true); ?> /> Host nxt-includes/ files</label><br />
    				<span class="description">If checked, NXTClass static core file types specified in the "nxt-includes file types to upload" field below will be hosted with the <acronym title="Content Delivery Network">CDN</acronym>.</span>
        		</th>
        		<?php if (! $cdn_mirror): ?>
        		<td>
        			<input class="button cdn_export {type: 'includes', nonce: '<?php echo nxt_create_nonce('w3tc'); ?>'}" type="button" value="Upload includes files" />
        		</td>
        		<?php endif; ?>
        	</tr>
        	<tr>
        		...
        		<?php if (! $cdn_mirror): ?>
        		<td>
    				<input class="button cdn_export {type: 'theme', nonce: '<?php echo nxt_create_nonce('w3tc'); ?>'}" type="button" value="Upload theme files" />
        		</td>
        		<?php endif; ?>
        	</tr>
        	<tr>
        		...
        		<?php if (! $cdn_mirror): ?>
        		<td>
    				<input class="button cdn_export {type: 'minify', nonce: '<?php echo nxt_create_nonce('w3tc'); ?>'}" type="button" value="Upload minify files"<?php if (!$minify_enabled): ?> disabled="disabled"<?php endif; ?> />
        		</td>
        		<?php endif; ?>
        	</tr>
        	<tr>
        		...
        		<?php if (! $cdn_mirror): ?>
        		<td>
    				<input class="button cdn_export {type: 'custom', nonce: '<?php echo nxt_create_nonce('w3tc'); ?>'}" type="button" value="Upload custom files" />
        		</td>
        		<?php endif; ?>
        	</tr>
        	<?php if (! $cdn_mirror): ?>
        ...
        <table class="form-table">
        	...
        	<?php endif; ?>
        	<tr>
        		<th style="width: 300px;"><label for="cdn_includes_files">nxt-includes file types to upload:</label></th>
        		<td>
        			...
        			<span class="description">Specify the file types within the NXTClass core to host with the <acronym title="Content Delivery Network">CDN</acronym>.</span>
        		</td>
        	</tr>
        	<tr>
