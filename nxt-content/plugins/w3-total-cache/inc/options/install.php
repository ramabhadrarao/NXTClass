...
<div id="install">
    <ol>
        <li>
        	Set the permissions of nxt-content/ back to 755, e.g.:
         	<pre class="console"># chmod 755 /var/www/vhosts/domain.com/httpdocs/nxt-content/</pre>
        </li>
...
# echo '/etc/init.d/memcached' &gt;&gt; /etc/rc.local</pre>
    ...
    <ol>
        ...
        <li>
        	...
        	<pre class="console"># cp /var/www/vhosts/domain.com/httpdocs/nxt-content/plugins/w3-total-cache/ini/memcache.ini /etc/php.d/</pre>
        </li>
        <li>
...
# whereis apxs</pre>
        ...
        <li>
			...
			<pre class="console"># cp /var/www/vhosts/domain.com/httpdocs/nxt-content/plugins/w3-total-cache/ini/apc.ini /etc/php.d/</pre>
		</li>
		<li>
    ...
    <ol>
        ...
        <li>
			...
			<pre class="console"># cp /var/www/vhosts/domain.com/httpdocs/nxt-content/plugins/w3-total-cache/ini/xcache.ini /etc/php.d/</pre>
		</li>
		<li>
			Restart apache when ready:
			<pre class="console"># /etc/init.d/httpd restart</pre>
    ...
    <ol>
        ...
        <li>
        	You can also use the eaccelerator.ini file we prepared for you:
			<pre class="console"># cp /var/www/vhosts/domain.com/httpdocs/nxt-content/plugins/w3-total-cache/ini/eaccelerator.ini /etc/php.d/</pre>
		</li>
		<li>
			Restart apache when ready:
			<pre class="console"># /etc/init.d/httpd restart</pre>
    ...
    <div class="metabox-holder">
        ...
        <table class="form-table">
            <tr>
                <th colspan="2">
                    <ul>
                        <li>The provided instructions are for 32-bit CentOS, however we can provide others based on <a href="mailto:nxtclassexperts@w3-edge.com">your requests</a>.</li>
                        ...
                        <li>In the case where Apache is not used, the .htaccess file located in the root directory of the NXTClass installation, nxt-content/w3tc/pgcache/.htaccess and nxt-content/w3tc/min/.htaccess contain directives that must be manually created for your web server software.</li>
                        <li>Restarting the web server will empty the opcode cache, which means it will have to be rebuilt over time and your site's performance will suffer during this period. Still, an opcode cache should be installed in any case to maximize NXTClass performance.</li>
