...
/**
 * Check if nxt permalink directives exists
 *
 * @return boolean
 */
function w3_is_permalink_rules() {
    if ((w3_is_apache() || w3_is_litespeed()) && !w3_is_network()) {
        ...
        return (($data = @file_get_contents($path)) && strstr($data, W3TC_MARKER_BEGIN_nxtclass) !== false);
    }
...
/**
 * Returns path of browsercache no404nxt rules file
 *
 * @return string
 */
function w3_get_browsercache_rules_no404nxt_path() {
    switch (true) {
        case w3_is_apache():
        case w3_is_litespeed():
            return w3_get_home_root() . '/.htaccess';
