<?php
/* Plugin Name: nxt-FB-AutoConnect
 ...
 * Plugin URI: http://www.justin-klein.com/projects/nxt-fb-autoconnect
 */
 ...
 * as a fully original work, nor attempt to confuse it with the original nxt-FB-AutoConnect by means of 
 ...
 * NOTE: The button tag itself maybe overwritten by the Premium addon (nxtfb_output_button filter)
 */
function jfb_output_facebook_btn()
{
    ...
    $btnTag = apply_filters('nxtfb_output_button', $btnTag );
    ...
    if( $email_perms && $stream_perms )    $attr = 'scope="'.apply_filters('nxtfb_extended_permissions','email,publish_stream').'"';
    else if( $email_perms )                $attr = 'scope="'.apply_filters('nxtfb_extended_permissions','email').'"';
    else if( $stream_perms )               $attr = 'scope="'.apply_filters('nxtfb_extended_permissions','publish_stream').'"';
    else                                   $attr = 'scope="'.apply_filters('nxtfb_extended_permissions','') . '"';
...
function jfb_output_facebook_instapopup( $callbackName=0 )
{
    ...
    add_action('nxtfb_add_to_asyncinit', 'jfb_invoke_instapopup');
    ?>
    <script type="text/javascript">//<!--
 ...
 */
add_action('nxt_footer', 'jfb_output_facebook_init');
function jfb_output_facebook_init()
{
    ...
    <script type="text/javascript">//<!--
      window.fbAsyncInit = function()
      {
        ...
        <?php do_action('nxtfb_add_to_asyncinit'); ?>            
      };

      (function() {
        var e = document.createElement('script');
        e.src = document.location.protocol + '//connect.facebook.net/<?php echo apply_filters('nxtfb_output_facebook_locale', 'en_US'); ?>/all.js';
        e.async = true;
 ...
 */
add_action('nxt_footer', 'jfb_output_facebook_callback');
...
?>
  ...
  <form id="nxt-fb-ac-fm" name="<?php echo $callbackName ?>_form" method="post" action="<?php echo plugins_url(dirname(plugin_basename(__FILE__))) . "/_process_login.php"?>" >
...
?>
      <?php nxt_nonce_field ($jfb_nonce_name, $jfb_nonce_name) ?>   
    </form>
<?php
...
<?php 
        ...
        echo    "    //Make sure the user logged in\n".
                ...
                "      if (!response.authResponse)\n".
                "      {\n".
                apply_filters('nxtfb_login_rejected', '').
                "      return;\n".
                "      }\n\n";
...
if( get_option($opt_jfb_show_credit) ) add_action('nxt_footer', 'jfb_show_credit');
function jfb_show_credit()
{
    global $jfb_homepage;
    echo "Facebook login by <a href=\"$jfb_homepage\">nxt-FB-AutoConnect</a>";
}
...
/**
 * Legacy Support: there used to be two separate options for nxt and BP; it's now just one option
 */
if( get_option($opt_jfb_bp_avatars) )
{
    delete_option($opt_jfb_bp_avatars);
    update_option($opt_jfb_nxt_avatars, 1);    
}


/**
  * Optionally replace nxtclass avatars with FACEBOOK profile pictures
  */
if( get_option($opt_jfb_nxt_avatars) ) add_filter('get_avatar', 'jfb_nxt_avatar', 10, 5);
function jfb_nxt_avatar($avatar, $id_or_email, $size, $default, $alt)
{
 ...
 */
if( get_option($opt_jfb_nxt_avatars) ) add_filter( 'bp_core_fetch_avatar', 'jfb_bp_avatar', 10, 4 );    
...
if( get_option($opt_jfb_username_style) == 1 || get_option($opt_jfb_username_style) == 2 || get_option($opt_jfb_username_style) == 3 ) add_filter( 'nxtfb_insert_user', 'jfb_pretty_username', 10, 2 );
function jfb_pretty_username( $nxt_userdata, $fb_userdata )
{
    global $jfb_log, $opt_jfb_username_style;
    $jfb_log .= "nxt: Converting username to \"pretty\" username...\n";
    ...
    {
        $jfb_log .= "nxt: Warning - Completely non-alphanumeric Facebook name cannot be used; leaving as default.\n";
        return $nxt_userdata;
    }
    ...
    //Done!
    $nxt_userdata['user_login']   = $username;
    $nxt_userdata['user_nicename']= $username;
    $jfb_log .= "nxt: Name successfully converted to $username.\n";
    return $nxt_userdata;
}
...
/**
  ...
  */
if( get_option($opt_jfb_ask_stream) ) add_action('nxtfb_inserted_user', 'jfb_post_to_wall');
...
/***************************Login Counting****************************/
/**********************************************************************/
add_action('nxtfb_login', 'jfb_count_login');
