...
/**
  ...
  **/
class Widget_LoginLogout extends nxt_Widget
{
    //////////////////////////////////////////////////////
    //Init the Widget
    function Widget_LoginLogout()
    { 
        $this->nxt_Widget( false, "nxt-FB AutoConnect Basic", array( 'description' => 'A sidebar Login/Logout form with Facebook Connect button' ) );
    }
    ...
    function widget( $args, $instance )
    {
        ...
        ?>
            <div style='text-align:center'>
              ...
              ?>!<br />
              <small>
                <a href="<?php echo get_option('siteurl')?>/nxt-admin/profile.php"><?php _e("Edit Profile")?></a> | <a href=" <?php echo nxt_logout_url( $_SERVER['REQUEST_URI'] )?>"><?php _e("Logout")?></a>
              </small>
            </div>
        <?php
        ...
        ?>
            <form name='loginform' id='loginform' action='<?php echo get_option('siteurl')?>/nxt-login.php' method='post'>
                <label>User:</label><br />
                ...
                <span id="forgotText"><a href="<?php echo get_option('siteurl')?>/nxt-login.php?action=lostpassword" rel="nofollow" ><?php _e('Forgot')?>?</a></span><br />
                <?php //echo "<input name='rememberme' type='hidden' id='rememberme' value='forever' />";?>
                <?php echo nxt_register('',''); ?>
