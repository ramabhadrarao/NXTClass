...
Tags: facebook connect, login with facebook, facebook autoconnect, facebook, connect, widget, login, logon, nxtclass, buddypress
...
The simple concept behind nxt-FB AutoConnect is to offer an easy-to-use widget that lets readers login to your blog with either their Facebook account or local nxtclass credentials. Although many "Facebook Connect" plugins do exist, most of them are either overly complex and difficult to customize, or fail to provide a seamless experience for new  visitors. I wrote this plugin to provide what the others didn't:

* Full support for both nxtclass and Buddypress.
...
1. Download the latest version from [here](http://nxtclass.org/extend/plugins/nxt-fb-autoconnect/), unzip it, and upload the extracted files to your plugins directory.
2. Login to your nxtclass admin panel and activate the plugin.
3. Navigate to Settings -> nxt-FB AutoConn.
4. Enter your App ID and Secret (obtained above), and click "Save."
5. If you're using BuddyPress, a Facebook button will automatically be added to its built-in login panel.  If not, navigate to Appearance -&gt; Widgets and add the nxt-FB AutoConnect widget to your sidebar. 

That's it - users should now be able to use the widget to login to your blog with their Facebook accounts.

For more information on exactly how this plugin's login process works and how it can be customized, see the [homepage](http://www.justin-klein.com/projects/nxt-fb-autoconnect).


== Frequently Asked Questions ==

[FAQ](http://www.justin-klein.com/projects/nxt-fb-autoconnect#faq)


== Screenshots ==

[Screenshots](http://www.justin-klein.com/projects/nxt-fb-autoconnect#demo)
...
* Check for compatibility with nxt3.3

= 2.1.9 (2011-12-13) =
* Removed an old (now unused) database option that was getting written on every pageload (thanks Aaron Frerichs!)

= 2.1.8 (2011-11-28) =
* Removed plugin sponsorship messages.  See [Automattic Bullies NXTClass Plugin Developers -- Again](http://gregsplugins.com/lib/2011/11/26/automattic-bullies/).

= 2.1.7 (2011-11-25) =
* Fix: nxtfb_extended_permissions filter was not being applied if neither "post to wall" nor "request email" options were checked.
* Add new BuddyPress XProfile mapping to the premium admin panel

= 2.1.6 (2011-11-24) =
* All new TABBED Admin panel!
* Better error reporting for "nxt_insert_user failed"
...
* Fix "nxt_insert_user failed" bug (since 2.1.0)
...
= 2.0.7 (2011-07-06) =
* Filtered avatars use the $alt provided by nxt core (or theme)
...
* Added a new "nxtfb_rememberme" filter, so you can override the above if desired

= 2.0.2 (2011-05-26) =
* Oops: nxtfb_userinfo_permissions was redundant; removed :)

= 2.0.1 (2011-05-26) =
* Handle relative paths in usermeta (for cached avatars)
* Get rid of some depreciated functions (get_usermeta -> get_user_meta)
* Add a new hook nxtfb_add_to_asyncinit, so you can output your own JS after the Facebook API initializes
* Add a new filter nxtfb_userinfo_permissions, so you can choose what userinfo permissions to ask for
* Check for function_exists('is_multisite') to support older versions of nxt
...
= 1.9.0 (2011-04-01) =
* jfb_output_facebook_callback() is now automatically called by nxt_footer; If you're manually outputting your own Facebook buttons, you should now ONLY call jfb_output_facebook_btn() (explicitly calling jfb_output_facebook_callback() is no longer needed!)
...
* New filter nxtfb_inserting_user to replace nxtfb_insert_user
...
= 1.8.5 (2011-03-24) =
* Add a new nxtfb_extended_permissions filter
...
= 1.6.5 (2011-01-28) =
* Add nxtfb_output_facebook_locale action

= 1.6.4 (2011-01-28) =
* Add nxtfb_login_rejected filter
...
* Add action nxtfb_after_button

= 1.6.2 (2011-01-27) =
* "Use Facebook profile pictures as avatars is now just one option" (they aren't separate for nxt and BP)
...
* Add nxtfb_admin_messages action
* Pass the callback name to nxtfb_add_to_js action

= 1.5.5 (2010-11-23) =
* Add support for a new option in the premium version
* More descriptive error message
* Add nxtfb_existing_user action
...
= 1.5.2 (2010-11-01) =
* Added new nxtfb_prelogin action
* Added new nxtfb_submit_loginfrm filter
* Added new nxtfb_output_button filter
...
= 1.4.4 (2010-10-30) =
* The nxtfb_inserted_user action now supplies the full userdata of the inserted user
...
* Added 2 new actions: nxtfb_add_to_js and nxtfb_add_to_form (Sponsored by [VideoUserManuals](http://9f200kliq7f39zam4ffc7wnk8b.hop.clickbank.net/))
...
= 1.3.2 (2010-08-15) =
* Do not fetch Facebook profile picture if not present (revert to default nxt/BP avatar)

= 1.3.1 (2010-08-14) =
* Fixed the "Object of class nxt_Error could not be converted to string" bug
...
= 1.2.5 (2010-08-08) =
* New Feature: Use Facebook profile pictures as nxtclass avatars
...
= 1.2.0 (2010-07-14) =
* BuddyPress usernames generated via "First Name + Last Name" instead of "Name" (as reported [here](http://www.justin-klein.com/projects/nxt-fb-autoconnect/comment-page-6#comment-12258))
...
= 1.1.8 (2010-05-17) =
* Added action nxtfb_inserted_user to run *after* a user is inserted
* Fixed "Require Email" option

= 1.1.7 (2010-04-11) =
* Minor change: Use nxt_generate_password() for autogenerated passwords
...
* Add nxtfb_insert_user filter to run just before inserting an auto-created user
...
= 1.0.4 (2010-03-17) =
* Include the Facebook javascript in jfb_output_facebook_init() instead of nxt_head
...
* Add nxtfb_connect hook that runs BEFORE a login is allowed
...
= 1.0.3 (2010-03-16) =
* Hopefully fix the "Call to undefined function nxt_insert_user()" bug
...
Please direct all support requests [here](http://www.justin-klein.com/projects/nxt-fb-autoconnect#feedback)