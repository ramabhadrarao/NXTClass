 ...
 * @subpackage HTTP
 * @since 2.7.0
 * @author Jacob Santos <nxtclass@santosj.name>
 */

/**
 * NXTClass HTTP Class for managing HTTP Transports and making HTTP requests.
 *
 ...
 * be added later and should be a patch on the NXTClass Trac.
 *
...
class nxt_Http {
	 ...
	 *
	 * PHP4 does not have the 'self' keyword and since NXTClass supports PHP4,
	...
	function __construct() {
		nxt_Http::_getTransport();
		nxt_Http::_postTransport();
	}

	/**
	 * Tests the NXTClass HTTP objects for an object to use and returns it.
	 *
	...
	function &_getTransport( $args = array() ) {
		...
		if ( is_null($working_transport) ) {
			if ( true === nxt_Http_ExtHttp::test($args) ) {
				$working_transport['exthttp'] = new nxt_Http_ExtHttp();
				$blocking_transport[] = &$working_transport['exthttp'];
			} else if ( true === nxt_Http_Fsockopen::test($args) ) {
				$working_transport['fsockopen'] = new nxt_Http_Fsockopen();
				$blocking_transport[] = &$working_transport['fsockopen'];
			} else if ( true === nxt_Http_Streams::test($args) ) {
				$working_transport['streams'] = new nxt_Http_Streams();
				$blocking_transport[] = &$working_transport['streams'];
			} else if ( true === nxt_Http_Fopen::test($args) ) {
				$working_transport['fopen'] = new nxt_Http_Fopen();
				$blocking_transport[] = &$working_transport['fopen'];
			} else if ( true === nxt_Http_Curl::test($args) ) {
				$working_transport['curl'] = new nxt_Http_Curl();
	...
	function request( $url, $args = array() ) {
		$defaults = array(
			...
			'user-agent' => apply_filters( 'http_headers_useragent', backpress_get_option( 'nxt_http_version' ) ),
			'blocking' => true,
			'headers' => array(),
			'cookies' => array(),
			'body' => null,
		...
		if ( $this->block_request( $url ) )
			return new nxt_Error('http_request_failed', 'User has blocked requests through HTTP.');

		// Determine if this is a https call and pass that on to the transport functions
		// so that we can blacklist the transports that do not support ssl verification
		$r['ssl'] = $arrURL['scheme'] == 'https' || $arrURL['scheme'] == 'ssl';

		// Determine if this request is to OUR install of NXTClass
		...
		if ( nxt_Http_Encoding::is_available() )
			$r['headers']['Accept-Encoding'] = nxt_Http_Encoding::accept_encoding();

		if ( is_null($r['body']) ) {
			// Some servers fail when sending content without the content-length
			// header being set.
			$r['headers']['Content-Length'] = 0;
			$transports = nxt_Http::_getTransport($r);
		} else {
			...
			$transports = nxt_Http::_postTransport($r);
		}
		...
		foreach ( $headers as $tempheader ) {
			...
			if ( !empty( $value ) ) {
				...
				if ( 'set-cookie' == strtolower( $key ) )
					$cookies[] = new nxt_Http_Cookie( $value );
			}
		}
	 ...
	 *
	 * If it's found, then it's assumed to contain nxt_Http_Cookie objects, which are each parsed
	 ...
	 *
	 * You block external URL requests by defining nxt_HTTP_BLOCK_EXTERNAL in your nxt-config.php file
	 ...
	 * nxt_ACCESSIBLE_HOSTS constant is a comma separated list of hostnames to allow.
	 *
	 * @since 2.8.0
	...
	function block_request($uri) {
		...
		if ( ! defined('nxt_HTTP_BLOCK_EXTERNAL') || ( defined('nxt_HTTP_BLOCK_EXTERNAL') && nxt_HTTP_BLOCK_EXTERNAL == false ) )
			return false;
		 ...
		 * NXTClass does not do any checking currently for non-proxy requests, so it is keeps with
		...
		if ( !defined('nxt_ACCESSIBLE_HOSTS') )
			return true;

		static $accessible_hosts;
		if ( null == $accessible_hosts )
			$accessible_hosts = preg_split('|,\s*|', nxt_ACCESSIBLE_HOSTS);
 ...
 * @since 2.7.0
 */
class nxt_Http_Fsockopen {
	/**
	 * Send a HTTP request to a URI using fsockopen().
	 *
	 * Does not support non-blocking mode.
	 *
	 * @see nxt_Http::request For default options descriptions.
	 *
	 * @since 2.7
	 * @access public
	 * @param string $url URI resource.
	...
	function request($url, $args = array()) {
		...
		if ( !defined('nxt_DEBUG') || ( defined('nxt_DEBUG') && false === nxt_DEBUG ) ) {
		...
		if ( false === $handle )
			return new nxt_Error('http_request_failed', $iError . ': ' . $strError);
		...
		$process = nxt_Http::processResponse($strResponse);
		$arrHeaders = nxt_Http::processHeaders($process['headers']);

		// Is the response code within the 400 range?
		if ( (int) $arrHeaders['response']['code'] >= 400 && (int) $arrHeaders['response']['code'] < 500 )
			return new nxt_Error('http_request_failed', $arrHeaders['response']['code'] . ': ' . $arrHeaders['response']['message']);
		...
		if ( ! empty( $process['body'] ) && isset( $arrHeaders['headers']['transfer-encoding'] ) && 'chunked' == $arrHeaders['headers']['transfer-encoding'] )
			$process['body'] = nxt_Http::chunkTransferDecode($process['body']);

		if ( true === $r['decompress'] && true === nxt_Http_Encoding::should_decode($arrHeaders['headers']) )
			$process['body'] = nxt_Http_Encoding::decompress( $process['body'] );
...
class nxt_Http_Fopen {
	 ...
	 *
	 * @see nxt_Http::retrieve For default options descriptions.
	 *
	 * @access public
	 * @since 2.7.0
	 *
	...
	function request($url, $args = array()) {
		...
		if ( false === $arrURL )
			return new nxt_Error('http_request_failed', sprintf(__('Malformed URL: %s'), $url));
		...
		if (! $handle)
			return new nxt_Error('http_request_failed', sprintf(__('Could not open handle for fopen() to %s'), $url));
		...
		$processedHeaders = nxt_Http::processHeaders($theHeaders);

		if ( ! empty( $strResponse ) && isset( $processedHeaders['headers']['transfer-encoding'] ) && 'chunked' == $processedHeaders['headers']['transfer-encoding'] )
			$strResponse = nxt_Http::chunkTransferDecode($strResponse);

		if ( true === $r['decompress'] && true === nxt_Http_Encoding::should_decode($processedHeaders['headers']) )
			$strResponse = nxt_Http_Encoding::decompress( $strResponse );
...
class nxt_Http_Streams {
	...
	function request($url, $args = array()) {
		...
		// NXTClass supports PHP 4.3, which has this function. Removed sanity checking for
		// performance reasons.
		stream_set_timeout($handle, $r['timeout'] );
		...
		if ( isset( $meta['wrapper_data']['headers'] ) )
			$processedHeaders = nxt_Http::processHeaders($meta['wrapper_data']['headers']);
		else
			$processedHeaders = nxt_Http::processHeaders($meta['wrapper_data']);
...
class nxt_Http_ExtHTTP {
	...
	function request($url, $args = array()) {
		...
		if ( !defined('nxt_DEBUG') || ( defined('nxt_DEBUG') && false === nxt_DEBUG ) ) //Emits warning level notices for max redirects and timeouts
		...
		if ( false === $strResponse || ! empty($info['error']) )
			return new nxt_Error('http_request_failed', $info['response_code'] . ': ' . $info['error']);
...
/**
 * Adds Proxy support to the NXTClass HTTP API.
 *
 * There are caveats to proxy support. It requires that defines be made in the nxt-config.php file to
 * enable proxy support. There are also a few filters that plugins can hook into for some of the
 * constants.
 *
 * The constants are as follows:
 * <ol>
 * <li>nxt_PROXY_HOST - Enable proxy support and host for connecting.</li>
 * <li>nxt_PROXY_PORT - Proxy port for connection. No default, must be defined.</li>
 * <li>nxt_PROXY_USERNAME - Proxy username, if it requires authentication.</li>
 ...
 * define('nxt_PROXY_BYPASS_HOSTS', 'localhost, www.example.com');
 * </code>
 *
...
class nxt_HTTP_Proxy {
	 ...
	 */
	function is_enabled() {
		return defined('nxt_PROXY_HOST') && defined('nxt_PROXY_PORT');
	}
	 ...
	 *
	 * @since 2.8
	 * @use nxt_PROXY_USERNAME
	 * @use nxt_PROXY_PASSWORD
	 *
	 * @return bool
	 */
	function use_authentication() {
		return defined('nxt_PROXY_USERNAME') && defined('nxt_PROXY_PASSWORD');
	}
	 ...
	 */
	function host() {
		if ( defined('nxt_PROXY_HOST') )
			return nxt_PROXY_HOST;
	 ...
	 */
	function port() {
		if ( defined('nxt_PROXY_PORT') )
			return nxt_PROXY_PORT;
	 ...
	 */
	function username() {
		if ( defined('nxt_PROXY_USERNAME') )
			return nxt_PROXY_USERNAME;
	 ...
	 */
	function password() {
		if ( defined('nxt_PROXY_PASSWORD') )
			return nxt_PROXY_PASSWORD;
	...
	function send_through_proxy( $uri ) {
		...
		if ( !defined('nxt_PROXY_BYPASS_HOSTS') )
			return true;

		static $bypass_hosts;
		if ( null == $bypass_hosts )
			$bypass_hosts = preg_split('|,\s*|', nxt_PROXY_BYPASS_HOSTS);
 ...
 * already a nxt_Http_Cookie() object, then they are turned into one.
 *
 * @todo The NXTClass convention is to use underscores instead of camelCase for function and method
...
class nxt_Http_Cookie {
	...
	/**
	 * Retrieve cookie header for usage in the rest of the NXTClass HTTP API.
	 *
	 * @access public
	 * @since 2.8.0
	 *
...
/**
 * Returns the initialized nxt_Http Object
 *
 * @since 2.7.0
 * @access private
 *
 * @return nxt_Http HTTP Transport object.
 */
function &_nxt_http_get_object() {
	static $http;
 ...
 */
function nxt_remote_request($url, $args = array()) {
 ...
 * @return nxt_Error|array The response or nxt_Error on failure.
 */
function nxt_remote_get($url, $args = array()) {
	$objFetchSite = _nxt_http_get_object();
 ...
 * @return nxt_Error|array The response or nxt_Error on failure.
 */
function nxt_remote_post($url, $args = array()) {
	$objFetchSite = _nxt_http_get_object();
 ...
 */
function nxt_remote_retrieve_headers(&$response) {
	if ( is_nxt_error($response) || ! isset($response['headers']) || ! is_array($response['headers']))
		return array();
 ...
 */
function nxt_remote_retrieve_header(&$response, $header) {
	if ( is_nxt_error($response) || ! isset($response['headers']) || ! is_array($response['headers']))
		return '';
 ...
 */
function nxt_remote_retrieve_response_code(&$response) {
	if ( is_nxt_error($response) || ! isset($response['response']) || ! is_array($response['response']))
		return '';
 ...
 */
function nxt_remote_retrieve_response_message(&$response) {
	if ( is_nxt_error($response) || ! isset($response['response']) || ! is_array($response['response']))
		return '';
