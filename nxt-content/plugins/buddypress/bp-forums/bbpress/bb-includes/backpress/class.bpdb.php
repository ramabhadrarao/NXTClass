...
class BPDB
{
	...
	function set_prefix( $prefix, $tables = false )
	{
		...
		if ( preg_match( '|[^a-z0-9_]|i', $prefix ) ) {
			return new nxt_Error( 'invalid_db_prefix', 'Invalid database prefix' ); // No gettext here
		}
	...
	function get_error( $str = '' )
	{
		...
		if ( class_exists( 'nxt_Error' ) ) {
			return new nxt_Error( 'db_query', $error_str, array( 'query' => $this->last_query, 'error' => $str, 'caller' => $caller ) );
		} else {
	...
	function print_error( $str = '' )
	{
		...
		if ( is_object( $error ) && is_a( $error, 'nxt_Error' ) ) {
	...
	function bail( $message )
	{
		if ( !$this->show_errors ) {
			if ( class_exists( 'nxt_Error' ) )
				$this->error = new nxt_Error( '500', $message );
			else
				$this->error = $message;
			return false;
		}
	...
	function check_database_version( $dbh_or_table = false )
	{
		...
		if ( version_compare( $this->db_version( $dbh_or_table ), '4.0.0', '<' ) ) {
			return new nxt_Error( 'database_version', BPDB__DB_VERSION_ERROR );
		}
	}
