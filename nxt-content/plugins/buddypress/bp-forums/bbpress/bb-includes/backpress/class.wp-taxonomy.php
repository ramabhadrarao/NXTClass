<?php
// Last sync [nxt11537] - Refactored into a class based on nxt-includes/taxonomy.php
...
/**
 * NXTClass Taxonomy based off of NXTClass revision 8782.
 *
 * @since 2.3.0
 */
class nxt_Taxonomy {
	...
	function &get_term($term, $taxonomy, $output = OBJECT, $filter = 'raw') {
		...
		if ( is_object($term) ) {
			...
			if ( ! $_term = nxt_cache_get($term, $taxonomy) ) {
				...
				nxt_cache_add($_term->term_taxonomy_id, $_term->term_id, "$taxonomy:tt_id" );
			}
		}
	...
	function &get_terms($taxonomies, $args = '') {
		...
		if ( empty($terms) ) {
			nxt_cache_add( $cache_key, array(), 'terms' );
		...
		nxt_cache_add( $cache_key, $terms, 'terms' );
	...
	function insert_term( $term, $taxonomy, $args = array() ) {
		...
		if ( ! $term_id = $this->is_term($slug) ) {
			if ( false === $this->db->insert( $this->db->terms, compact( 'name', 'slug', 'term_group' ) ) )
				return new nxt_Error('db_insert_error', __('Could not insert term into the database'), $this->db->last_error);
