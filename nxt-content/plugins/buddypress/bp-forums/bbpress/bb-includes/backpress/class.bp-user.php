...
class BP_User {
	...
	function BP_User( $id, $name = '' ) {
		...
		if ( ! empty( $id ) )
			$this->data = $nxt_users_object->get_user( $id );
		else
			$this->data = $nxt_users_object->get_user( $name, array( 'by' => 'login' ) );
	...
	function _init_caps() {
		global $nxt_users_object;
		$this->cap_key = $nxt_users_object->db->prefix . 'capabilities';
	...
	function get_role_caps() {
		global $nxt_roles, $nxt_users_object;

		if ( ! isset( $nxt_roles ) )
			$nxt_roles = new BP_Roles( $nxt_users_object->db );
	...
	function update_user() {
		global $nxt_users_object;
		$nxt_users_object->update_meta( array( 'id' => $this->ID, 'meta_key' => $this->cap_key, 'meta_value' => $this->caps ) );
		$this->get_role_caps();
		//$this->update_user_level_from_caps(); // nxt
	}
...
/*
	function update_user_level_from_caps() {
		...
		update_usermeta( $this->ID, $nxtdb->prefix.'user_level', $this->user_level );
	}
*/
...
*/
	...
	function remove_all_caps() {
		global $nxt_users_object;
		$this->caps = array();
		$nxt_users_object->delete_meta( $this->ID, $this->cap_key );
		$this->get_role_caps();
	}
	...
	function has_cap( $cap ) {
		...
		$caps = call_user_func_array( array(&$nxt_roles, 'map_meta_cap'), $args );
