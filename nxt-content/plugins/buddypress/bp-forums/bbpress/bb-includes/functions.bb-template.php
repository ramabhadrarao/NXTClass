...
function &bb_forums( $args = '' ) {
	...
	$args = nxt_parse_args( $args, array('hierarchical' => true, 'type' => $default_type, 'walker' => 'BB_Walker_Blank') );
...
function bb_get_topic_delete_link( $args = '' ) {
	...
	if ( 0 == $topic->topic_status ) {
		...
		$query   = array( 'id' => $topic->topic_id, '_nxt_http_referer' => $redirect ? rawurlencode( $redirect ) : false );
		...
		$query   = array('id' => $topic->topic_id, 'view' => 'all', '_nxt_http_referer' => $redirect ? rawurlencode( $redirect ) : false );
...
function bb_get_topic_close_link( $args = '' ) {
	...
	$uri = bb_get_uri('bb-admin/topic-toggle.php', array( 'id' => $topic->topic_id, '_nxt_http_referer' => $redirect ? rawurlencode( $redirect ) : false ), BB_URI_CONTEXT_A_HREF + BB_URI_CONTEXT_BB_ADMIN);
...
function bb_get_topic_move_dropdown( $args = '' )
{
	...
	$r .= $dropdown . "\n";
	$r .= bb_nonce_field( 'move-topic_' . $topic->topic_id, '_nxtnonce', true , false );
...
function bb_post_admin( $args = null )
{
	...
	if ( isset( $args['each'] ) ) {
		...
		foreach ( $each_args as $_part_name => $_part_args ) {
			...
			$_each_args[$_part_name] = nxt_parse_args( $_part_args, $defaults['each'][$_part_name] );
		}
	}
	$args = nxt_parse_args( $args, $defaults );
	if ( isset( $_each_args ) ) {
...
function bb_get_post_delete_link( $args = null )
{
	...
	$uri = bb_get_uri('bb-admin/delete-post.php', array(
		'id' => $bb_post->post_id,
		'status' => 1,
		'_nxt_http_referer' => $redirect ? rawurlencode( $redirect ) : false
...
function get_allowed_markup( $args = '' ) {
	$args = nxt_parse_args( $args, array('format' => 'flat') );
	extract($args, EXTR_SKIP);
...
function get_user_type_label( $type ) {
	global $nxt_roles;
	if ( $nxt_roles->is_role( $type ) )
		return apply_filters( 'get_user_type_label', $nxt_roles->role_names[$type], $type );
}
