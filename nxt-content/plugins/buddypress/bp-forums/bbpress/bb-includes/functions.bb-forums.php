...
function _bb_get_cached_data( $keys, $group, $callback ) {
	$return = array();
	foreach ( $keys as $key ) {
		// should use nxt_cache_get_multi if available
		if ( false === $value = nxt_cache_get( $key, $group ) ) {
...
function bb_get_forums( $args = null ) {
	...
	if ( false !== $forum_ids = nxt_cache_get( $key, 'bb_forums' ) ) {
		...
		foreach ( $_forums as $f ) {
			...
			nxt_cache_add( $f->forum_id, $f, 'bb_forum' );
			nxt_cache_add( $f->forum_slug, $f->forum_id, 'bb_forum_slug' );
		}
		nxt_cache_set( $key, $forum_ids, 'bb_forums' );
	}
