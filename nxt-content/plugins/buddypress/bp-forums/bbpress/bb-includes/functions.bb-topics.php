	...
	if ( 'topic' === bb_get_path(0, false, $uri) ) {
		...
		if ($parsed_uri = parse_url($uri)) {
			if (preg_match('@/topic\.php$@'  ,$parsed_uri['path'])) {
				$args = nxt_parse_args($parsed_uri['query']);
				if (isset($args['id'])) {
...
function bb_new_topic( $title, $forum, $tags = '', $args = '' ) {
	...
	$forum = (int) $forum;
	return bb_insert_topic( nxt_parse_args( $args ) + array( 'topic_title' => $title, 'forum_id' => $forum, 'tags' => $tags ) );
}
...
function bb_delete_topic( $topic_id, $new_status = 0 ) {
	...
	if ( $topic = get_topic( $topic_id ) ) {
		...
		nxt_cache_delete( $topic->forum_id, 'bb_forum' );
		nxt_cache_flush( 'bb_forums' );
		nxt_cache_flush( 'bb_query' );
		nxt_cache_flush( 'bb_cache_posts_post_ids' );
		return $topic_id;
