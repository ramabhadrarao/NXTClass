...
//			'post_id_only', // true, *false: this query is only returning post IDs
	...
	function parse_query() {
		...
		if ( is_array($this->query) )
			...
			nxt_parse_str($this->query, $this->query_vars);
	...
	function error( $code, $message ) {
		if ( is_nxt_error($this->errors) )
			$this->errors->add( $code, $message );
		else
			$this->errors = new nxt_Error( $code, $message );
	}
}
...
class BB_Query_Form extends BB_Query {
	...
	function BB_Query_Form( $type = 'topic', $defaults = '', $allowed = '', $id = '' ) {
		$this->defaults = nxt_parse_args( $defaults );
		$this->allowed  = nxt_parse_args( $allowed );
