...
function bb_default_scripts( &$scripts ) {
	...
	$scripts->add( 'topic', $scripts->base_url . 'js/topic.js', array('nxt-lists'), '20090602' );
	$scripts->add( 'profile-edit', $scripts->base_url . 'js/profile-edit.js', array('password-strength-meter'), '20080721' );
	$scripts->add( 'admin-forums', $scripts->base_url_admin . 'js/admin-forums.js', array('nxt-lists', 'interface'), '20090320' );
	...
	$scripts->add( 'nxt-lists', $scripts->base_url . 'js/nxt-lists.js', array('nxt-ajax-response','jquery-color'), '20080826' );
	$scripts->localize( 'nxt-lists', 'nxtListL10n', array(
		'url' => $scripts->base_url_admin . 'admin-ajax.php'
	) );
	$scripts->add( 'nxt-ajax-response', $scripts->base_url . 'js/nxt-ajax-response.js', array('jquery'), '20080316' );
...
function bb_just_in_time_script_localization() {
	nxt_localize_script( 'topic', 'bbTopicJS', array(
...
add_action( 'nxt_default_scripts', 'bb_default_scripts' );
add_filter( 'nxt_print_scripts', 'bb_just_in_time_script_localization' );
