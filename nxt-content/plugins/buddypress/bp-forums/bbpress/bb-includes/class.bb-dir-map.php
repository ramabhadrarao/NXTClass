...
class BB_Dir_Map {
	...
	function BB_Dir_Map( $root, $args = '' ) {
		if ( !is_dir( $root ) ) {
			$this->error = new nxt_Error( 'bb_dir_map', __('Not a valid directory') );
			return;
		}

		$this->parse_args( $args );
		if ( is_null($this->apply_to) || is_null($this->dots) ) {
			$this->error = new nxt_Error( 'bb_dir_map', __('Invalid arguments') );
			return;
		}
	...
	function get_results() {
		return is_nxt_error( $this->error ) ? $this->error : $this->flat;
	}
}
