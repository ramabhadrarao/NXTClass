...
function bb_delete_meta( $object_id = 0, $meta_key, $meta_value, $type, $global = false )
{
	...
	switch ( $type ) {
		...
		case 'user':
			global $nxt_users_object;
			$id = $object_id;
			return $nxt_users_object->delete_meta( compact( 'id', 'meta_key', 'meta_value' ) );
			break;
		case 'forum':
			$object_type = 'bb_forum';
			break;
			...
			if ( !$r ) {
				switch ( $option ) {
					...
					case 'nxt_table_prefix' :
						global $nxt_table_prefix; // This global is deprecated
						return $nxt_table_prefix; // Don't filter;
						break;
					case 'mod_rewrite':
						$r = 0;
						break;
...
function bb_get_option_from_db( $option )
{
	...
	if ( nxt_cache_get( $option, 'bb_option_not_set' ) ) {
		$r = null;
	} elseif ( false !== $_r = nxt_cache_get( $option, 'bb_option' ) ) {
		$r = $_r;
	} else {
		if ( BB_INSTALLING ) {
	...
	if ( $r === null ) {
		nxt_cache_set( $option, true, 'bb_option_not_set' );
	} else {
		nxt_cache_set( $option, $r, 'bb_option' );
	}
...
function bb_cache_all_options()
{
	$base_options = array(
		...
		'nxt_admin_cookie_path',
		'nxt_plugins_cookie_path',
		'nxtclass_mu_primary_blog_id',
		'enable_loginless',
...
// $meta_value defaults to null to conform to BackPress' nxt_User::delete_meta()
