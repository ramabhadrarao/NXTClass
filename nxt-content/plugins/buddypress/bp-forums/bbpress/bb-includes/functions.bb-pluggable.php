...
function bb_auth( $scheme = 'auth' ) { // Checks if a user has a valid cookie, if not redirects them to the main page
	if ( !bb_validate_auth_cookie( '', $scheme ) ) {
		...
		if ( 'auth' === $scheme && !bb_is_user_logged_in() ) {
			...
			nxt_redirect( bb_get_uri( 'bb-login.php', array( 'redirect_to' => $protocol . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'] ), BB_URI_CONTEXT_HEADER + BB_URI_CONTEXT_BB_USER_FORMS ) );
		} else {
...
function bb_check_login($user, $pass, $already_md5 = false) {
	...
	if ( !bb_get_option( 'email_login' ) || false === strpos( $user, '@' ) ) { // user_login
		$user = $nxt_users_object->get_user( $user, array( 'by' => 'login' ) );
	} else { // maybe an email
		$email_user = $nxt_users_object->get_user( $user, array( 'by' => 'email' ) );
		$user = $nxt_users_object->get_user( $user, array( 'by' => 'login' ) );
		// 9 cases.  each can be FALSE, USER, or nxt_ERROR
		if (
			( !$email_user && $user ) // FALSE && USER, FALSE && nxt_ERROR
		||
			( is_nxt_error( $email_user ) && $user && !is_nxt_error( $user ) ) // nxt_ERROR && USER
		) {
			// nope: it really was a user_login
			// [sic]: use $user
		} elseif (
			( $email_user && !$user ) // USER && FALSE, nxt_ERROR && FALSE
		||
			( $email_user && !is_nxt_error( $email_user ) && is_nxt_error( $user ) ) // USER && nxt_ERROR
		) {
			// yup: it was an email
			$user =& $email_user;
			...
			return false;
		} elseif ( is_nxt_error( $email_user ) && is_nxt_error( $user ) ) { // nxt_ERROR && nxt_ERROR
...
function bb_get_current_user() {
	global $nxt_auth_object;
	return $nxt_auth_object->get_current_user();
}
endif;
...
function bb_set_current_user( $id ) {
	global $nxt_auth_object;
	$current_user = $nxt_auth_object->set_current_user( $id );
...
function bb_validate_auth_cookie( $cookie = '', $scheme = 'auth' ) {
	...
	return $nxt_auth_object->validate_auth_cookie( $cookie, $scheme );
}
endif;
...
function bb_set_auth_cookie( $user_id, $remember = false, $schemes = false ) {
	...
	foreach ( $schemes as $scheme ) {
		$nxt_auth_object->set_auth_cookie( $user_id, $expiration, $expire, $scheme );
	}
}
endif;
...
function bb_clear_auth_cookie() {
	global $bb, $nxt_auth_object;
	
	$nxt_auth_object->clear_auth_cookie();
...
if ( !function_exists('nxt_redirect') ) : // [nxt11537]
/**
...
if ( !function_exists('nxt_sanitize_redirect') ) : // [nxt11537]
/**
...
if ( !function_exists('bb_safe_redirect') ) : // based on [nxt6145] (home is different)
/**
...
// Not verbatim nxt, constants have different names, uses helper functions.
if ( !function_exists( 'bb_salt' ) ) :
/**
 * Get salt to add to hashes to help prevent attacks.
 *
 * @since 0.9
 * @link http://api.nxtclass.org/secret-key/1.1/bbpress/ Create a set of keys for bb-config.php
 * @uses _bb_get_key()
 * @uses _bb_get_salt()
 *
...
function bb_generate_password( $length = 12, $special_chars = true ) {
	return nxt_Pass::generate_password( $length, $special_chars );
}
endif;

if ( !function_exists('bb_check_admin_referer') ) :
function bb_check_admin_referer( $action = -1, $query_arg = '_nxtnonce' ) {
	$nonce = '';
...
function bb_new_user( $user_login, $user_email, $user_url, $user_status = 1 ) {
	...
	if ( !$user_email = is_email( $user_email ) )
		return new nxt_Error( 'user_email', __( 'Invalid email address' ), $user_email );

	if ( !$user_login = sanitize_user( $user_login, true ) )
		return new nxt_Error( 'user_login', __( 'Invalid username' ), $user_login );
	...
	if ( is_nxt_error($user) ) {
		if ( 'user_nicename' == $user->get_error_code() )
			return new nxt_Error( 'user_login', $user->get_error_message() );
		return $user;
	}
