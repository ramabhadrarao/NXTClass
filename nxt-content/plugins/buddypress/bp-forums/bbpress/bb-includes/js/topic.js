...
jQuery( function($) {
	...
	$('#tags-list').nxtList( { alt: '', delBefore: tagsDelBefore } );

	// Favorites
	var favoritesToggle = $('#favorite-toggle')
		.addClass( 'list:favorite' )
		.nxtList( { alt: '', dimAfter: favLinkSetup } );
	...
	function favLinkSetup() {
		...
		favoritesToggle.get(0).nxtList.process( favoritesToggle );
	}
	...
	$('#thread').addClass( 'list:post' ).nxtList( { alt: 'alt', altOffset: 1, confirm: postConfirm, delAfter: function( r, s ) {
		try {
			// If we deleted the only post, we got an nxt AJAX Response object back with a URL to redirect to
