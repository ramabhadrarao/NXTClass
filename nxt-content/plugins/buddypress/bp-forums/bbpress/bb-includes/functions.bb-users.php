...
function bb_get_user( $user_id, $args = null ) {
	global $bbdb, $nxt_users_object;

	// Get user
	$user = $nxt_users_object->get_user( $user_id, $args );

	// Return on no user or error object
	if ( !is_object( $user ) || is_nxt_error( $user ) )
		return false;
...
function bb_cache_users( $ids ) {
	...
	$nxt_users_object->get_user( $ids );
}

function bb_get_user_by_nicename( $nicename ) {
	global $nxt_users_object;
	$user = $nxt_users_object->get_user( $nicename, array( 'by' => 'nicename' ) );
...
function bb_update_user_status( $user_id, $user_status = 0 ) {
	...
	$nxt_users_object->update_user( $user->ID, compact( 'user_status' ) );
}
...
function bb_is_trusted_user( $user ) { // ID, user_login, nxt_User, DB user obj
...
function bb_apply_nxt_role_map_to_user( $user, $reload = true ) {
	...
	if ( !$nxtclass_table_prefix = bb_get_option('nxt_table_prefix') ) {
		return;
	}

	if ( $nxtclass_mu_primary_blog_id = bb_get_option( 'nxtclass_mu_primary_blog_id' ) ) {
		$nxtclass_table_prefix .= $nxtclass_mu_primary_blog_id . '_';
	}

	if ( !$nxtclass_roles_map = bb_get_option( 'nxt_roles_map' ) ) {
		return;
	}
	...
	if ( !$bbpress_roles_map ) {
		...
		foreach ( $nxt_roles->get_names() as $_bbpress_role => $_bbpress_rolename ) {
			$bbpress_roles_map[$_bbpress_role] = 'subscriber';
		}
		unset( $_bbpress_role, $_bbpress_rolename );
		$bbpress_roles_map = array_merge( $bbpress_roles_map, array_flip( $nxtclass_roles_map ) );
		unset( $bbpress_roles_map['inactive'], $bbpress_roles_map['blocked'] );
	}

	static $nxtclass_userlevel_map = array(
		'administrator' => 10,
		'editor' => 7,
		'author' => 2,
		'contributor' => 1,
	...
	$nxtclass_roles = bb_get_usermeta( $user, $nxtclass_table_prefix . 'capabilities' );

	if ( !$bbpress_roles && is_array( $nxtclass_roles ) ) {
		$bbpress_roles_new = array();

		foreach ( $nxtclass_roles as $nxtclass_role => $nxtclass_role_value ) {
			if ( $nxtclass_roles_map[strtolower( $nxtclass_role )] && $nxtclass_role_value ) {
				$bbpress_roles_new[$nxtclass_roles_map[strtolower( $nxtclass_role )]] = true;
			}
		}
	...
	} elseif ( !$nxtclass_roles && is_array( $bbpress_roles ) ) {
		$nxtclass_roles_new = array();

		foreach ( $bbpress_roles as $bbpress_role => $bbpress_role_value ) {
			if ( $bbpress_roles_map[strtolower( $bbpress_role )] && $bbpress_role_value ) {
				$nxtclass_roles_new[$bbpress_roles_map[strtolower( $bbpress_role )]] = true;
				$nxtclass_userlevels_new[] = $nxtclass_userlevel_map[$bbpress_roles_map[strtolower( $bbpress_role )]];
			}
		}

		if ( count( $nxtclass_roles_new ) ) {
			bb_update_usermeta( $user, $nxtclass_table_prefix . 'capabilities', $nxtclass_roles_new );
			bb_update_usermeta( $user, $nxtclass_table_prefix . 'user_level', max( $nxtclass_userlevels_new ) );
		}
	}
}

function bb_apply_nxt_role_map_to_orphans() {
	if ( !$nxtclass_table_prefix = bb_get_option('nxt_table_prefix') ) {
		return;
	}
	...
	$role_query = <<<EOQ
		...
		LEFT JOIN `%2\$s` AS nxtrole
			ON ID = nxtrole.user_id
			AND nxtrole.meta_key = '%4\$scapabilities'
		WHERE
			bbrole.meta_key IS NULL OR
			bbrole.meta_value IS NULL OR
			nxtrole.meta_key IS NULL OR
			nxtrole.meta_value IS NULL
		ORDER BY
			ID
EOQ;
	...
	$role_query = $bbdb->prepare( $role_query, $bbdb->users, $bbdb->usermeta, $bbdb->prefix, $nxtclass_table_prefix );

	if ( $user_ids = $bbdb->get_col( $role_query ) ) {
		foreach ( $user_ids as $user_id ) {
			bb_apply_nxt_role_map_to_user( $user_id, false );
		}
	}
}
...
function bb_update_user( $user_id, $user_email, $user_url, $display_name ) {
	...
	$nxt_users_object->update_user( $user_id, compact( 'user_email', 'user_url', 'display_name' ) );
...
function bb_reset_email( $user_login )
{
	...
	if ( !$user = $bbdb->get_row( $bbdb->prepare( "SELECT * FROM $bbdb->users WHERE user_login = %s", $user_login ) ) ) {
		return new nxt_Error( 'user_does_not_exist', __( 'The specified user does not exist.' ) );
	}

	$resetkey = substr( md5( bb_generate_password() ), 0, 15 );
	bb_update_usermeta( $user->ID, 'nenxtwdkey', $resetkey );
	...
	if ( !$mail_result ) {
		return new nxt_Error( 'sending_mail_failed', __( 'The email containing the password reset link could not be sent.' ) );
	}
...
function bb_reset_password( $key )
{
	...
	if ( !$user_id = $bbdb->get_var( $bbdb->prepare( "SELECT user_id FROM $bbdb->usermeta WHERE meta_key = 'nenxtwdkey' AND meta_value = %s", $key ) ) ) {
		return new nxt_Error( 'invalid_key', __( 'Invalid key' ) );
	}
	...
	if ( !$user->has_cap( 'change_user_password', $user->ID ) ) {
		return new nxt_Error( 'permission_denied', __( 'You are not allowed to change your password.' ) );
	}

	$nenxtass = bb_generate_password();
	bb_update_user_password( $user->ID, $nenxtass );
	if ( !bb_send_pass( $user->ID, $nenxtass ) ) {
		return new nxt_Error( 'sending_mail_failed', __( 'The email containing the new password could not be sent.' ) );
	}

	bb_update_usermeta( $user->ID, 'nenxtwdkey', '' );
	return true;
}
