...
function bb_set_custom_user_tables()
{
	...
	// Check for older style nxt_table_prefix
	if ( $bb->nxt_table_prefix = bb_get_option( 'nxt_table_prefix' ) ) { // User has set old constant
		if ( !isset( $bb->custom_tables ) ) {
			$bb->custom_tables = array(
				'users'    => $bb->nxt_table_prefix . 'users',
				'usermeta' => $bb->nxt_table_prefix . 'usermeta'
			);
		} else {
			if ( !isset( $bb->custom_tables['users'] ) ) { // Don't stomp new setting style
				$bb->custom_tables['users'] = $bb->nxt_table_prefix . 'users';
			}
			if ( !isset( $bb->custom_tables['usermeta'] ) ) {
				$bb->custom_tables['usermeta'] = $bb->nxt_table_prefix . 'usermeta';
			}
		}
	}

	if ( bb_get_option( 'nxtclass_mu_primary_blog_id' ) ) {
...
function bb_paginate_links( $args = '' ) {
	$defaults = array(
		...
		'add_fragment' => '',
		'n_title'      => __( 'Page %d' ), // Not from nxt version
		'prev_title'   => __( 'Previous page' ), // Not from nxt version
		'next_title'   => __( 'Next page' ) // Not from nxt version
	);
...
define( 'BB_URI_CONTEXT_nxt_HTTP_REQUEST',      16384 );
...
function bb_register_view( $view, $title, $query_args = '', $feed = TRUE ) {
	...
	$query_args = nxt_parse_args( $query_args );
...
function bb_view_query( $view, $new_args = '' ) {
	...
	if ( $new_args ) {
		$new_args = nxt_parse_args( $new_args );
...
function bb_nonce_url( $actionurl, $action = -1 ) {
	...
	return esc_html( add_query_arg( '_nxtnonce', $nonce, $actionurl ) );
}
