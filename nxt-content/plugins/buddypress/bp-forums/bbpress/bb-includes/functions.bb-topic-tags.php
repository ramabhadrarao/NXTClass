...
function bb_add_topic_tags( $topic_id, $tags ) {
	...
	$tt_ids = $nxt_taxonomy_object->set_object_terms( $topic->topic_id, $tags, 'bb_topic_tag', array( 'append' => true, 'user_id' => $user_id ) );
	...
	if ( is_array($tt_ids) ) {
		...
		nxt_cache_delete( $topic->topic_id, 'bb_topic' );
...
function bb_create_tag( $tag ) {
	...
	if ( list($term_id, $tt_id) = $nxt_taxonomy_object->is_term( $tag, 'bb_topic_tag' ) )
		return $tt_id;

	list($term_id, $tt_id) = $nxt_taxonomy_object->insert_term( $tag, 'bb_topic_tag' );

	if ( is_nxt_error($term_id) || is_nxt_error($tt_id) || !$tt_id )
		return false;
...
function bb_remove_topic_tag( $tt_id, $user_id, $topic_id ) {
	...
	$currents = $nxt_taxonomy_object->get_object_terms( $topic_id, 'bb_topic_tag', array( 'user_id' => $user_id, 'fields' => 'all' ) );
	...
	$tt_ids = $nxt_taxonomy_object->set_object_terms( $topic_id, array_values($current_tag_term_ids), 'bb_topic_tag', array( 'user_id' => $user_id ) );
	if ( is_array( $tt_ids ) ) {
		global $bbdb;
		$bbdb->query( $bbdb->prepare(
...
function bb_remove_topic_tags( $topic_id ) {
	...
	$nxt_taxonomy_object->delete_object_term_relationships( $topic_id, 'bb_topic_tag' );
...
function bb_get_tag( $id, $user_id = 0, $topic_id = 0 ) {
	...
	if ( is_integer( $id ) ) {
		...
		if ( !$term = $nxt_taxonomy_object->get_term_by( 'slug', $id, 'bb_topic_tag' ) )
			return false;
	...
	if ( $user_id && $topic_id ) {
		...
		$tt_ids = nxt_cache_get( $cache_id, 'bb_topic_tag_terms' );
		if ( empty( $tt_ids ) ) {
			$tt_ids = $nxt_taxonomy_object->get_object_terms( $topic_id, 'bb_topic_tag', $args );
			nxt_cache_set( $cache_id, $tt_ids, 'bb_topic_tag_terms' );
		}
	...
	if ( !$term )
		$term = $nxt_taxonomy_object->get_term_by( 'tt_id', $tt_id, 'bb_topic_tag' );
...
function bb_get_tagged_topic_ids( $tag_id ) {
	global $nxt_taxonomy_object, $tagged_topic_count;
	
	if ( $topic_ids = (array) $nxt_taxonomy_object->get_objects_in_term( $tag_id, 'bb_topic_tag', array( 'field' => 'tt_id' ) ) ) {
...
function get_tagged_topics( $args ) {
	...
	if ( is_numeric( $args ) )
		...
		$args = nxt_parse_args( $args ); // Make sure it's an array
	if ( 1 < func_num_args() )
...
function bb_get_top_tags( $args = null ) {
	...
	$args = nxt_parse_args( $args, array( 'number' => 40 ) );
	$args['order'] = 'DESC';
	$args['orderby'] = 'count';
...
function bb_rename_tag( $tag_id, $tag_name ) {
	...
	global $nxt_taxonomy_object;
	$ret = $nxt_taxonomy_object->update_term( $tag_id, 'bb_topic_tag', array( 'name' => $raw_tag, 'slug' => $tag_name ) );
