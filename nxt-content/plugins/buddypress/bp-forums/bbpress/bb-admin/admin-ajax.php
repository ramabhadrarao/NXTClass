...
if ( !class_exists( 'nxt_Ajax_Response' ) )
	require_once( BACKPRESS_PATH . 'class.nxt-ajax-response.php' );
...
case 'add-post' : // Can put last_modified stuff back in later
	...
	if ( !$post_content = trim($_POST['post_content']) )
		$error = new nxt_Error( 'no-content', __('You need to actually submit some content!') );
	if ( !bb_current_user_can( 'write_post', $topic_id ) )
		die('-1');
	if ( !$topic = get_topic( $topic_id ) )
		die('0');
	if ( !topic_is_open( $topic_id ) )
		$error = new nxt_Error( 'topic-closed', __('This topic is closed.') );
	if ( $throttle_time = bb_get_option( 'throttle_time' ) )
		if ( isset($bb_current_user->data->last_posted) && time() < $bb_current_user->data->last_posted + $throttle_time && !bb_current_user_can('throttle') )
			$error = new nxt_Error( 'throttle-limit', sprintf( __('Slow down!  You can only post every %d seconds.'), $throttle_time );
	...
	$x = new nxt_Ajax_Response( array(
		'what' => 'post',
		'id' => $post_id,
		'data' => is_nxt_error($error) ? $error : $data
	) );
	$x->send();
	break;
*/
