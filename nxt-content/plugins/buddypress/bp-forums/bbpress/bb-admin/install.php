...
?>
						...
						<div class="toggle" id="toggle_2_1_target" style="<?php echo esc_attr( 'display:' . $bb_install->data[$bb_install->step]['form']['toggle_2_1']['display'] ); ?>;">
							<fieldset>
								...
								<p><?php _e('Integrating cookies allows you and your users to login to either your bbPress or your NXTClass site and be automatically logged into both.'); ?></p>
								<p><?php _e('You may need to make changes to your NXTClass configuration once installation is complete. See the "NXTClass Integration" section of the bbPress administration area when you are done.'); ?></p>
<?php
					$bb_install->input_text('nxt_siteurl', 'ltr');
					$bb_install->input_text('nxt_home', 'ltr');
					$bb_install->input_text('nxt_auth_key');
					$bb_install->input_text('nxt_auth_salt');
					$bb_install->input_text('nxt_secure_auth_key');
					$bb_install->input_text('nxt_secure_auth_salt');
					$bb_install->input_text('nxt_logged_in_key');
					$bb_install->input_text('nxt_logged_in_salt');
?>
							</fieldset>
						</div>
						<fieldset>
...
?>
						...
						<div class="toggle" id="toggle_2_2_target" style="<?php echo esc_attr( 'display:' . $bb_install->data[$bb_install->step]['form']['toggle_2_2']['display'] ); ?>;">
							<fieldset>
								...
								<p><?php _e('Integrating your NXTClass database user tables allows you to store user data in one location, instead of having separate user data for both bbPress and NXTClass.'); ?></p>
<?php
					$bb_install->input_text('nxt_table_prefix', 'ltr');
					$bb_install->input_text('nxtclass_mu_primary_blog_id', 'ltr');
...
?>
								...
								<fieldset>
									...
									<p><?php _e('Only set these options if your integrated user tables do not fit the usual mould of <em>nxt_user</em> and <em>nxt_usermeta</em>.'); ?></p>
<?php
...
?>
				...
				<script type="text/javascript" charset="utf-8">
					function updateNXTClassOptionURL () {
						var siteURLInputValue = document.getElementById('nxt_siteurl').value;
					...
					var siteURLInput = document.getElementById('nxt_siteurl');
