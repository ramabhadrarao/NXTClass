...
function bb_recount_topic_voices()
{
	...
	if ( is_nxt_error( $bbdb->query( $sql_delete ) ) ) {
		return array( 1, sprintf( $statement, $result ) );
	}
...
function bb_recount_user_topics_replied()
{
	...
	foreach ( $insert_values as $chunk ) {
		...
		if ( is_nxt_error( $bbdb->query( $sql_insert ) ) ) {
...
function bb_recount_tag_delete_empty()
{
	...
	if ( is_nxt_error( $bad_term_relationships ) || !is_array( $bad_term_relationships ) ) {
		$run_once = 2;
		return array( 2, sprintf( $statement, $result ) );
	}
	...
	if ( !empty( $bad_term_relationships ) ) {
		...
		if ( !empty( $values ) ) {
			...
			FROM `$bbdb->term_relationships`
			WHERE $values;";
			if ( is_nxt_error( $bbdb->query( $sql_bad_term_relationships_delete ) ) ) {
				$run_once = 3;
				return array( 3, sprintf( $statement, $result ) );
			}
		}
	...
	if ( is_nxt_error( $term_taxonomy_ids ) ) {
		$run_once = 4;
		return array( 4, sprintf( $statement, $result ) );
	}
	...
	if ( !empty( $term_taxonomy_ids ) ) {
		...
		if ( is_nxt_error( $bbdb->query( $sql_delete_term_relationships ) ) ) {
			$run_once = 5;
			return array( 5, sprintf( $statement, $result ) );
		}
	}
	...
	if ( !empty( $term_ids ) ) {
		...
		if ( is_nxt_error( $bbdb->query( $sql_delete_terms ) ) ) {
			$run_once = 7;
