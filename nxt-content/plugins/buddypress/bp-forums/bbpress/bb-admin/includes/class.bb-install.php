...
class BB_Install
{
	...
	function prepare_strings()
	{
		$this->strings = array(
			...
			2 => array(
				...
				'h2'          => sprintf( __( '%1$s - %2$s' ), __( 'Step 2' ), __( 'NXTClass integration (optional)' ) ),
				'status'      => __( '&laquo; skipped' ),
				'intro'       => array(
					__( 'bbPress can integrate login and user data seamlessly with NXTClass. You can safely skip this step if you do not wish to integrate with an existing NXTClass install.' )
				),
				'messages'    => array(),
				'form_errors' => array()
			),
	...
	function check_prerequisites()
	{
		...
		if ( defined( 'BB_IS_nxt_LOADED' ) && BB_IS_nxt_LOADED ) {
			$this->strings[-1]['messages']['error'][] = __( 'Please complete your installation before attempting to include NXTClass within bbPress' );
			$this->step = -1;
		}
			...
			2 => array(
				'form' => array(
					'toggle_2_0' => array(
						...
						'note'         => __( 'If you want to integrate bbPress with an existing NXTClass site.' ),
						'checked'      => '',
						'display'      => 'none',
						'toggle_value' => array(
							'target'    => 'forward_2_0',
							'off_value' => __( 'Skip NXTClass integration' ),
							'on_value'  => __( 'Save NXTClass integration settings' )
						)
					),
					'toggle_2_1' => array(
						'value'   => 0,
						'label'   => __( 'Add cookie integration settings' ),
						'note'    => __( 'If you want to allow shared logins with an existing NXTClass site.' ),
						'checked' => '',
						'display' => 'none',
					...
					'nxt_auth_key' => array(
						'value' => '',
						'label' => __( 'NXTClass "auth" cookie key' ),
						'note'  => __( 'This value must match the value of the constant named "AUTH_KEY" in your NXTClass <code>nxt-config.php</code> file. This will replace the bbPress "auth" cookie key set in the first step.' ),
						'prerequisite' => 'toggle_2_1',
						'autocomplete' => 'off'
					),
					'nxt_auth_salt' => array(
						'value' => '',
						'label' => __( 'NXTClass "auth" cookie salt' ),
						'note'  => __( 'This must match the value of the NXTClass setting named "auth_salt" in your NXTClass site. Look for the option labeled "auth_salt" in <a href="#" id="getAuthSaltOption" onclick="window.open(this.href); return false;">this NXTClass admin page</a>. If you leave this blank the installer will try to fetch the value based on your NXTClass database integration settings.' ),
						'prerequisite' => 'toggle_2_1',
						'autocomplete' => 'off'
					),
					'nxt_secure_auth_key' => array(
						'value' => '',
						'label' => __( 'NXTClass "secure auth" cookie key' ),
						'note'  => __( 'This value must match the value of the constant named "SECURE_AUTH_KEY" in your NXTClass <code>nxt-config.php</code> file. This will replace the bbPress "secure auth" cookie key set in the first step.' ),
						'prerequisite' => 'toggle_2_1',
						'autocomplete' => 'off'
					),
					'nxt_secure_auth_salt' => array(
						'value' => '',
						'label' => __( 'NXTClass "secure auth" cookie salt' ),
						'note'  => __( 'This must match the value of the NXTClass setting named "secure_auth_salt" in your NXTClass site. Look for the option labeled "secure_auth_salt" in <a href="#" id="getSecureAuthSaltOption" onclick="window.open(this.href); return false;">this NXTClass admin page</a>. If you leave this blank the installer will try to fetch the value based on your NXTClass database integration settings. Sometimes this value is not set in NXTClass, in that case you can leave this setting blank as well.' ),
						'prerequisite' => 'toggle_2_1',
						'autocomplete' => 'off'
					),
					'nxt_logged_in_key' => array(
						'value' => '',
						'label' => __( 'NXTClass "logged in" cookie key' ),
						'note'  => __( 'This value must match the value of the constant named "LOGGED_IN_KEY" in your NXTClass <code>nxt-config.php</code> file. This will replace the bbPress "logged in" cookie key set in the first step.' ),
						'prerequisite' => 'toggle_2_1',
						'autocomplete' => 'off'
					),
					'nxt_logged_in_salt' => array(
						'value' => '',
						'label' => __( 'NXTClass "logged in" cookie salt' ),
						'note'  => __( 'This must match the value of the NXTClass setting named "logged_in_salt" in your NXTClass site. Look for the option labeled "logged_in_salt" in <a href="#" id="getLoggedInSaltOption" onclick="window.open(this.href); return false;">this NXTClass admin page</a>. If you leave this blank the installer will try to fetch the value based on your NXTClass database integration settings.' ),
						'prerequisite' => 'toggle_2_1',
						'autocomplete' => 'off'
					),
					'toggle_2_2' => array(
						...
						'note'    => __( 'If you want to share user data with an existing NXTClass site.' ),
						'checked' => '',
						'display' => 'none',
						'prerequisite' => 'toggle_2_0'
					),
					...
					'nxtclass_mu_primary_blog_id' => array(
						'value' => '',
						'default_value' => '',
						'label' => __( 'NXTClass MU primary blog ID' ),
					...
					'toggle_2_3' => array(
						...
						'note'    => __( 'If your bbPress and NXTClass site do not share the same database, then you will need to add advanced settings.' ),
						'checked' => '',
						'display' => 'none',
						'prerequisite' => 'toggle_2_2'
					),
					...
					'forward_2_0' => array(
						'value' => __( 'Skip NXTClass integration' )
					),
					'back_2_1' => array(
						'value' => __( '&laquo; Go back' )
					),
	...
	function process_form()
	{
		if ( $this->is_posted() && !$this->stop_process ) {
			switch ( $this->step ) {
				...
				case 2:
					$this->process_form_nxtclass_integration();
					break;
	...
	/**
	 * Validates the NXTClass integration settings
	 *
	 * @return void
	 **/
	function process_form_nxtclass_integration()
	{
		// Check the referer
		...
		if ( !$data['toggle_2_0']['value'] && !$_POST['back_2_1'] ) {
			$this->step_status[2] = 'complete';
			$this->strings[2]['messages']['message'][] = __( 'You have chosen to skip the NXTClass integration step. You can always integrate NXTClass later from within the admin area of bbPress.' );
			return 'complete';
		}
		...
		if ( $data['toggle_2_0']['value'] ) {
			...
			if ( $data['toggle_2_1']['value'] ) {
				...
				// Check the nxt_siteurl URL for errors
				$data['nxt_siteurl']['value'] = $data['nxt_siteurl']['value'] ? rtrim( $data['nxt_siteurl']['value'], " \t\n\r\0\x0B/" ) . '/' : '';
				$this->strings[2]['form_errors']['nxt_siteurl'][] = empty( $data['nxt_siteurl']['value'] ) ? 'empty' : false;
				if ( $parsed = parse_url( $data['nxt_siteurl']['value'] ) ) {
					$this->strings[2]['form_errors']['nxt_siteurl'][] = preg_match( '/https?/i', $parsed['scheme'] ) ? false : 'urlscheme';
					$this->strings[2]['form_errors']['nxt_siteurl'][] = empty( $parsed['host'] ) ? 'urlhost' : false;
				} else {
					$this->strings[2]['form_errors']['nxt_siteurl'][] = 'urlparse';
				}

				// Check the nxt_home URL for errors
				$data['nxt_home']['value'] = $data['nxt_home']['value'] ? rtrim( $data['nxt_home']['value'], " \t\n\r\0\x0B/" ) . '/' : '';
				$this->strings[2]['form_errors']['nxt_home'][] = empty( $data['nxt_home']['value'] ) ? 'empty' : false;
				if ( $parsed = parse_url( $data['nxt_home']['value'] ) ) {
					$this->strings[2]['form_errors']['nxt_home'][] = preg_match( '/https?/i', $parsed['scheme'] ) ? false : 'urlscheme';
					$this->strings[2]['form_errors']['nxt_home'][] = empty( $parsed['host'] ) ? 'urlhost' : false;
				} else {
					$this->strings[2]['form_errors']['nxt_home'][] = 'urlparse';
				}

				// Deal with slashes in the keys
				$data['nxt_auth_key']['value']         = addslashes( stripslashes( $data['nxt_auth_key']['value'] ) );
				$data['nxt_auth_salt']['value']        = addslashes( stripslashes( $data['nxt_auth_salt']['value'] ) );
				$data['nxt_secure_auth_key']['value']  = addslashes( stripslashes( $data['nxt_secure_auth_key']['value'] ) );
				$data['nxt_secure_auth_salt']['value'] = addslashes( stripslashes( $data['nxt_secure_auth_salt']['value'] ) );
				$data['nxt_logged_in_key']['value']    = addslashes( stripslashes( $data['nxt_logged_in_key']['value'] ) );
				$data['nxt_logged_in_salt']['value']   = addslashes( stripslashes( $data['nxt_logged_in_salt']['value'] ) );

				// Check the keys for errors
				$this->strings[2]['form_errors']['nxt_auth_key'][]         = empty( $data['nxt_auth_key']['value'] ) ? 'empty' : false;
				$this->strings[2]['form_errors']['nxt_secure_auth_key'][]  = empty( $data['nxt_secure_auth_key']['value'] ) ? 'empty' : false;
				$this->strings[2]['form_errors']['nxt_logged_in_key'][]    = empty( $data['nxt_logged_in_key']['value'] ) ? 'empty' : false;

				// Salts can be taken from the database if specified
				if ( !$data['toggle_2_2']['value'] ) {
					$this->strings[2]['form_errors']['nxt_auth_salt'][]        = empty( $data['nxt_auth_salt']['value'] ) ? 'empty' : false;
					// NB; secure_auth_salt is not always set in NXTClass
					$this->strings[2]['form_errors']['nxt_logged_in_salt'][]   = empty( $data['nxt_logged_in_salt']['value'] ) ? 'empty' : false;
				}
			}
			...
			if ( $data['toggle_2_2']['value'] ) {
				...
				// Make the nxt_table_prefix valid
				$data['nxt_table_prefix']['value'] = preg_replace( '/[^0-9a-zA-Z_]/', '', $data['nxt_table_prefix']['value'] );
				$data['nxt_table_prefix']['value'] = empty( $data['nxt_table_prefix']['value'] ) ? 'nxt_' : $data['nxt_table_prefix']['value'];

				// Make the nxtclass_mu_primary_blog_id valid
				$data['nxtclass_mu_primary_blog_id']['value'] = preg_replace( '/[^0-9]/', '', $data['nxtclass_mu_primary_blog_id']['value'] );
			...
			if ( $data['toggle_2_2']['value'] ) {
				...
				global $bb;
				$bb->nxt_table_prefix = $data['nxt_table_prefix']['value'];
				if ( $data['toggle_2_3']['value'] ) {
				...
				if ( isset( $bb->custom_tables['users'] ) || isset( $bb->custom_tables['usermeta'] ) ) {
					...
					if ( is_nxt_error( $bbdb->set_prefix( $bb_table_prefix ) ) ) {
				...
				if ( !$result && $result_error ) {
					...
					$this->strings[2]['messages']['error'][] = __( 'There was a problem connecting to the NXTClass user database you specified. Please check the settings, then try again.' );
					return 'incomplete';
				}
				...
				if ( $result_error ) {
					...
					if ( $data['toggle_2_3']['value'] ) {
						$this->strings[2]['messages']['error'][] = __( 'Existing NXTClass user tables could not be found in the NXTClass database you specified.' );
					} else {
						$this->strings[2]['messages']['error'][] = __( 'Existing NXTClass user tables could not be found in the bbPress database you specified in step 1.<br /><br />This is probably because the database does not already contain working NXTClass tables. You may need to specify advanced database settings or leave integration until after installation.' );
					}
					$this->strings[2]['form_errors']['nxt_table_prefix'][] = __( '&bull; This may not be a valid user table prefix.' );
					return 'incomplete';
				}
		...
		$this->step_status[2] = 'complete';
		$this->strings[2]['messages']['message'][] = __( 'Your NXTClass integration cookie and database settings have been successfully validated. They will be saved after the next step.<br /><br />Once you have finished installing, you should visit the NXTClass integration section of the bbPress admin area for further options and integration instructions, including user mapping and the correct cookie settings to add to your NXTClass configuration file.' );
		return 'complete';
	}
	...
	function process_form_finalise_installation()
	{
		...
		if ( $data2['toggle_2_2']['value'] ) {
			...
			if ( !empty( $data2['nxt_table_prefix']['value'] ) ) {
				$bb->nxt_table_prefix = $data2['nxt_table_prefix']['value'];
			}
		...
		$installation_log[] = "\n" . __( 'Step 2 - NXTClass integration (optional)' );
		if ( $data2['toggle_2_0']['value'] ) {
			if ( $data2['toggle_2_1']['value'] ) {
				bb_update_option( 'nxt_siteurl', $data2['nxt_siteurl']['value'] );
				$installation_log[] = '>>> ' . __( 'NXTClass address (URL):' ) . ' ' . $data2['nxt_siteurl']['value'];

				bb_update_option( 'nxt_home', $data2['nxt_home']['value'] );
				$installation_log[] = '>>> ' . __( 'Blog address (URL):' ) . ' ' . $data2['nxt_home']['value'];

				$config_result = $this->write_lines_to_file(
					BB_PATH . 'bb-config.php',
					false,
					array(
						"define( 'BB_AUTH_KEY"  => array( "'" . BB_AUTH_KEY . "'",        "'" . $data2['nxt_auth_key']['value'] . "'" ),
						"define( 'BB_SECURE_A"  => array( "'" . BB_SECURE_AUTH_KEY . "'", "'" . $data2['nxt_secure_auth_key']['value'] . "'" ),
						"define( 'BB_LOGGED_I"  => array( "'" . BB_LOGGED_IN_KEY . "'",   "'" . $data2['nxt_logged_in_key']['value'] . "'" ),
					)
				);

				switch ( $config_result ) {
					case 1:
						$installation_log[] = '>>> ' . __( 'NXTClass cookie keys set.' );
						break;
					default:
						$error_log[] = '>>> ' . __( 'NXTClass cookie keys not set.' );
						...
						$installation_log[] = '>>> ' . __( 'NXTClass cookie keys not set.' );
						break;
				}

				if ( !empty( $data2['nxt_auth_salt']['value'] ) ) {
					bb_update_option( 'bb_auth_salt', $data2['nxt_auth_salt']['value'] );
					$installation_log[] = '>>> ' . __( 'NXTClass "auth" cookie salt set from input.' );
				}

				if ( !empty( $data2['nxt_secure_auth_salt']['value'] ) ) {
					bb_update_option( 'bb_secure_auth_salt', $data2['nxt_secure_auth_salt']['value'] );
					$installation_log[] = '>>> ' . __( 'NXTClass "secure auth" cookie salt set from input.' );
				}

				if ( !empty( $data2['nxt_logged_in_salt']['value'] ) ) {
					bb_update_option( 'bb_logged_in_salt', $data2['nxt_logged_in_salt']['value'] );
					$installation_log[] = '>>> ' . __( 'NXTClass "logged in" cookie salt set from input.' );
				}
			}
			...
			if ( $data2['toggle_2_2']['value'] ) {
				if (
					...
					$installation_log[] = '>>> ' . __( 'Fetching missing NXTClass cookie salts.' );

					$_prefix = $bb->nxt_table_prefix;
					if ( !empty( $data2['nxtclass_mu_primary_blog_id']['value'] ) ) {
						$_prefix .= $data2['nxtclass_mu_primary_blog_id']['value'] . '_';
					}
					...
					if ( !bb_get_option( 'bb_auth_salt' ) ) {
						$nxt_auth_salt = $bbdb->get_var( "SELECT `option_value` FROM $bbdb->options WHERE `option_name` = 'auth_salt' LIMIT 1" );
						if ( $nxt_auth_salt ) {
							bb_update_option( 'bb_auth_salt', $nxt_auth_salt );
							$installation_log[] = '>>>>>> ' . __( 'NXTClass "auth" cookie salt set.' );
						} else {
							$error_log[] = '>>> ' . __( 'NXTClass "auth" cookie salt not set.' );
							$error_log[] = '>>>>>> ' . __( 'Could not fetch "auth" cookie salt from the NXTClass options table.' );
							$error_log[] = '>>>>>> ' . __( 'You will need to manually define the "auth" cookie salt in your database.' );
							$installation_log[] = '>>>>>> ' . __( 'NXTClass "auth" cookie salt not set.' );
						}
					}

					if ( !bb_get_option( 'bb_secure_auth_salt' ) ) {
						$nxt_secure_auth_salt = $bbdb->get_var( "SELECT `option_value` FROM $bbdb->options WHERE `option_name` = 'secure_auth_salt' LIMIT 1" );
						if ( $nxt_secure_auth_salt ) {
							bb_update_option( 'bb_secure_auth_salt', $nxt_secure_auth_salt );
							$installation_log[] = '>>>>>> ' . __( 'NXTClass "secure auth" cookie salt set.' );
						} else {
							// This cookie salt is sometimes empty so don't error
							$installation_log[] = '>>>>>> ' . __( 'NXTClass "secure auth" cookie salt not set.' );
						}
					}

					if ( !bb_get_option( 'bb_logged_in_salt' ) ) {
						$nxt_logged_in_salt = $bbdb->get_var( "SELECT `option_value` FROM $bbdb->options WHERE `option_name` = 'logged_in_salt' LIMIT 1" );
						if ( $nxt_logged_in_salt ) {
							bb_update_option( 'bb_logged_in_salt', $nxt_logged_in_salt );
							$installation_log[] = '>>>>>> ' . __( 'NXTClass "logged in" cookie salt set.' );
						} else {
							$error_log[] = '>>> ' . __( 'NXTClass "logged in" cookie salt not set.' );
							$error_log[] = '>>>>>> ' . __( 'Could not fetch "logged in" cookie salt from the NXTClass options table.' );
							$error_log[] = '>>>>>> ' . __( 'You will need to manually define the "logged in" cookie salt in your database.' );
							$installation_log[] = '>>>>>> ' . __( 'NXTClass "logged in" cookie salt not set.' );
						}
					}
				}

				if ( !empty( $data2['nxt_table_prefix']['value'] ) ) {
					bb_update_option( 'nxt_table_prefix', $data2['nxt_table_prefix']['value'] );
					$installation_log[] = '>>> ' . __( 'User database table prefix:' ) . ' ' . $data2['nxt_table_prefix']['value'];
				}

				if ( !empty( $data2['nxtclass_mu_primary_blog_id']['value'] ) ) {
					bb_update_option( 'nxtclass_mu_primary_blog_id', $data2['nxtclass_mu_primary_blog_id']['value'] );
					$installation_log[] = '>>> ' . __( 'NXTClass MU primary blog ID:' ) . ' ' . $data2['nxtclass_mu_primary_blog_id']['value'];
				}
		...
		switch ( $data3['keymaster_user_type']['value'] ) {
			...
			case 'old':
				if ( $keymaster_user = bb_get_user( $data3['keymaster_user_login']['value'], array( 'by' => 'login' ) ) ) {
					// The keymaster is an existing bbPress or NXTClass user
					$bb_current_user = bb_set_current_user( $keymaster_user->ID );
	...
	/**
	 * Rewrites the admin user input into a select element containing existing NXTClass administrators.
	 *
	...
	function populate_keymaster_user_login_from_user_tables()
	{
		...
		// Get the existing NXTClass admin users

		// Setup variables and constants if available
		global $bb;
		if ( !empty( $this->data[2]['form']['nxt_table_prefix']['value'] ) ) {
			$bb->nxt_table_prefix = $this->data[2]['form']['nxt_table_prefix']['value'];
		}
		...
		$nxt_administrator_meta_key   = $bbdb->escape( $bb->nxt_table_prefix . 'capabilities' );
		if ( !empty( $this->data[2]['form']['nxtclass_mu_primary_blog_id']['value'] ) ) {
			$nxt_administrator_meta_key = $bb->nxt_table_prefix . $this->data[2]['form']['nxtclass_mu_primary_blog_id']['value'] . '_capabilities';
		}
		...
		$keymaster_query = <<<EOQ
			...
			WHERE
				(
					(
						meta_key = '$nxt_administrator_meta_key' AND
...
EOQ;
		...
		if ( $keymasters = $bbdb->get_results( $keymaster_query, ARRAY_A ) ) {
			...
			if ( count( $keymasters ) ) {
				...
				$data['note']     = __( 'Please select an existing bbPress Keymaster or NXTClass administrator.' );
