...
function bb_admin_menu_generator()
{
	...
	$bb_menu[320] = array( __( 'Settings' ), 'manage_options', 'options-general.php', '', 'bb-menu-settings' );
		...
		$bb_submenu['options-general.php'][35] = array( __( 'NXTClass Integration' ), 'manage_options', 'options-nxtclass.php' );
...
class BB_User_Search {
	...
	function display( $show_search = true, $show_email = false ) {
		...
		if ( $show_search ) {
			$roles = apply_filters( 'bb_user_search_form_roles', $nxt_roles->get_names() );
		...
		if ( $this->get_results() ) {
			...
			//foreach($roleclasses as $role => $roleclass) {
				//ksort($roleclass);
				//if ( !empty($role) )
				//	$r .= "\t\t<h3>{$nxt_roles->role_names[$role]}</h3>\n";
				//else
...
function bb_new_forum( $args ) {
	...
	$forum_name = apply_filters( 'bb_pre_forum_name', stripslashes( nxt_specialchars_decode( $forum_name, ENT_QUOTES ) ) );
