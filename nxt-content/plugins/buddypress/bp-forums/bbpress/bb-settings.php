...
/**
 * Remove filters and action that have been set by an included NXTClass install
 */
if ( defined( 'ABSPATH' ) ) {
	$nxt_filter = array();
	$nxt_actions = array();
	$merged_filters = array();
	$nxt_current_filter = array();
}
 ...
 */
$_internal_object_cache_functions_include = BACKPRESS_PATH . 'loader.nxt-object-cache.php';
$_memcached_object_cache_functions_include = BACKPRESS_PATH . 'loader.nxt-object-cache-memcached.php';
...
// See if a caching class is already loaded (by NXTClass)
if ( function_exists( 'nxt_cache_init' ) ) {
	if ( isset( $_nxt_using_ext_object_cache ) ) {
		$_bb_using_ext_object_cache = $_nxt_using_ext_object_cache;
	} else {
...
// Instantiate the $nxt_object_cache object using nxt_cache_init()
if ( function_exists( 'nxt_cache_init' ) ) {
	// Clear NXTClass cache if it exists already - maybe should save and re-load?
	unset( $nxt_object_cache );
	nxt_cache_init();
	if ( function_exists( 'nxt_cache_add_global_groups' ) ) {
		nxt_cache_add_global_groups( array( 'users', 'userlogins', 'usermeta', 'useremail', 'usernicename' ) );
	}
}
...
if ( !class_exists( 'nxt_Http' ) ) {
	require_once( BACKPRESS_PATH . 'class.nxt-http.php' );
}
...
// Is NXTClass loaded
if ( !defined( 'BB_IS_nxt_LOADED') ) {
	define( 'BB_IS_nxt_LOADED', defined( 'DB_NAME' ) );
}

// Only load this if NXTClass isn't loaded
if ( !BB_IS_nxt_LOADED ) {
...
/**
 * Sort out cookies so they work with NXTClass (if required)
 * Note that database integration is no longer a pre-requisite for cookie integration
 */

$bb->nxt_siteurl = bb_get_option( 'nxt_siteurl' );
if ( $bb->nxt_siteurl ) {
...
define( 'BB_HASH', $bb->nxt_cookies_integrated ? md5( $bb->nxt_siteurl ) : md5( $bb->uri ) );

if ( BB_LOAD_DEPRECATED ) {
	// Deprecated setting
	$bb->usercookie = bb_get_option( 'usercookie' );
	if ( !$bb->usercookie ) {
		$bb->usercookie = ( $bb->nxt_cookies_integrated ? 'nxtclassuser_' : 'bb_user_' ) . BB_HASH;
	} else {
	...
	if ( !$bb->passcookie ) {
		$bb->passcookie = ( $bb->nxt_cookies_integrated ? 'nxtclasspass_' : 'bb_pass_' ) . BB_HASH;
	} else {
...
if ( !$bb->authcookie ) {
	$bb->authcookie = ( $bb->nxt_cookies_integrated ? 'nxtclass_' : 'bbpress_' ) . BB_HASH;
}

$bb->secure_auth_cookie = bb_get_option( 'secure_auth_cookie' );
if ( !$bb->secure_auth_cookie ) {
	$bb->secure_auth_cookie = ( $bb->nxt_cookies_integrated ? 'nxtclass_sec_' : 'bbpress_sec_' ) . BB_HASH;
}

$bb->logged_in_cookie = bb_get_option( 'logged_in_cookie' );
if ( !$bb->logged_in_cookie ) {
	$bb->logged_in_cookie = ( $bb->nxt_cookies_integrated ? 'nxtclass_logged_in_' : 'bbpress_logged_in_' ) . BB_HASH;
}

// Cookie path was set before integration logic above
if ( !$bb->cookiepath ) {
	$bb->cookiepath = $bb->nxt_cookies_integrated ? preg_replace( '|https?://[^/]+|i', '', $bb->nxt_home ) : $bb->path;
}
...
if ( !$bb->sitecookiepath && $bb->nxt_cookies_integrated ) {
	$bb->sitecookiepath = preg_replace( '|https?://[^/]+|i', '', $bb->nxt_siteurl );
	$_bb_sitecookiepath = $bb->sitecookiepath;
}
$bb->sitecookiepath = rtrim( trim( $bb->sitecookiepath ), " \t\n\r\0\x0B/" ) . '/';

$bb->nxt_admin_cookie_path = bb_get_option( 'nxt_admin_cookie_path' );
if ( !$bb->nxt_admin_cookie_path && $bb->nxt_cookies_integrated ) {
	if ( $bb->nxtclass_mu_primary_blog_id ) {
		$bb->nxt_admin_cookie_path = $_bb_sitecookiepath;
	} else {
		$bb->nxt_admin_cookie_path = $_bb_sitecookiepath . '/nxt-admin';
	}
}
if ( '/' !== $bb->nxt_admin_cookie_path = trim( $bb->nxt_admin_cookie_path ) ) {
	$bb->nxt_admin_cookie_path = rtrim( $bb->nxt_admin_cookie_path, " \t\n\r\0\x0B/" );
}

$bb->nxt_plugins_cookie_path = bb_get_option( 'nxt_plugins_cookie_path' );
if ( !$bb->nxt_plugins_cookie_path && $bb->nxt_cookies_integrated ) {
	// This is a best guess only, should be manually set to match nxt_PLUGIN_URL
	$bb->nxt_plugins_cookie_path = $_bb_sitecookiepath . '/nxt-content/plugins';
}
if ( '/' !== $bb->nxt_plugins_cookie_path = trim( $bb->nxt_plugins_cookie_path ) ) {
	$bb->nxt_plugins_cookie_path = rtrim( $bb->nxt_plugins_cookie_path, " \t\n\r\0\x0B/" );
}
...
// nxt_Pass
if ( !class_exists( 'nxt_Pass' ) ) {
	require_once( BACKPRESS_PATH . 'class.nxt-pass.php' );
}

// nxt_Users
if ( !class_exists( 'nxt_Users' ) ) {
	require_once( BACKPRESS_PATH . 'class.nxt-users.php' );
	$nxt_users_object = new nxt_Users( $bbdb );
}
...
// nxt_Auth
if ( !class_exists( 'nxt_Auth' ) ) {
	require_once( BACKPRESS_PATH . 'class.nxt-auth.php' );
	...
	if ( $bb->nxt_admin_cookie_path ) {
		$cookies['auth'][] = array(
			'domain' => $bb->cookiedomain,
			'path' => $bb->nxt_admin_cookie_path,
			'name' => $bb->authcookie
		);
	...
	if ( $bb->nxt_plugins_cookie_path ) {
		$cookies['auth'][] = array(
			'domain' => $bb->cookiedomain,
			'path' => $bb->nxt_plugins_cookie_path,
			'name' => $bb->authcookie
		);
	 ...
	 *
	 * Version 1 is for NXTClass >= 2.6 and < 2.8
	 * Version 2 is for nxtclass >= 2.8
	 */
	if ( !defined( 'nxt_AUTH_COOKIE_VERSION' ) ) {
	 ...
	 * nxt_Auth object
	 */
	$nxt_auth_object = new nxt_Auth( $bbdb, $nxt_users_object, $cookies );
 ...
 * Current user object
 */
$bb_current_user =& $nxt_auth_object->current;

// nxt_Scripts/nxt_Styles
if ( !class_exists( 'nxt_Dependencies' ) ) {
	require_once( BACKPRESS_PATH . 'class.nxt-dependencies.php' );
}

if ( !class_exists( 'nxt_Scripts' ) ) {
	require_once( BACKPRESS_PATH . 'class.nxt-scripts.php' );
	require_once( BACKPRESS_PATH . 'functions.nxt-scripts.php' );
} else {
	unset( $nxt_scripts );
}

if ( !class_exists( 'nxt_Styles' ) ) {
	require_once( BACKPRESS_PATH . 'class.nxt-styles.php' );
	require_once( BACKPRESS_PATH . 'functions.nxt-styles.php' );
} else {
	unset( $nxt_styles );
}
...
// nxt_Taxonomy
if ( !class_exists( 'nxt_Taxonomy' ) ) {
	require_once( BACKPRESS_PATH . 'class.nxt-taxonomy.php' );
}
if ( !class_exists( 'BB_Taxonomy' ) ) {
	require_once( BB_PATH . BB_INC . 'class.bb-taxonomy.php' );
}
if ( !isset( $nxt_taxonomy_object ) ) {
	// Clean slate
	$nxt_taxonomy_object = new BB_Taxonomy( $bbdb );
} elseif ( !is_a( $nxt_taxonomy_object, 'BB_Taxonomy' ) ) {
	// exists, but it's not good enough, translate it

	// preserve the references
	$tax =& $nxt_taxonomy_object->taxonomies;
	$nxt_taxonomy_object = new BB_Taxonomy( $bbdb );
	$nxt_taxonomy_object->taxonomies =& $tax;
	unset( $tax );
}

$nxt_taxonomy_object->register_taxonomy( 'bb_topic_tag', 'bb_topic' );

$nxt_taxonomy_object->register_taxonomy( 'bb_subscribe', 'bb_user' );
