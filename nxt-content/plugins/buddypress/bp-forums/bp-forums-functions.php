...
function bp_forums_get_topic_extras( $topics ) {
	...
	$topic_ids = $nxtdb->escape( join( ',', (array)$topic_ids ) );

	// Fetch the topic's last poster details
	$poster_details = $nxtdb->get_results( $nxtdb->prepare( "SELECT t.topic_id, t.topic_last_poster, u.user_login, u.user_nicename, u.user_email, u.display_name FROM {$nxtdb->users} u, {$bbdb->topics} t WHERE u.ID = t.topic_last_poster AND t.topic_id IN ( {$topic_ids} )" ) );
	...
	if ( bp_is_active( 'xprofile' ) ) {
		$poster_names = $nxtdb->get_results( $nxtdb->prepare( "SELECT t.topic_id, pd.value FROM {$bp->profile->table_name_data} pd, {$bbdb->topics} t WHERE pd.user_id = t.topic_last_poster AND pd.field_id = 1 AND t.topic_id IN ( {$topic_ids} )" ) );
		for ( $i = 0, $count = count( $topics ); $i < $count; ++$i ) {
...
function bp_forums_get_post_extras( $posts ) {
	...
	$poster_details = $nxtdb->get_results( $nxtdb->prepare( "SELECT u.ID as user_id, u.user_login, u.user_nicename, u.user_email, u.display_name FROM {$nxtdb->users} u WHERE u.ID IN ( {$user_ids} )" ) );
	...
	if ( bp_is_active( 'xprofile' ) ) {
		$poster_names = $nxtdb->get_results( $nxtdb->prepare( "SELECT pd.user_id, pd.value FROM {$bp->profile->table_name_data} pd WHERE pd.user_id IN ( {$user_ids} )" ) );
		for ( $i = 0, $count = count( $posts ); $i < $count; ++$i ) {
			foreach ( (array)$poster_names as $name ) {
...
function bp_forums_get_forum_topicpost_count( $forum_id ) {
	...
	return $nxtdb->get_results( $nxtdb->prepare( "SELECT topics, posts from {$bbdb->forums} WHERE forum_id = %d", $forum_id ) );
}

function bp_forums_filter_caps( $allcaps ) {
	global $bp, $nxt_roles, $bb_table_prefix;
	...
	$bb_cap = $nxt_roles->get_role( $bb_cap[0] );
