...
add_filter( 'bp_forums_bbconfig_location', 'nxt_filter_kses', 1 );
add_filter( 'bp_forums_bbconfig_location', 'esc_attr', 1 );

add_filter( 'bp_get_the_topic_title', 'nxt_filter_kses', 1 );
...
add_filter( 'bp_get_the_topic_title', 'nxttexturize' );
add_filter( 'bp_get_the_topic_poster_name', 'nxttexturize' );
add_filter( 'bp_get_the_topic_last_poster_name', 'nxttexturize' );
add_filter( 'bp_get_the_topic_post_content', 'nxttexturize' );
add_filter( 'bp_get_the_topic_post_poster_name', 'nxttexturize' );
...
add_filter( 'bp_get_the_topic_post_content', 'nxtautop' );
add_filter( 'bp_get_the_topic_latest_post_excerpt', 'nxtautop' );
 ...
 * @since 1.5
 *
 * @global object $nxtdb The NXTClass database global
 * @param string $sql
 * @return string $sql
 */
function bp_forums_add_replied_distinct_sql( $sql ) {
	...
	$sql = $nxtdb->prepare( "DISTINCT t.topic_id, " );
...
function bp_forums_add_replied_join_sql( $sql ) {
	...
	$sql .= $nxtdb->prepare( " LEFT JOIN $bbdb->posts p ON p.topic_id = t.topic_id " );
...
function bp_forums_add_replied_where_sql( $sql ) {
	...
	$sql .= $nxtdb->prepare( " AND p.poster_id = %s ", bp_displayed_user_id() );
