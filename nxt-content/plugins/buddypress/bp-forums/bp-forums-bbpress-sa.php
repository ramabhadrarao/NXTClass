...
function bp_forums_load_bbpress() {
	global $bp, $nxtdb, $nxt_roles, $current_user, $nxt_users_object;
	global $bb, $bbdb, $bb_table_prefix, $bb_current_user;
	global $bb_roles, $nxt_taxonomy_object, $bb_queries;
	...
	require( BACKPRESS_PATH . 'class.nxt-taxonomy.php' );
	...
	$nxt_users_object = new BP_Forums_BB_Auth;
...
/**
 * bbPress needs the DB class to be BPDB, but we want to use nxtDB, so we can
 * extend it and use this.
 *
 * The class is pluggable, so that plugins that swap out nxtDB with a custom
...
if ( ! class_exists( 'BPDB' ) ) :
	class BPDB extends nxtDB {
		 ...
		 *
		 * Overriden here to work around differences between bbPress', and NXTClass', implementation differences.
		 * In particular, when BuddyPress tries to run bbPress' SQL installation script, the collation check always
		 * failed. The capability is long supported by NXTClass' minimum required MySQL version, so this is safe.
		 */
		...
		function init( $args ) {
			...
			return nxt_parse_args( $args, $defaults );
		}
...
/**
 * Copied from nxt-admin/includes/upgrade.php, this will take care of creating
 * the bbPress stand-alone tables without loading a conflicting nxt Admin.
 *
...
function bp_bb_dbDelta($queries, $execute = true) {
	...
	if ($tables = $nxtdb->get_col('SHOW TABLES;')) {
		// For every table in the database
		foreach ($tables as $table) {
			// Upgrade global tables only for the main site. Don't upgrade at all if DO_NOT_UPGRADE_GLOBAL_TABLES is defined.
			if ( in_array($table, $nxtdb->tables('global')) && ( !is_main_site() || defined('DO_NOT_UPGRADE_GLOBAL_TABLES') ) )
				continue;
