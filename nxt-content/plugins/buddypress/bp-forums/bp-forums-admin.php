	...
	switch( $step ) {
			...
			if ( isset( $_REQUEST['doinstall'] ) && ( 1 == (int) $_REQUEST['doinstall'] ) ) {
				if ( !bp_forums_configure_existing_install() ) {
					...
					<form action="" method="post">
						...
						<?php nxt_nonce_field( 'bp_forums_existing_install_init' ) ?>
					</form>
		...
		case 'new':
			if ( isset( $_REQUEST['doinstall'] ) && 1 == (int)$_REQUEST['doinstall'] ) {
				...
				<p><a class="button-primary" href="<?php echo nxt_nonce_url( $post_url . '&step=new&doinstall=1', 'bp_forums_new_install_init' ) ?>"><?php _e( 'Complete Installation', 'buddypress' ) ?></a></p>
		...
		default:
			if ( !file_exists( BP_PLUGIN_DIR . '/bp-forums/bbpress/' ) ) { ?>
				...
				<div id="message" class="error">
					<p><?php printf( __( 'bbPress files were not found. To install the forums component you must download a copy of bbPress and make sure it is in the folder: "%s"', 'buddypress' ), 'nxt-content/plugins/buddypress/bp-forums/bbpress/' ) ?></p>
				</div>
...
function bp_forums_bbpress_install() {
	...
	$file .= "\n" .   '$bb->custom_user_table = \'' . $nxtdb->users . '\';';
	$file .= "\n" .   '$bb->custom_user_meta_table = \'' . $nxtdb->usermeta . '\';';
	$file .= "\n\n" . '$bb->uri = \'' . BP_PLUGIN_URL . '/bp-forums/bbpress/\';';
	$file .= "\n" .   '$bb->name = \'' . get_blog_option( bp_get_root_blog_id(), 'blogname' ) . ' ' . __( 'Forums', 'buddypress' ) . '\';';

	if ( is_multisite() )
		$file .= "\n" .   '$bb->nxtclass_mu_primary_blog_id = ' . bp_get_root_blog_id() . ';';
