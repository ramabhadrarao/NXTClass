...
function bp_blogs_record_existing_blogs() {
	...
	$nxtdb->query( "TRUNCATE TABLE {$bp->blogs->table_name}" );

	if ( is_multisite() )
		$blog_ids = $nxtdb->get_col( $nxtdb->prepare( "SELECT blog_id FROM {$nxtdb->base_prefix}blogs WHERE mature = 0 AND spam = 0 AND deleted = 0" ) );
	else
		$blog_ids = 1;
...
add_action( 'nxtmu_new_blog', 'bp_blogs_record_blog', 10, 2 );

/**
 * Updates blogname in BuddyPress blogmeta table
 *
 * @global object $nxtdb DB Layer
...
function bp_blogs_update_option_blogname( $oldvalue, $newvalue ) {
	...
	bp_blogs_update_blogmeta( $nxtdb->blogid, 'name', $newvalue );
}
...
function bp_blogs_update_option_blogdescription( $oldvalue, $newvalue ) {
	...
	bp_blogs_update_blogmeta( $nxtdb->blogid, 'description', $newvalue );
}
...
function bp_blogs_record_post( $post_id, $post, $user_id = 0 ) {
	...
	$post_id = (int)$post_id;
	$blog_id = (int)$nxtdb->blogid;

	if ( !$user_id )
		$user_id = (int)$post->post_author;

	// Stop infinite loops with NXTClass MU Sitewide Tags.
...
add_action( 'nxt_set_comment_status', 'bp_blogs_manage_comment', 10, 2 );

function bp_blogs_add_user_to_blog( $user_id, $role = false, $blog_id = 0 ) {
	global $nxtdb;
	
	if ( empty( $blog_id ) ) {
		$blog_id = isset( $nxtdb->blogid ) ? $nxtdb->blogid : bp_get_root_blog_id();
	}

	if ( empty( $role ) ) {
		$key = $nxtdb->get_blog_prefix( $blog_id ). 'capabilities';
...
function bp_blogs_remove_comment( $comment_id ) {
	...
	// Delete activity stream item
	bp_blogs_delete_activity( array( 'item_id' => $nxtdb->blogid, 'secondary_item_id' => $comment_id, 'type' => 'new_blog_comment' ) );

	do_action( 'bp_blogs_remove_comment', $nxtdb->blogid, $comment_id, $bp->loggedin_user->id );
}
add_action( 'delete_comment', 'bp_blogs_remove_comment' );

function bp_blogs_total_blogs() {
	if ( !$count = nxt_cache_get( 'bp_total_blogs', 'bp' ) ) {
		$blogs = BP_Blogs_Blog::get_all();
		$count = $blogs['total'];
		nxt_cache_set( 'bp_total_blogs', $count, 'bp' );
	}
	return $count;
}
...
function bp_blogs_update_blogmeta( $blog_id, $meta_key, $meta_value ) {
	...
	$cur = $nxtdb->get_row( $nxtdb->prepare( "SELECT * FROM {$bp->blogs->table_name_blogmeta} WHERE blog_id = %d AND meta_key = %s", $blog_id, $meta_key ) );

	if ( !$cur )
		$nxtdb->query( $nxtdb->prepare( "INSERT INTO {$bp->blogs->table_name_blogmeta} ( blog_id, meta_key, meta_value ) VALUES ( %d, %s, %s )", $blog_id, $meta_key, $meta_value ) );
	else if ( $cur->meta_value != $meta_value )
		$nxtdb->query( $nxtdb->prepare( "UPDATE {$bp->blogs->table_name_blogmeta} SET meta_value = %s WHERE blog_id = %d AND meta_key = %s", $meta_value, $blog_id, $meta_key ) );
	else
		return false;

	nxt_cache_set( 'bp_blogs_blogmeta_' . $blog_id . '_' . $meta_key, $meta_value, 'bp' );
...
add_action( 'nxtmu_delete_user',  'bp_blogs_remove_data' );
