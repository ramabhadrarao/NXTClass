...
function bp_avatar_to_crop_src() {
	...
	function bp_get_avatar_to_crop_src() {
		...
		return apply_filters( 'bp_get_avatar_to_crop_src', str_replace( nxt_CONTENT_DIR, '', $bp->avatar_admin->image->dir ) );
	}
...
function bp_create_excerpt( $text, $length = 225, $options = array() ) {
	...
	$r = nxt_parse_args( $options, $defaults );
	extract( $r );
 ...
 * if someone copies the templates from the default theme into another NXTClass theme
 ...
 *
 * @param array $nxt_classes The body classes coming from nxt
