...
class BP_Core_Setup_Wizard {
	...
	function step_db_update() {
		...
		<div class="submit clear">
			...
			<?php nxt_nonce_field( 'bpwizard_db_update' ) ?>
	...
	function step_ms_update() {
		...
		<p><?php printf( __( 'BuddyPress has detected a recent change to NXTClass Multisite, which allows members of your community to have their own NXTClass sites. You can enable or disable this feature at any time at <a href="%s">Network Options</a>.', 'buddypress' ), network_admin_url( 'settings.php' ) ); ?></p>

		<p><?php __( "Please select the NXTClass page you would like to use to display the site directory. You can either choose an existing page or let BuddyPress auto-create a page for you. If you'd like, you can go to manually create pages now, and return to this step when you are finished.", 'buddypress' ) ?></p>
		...
		<table class="form-table">
			...
			<tr valign="top" id="site-tracking-page-selector">
				<th scope="row"><?php _e( 'Select a NXTClass page for the Sites directory.', 'buddypress' ); ?></th>
				...
				<td>
					...
					<p><input type="radio" name="bp_pages[blogs]" value="page" /> <?php _e( 'Use an existing page:', 'buddypress' ); ?> <?php echo nxt_dropdown_pages( "name=bp-blogs-page&echo=0&show_option_none=" . __( '- Select -', 'buddypress' ) . $existing_blog_page ); ?></p>
				</td>
			</tr>
		...
		<div class="submit clear">
			...
			<?php nxt_nonce_field( 'bpwizard_ms_update' ); ?>
	...
	function step_components() {
		...
		<div class="submit clear">
			...
			<?php nxt_nonce_field( 'bpwizard_components' ); ?>
	...
	function step_pages() {
		...
		<p><?php _e( 'BuddyPress now uses NXTClass pages to display content. This allows you to easily change the names of pages or move them to a sub page.', 'buddypress' ); ?></p>
		...
		<table class="form-table">
			...
			<tr valign="top">
				...
				<td>
					...
					<p><label><input type="radio" name="bp_pages[members]" <?php checked( !empty( $existing_pages['members'] ) ); ?> value="page" /> <?php _e( 'Use an existing page:', 'buddypress' ); ?> <?php echo nxt_dropdown_pages( "name=bp-members-page&echo=0&show_option_none=" . __( '- Select -', 'buddypress' ) . "&selected=" . $existing_pages['members'] ); ?></label></p>
				</td>
			</tr>
			...
			<?php if ( isset( $active_components['activity'] ) ) : ?>
				...
				<tr valign="top">
					...
					<td>
						...
						<p><label><input type="radio" name="bp_pages[activity]" <?php checked( !empty( $existing_pages['activity'] ) ); ?> value="page" /> <?php _e( 'Use an existing page:', 'buddypress' ); ?> <?php echo nxt_dropdown_pages( "name=bp-activity-page&echo=0&show_option_none=" . __( '- Select -', 'buddypress' ) . "&selected=" . $existing_pages['activity'] ); ?></label></p>
					</td>
				</tr>
			...
			<?php if ( isset( $active_components['groups'] ) ) : ?>
				...
				<tr valign="top">
					...
					<td>
						...
						<p><label><input type="radio" name="bp_pages[groups]" <?php checked( !empty( $existing_pages['groups'] ) ); ?> value="page" /> <?php _e( 'Use an existing page:', 'buddypress' ); ?> <?php echo nxt_dropdown_pages( "name=bp-groups-page&echo=0&show_option_none=" . __( '- Select -', 'buddypress' ) . "&selected=" . $existing_pages['groups'] ); ?></label></p>
					</td>
				</tr>
			...
			<?php if ( isset( $active_components['forums'] ) ) : ?>
				...
				<tr valign="top">
					...
					<td>
						...
						<p><label><input type="radio" name="bp_pages[forums]" <?php checked( !empty( $existing_pages['forums'] ) ); ?> value="page" /> <?php _e( 'Use an existing page:', 'buddypress' ); ?> <?php echo nxt_dropdown_pages( "name=bp-forums-page&echo=0&show_option_none=" . __( '- Select -', 'buddypress' ) . "&selected=" . $existing_pages['forums'] ); ?></label></p>
					</td>
				</tr>
			...
			<?php if ( is_multisite() && isset( $active_components['blogs'] ) ) : ?>
				...
				<tr valign="top">
					...
					<td>
						...
						<p><label><input type="radio" name="bp_pages[blogs]" <?php checked( !empty( $existing_pages['blogs'] ) ); ?> value="page" /> <?php _e( 'Use an existing page:', 'buddypress' ); ?> <?php echo nxt_dropdown_pages( "name=bp-blogs-page&echo=0&show_option_none=" . __( '- Select -', 'buddypress' ) . "&selected=" . $existing_pages['blogs'] ); ?></label></p>
					</td>
				</tr>
			...
			<tr valign="top">
				...
				<td>
					...
					<p><label><input type="radio" name="bp_pages[register]" <?php checked( !empty( $existing_pages['register'] ) ); ?> value="page" /> <?php _e( 'Use an existing page:', 'buddypress' ) ?> <?php echo nxt_dropdown_pages( "name=bp-register-page&echo=0&show_option_none=" . __( '- Select -', 'buddypress' ) . "&selected=" . $existing_pages['register'] ); ?></label></p>
				</td>
			</tr>
			...
			<tr valign="top">
				...
				<td>
					...
					<p><label><input type="radio" name="bp_pages[activate]" <?php checked( !empty( $existing_pages['activate'] ) ); ?> value="page" /> <?php _e( 'Use an existing page:', 'buddypress' ); ?> <?php echo nxt_dropdown_pages( "name=bp-activate-page&echo=0&show_option_none=" . __( '- Select -', 'buddypress' ) . "&selected=" . $existing_pages['activate'] ); ?></label></p>
				</td>
			</tr>
		</table>
		...
		<div class="submit clear">
			...
			<?php nxt_nonce_field( 'bpwizard_pages' ); ?>
	...
	function step_permalinks() {
		...
		<div class="submit clear">
			...
			<?php nxt_nonce_field( 'bpwizard_permalinks' ); ?>
	...
	?>
		...
		<table class="form-table">
			<tr>
				...
				<td>
					...
					<p><strong><?php _e( 'This is the best choice if you do not have an existing NXTClass theme, or want to start using BuddyPress immediately.', 'buddypress' ); ?></strong></p>
			...
			<tr>
				...
				<td>
					<p><?php _e( 'The BuddyPress Template Pack plugin will guide you through the process of manually upgrading your existing NXTClass theme. This usually involves following the step-by-step instructions and copying the BuddyPress template files into your theme. This option requires a working knowledge of CSS and HTML, as you will need to tweak the new templates to match your existing theme.', 'buddypress' ); ?></p>
					...
					<?php else : ?>
						...
						<p><label><input type="radio" name="theme" value="manual_nxt" /> <?php _e( 'Choose this option (go to Appearance &rarr; BP Compatibility after setup is complete)', 'buddypress' ); ?></label></p>
		...
		<div class="submit clear">
			...
			<?php nxt_nonce_field( 'bpwizard_theme' ) ?>
	...
	function step_finish() {
		...
		<div class="submit clear">
			...
			<?php nxt_nonce_field( 'bpwizard_finish' ); ?>
						...
						<?php
							...
							if ( !empty( $iis7_permalinks ) ) {
								...
								if ( !empty( $permalink_structure ) && empty( $usingpi ) && empty( $writable ) ) {
									...
									<textarea rows="9" class="large-text readonly" style="background: #fff;" name="rules" id="rules" readonly="readonly"><?php echo esc_html( $nxt_rewrite->iis7_url_rewrite_rules() ); ?></textarea>
								...
								<textarea rows="6" class="large-text readonly" style="background: #fff;" name="rules" id="rules" readonly="readonly"><?php echo esc_html( $nxt_rewrite->mod_rewrite_rules() ); ?></textarea>
		...
		foreach ( $pages as $key => $value ) {
			if ( 'page' == $value ) {
				...
				if ( !empty( $_POST['bp-' . $key . '-page'] ) )
					...
					$bp_pages[$key] = nxt_insert_post( array( 'comment_status' => 'closed', 'ping_status' => 'closed', 'post_title' => ucwords( $key ), 'post_status' => 'publish', 'post_type' => 'page' ) );
			} else {
				// Create a new page
				$bp_pages[$key] = nxt_insert_post( array( 'comment_status' => 'closed', 'ping_status' => 'closed', 'post_title' => ucwords( $value ), 'post_status' => 'publish', 'post_type' => 'page' ) );
			}
		}
...
function bp_core_wizard_thickbox() {
	...
	<script type="text/javascript">
		...
		if ( window.location != window.parent.location ) {
			jQuery('#adminmenu, #nxthead, #footer, #update-nag, #screen-meta').hide();
			jQuery('#nxtbody').css( 'margin', '15px' );
			jQuery('body').css( 'min-width', '30px' );
			jQuery('#nxtwrap').css( 'min-height', '30px' );
 ...
 * @uses add_submenu_page() nxt function to add a submenu item
 */
...
function bp_core_update_add_admin_menu_styles() {
	...
	<style type="text/css">
		...
		ul#adminmenu li.toplevel_page_bp-wizard .nxt-menu-image a { background-image: url( <?php echo plugins_url( 'buddypress/bp-core/images/admin_menu_icon.png' ) ?> ) !important; background-position: -1px -32px; }
		...
		ul#adminmenu li.toplevel_page_bp-general-settings .nxt-menu-image a { background-image: url( <?php echo plugins_url( 'buddypress/bp-core/images/admin_menu_icon.png' ) ?> ) !important; background-position: -1px -32px; }
