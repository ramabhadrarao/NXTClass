...
class BP_Core_User {
	...
	function get_users_by_letter( $letter, $limit = null, $page = 1, $populate_extras = true, $exclude = '' ) {
		...
		$total_users_sql = apply_filters( 'bp_core_users_by_letter_count_sql', $nxtdb->prepare( "SELECT COUNT(DISTINCT u.ID) FROM {$nxtdb->users} u LEFT JOIN {$bp->profile->table_name_data} pd ON u.ID = pd.user_id LEFT JOIN {$bp->profile->table_name_fields} pf ON pd.field_id = pf.id WHERE {$status_sql} AND pf.name = %s {$exclude_sql} AND pd.value LIKE '$letter%%'  ORDER BY pd.value ASC", bp_xprofile_fullname_field_name() ), $letter );
		$paged_users_sql = apply_filters( 'bp_core_users_by_letter_sql', $nxtdb->prepare( "SELECT DISTINCT u.ID as id, u.user_registered, u.user_nicename, u.user_login, u.user_email FROM {$nxtdb->users} u LEFT JOIN {$bp->profile->table_name_data} pd ON u.ID = pd.user_id LEFT JOIN {$bp->profile->table_name_fields} pf ON pd.field_id = pf.id WHERE {$status_sql} AND pf.name = %s {$exclude_sql} AND pd.value LIKE '$letter%%' ORDER BY pd.value ASC{$pag_sql}", bp_xprofile_fullname_field_name() ), $letter, $pag_sql );
	...
	function get_specific_users( $user_ids, $limit = null, $page = 1, $populate_extras = true ) {
		...
		$user_sql   = " AND user_id IN ( " . $nxtdb->escape( $user_ids ) . " ) ";
		$status_sql = bp_core_get_status_sql();

		$total_users_sql = apply_filters( 'bp_core_get_specific_users_count_sql', $nxtdb->prepare( "SELECT COUNT(DISTINCT ID) FROM {$nxtdb->users} WHERE {$status_sql} AND ID IN ( " . $nxtdb->escape( $user_ids ) . " ) " ), $nxtdb->escape( $user_ids ) );
		$paged_users_sql = apply_filters( 'bp_core_get_specific_users_count_sql', $nxtdb->prepare( "SELECT DISTINCT ID as id, user_registered, user_nicename, user_login, user_email FROM {$nxtdb->users} WHERE {$status_sql} AND ID IN ( " . $nxtdb->escape( $user_ids ) . " ) {$pag_sql}" ), $nxtdb->escape( $user_ids ) );
	...
	function get_user_extras( &$paged_users, &$user_ids, $type = false ) {
		...
		if ( bp_is_active( 'xprofile' ) && 'alphabetical' != $type ) {
			$names = $nxtdb->get_results( $nxtdb->prepare( "SELECT pd.user_id as id, pd.value as fullname FROM {$bp->profile->table_name_fields} pf, {$bp->profile->table_name_data} pd WHERE pf.id = pd.field_id AND pf.name = %s AND pd.user_id IN ( {$user_ids} )", bp_xprofile_fullname_field_name() ) );
		...
		if ( 'popular' != $type ) {
			$friend_count = $nxtdb->get_results( $nxtdb->prepare( "SELECT user_id as id, meta_value as total_friend_count FROM {$nxtdb->usermeta} WHERE meta_key = %s AND user_id IN ( {$user_ids} )", bp_get_user_meta_key( 'total_friend_count' ) ) );
			for ( $i = 0, $count = count( $paged_users ); $i < $count; ++$i ) {
		...
		if ( 'active' != $type ) {
			$user_activity = $nxtdb->get_results( $nxtdb->prepare( "SELECT user_id as id, meta_value as last_activity FROM {$nxtdb->usermeta} WHERE meta_key = %s AND user_id IN ( {$user_ids} )", bp_get_user_meta_key( 'last_activity' ) ) );
			for ( $i = 0, $count = count( $paged_users ); $i < $count; ++$i ) {
		...
		$user_update = $nxtdb->get_results( $nxtdb->prepare( "SELECT user_id as id, meta_value as latest_update FROM {$nxtdb->usermeta} WHERE meta_key = %s AND user_id IN ( {$user_ids} )", bp_get_user_meta_key( 'bp_latest_update' ) ) );
		for ( $i = 0, $count = count( $paged_users ); $i < $count; ++$i ) {
	...
	/**
	 * Get NXTClass user details for a specified user.
	 *
	 * @global nxtdb $nxtdb NXTClass database object
	...
	function get_core_userdata( $user_id ) {
		...
		if ( !$user = $nxtdb->get_row( $nxtdb->prepare( "SELECT * FROM $nxtdb->users WHERE ID = %d LIMIT 1", $user_id ) ) )
			return false;
...
class BP_Core_Notification {
	...
	function populate() {
		...
		if ( $notification = $nxtdb->get_row( $nxtdb->prepare( "SELECT * FROM {$bp->core->table_name_notifications} WHERE id = %d", $this->id ) ) ) {
	...
	function save() {
		...
		// Update
		if ( $this->id )
			$sql = $nxtdb->prepare( "UPDATE {$bp->core->table_name_notifications} SET item_id = %d, secondary_item_id = %d, user_id = %d, component_name = %s, component_action = %d, date_notified = %s, is_new = %d ) WHERE id = %d", $this->item_id, $this->secondary_item_id, $this->user_id, $this->component_name, $this->component_action, $this->date_notified, $this->is_new, $this->id );

		// Save
		else
			$sql = $nxtdb->prepare( "INSERT INTO {$bp->core->table_name_notifications} ( item_id, secondary_item_id, user_id, component_name, component_action, date_notified, is_new ) VALUES ( %d, %d, %d, %s, %s, %s, %d )", $this->item_id, $this->secondary_item_id, $this->user_id, $this->component_name, $this->component_action, $this->date_notified, $this->is_new );
	...
	function check_access( $user_id, $notification_id ) {
		...
		return $nxtdb->get_var( $nxtdb->prepare( "SELECT COUNT(id) FROM {$bp->core->table_name_notifications} WHERE id = %d AND user_id = %d", $notification_id, $user_id ) );
	}
	...
	function get_all_for_user( $user_id ) {
		global $bp, $nxtdb;
 		...
 		return $nxtdb->get_results( $nxtdb->prepare( "SELECT * FROM {$bp->core->table_name_notifications} WHERE user_id = %d AND is_new = 1", $user_id ) );
	}
	...
	function delete_for_user_by_type( $user_id, $component_name, $component_action ) {
		...
		return $nxtdb->query( $nxtdb->prepare( "DELETE FROM {$bp->core->table_name_notifications} WHERE user_id = %d AND component_name = %s AND component_action = %s", $user_id, $component_name, $component_action ) );
	}
	...
	function delete_for_user_by_item_id( $user_id, $item_id, $component_name, $component_action, $secondary_item_id = false ) {
		...
		$secondary_item_sql = !empty( $secondary_item_id ) ? $nxtdb->prepare( " AND secondary_item_id = %d", $secondary_item_id ) : '';

		return $nxtdb->query( $nxtdb->prepare( "DELETE FROM {$bp->core->table_name_notifications} WHERE user_id = %d AND item_id = %d AND component_name = %s AND component_action = %s{$secondary_item_sql}", $user_id, $item_id, $component_name, $component_action ) );
	}
	...
	function delete_from_user_by_type( $user_id, $component_name, $component_action ) {
		...
		return $nxtdb->query( $nxtdb->prepare( "DELETE FROM {$bp->core->table_name_notifications} WHERE item_id = %d AND component_name = %s AND component_action = %s", $user_id, $component_name, $component_action ) );
	}
	...
	function delete_all_by_type( $item_id, $component_name, $component_action, $secondary_item_id ) {
		...
		if ( $component_action )
			$component_action_sql = $nxtdb->prepare( "AND component_action = %s", $component_action );
		else
			$component_action_sql = '';

		if ( $secondary_item_id )
			$secondary_item_sql = $nxtdb->prepare( "AND secondary_item_id = %d", $secondary_item_id );
		else
			$secondary_item_sql = '';

		return $nxtdb->query( $nxtdb->prepare( "DELETE FROM {$bp->core->table_name_notifications} WHERE item_id = %d AND component_name = %s {$component_action_sql} {$secondary_item_sql}", $item_id, $component_name ) );
	}
}
 ...
 * BP_Embed
 *
 * Extends nxt_Embed class for use with BuddyPress.
 *
 * @package BuddyPress Core
 * @since 1.5
 * @see nxt_Embed
 */
class BP_Embed extends nxt_Embed {
	/**
	 * Constructor
	 *
	 * @global unknown $nxt_embed
	 */
	function __construct() {
		global $nxt_embed;

		// Make sure we populate the nxt_Embed handlers array.
		...
		// Used in nxt_Embed::shortcode()
		$this->handlers = $nxt_embed->handlers;
