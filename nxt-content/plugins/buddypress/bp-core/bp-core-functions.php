 ...
 *
 * @global object $nxtdb NXTClass database object
 * @return string Filtered database prefix
 */
function bp_core_get_table_prefix() {
	global $nxtdb;

	return apply_filters( 'bp_core_get_table_prefix', $nxtdb->base_prefix );
}
 ...
 * @since 1.5
 *
 * @param array $blog_page_ids The IDs of the nxt pages corresponding to BP component directories
 */
...
function bp_core_get_directory_pages() {
	...
	if ( $page_ids = bp_core_get_directory_page_ids() ) {
		...
		$posts_table_name = bp_is_multiblog_mode() ? $nxtdb->posts : $nxtdb->get_blog_prefix( bp_get_root_blog_id() ) . 'posts';
		$page_ids_sql     = implode( ',', (array)$page_ids );
		$page_names       = $nxtdb->get_results( $nxtdb->prepare( "SELECT ID, post_name, post_parent, post_title FROM {$posts_table_name} WHERE ID IN ({$page_ids_sql}) AND post_status = 'publish' " ) );
			...
			foreach ( (array)$page_names as $page_name ) {
				if ( $page_name->ID == $page_id ) {
					...
					while ( $page_name->post_parent != 0 ) {
						$parent                 = $nxtdb->get_results( $nxtdb->prepare( "SELECT post_name, post_parent FROM {$posts_table_name} WHERE ID = %d", $page_name->post_parent ) );
...
/**
 * Creates a default component slug from a nxt page root_slug
 *
 * Since 1.5, BP components get their root_slug (the slug used immediately
 * following the root domain) from the slug of a corresponding nxt page.
 *
 ...
 *
 * @package BuddyPress Core
 * @uses get_blog_option() NXTClass function to fetch blog meta.
...
/**
 * Adds a feedback (error/success) message to the nxt cookie so it can be
...
/**
 * Checks if there is a feedback message in the nxt cookie, if so, adds a
...
add_action( 'nxt_head', 'bp_core_record_activity' );
...
/**
 * Performs a status safe nxt_redirect() that is compatible with bp_catch_uri()
 *
 * @package BuddyPress Core
...
function bp_core_redirect( $location, $status = 302 ) {
	...
	// as this conflicts with nxt_redirect()
...
/**
 * Adds illegal names to nxt so that root components will not conflict with
		...
		switch ( $search_which ) {
			case 'posts':
				...
				if ( 'page' == get_option( 'show_on_front' ) ) {
					...
					if ( !is_nxt_error( $page ) && !empty( $page->post_name ) ) {
						$slug = $page->post_name;
						$var  = '?s=';
					}
				}
...
add_action( 'nxt_footer', 'bp_core_print_generation_time' );
...
function bp_core_load_buddypress_textdomain() {
	...
	$mofile_global = nxt_LANG_DIR . '/' . $mofile;
...
function bp_core_add_ajax_hook() {
	// Theme only, we already have the nxt_ajax_ hook firing in nxt-admin
	if ( !defined( 'nxt_ADMIN' ) && isset( $_REQUEST['action'] ) )
		do_action( 'nxt_ajax_' . $_REQUEST['action'] );
}
...
function bp_core_get_root_options() {
	...
	$blog_options_table	= bp_is_multiblog_mode() ? $nxtdb->options : $nxtdb->get_blog_prefix( bp_get_root_blog_id() ) . 'options';

	$blog_options_query     = $nxtdb->prepare( "SELECT option_name AS name, option_value AS value FROM {$blog_options_table} WHERE option_name IN ( {$blog_options_keys} )" );
	$root_blog_options_meta = $nxtdb->get_results( $blog_options_query );
	...
	if ( is_multisite() ) {
		...
		$sitemeta_options_query = $nxtdb->prepare( "SELECT meta_key AS name, meta_value AS value FROM {$nxtdb->sitemeta} WHERE meta_key IN ( {$sitemeta_options_keys} ) AND site_id = %d", $current_site->id );
		$network_options_meta   = $nxtdb->get_results( $sitemeta_options_query );
 ...
 * These root level pages are now handled by actual NXTClass pages and this function is now
...
function bp_core_create_root_component_page() {
	...
	foreach ( (array)$bp->add_root as $slug )
		$new_page_ids[$slug] = nxt_insert_post( array( 'comment_status' => 'closed', 'ping_status' => 'closed', 'post_title' => ucwords( $slug ), 'post_status' => 'publish', 'post_type' => 'page' ) );
 ...
 *
 * BuddyPress stores a number of pieces of userdata in the NXTClass central usermeta table. In
 * order to allow plugins to enable multiple instances of BuddyPress on a single nxt installation,
 ...
 * If you have to use nxt's _user_meta() functions for some reason, you should use this function, eg
 *    $last_active = get_user_meta( $user_id, bp_get_user_meta_key( 'last_activity' ), true );
 * If using the nxt functions, do not not hardcode your meta keys.
 *
 * @package BuddyPress
 * @since 1.5
 *
 ...
 *
 * Note that BP_ENABLE_MULTIBLOG is different from (but dependent on) nxt Multisite. "Multiblog" is
...
/**
 * Should we use the nxt admin bar?
 *
 * The nxt Admin Bar, introduced in nxt 3.1, is fully supported in BuddyPress as of BP 1.5.
 *
 * For the BP 1.5 development cycle, the BuddyBar will remain the default navigation for BP
 * installations. In the future, this behavior will be changed, so that the nxt Admin Bar is the
 * default.
 *
 * @package BuddyPress
 * @since 1.5
 *
 * @uses apply_filters() Filter 'bp_use_nxt_admin_bar' to alter
 * @return bool False when nxt Admin Bar support is disabled (default); true when enabled
 */
function bp_use_nxt_admin_bar() {
	return apply_filters( 'bp_use_nxt_admin_bar', defined( 'BP_USE_nxt_ADMIN_BAR' ) && BP_USE_nxt_ADMIN_BAR );
}
...
/**
 * Output the correct URL based on BuddyPress and NXTClass configuration
 *
 * @package BuddyPress
 * @since 1.5
 *
 ...
 *
 * @global object $bp Global BuddyPress settings object
 * @global nxt_Query $nxt_query NXTClass query object
 * @param string $redirect If 'remove_canonical_direct', remove NXTClass' "helpful" redirect_canonical action.
 * @since 1.5
 */
