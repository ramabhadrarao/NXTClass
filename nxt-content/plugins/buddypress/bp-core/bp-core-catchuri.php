...
Modified for BuddyPress by: Andy Peatling - http://apeatling.nxtclass.com/
*/
 ...
 * Loads:
 *   nxt-content/themes/[activated_theme]/members/index.php
 *
 * @package BuddyPress Core
...
function bp_core_load_template( $templates ) {
	global $post, $bp, $nxtdb, $nxt_query, $bp_unfiltered_uri, $bp_unfiltered_uri_offset;

	// Determine if the root object nxt page exists for this request (TODO: is there an API function for this?
	if ( !empty( $bp_unfiltered_uri_offset ) && !$page_exists = $nxtdb->get_var( $nxtdb->prepare( "SELECT ID FROM {$nxtdb->posts} WHERE post_name = %s", $bp_unfiltered_uri[$bp_unfiltered_uri_offset] ) ) )
		return false;

	// Set the root object as the current nxt_query-ied item
	$object_id = 0;
	...
	if ( !empty( $object_id ) ) {
		$nxt_query->queried_object    = &get_post( $object_id );
		$nxt_query->queried_object_id = $object_id;
		$post                        = $nxt_query->queried_object;
	}
...
add_action( 'nxt', 'bp_core_catch_no_access' );
...
function bp_core_no_access( $args = '' ) {
	...
	$defaults = array(
		'mode'     => '1',			    // 1 = $root, 2 = nxt-login.php
		'message'  => __( 'You must log in to access the page you requested.', 'buddypress' ),
		'redirect' => nxt_guess_url(),	// the URL you get redirected to when a user successfully logs in
...
/**
 * Adds an error message to nxt-login.php.
 ...
 * @since 1.5
 */
function bp_core_no_access_nxt_login_error() {
	global $error;

	$error = apply_filters( 'bp_nxt_login_error', __( 'You must log in to access the page you requested.', 'buddypress' ), $_REQUEST['redirect_to'] );

	// shake shake shake!
	add_action( 'login_head', 'nxt_shake_js', 12 );
}
add_action( 'login_form_bpnoaccess', 'bp_core_no_access_nxt_login_error' );
