...
add_action( 'nxt_head',    'bp_core_sort_nav_items' );
...
add_action( 'nxt_head',    'bp_core_sort_subnav_items' );
...
function bp_core_admin_bar() {
	...
	echo '<div id="nxt-admin-bar"><div class="padder">';
...
function bp_adminbar_login_menu() {
	...
	echo '<li class="bp-login no-arrow"><a href="' . bp_get_root_domain() . '/nxt-login.php?redirect_to=' . urlencode( bp_get_root_domain() ) . '">' . __( 'Log In', 'buddypress' ) . '</a></li>';
 ...
 *
 * This is a direct copy of nxt's private _get_admin_bar_pref()
 *
 * @since 1.5.0
 *
 ...
 * @uses add_action() To hook 'bp_core_admin_bar' to 'nxt_footer'
...
function bp_core_load_admin_bar() {
	...
	// Show the NXTClass admin bar
	if ( bp_use_nxt_admin_bar() && $nxt_version >= 3.1 ) {
	...
	// Hide the NXTClass admin bar
	} elseif ( !bp_use_nxt_admin_bar() ) {

		// Keep the nxt admin bar from loading
		...
		add_action( 'nxt_footer',    'bp_core_admin_bar', 8 );
