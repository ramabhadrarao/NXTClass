...
add_filter( 'nxt_list_pages_excludes', 'bp_core_exclude_pages' );
...
add_filter( 'nxt_mail_from_name', 'bp_core_email_from_name_filter' );
...
add_filter( 'nxt_mail_from', 'bp_core_email_from_address_filter' );
...
function bp_core_allow_default_theme( $themes ) {
	...
	if ( $nxtdb->blogid == bp_get_root_blog_id() ) {
		$themes['bp-default'] = 1;
	}
...
function bp_core_filter_comments( $comments, $post_id ) {
	...
	if ( !$userdata = $nxtdb->get_results( $nxtdb->prepare( "SELECT ID as user_id, user_login, user_nicename FROM {$nxtdb->users} WHERE ID IN ({$user_ids})" ) ) )
		return $comments;
 ...
 * @param string $welcome_email Complete email passed through NXTClass
...
if ( !is_admin() )
	add_filter( 'nxtmu_signup_blog_notification', 'bp_core_activation_signup_blog_notification', 1, 7 );
...
if ( !is_admin() || ( is_admin() && empty( $_POST['noconfirmation'] ) ) )
	add_filter( 'nxtmu_signup_user_notification', 'bp_core_activation_signup_user_notification', 1, 4 );
...
function bp_modify_page_title( $title, $sep, $seplocation ) {
	global $bp, $post, $nxt_query;

	// If this is not a BP page, just return the title produced by nxt
	if ( bp_is_blog_page() )
		return $title;

	// If this is the front page of the site, return nxt's title
...
add_filter( 'nxt_title', 'bp_modify_page_title', 10, 3 );
add_filter( 'bp_modify_page_title', 'nxttexturize'     );
