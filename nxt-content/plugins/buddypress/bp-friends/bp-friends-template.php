...
function bp_friends_random_friends() {
	...
	if ( !$friend_ids = nxt_cache_get( 'friends_friend_ids_' . $bp->displayed_user->id, 'bp' ) ) {
		$friend_ids = BP_Friends_Friendship::get_random_friends( $bp->displayed_user->id );
		nxt_cache_set( 'friends_friend_ids_' . $bp->displayed_user->id, $friend_ids, 'bp' );
	} ?>
...
function bp_friends_random_members( $total_members = 5 ) {
	...
	if ( !$user_ids = nxt_cache_get( 'friends_random_users', 'bp' ) ) {
		$user_ids = BP_Core_User::get_users( 'random', $total_members );
		nxt_cache_set( 'friends_random_users', $user_ids, 'bp' );
	}
...
function bp_friend_search_form() {
	...
	$label = __( 'Filter Friends', 'buddypress' ); ?>
		...
		<form action="<?php echo $action ?>" id="friend-search-form" method="post">
			...
			<?php nxt_nonce_field( 'friends_search', '_nxtnonce_friend_search' ) ?>
		...
		switch ( $is_friend ) {
			...
			case 'is_friend' :
				$button = array(
					...
					'wrapper_id'        => 'friendship-button-' . $potential_friend_id,
					'link_href'         => nxt_nonce_url( $bp->loggedin_user->domain . bp_get_friends_slug() . '/remove-friend/' . $potential_friend_id . '/', 'friends_remove_friend' ),
			...
			default:
				$button = array(
					...
					'wrapper_id'        => 'friendship-button-' . $potential_friend_id,
					'link_href'         => nxt_nonce_url( $bp->loggedin_user->domain . bp_get_friends_slug() . '/add-friend/' . $potential_friend_id . '/', 'friends_add_friend' ),
...
function bp_friend_accept_request_link() {
	...
	function bp_get_friend_accept_request_link() {
		...
		return apply_filters( 'bp_get_friend_accept_request_link', nxt_nonce_url( $bp->loggedin_user->domain . bp_get_friends_slug() . '/requests/accept/' . $friendship_id, 'friends_accept_friendship' ) );
	}
