...
class BP_Friends_Friendship {
	...
	function populate() {
		global $nxtdb, $bp, $creds;

		if ( $friendship = $nxtdb->get_row( $nxtdb->prepare( "SELECT * FROM {$bp->friends->table_name} WHERE id = %d", $this->id ) ) ) {
	...
	function save() {
		...
		if ( $this->id ) {
			// Update
			$result = $nxtdb->query( $nxtdb->prepare( "UPDATE {$bp->friends->table_name} SET initiator_user_id = %d, friend_user_id = %d, is_confirmed = %d, is_limited = %d, date_created = %s ) WHERE id = %d", $this->initiator_user_id, $this->friend_user_id, $this->is_confirmed, $this->is_limited, $this->date_created, $this->id ) );
		} else {
			// Save
			$result = $nxtdb->query( $nxtdb->prepare( "INSERT INTO {$bp->friends->table_name} ( initiator_user_id, friend_user_id, is_confirmed, is_limited, date_created ) VALUES ( %d, %d, %d, %d, %s )", $this->initiator_user_id, $this->friend_user_id, $this->is_confirmed, $this->is_limited, $this->date_created ) );
	...
	function delete() {
		...
		return $nxtdb->query( $nxtdb->prepare( "DELETE FROM {$bp->friends->table_name} WHERE id = %d", $this->id ) );
	}
	...
	function get_friend_user_ids( $user_id, $friend_requests_only = false, $assoc_arr = false ) {
		...
		if ( $friend_requests_only ) {
			$oc_sql = $nxtdb->prepare( "AND is_confirmed = 0" );
			$friend_sql = $nxtdb->prepare ( " WHERE friend_user_id = %d", $user_id );
		} else {
			$oc_sql = $nxtdb->prepare( "AND is_confirmed = 1" );
			$friend_sql = $nxtdb->prepare ( " WHERE (initiator_user_id = %d OR friend_user_id = %d)", $user_id, $user_id );
		}

		$friends = $nxtdb->get_results( $nxtdb->prepare( "SELECT friend_user_id, initiator_user_id FROM {$bp->friends->table_name} $friend_sql $oc_sql ORDER BY date_created DESC" ) );
		$fids = array();
	...
	function get_friendship_id( $user_id, $friend_id ) {
		...
		return $nxtdb->get_var( $nxtdb->prepare( "SELECT id FROM {$bp->friends->table_name} WHERE ( initiator_user_id = %d AND friend_user_id = %d ) OR ( initiator_user_id = %d AND friend_user_id = %d ) AND is_confirmed = 1", $user_id, $friend_id, $friend_id, $user_id ) );
	}

	function get_friendship_request_user_ids( $user_id ) {
		global $nxtdb, $bp;

		return $nxtdb->get_col( $nxtdb->prepare( "SELECT initiator_user_id FROM {$bp->friends->table_name} WHERE friend_user_id = %d AND is_confirmed = 0", $user_id ) );
	}
	...
	function search_friends( $filter, $user_id, $limit = null, $page = null ) {
		...
		if ( bp_is_active( 'xprofile' ) ) {
			...
			$sql = "SELECT DISTINCT user_id FROM {$nxtdb->usermeta} WHERE user_id IN ($fids) AND meta_key = 'nickname' AND meta_value LIKE '$filter%%' {$pag_sql}";
			$total_sql = "SELECT COUNT(DISTINCT user_id) FROM {$nxtdb->usermeta} WHERE user_id IN ($fids) AND meta_key = 'nickname' AND meta_value LIKE '$filter%%'";
		}

		$filtered_friend_ids = $nxtdb->get_col($sql);
		$total_friend_ids = $nxtdb->get_var($total_sql);
	...
	function check_is_friend( $loggedin_userid, $possible_friend_userid ) {
		...
		$result = $nxtdb->get_results( $nxtdb->prepare( "SELECT id, is_confirmed FROM {$bp->friends->table_name} WHERE (initiator_user_id = %d AND friend_user_id = %d) OR (initiator_user_id = %d AND friend_user_id = %d)", $loggedin_userid, $possible_friend_userid, $possible_friend_userid, $loggedin_userid ) );
	...
	function accept($friendship_id) {
		global $nxtdb, $bp;
	 	...
	 	return $nxtdb->query( $nxtdb->prepare( "UPDATE {$bp->friends->table_name} SET is_confirmed = 1, date_created = %s WHERE id = %d AND friend_user_id = %d", bp_core_current_time(), $friendship_id, $bp->loggedin_user->id ) );
	}

	function reject($friendship_id) {
		global $nxtdb, $bp;

		return $nxtdb->query( $nxtdb->prepare( "DELETE FROM {$bp->friends->table_name} WHERE id = %d AND friend_user_id = %d", $friendship_id, $bp->loggedin_user->id ) );
	}
	...
	function search_users( $filter, $user_id, $limit = null, $page = null ) {
		...
		$usermeta_table = $nxtdb->base_prefix . 'usermeta';
		$users_table = $nxtdb->base_prefix . 'users';

		if ( $limit && $page )
			$pag_sql = $nxtdb->prepare( " LIMIT %d, %d", intval( ( $page - 1 ) * intval( $limit ) ), intval( $limit ) );

		// filter the user_ids based on the search criteria.
		if ( bp_is_active( 'xprofile' ) ) {
			$sql = $nxtdb->prepare( "SELECT DISTINCT d.user_id as id FROM {$bp->profile->table_name_data} d, $users_table u WHERE d.user_id = u.id AND d.value LIKE '$filter%%' ORDER BY d.value DESC $pag_sql" );
		} else {
			$sql = $nxtdb->prepare( "SELECT DISTINCT user_id as id FROM $usermeta_table WHERE meta_value LIKE '$filter%%' ORDER BY d.value DESC $pag_sql" );
		}

		$filtered_fids = $nxtdb->get_col($sql);
	...
	function search_users_count( $filter ) {
		...
		$usermeta_table = $nxtdb->prefix . 'usermeta';
		$users_table = $nxtdb->base_prefix . 'users';

		// filter the user_ids based on the search criteria.
		if ( bp_is_active( 'xprofile' ) ) {
			$sql = $nxtdb->prepare( "SELECT COUNT(DISTINCT d.user_id) FROM {$bp->profile->table_name_data} d, $users_table u WHERE d.user_id = u.id AND d.value LIKE '$filter%%'" );
		} else {
			$sql = $nxtdb->prepare( "SELECT COUNT(DISTINCT user_id) FROM $usermeta_table WHERE meta_value LIKE '$filter%%'" );
		}

		$user_count = $nxtdb->get_col($sql);
	...
	function get_random_friends( $user_id, $total_friends = 5 ) {
		...
		$sql = $nxtdb->prepare( "SELECT friend_user_id, initiator_user_id FROM {$bp->friends->table_name} WHERE (friend_user_id = %d || initiator_user_id = %d) && is_confirmed = 1 ORDER BY rand() LIMIT %d", $user_id, $user_id, $total_friends );
	...
	function get_user_ids_for_friendship( $friendship_id ) {
		...
		return $nxtdb->get_row( $nxtdb->prepare( "SELECT friend_user_id, initiator_user_id FROM {$bp->friends->table_name} WHERE id = %d", $friendship_id ) );
	}
