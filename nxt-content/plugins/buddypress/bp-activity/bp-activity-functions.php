...
function bp_activity_get_meta( $activity_id = 0, $meta_key = '' ) {
	...
	if ( !empty( $meta_key ) ) {
		...
		// Check cache
		if ( !$metas = nxt_cache_get( 'bp_activity_meta_' . $meta_key . '_' . $activity_id, 'bp' ) ) {

			// No cache so hit the DB
			$metas = $nxtdb->get_col( $nxtdb->prepare("SELECT meta_value FROM {$bp->activity->table_name_meta} WHERE activity_id = %d AND meta_key = %s", $activity_id, $meta_key ) );

			// Set cache
			nxt_cache_set( 'bp_activity_meta_' . $meta_key . '_' . $activity_id, $metas, 'bp' );
		}

	// No key so get all for activity_id
	} else {
		$metas = $nxtdb->get_col( $nxtdb->prepare( "SELECT meta_value FROM {$bp->activity->table_name_meta} WHERE activity_id = %d", $activity_id ) );
	}
...
add_action( 'nxtmu_delete_user',  'bp_activity_remove_all_user_data' );
