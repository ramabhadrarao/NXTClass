...
Class BP_Activity_Activity {
	...
	function populate() {
		...
		if ( $row = $nxtdb->get_row( $nxtdb->prepare( "SELECT * FROM {$bp->activity->table_name} WHERE id = %d", $this->id ) ) ) {
			$this->id                = $row->id;
	...
	function save() {
		...
		if ( $this->id )
			$q = $nxtdb->prepare( "UPDATE {$bp->activity->table_name} SET user_id = %d, component = %s, type = %s, action = %s, content = %s, primary_link = %s, date_recorded = %s, item_id = %s, secondary_item_id = %s, hide_sitewide = %d WHERE id = %d", $this->user_id, $this->component, $this->type, $this->action, $this->content, $this->primary_link, $this->date_recorded, $this->item_id, $this->secondary_item_id, $this->hide_sitewide, $this->id );
		else
			$q = $nxtdb->prepare( "INSERT INTO {$bp->activity->table_name} ( user_id, component, type, action, content, primary_link, date_recorded, item_id, secondary_item_id, hide_sitewide ) VALUES ( %d, %s, %s, %s, %s, %s, %s, %s, %s, %d )", $this->user_id, $this->component, $this->type, $this->action, $this->content, $this->primary_link, $this->date_recorded, $this->item_id, $this->secondary_item_id, $this->hide_sitewide );
	...
	function get( $max = false, $page = 1, $per_page = 25, $sort = 'DESC', $search_terms = false, $filter = false, $display_comments = false, $show_hidden = false, $exclude = false, $in = false ) {
		...
		if ( $per_page && $page ) {
			$pag_sql = $nxtdb->prepare( "LIMIT %d, %d", intval( ( $page - 1 ) * $per_page ), intval( $per_page ) );
	...
	function get_id( $user_id, $component, $type, $item_id, $secondary_item_id, $action, $content, $date_recorded ) {
		...
		return $nxtdb->get_var( "SELECT id FROM {$bp->activity->table_name} {$where_sql}" );
	}
	...
	function delete_activity_item_comments( $activity_ids ) {
		...
		return $nxtdb->query( $nxtdb->prepare( "DELETE FROM {$bp->activity->table_name} WHERE type = 'activity_comment' AND item_id IN ({$activity_ids})" ) );
	}
	...
	function delete_activity_meta_entries( $activity_ids ) {
		...
		return $nxtdb->query( $nxtdb->prepare( "DELETE FROM {$bp->activity->table_name_meta} WHERE activity_id IN ({$activity_ids})" ) );
	}
	...
	function get_child_comments( $parent_id ) {
		...
		return $nxtdb->get_results( $nxtdb->prepare( "SELECT id FROM {$bp->activity->table_name} WHERE type = 'activity_comment' AND secondary_item_id = %d", $parent_id ) );
	}

	function get_recorded_components() {
		global $nxtdb, $bp;

		return $nxtdb->get_col( $nxtdb->prepare( "SELECT DISTINCT component FROM {$bp->activity->table_name} ORDER BY component ASC" ) );
	}
	...
	function get_in_operator_sql( $field, $items ) {
		...
		foreach ( $items_dirty as $item ) {
			...
			$items_prepared[] = ( is_numeric( $item ) ) ? $nxtdb->prepare( '%d', $item ) : $nxtdb->prepare( '%s', $item );
		}
	...
	function get_last_updated() {
		...
		return $nxtdb->get_var( $nxtdb->prepare( "SELECT date_recorded FROM {$bp->activity->table_name} ORDER BY date_recorded DESC LIMIT 1" ) );
	}
	...
	function check_exists_by_content( $content ) {
		...
		return $nxtdb->get_var( $nxtdb->prepare( "SELECT id FROM {$bp->activity->table_name} WHERE content = %s", $content ) );
	}

	function hide_all_for_user( $user_id ) {
		global $nxtdb, $bp;

		return $nxtdb->get_var( $nxtdb->prepare( "UPDATE {$bp->activity->table_name} SET hide_sitewide = 1 WHERE user_id = %d", $user_id ) );
	}
}
