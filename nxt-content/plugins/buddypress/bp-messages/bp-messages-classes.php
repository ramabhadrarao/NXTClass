...
Class BP_Messages_Thread {
	...
	function populate( $thread_id, $order ) {
		...
		if ( !$this->messages = $nxtdb->get_results( $nxtdb->prepare( "SELECT * FROM {$bp->messages->table_name_messages} WHERE thread_id = %d ORDER BY date_sent " . $order, $this->thread_id ) ) )
			return false;
	...
	function get_recipients() {
		...
		$results = $nxtdb->get_results( $nxtdb->prepare( "SELECT * FROM {$bp->messages->table_name_recipients} WHERE thread_id = %d", $this->thread_id ) );
	...
	function delete( $thread_id ) {
		...
		$delete_for_user = $nxtdb->query( $nxtdb->prepare( "UPDATE {$bp->messages->table_name_recipients} SET is_deleted = 1 WHERE thread_id = %d AND user_id = %d", $thread_id, $bp->loggedin_user->id ) );

		// Check to see if any more recipients remain for this message
		// if not, then delete the message from the database.
		$recipients = $nxtdb->get_results( $nxtdb->prepare( "SELECT id FROM {$bp->messages->table_name_recipients} WHERE thread_id = %d AND is_deleted = 0", $thread_id ) );

		if ( empty( $recipients ) ) {
			// Delete all the messages
			$nxtdb->query( $nxtdb->prepare( "DELETE FROM {$bp->messages->table_name_messages} WHERE thread_id = %d", $thread_id ) );

			// Delete all the recipients
			$nxtdb->query( $nxtdb->prepare( "DELETE FROM {$bp->messages->table_name_recipients} WHERE thread_id = %d", $thread_id ) );
		}
	...
	function get_current_threads_for_user( $user_id, $box = 'inbox', $type = 'all', $limit = null, $page = null ) {
		...
		if ( $type == 'unread' )
			$type_sql = $nxtdb->prepare( " AND r.unread_count != 0 " );
		elseif ( $type == 'read' )
			$type_sql = $nxtdb->prepare( " AND r.unread_count = 0 " );

		if ( 'sentbox' == $box ) {
			$thread_ids = $nxtdb->get_results( $nxtdb->prepare( "SELECT m.thread_id, MAX(m.date_sent) AS date_sent FROM {$bp->messages->table_name_recipients} r, {$bp->messages->table_name_messages} m WHERE m.thread_id = r.thread_id AND m.sender_id = r.user_id AND m.sender_id = %d AND r.is_deleted = 0 GROUP BY m.thread_id ORDER BY date_sent DESC {$pag_sql}", $user_id ) );
			$total_threads = $nxtdb->get_var( $nxtdb->prepare( "SELECT COUNT( DISTINCT m.thread_id ) FROM {$bp->messages->table_name_recipients} r, {$bp->messages->table_name_messages} m WHERE m.thread_id = r.thread_id AND m.sender_id = r.user_id AND m.sender_id = %d AND r.is_deleted = 0 ", $user_id ) );
		} else {
			$thread_ids = $nxtdb->get_results( $nxtdb->prepare( "SELECT m.thread_id, MAX(m.date_sent) AS date_sent FROM {$bp->messages->table_name_recipients} r, {$bp->messages->table_name_messages} m WHERE m.thread_id = r.thread_id AND r.is_deleted = 0 AND r.user_id = %d AND r.sender_only = 0 {$type_sql} GROUP BY m.thread_id ORDER BY date_sent DESC {$pag_sql}", $user_id ) );
			$total_threads = $nxtdb->get_var( $nxtdb->prepare( "SELECT COUNT( DISTINCT m.thread_id ) FROM {$bp->messages->table_name_recipients} r, {$bp->messages->table_name_messages} m WHERE m.thread_id = r.thread_id AND r.is_deleted = 0 AND r.user_id = %d AND r.sender_only = 0 {$type_sql}", $user_id ) );
		}
	...
	function mark_as_read( $thread_id ) {
		...
		$sql = $nxtdb->prepare( "UPDATE {$bp->messages->table_name_recipients} SET unread_count = 0 WHERE user_id = %d AND thread_id = %d", $bp->loggedin_user->id, $thread_id );
		$nxtdb->query($sql);
	}
	...
	function mark_as_unread( $thread_id ) {
		...
		$sql = $nxtdb->prepare( "UPDATE {$bp->messages->table_name_recipients} SET unread_count = 1 WHERE user_id = %d AND thread_id = %d", $bp->loggedin_user->id, $thread_id );
		$nxtdb->query($sql);
	}
	...
	function get_total_threads_for_user( $user_id, $box = 'inbox', $type = 'all' ) {
		...
		if ( $type == 'unread' )
			$type_sql = $nxtdb->prepare( " AND unread_count != 0 " );
		else if ( $type == 'read' )
			$type_sql = $nxtdb->prepare( " AND unread_count = 0 " );

		return (int) $nxtdb->get_var( $nxtdb->prepare( "SELECT COUNT(thread_id) FROM {$bp->messages->table_name_recipients} WHERE user_id = %d AND is_deleted = 0$exclude_sender $type_sql", $user_id ) );
	}

	function user_is_sender( $thread_id ) {
		global $nxtdb, $bp;

		$sender_ids = $nxtdb->get_col( $nxtdb->prepare( "SELECT sender_id FROM {$bp->messages->table_name_messages} WHERE thread_id = %d", $thread_id ) );
	...
	function get_last_sender( $thread_id ) {
		...
		if ( !$sender_id = $nxtdb->get_var( $nxtdb->prepare( "SELECT sender_id FROM {$bp->messages->table_name_messages} WHERE thread_id = %d GROUP BY sender_id ORDER BY date_sent LIMIT 1", $thread_id ) ) )
			return false;
	...
	function check_access( $thread_id, $user_id = 0 ) {
		...
		return $nxtdb->get_var( $nxtdb->prepare( "SELECT id FROM {$bp->messages->table_name_recipients} WHERE thread_id = %d AND user_id = %d", $thread_id, $user_id ) );
	}

	function is_valid( $thread_id ) {
		global $nxtdb, $bp;

		return $nxtdb->get_var( $nxtdb->prepare( "SELECT thread_id FROM {$bp->messages->table_name_messages} WHERE thread_id = %d LIMIT 1", $thread_id ) );
	}
	...
	function update_tables() {
		...
		$errors    = false;
		$threads   = $nxtdb->get_results( $nxtdb->prepare( "SELECT * FROM {$bp_prefix}bp_messages_threads" ) );
		...
		foreach( (array)$threads as $thread ) {
			...
			if ( !empty( $message_ids ) ) {
				...
				if ( !$nxtdb->query( $nxtdb->prepare( "UPDATE {$bp->messages->table_name_messages} SET thread_id = %d WHERE id IN ({$message_ids})", $thread->id ) ) )
					$errors = true;
			}
		}
...
Class BP_Messages_Message {
	...
	function populate( $id ) {
		...
		if ( $message = $nxtdb->get_row( $nxtdb->prepare( "SELECT * FROM {$bp->messages->table_name_messages} WHERE id = %d", $id ) ) ) {
			$this->id        = $message->id;
	...
	function get_recipients() {
		...
		return $nxtdb->get_results( $nxtdb->prepare( "SELECT user_id FROM {$bp->messages->table_name_recipients} WHERE thread_id = %d", $this->thread_id ) );
	}
	...
	function get_last_sent_for_user( $thread_id ) {
		...
		return $nxtdb->get_var( $nxtdb->prepare( "SELECT id FROM {$bp->messages->table_name_messages} WHERE sender_id = %d AND thread_id = %d ORDER BY date_sent DESC LIMIT 1", $bp->loggedin_user->id, $thread_id ) );
	}

	function is_user_sender( $user_id, $message_id ) {
		global $nxtdb, $bp;
		return $nxtdb->get_var( $nxtdb->prepare( "SELECT id FROM {$bp->messages->table_name_messages} WHERE sender_id = %d AND id = %d", $user_id, $message_id ) );
	}

	function get_message_sender( $message_id ) {
		global $nxtdb, $bp;
		return $nxtdb->get_var( $nxtdb->prepare( "SELECT sender_id FROM {$bp->messages->table_name_messages} WHERE id = %d", $message_id ) );
	}
}
...
Class BP_Messages_Notice {
	...
	function populate() {
		...
		$notice = $nxtdb->get_row( $nxtdb->prepare( "SELECT * FROM {$bp->messages->table_name_notices} WHERE id = %d", $this->id ) );
	...
	function delete() {
		...
		$sql = $nxtdb->prepare( "DELETE FROM {$bp->messages->table_name_notices} WHERE id = %d", $this->id );
	...
	function get_notices() {
		...
		$notices = $nxtdb->get_results( $nxtdb->prepare( "SELECT * FROM {$bp->messages->table_name_notices} ORDER BY date_sent DESC" ) );
		return $notices;
	}
	...
	function get_total_notice_count() {
		...
		$notice_count = $nxtdb->get_var( $nxtdb->prepare( "SELECT COUNT(id) FROM " . $bp->messages->table_name_notices ) );
	...
	function get_active() {
		...
		$notice_id = $nxtdb->get_var( $nxtdb->prepare( "SELECT id FROM {$bp->messages->table_name_notices} WHERE is_active = 1") );
