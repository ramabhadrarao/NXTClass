...
function bp_message_thread_delete_link() {
	...
	function bp_get_message_thread_delete_link() {
		global $messages_template, $bp;
		return apply_filters( 'bp_get_message_thread_delete_link', nxt_nonce_url( $bp->loggedin_user->domain . $bp->messages->slug . '/' . $bp->current_action . '/delete/' . $messages_template->thread->thread_id, 'messages_delete_thread' ) );
	}
...
function bp_message_notice_delete_link() {
	...
	function bp_get_message_notice_delete_link() {
		...
		return apply_filters( 'bp_get_message_notice_delete_link', nxt_nonce_url( $bp->loggedin_user->domain . $bp->messages->slug . '/notices/delete/' . $messages_template->thread->id, 'messages_delete_thread' ) );
	}
...
function bp_message_activate_deactivate_link() {
	...
	function bp_get_message_activate_deactivate_link() {
		...
		if ( 1 == (int)$messages_template->thread->is_active ) {
			$link = nxt_nonce_url( $bp->loggedin_user->domain . $bp->messages->slug . '/notices/deactivate/' . $messages_template->thread->id, 'messages_deactivate_notice' );
		} else {
			$link = nxt_nonce_url( $bp->loggedin_user->domain . $bp->messages->slug . '/notices/activate/' . $messages_template->thread->id, 'messages_activate_notice' );
		}
	...
	if ( is_array($closed_notices) ) {
		if ( !in_array( $notice->id, $closed_notices ) && $notice->id ) {
			...
			<div id="message" class="info notice" rel="n-<?php echo $notice->id ?>">
				<p>
					<strong><?php echo stripslashes( nxt_filter_kses( $notice->subject ) ) ?></strong><br />
					<?php echo stripslashes( nxt_filter_kses( $notice->message) ) ?>
