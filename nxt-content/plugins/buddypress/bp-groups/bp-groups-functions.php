...
function groups_get_groupmeta( $group_id, $meta_key = '') {
	...
	if ( !empty($meta_key) ) {
		...
		if ( !$metas = nxt_cache_get( 'bp_groups_groupmeta_' . $group_id . '_' . $meta_key, 'bp' ) ) {
			$metas = $nxtdb->get_col( $nxtdb->prepare("SELECT meta_value FROM " . $bp->groups->table_name_groupmeta . " WHERE group_id = %d AND meta_key = %s", $group_id, $meta_key) );
			nxt_cache_set( 'bp_groups_groupmeta_' . $group_id . '_' . $meta_key, $metas, 'bp' );
		}
	} else {
		$metas = $nxtdb->get_col( $nxtdb->prepare("SELECT meta_value FROM " . $bp->groups->table_name_groupmeta . " WHERE group_id = %d", $group_id) );
	}
...
add_action( 'nxtmu_delete_user',  'groups_remove_data_for_user' );
