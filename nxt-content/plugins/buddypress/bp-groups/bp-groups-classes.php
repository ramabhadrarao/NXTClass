...
Class BP_Groups_Group {
	...
	function populate() {
		...
		if ( $group = $nxtdb->get_row( $nxtdb->prepare( "SELECT g.*, gm.meta_value as last_activity, gm2.meta_value as total_member_count FROM {$bp->groups->table_name} g, {$bp->groups->table_name_groupmeta} gm, {$bp->groups->table_name_groupmeta} gm2 WHERE g.id = gm.group_id AND g.id = gm2.group_id AND gm.meta_key = 'last_activity' AND gm2.meta_key = 'total_member_count' AND g.id = %d", $this->id ) ) ) {
			...
			$admin_mods = $nxtdb->get_results( apply_filters( 'bp_group_admin_mods_user_join_filter', $nxtdb->prepare( "SELECT u.ID as user_id, u.user_login, u.user_email, u.user_nicename, m.is_admin, m.is_mod FROM {$nxtdb->users} u, {$bp->groups->table_name_members} m WHERE u.ID = m.user_id AND m.group_id = %d AND ( m.is_admin = 1 OR m.is_mod = 1 )", $this->id ) ) );
	...
	function group_exists( $slug, $table_name = false ) {
		...
		return $nxtdb->get_var( $nxtdb->prepare( "SELECT id FROM {$table_name} WHERE slug = %s", $slug ) );
	}
	...
	function get_invites( $user_id, $group_id ) {
		global $nxtdb, $bp;
		return $nxtdb->get_col( $nxtdb->prepare( "SELECT user_id FROM {$bp->groups->table_name_members} WHERE group_id = %d and is_confirmed = 0 AND inviter_id = %d", $group_id, $user_id ) );
	}
	...
	function filter_user_groups( $filter, $user_id = 0, $order = false, $limit = null, $page = null ) {
		...
		$filter = like_escape( $nxtdb->escape( $filter ) );

		if ( $limit && $page )
			$pag_sql = $nxtdb->prepare( " LIMIT %d, %d", intval( ( $page - 1 ) * $limit), intval( $limit ) );
		...
		$paged_groups = $nxtdb->get_results( $nxtdb->prepare( "SELECT id as group_id FROM {$bp->groups->table_name} WHERE ( name LIKE '{$filter}%%' OR description LIKE '{$filter}%%' ) AND id IN ({$gids}) {$pag_sql}" ) );
		$total_groups = $nxtdb->get_var( $nxtdb->prepare( "SELECT COUNT(id) FROM {$bp->groups->table_name} WHERE ( name LIKE '{$filter}%%' OR description LIKE '{$filter}%%' ) AND id IN ({$gids})" ) );
	...
	function search_groups( $filter, $limit = null, $page = null, $sort_by = false, $order = false ) {
		...
		if ( $sort_by && $order ) {
			$sort_by = $nxtdb->escape( $sort_by );
			$order = $nxtdb->escape( $order );
		...
		$paged_groups = $nxtdb->get_results( "SELECT id as group_id FROM {$bp->groups->table_name} WHERE ( name LIKE '%%$filter%%' OR description LIKE '%%$filter%%' ) {$hidden_sql} {$order_sql} {$pag_sql}" );
		$total_groups = $nxtdb->get_var( "SELECT COUNT(id) FROM {$bp->groups->table_name} WHERE ( name LIKE '%%$filter%%' OR description LIKE '%%$filter%%' ) {$hidden_sq}" );
	...
	function check_slug( $slug ) {
		...
		return $nxtdb->get_var( $nxtdb->prepare( "SELECT slug FROM {$bp->groups->table_name} WHERE slug = %s", $slug ) );
	}

	function get_slug( $group_id ) {
		global $nxtdb, $bp;

		return $nxtdb->get_var( $nxtdb->prepare( "SELECT slug FROM {$bp->groups->table_name} WHERE id = %d", $group_id ) );
	}

	function has_members( $group_id ) {
		global $nxtdb, $bp;

		$members = $nxtdb->get_var( $nxtdb->prepare( "SELECT COUNT(id) FROM {$bp->groups->table_name_members} WHERE group_id = %d", $group_id ) );
	...
	function has_membership_requests( $group_id ) {
		...
		return $nxtdb->get_var( $nxtdb->prepare( "SELECT COUNT(id) FROM {$bp->groups->table_name_members} WHERE group_id = %d AND is_confirmed = 0", $group_id ) );
	}
	...
	function get_membership_requests( $group_id, $limit = null, $page = null ) {
		...
		$paged_requests = $nxtdb->get_results( $nxtdb->prepare( "SELECT * FROM {$bp->groups->table_name_members} WHERE group_id = %d AND is_confirmed = 0 AND inviter_id = 0{$pag_sql}", $group_id ) );
		$total_requests = $nxtdb->get_var( $nxtdb->prepare( "SELECT COUNT(id) FROM {$bp->groups->table_name_members} WHERE group_id = %d AND is_confirmed = 0 AND inviter_id = 0", $group_id ) );
	...
	function get( $type = 'newest', $per_page = null, $page = null, $user_id = 0, $search_terms = false, $include = false, $populate_extras = true, $exclude = false, $show_hidden = false ) {
		...
		if ( !empty( $user_id ) )
			$sql['user'] = $nxtdb->prepare( " AND m.user_id = %d AND m.is_confirmed = 1 AND m.is_banned = 0", $user_id );

		if ( !empty( $include ) ) {
			if ( is_array( $include ) )
				$include = implode( ',', $include );

			$include = $nxtdb->escape( $include );
		...
		if ( $per_page && $page )
			$sql['pagination'] = $nxtdb->prepare( "LIMIT %d, %d", intval( ( $page - 1 ) * $per_page), intval( $per_page ) );

		/* Get paginated results */
		$paged_groups_sql = apply_filters( 'bp_groups_get_paged_groups_sql', join( ' ', (array)$sql ), $sql );
		$paged_groups     = $nxtdb->get_results( $paged_groups_sql );
		...
		$total_groups     = $nxtdb->get_var( $total_groups_sql );
		...
		if ( !empty( $populate_extras ) ) {
			...
			$group_ids = $nxtdb->escape( join( ',', (array)$group_ids ) );
	...
	function get_by_letter( $letter, $limit = null, $page = null, $populate_extras = true, $exclude = false ) {
		...
		if ( !is_super_admin() )
			$hidden_sql = $nxtdb->prepare( " AND status != 'hidden'");

		$letter = like_escape( $nxtdb->escape( $letter ) );

		if ( $limit && $page ) {
			$pag_sql = $nxtdb->prepare( " LIMIT %d, %d", intval( ( $page - 1 ) * $limit), intval( $limit ) );
			$total_groups = $nxtdb->get_var( $nxtdb->prepare( "SELECT COUNT(DISTINCT g.id) FROM {$bp->groups->table_name_groupmeta} gm1, {$bp->groups->table_name_groupmeta} gm2, {$bp->groups->table_name} g WHERE g.id = gm1.group_id AND g.id = gm2.group_id AND gm2.meta_key = 'last_activity' AND gm1.meta_key = 'total_member_count' AND g.name LIKE '$letter%%' {$hidden_sql} {$search_sql} {$exclude_sql}" ) );
		}

		$paged_groups = $nxtdb->get_results( $nxtdb->prepare( "SELECT g.*, gm1.meta_value as total_member_count, gm2.meta_value as last_activity FROM {$bp->groups->table_name_groupmeta} gm1, {$bp->groups->table_name_groupmeta} gm2, {$bp->groups->table_name} g WHERE g.id = gm1.group_id AND g.id = gm2.group_id AND gm2.meta_key = 'last_activity' AND gm1.meta_key = 'total_member_count' AND g.name LIKE '$letter%%' {$hidden_sql} {$search_sql} {$exclude_sql} ORDER BY g.name ASC {$pag_sql}"  ) );
	...
	function get_random( $limit = null, $page = null, $user_id = 0, $search_terms = false, $populate_extras = true, $exclude = false ) {
		...
		if ( $user_id ) {
			...
			$total_groups = $nxtdb->get_var( "SELECT COUNT(DISTINCT m.group_id) FROM {$bp->groups->table_name_members} m LEFT JOIN {$bp->groups->table_name_groupmeta} gm ON m.group_id = gm.group_id INNER JOIN {$bp->groups->table_name} g ON m.group_id = g.id WHERE gm.meta_key = 'last_activity'{$hidden_sql} {$search_sql} AND m.user_id = {$user_id} AND m.is_confirmed = 1 AND m.is_banned = 0 {$exclude_sql}" );
		} else {
			...
			$total_groups = $nxtdb->get_var( "SELECT COUNT(DISTINCT g.id) FROM {$bp->groups->table_name_groupmeta} gm INNER JOIN {$bp->groups->table_name} g ON gm.group_id = g.id WHERE gm.meta_key = 'last_activity'{$hidden_sql} {$search_sql} {$exclude_sql}" );
		}
	...
	function get_group_extras( &$paged_groups, &$group_ids, $type = false ) {
		...
		$user_status = $nxtdb->get_col( $nxtdb->prepare( "SELECT group_id FROM {$bp->groups->table_name_members} WHERE user_id = %d AND group_id IN ( {$group_ids} ) AND is_confirmed = 1 AND is_banned = 0", $bp->loggedin_user->id ) );
		...
		$user_banned = $nxtdb->get_col( $nxtdb->prepare( "SELECT group_id FROM {$bp->groups->table_name_members} WHERE is_banned = 1 AND user_id = %d AND group_id IN ( {$group_ids} )", $bp->loggedin_user->id ) );
		for ( $i = 0, $count = count( $paged_groups ); $i < $count; ++$i ) {
	...
	function delete_all_invites( $group_id ) {
		...
		return $nxtdb->query( $nxtdb->prepare( "DELETE FROM {$bp->groups->table_name_members} WHERE group_id = %d AND invite_sent = 1", $group_id ) );
	}
	...
	function get_total_group_count() {
		...
		return $nxtdb->get_var( $nxtdb->prepare( "SELECT COUNT(id) FROM {$bp->groups->table_name} {$hidden_sql}" ) );
	}

	function get_global_forum_topic_count( $type ) {
		global $bbdb, $nxtdb, $bp;
		...
		return $nxtdb->get_var( "SELECT COUNT(t.topic_id) FROM {$bbdb->topics} AS t, {$bp->groups->table_name} AS g LEFT JOIN {$bp->groups->table_name_groupmeta} AS gm ON g.id = gm.group_id WHERE (gm.meta_key = 'forum_id' AND gm.meta_value = t.forum_id) AND g.status = 'public' AND t.topic_status = '0' AND t.topic_sticky != '2' {$extra_sql} " );
	}

	function get_total_member_count( $group_id ) {
		global $nxtdb, $bp;

		return $nxtdb->get_var( $nxtdb->prepare( "SELECT COUNT(id) FROM {$bp->groups->table_name_members} WHERE group_id = %d AND is_confirmed = 1 AND is_banned = 0", $group_id ) );
	}
	...
	function get_global_topic_count( $status = 'public', $search_terms = false ) {
		...
		switch ( $status ) {
			...
			case 'hidden' :
				$status_sql = $nxtdb->prepare( "AND g.status = 'hidden'" );
				break;

			case 'private' :
				$status_sql = $nxtdb->prepare( "AND g.status = 'private'" );
				break;

			case 'public' :
			default :
				$status_sql = $nxtdb->prepare( "AND g.status = 'public'" );
				break;
		}
		...
		return $nxtdb->get_var( implode( ' ', $sql ) );
	}
}
...
Class BP_Groups_Member {
	...
	function populate() {
		...
		if ( $this->user_id && $this->group_id && !$this->id )
			$sql = $nxtdb->prepare( "SELECT * FROM {$bp->groups->table_name_members} WHERE user_id = %d AND group_id = %d", $this->user_id, $this->group_id );

		if ( $this->id )
			$sql = $nxtdb->prepare( "SELECT * FROM {$bp->groups->table_name_members} WHERE id = %d", $this->id );

		$member = $nxtdb->get_row($sql);
	...
	function save() {
		...
		if ( $this->id )
			$sql = $nxtdb->prepare( "UPDATE {$bp->groups->table_name_members} SET inviter_id = %d, is_admin = %d, is_mod = %d, is_banned = %d, user_title = %s, date_modified = %s, is_confirmed = %d, comments = %s, invite_sent = %d WHERE id = %d", $this->inviter_id, $this->is_admin, $this->is_mod, $this->is_banned, $this->user_title, $this->date_modified, $this->is_confirmed, $this->comments, $this->invite_sent, $this->id );
		else
			$sql = $nxtdb->prepare( "INSERT INTO {$bp->groups->table_name_members} ( user_id, group_id, inviter_id, is_admin, is_mod, is_banned, user_title, date_modified, is_confirmed, comments, invite_sent ) VALUES ( %d, %d, %d, %d, %d, %d, %s, %s, %d, %s, %d )", $this->user_id, $this->group_id, $this->inviter_id, $this->is_admin, $this->is_mod, $this->is_banned, $this->user_title, $this->date_modified, $this->is_confirmed, $this->comments, $this->invite_sent );
	...
	function remove() {
		...
		$sql = $nxtdb->prepare( "DELETE FROM {$bp->groups->table_name_members} WHERE user_id = %d AND group_id = %d", $this->user_id, $this->group_id );

		if ( !$result = $nxtdb->query( $sql ) )
			return false;
	...
	function delete( $user_id, $group_id ) {
		...
		return $nxtdb->query( $nxtdb->prepare( "DELETE FROM {$bp->groups->table_name_members} WHERE user_id = %d AND group_id = %d", $user_id, $group_id ) );
	}
	...
	function get_group_ids( $user_id, $limit = false, $page = false ) {
		...
		if ( $user_id != $bp->loggedin_user->id ) {
			$group_sql = $nxtdb->prepare( "SELECT DISTINCT m.group_id FROM {$bp->groups->table_name_members} m, {$bp->groups->table_name} g WHERE g.status != 'hidden' AND m.user_id = %d AND m.is_confirmed = 1 AND m.is_banned = 0{$pag_sql}", $user_id );
			$total_groups = $nxtdb->get_var( $nxtdb->prepare( "SELECT COUNT(DISTINCT m.group_id) FROM {$bp->groups->table_name_members} m, {$bp->groups->table_name} g WHERE g.status != 'hidden' AND m.user_id = %d AND m.is_confirmed = 1 AND m.is_banned = 0", $user_id ) );
		} else {
			$group_sql = $nxtdb->prepare( "SELECT DISTINCT group_id FROM {$bp->groups->table_name_members} WHERE user_id = %d AND is_confirmed = 1 AND is_banned = 0{$pag_sql}", $user_id );
			$total_groups = $nxtdb->get_var( $nxtdb->prepare( "SELECT COUNT(DISTINCT group_id) FROM {$bp->groups->table_name_members} WHERE user_id = %d AND is_confirmed = 1 AND is_banned = 0", $user_id ) );
		}

		$groups = $nxtdb->get_col( $group_sql );
	...
	function get_recently_joined( $user_id, $limit = false, $page = false, $filter = false ) {
		...
		$total_groups = $nxtdb->get_var( $nxtdb->prepare( "SELECT COUNT(DISTINCT m.group_id) FROM {$bp->groups->table_name_members} m, {$bp->groups->table_name} g WHERE m.group_id = g.id{$hidden_sql}{$filter_sql} AND m.user_id = %d AND m.is_banned = 0 AND m.is_confirmed = 1 ORDER BY m.date_modified DESC", $user_id ) );
	...
	function get_is_admin_of( $user_id, $limit = false, $page = false, $filter = false ) {
		...
		$total_groups = $nxtdb->get_var( $nxtdb->prepare( "SELECT COUNT(DISTINCT m.group_id) FROM {$bp->groups->table_name_members} m, {$bp->groups->table_name} g WHERE m.group_id = g.id{$hidden_sql}{$filter_sql} AND m.user_id = %d AND m.is_confirmed = 1 AND m.is_banned = 0 AND m.is_admin = 1 ORDER BY date_modified ASC", $user_id ) );
	...
	function get_is_mod_of( $user_id, $limit = false, $page = false, $filter = false ) {
		...
		$total_groups = $nxtdb->get_var( $nxtdb->prepare( "SELECT COUNT(DISTINCT m.group_id) FROM {$bp->groups->table_name_members} m, {$bp->groups->table_name} g WHERE m.group_id = g.id{$hidden_sql}{$filter_sql} AND m.user_id = %d AND m.is_confirmed = 1 AND m.is_banned = 0 AND m.is_mod = 1 ORDER BY date_modified ASC", $user_id ) );
	...
	function total_group_count( $user_id = 0 ) {
		...
		if ( $user_id != $bp->loggedin_user->id && !is_super_admin() ) {
			return $nxtdb->get_var( $nxtdb->prepare( "SELECT COUNT(DISTINCT m.group_id) FROM {$bp->groups->table_name_members} m, {$bp->groups->table_name} g WHERE m.group_id = g.id AND g.status != 'hidden' AND m.user_id = %d AND m.is_confirmed = 1 AND m.is_banned = 0", $user_id ) );
		} else {
			return $nxtdb->get_var( $nxtdb->prepare( "SELECT COUNT(DISTINCT m.group_id) FROM {$bp->groups->table_name_members} m, {$bp->groups->table_name} g WHERE m.group_id = g.id AND m.user_id = %d AND m.is_confirmed = 1 AND m.is_banned = 0", $user_id ) );
		}
	}
	...
	function get_invites( $user_id, $limit = false, $page = false, $exclude = false ) {
		...
		$pag_sql = ( !empty( $limit ) && !empty( $page ) ) ? $nxtdb->prepare( " LIMIT %d, %d", intval( ( $page - 1 ) * $limit), intval( $limit ) ) : '';

		$exclude_sql = !empty( $exclude ) ? $nxtdb->prepare( " AND g.id NOT IN (%s)", $exclude ) : '';
		...
		$total_groups = $nxtdb->get_var( $nxtdb->prepare( "SELECT COUNT(DISTINCT m.group_id) FROM {$bp->groups->table_name_members} m, {$bp->groups->table_name} g WHERE m.group_id = g.id AND m.is_confirmed = 0 AND m.inviter_id != 0 AND m.invite_sent = 1 AND m.user_id = %d {$exclude_sql} ORDER BY date_modified ASC", $user_id ) );
	...
	function check_has_invite( $user_id, $group_id, $type = 'sent' ) {
		...
		return $nxtdb->get_var( $nxtdb->prepare( $sql, $user_id, $group_id ) );
	}
	...
	function delete_invite( $user_id, $group_id ) {
		...
		return $nxtdb->query( $nxtdb->prepare( "DELETE FROM {$bp->groups->table_name_members} WHERE user_id = %d AND group_id = %d AND is_confirmed = 0 AND inviter_id != 0 AND invite_sent = 1", $user_id, $group_id ) );
	}
	...
	function delete_request( $user_id, $group_id ) {
		...
		if ( !$user_id )
 		...
 		return $nxtdb->query( $nxtdb->prepare( "DELETE FROM {$bp->groups->table_name_members} WHERE user_id = %d AND group_id = %d AND is_confirmed = 0 AND inviter_id = 0 AND invite_sent = 0", $user_id, $group_id ) );
	}
	...
	function check_is_admin( $user_id, $group_id ) {
		...
		return $nxtdb->query( $nxtdb->prepare( "SELECT id FROM {$bp->groups->table_name_members} WHERE user_id = %d AND group_id = %d AND is_admin = 1 AND is_banned = 0", $user_id, $group_id ) );
	}
	...
	function check_is_mod( $user_id, $group_id ) {
		...
		return $nxtdb->query( $nxtdb->prepare( "SELECT id FROM {$bp->groups->table_name_members} WHERE user_id = %d AND group_id = %d AND is_mod = 1 AND is_banned = 0", $user_id, $group_id ) );
	}
	...
	function check_is_member( $user_id, $group_id ) {
		...
		return $nxtdb->query( $nxtdb->prepare( "SELECT id FROM {$bp->groups->table_name_members} WHERE user_id = %d AND group_id = %d AND is_confirmed = 1 AND is_banned = 0", $user_id, $group_id ) );
	}
	...
	function check_is_banned( $user_id, $group_id ) {
		...
		return $nxtdb->get_var( $nxtdb->prepare( "SELECT is_banned FROM {$bp->groups->table_name_members} WHERE user_id = %d AND group_id = %d", $user_id, $group_id ) );
	}
	...
	function check_is_creator( $user_id, $group_id ) {
		...
		return $nxtdb->get_var( $nxtdb->prepare( "SELECT id FROM {$bp->groups->table_name} WHERE creator_id = %d AND id = %d", $user_id, $group_id ) );
	}
	...
	function check_for_membership_request( $user_id, $group_id ) {
		...
		return $nxtdb->query( $nxtdb->prepare( "SELECT id FROM {$bp->groups->table_name_members} WHERE user_id = %d AND group_id = %d AND is_confirmed = 0 AND is_banned = 0 AND inviter_id = 0", $user_id, $group_id ) );
	}
	...
	function get_random_groups( $user_id, $total_groups = 5 ) {
		...
		if ( bp_is_my_profile() ) {
			return $nxtdb->get_col( $nxtdb->prepare( "SELECT DISTINCT group_id FROM {$bp->groups->table_name_members} WHERE user_id = %d AND is_confirmed = 1 AND is_banned = 0 ORDER BY rand() LIMIT $total_groups", $user_id ) );
		} else {
			return $nxtdb->get_col( $nxtdb->prepare( "SELECT DISTINCT m.group_id FROM {$bp->groups->table_name_members} m, {$bp->groups->table_name} g WHERE m.group_id = g.id AND g.status != 'hidden' AND m.user_id = %d AND m.is_confirmed = 1 AND m.is_banned = 0 ORDER BY rand() LIMIT $total_groups", $user_id ) );
		}
	}
	...
	function get_group_member_ids( $group_id ) {
		...
		return $nxtdb->get_col( $nxtdb->prepare( "SELECT user_id FROM {$bp->groups->table_name_members} WHERE group_id = %d AND is_confirmed = 1 AND is_banned = 0", $group_id ) );
	}

	function get_group_administrator_ids( $group_id ) {
		global $bp, $nxtdb;

		return $nxtdb->get_results( $nxtdb->prepare( "SELECT user_id, date_modified FROM {$bp->groups->table_name_members} WHERE group_id = %d AND is_admin = 1 AND is_banned = 0", $group_id ) );
	}

	function get_group_moderator_ids( $group_id ) {
		global $bp, $nxtdb;

		return $nxtdb->get_results( $nxtdb->prepare( "SELECT user_id, date_modified FROM {$bp->groups->table_name_members} WHERE group_id = %d AND is_mod = 1 AND is_banned = 0", $group_id ) );
	}

	function get_all_membership_request_user_ids( $group_id ) {
		global $bp, $nxtdb;

		return $nxtdb->get_col( $nxtdb->prepare( "SELECT user_id FROM {$bp->groups->table_name_members} WHERE group_id = %d AND is_confirmed = 0 AND inviter_id = 0", $group_id ) );
	}
	...
	function get_all_for_group( $group_id, $limit = false, $page = false, $exclude_admins_mods = true, $exclude_banned = true, $exclude = false ) {
		...
		if ( $exclude_admins_mods )
			$exclude_admins_sql = $nxtdb->prepare( "AND is_admin = 0 AND is_mod = 0" );

		$banned_sql = '';
		if ( $exclude_banned )
			$banned_sql = $nxtdb->prepare( " AND is_banned = 0" );

		$exclude_sql = '';
		if ( $exclude )
			$exclude_sql = $nxtdb->prepare( " AND m.user_id NOT IN ({$exclude})" );

		if ( bp_is_active( 'xprofile' ) )
			$members = $nxtdb->get_results( apply_filters( 'bp_group_members_user_join_filter', $nxtdb->prepare( "SELECT m.user_id, m.date_modified, m.is_banned, u.user_login, u.user_nicename, u.user_email, pd.value as display_name FROM {$bp->groups->table_name_members} m, {$nxtdb->users} u, {$bp->profile->table_name_data} pd WHERE u.ID = m.user_id AND u.ID = pd.user_id AND pd.field_id = 1 AND group_id = %d AND is_confirmed = 1 {$banned_sql} {$exclude_admins_sql} {$exclude_sql} ORDER BY m.date_modified DESC {$pag_sql}", $group_id ) ) );
		else
			$members = $nxtdb->get_results( apply_filters( 'bp_group_members_user_join_filter', $nxtdb->prepare( "SELECT m.user_id, m.date_modified, m.is_banned, u.user_login, u.user_nicename, u.user_email, u.display_name FROM {$bp->groups->table_name_members} m, {$nxtdb->users} u WHERE u.ID = m.user_id AND group_id = %d AND is_confirmed = 1 {$banned_sql} {$exclude_admins_sql} {$exclude_sql} ORDER BY m.date_modified DESC {$pag_sql}", $group_id ) ) );
		...
		if ( empty( $pag_sql ) )
			...
			$total_member_count = $nxtdb->get_var( apply_filters( 'bp_group_members_count_user_join_filter', $nxtdb->prepare( "SELECT COUNT(user_id) FROM {$bp->groups->table_name_members} WHERE group_id = %d AND is_confirmed = 1 {$banned_sql} {$exclude_admins_sql} {$exclude_sql}", $group_id ) ) );
		...
		if ( bp_is_active( 'friends' ) ) {
			$friend_status = $nxtdb->get_results( $nxtdb->prepare( "SELECT initiator_user_id, friend_user_id, is_confirmed FROM {$bp->friends->table_name} WHERE (initiator_user_id = %d AND friend_user_id IN ( {$user_ids} ) ) OR (initiator_user_id IN ( {$user_ids} ) AND friend_user_id = %d )", $bp->loggedin_user->id, $bp->loggedin_user->id ) );
	...
	function delete_all( $group_id ) {
		...
		return $nxtdb->query( $nxtdb->prepare( "DELETE FROM {$bp->groups->table_name_members} WHERE group_id = %d", $group_id ) );
	}
	...
	function delete_all_for_user( $user_id ) {
		...
		return $nxtdb->query( $nxtdb->prepare( "DELETE FROM {$bp->groups->table_name_members} WHERE user_id = %d", $user_id ) );
	}
}
