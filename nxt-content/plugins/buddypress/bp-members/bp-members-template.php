...
function bp_member_name() {
	 ...
	 * empty, we try to get an alternative name from the nxt users table, in the following order
	...
	add_filter( 'bp_get_member_name', 'nxt_filter_kses' );
...
function bp_member_random_profile_data() {
	...
	if ( bp_is_active( 'xprofile' ) ) { ?>
		<?php $random_data = xprofile_get_random_profile_data( $members_template->member->id, true ); ?>
			<strong><?php echo nxt_filter_kses( $random_data[0]->name ) ?></strong>
			<?php echo nxt_filter_kses( $random_data[0]->value ) ?>
	<?php }
}
...
function bp_activation_page() {
	...
	function bp_get_activation_page() {
		...
		if ( bp_has_custom_activation_page() )
			...
			$page = trailingslashit( bp_get_root_domain() ) . 'nxt-activate.php';
