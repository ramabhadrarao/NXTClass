...
function bp_core_screen_activation() {
	...
	if ( isset( $_GET['key'] ) ) {
		...
		if ( is_multisite() )
			$hashed_key = nxt_hash( $_GET['key'] );
		else
			$hashed_key = nxt_hash( $user );
...
function bp_core_validate_user_signup( $user_name, $user_email ) {
	...
	// Apply nxtMU legacy filter
	$result = apply_filters( 'nxtmu_validate_user_signup', $result );
...
function bp_core_signup_blog( $blog_domain, $blog_path, $blog_title, $user_name, $user_email, $usermeta ) {
	if ( !is_multisite() || !function_exists( 'nxtmu_signup_blog' ) )
		return false;

	return apply_filters( 'bp_core_signup_blog', nxtmu_signup_blog( $blog_domain, $blog_path, $blog_title, $user_name, $user_email, $usermeta ) );
}
...
function bp_core_signup_disable_inactive( $auth_obj, $username ) {
	...
	$user_status = (int) $nxtdb->get_var( $nxtdb->prepare( "SELECT user_status FROM $nxtdb->users WHERE ID = %d", $user_id ) );

	if ( 2 == $user_status )
		return new nxt_Error( 'bp_account_not_activated', __( '<strong>ERROR</strong>: Your account has not been activated. Check your email for the activation link.', 'buddypress' ) );
	else
		return $auth_obj;
}
add_filter( 'authenticate', 'bp_core_signup_disable_inactive', 30, 2 );

// Kill the nxt-signup.php if custom registration signup templates are present
function bp_core_nxtsignup_redirect() {
	$action = !empty( $_GET['action'] ) ? $_GET['action'] : '';

	if ( is_admin() || is_network_admin() )
		return;
	
	// Not at the nxt core signup page and action is not register
	if ( false === strpos( $_SERVER['SCRIPT_NAME'], 'nxt-signup.php' ) && ( 'register' != $action ) )
		return;
...
add_action( 'bp_init', 'bp_core_nxtsignup_redirect' );
?>