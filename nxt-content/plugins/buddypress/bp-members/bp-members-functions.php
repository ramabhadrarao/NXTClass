...
/**
 * Define the slugs used for BuddyPress pages, based on the slugs of the nxt pages used.
 * These can be overridden manually by defining these slugs in nxt-config.php.
 *
...
function bp_core_get_user_domain( $user_id, $user_nicename = false, $user_login = false ) {
	...
	if ( !$domain = nxt_cache_get( 'bp_user_domain_' . $user_id, 'bp' ) ) {
		...
		// Cache the link
		if ( !empty( $domain ) )
			nxt_cache_set( 'bp_user_domain_' . $user_id, $domain, 'bp' );
	}
...
/**
 * Fetch everything in the nxt_users table for a user, without any usermeta.
 *
 * @package BuddyPress Core
 * @param user_id The ID of the user.
...
function bp_core_get_core_userdata( $user_id ) {
	...
	if ( !$userdata = nxt_cache_get( 'bp_core_userdata_' . $user_id, 'bp' ) ) {
		$userdata = BP_Core_User::get_core_userdata( $user_id );
		nxt_cache_set( 'bp_core_userdata_' . $user_id, $userdata, 'bp' );
	}
...
function bp_core_get_userid( $username ) {
	...
	return apply_filters( 'bp_core_get_userid', $nxtdb->get_var( $nxtdb->prepare( "SELECT ID FROM $nxtdb->users WHERE user_login = %s", $username ) ) );
}
...
function bp_core_get_userid_from_nicename( $user_nicename ) {
	...
	return apply_filters( 'bp_core_get_userid_from_nicename', $nxtdb->get_var( $nxtdb->prepare( "SELECT ID FROM $nxtdb->users WHERE user_nicename = %s", $user_nicename ) ) );
}
 ...
 * @uses get_userdata() NXTClass function to fetch the userdata for a user ID
 * @return false on no match
...
function bp_core_get_username( $user_id, $user_nicename = false, $user_login = false ) {
	...
	if ( !$username = nxt_cache_get( 'bp_user_username_' . $user_id, 'bp' ) ) {
	...
	if ( ( true == $update_cache ) && !empty( $username ) )
		nxt_cache_set( 'bp_user_username_' . $user_id, $username, 'bp' );
...
function bp_members_get_user_nicename( $user_id ) {
	...
	if ( !$user_nicename = nxt_cache_get( 'bp_members_user_nicename_' . $user_id, 'bp' ) ) {
		$update_cache = true;
	...
	if ( true == $update_cache && !empty( $user_nicename ) )
		nxt_cache_set( 'bp_members_user_nicename_' . $user_id, $user_nicename, 'bp' );
...
function bp_core_get_user_email( $uid ) {
	if ( !$email = nxt_cache_get( 'bp_user_email_' . $uid, 'bp' ) ) {
		// User exists
		...
		nxt_cache_set( 'bp_user_email_' . $uid, $email, 'bp' );
	}
 ...
 * Otherwise, it will fall back to the normal nxt display_name, or user_nicename, depending on what has been set.
 *
 * @package BuddyPress Core
 * @global object $bp Global BuddyPress settings object
 * @uses nxt_cache_get() Will try and fetch the value from the cache, rather than querying the DB again.
 * @uses get_userdata() Fetches the nxt userdata for a specific user.
 * @uses xprofile_set_field_data() Will update the field data for a user based on field name and user id.
 * @uses nxt_cache_set() Adds a value to the cache.
...
function bp_core_get_user_displayname( $user_id_or_username ) {
	...
	if ( !$fullname = nxt_cache_get( 'bp_user_fullname_' . $user_id, 'bp' ) ) {
		...
		if ( !empty( $fullname ) )
			nxt_cache_set( 'bp_user_fullname_' . $user_id, $fullname, 'bp' );
	}
 ...
 * @uses get_user_by() NXTClass function to get userdata via an email address
...
function bp_core_process_spammer_status( $user_id, $status ) {
	...
	// When marking as spam in the Dashboard, these actions are handled by NXTClass
	if ( !is_admin() ) {
		...
		$nxtdb->update( $nxtdb->users, array( 'user_status' => $is_spam ), array( 'ID' => $user_id ) );
				
		// Call multisite actions in single site mode for good measure
		if ( !is_multisite() ) {
			$nxt_action = $is_spam ? 'make_spam_user' : 'make_ham_user';
			do_action( $nxt_action, bp_displayed_user_id() );
		}
	}
...
/**
 * Hook to nxt's make_spam_user and run our custom BP spam functions
 *
 * @since 1.5.5
 * 
...
/**
 * Hook to nxt's make_ham_user and run our custom BP spam functions
 *
 * @since 1.5.5
 * 
 ...
 * @package BuddyPress Core
 * @global object $bp Global BuddyPress settings object
 * @global $nxtdb NXTClass user data for the current logged in user.
...
function bp_core_get_all_posts_for_user( $user_id = 0 ) {
	...
	return apply_filters( 'bp_core_get_all_posts_for_user', $nxtdb->get_col( $nxtdb->prepare( "SELECT ID FROM $nxtdb->posts WHERE post_author = %d AND post_status = 'publish' AND post_type = 'post'", $user_id ) ) );
}
...
/**
 * Strips spaces from usernames that are created using add_user() and nxt_insert_user()
 *
 * @package BuddyPress Core
 */
 ...
 *
 * @package BuddyPress Core
 * @param $auth_obj The nxt authorization object
...
function bp_core_boot_spammer( $auth_obj, $username ) {
	...
	if ( ( is_multisite() && (int)$user->spam ) || 1 == (int)$user->user_status )
		return new nxt_Error( 'invalid_username', __( '<strong>ERROR</strong>: Your account has been marked as a spammer.', 'buddypress' ) );
	else
		return $auth_obj;
}
...
add_action( 'nxtmu_delete_user',  'bp_core_remove_data' );
