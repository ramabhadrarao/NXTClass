 ...
 * filter hooks in NXTClass and BuddyPress to change core functionality.
 *
 * The first function, bp_dtheme_setup(), sets up the theme by registering support
 * for various features in NXTClass, such as post thumbnails and navigation menus, and
 * for BuddyPress, action buttons and javascript localisation.
 *
 * When using a child theme (see http://codex.nxtclass.org/Theme_Development, http://codex.nxtclass.org/Child_Themes
 ...
 *
 * For more information on hooks, actions, and filters, see http://codex.nxtclass.org/Plugin_API.
 *
 * @package BuddyPress
...
// If BuddyPress is not activated, switch back to the default nxt theme
...
add_action( 'nxt_enqueue_scripts', 'bp_dtheme_enqueue_scripts' );
endif;
...
add_action( 'nxt_enqueue_scripts', 'bp_dtheme_enqueue_styles' );
endif;
 ...
 *
 * Used as a callback by nxt_list_comments() for displaying the comments.
 *
 * @param mixed $comment Comment record from database
 * @param array $args Arguments from nxt_list_comments() call
 * @param int $depth Comment nesting level
 * @see nxt_list_comments()
 * @since 1.2
 */
...
/**
 * nxt_nav_menu() callback from the main navigation in header.php
 *
 * Used when the custom menus haven't been configured.
 *
 * @global object $bp Global BuddyPress settings object
 * @param array Menu arguments from nxt_nav_menu()
 * @see nxt_nav_menu()
 * @since 1.5
 */
...
/**
 * Get our nxt_nav_menu() fallback, bp_dtheme_main_nav(), to show a home link.
 *
 * @param array $args Default values for nxt_page_menu()
 * @see nxt_page_menu()
 * @since 1.5
 */
...
add_filter( 'nxt_page_menu_args', 'bp_dtheme_page_menu_args' );
endif;
...
function bp_dtheme_comment_form( $default_labels ) {
	...
	$new_labels = array(
		...
		'logged_in_as'   => '',
		'must_log_in'    => '<p class="alert">' . sprintf( __( 'You must be <a href="%1$s">logged in</a> to post a comment.', 'buddypress' ), nxt_login_url( get_permalink() ) )	. '</p>',
 ...
 * to body.js.
 *
 * This technique is borrowed from NXTClass, nxt-admin/admin-header.php.
 *
 * @package BuddyPress
 * @since 1.5.1
