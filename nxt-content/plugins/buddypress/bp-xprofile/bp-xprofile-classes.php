...
Class BP_XProfile_Group {
	...
	function populate( $id ) {
		...
		$sql = $nxtdb->prepare( "SELECT * FROM {$bp->profile->table_name_groups} WHERE id = %d", $id );

		if ( !$group = $nxtdb->get_row( $sql ) )
			return false;
	...
	function save() {
		...
		if ( $this->id )
			$sql = $nxtdb->prepare( "UPDATE {$bp->profile->table_name_groups} SET name = %s, description = %s WHERE id = %d", $this->name, $this->description, $this->id );
		else
			$sql = $nxtdb->prepare( "INSERT INTO {$bp->profile->table_name_groups} (name, description, can_delete) VALUES (%s, %s, 1)", $this->name, $this->description );

		if ( is_nxt_error( $nxtdb->query( $sql ) ) )
			return false;
	...
	function delete() {
		...
		/* Delete field group */
		if ( !$nxtdb->query( $nxtdb->prepare( "DELETE FROM {$bp->profile->table_name_groups} WHERE id = %d", $this->id ) ) ) {
			return false;
		} else {
	...
	function get( $args = '' ) {
		...
		if ( $profile_group_id )
			$where_sql = $nxtdb->prepare( 'WHERE g.id = %d', $profile_group_id );
		elseif ( $exclude_groups )
			$where_sql = $nxtdb->prepare( "WHERE g.id NOT IN ({$exclude_groups})");

		if ( $hide_empty_groups )
			$groups = $nxtdb->get_results( $nxtdb->prepare( "SELECT DISTINCT g.* FROM {$bp->profile->table_name_groups} g INNER JOIN {$bp->profile->table_name_fields} f ON g.id = f.group_id {$where_sql} ORDER BY g.group_order ASC" ) );
		else
			$groups = $nxtdb->get_results( $nxtdb->prepare( "SELECT DISTINCT g.* FROM {$bp->profile->table_name_groups} g {$where_sql} ORDER BY g.group_order ASC" ) );
		...
		$exclude_fields_sql = '';
		if ( $exclude_fields )
			$exclude_fields_sql = $nxtdb->prepare( "AND id NOT IN ({$exclude_fields})" );

		// Fetch the fields
		$fields = $nxtdb->get_results( $nxtdb->prepare( "SELECT id, name, description, type, group_id, is_required FROM {$bp->profile->table_name_fields} WHERE group_id IN ( {$group_ids} ) AND parent_id = 0 {$exclude_fields_sql} ORDER BY field_order" ) );
		...
		if ( $fetch_field_data ) {
			...
			if ( !empty( $field_ids ) )
				$field_data = $nxtdb->get_results( $nxtdb->prepare( "SELECT field_id, value FROM {$bp->profile->table_name_data} WHERE field_id IN ( {$field_ids_sql} ) AND user_id = %d", $user_id ) );
			...
			if ( !empty( $field_data ) && !is_nxt_error( $field_data ) ) {
	...
	function update_position( $field_group_id, $position ) {
		...
		return $nxtdb->query( $nxtdb->prepare( "UPDATE {$bp->profile->table_name_groups} SET group_order = %d WHERE id = %d", $position, $field_group_id ) );
	}
...
Class BP_XProfile_Field {
	...
	function populate( $id, $user_id, $get_data ) {
		global $nxtdb, $userdata, $bp;

		$user_id = 0;
		if ( is_null( $user_id ) )
			$user_id = $userdata->ID;

		$sql = $nxtdb->prepare( "SELECT * FROM {$bp->profile->table_name_fields} WHERE id = %d", $id );

		if ( $field = $nxtdb->get_row( $sql ) ) {
			$this->id               = $field->id;
	...
	function save() {
		...
		if ( $this->id != null )
			$sql = $nxtdb->prepare( "UPDATE {$bp->profile->table_name_fields} SET group_id = %d, parent_id = 0, type = %s, name = %s, description = %s, is_required = %d, order_by = %s, field_order = %d WHERE id = %d", $this->group_id, $this->type, $this->name, $this->description, $this->is_required, $this->order_by, $this->field_order, $this->id );
		else
			$sql = $nxtdb->prepare( "INSERT INTO {$bp->profile->table_name_fields} (group_id, parent_id, type, name, description, is_required, order_by, field_order ) VALUES (%d, %d, %s, %s, %s, %d, %s, %d )", $this->group_id, $this->parent_id, $this->type, $this->name, $this->description, $this->is_required, $this->order_by, $this->field_order );
		 ...
		 */
		if ( $nxtdb->query( $sql ) !== null ) {
			if ( $this->id )
				$field_id = $this->id;
			else
				$field_id = $nxtdb->insert_id;
			...
			if ( 'radio' == $this->type || 'selectbox' == $this->type || 'checkbox' == $this->type || 'multiselectbox' == $this->type ) {
				...
				if ( $options ) {
					foreach ( (array)$options as $option_key => $option_value ) {
						...
						if ( '' != $option_value ) {
							if ( !$nxtdb->query( $nxtdb->prepare( "INSERT INTO {$bp->profile->table_name_fields} (group_id, parent_id, type, name, description, is_required, option_order, is_default_option) VALUES (%d, %d, 'option', %s, '', 0, %d, %d)", $this->group_id, $parent_id, $option_value, $counter, $is_default ) ) )
								return false;
						}
	...
	function get_children( $for_editing = false ) {
		...
		$sql = $nxtdb->prepare( "SELECT * FROM {$bp->profile->table_name_fields} WHERE parent_id = %d AND group_id = %d $sort_sql", $parent_id, $this->group_id );

		if ( !$children = $nxtdb->get_results( $sql ) )
			return false;
	...
	function delete_children() {
		...
		$sql = $nxtdb->prepare( "DELETE FROM {$bp->profile->table_name_fields} WHERE parent_id = %d", $this->id );
	...
	function get_type( $field_id ) {
		...
		if ( $field_id ) {
			$sql = $nxtdb->prepare( "SELECT type FROM {$bp->profile->table_name_fields} WHERE id = %d", $field_id );

			if ( !$field_type = $nxtdb->get_var( $sql ) )
				return false;
	...
	function delete_for_group( $group_id ) {
		...
		if ( $group_id ) {
			$sql = $nxtdb->prepare( "DELETE FROM {$bp->profile->table_name_fields} WHERE group_id = %d", $group_id );

			if ( $nxtdb->get_var( $sql ) === false )
				return false;
	...
	function get_id_from_name( $field_name ) {
		...
		return $nxtdb->get_var( $nxtdb->prepare( "SELECT id FROM {$bp->profile->table_name_fields} WHERE name = %s", $field_name ) );
	}
	...
	function update_position( $field_id, $position, $field_group_id ) {
		...
		if ( $parent = $nxtdb->query( $nxtdb->prepare( "UPDATE {$bp->profile->table_name_fields} SET field_order = %d, group_id = %d WHERE id = %d", $position, $field_group_id, $field_id ) ) ) {;
			/* Update any children of this $field_id */
			$children = $nxtdb->query( $nxtdb->prepare( "UPDATE {$bp->profile->table_name_fields} SET group_id = %d WHERE parent_id = %d", $field_group_id, $field_id ) );
...
<?php	} ?>
			...
			<?php nxt_nonce_field( 'xprofile_delete_option' ); ?>
...
Class BP_XProfile_ProfileData {
	...
	function populate( $field_id, $user_id )  {
		...
		$sql = $nxtdb->prepare( "SELECT * FROM {$bp->profile->table_name_data} WHERE field_id = %d AND user_id = %d", $field_id, $user_id );

		if ( $profiledata = $nxtdb->get_row( $sql ) ) {
	...
	function exists() {
		...
		$retval = $nxtdb->get_row( $nxtdb->prepare( "SELECT id FROM {$bp->profile->table_name_data} WHERE user_id = %d AND field_id = %d", $this->user_id, $this->field_id ) );
	...
	function is_valid_field() {
		...
		$retval = $nxtdb->get_row( $nxtdb->prepare( "SELECT id FROM {$bp->profile->table_name_fields} WHERE id = %d", $this->field_id ) );
	...
	function save() {
		...
		if ( $this->is_valid_field() ) {
			if ( $this->exists() && !empty( $this->value ) && strlen( trim( $this->value ) ) ) {
				$result   = $nxtdb->query( $nxtdb->prepare( "UPDATE {$bp->profile->table_name_data} SET value = %s, last_updated = %s WHERE user_id = %d AND field_id = %d", $this->value, $this->last_updated, $this->user_id, $this->field_id ) );
				...
				$result   = $nxtdb->query( $nxtdb->prepare("INSERT INTO {$bp->profile->table_name_data} (user_id, field_id, value, last_updated) VALUES (%d, %d, %s, %s)", $this->user_id, $this->field_id, $this->value, $this->last_updated ) );
	...
	function delete() {
		...
		if ( !$nxtdb->query( $nxtdb->prepare( "DELETE FROM {$bp->profile->table_name_data} WHERE field_id = %d AND user_id = %d", $this->field_id, $this->user_id ) ) )
			return false;
	...
	function get_all_for_user( $user_id ) {
		...
		$results      = $nxtdb->get_results( $nxtdb->prepare( "SELECT g.id as field_group_id, g.name as field_group_name, f.id as field_id, f.name as field_name, f.type as field_type, d.value as field_data, u.user_login, u.user_nicename, u.user_email FROM {$bp->profile->table_name_groups} g LEFT JOIN {$bp->profile->table_name_fields} f ON g.id = f.group_id INNER JOIN {$bp->profile->table_name_data} d ON f.id = d.field_id LEFT JOIN {$nxtdb->users} u ON d.user_id = u.ID WHERE d.user_id = %d AND d.value != ''", $user_id ) );
		$profile_data = array();
	...
	function get_value_byid( $field_id, $user_ids = null ) {
		...
		if ( is_array( $user_ids ) ) {
			...
			$data = $nxtdb->get_results( $nxtdb->prepare( "SELECT user_id, value FROM {$bp->profile->table_name_data} WHERE field_id = %d AND user_id IN ({$user_ids})", $field_id ) );
		} else {
			$data = $nxtdb->get_var( $nxtdb->prepare( "SELECT value FROM {$bp->profile->table_name_data} WHERE field_id = %d AND user_id = %d", $field_id, $user_ids ) );
		}
	...
	function get_value_byfieldname( $fields, $user_id = null ) {
		...
		if ( is_array( $fields ) ) {
			for ( $i = 0, $count = count( $fields ); $i < $count; ++$i ) {
				if ( $i == 0 )
					$field_sql .= $nxtdb->prepare( "AND ( f.name = %s ", $fields[$i] );
				else
					$field_sql .= $nxtdb->prepare( "OR f.name = %s ", $fields[$i] );
			}

			$field_sql .= ')';
		} else {
			$field_sql .= $nxtdb->prepare( "AND f.name = %s", $fields );
		}

		$sql = $nxtdb->prepare( "SELECT d.value, f.name FROM {$bp->profile->table_name_data} d, {$bp->profile->table_name_fields} f WHERE d.field_id = f.id AND d.user_id = %d AND f.parent_id = 0 $field_sql", $user_id );

		if ( !$values = $nxtdb->get_results( $sql ) )
			return false;
	...
	function delete_for_field( $field_id ) {
		...
		if ( !$nxtdb->query( $nxtdb->prepare( "DELETE FROM {$bp->profile->table_name_data} WHERE field_id = %d", $field_id ) ) )
			return false;
	...
	function get_last_updated( $user_id ) {
		...
		$last_updated = $nxtdb->get_var( $nxtdb->prepare( "SELECT last_updated FROM {$bp->profile->table_name_data} WHERE user_id = %d ORDER BY last_updated LIMIT 1", $user_id ) );
	...
	function delete_data_for_user( $user_id ) {
		...
		return $nxtdb->query( $nxtdb->prepare( "DELETE FROM {$bp->profile->table_name_data} WHERE user_id = %d", $user_id ) );
	}
	...
	function get_random( $user_id, $exclude_fullname ) {
		...
		if ( $exclude_fullname )
			$exclude_sql = $nxtdb->prepare( " AND pf.id != 1" );

		return $nxtdb->get_results( $nxtdb->prepare( "SELECT pf.type, pf.name, pd.value FROM {$bp->profile->table_name_data} pd INNER JOIN {$bp->profile->table_name_fields} pf ON pd.field_id = pf.id AND pd.user_id = %d {$exclude_sql} ORDER BY RAND() LIMIT 1", $user_id ) );
	}
