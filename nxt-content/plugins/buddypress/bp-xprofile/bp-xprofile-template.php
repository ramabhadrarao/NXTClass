...
function bp_profile_group_tabs() {
	...
	if ( !$groups = nxt_cache_get( 'xprofile_groups_inc_empty', 'bp' ) ) {
		$groups = BP_XProfile_Group::get( array( 'fetch_fields' => true ) );
		nxt_cache_set( 'xprofile_groups_inc_empty', $groups, 'bp' );
	}
...
function bp_profile_group_name( $deprecated = true ) {
	...
	function bp_get_profile_group_name() {
		...
		if ( !$group = nxt_cache_get( 'xprofile_group_' . $group_id, 'bp' ) ) {
			$group = new BP_XProfile_Group($group_id);
			nxt_cache_set( 'xprofile_group_' . $group_id, $group, 'bp' );
		}
...
function bp_avatar_delete_link() {
	...
	function bp_get_avatar_delete_link() {
		...
		return apply_filters( 'bp_get_avatar_delete_link', nxt_nonce_url( $bp->displayed_user->domain . $bp->profile->slug . '/change-avatar/delete-avatar/', 'bp_delete_avatar_link' ) );
	}
