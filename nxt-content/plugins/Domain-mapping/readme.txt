=== NXTClass MU Domain Mapping ===
Contributors: donncha, nxtmuguru, automattic
Tags: nxtclassmu, domain-mapping, multisite
Tested up to: 3.3
Stable tag: 0.5.4.2
Requires at least: 2.9.2

Map any blog/site on a NXTClassMU or NXTClass 3.X network to an external domain.

== Description ==
This plugin allows users of a NXTClass MU site or NXTClass 3.0 network to map their blog/site to another domain.

It requires manual installation as one file must be copied to nxt-content/. When upgrading the plugin, remember to update domain_mapping.php and sunrise.php. Full instructions are on the Installation page and are quite easy to follow. You should also read [this page](http://ottopress.com/2010/nxtclass-3-0-multisite-domain-mapping-tutorial/) too.
...
* For localization: place translation files (ie. nxtclass-mu-domain-mapping-xx_XX.mo) in the directory nxt-content/plugins/nxtclass-mu-domain-mapping/languages. You will probably have to create that directory yourself.

== Upgrade Notice ==

= 0.5.4.2 =
NXTClass 3.3 compatibility, bugfixes
Set $current_site->site_name

= 0.5.4.1 =
NXTClass 3.2 compatibility, bugfixes

= 0.5.4 =
NXTClass 3.1 compatibility, localization, IDN warnings, settings page updates, support for SSL and lots of bugfixes

== Changelog ==

= 0.5.4 =
* NXTClass 3.1 compatibility.
* Localization fixes.
...
* Added check for COOKIE_DOMAIN, props Scott, http://ocaoimh.ie/nxtclass-mu-domain-mapping-05/comment-page-1/#comment-671821
...
* Bug fixes: cache domain maps correctly, blogid, not blog_id in $nxtdb.
...
== Installation ==
1. Install the plugin in the usual way into the regular NXTClass plugins folder. Network activate the plugin.
2. Move sunrise.php into nxt-content/. If there is a sunrise.php there already, you'll just have to merge them as best you can.
3. Edit nxt-config.php and uncomment or add the SUNRISE definition line. If it does not exist please ensure it's on the line above the last "require_once" command.
    `define( 'SUNRISE', 'on' );`
...
5. Make sure the default Apache virtual host points at your NXTClass MU site or NXTClass 3.0 network so it will handle unknown domains correctly. On some hosts you may be required to get a dedicated IP address. A quick check: in a web broswer, type in the IP address of your install. If you are using CPanel, use the Park a Domain menu to set the mapped domain to your main installtion.
6. Do not define COOKIE_DOMAIN in your nxt-config.php as it conflicts with logins on your mapped domains.

Illustrated installation instructions can be found [here](http://ottopress.com/2010/nxtclass-3-0-multisite-domain-mapping-tutorial/) but you can ignore the instructions to place domain_mapping.php in mu-plugins. Thanks Otto.
...
* Go to "Edit DNS Zone" and select the domain of your nxtMU installation and click "Edit".
...
function dm_extended_handle_actions_add($domain) {
	...
	if ($dmip != $urip) {
		nxt_redirect( '?page=domainmapping&updated=dnslookup' );
		exit;
	}
