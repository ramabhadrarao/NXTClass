<?php
/*
Plugin Name: NXTClass MU Domain Mapping
Plugin URI: http://ocaoimh.ie/nxtclass-mu-domain-mapping/
Description: Map any blog on a NXTClass website to another domain.
Version: 0.5.4.2
Author: Donncha O Caoimh
Author URI: http://ocaoimh.ie/
*/
/*  Copyright Donncha O Caoimh (http://ocaoimh.ie/)
    With contributions by Ron Rennick(http://nxtmututorials.com/), Greg Sidberry and others.
...
function dm_text_domain() {
	load_plugin_textdomain( 'nxtclass-mu-domain-mapping', basename( dirname( __FILE__ ) )  . 'languages', 'nxtclass-mu-domain-mapping/languages' );
}
add_action( 'init', 'dm_text_domain' );

function domain_mapping_warning() {
	echo "<div id='domain-mapping-warning' class='updated fade'><p><strong>".__( 'Domain Mapping Disabled.', 'nxtclass-mu-domain-mapping' )."</strong> ".sprintf(__('You must <a href="%1$s">create a network</a> for it to work.', 'nxtclass-mu-domain-mapping' ), "http://codex.nxtclass.org/Create_A_Network")."</p></div>";
}

function dm_add_pages() {
	global $current_site, $nxtdb, $nxt_db_version, $nxt_version;

	if ( !isset( $current_site ) && $nxt_db_version >= 15260 ) { // nxt 3.0 network hasn't been configured
		add_action('admin_notices', 'domain_mapping_warning');
		return false;
	}
	if ( $current_site->path != "/" ) {
		nxt_die( __( "The domain mapping plugin only works if the site is installed in /. This is a limitation of how virtual servers work and is very difficult to work around.", 'nxtclass-mu-domain-mapping' ) );
	}

	if ( get_site_option( 'dm_user_settings' ) && $current_site->blog_id != $nxtdb->blogid && !dm_sunrise_warning( false ) ) {
		add_management_page(__( 'Domain Mapping', 'nxtclass-mu-domain-mapping'), __( 'Domain Mapping', 'nxtclass-mu-domain-mapping'), 'manage_options', 'domainmapping', 'dm_manage_page' );
	}

	if ( dm_site_admin() && version_compare( $nxt_version, '3.0.9', '<=' ) ) {
		if ( version_compare( $nxt_version, '3.0.1', '<=' ) ) {
			add_submenu_page('nxtmu-admin.php', __( 'Domain Mapping', 'nxtclass-mu-domain-mapping' ), __( 'Domain Mapping', 'nxtclass-mu-domain-mapping'), 'manage_options', 'dm_admin_page', 'dm_admin_page');
			add_submenu_page('nxtmu-admin.php', __( 'Domains', 'nxtclass-mu-domain-mapping' ), __( 'Domains', 'nxtclass-mu-domain-mapping'), 'manage_options', 'dm_domains_admin', 'dm_domains_admin');
		} else {
			add_submenu_page('ms-admin.php', __( 'Domain Mapping', 'nxtclass-mu-domain-mapping' ), 'Domain Mapping', 'manage_options', 'dm_admin_page', 'dm_admin_page');
			add_submenu_page('ms-admin.php', __( 'Domains', 'nxtclass-mu-domain-mapping' ), 'Domains', 'manage_options', 'dm_domains_admin', 'dm_domains_admin');
		}
	}
}
...
function dm_echo_default_updated_msg() {
	switch( $_GET[ 'updated' ] ) {
		case "add":
			$msg = __( 'New domain added.', 'nxtclass-mu-domain-mapping' );
			break;
		case "exists":
			$msg = __( 'New domain already exists.', 'nxtclass-mu-domain-mapping' );
			break;
		case "primary":
			$msg = __( 'New primary domain.', 'nxtclass-mu-domain-mapping' );
			break;
		case "del":
			$msg = __( 'Domain deleted.', 'nxtclass-mu-domain-mapping' );
			break;
	}
...
function maybe_create_db() {
	...
	if ( dm_site_admin() ) {
		$created = 0;
		if ( $nxtdb->get_var("SHOW TABLES LIKE '{$nxtdb->dmtable}'") != $nxtdb->dmtable ) {
			$nxtdb->query( "CREATE TABLE IF NOT EXISTS `{$nxtdb->dmtable}` (
		...
		if ( $nxtdb->get_var("SHOW TABLES LIKE '{$nxtdb->dmtablelogins}'") != $nxtdb->dmtablelogins ) {
			$nxtdb->query( "CREATE TABLE IF NOT EXISTS `{$nxtdb->dmtablelogins}` (
		...
		if ( $created ) {
			?> <div id="message" class="updated fade"><p><strong><?php _e( 'Domain mapping database table created.', 'nxtclass-mu-domain-mapping' ) ?></strong></p></div> <?php
		}
	}
...
function dm_domains_admin() {
	...
	echo '<h2>' . __( 'Domain Mapping: Domains', 'nxtclass-mu-domain-mapping' ) . '</h2>';
	if ( !empty( $_POST[ 'action' ] ) ) {
		check_admin_referer( 'domain_mapping' );
		$domain = strtolower( $_POST[ 'domain' ] );
		switch( $_POST[ 'action' ] ) {
			case "edit":
				$row = $nxtdb->get_row( $nxtdb->prepare( "SELECT * FROM {$nxtdb->dmtable} WHERE domain = %s", $domain ) );
				if ( $row ) {
					dm_edit_domain( $row );
				} else {
					echo "<h3>" . __( 'Domain not found', 'nxtclass-mu-domain-mapping' ) . "</h3>";
				}
			break;
			case "save":
				if ( $_POST[ 'blog_id' ] != 0 AND 
					$_POST[ 'blog_id' ] != 1 AND 
					null == $nxtdb->get_var( $nxtdb->prepare( "SELECT domain FROM {$nxtdb->dmtable} WHERE blog_id != %d AND domain = %s", $_POST[ 'blog_id' ], $domain ) ) 
				) {
					if ( $_POST[ 'orig_domain' ] == '' ) {
						$nxtdb->query( $nxtdb->prepare( "INSERT INTO {$nxtdb->dmtable} ( `blog_id`, `domain`, `active` ) VALUES ( %d, %s, %d )", $_POST[ 'blog_id' ], $domain, $_POST[ 'active' ] ) );
						echo "<p><strong>" . __( 'Domain Add', 'nxtclass-mu-domain-mapping' ) . "</strong></p>";
					} else {
						$nxtdb->query( $nxtdb->prepare( "UPDATE {$nxtdb->dmtable} SET blog_id = %d, domain = %s, active = %d WHERE domain = %s", $_POST[ 'blog_id' ], $domain, $_POST[ 'active' ], $_POST[ 'orig_domain' ] ) );
						echo "<p><strong>" . __( 'Domain Updated', 'nxtclass-mu-domain-mapping' ) . "</strong></p>";
					}
				}
			break;
			case "del":
				$nxtdb->query( $nxtdb->prepare( "DELETE FROM {$nxtdb->dmtable} WHERE domain = %s", $domain ) );
				echo "<p><strong>" . __( 'Domain Deleted', 'nxtclass-mu-domain-mapping' ) . "</strong></p>";
			break;
			case "search":
				$rows = $nxtdb->get_results( $nxtdb->prepare( "SELECT * FROM {$nxtdb->dmtable} WHERE domain LIKE %s", $domain ) );
				dm_domain_listing( $rows, sprintf( __( "Searching for %s", 'nxtclass-mu-domain-mapping' ), esc_html( $domain ) ) );
			break;
		}
	...
	echo "<h3>" . __( 'Search Domains', 'nxtclass-mu-domain-mapping' ) . "</h3>";
	echo '<form method="POST">';
	nxt_nonce_field( 'domain_mapping' );
	echo '<input type="hidden" name="action" value="search" />';
	echo '<p>';
	echo _e( "Domain:", 'nxtclass-mu-domain-mapping' );
	echo " <input type='text' name='domain' value='' /></p>";
	echo "<p><input type='submit' class='button-secondary' value='" . __( 'Search', 'nxtclass-mu-domain-mapping' ) . "' /></p>";
	echo "</form><br />";
	dm_edit_domain();
	$rows = $nxtdb->get_results( "SELECT * FROM {$nxtdb->dmtable} ORDER BY id DESC LIMIT 0,20" );
...
function dm_edit_domain( $row = false ) {
	if ( is_object( $row ) ) {
		echo "<h3>" . __( 'Edit Domain', 'nxtclass-mu-domain-mapping' ) . "</h3>";
	}  else {
		echo "<h3>" . __( 'New Domain', 'nxtclass-mu-domain-mapping' ) . "</h3>";
		$row->blog_id = '';
		$row->domain = '';
		$_POST[ 'domain' ] = '';
		$row->active = 1;
	...
	echo "<tr><th>" . __( 'Site ID', 'nxtclass-mu-domain-mapping' ) . "</th><td><input type='text' name='blog_id' value='{$row->blog_id}' /></td></tr>\n";
	echo "<tr><th>" . __( 'Domain', 'nxtclass-mu-domain-mapping' ) . "</th><td><input type='text' name='domain' value='{$row->domain}' /></td></tr>\n";
	echo "<tr><th>" . __( 'Primary', 'nxtclass-mu-domain-mapping' ) . "</th><td><input type='checkbox' name='active' value='1' ";
	echo $row->active == 1 ? 'checked=1 ' : ' ';
	echo "/></td></tr>\n";
	if ( get_site_option( 'dm_no_primary_domain' ) == 1 ) {
		echo "<tr><td colspan='2'>" . __( '<strong>Warning!</strong> Primary domains are currently disabled.', 'nxtclass-mu-domain-mapping' ) . "</td></tr>";
	}
	echo "</table>";
	echo "<p><input type='submit' class='button-primary' value='" .__( 'Save', 'nxtclass-mu-domain-mapping' ). "' /></p></form><br /><br />";
}

function dm_domain_listing( $rows, $heading = '' ) {
	if ( $rows ) {
		if ( file_exists( ABSPATH . 'nxt-admin/network/site-info.php' ) ) {
			$edit_url = network_admin_url( 'site-info.php' );
		} elseif ( file_exists( ABSPATH . 'nxt-admin/ms-sites.php' ) ) {
			$edit_url = admin_url( 'ms-sites.php' );
		} else {
			$edit_url = admin_url( 'nxtmu-blogs.php' );
		}
		if ( $heading != '' )
			echo "<h3>$heading</h3>";
		echo '<table class="widefat" cellspacing="0"><thead><tr><th>'.__( 'Site ID', 'nxtclass-mu-domain-mapping' ).'</th><th>'.__( 'Domain', 'nxtclass-mu-domain-mapping' ).'</th><th>'.__( 'Primary', 'nxtclass-mu-domain-mapping' ).'</th><th>'.__( 'Edit', 'nxtclass-mu-domain-mapping' ).'</th><th>'.__( 'Delete', 'nxtclass-mu-domain-mapping' ).'</th></tr></thead><tbody>';
		foreach( $rows as $row ) {
			...
			echo $row->active == 1 ? __( 'Yes',  'nxtclass-mu-domain-mapping' ) : __( 'No',  'nxtclass-mu-domain-mapping' );
			echo "</td><td><form method='POST'><input type='hidden' name='action' value='edit' /><input type='hidden' name='domain' value='{$row->domain}' />";
			nxt_nonce_field( 'domain_mapping' );
			echo "<input type='submit' class='button-secondary' value='" .__( 'Edit', 'nxtclass-mu-domain-mapping' ). "' /></form></td><td><form method='POST'><input type='hidden' name='action' value='del' /><input type='hidden' name='domain' value='{$row->domain}' />";
			nxt_nonce_field( 'domain_mapping' );
			echo "<input type='submit' class='button-secondary' value='" .__( 'Del', 'nxtclass-mu-domain-mapping' ). "' /></form>";
			echo "</td></tr>";
		}
		echo '</table>';
		if ( get_site_option( 'dm_no_primary_domain' ) == 1 ) {
			echo "<p>" . __( '<strong>Warning!</strong> Primary domains are currently disabled.', 'nxtclass-mu-domain-mapping' ) . "</p>";
		}
	}
}
...
function dm_handle_actions() {
	...
	if ( !empty( $_POST[ 'action' ] ) ) {
		$domain = $nxtdb->escape( $_POST[ 'domain' ] );
		if ( $domain == '' ) {
			nxt_die( "You must enter a domain" );
		}
		check_admin_referer( 'domain_mapping' );
		...
		switch( $_POST[ 'action' ] ) {
			case "add":
				...
				if( null == $nxtdb->get_row( "SELECT blog_id FROM {$nxtdb->blogs} WHERE domain = '$domain'" ) && null == $nxtdb->get_row( "SELECT blog_id FROM {$nxtdb->dmtable} WHERE domain = '$domain'" ) ) {
					if ( $_POST[ 'primary' ] ) {
						$nxtdb->query( $nxtdb->prepare( "UPDATE {$nxtdb->dmtable} SET active = 0 WHERE blog_id = %d", $nxtdb->blogid ) );
					}
					$nxtdb->query( $nxtdb->prepare( "INSERT INTO {$nxtdb->dmtable} ( `id` , `blog_id` , `domain` , `active` ) VALUES ( NULL, %d, %s, %d )", $nxtdb->blogid, $domain, $_POST[ 'primary' ] ) );
					nxt_redirect( add_query_arg( array( 'updated' => 'add' ), $url ) );
					exit;
				} else {
					nxt_redirect( add_query_arg( array( 'updated' => 'exists' ), $url ) );
					exit;
				}
			break;
			case "primary":
				...
				if( $domain != $orig_url[ 'host' ] ) {
					$nxtdb->query( $nxtdb->prepare( "UPDATE {$nxtdb->dmtable} SET active = 1 WHERE domain = %s", $domain ) );
				}
				nxt_redirect( add_query_arg( array( 'updated' => 'primary' ), $url ) );
				exit;
			break;
		}
	} elseif( $_GET[ 'action' ] == 'delete' ) {
		$domain = $nxtdb->escape( $_GET[ 'domain' ] );
		if ( $domain == '' ) {
			nxt_die( __( "You must enter a domain", 'nxtclass-mu-domain-mapping' ) );
		}
		check_admin_referer( "delete" . $_GET['domain'] );
		do_action('dm_handle_actions_del', $domain);
		$nxtdb->query( "DELETE FROM {$nxtdb->dmtable} WHERE domain = '$domain'" );
		nxt_redirect( add_query_arg( array( 'updated' => 'del' ), $url ) );
		exit;
	}
...
function dm_sunrise_warning( $die = true ) {
	if ( !file_exists( nxt_CONTENT_DIR . '/sunrise.php' ) ) {
		if ( !$die )
			return true;

		if ( dm_site_admin() ) {
			nxt_die( sprintf( __( "Please copy sunrise.php to %s/sunrise.php and ensure the SUNRISE definition is in %snxt-config.php", 'nxtclass-mu-domain-mapping' ), nxt_CONTENT_DIR, ABSPATH ) );
		} else {
			nxt_die( __( "This plugin has not been configured correctly yet.", 'nxtclass-mu-domain-mapping' ) );
		}
		...
		if ( dm_site_admin() ) {
			nxt_die( sprintf( __( "Please uncomment the line <em>define( 'SUNRISE', 'on' );</em> or add it to your %snxt-config.php", 'nxtclass-mu-domain-mapping' ), ABSPATH ) );
		} else {
			nxt_die( __( "This plugin has not been configured correctly yet.", 'nxtclass-mu-domain-mapping' ) );
		}
		...
		if ( dm_site_admin() ) {
			nxt_die( sprintf( __( "Please edit your %snxt-config.php and move the line <em>define( 'SUNRISE', 'on' );</em> above the last require_once() in that file or make sure you updated sunrise.php.", 'nxtclass-mu-domain-mapping' ), ABSPATH ) );
		} else {
			nxt_die( __( "This plugin has not been configured correctly yet.", 'nxtclass-mu-domain-mapping' ) );
		}
	}
...
function domain_mapping_siteurl( $setting ) {
	...
	if ( !isset( $return_url[ $nxtdb->blogid ] ) ) {
		$s = $nxtdb->suppress_errors();

		if ( get_site_option( 'dm_no_primary_domain' ) == 1 ) {
			$domain = $nxtdb->get_var( "SELECT domain FROM {$nxtdb->dmtable} WHERE blog_id = '{$nxtdb->blogid}' AND domain = '" . $nxtdb->escape( $_SERVER[ 'HTTP_HOST' ] ) . "' LIMIT 1" );
			if ( null == $domain ) {
				$return_url[ $nxtdb->blogid ] = untrailingslashit( get_original_url( "siteurl" ) );
				return $return_url[ $nxtdb->blogid ];
			}
		} else {
			// get primary domain, if we don't have one then return original url.
			$domain = $nxtdb->get_var( "SELECT domain FROM {$nxtdb->dmtable} WHERE blog_id = '{$nxtdb->blogid}' AND active = 1 LIMIT 1" );
			if ( null == $domain ) {
				$return_url[ $nxtdb->blogid ] = untrailingslashit( get_original_url( "siteurl" ) );
				return $return_url[ $nxtdb->blogid ];
			}
		...
		$nxtdb->suppress_errors( $s );
		if ( false == isset( $_SERVER[ 'HTTPS' ] ) )
			$_SERVER[ 'HTTPS' ] = 'Off';
		$protocol = ( 'on' == strtolower( $_SERVER[ 'HTTPS' ] ) ) ? 'https://' : 'http://';
		if ( $domain ) {
			$return_url[ $nxtdb->blogid ] = untrailingslashit( $protocol . $domain  );
			$setting = $return_url[ $nxtdb->blogid ];
		} else {
			$return_url[ $nxtdb->blogid ] = false;
		}
	} elseif ( $return_url[ $nxtdb->blogid ] !== FALSE) {
		$setting = $return_url[ $nxtdb->blogid ];
	}
...
function get_original_url( $url, $blog_id = 0 ) {
	...
	if ( ! isset( $orig_urls[ $id ] ) ) {
		...
		if ( $blog_id == 0 ) {
			$orig_urls[ $nxtdb->blogid ] = $orig_url;
		} else {
...
function dm_redirect_admin() {
	...
	if ( strpos( $_SERVER['REQUEST_URI'], 'nxt-admin/admin-ajax.php' ) !== false )
		return;
	...
	if ( get_site_option( 'dm_redirect_admin' ) ) {
		...
		if ( false === strpos( $url, $_SERVER[ 'HTTP_HOST' ] ) ) {
			nxt_redirect( untrailingslashit( $url ) . $_SERVER[ 'REQUEST_URI' ] );
			exit;
		}
	} else {
		global $current_blog;
		// redirect original url to primary domain nxt-admin/ - remote login is disabled!
		$url = domain_mapping_siteurl( false );
		$request_uri = str_replace( $current_blog->path, '/', $_SERVER[ 'REQUEST_URI' ] );
		if ( false === strpos( $url, $_SERVER[ 'HTTP_HOST' ] ) ) {
			nxt_redirect( str_replace( '//nxt-admin', '/nxt-admin', trailingslashit( $url ) . $request_uri ) );
			exit;
		}
	}
}
...
if ( defined( 'DOMAIN_MAPPING' ) ) {
	...
	add_action( 'nxt_head', 'remote_login_js_loader' );
	add_action( 'login_head', 'redirect_login_to_orig' );
	add_action( 'nxt_logout', 'remote_logout_loader', 9999 );
...
function remote_logout_loader() {
	global $current_site, $current_blog, $nxtdb;
	$nxtdb->dmtablelogins = $nxtdb->base_prefix . 'domain_mapping_logins';
	if ( false == isset( $_SERVER[ 'HTTPS' ] ) )
		$_SERVER[ 'HTTPS' ] = 'Off';
	$protocol = ( 'on' == strtolower( $_SERVER[ 'HTTPS' ] ) ) ? 'https://' : 'http://';
	$hash = get_dm_hash();
	$key = md5( time() );
	$nxtdb->query( $nxtdb->prepare( "INSERT INTO {$nxtdb->dmtablelogins} ( `id`, `user_id`, `blog_id`, `t` ) VALUES( %s, 0, %d, NOW() )", $key, $current_blog->blog_id ) );
	if ( get_site_option( 'dm_redirect_admin' ) ) {
		nxt_redirect( $protocol . $current_site->domain . $current_site->path . "?dm={$hash}&action=logout&blogid={$current_blog->blog_id}&k={$key}&t=" . mt_rand() );
		exit;
	} 
}
		...
		if ( $_GET[ 'action' ] == 'load' ) {
			...
			if ( $details = $nxtdb->get_row( $nxtdb->prepare( "SELECT * FROM {$nxtdb->dmtablelogins} WHERE id = %s AND blog_id = %d", $_GET[ 'k' ], $nxtdb->blogid ) ) ) {
				if ( $details->blog_id == $nxtdb->blogid ) {
					...
					$nxtdb->query( $nxtdb->prepare( "DELETE FROM {$nxtdb->dmtablelogins} WHERE t < %d", ( time() - 120 ) ) ); // remote logins survive for only 2 minutes if not used.
...
function delete_blog_domain_mapping( $blog_id, $drop ) {
	...
	if ( $blog_id && $drop ) {
		...
		$domains = $nxtdb->get_col( $nxtdb->prepare( "SELECT domain FROM {$nxtdb->dmtable} WHERE blog_id  = %d", $blog_id ) );
		do_action('dm_delete_blog_domain_mappings', $domains);
		
		$nxtdb->query( $nxtdb->prepare( "DELETE FROM {$nxtdb->dmtable} WHERE blog_id  = %d", $blog_id ) );
	}
}
...
add_filter( 'nxtmu_blogs_columns', 'ra_domain_mapping_columns' );
...
function ra_domain_mapping_field( $column, $blog_id ) {
	...
	if ( $column == 'map' ) {
		if ( $maps === false ) {
			$nxtdb->dmtable = $nxtdb->base_prefix . 'domain_mapping';
			$work = $nxtdb->get_results( "SELECT blog_id, domain FROM {$nxtdb->dmtable} ORDER BY blog_id" );
			$maps = array();
			if($work) {
...
function dm_idn_warning() {
	return sprintf( __( 'International Domain Names should be in <a href="%s">punycode</a> format.', 'nxtclass-mu-domain-mapping' ), "http://api.webnic.cc/idnconversion.html" );
}
