...
class BPSP_Roles {    
    ...
    function profile_screen_new_request( $field_id, $field_value ) {
        ...
        if( $field_value == __( 'Teacher', 'bpsp' ) && !is_super_admin() )
            nxt_die( __( 'BuddyPress Courseware error, you are not allowed to assign Teachers.', 'bpsp' ) );
    ...
    function register_profile_fields() {
        ...
        if( !$bpsp_group_id )
            nxt_die( __( 'BuddyPress Courseware error when saving xProfile group.', 'bpsp' ) );
        ...
        if( !$bpsp_field_id )
            nxt_die( __( 'BuddyPress Courseware error when saving xProfile field.', 'bpsp' ) );
        ...
        if( !xprofile_get_field_id_from_name( __( 'Teacher', 'bpsp' ) ) ||
            ...
            nxt_die( __( 'BuddyPress Courseware error when saving xProfile field options.', 'bpsp' ) );
     ...
     * get_admins()
     *
     * A wrapper for NXTClass 3.1 `get_users()` with backcompat
    ...
    function get_admins() {
        ...
        if( function_exists( 'get_users' ) ) {
            ...
            $users_search = new nxt_User_Search( null, null, 'administrator' );
