...
class BPSP_Groups {
    ...
    function BPSP_Groups() {
        ...
        add_action( 'nxt', array( &$this, 'group_admin_screen' ), 4 );
    ...
    function extend_search( $groups, $params ) {
        ...
        // A hack to make NXTClass believe the taxonomy is registered
        if( !taxonomy_exists( 'group_id' ) ) {
            global $nxt_taxonomies;
            $nxt_taxonomies['group_id'] = '';
        }
     ...
     * group_admin_screen()
     *
     * Hooks into nxt, adds a new screen to group Admin screens
     */
     ...
     */
    function bpsp_restrict_uploads( $nxt_the_query ) {
        // Check if current user is admin or sort of
        if ( !current_user_can( 'manage_options' ) )
            // If not, he will only see his own attachments
            $nxt_the_query->query_vars['author'] = get_current_user_id();
    }
