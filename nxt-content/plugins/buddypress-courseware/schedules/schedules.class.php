...
class BPSP_Schedules {
    ...
    function register_post_types() {
        ...
        if( !register_post_type( 'schedule', $schedule_post_def ) )
            nxt_die( __( 'BuddyPress Courseware error while registering schedule post type.', 'bpsp' ) );
    }
    ...
    function is_schedule( $schedule_identifier = null ) {
        ...
        $course_id = nxt_get_object_terms( $schedule->ID, 'course_id' );
        if( !empty( $course_id ) )
            ...
            if( true != $is_nonce ) {
                ...
                if( !empty( $new_schedule['desc'] ) ||
                    ...
                    foreach( $new_schedules as $newschedule ) {
                        $newschedule_id = nxt_insert_post( $newschedule );
                        if( $newschedule_id ) {
                            nxt_set_post_terms( $newschedule_id, $newschedule['cw_group_id'], 'group_id' );
                            if( $newschedule['cw_course_id'] )
                                nxt_set_post_terms( $newschedule_id, $newschedule['cw_course_id'], 'course_id' );
        ...
        if( isset( $_POST['schedule'] ) && $_POST['schedule']['object'] == 'group' && isset( $_POST['_nxtnonce'] ) ) {
            ...
            if( true != $is_nonce )
                ...
                if( !empty( $updated_schedule['group_id'] ) && $valid_dates ) {
                    $updated_schedule_id =  nxt_update_post( array(
                        'ID'            => $old_schedule->ID,
