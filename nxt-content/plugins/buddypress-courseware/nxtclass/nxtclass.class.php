<?php
/**
 * Class to handle all NXTClass admin backend stuff
 */
class BPSP_NXTClass {
    /**
     * BPSP_NXTClass()
    ...
    function load_template( $vars ) {
        ob_start();
        extract( $vars );
        if( file_exists( BPSP_PLUGIN_DIR . '/nxtclass/templates/' . $name . '.php' ) )
            include( BPSP_PLUGIN_DIR . '/nxtclass/templates/' . $name . '.php' );
