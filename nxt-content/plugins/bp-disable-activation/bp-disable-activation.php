...
/*The Functions to automatically activate for Single nxt Installs*/
if ( !bp_core_is_multisite() ) {
	...
	function disable_validation( $user_id ) {
		...
		$nxtdb->query( $nxtdb->prepare( "UPDATE $nxtdb->users SET user_status = 0 WHERE ID = %d", $user_id ) );
		...
		//Thanks to Justin Klein's  nxt-fb-autoconnect plugin for the basic code to login automatically
		$user_info = get_userdata($user_id);
		nxt_set_auth_cookie($user_id);

		do_action('nxt_signon', $user_info->user_login);
...
/*END Functions to automatically activate for Single nxt Installs*/

} else {

/*START Functions to automatically activate for nxtMU (multi-site)  Installs (Activates User and Blogs)*/

/*
 Credit for most of the nxtMU code goes to Brajesh Singh and his plugin "BP Auto activate User and Blog at Signup"
*/

//Remove filters which notifies users
remove_filter( 'nxtmu_signup_user_notification', 'bp_core_activation_signup_user_notification', 1, 4 );

add_filter( 'nxtmu_signup_user_notification', 'cc_auto_activate_on_user_signup', 1, 4 );
...
global $bp, $nxtdb;
	...
	// just do what nxt-activate.php does
    ob_start();
		...
		/* Activate the signup */
		$signup = apply_filters( 'bp_core_activate_account', nxtmu_activate_signup( $key ) );
		...
		if ( !empty( $signup['meta']['password'] ) )
			$nxtdb->update( $nxtdb->users, array( 'user_pass' => $signup['meta']['password'] ), array( 'ID' => $signup['user_id'] ), array( '%s' ), array( '%d' ) );
		...
		nxt_set_auth_cookie($signup['user_id']);
		do_action('nxt_login', $user_info->user_login);
		
		do_action('bp_auto_activate_after_login');
	
	return false; //stop  nxtmu_signup_blog_notification () from sending mail to user
}
...
/*END Functions to automatically activate for nxtMU (multi-site)  Installs*/
}
