...
if ( ! function_exists( 'optionsframework_mlu_css' ) ) {
	...
	function optionsframework_mlu_css () {
		...
		$_html = '';
		$_html .= '<link rel="stylesheet" href="' . site_url() . '/' . nxtINC . '/js/thickbox/thickbox.css" type="text/css" media="screen" />' . "\n";
		$_html .= '<script type="text/javascript">
		var tb_pathToImage = "' . site_url() . '/' . nxtINC . '/js/thickbox/loadingAnimation.gif";
	    var tb_closeImage = "' . site_url() . '/' . nxtINC . '/js/thickbox/tb-close.png";
	    </script>' . "\n";
...
/**
 * Media Uploader Using the NXTClass Media Library.
 *
 * Parameters:
...
if ( ! function_exists( 'optionsframework_mlu_get_silentpost' ) ) {
	...
	function optionsframework_mlu_get_silentpost ( $_token ) {
		...
		if ( $_token ) {
			...
			$query = 'SELECT ID FROM ' . $nxtdb->posts . ' WHERE post_parent = 0';
			foreach ( $_args as $k => $v ) {
				$query .= ' AND ' . $k . ' = "' . $v . '"';
			} // End FOREACH Loop
			
			$query .= ' LIMIT 1';
			$_posts = $nxtdb->get_row( $query );
