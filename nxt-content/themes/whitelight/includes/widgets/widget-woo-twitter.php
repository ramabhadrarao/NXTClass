...
class lok_Twitter extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ( $title ) {
			...
			?><h3 class="tlogo"><img src="<?php bloginfo('template_directory'); ?>/images/twitter.png" alt="Twitter" /><?php _e('Twitter','lokthemes'); ?></h3><?php
		}
		?><div class="back"><ul id="twitter_update_list_<?php echo $unique_id; ?>"><li></li></ul>
			<p><?php _e('Follow','lokthemes'); ?> <a href="http<?php ( is_ssl() ? 's' : '' ) ?>://twitter.com/<?php echo $username; ?>"><strong>@<?php echo $username; ?></strong></a> <?php _e('on Twitter','lokthemes'); ?></p></div><div class="clear"></div><?php

		echo lok_twitter_script($unique_id,$username,$limit); //Javascript output function
		echo $after_widget;
	}
	...
	function form( $instance ) {
		...
		?>
			...
			<p>
				<label for="<?php echo $this->get_field_id('username'); ?>"><?php _e('Username:','lokthemes'); ?></label>
			...
			<p>
				<label for="<?php echo $this->get_field_id('limit'); ?>"><?php _e('Limit:','lokthemes'); ?></label>
