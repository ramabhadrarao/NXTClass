...
- express_uploadFile()
- express_lok_taxonomy()
- express_lok_post_format()
- express_newPost()
- express_editPost()
...
function express_getPostsWithOffset($args){
	...
	foreach ($posts_list as $entry) {
		...
		$struct[] = array(
			...
			'nxt_author_id' => $author->ID,
			'nxt_author_display_name' => $author->display_name,
			'date_created_gmt' => new IXR_Date($post_date_gmt),
			'post_status' => $entry['post_status'],
			'custom_fields' => $nxt_xmlrpc_server->get_custom_fields($entry['ID'])
		);
...
function express_lok_post_format($args) {
	$content_struct = $args[3];
	...
	if (is_array($taxonomies)) {
		...
		$content_struct['nxt_post_format'] = $post_format;
		$args[3] = $content_struct;
	}
...
function express_newPost($args) {
	...
	if (get_option('lok_tumblog_content_method') == 'post_format') {	
		$args = express_lok_post_format($args);
	}
	else {
