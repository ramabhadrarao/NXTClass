...
class WooThemes_Backup {
	 ...
	 *
	 * Register the admin screen within NXTClass.
	 *
	 * @since 1.0.0
	 */
...
<?php
	...
	function import() {
		...
		foreach ( (array)$options as $key => $settings ) {
			...
			// We can run checks using get_option(), as the options are all cached. See nxt-includes/functions.php for more information.
		...
		nxt_redirect( admin_url( 'admin.php?page=' . $this->token . '&imported=true' ) );
		exit;
