...
function woo_custom_navigation_setup() {
	...
	if ($page_var == 'custom_navigation')
	{
		...
		if ( ! empty($nxtdb->collate) ) {
			$charset_collate .= " COLLATE $nxtdb->collate";
		}

		if(($nxtdb->get_var( "show tables like '$table_name'") != $table_name) || ($nav_version_in_db <> $nav_version))
		{
		...
		if(($nxtdb->get_var( "show tables like '$table_name_menus'") != $table_name_menus) || ($nav_version_in_db <> $nav_version))
		{
			...
			if ($nxtdb->get_var( "show tables like '$table_name_menus'") != $table_name_menus) {
				$data_insert = true;
			}
				...
				foreach ($pages_array as $post)
				{
					...
					if ($post->post_parent == 0)
					{
						...
						//$insert = "INSERT INTO ".$table_name." (position,post_id,parent_id,custom_title,custom_link,custom_description,menu_icon,link_type,menu_id,custom_anchor_title) "."VALUES ( '".$counter."','".$post->ID."','".$parent_id."','".$insert_title."','".get_permalink($post->ID)."','','','page','1','".$insert_title."')";
	  					//$results = $nxtdb->query( $insert );
	  					$results = $nxtdb->insert( $table_name, array( 'position' => $counter, 'post_id' => $post->ID, 'parent_id' => $parent_id, 'custom_title' => $str_converted, 'custom_link' => get_permalink($post->ID), 'custom_description' => '', 'menu_icon' => '', 'link_type' => 'page', 'menu_id' => '1', 'custom_anchor_title' => $str_converted ));
				...
				foreach ($categories_array as $cat_item) {
					...
					if ($cat_item->parent == 0)
					{
						...
						//$insert = "INSERT INTO ".$table_name." (position,post_id,parent_id,custom_title,custom_link,custom_description,menu_icon,link_type,menu_id) "."VALUES ( '".$counter."','".$cat_item->cat_ID."','".$parent_id."','".$insert_title."','".get_category_link($cat_item->cat_ID)."','','','category','2')";
	  					//$results = $nxtdb->query( $insert );
	  					$results = $nxtdb->insert( $table_name, array( 'position' => $counter, 'post_id' => $cat_item->cat_ID, 'parent_id' => $parent_id, 'custom_title' => $str_converted, 'custom_link' => get_category_link($cat_item->cat_ID), 'custom_description' => '', 'menu_icon' => '', 'link_type' => 'category', 'menu_id' => '2', 'custom_anchor_title' => $str_converted ));
	...
	<div id="no-js"><h3>You do not have JavaScript enabled in your browser. Please enabled it to access the Custom Menu functionality.</h3></div>
		...
		if (isset($_POST['add_menu'])) {
			...
			$table_name_custom_menu = $nxtdb->prefix . "woo_custom_nav_menus";
	 		...
	 		if ($insert_menu_name <> '') {
	 			...
	 			{
	 				$nxtdb->insert( $table_name_custom_menu, array( 'menu_name' => $insert_menu_name ));
	 				$menu_selected_id = $nxtdb->insert_id;
	 				$menu_id_in_edit = $menu_selected_id;
		...
		if ($postCounter > 0)
		{
			...
			elseif (isset($_POST['reset_woo_menu'])) {
				...
				$nxtdb->query( "DELETE FROM ".$table_name." WHERE menu_id='".$menu_id_in_edit."'" );
 				...
 				for ($k = 1;$k<= $postCounter; $k++) {
 					...
 					else
 					{
	 					...
	 					if ($parent_id == 0)
	 					{
	 						...
	 						$nxtdb->insert( $table_name, array( 'position' => $position, 'post_id' => $post_id, 'parent_id' => $parent_id, 'custom_title' => $custom_title, 'custom_link' => $custom_linkurl, 'custom_description' => $custom_description, 'menu_icon' => $icon, 'link_type' => $linktype, 'menu_id' => $menu_id_in_edit, 'custom_anchor_title' => $custom_anchor_title, 'new_window' => $new_window ));
	 					}
	 					//If not top level menu item
	 					else
	 					{
	 						//INSERT menu item record
	 						$nxtdb->insert( $table_name, array( 'position' => $position, 'post_id' => $post_id, 'parent_id' => '8000', 'custom_title' => $custom_title, 'custom_link' => $custom_linkurl, 'custom_description' => $custom_description, 'menu_icon' => $icon, 'link_type' => $linktype, 'menu_id' => $menu_id_in_edit, 'custom_anchor_title' => $custom_anchor_title, 'new_window' => $new_window  ));
	 						$lastid = $nxtdb->insert_id;

	 						//GET the correct parent record
	 						$parentrecords = $nxtdb->get_results( "SELECT id FROM ".$table_name." WHERE position='".($parent_id)."' AND menu_id='".$menu_id_in_edit."'" );
		 					...
		 					$nxtdb->update( $table_name, array( 'parent_id' => $parent_id_update ), array( 'id' => $lastid, 'menu_id' => $menu_id_in_edit ));
	 					}
 					}
 				}
		...
		<div id="pages-left">
			...
			<?php
				...
				if ( function_exists( 'nxt_nav_menu') ) {
					echo '<div id="message-enabled" class="updated fade below-h2"><p><strong>You have NXTClass 3.0.x installed! </strong></p><p>We suggest that you use the <a href="'.admin_url().'nav-menus.php">NXTClass Menu Management system</a> instead of the Custom Navigation.</p></div>';
				}
			...
			<div class="sidebar-name">
				...
				//CHECK for existing woo custom menu
	 			...
	 			if ($menu_id_in_edit > 0)
	 			{
	 				...
	 				$custom_menu_name = $nxtdb->get_results( "SELECT menu_name FROM ".$table_name_menus." WHERE id='".$menu_selected_id."'" );
	 			}
		...
		<div id="menu-right">
			...
			<div class="widgets-holder-wrap">
				...
				<div class="widget-holder">
					...
					$categories = $nxtdb->get_results( "SELECT term_id FROM $nxtdb->term_taxonomy WHERE taxonomy = 'category' ORDER BY term_id ASC" );
					$cat_name = '';
					//CHECK for results
					if ($categories)
					{
...
function woo_custom_navigation_output($args = array()) {
		...
		global $nxt_query;
	    if (is_page()) {
	    	$queried_id = $nxt_query->post->ID;
	    	$type_settings = 'page';
	    }
	    elseif (is_category()) {
	    	$queried_id = $nxt_query->query_vars['cat'];
			...
			?><?php
						...
						elseif ($output_type == "backend")
						{
							...
							<dt>
								...
								<a id="view<?php echo esc_attr( $counter ); ?>" target="_blank" href="<?php echo $link; ?>"><img alt="View Page" title="View Page" src="<?php echo get_template_directory_uri(); ?>/functions/images/ico-vienxtage.png" /></a>
								</span>
							</dt>
							</dl>
					...
					<li id="menu-<?php echo esc_attr( $intCounter ); ?>" value="<?php echo esc_attr( $intCounter ); ?>">
						...
						<span class="controls">
							...
							<a target="_blank" href="<?php echo get_permalink($post->ID); ?>">
								<img alt="View Page" title="View Page" src="<?php echo get_template_directory_uri(); ?>/functions/images/ico-vienxtage.png" />
							</a>
						</span>
		...
		foreach ($sub_array as $sub_item)
		{
			...
			if ($sub_item_parent == $childof)
			{
				...
				//$insert = "INSERT INTO ".$table_name." (position,post_id,parent_id,custom_title,custom_link,custom_description,menu_icon,link_type,menu_id,custom_anchor_title) "."VALUES ( '".$counter."','".$itemid."','".$parent_id."','".$title."','".$link."','','','".$linktype."','".$menu_id."','".$title."')";
	  			//$results = $nxtdb->query( $insert );
	  			$results = $nxtdb->insert( $table_name, array( 'position' => $counter, 'post_id' => $itemid, 'parent_id' => $parent_id, 'custom_title' => $str_converted, 'custom_link' => $link, 'custom_description' => '', 'menu_icon' => '', 'link_type' => $linktype, 'menu_id' => $menu_id, 'custom_anchor_title' => $str_converted ));
