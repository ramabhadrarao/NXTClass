...
-----------------------------------------------------------------------------------*/
	...
	function WooThemes_Shortcode_Generator () {
		...
		add_action( 'nxt_ajax_woo_shortcodes_nonce', array( &$this, 'ajax_action_generate_nonce' ) );
...
-----------------------------------------------------------------------------------*/
	...
	function init() {
		...
		if ( ( current_user_can( 'edit_posts' ) || current_user_can( 'edit_pages' ) ) && get_user_option( 'rich_editing' ) == 'true' && ( in_array( $pagenow, array( 'post.php', 'post-new.php', 'page-new.php', 'page.php' ) ) ) )  {
			...
			nxt_register_script( 'woo-colourpicker', esc_url( $this->framework_url() . 'js/colorpicker.js' ), array( 'jquery' ), '3.6', true ); // Loaded into the footer.
			nxt_enqueue_script( 'woo-colourpicker' );
			
			// Register the colourpicker CSS.
			nxt_register_style( 'woo-colourpicker', esc_url( $this->framework_url() . 'css/colorpicker.css' ) );
			nxt_enqueue_style( 'woo-colourpicker' );
			
			// Register the custom CSS styles.
			nxt_register_style( 'woo-shortcode-generator', esc_url( $this->framework_url() . 'css/shortcode-generator.css' ) );
...
-----------------------------------------------------------------------------------*/
	...
	function filter_mce_external_plugins( $plugins ) {
        ...
        $plugins['WooThemesShortcodes'] = nxt_nonce_url( esc_url( $this->framework_url() . 'js/shortcode-generator/editor_plugin.js' ), 'wooframework-shortcode-generator' );
