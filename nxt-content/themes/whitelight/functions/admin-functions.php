...
function woo_title(){
	...
	// Check if 3rd party plugin is in use, and output default nxt_title() if it is
	if(
		(
...
if ( ! function_exists( 'woo_pagination' ) ) {
	...
	function woo_pagination( $args = array(), $query = '' ) {
		...
		if( $nxt_rewrite->using_permalinks() && ! is_search() )
...
add_action( 'nxt_ajax_wooframework_banner_close', 'wooframework_ajax_banner_close' );
...
function wooframework_add_woodojo_banner () {
	...
	$close_url = nxt_nonce_url( admin_url( 'admin-ajax.php?action=wooframework_banner_close&banner=woodojo' ), 'wooframework_banner_close' );
	$html = '';
...
function wooframework_add_presstrends_banner () {
	...
	$close_url = nxt_nonce_url( admin_url( 'admin-ajax.php?action=wooframework_banner_close&banner=presstrends' ), 'wooframework_banner_close' );
	$html = '';
...
function wooframework_ajax_banner_close () {
	if( ! current_user_can( 'install_plugins' ) ) nxt_die( __( 'You do not have sufficient permissions to access this page.', 'woothemes' ) );
	
	if( ! check_admin_referer( 'wooframework_banner_close' ) ) nxt_die( __( 'You have taken too long. Please go back and retry.', 'woothemes' ) );
